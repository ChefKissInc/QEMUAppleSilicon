...
static void usage(FILE* out)
{
    fprintf(out,
            ...
            "\n",
            names.model, names.layout, names.variant ?: "-", names.options ?: "-");
}
...
static xkb_mod_mask_t get_mod(struct xkb_keymap* map, const char* name)
{
    ...
    if (mod != XKB_MOD_INVALID) { mask = (1 << mod); }
    return mask;
}
...
int main(int argc, char* argv[])
{
    ...
    for (;;) {
        ...
        if (rc == -1) { break; }
        switch (rc) {
            case 'm': names.model = optarg; break;
            case 'l': names.layout = optarg; break;
            case 'v': names.variant = optarg; break;
            case 'o': names.options = optarg; break;
            case 'f':
    ...
    for (mod = 0; mod < mods; mod++) { fprintf(outfile, "#    %2d: %s\n", mod, xkb_keymap_mod_get_name(map, mod)); }
