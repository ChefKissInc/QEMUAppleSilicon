...
static bool qauthz_pam_is_allowed(QAuthZ* authz, const char* identity, Error** errp)
{
    ...
    ret = pam_start(pauthz->service, identity, &pam_conversation, &pamh);
    if (ret != PAM_SUCCESS) {
        error_setg(errp, "Unable to start PAM transaction: %s", pam_strerror(NULL, ret));
        return false;
    }
    ...
    pam_end(pamh, ret);
    if (ret != PAM_SUCCESS) {
        error_setg(errp, "Unable to authorize user '%s': %s", identity, pam_strerror(pamh, ret));
        return false;
    }
...
static void qauthz_pam_prop_set_service(Object* obj, const char* service, Error** errp G_GNUC_UNUSED)
{
...
static char* qauthz_pam_prop_get_service(Object* obj, Error** errp G_GNUC_UNUSED)
{
    QAuthZPAM* pauthz = QAUTHZ_PAM(obj);
...
static void qauthz_pam_complete(UserCreatable* uc, Error** errp)
{
...
static void qauthz_pam_finalize(Object* obj)
{
    QAuthZPAM* pauthz = QAUTHZ_PAM(obj);
...
static void qauthz_pam_class_init(ObjectClass* oc, const void* data)
{
    ...
    object_class_property_add_str(oc, "service", qauthz_pam_prop_get_service, qauthz_pam_prop_set_service);
}

QAuthZPAM* qauthz_pam_new(const char* id, const char* service, Error** errp)
{
    return QAUTHZ_PAM(
