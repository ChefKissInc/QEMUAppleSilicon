...
{
    switch (vece) {
        case MO_8 : return 0x0101010101010101ull * (uint8_t)c;
        case MO_16: return 0x0001000100010001ull * (uint16_t)c;
        case MO_32: return 0x0000000100000001ull * (uint32_t)c;
        case MO_64: return c;
...
void tcg_gen_dup_i32(unsigned vece, TCGv_i32 out, TCGv_i32 in)
{
    switch (vece) {
        case MO_8:
            ...
            break;
        case MO_16: tcg_gen_deposit_i32(out, in, in, 16, 16); break;
        case MO_32: tcg_gen_mov_i32(out, in); break;
        default   : assert_not_reached();
    }
}
...
void tcg_gen_dup_i64(unsigned vece, TCGv_i64 out, TCGv_i64 in)
{
    switch (vece) {
        ...
        case MO_16:
            ...
            break;
        case MO_32: tcg_gen_deposit_i64(out, in, in, 32, 32); break;
        case MO_64: tcg_gen_mov_i64(out, in); break;
        default   : assert_not_reached();
    }
}
...
static void do_dup_store(TCGType type, TCGv_ptr dbase, uint32_t dofs, uint32_t oprsz, uint32_t maxsz, TCGv_vec t_vec)
{
    ...
    switch (type) {
        case TCG_TYPE_V256:
             ...
             * that e.g. size == 80 would be expanded with 2x32 + 1x16.
             */
            for (; i + 32 <= oprsz; i += 32) { tcg_gen_stl_vec(t_vec, dbase, dofs + i, TCG_TYPE_V256); }
            /* fallthru */
        case TCG_TYPE_V128:
            for (; i + 16 <= oprsz; i += 16) { tcg_gen_stl_vec(t_vec, dbase, dofs + i, TCG_TYPE_V128); }
            break;
        case TCG_TYPE_V64:
            for (; i < oprsz; i += 8) { tcg_gen_stl_vec(t_vec, dbase, dofs + i, TCG_TYPE_V64); }
            break;
        default: assert_not_reached();
    }

    if (oprsz < maxsz) { expand_clr(dbase, dofs + oprsz, maxsz - oprsz); }
}
                   ...
                   TCGv_i64 in_64, uint64_t in_c)
{
    ...
    if (type != 0) {
        ...
        if (in_32) { tcg_gen_dup_i32_vec(vece, t_vec, in_32); }
        else if (in_64) {
    ...
    if (check_size_impl(oprsz, TCG_TARGET_REG_BITS / 8)) {
        ...
        if (in_32) {
            ...
            if (vece == MO_64 || (TCG_TARGET_REG_BITS == 64 && (in_c == 0 || in_c == -1 || !check_size_impl(oprsz, 4))))
            {
        ...
        if (t_32) {
            for (i = 0; i < oprsz; i += 4) { tcg_gen_st_i32(t_32, dbase, dofs + i); }
            tcg_temp_free_i32(t_32);
            goto done;
        }
        if (t_64) {
            for (i = 0; i < oprsz; i += 8) { tcg_gen_st_i64(t_64, dbase, dofs + i); }
    ...
    if (oprsz == maxsz && vece == MO_8) {
        ...
        if (in_64) { tcg_temp_free_i32(t_val); }
    ...
    if (vece == MO_64) {
        if (in_64) { gen_helper_gvec_dup64(t_ptr, t_desc, in_64); }
        else {
            t_64 = tcg_constant_i64(in_c);
        ...
        if (in_32) { fns[vece](t_ptr, t_desc, in_32); }
        else if (in_64) {
            t_32 = tcg_temp_ebb_new_i32();
                         ...
                         void (*fni)(TCGv_i32, TCGv_i32))
{
    ...
    for (i = 0; i < oprsz; i += 4) {
        ...
        if (load_dest) { tcg_gen_ld_i32(t1, dbase, dofs + i); }
        fni(t1, t0);
                          ...
                          void (*fni)(TCGv_i32, TCGv_i32, int32_t))
{
    ...
    for (i = 0; i < oprsz; i += 4) {
        ...
        if (load_dest) { tcg_gen_ld_i32(t1, tcg_env, dofs + i); }
        fni(t1, t0, c);
                         ...
                         uint32_t oprsz, bool load_dest, void (*fni)(TCGv_i32, TCGv_i32, TCGv_i32))
{
    ...
    for (i = 0; i < oprsz; i += 4) {
        ...
        if (load_dest) { tcg_gen_ld_i32(t2, dbase, dofs + i); }
        fni(t2, t0, t1);
                          ...
                          bool write_aofs, void (*fni)(TCGv_i32, TCGv_i32, TCGv_i32, int32_t))
{
    ...
    for (i = 0; i < oprsz; i += 4) {
        ...
        if (load_dest) { tcg_gen_ld_i32(t2, tcg_env, dofs + i); }
        fni(t2, t0, t1, c);
        tcg_gen_st_i32(t2, tcg_env, dofs + i);
        if (write_aofs) { tcg_gen_st_i32(t0, tcg_env, aofs + i); }
    }
    tcg_temp_free_i32(t0);
    tcg_temp_free_i32(t1);
    tcg_temp_free_i32(t2);
                         ...
                         void (*fni)(TCGv_i32, TCGv_i32, TCGv_i32, TCGv_i32))
{
    ...
    for (i = 0; i < oprsz; i += 4) {
        ...
        if (write_aofs) { tcg_gen_st_i32(t1, tcg_env, aofs + i); }
    }
                         ...
                         void (*fni)(TCGv_i64, TCGv_i64))
{
    ...
    for (i = 0; i < oprsz; i += 8) {
        ...
        if (load_dest) { tcg_gen_ld_i64(t1, dbase, dofs + i); }
        fni(t1, t0);
                          ...
                          void (*fni)(TCGv_i64, TCGv_i64, int64_t))
{
    ...
    for (i = 0; i < oprsz; i += 8) {
        ...
        if (load_dest) { tcg_gen_ld_i64(t1, tcg_env, dofs + i); }
        fni(t1, t0, c);
                         ...
                         uint32_t oprsz, bool load_dest, void (*fni)(TCGv_i64, TCGv_i64, TCGv_i64))
{
    ...
    for (i = 0; i < oprsz; i += 8) {
        ...
        if (load_dest) { tcg_gen_ld_i64(t2, dbase, dofs + i); }
        fni(t2, t0, t1);
                          ...
                          bool write_aofs, void (*fni)(TCGv_i64, TCGv_i64, TCGv_i64, int64_t))
{
    ...
    for (i = 0; i < oprsz; i += 8) {
        ...
        if (load_dest) { tcg_gen_ld_i64(t2, tcg_env, dofs + i); }
        fni(t2, t0, t1, c);
        tcg_gen_st_i64(t2, tcg_env, dofs + i);
        if (write_aofs) { tcg_gen_st_i64(t0, tcg_env, aofs + i); }
    }
    tcg_temp_free_i64(t0);
    tcg_temp_free_i64(t1);
    tcg_temp_free_i64(t2);
                         ...
                         void (*fni)(TCGv_i64, TCGv_i64, TCGv_i64, TCGv_i64))
{
    ...
    for (i = 0; i < oprsz; i += 8) {
        ...
        if (write_aofs) { tcg_gen_st_i64(t1, tcg_env, aofs + i); }
    }
                         ...
                         uint32_t tysz, TCGType type, bool load_dest, void (*fni)(unsigned, TCGv_vec, TCGv_vec))
{
    for (uint32_t i = 0; i < oprsz; i += tysz) {
        ...
        if (load_dest) { tcg_gen_ld_vec(t1, dbase, dofs + i); }
        fni(vece, t1, t0);
                          ...
                          int64_t c, bool load_dest, void (*fni)(unsigned, TCGv_vec, TCGv_vec, int64_t))
{
    for (uint32_t i = 0; i < oprsz; i += tysz) {
        ...
        if (load_dest) { tcg_gen_ld_vec(t1, tcg_env, dofs + i); }
        fni(vece, t1, t0, c);
                          ...
                          TCGv_vec c, bool scalar_first, void (*fni)(unsigned, TCGv_vec, TCGv_vec, TCGv_vec))
{
    for (uint32_t i = 0; i < oprsz; i += tysz) {
        ...
        tcg_gen_ld_vec(t0, tcg_env, aofs + i);
        if (scalar_first) { fni(vece, t1, c, t0); }
        else {
            fni(vece, t1, t0, c);
        }
        tcg_gen_st_vec(t1, tcg_env, dofs + i);
                         ...
                         void (*fni)(unsigned, TCGv_vec, TCGv_vec, TCGv_vec))
{
    for (uint32_t i = 0; i < oprsz; i += tysz) {
        ...
        if (load_dest) { tcg_gen_ld_vec(t2, dbase, dofs + i); }
        fni(vece, t2, t0, t1);
                          ...
                          void (*fni)(unsigned, TCGv_vec, TCGv_vec, TCGv_vec, int64_t))
{
    for (uint32_t i = 0; i < oprsz; i += tysz) {
        ...
        if (load_dest) { tcg_gen_ld_vec(t2, tcg_env, dofs + i); }
        fni(vece, t2, t0, t1, c);
        tcg_gen_st_vec(t2, tcg_env, dofs + i);
        if (write_aofs) { tcg_gen_st_vec(t0, tcg_env, aofs + i); }
    }
}
                         ...
                         void (*fni)(unsigned, TCGv_vec, TCGv_vec, TCGv_vec, TCGv_vec))
{
    for (uint32_t i = 0; i < oprsz; i += tysz) {
        ...
        if (write_aofs) { tcg_gen_st_vec(t1, tcg_env, aofs + i); }
    }
}
                        ...
                        const GVecGen2* g)
{
    ...
    type = 0;
    if (g->fniv) { type = choose_vector_type(g->opt_opc, g->vece, oprsz, g->prefer_i64); }
    switch (type) {
        case TCG_TYPE_V256:
            /* Recall that ARM SVE allows vector sizes that are not a
             * power of 2, but always a multiple of 16.  The intent is
...
void tcg_gen_gvec_2i(uint32_t dofs, uint32_t aofs, uint32_t oprsz, uint32_t maxsz, int64_t c, const GVecGen2i* g)
{
    ...
    switch (type) {
        ...
        case TCG_TYPE_V128:
            ...
            break;
        case TCG_TYPE_V64: expand_2i_vec(g->vece, dofs, aofs, oprsz, 8, TCG_TYPE_V64, c, g->load_dest, g->fniv); break;

        case 0:
            if (g->fni8 && check_size_impl(oprsz, 8)) { expand_2i_i64(dofs, aofs, oprsz, c, g->load_dest, g->fni8); }
            else if (g->fni4 && check_size_impl(oprsz, 4)) {
                expand_2i_i32(dofs, aofs, oprsz, c, g->load_dest, g->fni4);
            }
            else {
                if (g->fno) { tcg_gen_gvec_2_ool(dofs, aofs, oprsz, maxsz, c, g->fno); }
                else {
    ...
    if (oprsz < maxsz) { expand_clr(tcg_env, dofs + oprsz, maxsz - oprsz); }
}
                     ...
                     int64_t c, const GVecGen4i* g)
{
    ...
    switch (type) {
        case TCG_TYPE_V256:
            ...
            maxsz -= some;
            /* fallthru */
        case TCG_TYPE_V128: expand_4i_vec(g->vece, dofs, aofs, bofs, cofs, oprsz, 16, TCG_TYPE_V128, c, g->fniv); break;
        case TCG_TYPE_V64 : expand_4i_vec(g->vece, dofs, aofs, bofs, cofs, oprsz, 8, TCG_TYPE_V64, c, g->fniv); break;

        case 0:
            if (g->fni8 && check_size_impl(oprsz, 8)) { expand_4i_i64(dofs, aofs, bofs, cofs, oprsz, c, g->fni8); }
            else if (g->fni4 && check_size_impl(oprsz, 4)) {
...
static void vec_mov2(unsigned vece, TCGv_vec a, TCGv_vec b) { tcg_gen_mov_vec(a, b); }
...
void tcg_gen_gvec_dup_mem(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t oprsz, uint32_t maxsz)
{
    ...
    if (vece <= MO_64) {
        ...
        if (type != 0) {
            ...
            switch (vece) {
                case MO_8 : tcg_gen_ld8u_i32(in, tcg_env, aofs); break;
                case MO_16: tcg_gen_ld16u_i32(in, tcg_env, aofs); break;
                default   : tcg_gen_ld_i32(in, tcg_env, aofs); break;
            }
            do_dup(vece, tcg_env, dofs, oprsz, maxsz, in, NULL, 0);
        ...
        if (TCG_TARGET_HAS_v128) {
            ...
            for (i = (aofs == dofs) * 16; i < oprsz; i += 16) { tcg_gen_st_vec(in, tcg_env, dofs + i); }
        }
        else {
        ...
        if (TCG_TARGET_HAS_v256) {
            ...
            tcg_gen_ld_vec(in, tcg_env, aofs);
            for (i = (aofs == dofs) * 32; i < oprsz; i += 32) { tcg_gen_st_vec(in, tcg_env, dofs + i); }
        }
            ...
            for (i = (aofs == dofs) * 32; i < oprsz; i += 32) {
                for (j = 0; j < 4; ++j) { tcg_gen_st_i64(in[j], tcg_env, dofs + i + j * 8); }
            }
            for (j = 0; j < 4; ++j) { tcg_temp_free_i64(in[j]); }
        }
        if (oprsz < maxsz) { expand_clr(tcg_env, dofs + oprsz, maxsz - oprsz); }
    }
    else {
...
void tcg_gen_gvec_dup_imm_var(unsigned vece, TCGv_ptr dbase, uint32_t dofs, uint32_t oprsz, uint32_t maxsz, uint64_t x)
{
    check_size_align(oprsz, maxsz, dofs);
...
static void tcg_gen_vec_abs8_i64(TCGv_i64 d, TCGv_i64 b) { gen_absv_mask(d, b, MO_8); }

static void tcg_gen_vec_abs16_i64(TCGv_i64 d, TCGv_i64 b) { gen_absv_mask(d, b, MO_16); }
...
void tcg_gen_gvec_and(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz)
{
    ...
    if (aofs == bofs) { tcg_gen_gvec_mov(vece, dofs, aofs, oprsz, maxsz); }
    else {
...
void tcg_gen_gvec_xor(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz)
{
    ...
    if (aofs == bofs) { tcg_gen_gvec_dup_imm(MO_64, dofs, oprsz, maxsz, 0); }
    else {
        tcg_gen_gvec_3(dofs, aofs, bofs, oprsz, maxsz, &g);
    }
}
...
void tcg_gen_gvec_orc(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz)
{
    ...
    if (aofs == bofs) { tcg_gen_gvec_dup_imm(MO_64, dofs, oprsz, maxsz, -1); }
    else {
        tcg_gen_gvec_3(dofs, aofs, bofs, oprsz, maxsz, &g);
    }
}
...
void tcg_gen_gvec_nand(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz)
{
    ...
    if (aofs == bofs) { tcg_gen_gvec_not(vece, dofs, aofs, oprsz, maxsz); }
    else {
        tcg_gen_gvec_3(dofs, aofs, bofs, oprsz, maxsz, &g);
    }
}
...
void tcg_gen_gvec_shli(unsigned vece, uint32_t dofs, uint32_t aofs, int64_t shift, uint32_t oprsz, uint32_t maxsz)
{
    ...
    if (shift == 0) { tcg_gen_gvec_mov(vece, dofs, aofs, oprsz, maxsz); }
    else {
                           ...
                           const GVecGen2sh* g)
{
    ...
    if (type) {
        ...
        switch (type) {
            case TCG_TYPE_V256:
                ...
                maxsz -= some;
                /* fallthru */
            case TCG_TYPE_V128: expand_2sh_vec(vece, dofs, aofs, oprsz, 16, TCG_TYPE_V128, shift, g->fniv_s); break;
            case TCG_TYPE_V64 : expand_2sh_vec(vece, dofs, aofs, oprsz, 8, TCG_TYPE_V64, shift, g->fniv_s); break;
            default           : assert_not_reached();
        }
        tcg_swap_vecop_list(hold_list);
        goto clear_tail;
    ...
    if (vece == MO_32 && check_size_impl(oprsz, 4)) { expand_2s_i32(dofs, aofs, oprsz, shift, false, g->fni4); }
                      ...
                      uint32_t maxsz)
{
    ...
    switch (type) {
        case TCG_TYPE_V256:
            ...
            maxsz -= some;
            /* fallthru */
        case TCG_TYPE_V128: expand_cmp_vec(vece, dofs, aofs, bofs, oprsz, 16, TCG_TYPE_V128, cond); break;
        case TCG_TYPE_V64 : expand_cmp_vec(vece, dofs, aofs, bofs, oprsz, 8, TCG_TYPE_V64, cond); break;

        case 0:
            if (vece == MO_64 && check_size_impl(oprsz, 8)) { expand_cmp_i64(dofs, aofs, bofs, oprsz, cond); }
