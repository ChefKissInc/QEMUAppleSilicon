...
static QTAILQ_HEAD(, StatsCallbacks) stats_callbacks = QTAILQ_HEAD_INITIALIZER(stats_callbacks);
...
static bool invoke_stats_cb(StatsCallbacks* entry, StatsResultList** stats_results, StatsFilter* filter,
                            StatsRequest* request, Error** errp)
{
    ERRP_GUARD();
    ...
    if (request) {
        if (request->provider != entry->provider) { return true; }
        if (request->has_names && !request->names) { return true; }
    ...
    QTAILQ_FOREACH (entry, &stats_callbacks, next) {
        if (filter->has_providers) {
            for (request = filter->providers; request; request = request->next) {
                if (!invoke_stats_cb(entry, &stats_results, filter, request->value, errp)) { break; }
            }
        }
        else {
            if (!invoke_stats_cb(entry, &stats_results, filter, NULL, errp)) { break; }
        }
    }
...
void add_stats_entry(StatsResultList** stats_results, StatsProvider provider, const char* qom_path,
...
void add_stats_schema(StatsSchemaList** schema_results, StatsProvider provider, StatsTarget target,
...
bool apply_str_list_filter(const char* string, strList* list)
{
    ...
    for (str_list = list; str_list; str_list = str_list->next) {
        if (g_str_equal(string, str_list->value)) { return true; }
    }
    return false;
}
