...
static void riscv_tr_init_disas_context(DisasContextBase *dcbase, CPUState *cs)
{
    ...
    ctx->mem_idx = REG_FIELD_EX32(tb_flags, TB_FLAGS, MEM_IDX);
    ctx->mstatus_fs = REG_FIELD_EX32(tb_flags, TB_FLAGS, FS);
    ctx->mstatus_vs = REG_FIELD_EX32(tb_flags, TB_FLAGS, VS);
    ctx->priv_ver = env->priv_ver;
    ctx->virt_enabled = REG_FIELD_EX32(tb_flags, TB_FLAGS, VIRT_ENABLED);
    ctx->misa_ext = env->misa_ext;
    ctx->frm = -1;  /* unknown rounding mode */
    ctx->cfg_ptr = &(cpu->cfg);
    ctx->vill = REG_FIELD_EX32(tb_flags, TB_FLAGS, VILL);
    ctx->sew = REG_FIELD_EX32(tb_flags, TB_FLAGS, SEW);
    ctx->lmul = sextract32(FIELD_EX32(tb_flags, TB_FLAGS, LMUL), 0, 3);
    ctx->vta = REG_FIELD_EX32(tb_flags, TB_FLAGS, VTA) && cpu->cfg.rvv_ta_all_1s;
    ctx->vma = REG_FIELD_EX32(tb_flags, TB_FLAGS, VMA) && cpu->cfg.rvv_ma_all_1s;
    ctx->cfg_vta_all_1s = cpu->cfg.rvv_ta_all_1s;
    ctx->vstart_eq_zero = REG_FIELD_EX32(tb_flags, TB_FLAGS, VSTART_EQ_ZERO);
    ctx->vl_eq_vlmax = REG_FIELD_EX32(tb_flags, TB_FLAGS, VL_EQ_VLMAX);
    ctx->misa_mxl_max = mcc->def->misa_mxl_max;
    ctx->xl = REG_FIELD_EX32(tb_flags, TB_FLAGS, XL);
    ctx->address_xl = REG_FIELD_EX32(tb_flags, TB_FLAGS, AXL);
    ctx->cs = cs;
    if (get_xl(ctx) == MXL_RV32) {
        ctx->addr_xl = 32;
        ctx->addr_signed = false;
    } else {
        int pm_pmm = REG_FIELD_EX32(tb_flags, TB_FLAGS, PM_PMM);
        ctx->addr_xl = 64 - riscv_pm_get_pmlen(pm_pmm);
        ctx->addr_signed = REG_FIELD_EX32(tb_flags, TB_FLAGS, PM_SIGNEXTEND);
    }
    ctx->ztso = cpu->cfg.ext_ztso;
    ctx->itrigger = REG_FIELD_EX32(tb_flags, TB_FLAGS, ITRIGGER);
    ctx->bcfi_enabled = REG_FIELD_EX32(tb_flags, TB_FLAGS, BCFI_ENABLED);
    ctx->fcfi_lp_expected = REG_FIELD_EX32(tb_flags, TB_FLAGS, FCFI_LP_EXPECTED);
    ctx->fcfi_enabled = REG_FIELD_EX32(tb_flags, TB_FLAGS, FCFI_ENABLED);
