...
static bool get_phys_addr_nogpc(CPUARMState* env, S1Translate* ptw, vaddr address, MMUAccessType access_type,
                                MemOp memop, GetPhysAddrResult* result, ARMMMUFaultInfo* fi);

static bool get_phys_addr_gpc(CPUARMState* env, S1Translate* ptw, vaddr address, MMUAccessType access_type, MemOp memop,
                              GetPhysAddrResult* result, ARMMMUFaultInfo* fi);

static int get_S1prot(CPUARMState* env, ARMMMUIdx mmu_idx, bool is_aa64, int user_rw, int prot_rw, int xn, int pxn,
...
uint8_t round_down_to_parange_index(uint8_t bit_size)
{
    for (int i = ARRAY_SIZE(pamax_map) - 1; i >= 0; i--) {
        if (pamax_map[i] <= bit_size) { return i; }
    }
    assert_not_reached();
}

uint8_t round_down_to_parange_bit_size(uint8_t bit_size) { return pamax_map[round_down_to_parange_index(bit_size)]; }
...
unsigned int arm_pamax(ARMCPU* cpu)
{
    if (arm_feature(&cpu->env, ARM_FEATURE_AARCH64)) {
        unsigned int parange = FIELD_EX64_IDREG(&cpu->isar, ID_AA64MMFR0, PARANGE);
...
{
    switch (mmu_idx) {
        case ARMMMUIdx_E10_0     : return ARMMMUIdx_Stage1_E0;
        case ARMMMUIdx_E10_1     : return ARMMMUIdx_Stage1_E1;
        case ARMMMUIdx_E10_1_PAN : return ARMMMUIdx_Stage1_E1_PAN;
        case ARMMMUIdx_GE10_1    : return ARMMMUIdx_Stage1_GE1;
        case ARMMMUIdx_GE10_1_PAN: return ARMMMUIdx_Stage1_GE1_PAN;
        default                  : return mmu_idx;
    }
}

ARMMMUIdx arm_stage1_mmu_idx(CPUARMState* env) { return stage_1_mmu_idx(arm_mmu_idx(env)); }
...
static ARMMMUIdx ptw_idx_for_stage_2(CPUARMState* env, ARMMMUIdx stage2idx)
{
     ...
     */
    if (!arm_el_is_aa64(env, 3)) { return ARMMMUIdx_Phys_NS; }

    switch (arm_security_space_below_el3(env)) {
        case ARMSS_NonSecure: return ARMMMUIdx_Phys_NS;
        case ARMSS_Realm    : return ARMMMUIdx_Phys_Realm;
        case ARMSS_Secure:
            if (stage2idx == ARMMMUIdx_Stage2_S) { s2walk_secure = !(env->cp15.vstcr_el2 & VSTCR_SW); }
            else {
...
static bool regime_translation_disabled(CPUARMState* env, ARMMMUIdx mmu_idx, ARMSecuritySpace space)
{
    uint64_t hcr_el2;
...
static bool granule_protection_check(CPUARMState* env, uint64_t paddress, ARMSecuritySpace pspace, ARMMMUFaultInfo* fi)
{
    MemTxAttrs attrs = {
        .secure = true,
        .space  = ARMSS_Root,
    ...
    if (!REG_FIELD_EX64(gpccr, GPCCR, GPC)) { return true; }
    ...
    if (pps > FIELD_EX64_IDREG(&cpu->isar, ID_AA64MMFR0, PARANGE)) { goto fault_walk; }
    pps      = pamax_map[pps];
    pps_mask = MAKE_64BIT_MASK(0, pps);

    switch (REG_FIELD_EX64(gpccr, GPCCR, SH)) {
        case 0b10: /* outer shareable */ break;
        case 0b00: /* non-shareable */
        case 0b11: /* inner shareable */
            /* Inner and Outer non-cacheable requires Outer shareable. */
            if (REG_FIELD_EX64(gpccr, GPCCR, ORGN) == 0 && REG_FIELD_EX64(gpccr, GPCCR, IRGN) == 0) { goto fault_walk; }
            break;
    ...
    if (paddress & ~pps_mask) {
        if (pspace == ARMSS_NonSecure) { return true; }
        goto fault_size;
    }

    /* GPC Priority 3: the base address of GPTBR_EL3 exceeds PPS. */
    tableaddr = env->cp15.gptbr_el3 << 12;
    if (tableaddr & ~pps_mask) { goto fault_size; }
    ...
    if (result != MEMTX_OK) { goto fault_eabt; }

    switch (extract32(entry, 0, 4)) {
        case 1: /* block descriptor */
            if (entry >> 8) { goto fault_walk; /* RES0 bits not 0 */ }
        ...
        case 3: /* table descriptor */
            ...
            align     = MAKE_64BIT_MASK(0, align);
            if (tableaddr & (~pps_mask | align)) { goto fault_walk; /* RES0 bits not 0 */ }
            break;
        default: /* invalid */ goto fault_walk;
    }
    ...
    switch (extract32(entry, 0, 4)) {
        case 1: /* contiguous descriptor */
            if (entry >> 10) { goto fault_walk; /* RES0 bits not 0 */ }
            /*
             ...
             */
            if (extract32(entry, 8, 2) == 0) { goto fault_walk; /* reserved contig */ }
            gpi = extract32(entry, 4, 4);
            break;
        default:
...
found:
    switch (gpi) {
        case 0b0000: /* no access */ break;
        case 0b1111: /* all access */ return true;
        case 0b1000:
        case 0b1001:
        case 0b1010:
        case 0b1011:
            if (pspace == (gpi & 3)) { return true; }
            break;
        default: goto fault_walk; /* reserved */
    }
...
static bool S2_attrs_are_device(uint64_t hcr, uint8_t attrs)
{
     ...
     */
    if (hcr & HCR_FWB) { return (attrs & 0x4) == 0; }
    else {
        return (attrs & 0xc) == 0;
    }
}

static ARMSecuritySpace S2_security_space(ARMSecuritySpace s1_space, ARMMMUIdx s2_mmu_idx)
{
    /*
    ...
    if (regime_is_stage2(s2_mmu_idx)) {
         ...
         */
        if (s1_space == ARMSS_Secure) { return arm_secure_to_space(s2_mmu_idx == ARMMMUIdx_Stage2_S); }
        else {
...
static bool fault_s1ns(ARMSecuritySpace space, ARMMMUIdx s2_mmu_idx)
{
     ...
     */
    return space == ARMSS_Secure && regime_is_stage2(s2_mmu_idx) && s2_mmu_idx == ARMMMUIdx_Stage2_S;
}

/* Translate a S1 pagetable walk through S2 if needed.  */
static bool S1_ptw_translate(CPUARMState* env, S1Translate* ptw, hwaddr addr, ARMMMUFaultInfo* fi)
{
    ...
    if (unlikely(ptw->in_debug)) {
        ...
        if (get_phys_addr_gpc(env, &s2ptw, addr, MMU_DATA_LOAD, 0, &s2, fi)) { goto fail; }
        ...
        env->tlb_fi = fi;
        flags =
            probe_access_full_mmu(env, addr, 0, MMU_DATA_LOAD, arm_to_core_mmu_idx(s2_mmu_idx), &ptw->out_host, &full);
        env->tlb_fi = NULL;

        if (unlikely(flags & TLB_INVALID_MASK)) { goto fail; }
...
fail:
    ...
    if (fi->type == ARMFault_GPCFOnOutput) { fi->type = ARMFault_GPCFOnWalk; }
    fi->s2addr = addr;
...
static uint32_t arm_ldl_ptw(CPUARMState* env, S1Translate* ptw, ARMMMUFaultInfo* fi)
{
    ...
    if (likely(host)) {
        ...
        if (ptw->out_be) { data = be32_to_cpu(data); }
        else {
        ...
        if (ptw->out_be) { data = address_space_ldl_be(as, ptw->out_phys, attrs, &result); }
        else {
...
static uint64_t arm_ldq_ptw(CPUARMState* env, S1Translate* ptw, ARMMMUFaultInfo* fi)
{
    CPUState* cs   = env_cpu(env);
    void*     host = ptw->out_host;
    uint64_t  data;
    ...
    if (likely(host)) {
        ...
        if (ptw->out_be) { data = be64_to_cpu(data); }
        else {
            data = le64_to_cpu(data);
        }
#else
        if (ptw->out_be) { data = ldq_be_p(host); }
        else {
            data = ldq_le_p(host);
        }
#endif
        ...
        if (ptw->out_be) { data = address_space_ldq_be(as, ptw->out_phys, attrs, &result); }
        else {
...
static uint64_t arm_casq_ptw(CPUARMState* env, uint64_t old_val, uint64_t new_val, S1Translate* ptw,
                             ARMMMUFaultInfo* fi)
{
    ...
    if (unlikely(!ptw->out_rw)) {
        ...
        env->tlb_fi = fi;
        flags = probe_access_full_mmu(env, ptw->out_virt, 0, MMU_DATA_STORE, arm_to_core_mmu_idx(ptw->in_ptw_idx), NULL,
                                      NULL);
        env->tlb_fi = NULL;
...
static bool get_level1_table_address(CPUARMState* env, ARMMMUIdx mmu_idx, uint32_t* table, uint32_t address)
{
 ...
 */
static int ap_to_rw_prot_is_user(CPUARMState* env, ARMMMUIdx mmu_idx, int ap, int domain_prot, bool is_user)
{
    if (domain_prot == 3) { return PAGE_READ | PAGE_WRITE; }

    switch (ap) {
        case 0:
            if (arm_feature(env, ARM_FEATURE_V7)) { return 0; }
            switch (regime_sctlr(env, mmu_idx) & (SCTLR_S | SCTLR_R)) {
                case SCTLR_S: return is_user ? 0 : PAGE_READ;
                case SCTLR_R: return PAGE_READ;
                default     : return 0;
            }
        case 1: return is_user ? 0 : PAGE_READ | PAGE_WRITE;
        case 2:
            if (is_user) { return PAGE_READ; }
            else {
                return PAGE_READ | PAGE_WRITE;
            }
        case 3: return PAGE_READ | PAGE_WRITE;
        case 4: /* Reserved.  */ return 0;
        case 5: return is_user ? 0 : PAGE_READ;
        case 6: return PAGE_READ;
        case 7:
...
{
    switch (ap) {
        case 0 : return is_user ? 0 : PAGE_READ | PAGE_WRITE;
        case 1 : return PAGE_READ | PAGE_WRITE;
        case 2 : return is_user ? 0 : PAGE_READ;
        case 3 : return PAGE_READ;
        default: assert_not_reached();
    }
}
 ...
 */
static inline int pte_to_sprr_prot_is_guarded(CPUARMState* env, int ap, int xn, int pxn, bool guarded)
{
    int      el;
    int      sprr_idx;
    uint64_t sprr_perm;
    ...
    if (!arm_is_sprr_enabled(env)) { return PAGE_READ | PAGE_WRITE | PAGE_EXEC; }
 ...
 * @pxn:     PXN (privileged-execute-never) bits
 */
static inline int pte_to_sprr_prot(CPUARMState* env, int ap, int xn, int pxn)
{ return pte_to_sprr_prot_is_guarded(env, ap, xn, pxn, arm_is_guarded(env)); }

static bool get_phys_addr_v5(CPUARMState* env, S1Translate* ptw, uint32_t address, MMUAccessType access_type,
                             GetPhysAddrResult* result, ARMMMUFaultInfo* fi)
{
    int      level = 1;
    uint32_t table;
    ...
    if (!S1_ptw_translate(env, ptw, table, fi)) { goto do_fault; }
    ...
    if (type == 2) {
        ...
        if (fi->type != ARMFault_None) { goto do_fault; }
        switch (desc & 3) {
            case 0: /* Page translation fault.  */ fi->type = ARMFault_Translation; goto do_fault;
            case 1: /* 64k page.  */
...
static bool get_phys_addr_v6(CPUARMState* env, S1Translate* ptw, uint32_t address, MMUAccessType access_type,
                             GetPhysAddrResult* result, ARMMMUFaultInfo* fi)
{
    ARMCPU*          cpu     = env_archcpu(env);
    ARMMMUIdx        mmu_idx = ptw->in_mmu_idx;
    ...
    if (type != 1) {
        ...
        if (cpu_isar_feature(aa32_pxn, cpu)) { pxn = (desc >> 2) & 1; }
    ...
    if (domain_prot == 3) { result->f.prot = PAGE_READ | PAGE_WRITE | PAGE_EXEC; }
    else {
        int user_rw, prot_rw;

        if (arm_feature(env, ARM_FEATURE_V6K) && (regime_sctlr(env, mmu_idx) & SCTLR_AFE)) {
        ...
        result->f.prot = get_S1prot(env, mmu_idx, false, user_rw, prot_rw, xn, pxn, result->f.attrs.space, out_space);
        if (ptw->in_prot_check & ~result->f.prot) {
            /* Access permission fault.  */
            fi->type = ARMFault_Permission;
            goto do_fault;
...
static int get_S2prot(CPUARMState* env, int s2ap, int xn, bool s1_is_el0)
{
    ...
    if (cpu_isar_feature(any_tts2uxn, env_archcpu(env))) {
        ...
        if (!extract32(xn, 1, 1)) {
            if (arm_el_is_aa64(env, 2) || prot & PAGE_READ) { prot |= PAGE_EXEC; }
        }
    }
    return prot;
}
                      ...
                      ARMSecuritySpace in_pa, ARMSecuritySpace out_pa)
{
    ...
    if (is_user) {
        if (!arm_is_sprr_enabled(env)) { prot_rw = user_rw; }
    }
    else {
        /*
    ...
    if (in_pa != out_pa) {
        switch (in_pa) {
            ...
            case ARMSS_Realm:
                ...
                break;
            case ARMSS_Secure:
                if (env->cp15.scr_el3 & SCR_SIF) { return prot_rw; }
                break;
            default:
    ...
    if (have_wxn) { wxn = regime_sctlr(env, mmu_idx) & SCTLR_WXN; }

    if (arm_is_sprr_enabled(env)) { xn = pxn; }
        ...
        switch (regime_el(env, mmu_idx)) {
            ...
            case 3:
                if (is_user) { xn = xn || !(user_rw & PAGE_READ); }
                    ...
                    int uwxn = 0;
                    if (have_wxn) { uwxn = regime_sctlr(env, mmu_idx) & SCTLR_UWXN; }
                    xn = xn || !(prot_rw & PAGE_READ) || pxn || (uwxn && (user_rw & PAGE_WRITE));
                }
                break;
            case 2: break;
        }
    ...
    if (xn || (wxn && (prot_rw & PAGE_WRITE))) { return prot_rw; }
...
static ARMVAParameters aa32_va_parameters(CPUARMState* env, uint32_t va, ARMMMUIdx mmu_idx)
{
    ...
    if (mmu_idx == ARMMMUIdx_Stage2) {
         ...
         */
        if (sign != sext) { qemu_log_mask(LOG_GUEST_ERROR, "AArch32: VTCR.S / VTCR.T0SZ[3] mismatch\n"); }
        ...
        if (t1sz == 0) { select = va > (0xffffffffu >> t0sz); }
        else {
 ...
 */
static int check_s2_mmu_setup(ARMCPU* cpu, bool is_aa64, uint64_t tcr, bool ds, int iasize, int stride)
{
        ...
        switch (stride) {
            case 9: /* 4KB */
                ...
                if (ds && sl2) {
                    ...
                    switch (sl0) {
                        case 2:
                            if (arm_pamax(cpu) < 44) { goto fail; }
                            break;
                        case 3:
                            if (!cpu_isar_feature(aa64_st, cpu)) { goto fail; }
                            startlevel = 3;
                            break;
                    }
                }
    ...
    if (iasize >= s1_min_iasize && iasize <= s1_max_iasize) { return startlevel; }

fail:
    return INT_MIN;
}

static bool lpae_block_desc_valid(ARMCPU* cpu, bool ds, ARMGranuleSize gran, int level)
{
    /*
     ...
     */
    switch (gran) {
        case Gran4K : return (level == 0 && ds) || level == 1 || level == 2;
 ...
 */
static bool get_phys_addr_lpae(CPUARMState* env, S1Translate* ptw, uint64_t address, MMUAccessType access_type,
                               MemOp memop, GetPhysAddrResult* result, ARMMMUFaultInfo* fi)
{
    ARMCPU*          cpu     = env_archcpu(env);
    ARMMMUIdx        mmu_idx = ptw->in_mmu_idx;
    ...
    if (aarch64) {
        ...
        param = aa64_va_parameters(env, address, mmu_idx, access_type != MMU_INST_FETCH, !arm_el_is_aa64(env, 1));
        level = 0;
         ...
         */
        if (param.tsz_oob) { goto do_translation_fault; }
         ...
         * unless TCR.DS == 1
         */
        if (!param.ds && param.gran != Gran64K) { outputsize = MIN(outputsize, 48); }
    }
    else {
    ...
    if (inputsize < addrsize) {
        uint64_t top_bits = sextract64(address, inputsize, addrsize - inputsize);
    ...
    if (!regime_is_stage2(mmu_idx)) {
        ...
        int startlevel = check_s2_mmu_setup(cpu, aarch64, tcr, param.ds, inputsize, stride);
     ...
     */
    if (outputsize > 48) { descaddr |= extract64(ttbr, 2, 4) << 48; }
     ...
     */
    if (param.ds) { descaddrmask = MAKE_64BIT_MASK(0, 50); }
...
next_level:
     ...
     */
    if (ptw->in_space == ARMSS_Secure && !regime_is_stage2(mmu_idx) && extract32(tableattrs, 4, 1)) {
        /*
    ...
    if (!S1_ptw_translate(env, ptw, descaddr, fi)) { goto do_fault; }
    descriptor = arm_ldq_ptw(env, ptw, fi);
    if (fi->type != ARMFault_None) { goto do_fault; }
    new_descriptor = descriptor;

restart_atomic_update:
    if (!(descriptor & 1) || (!(descriptor & 2) && !lpae_block_desc_valid(cpu, param.ds, param.gran, level))) {
     ...
     */
    if (outputsize > 48) {
        if (param.ds) { descaddr |= extract64(descriptor, 8, 2) << 50; }
        else {
    ...
    if (likely(!ptw->in_at)) {
         ...
         */
        if (!(descriptor & (1 << 10))) {
            if (param.ha) { new_descriptor |= 1 << 10; /* AF */ }
            else {
                fi->type = ARMFault_AccessFlag;
                goto do_fault;
            }
        ...
        {
            if (regime_is_stage2(mmu_idx)) { new_descriptor |= 1ull << 7; /* set S2AP[1] */ }
            else {
    ...
    if (regime_is_stage2(mmu_idx)) {
         ...
         */
        device = S2_attrs_are_device(arm_hcr_el2_eff(env), result->cacheattrs.attrs);
    }
    else {
        ...
        switch (out_space) {
            case ARMSS_Root:
                ...
                out_space = (nse << 1) | ns;
                if (out_space == ARMSS_Secure && !cpu_isar_feature(aa64_sel2, cpu)) { out_space = ARMSS_NonSecure; }
                break;
            case ARMSS_Secure:
         ...
         */
        result->f.prot = get_S1prot(env, mmu_idx, aarch64, user_rw, prot_rw, xn, pxn, result->f.attrs.space, out_space);

        if (access_type == MMU_INST_FETCH) {
            if (arm_is_sprr_enabled(env) && !arm_is_guarded(env)) {
                if (!(result->f.prot & (1 << access_type))) {
                    int gl_prot = pte_to_sprr_prot_is_guarded(env, ap, xn, pxn, true);
     ...
     */
    if (param.ds) { result->cacheattrs.shareability = param.sh; }
    else {
 ...
 */
static uint8_t combined_attrs_nofwb(uint64_t hcr, ARMCacheAttrs s1, ARMCacheAttrs s2)
{
    uint8_t s1lo, s2lo, s1hi, s2hi, s2_mair_attrs, ret_attrs;

    if (s2.is_s2_format) { s2_mair_attrs = convert_stage2_attrs(hcr, s2.attrs); }
    else {
    ...
    else { /* Normal memory */ /* Outer/inner cacheability combine independently */
        ret_attrs = combine_cacheattr_nibble(s1hi, s2hi) << 4 | combine_cacheattr_nibble(s1lo, s2lo);
    }
    return ret_attrs;
}
...
static uint8_t combined_attrs_fwb(ARMCacheAttrs s1, ARMCacheAttrs s2)
{
    ...
    switch (s2.attrs) {
        ...
        case 6:
            ...
            return force_cacheattr_nibble_wb(s1.attrs & 0xf) | force_cacheattr_nibble_wb(s1.attrs >> 4) << 4;
        case 5:
            /* If S1 attrs are Device, use them; otherwise Normal Non-cacheable */
            if ((s1.attrs & 0xf0) == 0) { return s1.attrs; }
            return 0x44;
        case 0 ... 3:
 ...
 */
static ARMCacheAttrs combine_cacheattrs(uint64_t hcr, ARMCacheAttrs s1, ARMCacheAttrs s2)
{
    ARMCacheAttrs ret;
    bool          tagged = false;
    ...
    /* Combine memory type and cacheability attributes */
    if (hcr & HCR_FWB) { ret.attrs = combined_attrs_fwb(s1, s2); }
    else {
     ...
     */
    if ((ret.attrs & 0xf0) == 0 || ret.attrs == 0x44) { ret.shareability = 2; }

    /* TODO: CombineS1S2Desc does not consider transient, only WB, RWA. */
    if (tagged && ret.attrs == 0xff) { ret.attrs = 0xf0; }
 ...
 */
static bool get_phys_addr_disabled(CPUARMState* env, S1Translate* ptw, vaddr address, MMUAccessType access_type,
                                   GetPhysAddrResult* result, ARMMMUFaultInfo* fi)
{
    ARMMMUIdx mmu_idx      = ptw->in_mmu_idx;
    ...
    switch (mmu_idx) {
        ...
        default:
            ...
            if (arm_el_is_aa64(env, r_el)) {
                ...
                if (access_type == MMU_INST_FETCH) { tbi &= ~aa64_va_parameter_tbid(tcr, mmu_idx); }
            ...
            if (r_el == 1) {
                uint64_t hcr = arm_hcr_el2_eff_secstate(env, ptw->in_space);
                if (hcr & HCR_DC) {
                    if (hcr & HCR_DCT) { memattr = 0xf0; /* Tagged, Normal, WB, RWA */ }
                    else {
            ...
            if (memattr == 0) {
                if (access_type == MMU_INST_FETCH) {
                    if (regime_sctlr(env, mmu_idx) & SCTLR_I) { memattr = 0xee; /* Normal, WT, RA, NT */ }
                    else {
...
static bool get_phys_addr_twostage(CPUARMState* env, S1Translate* ptw, vaddr address, MMUAccessType access_type,
                                   MemOp memop, GetPhysAddrResult* result, ARMMMUFaultInfo* fi)
{
    hwaddr           ipa;
    ...
    ret = get_phys_addr_nogpc(env, ptw, address, access_type, memop, result, fi);
     ...
     */
    if (result->f.lg_page_size < TARGET_PAGE_BITS || s1_lgpgsz < TARGET_PAGE_BITS) { result->f.lg_page_size = 0; }
    ...
    if (hcr & HCR_DC) {
         ...
         */
        if (cacheattrs1.attrs != 0xf0) { cacheattrs1.attrs = 0xff; }
        cacheattrs1.shareability = 0;
    }
    result->cacheattrs = combine_cacheattrs(hcr, cacheattrs1, result->cacheattrs);
    ...
    if (in_space == ARMSS_Secure) {
        result->f.attrs.secure = !(env->cp15.vstcr_el2 & (VSTCR_SA | VSTCR_SW))
                                ...
                                MemOp memop, GetPhysAddrResult* result, ARMMMUFaultInfo* fi)
{
    ...
    switch (mmu_idx) {
        ...
        case ARMMMUIdx_Stage1_GE1_PAN:
             ...
             */
            ptw->in_ptw_idx = (ptw->in_space == ARMSS_Secure) ? ARMMMUIdx_Stage2_S : ARMMMUIdx_Stage2;
            break;
        ...
        case ARMMMUIdx_E10_1_PAN: s1_mmu_idx = ARMMMUIdx_Stage1_E1_PAN; goto do_twostage;
        case ARMMMUIdx_GE10_1   : s1_mmu_idx = ARMMMUIdx_Stage1_GE1; goto do_twostage;
        case ARMMMUIdx_GE10_1_PAN:
        ...
        do_twostage:
            ...
            if (arm_feature(env, ARM_FEATURE_EL2) && !regime_translation_disabled(env, ARMMMUIdx_Stage2, ptw->in_space))
            {
                return get_phys_addr_twostage(env, ptw, address, access_type, memop, result, fi);
            }
            /* fall through */
    ...
    if (regime_using_lpae_format(env, mmu_idx)) {
        return get_phys_addr_lpae(env, ptw, address, access_type, memop, result, fi);
    }
    else if (arm_feature(env, ARM_FEATURE_V7) || regime_sctlr(env, mmu_idx) & SCTLR_XP) {
...
static bool get_phys_addr_gpc(CPUARMState* env, S1Translate* ptw, vaddr address, MMUAccessType access_type, MemOp memop,
                              GetPhysAddrResult* result, ARMMMUFaultInfo* fi)
{
    if (get_phys_addr_nogpc(env, ptw, address, access_type, memop, result, fi)) { return true; }
    if (!granule_protection_check(env, result->f.phys_addr, result->f.attrs.space, fi)) {
                          ...
                          ARMSecuritySpace space, GetPhysAddrResult* result, ARMMMUFaultInfo* fi)
{
     ...
     */
    return get_phys_addr_nogpc(env, &ptw, address, MMU_DATA_LOAD, MO_8, result, fi);
}

static ARMSecuritySpace arm_mmu_idx_to_security_space(CPUARMState* env, ARMMMUIdx mmu_idx)
{
    ARMSecuritySpace ss;
    ...
    switch (mmu_idx) {
        ...
        case ARMMMUIdx_GE30_3_PAN:
            ...
            break;
        case ARMMMUIdx_Phys_Root : ss = ARMSS_Root; break;
        case ARMMMUIdx_Phys_Realm: ss = ARMSS_Realm; break;
        default                  : assert_not_reached();
    }
                   ...
                   GetPhysAddrResult* result, ARMMMUFaultInfo* fi)
{
    ...
    return get_phys_addr_gpc(env, &ptw, address, access_type, memop, result, fi);
}

static hwaddr arm_cpu_get_phys_page(CPUARMState* env, vaddr addr, MemTxAttrs* attrs, ARMMMUIdx mmu_idx)
{
    S1Translate ptw = {
        .in_mmu_idx    = mmu_idx,
        .in_space      = arm_mmu_idx_to_security_space(env, mmu_idx),
...
hwaddr arm_cpu_get_phys_page_attrs_debug(CPUState* cs, vaddr addr, MemTxAttrs* attrs)
{
    ...
    switch (mmu_idx) {
        ...
        case ARMMMUIdx_E20_2_PAN :
        case ARMMMUIdx_GE20_2    :
        case ARMMMUIdx_GE20_2_PAN: return arm_cpu_get_phys_page(env, addr, attrs, ARMMMUIdx_E20_0);
        default                  : return -1;
    }
}
