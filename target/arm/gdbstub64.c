...
int aarch64_cpu_gdb_read_register(CPUState* cs, GByteArray* mem_buf, int n)
{
    ...
    switch (n) {
        case 31: return gdb_get_reg64(mem_buf, env->xregs[31]);
        case 32: return gdb_get_reg64(mem_buf, env->pc);
        case 33: return gdb_get_reg32(mem_buf, pstate_read(env));
    }
    /* Unknown register.  */
    return 0;
}
...
int aarch64_cpu_gdb_write_register(CPUState* cs, uint8_t* mem_buf, int n)
{
    ...
    switch (n) {
        case 31: env->xregs[31] = tmp; return 8;
        case 32: env->pc = tmp; return 8;
        case 33:
            /* CPSR */
    ...
    switch (reg) {
        ...
        case 0 ... 31: {
            ...
            for (vq = 0; vq < cpu->sve_max_vq; vq++) {
                len += gdb_get_reg128(buf, env->vfp.zregs[reg].d[vq * 2 + 1], env->vfp.zregs[reg].d[vq * 2]);
            }
            return len;
        }
        case 32: return gdb_get_reg32(buf, vfp_get_fpsr(env));
        case 33: return gdb_get_reg32(buf, vfp_get_fpcr(env));
...
int aarch64_gdb_set_sve_reg(CPUState* cs, uint8_t* buf, int reg)
{
    ...
    switch (reg) {
        ...
        case 32       : vfp_set_fpsr(env, *(uint32_t*)buf); return 4;
        case 33       : vfp_set_fpcr(env, *(uint32_t*)buf); return 4;
        case 34 ... 50: {
            int preg = reg - 34;
            int vq, len = 0;
            for (vq = 0; vq < cpu->sve_max_vq; vq = vq + 4) {
    ...
    switch (reg) {
        case 0: /* svg register */
        {
            ...
            if (REG_FIELD_EX64(env->svcr, SVCR, SM)) {
                vq = sve_vqm1_for_el_sm(env, arm_current_el(env), REG_FIELD_EX64(env->svcr, SVCR, SM)) + 1;
            }
            /* svg = vector granules (2 * vector quardwords) in streaming mode */
            return gdb_get_reg64(buf, vq * 2);
        }
        case 1: /* svcr register */ return gdb_get_reg64(buf, env->svcr);
        case 2: /* za register */
        {
            int len = 0;
            ...
            for (int i = 0; i < svl; i++) {
                for (int q = 0; q < vq; q++) {
                    len += gdb_get_reg128(buf, env->za_state.za[i].d[q * 2 + 1], env->za_state.za[i].d[q * 2]);
                }
            }
            return len;
        }
...
int aarch64_gdb_set_sme_reg(CPUState* cs, uint8_t* buf, int reg)
{
    ...
    switch (reg) {
        case 0: /* svg register */
            ...
            return 8;
        case 1: /* svcr register */ aarch64_set_svcr(env, ldq_le_p(buf), R_SVCR_SM_MASK | R_SVCR_ZA_MASK); return 8;
        case 2: /* za register */
        {
            int len = 0;
            int vq  = cpu->sme_max_vq;
...
static void output_vector_union_type(GDBFeatureBuilder* builder, int reg_width, const char* name)
{
    struct TypeSize
    {
        const char* gdb_type;
    ...
    for (i = 0; i < ARRAY_SIZE(vec_lanes); i++) {
        gdb_feature_builder_append_tag(builder, "<vector id=\"%s%c%c\" type=\"%s\" count=\"%d\"/>", name,
    ...
    for (i = 0; i < ARRAY_SIZE(suf); i++) {
        ...
        gdb_feature_builder_append_tag(builder, "<union id=\"%sn%c\">", name, suf[i]);
        for (j = 0; j < ARRAY_SIZE(vec_lanes); j++) {
            if (vec_lanes[j].size == bits) {
                gdb_feature_builder_append_tag(builder, "<field name=\"%c\" type=\"%s%c%c\"/>", vec_lanes[j].suffix,
    ...
    for (i = ARRAY_SIZE(suf) - 1; i >= 0; i--) {
        gdb_feature_builder_append_tag(builder, "<field name=\"%c\" type=\"%sn%c\"/>", suf[i], name, suf[i]);
    }
    gdb_feature_builder_append_tag(builder, "</union>");
}
...
GDBFeature* arm_gen_dynamic_svereg_feature(CPUState* cs, int base_reg)
{
    ...
    gdb_feature_builder_init(&builder, &cpu->dyn_svereg_feature.desc, "org.gnu.gdb.aarch64.sve", "sve-registers.xml",
                             base_reg);
    ...
    gdb_feature_builder_append_tag(&builder, "<vector id=\"svep\" type=\"uint8\" count=\"%d\"/>", pred_width / 8);

    /* Define the vector registers. */
    for (i = 0; i < 32; i++) {
        name = g_strdup_printf("z%d", i);
        gdb_feature_builder_append_reg(&builder, name, reg_width, reg++, "svev", NULL);
    }

    /* fpscr & status registers */
    gdb_feature_builder_append_reg(&builder, "fpsr", 32, reg++, "int", "float");
    gdb_feature_builder_append_reg(&builder, "fpcr", 32, reg++, "int", "float");

    /* Define the predicate registers. */
    for (i = 0; i < 16; i++) {
        name = g_strdup_printf("p%d", i);
        gdb_feature_builder_append_reg(&builder, name, pred_width, reg++, "svep", NULL);
    }
    gdb_feature_builder_append_reg(&builder, "ffr", pred_width, reg++, "svep", "vector");
...
GDBFeature* arm_gen_dynamic_smereg_feature(CPUState* cs, int base_reg)
{
    ...
    gdb_feature_builder_init(&builder, &cpu->dyn_smereg_feature.desc, "org.gnu.gdb.aarch64.sme", "sme-registers.xml",
                             base_reg);

    /* Create the sme_bv vector type. */
    gdb_feature_builder_append_tag(&builder, "<vector id=\"sme_bv\" type=\"uint8\" count=\"%d\"/>", svl);

    /* Create the sme_bvv vector type. */
    gdb_feature_builder_append_tag(&builder, "<vector id=\"sme_bvv\" type=\"sme_bv\" count=\"%d\"/>", svl);
    ...
    gdb_feature_builder_append_reg(&builder, "za", svl * svl * 8, reg++, "sme_bvv", NULL);
...
static void handle_q_memtag(GArray* params, void* user_ctx)
{
    ...
    tags = allocation_tag_mem_probe(env, mmu_index, addr, MMU_DATA_LOAD, 1, MMU_DATA_LOAD, true, 0);
    if (!tags) {
...
static void handle_Q_memtag(GArray* params, void* user_ctx)
{
    ...
    uint64_t    len          = gdb_get_cmd_param(params, 1)->val_ul;
    int         type         = gdb_get_cmd_param(params, 2)->val_ul;
    const char* new_tags_str = gdb_get_cmd_param(params, 3)->data;
    ...
    /* Find out the current translation regime for probe. */
    mmu_index = cpu_mmu_index(env_cpu(env), false);
    tags      = allocation_tag_mem_probe(env, mmu_index, start_addr, MMU_DATA_STORE, 1, MMU_DATA_STORE, true, 0);
    if (!tags) {
        /* Address is not in a tagged region. */
        gdb_put_packet("E04");
        return;
...
static const GdbCmdParseEntry cmd_handler_table[NUM_CMDS] = {
    [qMemTags]         = {.handler          = handle_q_memtag,
                          .cmd_startswith   = true,
                          .cmd              = "MemTags:",
                          .schema           = "L,l:l0",
                          .need_cpu_context = true},
    [qIsAddressTagged] = {.handler          = handle_q_isaddresstagged,
                          .cmd_startswith   = true,
                          .cmd              = "IsAddressTagged:",
                          .schema           = "L0",
                          .need_cpu_context = true},
...
void aarch64_cpu_register_gdb_commands(ARMCPU* cpu, GString* qsupported, GPtrArray* qtable, GPtrArray* stable)
{
    /* MTE */
#ifdef CONFIG_TCG
