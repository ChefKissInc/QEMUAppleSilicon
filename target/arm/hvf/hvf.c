...
static inline int hvf_arm_num_brps(hv_vcpu_config_t config)
{
    uint64_t    val;
    hv_return_t ret;
    ret = hv_vcpu_config_get_feature_reg(config, HV_FEATURE_REG_ID_AA64DFR0_EL1, &val);
    assert_hvf_ok(ret);
...
#define SYSREG(op0, op1, crn, crm, op2)                                                                            \
    ((op0 << SYSREG_OP0_SHIFT) | (op1 << SYSREG_OP1_SHIFT) | (crn << SYSREG_CRN_SHIFT) | (crm << SYSREG_CRM_SHIFT) \
     | (op2 << SYSREG_OP2_SHIFT))
#define SYSREG_MASK          SYSREG(SYSREG_OP0_MASK, SYSREG_OP1_MASK, SYSREG_CRN_MASK, SYSREG_CRM_MASK, SYSREG_OP2_MASK)
...
static const struct hvf_reg_match hvf_reg_match[] = {
    {HV_REG_X0, offsetof(CPUARMState, xregs[0])},   {HV_REG_X1, offsetof(CPUARMState, xregs[1])},
    {HV_REG_X2, offsetof(CPUARMState, xregs[2])},   {HV_REG_X3, offsetof(CPUARMState, xregs[3])},
    {HV_REG_X4, offsetof(CPUARMState, xregs[4])},   {HV_REG_X5, offsetof(CPUARMState, xregs[5])},
    {HV_REG_X6, offsetof(CPUARMState, xregs[6])},   {HV_REG_X7, offsetof(CPUARMState, xregs[7])},
    {HV_REG_X8, offsetof(CPUARMState, xregs[8])},   {HV_REG_X9, offsetof(CPUARMState, xregs[9])},
    {HV_REG_X10, offsetof(CPUARMState, xregs[10])}, {HV_REG_X11, offsetof(CPUARMState, xregs[11])},
    {HV_REG_X12, offsetof(CPUARMState, xregs[12])}, {HV_REG_X13, offsetof(CPUARMState, xregs[13])},
    {HV_REG_X14, offsetof(CPUARMState, xregs[14])}, {HV_REG_X15, offsetof(CPUARMState, xregs[15])},
    {HV_REG_X16, offsetof(CPUARMState, xregs[16])}, {HV_REG_X17, offsetof(CPUARMState, xregs[17])},
    {HV_REG_X18, offsetof(CPUARMState, xregs[18])}, {HV_REG_X19, offsetof(CPUARMState, xregs[19])},
    {HV_REG_X20, offsetof(CPUARMState, xregs[20])}, {HV_REG_X21, offsetof(CPUARMState, xregs[21])},
    {HV_REG_X22, offsetof(CPUARMState, xregs[22])}, {HV_REG_X23, offsetof(CPUARMState, xregs[23])},
    {HV_REG_X24, offsetof(CPUARMState, xregs[24])}, {HV_REG_X25, offsetof(CPUARMState, xregs[25])},
    {HV_REG_X26, offsetof(CPUARMState, xregs[26])}, {HV_REG_X27, offsetof(CPUARMState, xregs[27])},
    {HV_REG_X28, offsetof(CPUARMState, xregs[28])}, {HV_REG_X29, offsetof(CPUARMState, xregs[29])},
    {HV_REG_X30, offsetof(CPUARMState, xregs[30])}, {HV_REG_PC, offsetof(CPUARMState, pc)},
};
...
#define HVF_TO_KVMID(HVF) (CP_REG_ARM64 | CP_REG_SIZE_U64 | CP_REG_ARM64_SYSREG | (HVF))

/* Verify this at compile-time. */

#define DEF_SYSREG(HVF_ID, ...) QEMU_BUILD_BUG_ON(HVF_ID != KVMID_TO_HVF(KVMID_AA64_SYS_REG64(__VA_ARGS__)));
...
int hvf_arch_get_registers(CPUState* cpu)
{
    ...
    for (i = 0; i < ARRAY_SIZE(hvf_fpreg_match); i++) {
        ret = hv_vcpu_get_simd_fp_reg(cpu->accel->fd, hvf_fpreg_match[i].reg, &fpval);
...
int hvf_arch_put_registers(CPUState* cpu)
{
    ...
    for (i = 0; i < ARRAY_SIZE(hvf_fpreg_match); i++) {
        ...
        ret = hv_vcpu_set_simd_fp_reg(cpu->accel->fd, hvf_fpreg_match[i].reg, fpval);
        assert_hvf_ok(ret);
    }
...
{
    uint32_t ipa_size = chosen_ipa_bit_size ? chosen_ipa_bit_size : hvf_arm_get_max_ipa_bit_size();
...
static bool hvf_arm_get_host_cpu_features(ARMHostCPUFeatures* ahcf)
{
     ...
     */
    SET_IDREG(&host_isar, ID_AA64PFR1, GET_IDREG(&host_isar, ID_AA64PFR1) & ~R_ID_AA64PFR1_SME_MASK);
    ...
    if ((GET_IDREG(&host_isar, ID_AA64PFR0) & 0xff) != 0x11) { return false; }
...
hv_return_t hvf_arch_vm_create(MachineState* ms, uint32_t pa_range)
{
    ...
    if (ret != HV_SUCCESS) { goto cleanup; }
...
int hvf_arch_init_vcpu(CPUState* cpu)
{
    ...
    arm_cpu->cpreg_indexes         = g_renew(uint64_t, arm_cpu->cpreg_indexes, sregs_match_len);
    arm_cpu->cpreg_values          = g_renew(uint64_t, arm_cpu->cpreg_values, sregs_match_len);
    arm_cpu->cpreg_vmstate_indexes = g_renew(uint64_t, arm_cpu->cpreg_vmstate_indexes, sregs_match_len);
    arm_cpu->cpreg_vmstate_values  = g_renew(uint64_t, arm_cpu->cpreg_vmstate_values, sregs_match_len);
    ...
    ret = hv_vcpu_get_sys_reg(cpu->accel->fd, HV_SYS_REG_ID_AA64MMFR0_EL1, &arm_cpu->isar.idregs[ID_AA64MMFR0_EL1_IDX]);
    assert_hvf_ok(ret);

    clamp_id_aa64mmfr0_parange_to_ipa_size(&arm_cpu->isar);
    ret = hv_vcpu_set_sys_reg(cpu->accel->fd, HV_SYS_REG_ID_AA64MMFR0_EL1, arm_cpu->isar.idregs[ID_AA64MMFR0_EL1_IDX]);
    assert_hvf_ok(ret);
...
static void hvf_raise_exception(CPUState* cpu, uint32_t excp, uint32_t syndrome, int target_el)
{
    ARMCPU*      arm_cpu = ARM_CPU(cpu);
    CPUARMState* env     = &arm_cpu->env;
...
{
    return SYSREG_OP0(reg) == 3 && SYSREG_OP1(reg) == 0 && SYSREG_CRN(reg) == 0 && SYSREG_CRM(reg) >= 1
           && SYSREG_CRM(reg) < 8;
}

static uint32_t hvf_reg2cp_reg(uint32_t reg)
{
    return ENCODE_AA64_CP_REG(CP_REG_ARM64_SYSREG_CP, (reg >> SYSREG_CRN_SHIFT) & SYSREG_CRN_MASK,
                              (reg >> SYSREG_CRM_SHIFT) & SYSREG_CRM_MASK, (reg >> SYSREG_OP0_SHIFT) & SYSREG_OP0_MASK,
                              (reg >> SYSREG_OP1_SHIFT) & SYSREG_OP1_MASK, (reg >> SYSREG_OP2_SHIFT) & SYSREG_OP2_MASK);
}

static bool hvf_sysreg_read_cp(CPUState* cpu, const char* cpname, uint32_t reg, uint64_t* val)
{
    ARMCPU*             arm_cpu = ARM_CPU(cpu);
    CPUARMState*        env     = &arm_cpu->env;
    ...
    if (ri) {
        if (!cp_access_ok(1, ri, true)) { return false; }
        if (ri->accessfn) {
            if (ri->accessfn(env, ri, true) != CP_ACCESS_OK) { return false; }
        }
        if (ri->type & ARM_CP_CONST) { *val = ri->resetvalue; }
        else if (ri->readfn) {
...
static bool hvf_sysreg_write_cp(CPUState* cpu, const char* cpname, uint32_t reg, uint64_t val)
{
    ARMCPU*             arm_cpu = ARM_CPU(cpu);
    CPUARMState*        env     = &arm_cpu->env;
    const ARMCPRegInfo* ri;
    ...
    if (ri) {
        if (!cp_access_ok(1, ri, false)) { return false; }
        if (ri->accessfn) {
            if (ri->accessfn(env, ri, false) != CP_ACCESS_OK) { return false; }
        }
        if (ri->writefn) { ri->writefn(env, ri, val); }
        else {
...
static int hvf_sysreg_read(CPUState* cpu, uint32_t reg, uint64_t* val)
{
    ...
    if (arm_feature(env, ARM_FEATURE_PMU)) {
        switch (reg) {
            case SYSREG_PMCR_EL0: *val = env->cp15.c9_pmcr; return 0;
            case SYSREG_PMCCNTR_EL0:
                pmu_op_start(env);
                *val = env->cp15.c15_ccnt;
                pmu_op_finish(env);
                return 0;
            case SYSREG_PMCNTENCLR_EL0: *val = env->cp15.c9_pmcnten; return 0;
            case SYSREG_PMOVSCLR_EL0  : *val = env->cp15.c9_pmovsr; return 0;
            case SYSREG_PMSELR_EL0    : *val = env->cp15.c9_pmselr; return 0;
            case SYSREG_PMINTENCLR_EL1: *val = env->cp15.c9_pminten; return 0;
    ...
    switch (reg) {
        case SYSREG_CNTPCT_EL0:
            *val = muldiv64(qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL), arm_cpu->gt_cntfrq_hz, NANOSECONDS_PER_SECOND);
            return 0;
        case SYSREG_OSLSR_EL1: *val = env->cp15.oslsr_el1; return 0;
        ...
        case SYSREG_DBGBCR15_EL1: *val = env->cp15.dbgbcr[SYSREG_CRM(reg)]; return 0;
        ...
        case SYSREG_DBGWVR15_EL1: *val = env->cp15.dbgwvr[SYSREG_CRM(reg)]; return 0;
        ...
        case SYSREG_DBGWCR15_EL1: *val = env->cp15.dbgwcr[SYSREG_CRM(reg)]; return 0;
        default:
            if (is_id_sysreg(reg)) {
    ...
    trace_hvf_unhandled_sysreg_read(env->pc, reg, SYSREG_OP0(reg), SYSREG_OP1(reg), SYSREG_CRN(reg), SYSREG_CRM(reg),
                                    SYSREG_OP2(reg));
...
static bool pmu_event_supported(uint16_t number) { return false; }
...
static bool pmu_counter_enabled(CPUARMState* env, uint8_t counter)
{
    ...
    enabled = (env->cp15.c9_pmcr & PMCRE) && (env->cp15.c9_pmcnten & (1 << counter));

    if (counter == 31) { filter = env->cp15.pmccfiltr_el0; }
    else {
        filter = env->cp15.c14_pmevtyper[counter];
    }

    if (el == 0) { filtered = filter & PMXEVTYPER_U; }
    else if (el == 1) {
    ...
    if (counter != 31) {
        ...
        if (!pmu_event_supported(event)) { return false; }
    }
...
static int hvf_sysreg_write(CPUState* cpu, uint32_t reg, uint64_t val)
{
    ...
    if (arm_feature(env, ARM_FEATURE_PMU)) {
        switch (reg) {
            ...
            case SYSREG_PMCR_EL0:
                ...
                if (val & PMCRP) {
                    unsigned int i;
                    for (i = 0; i < pmu_num_counters(env); i++) { env->cp15.c14_pmevcntr[i] = 0; }
                }
                ...
                pmu_op_finish(env);
                return 0;
            case SYSREG_PMUSERENR_EL0 : env->cp15.c9_pmuserenr = val & 0xf; return 0;
            case SYSREG_PMCNTENSET_EL0: env->cp15.c9_pmcnten |= (val & pmu_counter_mask(env)); return 0;
            case SYSREG_PMCNTENCLR_EL0: env->cp15.c9_pmcnten &= ~(val & pmu_counter_mask(env)); return 0;
            ...
            case SYSREG_PMSWINC_EL0:
                ...
                pmu_op_finish(env);
                return 0;
            case SYSREG_PMSELR_EL0: env->cp15.c9_pmselr = val & 0x1f; return 0;
    ...
    switch (reg) {
        ...
        case SYSREG_DBGBCR13_EL1:
        case SYSREG_DBGBCR14_EL1:
        case SYSREG_DBGBCR15_EL1: env->cp15.dbgbcr[SYSREG_CRM(reg)] = val; return 0;
        case SYSREG_DBGWVR0_EL1 :
        case SYSREG_DBGWVR1_EL1 :
        case SYSREG_DBGWVR2_EL1 :
        case SYSREG_DBGWVR3_EL1 :
        ...
        case SYSREG_DBGWVR13_EL1:
        case SYSREG_DBGWVR14_EL1:
        case SYSREG_DBGWVR15_EL1: env->cp15.dbgwvr[SYSREG_CRM(reg)] = val; return 0;
        case SYSREG_DBGWCR0_EL1 :
        case SYSREG_DBGWCR1_EL1 :
        case SYSREG_DBGWCR2_EL1 :
        case SYSREG_DBGWCR3_EL1 :
        ...
        case SYSREG_DBGWCR13_EL1:
        case SYSREG_DBGWCR14_EL1:
        case SYSREG_DBGWCR15_EL1: env->cp15.dbgwcr[SYSREG_CRM(reg)] = val; return 0;
        default:
            if (!is_id_sysreg(reg) && hvf_sysreg_write_cp(cpu, "fallback", reg, val)) { return 0; }
            break;
    }
...
static int hvf_inject_interrupts(CPUState* cpu)
{
    if (cpu_test_interrupt(cpu, CPU_INTERRUPT_FIQ)) {
        ...
        hv_vcpu_set_pending_interrupt(cpu->accel->fd, HV_INTERRUPT_TYPE_FIQ, true);
    }

    if (cpu_test_interrupt(cpu, CPU_INTERRUPT_HARD)) {
        trace_hvf_inject_irq();
        hv_vcpu_set_pending_interrupt(cpu->accel->fd, HV_INTERRUPT_TYPE_IRQ, true);
    }
...
static int hvf_handle_exception(CPUState* cpu, hv_vcpu_exit_exception_t* excp)
{
    ...
    switch (ec) {
        case EC_SOFTWARESTEP: {
            ...
            if (!cpu->singlestep_enabled) { error_report("EC_SOFTWARESTEP but single-stepping not enabled"); }
            break;
        }
        case EC_AA64_BKPT: {
            ret = EXCP_DEBUG;
        ...
        case EC_BREAKPOINT: {
            ...
            if (!find_hw_breakpoint(cpu, env->pc)) { error_report("EC_BREAKPOINT but unknown hw breakpoint"); }
            break;
        }
        case EC_WATCHPOINT: {
            ret = EXCP_DEBUG;
            ...
            CPUWatchpoint* wp = find_hw_watchpoint(cpu, excp->virtual_address);
            if (!wp) { error_report("EXCP_DEBUG but unknown hw watchpoint"); }
        ...
        case EC_DATAABORT: {
            ...
            trace_hvf_data_abort(excp->virtual_address, excp->physical_address, isv, iswrite, s1ptw, len, srt);
            ...
            if (isv) {
                if (iswrite) {
                    ...
                    if (address_space_write(as, excp->physical_address, MEMTXATTRS_UNSPECIFIED, &val, len) != MEMTX_OK)
                    {
                    ...
                    if (address_space_read(as, excp->physical_address, MEMTXATTRS_UNSPECIFIED, &val, len) != MEMTX_OK) {
                        fprintf(stderr, "%s: Failed to read MMIO at 0x%llX\n", __func__, excp->physical_address);
                        hvf_raise_exception(cpu, EXCP_DATA_ABORT, syndrome, 1);
                        break;
                    }
                    if (sse) { val = sextract64(val, 0, len * 8); }
                    hvf_set_reg(cpu, srt, val);
                }
            }
            else if (!arm_aarch64_fallback_emu_single(cpu, as, hvf_get_reg, hvf_set_reg)) {
                hvf_raise_exception(cpu, EXCP_DATA_ABORT, syndrome, 1);
        ...
        case EC_SYSTEMREGISTERTRAP: {
            ...
            if (isread) {
                ...
                if (!sysreg_ret) {
                    trace_hvf_sysreg_read(reg, SYSREG_OP0(reg), SYSREG_OP1(reg), SYSREG_CRN(reg), SYSREG_CRM(reg),
                                          SYSREG_OP2(reg), val);
    ...
    if (advance_pc) {
        ...
        if (cpu->singlestep_enabled) { ret = EXCP_DEBUG; }
    }
...
int hvf_arch_remove_sw_breakpoint(CPUState* cpu, struct hvf_sw_breakpoint* bp)
{
    ...
    if (cpu_memory_rw_debug(cpu, bp->pc, (uint8_t*)&brk, 4, 0) || brk != brk_insn
...
{
    switch (type) {
        case GDB_BREAKPOINT_HW    : return insert_hw_breakpoint(addr);
        case GDB_WATCHPOINT_READ  :
        case GDB_WATCHPOINT_WRITE :
        case GDB_WATCHPOINT_ACCESS: return insert_hw_watchpoint(addr, len, type);
...
{
    switch (type) {
        case GDB_BREAKPOINT_HW    : return delete_hw_breakpoint(addr);
        case GDB_WATCHPOINT_READ  :
        case GDB_WATCHPOINT_WRITE :
        case GDB_WATCHPOINT_ACCESS: return delete_hw_watchpoint(addr, len, type);
        default                   : return -ENOSYS;
    }
}
...
{
    if (cur_hw_wps > 0) { g_array_remove_range(hw_watchpoints, 0, cur_hw_wps); }
    if (cur_hw_bps > 0) { g_array_remove_range(hw_breakpoints, 0, cur_hw_bps); }
}
...
static void hvf_put_guest_debug_registers(CPUState* cpu)
{
    ...
    for (i = 0; i < max_hw_bps; i++) {
        r = hv_vcpu_set_sys_reg(cpu->accel->fd, dbgbcr_regs[i], env->cp15.dbgbcr[i]);
        assert_hvf_ok(r);
        r = hv_vcpu_set_sys_reg(cpu->accel->fd, dbgbvr_regs[i], env->cp15.dbgbvr[i]);
        assert_hvf_ok(r);
    }

    for (i = 0; i < max_hw_wps; i++) {
        r = hv_vcpu_set_sys_reg(cpu->accel->fd, dbgwcr_regs[i], env->cp15.dbgwcr[i]);
        assert_hvf_ok(r);
        r = hv_vcpu_set_sys_reg(cpu->accel->fd, dbgwvr_regs[i], env->cp15.dbgwvr[i]);
        assert_hvf_ok(r);
    }
}

static inline bool hvf_arm_hw_debug_active(CPUState* cpu) { return ((cur_hw_wps > 0) || (cur_hw_bps > 0)); }
...
static void hvf_arch_set_traps(CPUState* cpu)
{
    ...
    r = hv_vcpu_set_trap_debug_exceptions(cpu->accel->fd, should_enable_traps);
    assert_hvf_ok(r);

    /* Set whether accesses to debug registers exit the guest */
    r = hv_vcpu_set_trap_debug_reg_accesses(cpu->accel->fd, should_enable_traps);
    assert_hvf_ok(r);
}
...
void hvf_arch_update_guest_debug(CPUState* cpu)
{
    ...
    cpu->accel->guest_debug_enabled =
        cpu->singlestep_enabled || hvf_sw_breakpoints_active(cpu) || hvf_arm_hw_debug_active(cpu);

    /* Update debug registers */
    if (cpu->accel->guest_debug_enabled) { hvf_put_gdbstub_debug_registers(cpu); }
    else {
    ...
    if (cpu->singlestep_enabled) {
        env->cp15.mdscr_el1 = deposit64(env->cp15.mdscr_el1, MDSCR_EL1_SS_SHIFT, 1, 1);
        pstate_write(env, pstate_read(env) | PSTATE_SS);
    }
    else {
        env->cp15.mdscr_el1 = deposit64(env->cp15.mdscr_el1, MDSCR_EL1_SS_SHIFT, 1, 0);
    }

    /* Enable/disable Breakpoint exceptions */
    if (hvf_arm_hw_debug_active(cpu)) {
        env->cp15.mdscr_el1 = deposit64(env->cp15.mdscr_el1, MDSCR_EL1_MDE_SHIFT, 1, 1);
    }
    else {
        env->cp15.mdscr_el1 = deposit64(env->cp15.mdscr_el1, MDSCR_EL1_MDE_SHIFT, 1, 0);
    }

    hvf_arch_set_traps(cpu);
}

bool hvf_arch_supports_guest_debug(void) { return true; }
