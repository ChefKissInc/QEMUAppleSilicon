...
#ifdef CONFIG_TCG
static inline bool arm_excp_unmasked(CPUState* cs, unsigned int excp_idx, unsigned int target_el, unsigned int cur_el,
                                     bool secure, uint64_t hcr_el2)
{
    ...
    if (cpu_isar_feature(aa64_nmi, env_archcpu(env)) && env->cp15.sctlr_el[target_el] & SCTLR_NMI
        && cur_el == target_el)
    {
        allIntMask = env->pstate & PSTATE_ALLINT
                     || ((env->cp15.sctlr_el[target_el] & SCTLR_SPINTMASK) && (env->pstate & PSTATE_SP));
    }
    ...
    switch (excp_idx) {
        ...
        case EXCP_VFNMI:
            ...
            return !allIntMask;
        case EXCP_FIQ: pstate_unmasked = (!(env->daif & PSTATE_F)) && (!allIntMask); break;

        case EXCP_IRQ: pstate_unmasked = (!(env->daif & PSTATE_I)) && (!allIntMask); break;
        ...
        if (arm_feature(env, ARM_FEATURE_AARCH64)) {
            switch (target_el) {
                case 2:
                     ...
                     */
                    if ((hcr_el2 & (HCR_E2H | HCR_TGE)) != (HCR_E2H | HCR_TGE)) { unmasked = true; }
                    break;
                case 3:
            ...
            if ((scr || hcr) && !secure) { unmasked = true; }
        }
    }
...
bool arm_cpu_exec_interrupt(CPUState* cs, int interrupt_request)
{
    ...
    if (cpu_isar_feature(aa64_nmi, env_archcpu(env)) && (arm_sctlr(env, cur_el) & SCTLR_NMI)) {
        if (interrupt_request & CPU_INTERRUPT_NMI) {
            excp_idx  = EXCP_NMI;
            target_el = arm_phys_excp_target_el(cs, excp_idx, cur_el, secure);
            if (arm_excp_unmasked(cs, excp_idx, target_el, cur_el, secure, hcr_el2)) { goto found; }
        }
         ...
         */
        if (interrupt_request & CPU_INTERRUPT_NMI) { interrupt_request |= CPU_INTERRUPT_HARD; }
        if (interrupt_request & CPU_INTERRUPT_VINMI) { interrupt_request |= CPU_INTERRUPT_VIRQ; }
        if (interrupt_request & CPU_INTERRUPT_VFNMI) { interrupt_request |= CPU_INTERRUPT_VFIQ; }
    }
...
void arm_cpu_update_vfnmi(ARMCPU* cpu)
{
    ...
    if (new_state != cpu_test_interrupt(cs, CPU_INTERRUPT_VFNMI)) {
        if (new_state) { cpu_interrupt(cs, CPU_INTERRUPT_VFNMI); }
        else {
...
void arm_cpu_update_vserr(ARMCPU* cpu)
{
    ...
    if (new_state != cpu_test_interrupt(cs, CPU_INTERRUPT_VSERR)) {
        if (new_state) { cpu_interrupt(cs, CPU_INTERRUPT_VSERR); }
        else {
            cpu_reset_interrupt(cs, CPU_INTERRUPT_VSERR);
        }
    }
