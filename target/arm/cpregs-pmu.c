...
static CPAccessResult access_tpm(CPUARMState* env, const ARMCPRegInfo* ri, bool isread)
{
    ...
    if (el < 2 && (mdcr_el2 & MDCR_TPM)) { return CP_ACCESS_TRAP_EL2; }
    if (el < 3 && (env->cp15.mdcr_el3 & MDCR_TPM)) { return CP_ACCESS_TRAP_EL3; }
    return CP_ACCESS_OK;
}
...
static bool event_always_supported(CPUARMState* env) { return true; }
...
{ return muldiv64(qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL), ARM_CPU_FREQ, NANOSECONDS_PER_SECOND); }

static int64_t cycles_ns_per(uint64_t cycles) { return (ARM_CPU_FREQ / NANOSECONDS_PER_SECOND) * cycles; }
...
void pmu_init(ARMCPU* cpu)
{
     ...
     * events to them
     */
    for (i = 0; i < ARRAY_SIZE(supported_event_map); i++) { supported_event_map[i] = UNSUPPORTED_EVENT; }
    cpu->pmceid0 = 0;
    cpu->pmceid1 = 0;
    ...
    for (i = 0; i < ARRAY_SIZE(pm_events); i++) {
        ...
        if (cnt->supported(&cpu->env)) {
            ...
            if (cnt->number & 0x20) { cpu->pmceid1 |= event_mask; }
            else {
...
{
    if (number > MAX_EVENT_ID) { return false; }
...
static CPAccessResult do_pmreg_access(CPUARMState* env, bool is_pmcr)
{
    ...
    if (el == 0 && !(env->cp15.c9_pmuserenr & 1)) { return CP_ACCESS_TRAP_EL1; }
    if (el < 2) {
        uint64_t mdcr_el2 = arm_mdcr_el2_eff(env);

        if (mdcr_el2 & MDCR_TPM) { return CP_ACCESS_TRAP_EL2; }
        if (is_pmcr && (mdcr_el2 & MDCR_TPMCR)) { return CP_ACCESS_TRAP_EL2; }
    }
    if (el < 3 && (env->cp15.mdcr_el3 & MDCR_TPM)) { return CP_ACCESS_TRAP_EL3; }
...
static CPAccessResult pmreg_access(CPUARMState* env, const ARMCPRegInfo* ri, bool isread)
{ return do_pmreg_access(env, false); }

static CPAccessResult pmreg_access_pmcr(CPUARMState* env, const ARMCPRegInfo* ri, bool isread)
{ return do_pmreg_access(env, true); }

static CPAccessResult pmreg_access_xevcntr(CPUARMState* env, const ARMCPRegInfo* ri, bool isread)
{
    /* ER: event counter read trap control */
    if (arm_feature(env, ARM_FEATURE_V8) && arm_current_el(env) == 0 && (env->cp15.c9_pmuserenr & (1 << 3)) != 0
        && isread)
    {
        return CP_ACCESS_OK;
    }
...
static CPAccessResult pmreg_access_swinc(CPUARMState* env, const ARMCPRegInfo* ri, bool isread)
{
    /* SW: software increment write trap control */
    if (arm_feature(env, ARM_FEATURE_V8) && arm_current_el(env) == 0 && (env->cp15.c9_pmuserenr & (1 << 1)) != 0
        && !isread)
    {
        return CP_ACCESS_OK;
    }
...
static CPAccessResult pmreg_access_selr(CPUARMState* env, const ARMCPRegInfo* ri, bool isread)
{
    /* ER: event counter read trap control */
    if (arm_feature(env, ARM_FEATURE_V8) && arm_current_el(env) == 0 && (env->cp15.c9_pmuserenr & (1 << 3)) != 0) {
        return CP_ACCESS_OK;
...
static CPAccessResult pmreg_access_ccntr(CPUARMState* env, const ARMCPRegInfo* ri, bool isread)
{
    /* CR: cycle counter read trap control */
    if (arm_feature(env, ARM_FEATURE_V8) && arm_current_el(env) == 0 && (env->cp15.c9_pmuserenr & (1 << 2)) != 0
        && isread)
    {
        return CP_ACCESS_OK;
    }
...
static bool pmu_counter_enabled(CPUARMState* env, uint8_t counter)
{
    ...
    if (!arm_feature(env, ARM_FEATURE_EL2) || (counter < hpmn || counter == 31)) { e = env->cp15.c9_pmcr & PMCRE; }
    else {
    ...
    if (el == 2 && (counter < hpmn || counter == 31)) { prohibited = mdcr_el2 & MDCR_HPMD; }
    if (secure) { prohibited = prohibited || !(env->cp15.mdcr_el3 & MDCR_SPME); }
    ...
    if (counter == 31) {
        ...
        if (cpu_isar_feature(any_pmuv3p5, env_archcpu(env))) {
            if (secure) { prohibited = prohibited || (env->cp15.mdcr_el3 & MDCR_SCCD); }
            if (el == 2) { prohibited = prohibited || (mdcr_el2 & MDCR_HCCD); }
        }
    }
    ...
    m   = arm_el_is_aa64(env, 1) && arm_feature(env, ARM_FEATURE_EL3) && (filter & PMXEVTYPER_M);

    if (el == 0) { filtered = secure ? u : u != nsu; }
    else if (el == 1) {
    ...
    if (counter != 31) {
        ...
        if (!event_supported(event)) { return false; }
    }
...
static bool pmevcntr_is_64_bit(CPUARMState* env, int counter)
{
    ...
    if (!cpu_isar_feature(any_pmuv3p5, env_archcpu(env))) { return false; }
...
static void pmccntr_op_start(CPUARMState* env)
{
    ...
    if (pmu_counter_enabled(env, 31)) {
        ...
        if (pmccntr_clockdiv_enabled(env)) { eff_cycles /= 64; }

        uint64_t new_pmccntr = eff_cycles - env->cp15.c15_ccnt_delta;

        uint64_t overflow_mask = env->cp15.c9_pmcr & PMCRLC ? 1ull << 63 : 1ull << 31;
...
static void pmccntr_op_finish(CPUARMState* env)
{
    if (pmu_counter_enabled(env, 31)) {
        ...
        if (!(env->cp15.c9_pmcr & PMCRLC)) { remaining_cycles = (uint32_t)remaining_cycles; }
        int64_t overflow_in = cycles_ns_per(remaining_cycles);

        if (overflow_in > 0) {
            int64_t overflow_at;

            if (!sadd64_overflow(qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL), overflow_in, &overflow_at)) {
                ARMCPU* cpu = env_archcpu(env);
...
static void pmevcntr_op_start(CPUARMState* env, uint8_t counter)
{
    ...
    if (pmu_counter_enabled(env, counter)) {
        ...
        uint64_t overflow_mask = pmevcntr_is_64_bit(env, counter) ? 1ULL << 63 : 1ULL << 31;
...
static void pmevcntr_op_finish(CPUARMState* env, uint8_t counter)
{
    if (pmu_counter_enabled(env, counter)) {
        ...
        if (!pmevcntr_is_64_bit(env, counter)) { delta = (uint32_t)delta; }
...
void pmu_op_start(CPUARMState* env)
{
    ...
    for (i = 0; i < pmu_num_counters(env); i++) { pmevcntr_op_start(env, i); }
}
...
void pmu_op_finish(CPUARMState* env)
{
    ...
    for (i = 0; i < pmu_num_counters(env); i++) { pmevcntr_op_finish(env, i); }
}

void pmu_pre_el_change(ARMCPU* cpu, void* ignored) { pmu_op_start(&cpu->env); }

void pmu_post_el_change(ARMCPU* cpu, void* ignored) { pmu_op_finish(&cpu->env); }
...
static void pmselr_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    /*
...
static void pmccntr_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    pmccntr_op_start(env);
...
static void pmccntr_write32(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
...
static void pmccfiltr_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    pmccntr_op_start(env);
...
static void pmccfiltr_write_a32(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    pmccntr_op_start(env);
    /* M is not accessible from AArch32 */
    env->cp15.pmccfiltr_el0 = (env->cp15.pmccfiltr_el0 & PMCCFILTR_M) | (value & PMCCFILTR);
    pmccntr_op_finish(env);
}
...
static void pmcntenset_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    pmu_op_start(env);
...
static void pmcntenclr_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    pmu_op_start(env);
    value                &= pmu_counter_mask(env);
    env->cp15.c9_pmcnten &= ~value;
...
static void pmevtyper_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value, const uint8_t counter)
{
    if (counter == 31) { pmccfiltr_write(env, ri, value); }
...
static uint64_t pmevtyper_read(CPUARMState* env, const ARMCPRegInfo* ri, const uint8_t counter)
{
    if (counter == 31) { return env->cp15.pmccfiltr_el0; }
    else if (counter < pmu_num_counters(env)) {
...
static void pmevtyper_writefn(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    uint8_t counter = ((ri->crm & 3) << 3) | (ri->opc2 & 7);
    pmevtyper_write(env, ri, value, counter);
}

static void pmevtyper_rawwrite(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    uint8_t counter                  = ((ri->crm & 3) << 3) | (ri->opc2 & 7);
...
static void pmxevtyper_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{ pmevtyper_write(env, ri, value, env->cp15.c9_pmselr & 31); }

static uint64_t pmxevtyper_read(CPUARMState* env, const ARMCPRegInfo* ri)
{ return pmevtyper_read(env, ri, env->cp15.c9_pmselr & 31); }

static void pmevcntr_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value, uint8_t counter)
{
    if (!cpu_isar_feature(any_pmuv3p5, env_archcpu(env))) {
...
static uint64_t pmevcntr_read(CPUARMState* env, const ARMCPRegInfo* ri, uint8_t counter)
{
    if (counter < pmu_num_counters(env)) {
        uint64_t ret;
        pmevcntr_op_start(env, counter);
...
static void pmevcntr_writefn(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    uint8_t counter = ((ri->crm & 3) << 3) | (ri->opc2 & 7);
...
static void pmevcntr_rawwrite(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    uint8_t counter = ((ri->crm & 3) << 3) | (ri->opc2 & 7);
...
static void pmxevcntr_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{ pmevcntr_write(env, ri, value, env->cp15.c9_pmselr & 31); }

static uint64_t pmxevcntr_read(CPUARMState* env, const ARMCPRegInfo* ri)
{ return pmevcntr_read(env, ri, env->cp15.c9_pmselr & 31); }

static void pmuserenr_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    if (arm_feature(env, ARM_FEATURE_V8)) { env->cp15.c9_pmuserenr = value & 0xf; }
    else {
...
void define_pm_cpregs(ARMCPU* cpu)
{
    ...
    if (arm_feature(env, ARM_FEATURE_V7)) {
        ...
        if (arm_feature(env, ARM_FEATURE_V8)) { pmccntr.type |= ARM_CP_NO_GDB; }
    ...
    if (arm_feature(env, ARM_FEATURE_V7VE)) { define_arm_cp_regs(cpu, pmovsset_cp_reginfo); }
