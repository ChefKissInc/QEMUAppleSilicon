...
static bool sme2_zt0_enabled_check(DisasContext* s)
{
    ...
    if (s->zt0_excp_el) {
        gen_exception_insn_el(s, 0, EXCP_UDEF, syn_smetrap(SME_ET_InaccessibleZT0, false), s->zt0_excp_el);
        return false;
    }
    return true;
}
...
static TCGv_ptr get_tile_rowcol(DisasContext* s, int esz, int rs, int tile, int imm, int div_len, int vec_mod,
                                bool vertical)
{
    int      pos, len, offset;
    TCGv_i32 tmp;
     ...
     */
    if (vec_mod > 1) { tcg_gen_andc_i32(tmp, tmp, tcg_constant_i32(vec_mod - 1)); }
    ...
    if (!len) {
         ...
         */
        if (HOST_BIG_ENDIAN && esz < MO_64) { tcg_gen_xori_i32(tmp, tmp, 8 - (1 << esz)); }
    }
    else {
        /*
         * Compute the byte offset of the index within the tile:
...
static TCGv_ptr get_zarray(DisasContext* s, int rs, int imm, int div_len, int vec_mod)
{
...
static bool trans_ZERO(DisasContext* s, arg_ZERO* a)
{
    ...
    if (sme_za_enabled_check(s)) {
        gen_helper_sme_zero(tcg_env, tcg_constant_i32(a->imm), tcg_constant_i32(streaming_vec_reg_size(s)));
    }
    return true;
}
...
{
    if (!dc_isar_feature(aa64_sme2, s)) { return false; }
    if (sme_enabled_check(s) && sme2_zt0_enabled_check(s)) {
        tcg_gen_gvec_dup_imm(MO_64, offsetof(CPUARMState, za_state.zt0), sizeof_field(CPUARMState, za_state.zt0),
...
{
    if (!dc_isar_feature(aa64_sme2p1, s)) { return false; }
...
{
    static gen_helper_gvec_4* const h_fns[5]  = {gen_helper_sve_sel_zpzz_b, gen_helper_sve_sel_zpzz_h,
                                                 gen_helper_sve_sel_zpzz_s, gen_helper_sve_sel_zpzz_d,
                                                 gen_helper_sve_sel_zpzz_q};
    static gen_helper_gvec_3* const cz_fns[5] = {
        gen_helper_sme_mova_cz_b, gen_helper_sme_mova_cz_h, gen_helper_sme_mova_cz_s,
        gen_helper_sme_mova_cz_d, gen_helper_sme_mova_cz_q,
    };
    static gen_helper_gvec_3* const zc_fns[5] = {
        gen_helper_sme_mova_zc_b, gen_helper_sme_mova_zc_h, gen_helper_sme_mova_zc_s,
        gen_helper_sme_mova_zc_d, gen_helper_sme_mova_zc_q,
    };
    ...
    if (!sme_smza_enabled_check(s)) { return true; }
    ...
    if (a->v) {
        ...
        if (to_vec) { zc_fns[a->esz](t_zr, t_za, t_pg, t_desc); }
        else {
        ...
        if (to_vec) { h_fns[a->esz](t_zr, t_za, t_zr, t_pg, t_desc); }
        else {
...
static bool do_mova_tile_n(DisasContext* s, arg_mova_t* a, int n, bool to_vec, bool zero)
{
    static gen_helper_gvec_2* const cz_fns[] = {
        gen_helper_sme2_mova_cz_b,
        gen_helper_sme2_mova_cz_h,
        gen_helper_sme2_mova_cz_s,
        gen_helper_sme2_mova_cz_d,
    };
    static gen_helper_gvec_2* const zc_fns[] = {
        gen_helper_sme2_mova_zc_b,
        gen_helper_sme2_mova_zc_h,
        gen_helper_sme2_mova_zc_s,
        gen_helper_sme2_mova_zc_d,
    };
    static gen_helper_gvec_2* const zc_z_fns[] = {
        gen_helper_sme2p1_movaz_zc_b, gen_helper_sme2p1_movaz_zc_h, gen_helper_sme2p1_movaz_zc_s,
        gen_helper_sme2p1_movaz_zc_d, gen_helper_sme2p1_movaz_zc_q,
    };
    TCGv_ptr t_za;
    ...
    if (a->v) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            t_za          = get_tile_rowcol(s, a->esz, a->rs, a->za, a->off * n + i, 1, n, a->v);
            if (zero) { zc_z_fns[a->esz](t_zr, t_za, t_desc); }
            else if (to_vec) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (to_vec) {
                ...
                if (zero) { tcg_gen_gvec_dup_imm_var(MO_8, t_za, 0, svl, svl, 0); }
            }
            else {
...
static bool do_mova_array_n(DisasContext* s, arg_mova_a* a, int n, bool to_vec, bool zero)
{
    TCGv_ptr t_za;
    int      svl;
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (to_vec) {
            ...
            if (zero) { tcg_gen_gvec_dup_imm_var(MO_8, t_za, o_za, svl, svl, 0); }
        }
        else {
...
static bool do_movt(DisasContext* s, arg_MOVT_rzt* a, void (*func)(TCGv_i64, TCGv_ptr, tcg_target_long))
{
    if (sme2_zt0_enabled_check(s)) {
        func(cpu_reg(s, a->rt), tcg_env, offsetof(CPUARMState, za_state.zt0) + a->off * 8);
    }
    return true;
}
...
#define FN_END(L, B)       \
    ...
    static GenLdSt1* const fns[5][2][2][2][2] = {
        FN_END(b, b), FN_END(h_le, h_be), FN_END(s_le, s_be), FN_END(d_le, d_be), FN_END(q_le, q_be),
    };
...
#undef FN_END
    ...
    fns[a->esz][be][a->v][mte][a->st](tcg_env, t_za, t_pg, addr, tcg_constant_i64(desc));
    return true;
}
...
static bool do_ldst_r(DisasContext* s, arg_ldstr* a, GenLdStR* fn)
{
    if (sme_za_enabled_check(s)) {
        ...
        fn(s, base, 0, svl, a->rn, imm * svl, s->align_mem ? MO_ALIGN_16 : MO_UNALN);
    }
    return true;
}
...
{
    if (sme2_zt0_enabled_check(s)) {
        fn(s, tcg_env, offsetof(CPUARMState, za_state.zt0), sizeof_field(CPUARMState, za_state.zt0), a->rn, 0,
...
static bool do_adda(DisasContext* s, arg_adda* a, MemOp esz, gen_helper_gvec_4* fn)
{
    int      svl  = streaming_vec_reg_size(s);
...
static bool do_outprod(DisasContext* s, arg_op* a, MemOp esz, gen_helper_gvec_5* fn)
{
    int      svl  = streaming_vec_reg_size(s);
...
static bool do_outprod_fpst(DisasContext* s, arg_op* a, MemOp esz, ARMFPStatusFlavour e_fpst, gen_helper_gvec_5_ptr* fn)
{
    int      svl  = streaming_vec_reg_size(s);
    uint32_t desc = simd_desc(svl, svl, 0);
...
static bool do_outprod_env(DisasContext* s, arg_op* a, MemOp esz, gen_helper_gvec_5_ptr* fn)
{
    int      svl  = streaming_vec_reg_size(s);
    uint32_t desc = simd_desc(svl, svl, 0);
    TCGv_ptr za, zn, zm, pn, pm;
...
static bool do_z2z_n1(DisasContext* s, arg_z2z_en* a, GVecGen3Fn* fn)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (dofs == mofs) { overlap = true; }
        else {
            fn(esz, dofs, dofs, mofs, vsz, vsz);
        }
    }
    if (overlap) { fn(esz, mofs, mofs, mofs, vsz, vsz); }
    return true;
}

static void gen_sme2_srshl(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz,
                           uint32_t max_sz)
{
    static gen_helper_gvec_3* const fns[] = {
        gen_helper_gvec_srshl_b,
        gen_helper_sme2_srshl_h,
        gen_helper_sme2_srshl_s,
        gen_helper_sme2_srshl_d,
    };
    tcg_debug_assert(vece <= MO_64);
    tcg_gen_gvec_3_ool(rd_ofs, rn_ofs, rm_ofs, opr_sz, max_sz, 0, fns[vece]);
}

static void gen_sme2_urshl(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz,
                           uint32_t max_sz)
{
    static gen_helper_gvec_3* const fns[] = {
        gen_helper_gvec_urshl_b,
        gen_helper_sme2_urshl_h,
        gen_helper_sme2_urshl_s,
        gen_helper_sme2_urshl_d,
    };
    tcg_debug_assert(vece <= MO_64);
    tcg_gen_gvec_3_ool(rd_ofs, rn_ofs, rm_ofs, opr_sz, max_sz, 0, fns[vece]);
}
...
static bool do_z2z_n1_fpst(DisasContext* s, arg_z2z_en* a, gen_helper_gvec_3_ptr* const fns[4])
{
    ...
    if (esz == MO_8 && !dc_isar_feature(aa64_sme_b16b16, s)) { return false; }
    if (!sme_sm_enabled_check(s)) { return true; }
    ...
    if (overlap) { tcg_gen_gvec_3_ptr(mofs, mofs, mofs, fpst, vsz, vsz, 0, fn); }
    return true;
}

static bool do_z2z_nn_fpst(DisasContext* s, arg_z2z_en* a, gen_helper_gvec_3_ptr* const fns[4])
{
...
static gen_helper_gvec_3_ptr* const f_vector_fmax[2][4] = {
    {gen_helper_gvec_fmax_b16, gen_helper_gvec_fmax_h, gen_helper_gvec_fmax_s, gen_helper_gvec_fmax_d},
    {gen_helper_gvec_ah_fmax_b16, gen_helper_gvec_ah_fmax_h, gen_helper_gvec_ah_fmax_s, gen_helper_gvec_ah_fmax_d},
};
...
static gen_helper_gvec_3_ptr* const f_vector_fmin[2][4] = {
    {gen_helper_gvec_fmin_b16, gen_helper_gvec_fmin_h, gen_helper_gvec_fmin_s, gen_helper_gvec_fmin_d},
    {gen_helper_gvec_ah_fmin_b16, gen_helper_gvec_ah_fmin_h, gen_helper_gvec_ah_fmin_s, gen_helper_gvec_ah_fmin_d},
};
...
static bool do_azz_n1(DisasContext* s, arg_azz_n* a, int esz, GVecGen3FnVar* fn)
{
    TCGv_ptr t_za;
    int      svl, n, o_zm;
...
static bool do_azz_nn(DisasContext* s, arg_azz_n* a, int esz, GVecGen3FnVar* fn)
{
    TCGv_ptr t_za;
    int      svl, n;
...
static bool do_azz_fp(DisasContext* s, int nreg, int nsel, int rv, int off, int zn, int zm, int data, int shsel,
                      bool multi, int fpst, gen_helper_gvec_3_ptr* fn)
{
    if (sme_smza_enabled_check(s)) {
        int      svl     = streaming_vec_reg_size(s);
        ...
        if (fpst >= 0) { ptr = fpstatus_ptr(fpst); }
        else {
            ptr = tcg_env;
        }
        t = tcg_temp_new_ptr();
...
static bool do_azz_acc_fp(DisasContext* s, int nreg, int nsel, int rv, int off, int zn, int zm, int data, int shsel,
                          bool multi, int fpst, gen_helper_gvec_4_ptr* fn)
{
    if (sme_smza_enabled_check(s)) {
        int      svl     = streaming_vec_reg_size(s);
        int      vstride = svl / nreg;
...
{
    return do_azz_acc_fp(s, a->n, 2, a->rv, a->off, a->zn, a->zm, (1 << 2) | sub, 1, multi, FPST_ENV,
...
{
    return do_azz_acc_fp(s, a->n, 2, a->rv, a->off, a->zn, a->zm, (a->idx << 3) | (1 << 2) | sub, 1, false, FPST_ENV,
...
{
    return do_azz_acc_fp(s, a->n, 2, a->rv, a->off, a->zn, a->zm, 0, 0, multi, FPST_ZA,
...
{
    return do_azz_acc_fp(s, a->n, 2, a->rv, a->off, a->zn, a->zm, a->idx << 1, 0, false, FPST_ZA,
...
{ return do_azz_acc_fp(s, a->n, 1, a->rv, a->off, a->zn, a->zm, 1, 0, multi, FPST_ENV, gen_helper_sme2_fdot_h); }
...
{
    return do_azz_acc_fp(s, a->n, 1, a->rv, a->off, a->zn, a->zm, a->idx | (1 << 2), 0, false, FPST_ENV,
...
{ return do_azz_acc_fp(s, a->n, 1, a->rv, a->off, a->zn, a->zm, 0, 0, multi, FPST_ENV, gen_helper_gvec_bfdot); }
...
{
    return do_azz_acc_fp(s, a->n, 1, a->rv, a->off, a->zn, a->zm, a->idx, 0, false, FPST_ENV,
...
static bool do_fmla(DisasContext* s, arg_azz_n* a, bool multi, ARMFPStatusFlavour fpst, gen_helper_gvec_3_ptr* fn)
{ return do_azz_fp(s, a->n, 1, a->rv, a->off, a->zn, a->zm, 0, 0, multi, fpst, fn); }

TRANS_FEAT(FMLA_n1_h, aa64_sme_f16f16, do_fmla, a, false, FPST_ZA_F16, gen_helper_gvec_vfma_h)
...
TRANS_FEAT(FMLA_nn_h, aa64_sme_f16f16, do_fmla, a, true, FPST_ZA_F16, gen_helper_gvec_vfma_h)
TRANS_FEAT(FMLS_nn_h, aa64_sme_f16f16, do_fmla, a, true, FPST_ZA_F16,
           s->fpcr_ah ? gen_helper_gvec_ah_vfms_h : gen_helper_gvec_vfms_h)

TRANS_FEAT(FMLA_n1_s, aa64_sme2, do_fmla, a, false, FPST_ZA, gen_helper_gvec_vfma_s)
TRANS_FEAT(FMLS_n1_s, aa64_sme2, do_fmla, a, false, FPST_ZA,
           s->fpcr_ah ? gen_helper_gvec_ah_vfms_s : gen_helper_gvec_vfms_s)
TRANS_FEAT(FMLA_nn_s, aa64_sme2, do_fmla, a, true, FPST_ZA, gen_helper_gvec_vfma_s)
TRANS_FEAT(FMLS_nn_s, aa64_sme2, do_fmla, a, true, FPST_ZA,
           s->fpcr_ah ? gen_helper_gvec_ah_vfms_s : gen_helper_gvec_vfms_s)

TRANS_FEAT(FMLA_n1_d, aa64_sme2_f64f64, do_fmla, a, false, FPST_ZA, gen_helper_gvec_vfma_d)
TRANS_FEAT(FMLS_n1_d, aa64_sme2_f64f64, do_fmla, a, false, FPST_ZA,
           s->fpcr_ah ? gen_helper_gvec_ah_vfms_d : gen_helper_gvec_vfms_d)
TRANS_FEAT(FMLA_nn_d, aa64_sme2_f64f64, do_fmla, a, true, FPST_ZA, gen_helper_gvec_vfma_d)
TRANS_FEAT(FMLS_nn_d, aa64_sme2_f64f64, do_fmla, a, true, FPST_ZA,
           s->fpcr_ah ? gen_helper_gvec_ah_vfms_d : gen_helper_gvec_vfms_d)

TRANS_FEAT(BFMLA_n1, aa64_sme_b16b16, do_fmla, a, false, FPST_ZA, gen_helper_gvec_bfmla)
TRANS_FEAT(BFMLS_n1, aa64_sme_b16b16, do_fmla, a, false, FPST_ZA,
           s->fpcr_ah ? gen_helper_gvec_ah_bfmls : gen_helper_gvec_bfmls)
TRANS_FEAT(BFMLA_nn, aa64_sme_b16b16, do_fmla, a, true, FPST_ZA, gen_helper_gvec_bfmla)
TRANS_FEAT(BFMLS_nn, aa64_sme_b16b16, do_fmla, a, true, FPST_ZA,
           s->fpcr_ah ? gen_helper_gvec_ah_bfmls : gen_helper_gvec_bfmls)

static bool do_fmla_nx(DisasContext* s, arg_azx_n* a, ARMFPStatusFlavour fpst, gen_helper_gvec_4_ptr* fn)
{ return do_azz_acc_fp(s, a->n, 1, a->rv, a->off, a->zn, a->zm, a->idx, 0, false, fpst, fn); }

TRANS_FEAT(FMLA_nx_h, aa64_sme_f16f16, do_fmla_nx, a, FPST_ZA_F16, gen_helper_gvec_fmla_idx_h)
...
TRANS_FEAT(FMLA_nx_s, aa64_sme2, do_fmla_nx, a, FPST_ZA, gen_helper_gvec_fmla_idx_s)
TRANS_FEAT(FMLS_nx_s, aa64_sme2, do_fmla_nx, a, FPST_ZA,
           s->fpcr_ah ? gen_helper_gvec_ah_fmls_idx_s : gen_helper_gvec_fmls_idx_s)
TRANS_FEAT(FMLA_nx_d, aa64_sme2_f64f64, do_fmla_nx, a, FPST_ZA, gen_helper_gvec_fmla_idx_d)
...
TRANS_FEAT(BFMLA_nx, aa64_sme_b16b16, do_fmla_nx, a, FPST_ZA, gen_helper_gvec_bfmla_idx)
TRANS_FEAT(BFMLS_nx, aa64_sme_b16b16, do_fmla_nx, a, FPST_ZA,
           s->fpcr_ah ? gen_helper_gvec_ah_bfmls_idx : gen_helper_gvec_bfmls_idx)

static bool do_faddsub(DisasContext* s, arg_az_n* a, ARMFPStatusFlavour fpst, gen_helper_gvec_3_ptr* fn)
{
    if (sme_smza_enabled_check(s)) {
        int      svl     = streaming_vec_reg_size(s);
        int      n       = a->n;
...
TRANS_FEAT(FADD_nn_h, aa64_sme_f16f16, do_faddsub, a, FPST_ZA_F16, gen_helper_gvec_fadd_h)
TRANS_FEAT(FSUB_nn_h, aa64_sme_f16f16, do_faddsub, a, FPST_ZA_F16, gen_helper_gvec_fsub_h)

TRANS_FEAT(FADD_nn_s, aa64_sme2, do_faddsub, a, FPST_ZA, gen_helper_gvec_fadd_s)
TRANS_FEAT(FSUB_nn_s, aa64_sme2, do_faddsub, a, FPST_ZA, gen_helper_gvec_fsub_s)

TRANS_FEAT(FADD_nn_d, aa64_sme2_f64f64, do_faddsub, a, FPST_ZA, gen_helper_gvec_fadd_d)
TRANS_FEAT(FSUB_nn_d, aa64_sme2_f64f64, do_faddsub, a, FPST_ZA, gen_helper_gvec_fsub_d)

TRANS_FEAT(BFADD_nn, aa64_sme_b16b16, do_faddsub, a, FPST_ZA, gen_helper_gvec_bfadd)
TRANS_FEAT(BFSUB_nn, aa64_sme_b16b16, do_faddsub, a, FPST_ZA, gen_helper_gvec_bfsub)
 ...
 *   data: stuff for simd_data, including any index.
 */
static bool do_azz_acc(DisasContext* s, int nreg, int nsel, int rv, int off, int zn, int zm, int data, int shsel,
                       bool multi, gen_helper_gvec_4* fn)
{
    if (sme_smza_enabled_check(s)) {
        int      svl     = streaming_vec_reg_size(s);
        int      vstride = svl / nreg;
...
static bool do_dot(DisasContext* s, arg_azz_n* a, bool multi, gen_helper_gvec_4* fn)
{ return do_azz_acc(s, a->n, 1, a->rv, a->off, a->zn, a->zm, 0, 0, multi, fn); }

static void gen_helper_gvec_sudot_4b(TCGv_ptr d, TCGv_ptr n, TCGv_ptr m, TCGv_ptr a, TCGv_i32 desc)
...
{ return do_azz_acc(s, a->n, 1, a->rv, a->off, a->zn, a->zm, a->idx, 0, false, fn); }
...
static bool do_vdot_nx(DisasContext* s, arg_azx_n* a, gen_helper_gvec_3* fn)
{
    if (sme_smza_enabled_check(s)) {
        int svl = streaming_vec_reg_size(s);
        fn(get_zarray(s, a->rv, a->off, a->n, 0), vec_full_reg_ptr(s, a->zn), vec_full_reg_ptr(s, a->zm),
...
static bool do_smlal(DisasContext* s, arg_azz_n* a, bool multi, gen_helper_gvec_4* fn)
{ return do_azz_acc(s, a->n, 2, a->rv, a->off, a->zn, a->zm, 0, 0, multi, fn); }
...
static bool do_smlal_nx(DisasContext* s, arg_azx_n* a, gen_helper_gvec_4* fn)
{ return do_azz_acc(s, a->n, 2, a->rv, a->off, a->zn, a->zm, a->idx << 1, 0, false, fn); }
...
static bool do_smlall(DisasContext* s, arg_azz_n* a, bool multi, gen_helper_gvec_4* fn)
{ return do_azz_acc(s, a->n, 4, a->rv, a->off, a->zn, a->zm, 0, 0, multi, fn); }

static void gen_helper_sme2_sumlall_s(TCGv_ptr d, TCGv_ptr n, TCGv_ptr m, TCGv_ptr a, TCGv_i32 desc)
...
static bool do_smlall_nx(DisasContext* s, arg_azx_n* a, gen_helper_gvec_4* fn)
{ return do_azz_acc(s, a->n, 4, a->rv, a->off, a->zn, a->zm, a->idx << 2, 0, false, fn); }
...
static bool do_zz_fpst(DisasContext* s, arg_zz_n* a, int data, ARMFPStatusFlavour type, gen_helper_gvec_2_ptr* fn)
{
    if (sme_sm_enabled_check(s)) {
        ...
        for (int i = 0, n = a->n; i < n; ++i) {
            tcg_gen_gvec_2_ptr(vec_full_reg_offset(s, a->zd + i), vec_full_reg_offset(s, a->zn + i), fpst, svl, svl,
                               data, fn);
        }
    }
    return true;
...
TRANS_FEAT(BFCVT, aa64_sme2, do_zz_fpst, a, 0, FPST_A64, gen_helper_sme2_bfcvt)
TRANS_FEAT(BFCVTN, aa64_sme2, do_zz_fpst, a, 0, FPST_A64, gen_helper_sme2_bfcvtn)
TRANS_FEAT(FCVT_n, aa64_sme2, do_zz_fpst, a, 0, FPST_A64, gen_helper_sme2_fcvt_n)
TRANS_FEAT(FCVTN, aa64_sme2, do_zz_fpst, a, 0, FPST_A64, gen_helper_sme2_fcvtn)

TRANS_FEAT(FCVT_w, aa64_sme_f16f16, do_zz_fpst, a, 0, FPST_A64_F16, gen_helper_sme2_fcvt_w)
TRANS_FEAT(FCVTL, aa64_sme_f16f16, do_zz_fpst, a, 0, FPST_A64_F16, gen_helper_sme2_fcvtl)

TRANS_FEAT(FCVTZS, aa64_sme2, do_zz_fpst, a, 0, FPST_A64, gen_helper_gvec_vcvt_rz_fs)
TRANS_FEAT(FCVTZU, aa64_sme2, do_zz_fpst, a, 0, FPST_A64, gen_helper_gvec_vcvt_rz_fu)

TRANS_FEAT(SCVTF, aa64_sme2, do_zz_fpst, a, 0, FPST_A64, gen_helper_sme2_scvtf)
TRANS_FEAT(UCVTF, aa64_sme2, do_zz_fpst, a, 0, FPST_A64, gen_helper_sme2_ucvtf)

TRANS_FEAT(FRINTN, aa64_sme2, do_zz_fpst, a, float_round_nearest_even, FPST_A64, gen_helper_gvec_vrint_rm_s)
TRANS_FEAT(FRINTP, aa64_sme2, do_zz_fpst, a, float_round_up, FPST_A64, gen_helper_gvec_vrint_rm_s)
TRANS_FEAT(FRINTM, aa64_sme2, do_zz_fpst, a, float_round_down, FPST_A64, gen_helper_gvec_vrint_rm_s)
TRANS_FEAT(FRINTA, aa64_sme2, do_zz_fpst, a, float_round_ties_away, FPST_A64, gen_helper_gvec_vrint_rm_s)

static bool do_zz(DisasContext* s, arg_zz_n* a, int data, gen_helper_gvec_2* fn)
{
    if (sme_sm_enabled_check(s)) {
        int svl = streaming_vec_reg_size(s);

        for (int i = 0, n = a->n; i < n; ++i) {
            tcg_gen_gvec_2_ool(vec_full_reg_offset(s, a->zd + i), vec_full_reg_offset(s, a->zn + i), svl, svl, data,
                               fn);
        }
    }
    return true;
...
static bool do_zipuzp_4(DisasContext* s, arg_zz_e* a, gen_helper_gvec_2* const fn[5])
{
    int bytes_per_op = 4 << a->esz;

    /* Both MO_64 and MO_128 can fail the size test. */
    if (s->max_svl < bytes_per_op) { unallocated_encoding(s); }
    else if (sme_sm_enabled_check(s)) {
        int svl = streaming_vec_reg_size(s);
        if (svl < bytes_per_op) { unallocated_encoding(s); }
        else {
            tcg_gen_gvec_2_ool(vec_full_reg_offset(s, a->zd), vec_full_reg_offset(s, a->zn), svl, svl, 0, fn[a->esz]);
        }
    }
    return true;
}
...
static gen_helper_gvec_2* const zip4_fns[] = {
    gen_helper_sme2_zip4_b, gen_helper_sme2_zip4_h, gen_helper_sme2_zip4_s,
    gen_helper_sme2_zip4_d, gen_helper_sme2_zip4_q,
};
TRANS_FEAT(ZIP_4, aa64_sme2, do_zipuzp_4, a, zip4_fns)

static gen_helper_gvec_2* const uzp4_fns[] = {
    gen_helper_sme2_uzp4_b, gen_helper_sme2_uzp4_h, gen_helper_sme2_uzp4_s,
    gen_helper_sme2_uzp4_d, gen_helper_sme2_uzp4_q,
};
...
static bool do_zz_rshr(DisasContext* s, arg_rshr* a, gen_helper_gvec_2* fn)
{
    if (sve_access_check(s)) {
        ...
        tcg_gen_gvec_2_ool(vec_full_reg_offset(s, a->zd), vec_full_reg_offset(s, a->zn), vl, vl, a->shift, fn);
    }
    return true;
}
...
static bool do_zipuzp_2(DisasContext* s, arg_zzz_e* a, gen_helper_gvec_3* const fn[5])
{
    ...
    if (s->max_svl < bytes_per_op) { unallocated_encoding(s); }
        ...
        if (svl < bytes_per_op) { unallocated_encoding(s); }
            ...
            tcg_gen_gvec_3_ool(vec_full_reg_offset(s, a->zd), vec_full_reg_offset(s, a->zn),
                               vec_full_reg_offset(s, a->zm), svl, svl, 0, fn[a->esz]);
        }
    }
    return true;
}
...
static gen_helper_gvec_3* const zip2_fns[] = {
    gen_helper_sme2_zip2_b, gen_helper_sme2_zip2_h, gen_helper_sme2_zip2_s,
    gen_helper_sme2_zip2_d, gen_helper_sme2_zip2_q,
};
TRANS_FEAT(ZIP_2, aa64_sme2, do_zipuzp_2, a, zip2_fns)

static gen_helper_gvec_3* const uzp2_fns[] = {
    gen_helper_sme2_uzp2_b, gen_helper_sme2_uzp2_h, gen_helper_sme2_uzp2_s,
    gen_helper_sme2_uzp2_d, gen_helper_sme2_uzp2_q,
};
...
static bool trans_FCLAMP(DisasContext* s, arg_zzz_en* a)
{
    ...
    if (a->esz == MO_8 && !dc_isar_feature(aa64_sme_b16b16, s)) { return false; }
    if (!sme_sm_enabled_check(s)) { return true; }

    fpst = fpstatus_ptr(a->esz == MO_16 ? FPST_A64_F16 : FPST_A64);
    vl   = vec_full_reg_size(s);

    tcg_gen_gvec_3_ptr(vec_full_reg_offset(s, a->zd), vec_full_reg_offset(s, a->zn), vec_full_reg_offset(s, a->zm),
                       fpst, vl, vl, a->n, fn[a->esz]);
    return true;
}

static bool do_clamp(DisasContext* s, arg_zzz_en* a, gen_helper_gvec_3* const fn[4])
{
    int vl;
     ...
     */
    vl = vec_full_reg_size(s);
    tcg_gen_gvec_3_ool(vec_full_reg_offset(s, a->zd), vec_full_reg_offset(s, a->zn), vec_full_reg_offset(s, a->zm), vl,
                       vl, a->n, fn[a->esz]);
    return true;
}
...
static bool trans_SEL(DisasContext* s, arg_SEL* a)
{
    ...
    static sme_sel_fn* const fns[4] = {gen_helper_sme2_sel_b, gen_helper_sme2_sel_h, gen_helper_sme2_sel_s,
                                       gen_helper_sme2_sel_d};
    ...
    if (sme_sm_enabled_check(s)) {
        ...
        tcg_gen_ld16u_i32(png, tcg_env, pred_full_reg_offset(s, a->pg) ^ (HOST_BIG_ENDIAN ? 6 : 0));
...
static bool do_lut(DisasContext* s, arg_lut* a, gen_helper_gvec_2_ptr* fn, bool strided)
{
    if (sme_sm_enabled_check(s) && sme2_zt0_enabled_check(s)) {
        int svl = streaming_vec_reg_size(s);
        tcg_gen_gvec_2_ptr(vec_full_reg_offset(s, a->zd), vec_full_reg_offset(s, a->zn), tcg_env, svl, svl,
                           strided | (a->idx << 1), fn);
    }
    return true;
}
