...
#define SET_QPOS_AREA_START(compr_dest_buf, qpos_start_addr)   (compr_dest_buf[0] = qpos_start_addr - compr_dest_buf)
#define SET_LOW_BITS_AREA_START(compr_dest_buf, lb_start_addr) (compr_dest_buf[1] = lb_start_addr - compr_dest_buf)
#define SET_LOW_BITS_AREA_END(compr_dest_buf, lb_end_addr)     (compr_dest_buf[2] = lb_end_addr - compr_dest_buf)
...
#define TAGS_AREA_END(decomp_src_buf)   (TAGS_AREA_START(decomp_src_buf) + TAGS_AREA_SIZE)
#define FULL_WORD_AREA_START(the_buf) \
    TAGS_AREA_END                     \
    (the_buf)
#define QPOS_AREA_START(decomp_src_buf)     (decomp_src_buf + decomp_src_buf[0])
#define LOW_BITS_AREA_START(decomp_src_buf) (decomp_src_buf + (decomp_src_buf[1]))
...
#define HASH_LOOKUP_TABLE_CONTENTS                                                                              \
    {                                                                                                           \
        0,  52, 8,  56, 16, 12, 28, 20, 4,  36, 48, 24, 44, 40, 32, 60, 8,  12, 28, 20, 4,  60, 16, 36, 24, 48, \
        44, 32, 52, 56, 40, 12, 8,  48, 16, 52, 60, 28, 56, 32, 20, 24, 36, 40, 44, 4,  8,  40, 60, 32, 20, 44, \
        4,  36, 52, 24, 16, 56, 48, 12, 28, 16, 8,  40, 36, 28, 32, 12, 4,  44, 52, 20, 24, 48, 60, 56, 40, 48, \
        8,  32, 28, 36, 4,  44, 20, 56, 60, 24, 52, 16, 12, 12, 4,  48, 20, 8,  52, 16, 60, 24, 36, 44, 28, 56, \
        40, 32, 36, 20, 24, 60, 40, 44, 52, 16, 32, 4,  48, 8,  28, 56, 12, 28, 32, 40, 52, 36, 16, 20, 48, 8,  \
        4,  60, 24, 56, 44, 12, 8,  36, 24, 28, 16, 60, 20, 56, 32, 40, 48, 12, 4,  44, 52, 44, 40, 12, 56, 8,  \
        36, 24, 60, 28, 48, 4,  32, 20, 16, 52, 60, 12, 24, 36, 8,  4,  16, 56, 48, 44, 40, 52, 32, 20, 28, 32, \
        12, 36, 28, 24, 56, 40, 16, 52, 44, 4,  20, 60, 8,  48, 48, 52, 12, 20, 32, 44, 36, 28, 4,  40, 24, 8,  \
        56, 60, 16, 36, 32, 8,  40, 4,  52, 24, 44, 20, 12, 28, 48, 56, 16, 60, 4,  52, 60, 48, 20, 16, 56, 44, \
        24, 8,  40, 12, 32, 28, 36, 24, 32, 12, 4,  20, 16, 60, 36, 28, 8,  52, 40, 48, 44, 56}

#define HASH_TO_DICT_BYTE_OFFSET(pattern) (hashLookupTable[((pattern) >> 10) & 0xFF])
