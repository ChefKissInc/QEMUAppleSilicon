...
static void gen_xar_vec(unsigned vece, TCGv_vec d, TCGv_vec n, TCGv_vec m, int64_t sh)
{
                  ...
                  uint32_t max_sz)
{
    ...
    static const GVecGen3i ops[4]  = {
        {.fni8 = gen_xar8_i64, .fniv = gen_xar_vec, .fno = gen_helper_sve2_xar_b, .opt_opc = vecop, .vece = MO_8},
        {.fni8 = gen_xar16_i64, .fniv = gen_xar_vec, .fno = gen_helper_sve2_xar_h, .opt_opc = vecop, .vece = MO_16},
        {.fni4 = gen_xar_i32, .fniv = gen_xar_vec, .fno = gen_helper_sve2_xar_s, .opt_opc = vecop, .vece = MO_32},
        {.fni8 = gen_xar_i64, .fniv = gen_xar_vec, .fno = gen_helper_gvec_xar_d, .opt_opc = vecop, .vece = MO_64}};
    int esize = 8 << vece;
    ...
    if (shift == 0) {
        ...
        tcg_gen_gvec_3i(rd_ofs, rn_ofs, rm_ofs, opr_sz, max_sz, shift, &ops[vece]);
    }
}
...
static void gen_eor3_vec(unsigned vece, TCGv_vec d, TCGv_vec n, TCGv_vec m, TCGv_vec k)
{
    tcg_gen_xor_vec(vece, d, n, m);
...
static void gen_bcax_vec(unsigned vece, TCGv_vec d, TCGv_vec n, TCGv_vec m, TCGv_vec k)
{
...
static void gen_suqadd_vec(unsigned vece, TCGv_vec t, TCGv_vec qc, TCGv_vec a, TCGv_vec b)
{
    TCGv_vec max = tcg_constant_vec_matching(t, vece, (1ull << ((8 << vece) - 1)) - 1);
...
void gen_gvec_suqadd_qc(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz,
                        uint32_t max_sz)
{
    static const TCGOpcode vecop_list[] = {INDEX_op_add_vec, INDEX_op_sub_vec, INDEX_op_umin_vec, 0};
...
static void gen_usqadd_vec(unsigned vece, TCGv_vec t, TCGv_vec qc, TCGv_vec a, TCGv_vec b)
{
    TCGv_vec u = tcg_temp_new_vec_matching(t);
...
void gen_gvec_usqadd_qc(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz,
                        uint32_t max_sz)
{
    static const TCGOpcode vecop_list[] = {INDEX_op_neg_vec,   INDEX_op_add_vec,    INDEX_op_usadd_vec,
                                           INDEX_op_ussub_vec, INDEX_op_cmpsel_vec, 0};
    static const GVecGen4  ops[4]       = {
        {.fniv       = gen_usqadd_vec,
