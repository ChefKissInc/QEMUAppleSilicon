...
static bool vectors_overlap(ARMVectorReg* x, unsigned nx, ARMVectorReg* y, unsigned ny)
{ return !(x + nx <= y || y + ny <= x); }

void helper_set_svcr(CPUARMState* env, uint32_t val, uint32_t mask) { aarch64_set_svcr(env, val, mask); }
...
void helper_sme_zero(CPUARMState* env, uint32_t imm, uint32_t svl)
{
     ...
     */
    for (i = 0; i < svl; i++) {
        if (imm & (1 << (i % 8))) { memset(&env->za_state.za[i], 0, svl); }
    }
}
...
#define DO_MOVA_C(NAME, TYPE, H)                                                            \
    ...
    {                                                                                       \
        ...
        for (i = 0; i < oprsz;) {                                                           \
            ...
            do {                                                                            \
                if (pg & 1) { *(TYPE*)(za + tile_vslice_offset(i)) = *(TYPE*)(vn + H(i)); } \
                i   += sizeof(TYPE);                                                        \
                pg >>= sizeof(TYPE);                                                        \
            }                                                                               \
            while (i & 15);                                                                 \
...
void HELPER(sme_mova_cz_d)(void* za, void* vn, void* vg, uint32_t desc)
{
    ...
    for (i = 0; i < oprsz; i++) {
        if (pg[H1(i)] & 1) { a[tile_vslice_index(i)] = n[i]; }
    }
}
...
void HELPER(sme_mova_cz_q)(void* za, void* vn, void* vg, uint32_t desc)
{
     ...
     */
    for (i = 0; i < oprsz; i++) {
        if (pg[H2(i)] & 1) { a[tile_vslice_index(i)] = n[i]; }
    }
}
...
#define DO_MOVA_Z(NAME, TYPE, H)                                                            \
    ...
    {                                                                                       \
        ...
        for (i = 0; i < oprsz;) {                                                           \
            uint16_t pg = *(uint16_t*)(vg + H1_2(i >> 3));                                  \
            do {                                                                            \
                if (pg & 1) { *(TYPE*)(vd + H(i)) = *(TYPE*)(za + tile_vslice_offset(i)); } \
                i   += sizeof(TYPE);                                                        \
                pg >>= sizeof(TYPE);                                                        \
            }                                                                               \
            while (i & 15);                                                                 \
...
void HELPER(sme_mova_zc_d)(void* vd, void* za, void* vg, uint32_t desc)
{
    ...
    for (i = 0; i < oprsz; i++) {
        if (pg[H1(i)] & 1) { d[i] = a[tile_vslice_index(i)]; }
    }
}
...
void HELPER(sme_mova_zc_q)(void* vd, void* za, void* vg, uint32_t desc)
{
    ...
    for (i = 0; i < oprsz; i++, za += sizeof(ARMVectorReg)) {
        if (pg[H2(i)] & 1) { d[i] = a[tile_vslice_index(i)]; }
    }
}
...
void HELPER(sme2_mova_zc_b)(void* vdst, void* vsrc, uint32_t desc)
{
    ...
    for (i = 0; i < n; ++i) { dst[i] = src[tile_vslice_index(i)]; }
}
...
static void clear_horizontal(void* ptr, size_t off, size_t len) { memset(ptr + off, 0, len); }

static void clear_vertical_b(void* vptr, size_t off, size_t len)
{
    for (size_t i = 0; i < len; ++i) { *(uint8_t*)(vptr + tile_vslice_offset(i + off)) = 0; }
}

static void clear_vertical_h(void* vptr, size_t off, size_t len)
{
    for (size_t i = 0; i < len; i += 2) { *(uint16_t*)(vptr + tile_vslice_offset(i + off)) = 0; }
}

static void clear_vertical_s(void* vptr, size_t off, size_t len)
{
    for (size_t i = 0; i < len; i += 4) { *(uint32_t*)(vptr + tile_vslice_offset(i + off)) = 0; }
}

static void clear_vertical_d(void* vptr, size_t off, size_t len)
{
    for (size_t i = 0; i < len; i += 8) { *(uint64_t*)(vptr + tile_vslice_offset(i + off)) = 0; }
}

static void clear_vertical_q(void* vptr, size_t off, size_t len)
{
    for (size_t i = 0; i < len; i += 16) { memset(vptr + tile_vslice_offset(i + off), 0, 16); }
}
...
static void copy_horizontal(void* dst, const void* src, size_t len) { memcpy(dst, src, len); }
...
static void copy_vertical_b(void* vdst, const void* vsrc, size_t len)
{
    ...
    for (i = 0; i < len; ++i) { dst[tile_vslice_index(i)] = src[i]; }
}
...
static void copy_vertical_h(void* vdst, const void* vsrc, size_t len)
{
    ...
    for (i = 0; i < len / 2; ++i) { dst[tile_vslice_index(i)] = src[i]; }
}
...
static void copy_vertical_s(void* vdst, const void* vsrc, size_t len)
{
    ...
    for (i = 0; i < len / 4; ++i) { dst[tile_vslice_index(i)] = src[i]; }
}
...
static void copy_vertical_d(void* vdst, const void* vsrc, size_t len)
{
    ...
    for (i = 0; i < len / 8; ++i) { dst[tile_vslice_index(i)] = src[i]; }
}

static void copy_vertical_q(void* vdst, const void* vsrc, size_t len)
{
    for (size_t i = 0; i < len; i += 16) { memcpy(vdst + tile_vslice_offset(i), vsrc + i, 16); }
}

void HELPER(sme2_mova_cz_b)(void* vdst, void* vsrc, uint32_t desc) { copy_vertical_b(vdst, vsrc, simd_oprsz(desc)); }

void HELPER(sme2_mova_cz_h)(void* vdst, void* vsrc, uint32_t desc) { copy_vertical_h(vdst, vsrc, simd_oprsz(desc)); }

void HELPER(sme2_mova_cz_s)(void* vdst, void* vsrc, uint32_t desc) { copy_vertical_s(vdst, vsrc, simd_oprsz(desc)); }

void HELPER(sme2_mova_cz_d)(void* vdst, void* vsrc, uint32_t desc) { copy_vertical_d(vdst, vsrc, simd_oprsz(desc)); }
...
#define DO_LD(NAME, TYPE, HOST, TLB)                                                                                 \
    ...
    static inline void sme_##NAME##_v_tlb(CPUARMState* env, void* za, intptr_t off, target_ulong addr, uintptr_t ra) \
    {                                                                                                                \
...
#define DO_LDQ(HNAME, VNAME)                                                                                    \
    ...
    static inline void VNAME##_v_tlb(CPUARMState* env, void* za, intptr_t off, target_ulong addr, uintptr_t ra) \
...
static inline QEMU_ALWAYS_INLINE void sme_ld1(CPUARMState* env, void* za, uint64_t* vg, const target_ulong addr,
                                              ...
                                              ClearFn* clr_fn, CopyFn* cpy_fn)
{
    ...
    sve_cont_ldst_watchpoints(&info, env, vg, addr, esize, esize, BP_MEM_READ, ra);

    /*
     * Handle mte checks for all active elements.
     * Since TBI must be set for MTE, !mtedesc => !mte_active.
     */
    if (mtedesc) { sve_cont_ldst_mte_check(&info, env, vg, addr, esize, esize, mtedesc, ra); }
    ...
    if (unlikely(flags != 0)) {
        ...
        do {
            ...
            do {
                if ((pg >> (reg_off & 63)) & 1) { tlb_fn(env, &scratch, reg_off, addr + reg_off, ra); }
                reg_off += esize;
            }
            while (reg_off & 63);
        }
    ...
    if (!vertical) { memset(za, 0, reg_max); }
    else if (reg_off) {
        clr_fn(za, 0, reg_off);
    }
    ...
    while (reg_off <= reg_last) {
        uint64_t pg = vg[reg_off >> 6];
        do {
            if ((pg >> (reg_off & 63)) & 1) { host_fn(za, reg_off, host + reg_off); }
            else if (vertical) {
    ...
    if (unlikely(reg_off >= 0)) { tlb_fn(env, za, reg_off, addr + reg_off, ra); }
...
static inline QEMU_ALWAYS_INLINE void sme_ld1_mte(CPUARMState* env, void* za, uint64_t* vg, target_ulong addr,
                                                  uint64_t desc, uintptr_t ra, const int esz, bool vertical,
                                                  sve_ldst1_host_fn* host_fn, sve_ldst1_tlb_fn* tlb_fn, ClearFn* clr_fn,
                                                  CopyFn* cpy_fn)
{
    ...
    sme_ld1(env, za, vg, addr, desc, ra, esz, mtedesc, vertical, host_fn, tlb_fn, clr_fn, cpy_fn);
}

#define DO_LD(L, END, ESZ)                                                                                             \
    void HELPER(sme_ld1##L##END##_h)(CPUARMState * env, void* za, void* vg, target_ulong addr, uint64_t desc)          \
    {                                                                                                                  \
        sme_ld1(env, za, vg, addr, desc, GETPC(), ESZ, 0, false, sve_ld1##L##L##END##_host, sve_ld1##L##L##END##_tlb,  \
                clear_horizontal, copy_horizontal);                                                                    \
    }                                                                                                                  \
    void HELPER(sme_ld1##L##END##_v)(CPUARMState * env, void* za, void* vg, target_ulong addr, uint64_t desc)          \
    {                                                                                                                  \
        sme_ld1(env, za, vg, addr, desc, GETPC(), ESZ, 0, true, sme_ld1##L##END##_v_host, sme_ld1##L##END##_v_tlb,     \
                clear_vertical_##L, copy_vertical_##L);                                                                \
    }                                                                                                                  \
    void HELPER(sme_ld1##L##END##_h_mte)(CPUARMState * env, void* za, void* vg, target_ulong addr, uint64_t desc)      \
    {                                                                                                                  \
        sme_ld1_mte(env, za, vg, addr, desc, GETPC(), ESZ, false, sve_ld1##L##L##END##_host, sve_ld1##L##L##END##_tlb, \
                    clear_horizontal, copy_horizontal);                                                                \
    }                                                                                                                  \
    void HELPER(sme_ld1##L##END##_v_mte)(CPUARMState * env, void* za, void* vg, target_ulong addr, uint64_t desc)      \
    {                                                                                                                  \
        sme_ld1_mte(env, za, vg, addr, desc, GETPC(), ESZ, true, sme_ld1##L##END##_v_host, sme_ld1##L##END##_v_tlb,    \
                    clear_vertical_##L, copy_vertical_##L);                                                            \
    }
...
static inline QEMU_ALWAYS_INLINE void sme_st1(CPUARMState* env, void* za, uint64_t* vg, const target_ulong addr,
                                              uint32_t desc, const uintptr_t ra, const int esz, uint32_t mtedesc,
                                              bool vertical, sve_ldst1_host_fn* host_fn, sve_ldst1_tlb_fn* tlb_fn)
{
    const intptr_t reg_max = simd_oprsz(desc);
    ...
    /* Handle watchpoints for all active elements. */
    sve_cont_ldst_watchpoints(&info, env, vg, addr, esize, esize, BP_MEM_WRITE, ra);
    ...
    if (unlikely(flags != 0)) {
        ...
        do {
            uint64_t pg = vg[reg_off >> 6];
            do {
                if ((pg >> (reg_off & 63)) & 1) { tlb_fn(env, za, reg_off, addr + reg_off, ra); }
                reg_off += esize;
            }
            while (reg_off & 63);
        }
...
static inline QEMU_ALWAYS_INLINE void sme_st1_mte(CPUARMState* env, void* za, uint64_t* vg, target_ulong addr,
                                                  ...
                                                  sve_ldst1_host_fn* host_fn, sve_ldst1_tlb_fn* tlb_fn)
{
    ...
    sme_st1(env, za, vg, addr, desc, ra, esz, mtedesc, vertical, host_fn, tlb_fn);
}

#define DO_ST(L, END, ESZ)                                                                                             \
    void HELPER(sme_st1##L##END##_h)(CPUARMState * env, void* za, void* vg, target_ulong addr, uint64_t desc)          \
    { sme_st1(env, za, vg, addr, desc, GETPC(), ESZ, 0, false, sve_st1##L##L##END##_host, sve_st1##L##L##END##_tlb); } \
    void HELPER(sme_st1##L##END##_v)(CPUARMState * env, void* za, void* vg, target_ulong addr, uint64_t desc)          \
    { sme_st1(env, za, vg, addr, desc, GETPC(), ESZ, 0, true, sme_st1##L##END##_v_host, sme_st1##L##END##_v_tlb); }    \
    void HELPER(sme_st1##L##END##_h_mte)(CPUARMState * env, void* za, void* vg, target_ulong addr, uint64_t desc)      \
    {                                                                                                                  \
        sme_st1_mte(env, za, vg, addr, desc, GETPC(), ESZ, false, sve_st1##L##L##END##_host,                           \
                    sve_st1##L##L##END##_tlb);                                                                         \
    }                                                                                                                  \
    void HELPER(sme_st1##L##END##_v_mte)(CPUARMState * env, void* za, void* vg, target_ulong addr, uint64_t desc)      \
    { sme_st1_mte(env, za, vg, addr, desc, GETPC(), ESZ, true, sme_st1##L##END##_v_host, sme_st1##L##END##_v_tlb); }
...
void HELPER(sme_addha_s)(void* vzda, void* vzn, void* vpn, void* vpm, uint32_t desc)
{
        ...
        do {
            if (pa & 1) {
                for (col = 0; col < oprsz;) {
                    ...
                    do {
                        if (pb & 1) { zda[tile_vslice_index(row) + H4(col)] += zn[H4(col)]; }
                        pb >>= 4;
                    }
                    while (++col & 15);
                }
...
void HELPER(sme_addha_d)(void* vzda, void* vzn, void* vpn, void* vpm, uint32_t desc)
{
    ...
    for (row = 0; row < oprsz; ++row) {
        if (pn[H1(row)] & 1) {
            for (col = 0; col < oprsz; ++col) {
                if (pm[H1(col)] & 1) { zda[tile_vslice_index(row) + col] += zn[col]; }
            }
        }
    }
}

void HELPER(sme_addva_s)(void* vzda, void* vzn, void* vpn, void* vpm, uint32_t desc)
{
    intptr_t  row, col, oprsz = simd_oprsz(desc) / 4;
    uint64_t *pn = vpn, *pm = vpm;
    uint32_t *zda = vzda, *zn = vzn;
        ...
        do {
            if (pa & 1) {
                ...
                for (col = 0; col < oprsz;) {
                    uint64_t pb = pm[col >> 4];
                    do {
                        if (pb & 1) { zda[tile_vslice_index(row) + H4(col)] += zn_row; }
                        pb >>= 4;
                    }
                    while (++col & 15);
                }
...
void HELPER(sme_addva_d)(void* vzda, void* vzn, void* vpn, void* vpm, uint32_t desc)
{
    intptr_t  row, col, oprsz = simd_oprsz(desc) / 8;
    uint8_t * pn = vpn, *pm = vpm;
    uint64_t *zda = vzda, *zn = vzn;
    ...
    for (row = 0; row < oprsz; ++row) {
        if (pn[H1(row)] & 1) {
            ...
            for (col = 0; col < oprsz; ++col) {
                if (pm[H1(col)] & 1) { zda[tile_vslice_index(row) + col] += zn_row; }
            }
        }
    }
}

static void do_fmopa_h(void* vza, void* vzn, void* vzm, uint16_t* pn, uint16_t* pm, float_status* fpst, uint32_t desc,
...
void HELPER(sme_fmopa_h)(void* vza, void* vzn, void* vzm, void* vpn, void* vpm, float_status* fpst, uint32_t desc)
{ do_fmopa_h(vza, vzn, vzm, vpn, vpm, fpst, desc, 0, 0); }

void HELPER(sme_fmops_h)(void* vza, void* vzn, void* vzm, void* vpn, void* vpm, float_status* fpst, uint32_t desc)
{ do_fmopa_h(vza, vzn, vzm, vpn, vpm, fpst, desc, 1u << 15, 0); }

void HELPER(sme_ah_fmops_h)(void* vza, void* vzn, void* vzm, void* vpn, void* vpm, float_status* fpst, uint32_t desc)
{ do_fmopa_h(vza, vzn, vzm, vpn, vpm, fpst, desc, 0, float_muladd_negate_product); }

static void do_fmopa_s(void* vza, void* vzn, void* vzm, uint16_t* pn, uint16_t* pm, float_status* fpst, uint32_t desc,
...
void HELPER(sme_fmopa_s)(void* vza, void* vzn, void* vzm, void* vpn, void* vpm, float_status* fpst, uint32_t desc)
{ do_fmopa_s(vza, vzn, vzm, vpn, vpm, fpst, desc, 0, 0); }

void HELPER(sme_fmops_s)(void* vza, void* vzn, void* vzm, void* vpn, void* vpm, float_status* fpst, uint32_t desc)
{ do_fmopa_s(vza, vzn, vzm, vpn, vpm, fpst, desc, 1u << 31, 0); }

void HELPER(sme_ah_fmops_s)(void* vza, void* vzn, void* vzm, void* vpn, void* vpm, float_status* fpst, uint32_t desc)
{ do_fmopa_s(vza, vzn, vzm, vpn, vpm, fpst, desc, 0, float_muladd_negate_product); }

static void do_fmopa_d(uint64_t* za, uint64_t* zn, uint64_t* zm, uint8_t* pn, uint8_t* pm, float_status* fpst,
                       uint32_t desc, uint64_t negx, int negf)
{
    intptr_t row, col, oprsz = simd_oprsz(desc) / 8;
...
void HELPER(sme_fmopa_d)(void* vza, void* vzn, void* vzm, void* vpn, void* vpm, float_status* fpst, uint32_t desc)
{ do_fmopa_d(vza, vzn, vzm, vpn, vpm, fpst, desc, 0, 0); }

void HELPER(sme_fmops_d)(void* vza, void* vzn, void* vzm, void* vpn, void* vpm, float_status* fpst, uint32_t desc)
{ do_fmopa_d(vza, vzn, vzm, vpn, vpm, fpst, desc, 1ull << 63, 0); }

void HELPER(sme_ah_fmops_d)(void* vza, void* vzn, void* vzm, void* vpn, void* vpm, float_status* fpst, uint32_t desc)
{ do_fmopa_d(vza, vzn, vzm, vpn, vpm, fpst, desc, 0, float_muladd_negate_product); }

static void do_bfmopa(void* vza, void* vzn, void* vzm, uint16_t* pn, uint16_t* pm, float_status* fpst, uint32_t desc,
                      uint16_t negx, int negf)
{
    intptr_t row, col, oprsz = simd_maxsz(desc);
...
void HELPER(sme_bfmopa)(void* vza, void* vzn, void* vzm, void* vpn, void* vpm, float_status* fpst, uint32_t desc)
{ do_bfmopa(vza, vzn, vzm, vpn, vpm, fpst, desc, 0, 0); }

void HELPER(sme_bfmops)(void* vza, void* vzn, void* vzm, void* vpn, void* vpm, float_status* fpst, uint32_t desc)
{ do_bfmopa(vza, vzn, vzm, vpn, vpm, fpst, desc, 1u << 15, 0); }

void HELPER(sme_ah_bfmops)(void* vza, void* vzn, void* vzm, void* vpn, void* vpm, float_status* fpst, uint32_t desc)
{ do_bfmopa(vza, vzn, vzm, vpn, vpm, fpst, desc, 0, float_muladd_negate_product); }
...
static inline uint32_t f16mop_adj_pair(uint32_t pair, uint32_t pg, uint32_t neg)
{
     ...
     */
    pair ^= neg;
    if (!(pg & 1)) { pair &= 0xffff0000u; }
    if (!(pg & 4)) { pair &= 0x0000ffffu; }
    return pair;
}
...
static float32 f16_dotadd(float32 sum, uint32_t e1, uint32_t e2, float_status* s_f16, float_status* s_std,
                          float_status* s_odd)
{
    /*
    ...
    /* C.f. FPProcessNaNs4 */
    if (float16_is_any_nan(h1r) || float16_is_any_nan(h1c) || float16_is_any_nan(h2r) || float16_is_any_nan(h2c)) {
        float16 t16;

        if (float16_is_signaling_nan(h1r, s_f16)) { t16 = h1r; }
...
static void do_fmopa_w_h(void* vza, void* vzn, void* vzm, uint16_t* pn, uint16_t* pm, CPUARMState* env, uint32_t desc,
                         uint32_t negx, bool ah_neg)
{
    ...
    for (row = 0; row < oprsz;) {
        ...
        do {
            ...
            if (ah_neg) { n = f16mop_ah_neg_adj_pair(n, prow); }
            else {
            ...
            for (col = 0; col < oprsz;) {
                ...
                do {
                    if (prow & pcol & 0b0101) {
                        ...
                        *a = f16_dotadd(*a, n, m, &env->vfp.fp_status[FPST_ZA_F16], &env->vfp.fp_status[FPST_ZA],
                                        &fpst_odd);
                    }
                    col   += 4;
                    pcol >>= 4;
...
void HELPER(sme_fmopa_w_h)(void* vza, void* vzn, void* vzm, void* vpn, void* vpm, CPUARMState* env, uint32_t desc)
{ do_fmopa_w_h(vza, vzn, vzm, vpn, vpm, env, desc, 0, false); }

void HELPER(sme_fmops_w_h)(void* vza, void* vzn, void* vzm, void* vpn, void* vpm, CPUARMState* env, uint32_t desc)
{ do_fmopa_w_h(vza, vzn, vzm, vpn, vpm, env, desc, 0x80008000u, false); }

void HELPER(sme_ah_fmops_w_h)(void* vza, void* vzn, void* vzm, void* vpn, void* vpm, CPUARMState* env, uint32_t desc)
{ do_fmopa_w_h(vza, vzn, vzm, vpn, vpm, env, desc, 0, true); }

void HELPER(sme2_fdot_h)(void* vd, void* vn, void* vm, void* va, CPUARMState* env, uint32_t desc)
{
    ...
    for (i = 0; i < oprsz / sizeof(float32); ++i) {
        d[H4(i)] = f16_dotadd(a[H4(i)], n[H4(i)], m[H4(i)], fpst_f16, fpst_std, &fpst_odd);
    }
}

void HELPER(sme2_fdot_idx_h)(void* vd, void* vn, void* vm, void* va, CPUARMState* env, uint32_t desc)
{
    ...
    for (i = 0; i < elements; i += eltspersegment) {
        ...
        for (j = 0; j < eltspersegment; ++j) {
            d[H4(i + j)] = f16_dotadd(a[H4(i + j)], n[H4(i + j)], mm, fpst_f16, fpst_std, &fpst_odd);
        }
    }
}

void HELPER(sme2_fvdot_idx_h)(void* vd, void* vn, void* vm, void* va, CPUARMState* env, uint32_t desc)
{
    intptr_t     i, j, oprsz = simd_maxsz(desc);
    intptr_t     elements       = oprsz / sizeof(float32);
    intptr_t     eltspersegment = MIN(4, elements);
    ...
    for (i = 0; i < elements; i += eltspersegment) {
        ...
        for (j = 0; j < eltspersegment; ++j) {
            ...
            d[i + H4(j)] = f16_dotadd(a[i + H4(j)], nn, mm, fpst_f16, fpst_std, &fpst_odd);
        }
    }
}

static void do_bfmopa_w(void* vza, void* vzn, void* vzm, uint16_t* pn, uint16_t* pm, CPUARMState* env, uint32_t desc,
                        uint32_t negx, bool ah_neg)
{
    intptr_t     row, col, oprsz = simd_maxsz(desc);
    ...
    if (is_ebf(env, &fpst, &fpst_odd)) {
        for (row = 0; row < oprsz;) {
            ...
            do {
                ...
                if (ah_neg) { n = bf16mop_ah_neg_adj_pair(n, prow); }
                else {
                    n = f16mop_adj_pair(n, prow, negx);
                }
...
void HELPER(sme_bfmopa_w)(void* vza, void* vzn, void* vzm, void* vpn, void* vpm, CPUARMState* env, uint32_t desc)
{ do_bfmopa_w(vza, vzn, vzm, vpn, vpm, env, desc, 0, false); }

void HELPER(sme_bfmops_w)(void* vza, void* vzn, void* vzm, void* vpn, void* vpm, CPUARMState* env, uint32_t desc)
{ do_bfmopa_w(vza, vzn, vzm, vpn, vpm, env, desc, 0x80008000u, false); }

void HELPER(sme_ah_bfmops_w)(void* vza, void* vzn, void* vzm, void* vpn, void* vpm, CPUARMState* env, uint32_t desc)
{ do_bfmopa_w(vza, vzn, vzm, vpn, vpm, env, desc, 0, true); }

typedef uint32_t   IMOPFn32(uint32_t, uint32_t, uint32_t, uint8_t, bool);
static inline void do_imopa_s(uint32_t* za, uint32_t* zn, uint32_t* zm, uint8_t* pn, uint8_t* pm, uint32_t desc,
                              IMOPFn32* fn)
{
    intptr_t row, col, oprsz = simd_oprsz(desc) / 4;
...
static inline void do_imopa_d(uint64_t* za, uint64_t* zn, uint64_t* zm, uint8_t* pn, uint8_t* pm, uint32_t desc,
                              IMOPFn64* fn)
{
    intptr_t row, col, oprsz = simd_oprsz(desc) / 8;
    bool     neg = simd_data(desc);
...
DEF_IMOP_8x4_32(smopa_s, int8_t, int8_t) DEF_IMOP_8x4_32(umopa_s, uint8_t, uint8_t)
    DEF_IMOP_8x4_32(sumopa_s, int8_t, uint8_t) DEF_IMOP_8x4_32(usmopa_s, uint8_t, int8_t)

        DEF_IMOP_16x4_64(smopa_d, int16_t, int16_t) DEF_IMOP_16x4_64(umopa_d, uint16_t, uint16_t)
            DEF_IMOP_16x4_64(sumopa_d, int16_t, uint16_t) DEF_IMOP_16x4_64(usmopa_d, uint16_t, int16_t)

#define DEF_IMOPH(P, NAME, S)                                                                           \
    void HELPER(P##_##NAME##_##S)(void* vza, void* vzn, void* vzm, void* vpn, void* vpm, uint32_t desc) \
    { do_imopa_##S(vza, vzn, vzm, vpn, vpm, desc, NAME##_##S); }

                DEF_IMOPH(sme, smopa, s) DEF_IMOPH(sme, umopa, s) DEF_IMOPH(sme, sumopa, s) DEF_IMOPH(sme, usmopa, s)

                    DEF_IMOPH(sme, smopa, d) DEF_IMOPH(sme, umopa, d) DEF_IMOPH(sme, sumopa, d)
                        DEF_IMOPH(sme, usmopa, d) static uint32_t
    bmopa_s(uint32_t n, uint32_t m, uint32_t a, uint8_t p, bool neg)
{
...
DEF_IMOP_16x2_32(smopa2_s, int16_t, int16_t) DEF_IMOP_16x2_32(umopa2_s, uint16_t, uint16_t) DEF_IMOPH(sme2, smopa2, s)
...
#define DO_VDOT_IDX(NAME, TYPED, TYPEN, TYPEM, HD, HN)                 \
    ...
    {                                                                  \
        ...
        DO_VDOT_IDX(sme2_svdot_idx_4b, int32_t, int8_t, int8_t, H4,
                    H1) DO_VDOT_IDX(sme2_uvdot_idx_4b, uint32_t, uint8_t, uint8_t, H4,
                                    H1) DO_VDOT_IDX(sme2_suvdot_idx_4b, int32_t, int8_t, uint8_t, H4, H1)
            DO_VDOT_IDX(sme2_usvdot_idx_4b, int32_t, uint8_t, int8_t, H4,
                        H1) DO_VDOT_IDX(sme2_svdot_idx_4h, int64_t, int16_t, int16_t, H8,
                                        H2) DO_VDOT_IDX(sme2_uvdot_idx_4h, uint64_t, uint16_t, uint16_t, H8, H2)

                DO_VDOT_IDX(sme2_svdot_idx_2h, int32_t, int16_t, int16_t, H4,
                            H2) DO_VDOT_IDX(sme2_uvdot_idx_2h, uint32_t, uint16_t, uint16_t, H4, H2)
                                            ...
                                            DO_MLALL_IDX(sme2_smlall_idx_s, int32_t, int8_t, int8_t, H4, H1, +)
                                                DO_MLALL_IDX(sme2_smlall_idx_d, int64_t, int16_t, int16_t, H8, H2, +)
                                                    DO_MLALL_IDX(sme2_smlsll_idx_s, int32_t, int8_t, int8_t, H4, H1, -)
                                                        DO_MLALL_IDX(sme2_smlsll_idx_d, int64_t, int16_t, int16_t, H8,
                                                                     H2, -) DO_MLALL_IDX(sme2_umlall_idx_s, uint32_t,
                                                                                         uint8_t, uint8_t, H4, H1, +)
                                                            DO_MLALL_IDX(sme2_umlall_idx_d, uint64_t, uint16_t,
                                                                         uint16_t, H8, H2, +)
                                                                DO_MLALL_IDX(sme2_umlsll_idx_s, uint32_t, uint8_t,
                                                                             uint8_t, H4, H1, -)
                                                                    DO_MLALL_IDX(sme2_umlsll_idx_d, uint64_t, uint16_t,
                                                                                 uint16_t, H8, H2, -)

                                                                        DO_MLALL_IDX(sme2_usmlall_idx_s, uint32_t,
                                                                                     uint8_t, int8_t, H4, H1, +)
                                                                            DO_MLALL_IDX(sme2_sumlall_idx_s, uint32_t,
                                                                                         int8_t, uint8_t, H4, H1, +)
    ...
    void HELPER(sme2_bfcvt)(void* vd, void* vs, float_status* fpst, uint32_t desc)
{
    ...
    if (vd == s1) { s1 = memcpy(&scratch, s1, oprsz); }

    for (i = 0; i < n; ++i) { d[H2(i)] = float32_to_bfloat16(s0[H4(i)], fpst); }
...
void HELPER(sme2_fcvt_n)(void* vd, void* vs, float_status* fpst, uint32_t desc)
{
    ...
    for (i = 0; i < n; ++i) { d[H2(i)] = sve_f32_to_f16(s0[H4(i)], fpst); }
...
#define SQCVT2(NAME, TW, TN, HW, HN, SAT)                              \
    ...
    {                                                                  \
        ...
        TN*          d = vd;                                           \
        if (vectors_overlap(vd, 1, vs, 2)) { d = (TN*)&scratch; }      \
        for (size_t i = 0; i < n; ++i) {                               \
            d[HN(i)]     = SAT(s0[HW(i)]);                             \
            d[HN(i + n)] = SAT(s1[HW(i)]);                             \
        }                                                              \
        if (d != vd) { memcpy(vd, d, oprsz); }                         \
    }
...
#define SQCVT4(NAME, TW, TN, HW, HN, SAT)                              \
    ...
    {                                                                  \
        ...
        TN*          d  = vd;                                          \
        if (vectors_overlap(vd, 1, vs, 4)) { d = (TN*)&scratch; }      \
        for (size_t i = 0; i < n; ++i) {                               \
            d[HN(i)]         = SAT(s0[HW(i)]);                         \
            d[HN(i + n)]     = SAT(s1[HW(i)]);                         \
...
void HELPER(sme2_fcvt_w)(void* vd, void* vs, float_status* fpst, uint32_t desc)
{
    ...
    if (vectors_overlap(vd, 1, vs, 2)) { s = memcpy(&scratch, s, oprsz); }

    for (i = 0; i < n; ++i) { d0[H4(i)] = sve_f16_to_f32(s[H2(i)], fpst); }
...
#define UNPK(NAME, SREG, TW, TN, HW, HN)                                                            \
    ...
    {                                                                                               \
        ...
        if (vectors_overlap(vd, 2 * SREG, vs, SREG)) { vs = memcpy(scratch, vs, sizeof(scratch)); } \
        for (size_t r = 0; r < SREG; ++r) {                                                         \
            TN* s = vs + r * sizeof(ARMVectorReg);                                                  \
            for (size_t i = 0; i < 2; ++i) {                                                        \
                TW* d = vd + (2 * r + i) * sizeof(ARMVectorReg);                                    \
                for (size_t e = 0; e < n; ++e) { d[HW(e)] = s[HN(i * n + e)]; }                     \
            }                                                                                       \
        }                                                                                           \
    }
...
void HELPER(sme2_scvtf)(void* vd, void* vs, float_status* fpst, uint32_t desc)
{
    ...
    for (i = 0; i < n; ++i) { d[i] = int32_to_float32(s[i], fpst); }
}
...
void HELPER(sme2_ucvtf)(void* vd, void* vs, float_status* fpst, uint32_t desc)
{
    ...
    for (i = 0; i < n; ++i) { d[i] = uint32_to_float32(s[i], fpst); }
}
...
#define ZIP2(NAME, TYPE, H)                                                        \
    ...
    {                                                                              \
        ...
        TYPE *       n = vn, *m = vm;                                              \
        if (vectors_overlap(vd, 2, vn, 1)) { n = memcpy(&scratch[0], vn, oprsz); } \
        if (vectors_overlap(vd, 2, vm, 1)) { m = memcpy(&scratch[1], vm, oprsz); } \
...
#define ZIP4(NAME, TYPE, H)                                          \
    ...
    {                                                                \
        ...
        TYPE *       s0, *s1, *s2, *s3;                              \
        if (vs == vd) { vs = memcpy(scratch, vs, sizeof(scratch)); } \
        s0 = vs;                                                     \
...
#define UZP2(NAME, TYPE, H)                                                         \
    ...
    {                                                                               \
        ...
        if (vectors_overlap(vd, 2, vn, 1)) { vn = memcpy(&scratch[0], vn, oprsz); } \
        if (vectors_overlap(vd, 2, vm, 1)) { vm = memcpy(&scratch[1], vm, oprsz); } \
        for (size_t r = 0; r < 2; ++r) {                                            \
            TYPE*  s    = r ? vm : vn;                                              \
            size_t base = r * pairs;                                                \
            for (size_t p = 0; p < pairs; ++p) {                                    \
...
#define FCLAMP(NAME, TYPE, H)                                                          \
    void HELPER(NAME)(void* vd, void* vn, void* vm, float_status* fpst, uint32_t desc) \
    {                                                                                  \
        size_t stride   = sizeof(ARMVectorReg) / sizeof(TYPE);                         \
        size_t elements = simd_oprsz(desc) / sizeof(TYPE);                             \
        size_t nreg     = simd_data(desc);                                             \
...
void HELPER(sme2_sel_b)(void* vd, void* vn, void* vm, uint32_t png, uint32_t desc)
{
    ...
    if (p.lg2_stride == 0) {
        if (p.invert) {
            for (int r = 0; r < nreg; r++) {
                ...
                if (split <= 0) { memcpy(d, n, vl); /* all true */ }
                else if (elements <= split) {
                    memcpy(d, m, vl); /* all false */
                }
                else {
                    for (int e = 0; e < split; e++) { d[H1(e)] = m[H1(e)]; }
                    for (int e = split; e < elements; e++) { d[H1(e)] = n[H1(e)]; }
                }
            }
        }
        else {
            for (int r = 0; r < nreg; r++) {
                ...
                if (split <= 0) { memcpy(d, m, vl); /* all false */ }
                else if (elements <= split) {
                    memcpy(d, n, vl); /* all true */
                }
                else {
                    for (int e = 0; e < split; e++) { d[H1(e)] = n[H1(e)]; }
                    for (int e = split; e < elements; e++) { d[H1(e)] = m[H1(e)]; }
                }
            }
        }
    }
        ...
        if (p.invert) {
            for (int r = 0; r < nreg; r++) {
                ...
                for (; e < MIN(split, elements); e++) { d[H1(e)] = m[H1(e)]; }
                for (; e < elements; e += estride) {
                    d[H1(e)] = n[H1(e)];
                    for (int i = 1; i < estride; i++) { d[H1(e + i)] = m[H1(e + i)]; }
                }
            }
        }
        else {
            for (int r = 0; r < nreg; r++) {
                ...
                for (; e < elements; e++) { d[H1(e)] = m[H1(e)]; }
            }
        }
    }
}

void HELPER(sme2_sel_h)(void* vd, void* vn, void* vm, uint32_t png, uint32_t desc)
{
    int           vl       = simd_oprsz(desc);
    int           nreg     = simd_data(desc);
    ...
    if (p.lg2_stride == 0) {
        if (p.invert) {
            for (int r = 0; r < nreg; r++) {
                ...
                if (split <= 0) { memcpy(d, n, vl); /* all true */ }
                    ...
                    for (int e = 0; e < split; e++) { d[H2(e)] = m[H2(e)]; }
                    for (int e = split; e < elements; e++) { d[H2(e)] = n[H2(e)]; }
                }
            }
        }
        else {
            for (int r = 0; r < nreg; r++) {
                ...
                if (split <= 0) { memcpy(d, m, vl); /* all false */ }
                    ...
                    for (int e = 0; e < split; e++) { d[H2(e)] = n[H2(e)]; }
                    for (int e = split; e < elements; e++) { d[H2(e)] = m[H2(e)]; }
                }
            }
        }
    }
        ...
        if (p.invert) {
            for (int r = 0; r < nreg; r++) {
                ...
                for (; e < MIN(split, elements); e++) { d[H2(e)] = m[H2(e)]; }
                for (; e < elements; e += estride) {
                    d[H2(e)] = n[H2(e)];
                    for (int i = 1; i < estride; i++) { d[H2(e + i)] = m[H2(e + i)]; }
                }
            }
        }
        else {
            for (int r = 0; r < nreg; r++) {
                ...
                for (; e < elements; e++) { d[H2(e)] = m[H2(e)]; }
            }
        }
    }
}

void HELPER(sme2_sel_s)(void* vd, void* vn, void* vm, uint32_t png, uint32_t desc)
{
    int           vl       = simd_oprsz(desc);
    int           nreg     = simd_data(desc);
    ...
    if (p.lg2_stride == 0) {
        if (p.invert) {
            for (int r = 0; r < nreg; r++) {
                ...
                if (split <= 0) { memcpy(d, n, vl); /* all true */ }
                    ...
                    for (int e = 0; e < split; e++) { d[H4(e)] = m[H4(e)]; }
                    for (int e = split; e < elements; e++) { d[H4(e)] = n[H4(e)]; }
                }
            }
        }
        else {
            for (int r = 0; r < nreg; r++) {
                ...
                if (split <= 0) { memcpy(d, m, vl); /* all false */ }
                    ...
                    for (int e = 0; e < split; e++) { d[H4(e)] = n[H4(e)]; }
                    for (int e = split; e < elements; e++) { d[H4(e)] = m[H4(e)]; }
                }
            }
        }
    }
        ...
        if (p.invert) {
            for (int r = 0; r < nreg; r++) {
                ...
                for (; e < MIN(split, elements); e++) { d[H4(e)] = m[H4(e)]; }
                for (; e < elements; e += 2) {
                    d[H4(e)]     = n[H4(e)];
                    d[H4(e + 1)] = m[H4(e + 1)];
                }
            ...
            for (int r = 0; r < nreg; r++) {
                ...
                for (; e < elements; e++) { d[H4(e)] = m[H4(e)]; }
            }
        }
    }
}

void HELPER(sme2_sel_d)(void* vd, void* vn, void* vm, uint32_t png, uint32_t desc)
{
    int           vl       = simd_oprsz(desc);
    int           nreg     = simd_data(desc);
    ...
    if (p.invert) {
        for (int r = 0; r < nreg; r++) {
            ...
            if (split <= 0) { memcpy(d, n, vl); /* all true */ }
                ...
                memcpy(d + split, n + split, (elements - split) * sizeof(uint64_t));
            }
        }
    }
    else {
        for (int r = 0; r < nreg; r++) {
            ...
            if (split <= 0) { memcpy(d, m, vl); /* all false */ }
                ...
                memcpy(d + split, m + split, (elements - split) * sizeof(uint64_t));
            }
        }
    }
}
