...
static inline uint64_t expand_pred_d(uint8_t byte) { return -(uint64_t)(byte & 1); }
...
#define LOGICAL_PPPP(NAME, FUNC)                                             \
    ...
    {                                                                        \
        ...
        uintptr_t i;                                                         \
        for (i = 0; i < opr_sz / 8; ++i) { d[i] = FUNC(n[i], m[i], g[i]); }  \
    }
...
static inline uint8_t do_mulh_b(int32_t n, int32_t m) { return (n * m) >> 8; }

static inline uint16_t do_mulh_h(int32_t n, int32_t m) { return (n * m) >> 16; }

static inline uint32_t do_mulh_s(int64_t n, int64_t m) { return (n * m) >> 32; }
...
#define do_sqshl_b(n, m)                         \
    ({                                           \
        uint32_t discard;                        \
        do_sqrshl_bhs(n, m, 8, false, &discard); \
    })
#define do_sqshl_h(n, m)                          \
    ({                                            \
        uint32_t discard;                         \
        do_sqrshl_bhs(n, m, 16, false, &discard); \
    })
#define do_sqshl_s(n, m)                          \
    ({                                            \
        uint32_t discard;                         \
        do_sqrshl_bhs(n, m, 32, false, &discard); \
    })
#define do_sqshl_d(n, m)                    \
    ({                                      \
        uint32_t discard;                   \
        do_sqrshl_d(n, m, false, &discard); \
    })
...
#define do_uqshl_b(n, m)                                 \
    ({                                                   \
        uint32_t discard;                                \
        do_uqrshl_bhs(n, (int8_t)m, 8, false, &discard); \
    })
#define do_uqshl_h(n, m)                                   \
    ({                                                     \
        uint32_t discard;                                  \
        do_uqrshl_bhs(n, (int16_t)m, 16, false, &discard); \
    })
#define do_uqshl_s(n, m)                          \
    ({                                            \
        uint32_t discard;                         \
        do_uqrshl_bhs(n, m, 32, false, &discard); \
    })
#define do_uqshl_d(n, m)                    \
    ({                                      \
        uint32_t discard;                   \
        do_uqrshl_d(n, m, false, &discard); \
    })
...
#define do_sqrshl_b(n, m)                       \
    ({                                          \
        uint32_t discard;                       \
        do_sqrshl_bhs(n, m, 8, true, &discard); \
    })
#define do_sqrshl_h(n, m)                        \
    ({                                           \
        uint32_t discard;                        \
        do_sqrshl_bhs(n, m, 16, true, &discard); \
    })
#define do_sqrshl_s(n, m)                        \
    ({                                           \
        uint32_t discard;                        \
        do_sqrshl_bhs(n, m, 32, true, &discard); \
    })
#define do_sqrshl_d(n, m)                  \
    ({                                     \
        uint32_t discard;                  \
        do_sqrshl_d(n, m, true, &discard); \
    })
...
#define do_uqrshl_b(n, m)                               \
    ({                                                  \
        uint32_t discard;                               \
        do_uqrshl_bhs(n, (int8_t)m, 8, true, &discard); \
    })
#define do_uqrshl_h(n, m)                                 \
    ({                                                    \
        uint32_t discard;                                 \
        do_uqrshl_bhs(n, (int16_t)m, 16, true, &discard); \
    })
#define do_uqrshl_s(n, m)                        \
    ({                                           \
        uint32_t discard;                        \
        do_uqrshl_bhs(n, m, 32, true, &discard); \
    })
#define do_uqrshl_d(n, m)                  \
    ({                                     \
        uint32_t discard;                  \
        do_uqrshl_d(n, m, true, &discard); \
    })
...
static inline uint64_t do_uqsub_d(uint64_t n, uint64_t m) { return n > m ? n - m : 0; }
...
static inline int64_t do_suqadd_d(int64_t n, uint64_t m)
{
    ...
    if (n < 0) {
        ...
        if (r < m || r > INT64_MAX) { return INT64_MAX; }
    }
    return r;
}
    ...
    {                                                                        \
        ...
        for (i = 0; i < opr_sz;) {                                           \
            ...
            do {                                                             \
                ...
                if (pg & 1) { *(TYPE*)(vd + H(i)) = OP(n0, n1); }            \
                i += sizeof(TYPE), pg >>= sizeof(TYPE);                      \
                if (pg & 1) { *(TYPE*)(vd + H(i)) = OP(m0, m1); }            \
                i += sizeof(TYPE), pg >>= sizeof(TYPE);                      \
            }                                                                \
            while (i & 15);                                                  \
        }                                                                    \
...
#define DO_ZPZZ_PAIR_D(NAME, TYPE, OP)                                       \
    ...
    {                                                                        \
        ...
        for (i = 0; i < opr_sz; i += 2) {                                    \
            ...
            if (pg[H1(i)] & 1) { d[i] = OP(n0, n1); }                        \
            if (pg[H1(i + 1)] & 1) { d[i + 1] = OP(m0, m1); }                \
        }                                                                    \
    }
...
#define DO_ZPZZ_PAIR_FP(NAME, TYPE, H, OP)                                                         \
    void HELPER(NAME)(void* vd, void* vn, void* vm, void* vg, float_status* status, uint32_t desc) \
    {                                                                                              \
        intptr_t i, opr_sz = simd_oprsz(desc);                                                     \
        for (i = 0; i < opr_sz;) {                                                                 \
            uint16_t pg = *(uint16_t*)(vg + H1_2(i >> 3));                                         \
            do {                                                                                   \
                ...
                TYPE n1 = *(TYPE*)(vn + H(i + sizeof(TYPE)));                                      \
                TYPE m1 = *(TYPE*)(vm + H(i + sizeof(TYPE)));                                      \
                if (pg & 1) { *(TYPE*)(vd + H(i)) = OP(n0, n1, status); }                          \
                i += sizeof(TYPE), pg >>= sizeof(TYPE);                                            \
                if (pg & 1) { *(TYPE*)(vd + H(i)) = OP(m0, m1, status); }                          \
                i += sizeof(TYPE), pg >>= sizeof(TYPE);                                            \
            }                                                                                      \
            while (i & 15);                                                                        \
        }                                                                                          \
...
DO_SQDMLAL(sve2_sqdmlal_zzzw_h, int16_t, int8_t, H1_2, H1, do_sqdmull_h, DO_SQADD_H)
DO_SQDMLAL(sve2_sqdmlal_zzzw_s, int32_t, int16_t, H1_4, H1_2, do_sqdmull_s, DO_SQADD_S)
DO_SQDMLAL(sve2_sqdmlal_zzzw_d, int64_t, int32_t, H1_8, H1_4, do_sqdmull_d, do_sqadd_d)

DO_SQDMLAL(sve2_sqdmlsl_zzzw_h, int16_t, int8_t, H1_2, H1, do_sqdmull_h, DO_SQSUB_H)
DO_SQDMLAL(sve2_sqdmlsl_zzzw_s, int32_t, int16_t, H1_4, H1_2, do_sqdmull_s, DO_SQSUB_S)
DO_SQDMLAL(sve2_sqdmlsl_zzzw_d, int64_t, int32_t, H1_8, H1_4, do_sqdmull_d, do_sqsub_d)
...
#define DO_SQRDMLAH_B(N, M, A, S) do_sqrdmlah_b(N, M, A, S, true)
#define DO_SQRDMLAH_H(N, M, A, S)                  \
    ({                                             \
        uint32_t discard;                          \
        do_sqrdmlah_h(N, M, A, S, true, &discard); \
    })
#define DO_SQRDMLAH_S(N, M, A, S)                  \
    ({                                             \
        uint32_t discard;                          \
        do_sqrdmlah_s(N, M, A, S, true, &discard); \
    })
#define DO_SQRDMLAH_D(N, M, A, S) do_sqrdmlah_d(N, M, A, S, true)
...
static int32_t do_cdot_s(uint32_t n, uint32_t m, int32_t a, int sel_a, int sel_b, int sub_i)
{
    for (int i = 0; i <= 1; i++) {
...
static int64_t do_cdot_d(uint64_t n, uint64_t m, int64_t a, int sel_a, int sel_b, int sub_i)
{
    for (int i = 0; i <= 1; i++) {
...
void HELPER(sve2_cdot_zzzz_s)(void* vd, void* vn, void* vm, void* va, uint32_t desc)
{
    ...
    for (int e = 0; e < opr_sz / 4; e++) { d[e] = do_cdot_s(n[e], m[e], a[e], sel_a, sel_b, sub_i); }
}

void HELPER(sve2_cdot_zzzz_d)(void* vd, void* vn, void* vm, void* va, uint32_t desc)
{
    int       opr_sz = simd_oprsz(desc);
    int       rot    = simd_data(desc);
    int       sel_a  = rot & 1;
    ...
    for (int e = 0; e < opr_sz / 8; e++) { d[e] = do_cdot_d(n[e], m[e], a[e], sel_a, sel_b, sub_i); }
}

void HELPER(sve2_cdot_idx_s)(void* vd, void* vn, void* vm, void* va, uint32_t desc)
{
    int       opr_sz = simd_oprsz(desc);
    int       rot    = extract32(desc, SIMD_DATA_SHIFT, 2);
    ...
    for (int seg = 0; seg < opr_sz / 4; seg += 4) {
        ...
        for (int e = 0; e < 4; e++) { d[seg + e] = do_cdot_s(n[seg + e], seg_m, a[seg + e], sel_a, sel_b, sub_i); }
    }
}

void HELPER(sve2_cdot_idx_d)(void* vd, void* vn, void* vm, void* va, uint32_t desc)
{
    ...
    for (seg = 0; seg < opr_sz / 8; seg += 2) {
        ...
        for (int e = 0; e < 2; e++) { d[seg + e] = do_cdot_d(n[seg + e], seg_m, a[seg + e], sel_a, sel_b, sub_i); }
    }
}
...
#define DO_ZZXZ(NAME, TYPE, H, OP)                                                   \
    ...
    {                                                                                \
        ...
        for (i = 0; i < oprsz / sizeof(TYPE); i += segment) {                        \
            TYPE mm = m[i];                                                          \
            for (j = 0; j < segment; j++) { d[i + j] = OP(n[i + j], mm, a[i + j]); } \
        }                                                                            \
    }
...
#define DO_SQRDMLAH_H(N, M, A)                         \
    ({                                                 \
        uint32_t discard;                              \
        do_sqrdmlah_h(N, M, A, false, true, &discard); \
    })
#define DO_SQRDMLAH_S(N, M, A)                         \
    ({                                                 \
        uint32_t discard;                              \
        do_sqrdmlah_s(N, M, A, false, true, &discard); \
    })
...
#define DO_SQRDMLSH_H(N, M, A)                        \
    ({                                                \
        uint32_t discard;                             \
        do_sqrdmlah_h(N, M, A, true, true, &discard); \
    })
#define DO_SQRDMLSH_S(N, M, A)                        \
    ({                                                \
        uint32_t discard;                             \
        do_sqrdmlah_s(N, M, A, true, true, &discard); \
    })
    ...
    {                                                                              \
        ...
        for (uintptr_t s = 0; s < nseg; s++) {                                     \
            ...
            for (uintptr_t e = 0; e < nsegelt; e++, pg >>= sizeof(TYPE)) {         \
                if (pg & 1) { tmp[e] = OP(tmp[H(e)], n[s * nsegelt + H(e)]); }     \
            }                                                                      \
        }                                                                          \
        memcpy(vd, tmp, 16);                                                       \
...
#define DO_ZZI(NAME, TYPE, OP)                                         \
    ...
    {                                                                  \
        ...
        for (i = 0; i < opr_sz; ++i) { d[i] = OP(n[i], s); }           \
    }
...
static intptr_t last_active_element(uint64_t* g, intptr_t words, intptr_t esz)
{
    ...
    do {
        ...
        if (this_g) { return i * 64 + (63 - clz64(this_g)); }
    }
    while (i > 0);
...
uint32_t HELPER(sve_pnext)(void* vd, void* vg, uint32_t pred_desc)
{
    ...
    do {
        uint64_t this_d = 0;
        if (i == next / 64) { this_d = 1ull << (next & 63); }
        d[i]  = this_d;
...
void HELPER(sve_movz_b)(void* vd, void* vn, void* vg, uint32_t desc)
{
    ...
    for (i = 0; i < opr_sz; i += 1) { d[i] = n[i] & (expand_pred_b(pg[H1(i)]) ^ inv); }
}
...
void HELPER(sve_movz_h)(void* vd, void* vn, void* vg, uint32_t desc)
{
    ...
    for (i = 0; i < opr_sz; i += 1) { d[i] = n[i] & (expand_pred_h(pg[H1(i)]) ^ inv); }
}
...
void HELPER(sve_movz_s)(void* vd, void* vn, void* vg, uint32_t desc)
{
    ...
    for (i = 0; i < opr_sz; i += 1) { d[i] = n[i] & (expand_pred_s(pg[H1(i)]) ^ inv); }
}
...
void HELPER(sve_movz_d)(void* vd, void* vn, void* vg, uint32_t desc)
{
    ...
    for (i = 0; i < opr_sz; i += 1) { d[i] = n[i] & -(uint64_t)((pg[H1(i)] ^ inv) & 1); }
}
...
#define do_suqrshl_b(n, m)                                \
    ({                                                    \
        uint32_t discard;                                 \
        do_suqrshl_bhs(n, (int8_t)m, 8, false, &discard); \
    })
#define do_suqrshl_h(n, m)                                  \
    ({                                                      \
        uint32_t discard;                                   \
        do_suqrshl_bhs(n, (int16_t)m, 16, false, &discard); \
    })
#define do_suqrshl_s(n, m)                         \
    ({                                             \
        uint32_t discard;                          \
        do_suqrshl_bhs(n, m, 32, false, &discard); \
    })
#define do_suqrshl_d(n, m)                   \
    ({                                       \
        uint32_t discard;                    \
        do_suqrshl_d(n, m, false, &discard); \
    })
...
#define DO_ZPZZZ(NAME, TYPE, H, OP)                                                    \
    void HELPER(NAME)(void* vd, void* va, void* vn, void* vm, void* vg, uint32_t desc) \
    {                                                                                  \
        intptr_t i, opr_sz = simd_oprsz(desc);                                         \
        for (i = 0; i < opr_sz;) {                                                     \
            uint16_t pg = *(uint16_t*)(vg + H1_2(i >> 3));                             \
...
void HELPER(sve_index_h)(void* vd, uint32_t start, uint32_t incr, uint32_t desc)
{
    ...
    uint16_t* d = vd;
    for (i = 0; i < opr_sz; i += 1) { d[H2(i)] = start + i * incr; }
}
...
void HELPER(sve_index_s)(void* vd, uint32_t start, uint32_t incr, uint32_t desc)
{
    ...
    uint32_t* d = vd;
    for (i = 0; i < opr_sz; i += 1) { d[H4(i)] = start + i * incr; }
}
...
void HELPER(sve_index_d)(void* vd, uint64_t start, uint64_t incr, uint32_t desc)
{
    intptr_t  i, opr_sz = simd_oprsz(desc) / 8;
    uint64_t* d = vd;
    for (i = 0; i < opr_sz; i += 1) { d[i] = start + i * incr; }
}
...
void HELPER(sve_adr_p32)(void* vd, void* vn, void* vm, uint32_t desc)
{
    ...
    for (i = 0; i < opr_sz; i += 1) { d[i] = n[i] + (m[i] << sh); }
}
...
void HELPER(sve_adr_s32)(void* vd, void* vn, void* vm, uint32_t desc)
{
    ...
    uint64_t  sh = simd_data(desc);
    uint64_t *d = vd, *n = vn, *m = vm;
    for (i = 0; i < opr_sz; i += 1) { d[i] = n[i] + ((uint64_t)(int32_t)m[i] << sh); }
}
...
void HELPER(sve_adr_u32)(void* vd, void* vn, void* vm, uint32_t desc)
{
    ...
    uint64_t  sh = simd_data(desc);
    uint64_t *d = vd, *n = vn, *m = vm;
    for (i = 0; i < opr_sz; i += 1) { d[i] = n[i] + ((uint64_t)(uint32_t)m[i] << sh); }
}
...
void HELPER(sve_fexpa_h)(void* vd, void* vn, uint32_t desc)
{
    ...
    static const uint16_t coeff[] = {
        0x0000, 0x0016, 0x002d, 0x0045, 0x005d, 0x0075, 0x008e, 0x00a8, 0x00c2, 0x00dc, 0x00f8,
        0x0114, 0x0130, 0x014d, 0x016b, 0x0189, 0x01a8, 0x01c8, 0x01e8, 0x0209, 0x022b, 0x024e,
        0x0271, 0x0295, 0x02ba, 0x02e0, 0x0306, 0x032e, 0x0356, 0x037f, 0x03a9, 0x03d4,
    };
    intptr_t  i, opr_sz = simd_oprsz(desc) / 2;
    uint16_t *d = vd, *n = vn;
...
void HELPER(sve_fexpa_s)(void* vd, void* vn, uint32_t desc)
{
    ...
    static const uint32_t coeff[] = {
        0x000000, 0x0164d2, 0x02cd87, 0x043a29, 0x05aac3, 0x071f62, 0x08980f, 0x0a14d5, 0x0b95c2, 0x0d1adf, 0x0ea43a,
        0x1031dc, 0x11c3d3, 0x135a2b, 0x14f4f0, 0x16942d, 0x1837f0, 0x19e046, 0x1b8d3a, 0x1d3eda, 0x1ef532, 0x20b051,
        0x227043, 0x243516, 0x25fed7, 0x27cd94, 0x29a15b, 0x2b7a3a, 0x2d583f, 0x2f3b79, 0x3123f6, 0x3311c4, 0x3504f3,
        0x36fd92, 0x38fbaf, 0x3aff5b, 0x3d08a4, 0x3f179a, 0x412c4d, 0x4346cd, 0x45672a, 0x478d75, 0x49b9be, 0x4bec15,
        0x4e248c, 0x506334, 0x52a81e, 0x54f35b, 0x5744fd, 0x599d16, 0x5bfbb8, 0x5e60f5, 0x60ccdf, 0x633f89, 0x65b907,
        0x68396a, 0x6ac0c7, 0x6d4f30, 0x6fe4ba, 0x728177, 0x75257d, 0x77d0df, 0x7a83b3, 0x7d3e0c,
    };
    intptr_t  i, opr_sz = simd_oprsz(desc) / 4;
    uint32_t *d = vd, *n = vn;
...
void HELPER(sve_fexpa_d)(void* vd, void* vn, uint32_t desc)
{
    ...
    static const uint64_t coeff[] = {
        0x0000000000000ull, 0x02C9A3E778061ull, 0x059B0D3158574ull, 0x0874518759BC8ull, 0x0B5586CF9890Full,
        0x0E3EC32D3D1A2ull, 0x11301D0125B51ull, 0x1429AAEA92DE0ull, 0x172B83C7D517Bull, 0x1A35BEB6FCB75ull,
        0x1D4873168B9AAull, 0x2063B88628CD6ull, 0x2387A6E756238ull, 0x26B4565E27CDDull, 0x29E9DF51FDEE1ull,
        0x2D285A6E4030Bull, 0x306FE0A31B715ull, 0x33C08B26416FFull, 0x371A7373AA9CBull, 0x3A7DB34E59FF7ull,
        0x3DEA64C123422ull, 0x4160A21F72E2Aull, 0x44E086061892Dull, 0x486A2B5C13CD0ull, 0x4BFDAD5362A27ull,
        0x4F9B2769D2CA7ull, 0x5342B569D4F82ull, 0x56F4736B527DAull, 0x5AB07DD485429ull, 0x5E76F15AD2148ull,
        0x6247EB03A5585ull, 0x6623882552225ull, 0x6A09E667F3BCDull, 0x6DFB23C651A2Full, 0x71F75E8EC5F74ull,
        0x75FEB564267C9ull, 0x7A11473EB0187ull, 0x7E2F336CF4E62ull, 0x82589994CCE13ull, 0x868D99B4492EDull,
        0x8ACE5422AA0DBull, 0x8F1AE99157736ull, 0x93737B0CDC5E5ull, 0x97D829FDE4E50ull, 0x9C49182A3F090ull,
        0xA0C667B5DE565ull, 0xA5503B23E255Dull, 0xA9E6B5579FDBFull, 0xAE89F995AD3ADull, 0xB33A2B84F15FBull,
        0xB7F76F2FB5E47ull, 0xBCC1E904BC1D2ull, 0xC199BDD85529Cull, 0xC67F12E57D14Bull, 0xCB720DCEF9069ull,
        0xD072D4A07897Cull, 0xD5818DCFBA487ull, 0xDA9E603DB3285ull, 0xDFC97337B9B5Full, 0xE502EE78B3FF6ull,
        0xEA4AFA2A490DAull, 0xEFA1BEE615A27ull, 0xF50765B6E4540ull, 0xFA7C1819E90D8ull,
    };
    intptr_t  i, opr_sz = simd_oprsz(desc) / 8;
    uint64_t *d = vd, *n = vn;
...
void HELPER(sve_ftssel_h)(void* vd, void* vn, void* vm, uint32_t desc)
{
    ...
    for (i = 0; i < opr_sz; i += 1) {
        uint16_t nn = n[i];
        uint16_t mm = m[i];
        if (mm & 1) { nn = float16_one; }
        if (mm & 2) { nn = float16_maybe_ah_chs(nn, fpcr_ah); }
        d[i] = nn;
    }
}
...
void HELPER(sve_ftssel_s)(void* vd, void* vn, void* vm, uint32_t desc)
{
    ...
    for (i = 0; i < opr_sz; i += 1) {
        uint32_t nn = n[i];
        uint32_t mm = m[i];
        if (mm & 1) { nn = float32_one; }
        if (mm & 2) { nn = float32_maybe_ah_chs(nn, fpcr_ah); }
        d[i] = nn;
    }
}
...
void HELPER(sve_ftssel_d)(void* vd, void* vn, void* vm, uint32_t desc)
{
    ...
    for (i = 0; i < opr_sz; i += 1) {
        uint64_t nn = n[i];
        uint64_t mm = m[i];
        if (mm & 1) { nn = float64_one; }
        if (mm & 2) { nn = float64_maybe_ah_chs(nn, fpcr_ah); }
        d[i] = nn;
    }
}
...
void HELPER(sve_sqaddi_b)(void* d, void* a, int32_t b, uint32_t desc)
{
    ...
    for (i = 0; i < oprsz; i += sizeof(int8_t)) { *(int8_t*)(d + i) = DO_SQADD_B(b, *(int8_t*)(a + i)); }
}
...
void HELPER(sve_sqaddi_h)(void* d, void* a, int32_t b, uint32_t desc)
{
    ...
    for (i = 0; i < oprsz; i += sizeof(int16_t)) { *(int16_t*)(d + i) = DO_SQADD_H(b, *(int16_t*)(a + i)); }
}
...
void HELPER(sve_sqaddi_s)(void* d, void* a, int64_t b, uint32_t desc)
{
    ...
    for (i = 0; i < oprsz; i += sizeof(int32_t)) { *(int32_t*)(d + i) = DO_SQADD_S(b, *(int32_t*)(a + i)); }
}
...
void HELPER(sve_sqaddi_d)(void* d, void* a, int64_t b, uint32_t desc)
{
    ...
    for (i = 0; i < oprsz; i += sizeof(int64_t)) { *(int64_t*)(d + i) = do_sqadd_d(b, *(int64_t*)(a + i)); }
}
...
void HELPER(sve_uqaddi_b)(void* d, void* a, int32_t b, uint32_t desc)
{
    ...
    for (i = 0; i < oprsz; i += sizeof(uint8_t)) { *(uint8_t*)(d + i) = DO_UQADD_B(b, *(uint8_t*)(a + i)); }
}
...
void HELPER(sve_uqaddi_h)(void* d, void* a, int32_t b, uint32_t desc)
{
    ...
    for (i = 0; i < oprsz; i += sizeof(uint16_t)) { *(uint16_t*)(d + i) = DO_UQADD_H(b, *(uint16_t*)(a + i)); }
}
...
void HELPER(sve_uqaddi_s)(void* d, void* a, int64_t b, uint32_t desc)
{
    ...
    for (i = 0; i < oprsz; i += sizeof(uint32_t)) { *(uint32_t*)(d + i) = DO_UQADD_S(b, *(uint32_t*)(a + i)); }
}
...
void HELPER(sve_uqaddi_d)(void* d, void* a, uint64_t b, uint32_t desc)
{
    ...
    for (i = 0; i < oprsz; i += sizeof(uint64_t)) { *(uint64_t*)(d + i) = do_uqadd_d(b, *(uint64_t*)(a + i)); }
}
...
void HELPER(sve_uqsubi_d)(void* d, void* a, uint64_t b, uint32_t desc)
{
    ...
    for (i = 0; i < oprsz; i += sizeof(uint64_t)) { *(uint64_t*)(d + i) = do_uqsub_d(*(uint64_t*)(a + i), b); }
}
 ...
 */
void HELPER(sve_cpy_m_b)(void* vd, void* vn, void* vg, uint64_t mm, uint32_t desc)
{
    intptr_t  i, opr_sz = simd_oprsz(desc) / 8;
    uint64_t *d = vd, *n = vn;
    uint8_t*  pg = vg;
...
void HELPER(sve_cpy_m_h)(void* vd, void* vn, void* vg, uint64_t mm, uint32_t desc)
{
    intptr_t  i, opr_sz = simd_oprsz(desc) / 8;
    uint64_t *d = vd, *n = vn;
    uint8_t*  pg = vg;
...
void HELPER(sve_cpy_m_s)(void* vd, void* vn, void* vg, uint64_t mm, uint32_t desc)
{
    intptr_t  i, opr_sz = simd_oprsz(desc) / 8;
    uint64_t *d = vd, *n = vn;
    uint8_t*  pg = vg;
...
void HELPER(sve_cpy_m_d)(void* vd, void* vn, void* vg, uint64_t mm, uint32_t desc)
{
    intptr_t  i, opr_sz = simd_oprsz(desc) / 8;
    uint64_t *d = vd, *n = vn;
    uint8_t*  pg = vg;
...
void HELPER(sve_cpy_z_b)(void* vd, void* vg, uint64_t val, uint32_t desc)
{
    ...
    for (i = 0; i < opr_sz; i += 1) { d[i] = val & expand_pred_b(pg[H1(i)]); }
}
...
void HELPER(sve_cpy_z_h)(void* vd, void* vg, uint64_t val, uint32_t desc)
{
    ...
    for (i = 0; i < opr_sz; i += 1) { d[i] = val & expand_pred_h(pg[H1(i)]); }
}
...
void HELPER(sve_cpy_z_s)(void* vd, void* vg, uint64_t val, uint32_t desc)
{
    ...
    for (i = 0; i < opr_sz; i += 1) { d[i] = val & expand_pred_s(pg[H1(i)]); }
}
...
void HELPER(sve_cpy_z_d)(void* vd, void* vg, uint64_t val, uint32_t desc)
{
    ...
    for (i = 0; i < opr_sz; i += 1) { d[i] = (pg[H1(i)] & 1 ? val : 0); }
}
...
#endif
    switch (o) {
        case 0: memmove(vd, vs, n); break;

        case 4:
            if (d < s || d >= s + n) {
                for (i = 0; i < n; i += 4) { *(uint32_t*)H1_4(d + i) = *(uint32_t*)H1_4(s + i); }
            }
            else {
                for (i = n; i > 0;) {
                    i                       -= 4;
        ...
        case 6:
            if (d < s || d >= s + n) {
                for (i = 0; i < n; i += 2) { *(uint16_t*)H1_2(d + i) = *(uint16_t*)H1_2(s + i); }
            }
            else {
                for (i = n; i > 0;) {
                    i                       -= 2;
        ...
        default:
            if (d < s || d >= s + n) {
                for (i = 0; i < n; i++) { *(uint8_t*)H1(d + i) = *(uint8_t*)H1(s + i); }
            }
            else {
                for (i = n; i > 0;) {
                    i                    -= 1;
...
#endif
    switch (o) {
        case 0: memset(vd, 0, n); break;

        case 4:
            for (i = 0; i < n; i += 4) { *(uint32_t*)H1_4(d + i) = 0; }
            break;

        case 2:
        case 6:
            for (i = 0; i < n; i += 2) { *(uint16_t*)H1_2(d + i) = 0; }
            break;

        default:
            for (i = 0; i < n; i++) { *(uint8_t*)H1(d + i) = 0; }
            break;
    }
}
...
static inline void do_tbl1(void* vd, void* vn, void* vm, uint32_t desc, bool is_tbx, tb_impl_fn* fn)
{
    ARMVectorReg scratch;
    uintptr_t    oprsz = simd_oprsz(desc);

    if (unlikely(vd == vn)) { vn = memcpy(&scratch, vn, oprsz); }

    fn(vd, vn, NULL, vm, oprsz, is_tbx);
}

static inline void do_tbl2(void* vd, void* vn0, void* vn1, void* vm, uint32_t desc, bool is_tbx, tb_impl_fn* fn)
{
    ARMVectorReg scratch;
    uintptr_t    oprsz = simd_oprsz(desc);
...
#define DO_TB(SUFF, TYPE, H)                                                                                \
    static inline void do_tb_##SUFF(void* vd, void* vt0, void* vt1, void* vm, uintptr_t oprsz, bool is_tbx) \
    {                                                                                                       \
        TYPE *    d = vd, *tbl0 = vt0, *tbl1 = vt1, *indexes = vm;                                          \
        uintptr_t i, nelem = oprsz / sizeof(TYPE);                                                          \
        for (i = 0; i < nelem; ++i) {                                                                       \
            TYPE index = indexes[H1(i)], val = 0;                                                           \
            if (index < nelem) { val = tbl0[H(index)]; }                                                    \
            else {                                                                                          \
                index -= nelem;                                                                             \
                if (tbl1 && index < nelem) { val = tbl1[H(index)]; }                                        \
                else if (is_tbx) {                                                                          \
                    continue;                                                                               \
                }                                                                                           \
            }                                                                                               \
    ...
    void HELPER(sve2_tbl_##SUFF)(void* vd, void* vn0, void* vn1, void* vm, uint32_t desc)                   \
...
#define DO_UNPK(NAME, TYPED, TYPES, HD, HS)                                   \
    ...
    {                                                                         \
        ...
        TYPES*       n = vn;                                                  \
        ARMVectorReg tmp;                                                     \
        if (unlikely(vn - vd < opr_sz)) { n = memcpy(&tmp, n, opr_sz / 2); }  \
        for (i = 0; i < opr_sz / sizeof(TYPED); i++) { d[HD(i)] = n[HS(i)]; } \
    }
...
static const uint64_t even_bit_esz_masks[5] = {
    0x5555555555555555ull, 0x3333333333333333ull, 0x0f0f0f0f0f0f0f0full, 0x00ff00ff00ff00ffull, 0x0000ffff0000ffffull,
};
...
void HELPER(sve_uzp_p)(void* vd, void* vn, void* vm, uint32_t pred_desc)
{
    ...
    if (oprsz <= 8) {
        ...
        if ((vm - vd) < (uintptr_t)oprsz) { m = memcpy(&tmp_m, vm, oprsz); }
...
static uint8_t reverse_bits_8(uint8_t x, int n)
{
    ...
    for (i = 2, sh = 4; i >= n; i--, sh >>= 1) { x = ((x & mask[i]) << sh) | ((x >> sh) & mask[i]); }
    return x;
}
...
void HELPER(sve_punpk_p)(void* vd, void* vn, uint32_t pred_desc)
{
    ...
    if (oprsz <= 8) {
        ...
        /* We produce output faster than we consume input.
           Therefore we must be mindful of possible overlap.  */
        if ((vn - vd) < (uintptr_t)oprsz) { vn = memcpy(&tmp_n, vn, oprsz); }
        if (high) { high = oprsz >> 1; }
...
#define DO_ZIP(NAME, TYPE, H)                                                               \
    ...
    {                                                                                       \
        ...
        /* We produce output faster than we consume input.                                  \
           Therefore we must be mindful of possible overlap.  */                            \
        if (unlikely((vn - vd) < (uintptr_t)oprsz)) { vn = memcpy(&tmp_n, vn, oprsz); }     \
        if (unlikely((vm - vd) < (uintptr_t)oprsz)) { vm = memcpy(&tmp_m, vm, oprsz); }     \
        for (i = 0; i < oprsz_2; i += sizeof(TYPE)) {                                       \
            *(TYPE*)(vd + H(2 * i + 0))            = *(TYPE*)(vn + odd_ofs + H(i));         \
            *(TYPE*)(vd + H(2 * i + sizeof(TYPE))) = *(TYPE*)(vm + odd_ofs + H(i));         \
        }                                                                                   \
        if (sizeof(TYPE) == 16 && unlikely(oprsz & 16)) { memset(vd + oprsz - 16, 0, 16); } \
    }
...
static void do_perseg_zzz(void* vd, void* vn, void* vm, uint32_t desc, perseg_zzz_fn* fn)
{
    intptr_t oprsz = simd_oprsz(desc);

    desc = simd_desc(16, 16, simd_data(desc));
    for (intptr_t i = 0; i < oprsz; i += 16) { fn(vd + i, vn + i, vm + i, desc); }
}

#define DO_PERSEG_ZZZ(NAME, FUNC)                                                                             \
    void HELPER(NAME)(void* vd, void* vn, void* vm, uint32_t desc) { do_perseg_zzz(vd, vn, vm, desc, FUNC); }
...
void HELPER(sve_splice)(void* vd, void* vn, void* vm, void* vg, uint32_t desc)
{
    ...
    if (first_g != 0) {
        ...
        if (vd == vm) { vm = memcpy(&tmp, vm, opr_sz * 8); }
        swap_memmove(vd, vn + first_i, len);
    }
    swap_memmove(vd + len, vm, opr_sz * 8 - len);
}

void HELPER(sve_sel_zpzz_b)(void* vd, void* vn, void* vm, void* vg, uint32_t desc)
{
    intptr_t  i, opr_sz = simd_oprsz(desc) / 8;
    uint64_t *d = vd, *n = vn, *m = vm;
    uint8_t*  pg = vg;
...
void HELPER(sve_sel_zpzz_h)(void* vd, void* vn, void* vm, void* vg, uint32_t desc)
{
    intptr_t  i, opr_sz = simd_oprsz(desc) / 8;
    uint64_t *d = vd, *n = vn, *m = vm;
    uint8_t*  pg = vg;
...
void HELPER(sve_sel_zpzz_s)(void* vd, void* vn, void* vm, void* vg, uint32_t desc)
{
    intptr_t  i, opr_sz = simd_oprsz(desc) / 8;
    uint64_t *d = vd, *n = vn, *m = vm;
    uint8_t*  pg = vg;
...
void HELPER(sve_sel_zpzz_d)(void* vd, void* vn, void* vm, void* vg, uint32_t desc)
{
    intptr_t  i, opr_sz = simd_oprsz(desc) / 8;
    uint64_t *d = vd, *n = vn, *m = vm;
    uint8_t*  pg = vg;
...
void HELPER(sve_sel_zpzz_q)(void* vd, void* vn, void* vm, void* vg, uint32_t desc)
{
    intptr_t  i, opr_sz = simd_oprsz(desc) / 16;
    Int128 *  d = vd, *n = vn, *m = vm;
    uint16_t* pg = vg;

    for (i = 0; i < opr_sz; i += 1) { d[i] = (pg[H2(i)] & 1 ? n : m)[i]; }
}
...
#define DO_CMP_PPZZ_B(NAME, TYPE, OP) DO_CMP_PPZZ(NAME, TYPE, OP, H1, 0xffffffffffffffffull)
#define DO_CMP_PPZZ_H(NAME, TYPE, OP) DO_CMP_PPZZ(NAME, TYPE, OP, H1_2, 0x5555555555555555ull)
#define DO_CMP_PPZZ_S(NAME, TYPE, OP) DO_CMP_PPZZ(NAME, TYPE, OP, H1_4, 0x1111111111111111ull)
#define DO_CMP_PPZZ_D(NAME, TYPE, OP) DO_CMP_PPZZ(NAME, TYPE, OP, H1_8, 0x0101010101010101ull)
...
#define DO_CMP_PPZW_B(NAME, TYPE, TYPEW, OP) DO_CMP_PPZW(NAME, TYPE, TYPEW, OP, H1, 0xffffffffffffffffull)
#define DO_CMP_PPZW_H(NAME, TYPE, TYPEW, OP) DO_CMP_PPZW(NAME, TYPE, TYPEW, OP, H1_2, 0x5555555555555555ull)
#define DO_CMP_PPZW_S(NAME, TYPE, TYPEW, OP) DO_CMP_PPZW(NAME, TYPE, TYPEW, OP, H1_4, 0x1111111111111111ull)
...
#define DO_CMP_PPZI_B(NAME, TYPE, OP) DO_CMP_PPZI(NAME, TYPE, OP, H1, 0xffffffffffffffffull)
#define DO_CMP_PPZI_H(NAME, TYPE, OP) DO_CMP_PPZI(NAME, TYPE, OP, H1_2, 0x5555555555555555ull)
#define DO_CMP_PPZI_S(NAME, TYPE, OP) DO_CMP_PPZI(NAME, TYPE, OP, H1_4, 0x1111111111111111ull)
#define DO_CMP_PPZI_D(NAME, TYPE, OP) DO_CMP_PPZI(NAME, TYPE, OP, H1_8, 0x0101010101010101ull)
...
static bool last_active_pred(void* vd, void* vg, intptr_t oprsz)
{
    ...
    for (i = QEMU_ALIGN_UP(oprsz, 8) - 8; i >= 0; i -= 8) {
        ...
        if (pg) { return (pow2floor(pg) & *(uint64_t*)(vd + i)) != 0; }
    }
    return 0;
}
 ...
 */
static bool compute_brk(uint64_t* retb, uint64_t n, uint64_t g, bool brk, bool after)
{
    uint64_t b;
    ...
    if (brk) { b = 0; }
        ...
        b = b & -b; /* first such */
        if (after) { b = b | (b - 1); /* break after same */ }
        else {
...
static void compute_brk_z(uint64_t* d, uint64_t* n, uint64_t* g, intptr_t oprsz, bool after)
{
    bool     brk = false;
    intptr_t i;
...
static uint32_t compute_brks_z(uint64_t* d, uint64_t* n, uint64_t* g, intptr_t oprsz, bool after)
{
    uint32_t flags = PREDTEST_INIT;
    bool     brk   = false;
    intptr_t i;
...
static void compute_brk_m(uint64_t* d, uint64_t* n, uint64_t* g, intptr_t oprsz, bool after)
{
    bool     brk = false;
    intptr_t i;
...
/* Likewise, but also compute flags.  */
static uint32_t compute_brks_m(uint64_t* d, uint64_t* n, uint64_t* g, intptr_t oprsz, bool after)
{
    uint32_t flags = PREDTEST_INIT;
    bool     brk   = false;
    intptr_t i;
...
void HELPER(sve_brkpa)(void* vd, void* vn, void* vm, void* vg, uint32_t pred_desc)
{
    intptr_t oprsz = REG_FIELD_EX32(pred_desc, PREDDESC, OPRSZ);
    if (last_active_pred(vn, vg, oprsz)) { compute_brk_z(vd, vm, vg, oprsz, true); }
    else {
        memset(vd, 0, sizeof(ARMPredicateReg));
    }
}

uint32_t HELPER(sve_brkpas)(void* vd, void* vn, void* vm, void* vg, uint32_t pred_desc)
{
    intptr_t oprsz = REG_FIELD_EX32(pred_desc, PREDDESC, OPRSZ);
    if (last_active_pred(vn, vg, oprsz)) { return compute_brks_z(vd, vm, vg, oprsz, true); }
    else {
        memset(vd, 0, sizeof(ARMPredicateReg));
...
void HELPER(sve_brkpb)(void* vd, void* vn, void* vm, void* vg, uint32_t pred_desc)
{
    intptr_t oprsz = REG_FIELD_EX32(pred_desc, PREDDESC, OPRSZ);
    if (last_active_pred(vn, vg, oprsz)) { compute_brk_z(vd, vm, vg, oprsz, false); }
    else {
        memset(vd, 0, sizeof(ARMPredicateReg));
    }
}

uint32_t HELPER(sve_brkpbs)(void* vd, void* vn, void* vm, void* vg, uint32_t pred_desc)
{
    intptr_t oprsz = REG_FIELD_EX32(pred_desc, PREDDESC, OPRSZ);
    if (last_active_pred(vn, vg, oprsz)) { return compute_brks_z(vd, vm, vg, oprsz, false); }
    else {
        memset(vd, 0, sizeof(ARMPredicateReg));
        return PREDTEST_INIT;
    }
...
{
    intptr_t oprsz = REG_FIELD_EX32(pred_desc, PREDDESC, OPRSZ);
    if (!last_active_pred(vn, vg, oprsz)) { memset(vd, 0, sizeof(ARMPredicateReg)); }
}
...
uint32_t HELPER(sve_brkns)(void* vd, void* vn, void* vg, uint32_t pred_desc)
{
    ...
    if (last_active_pred(vn, vg, oprsz)) {
        ...
        for (i = 0; i < oprsz / 8; i++) { flags = iter_predtest_fwd(d->p[i], -1, flags); }
        if (oprsz & 7) {
...
static uint64_t encode_pred_count(uint32_t elements, uint32_t count, uint32_t esz, bool invert)
{
    uint32_t pred;

    if (count == 0) { return 0; }
    if (invert) { count = elements - count; }
...
static void do_whilel(ARMPredicateReg* d, uint64_t esz_mask, uint32_t count, uint32_t oprbits)
{
    ...
    if (count) {
        ...
        for (i = 0; i < count / 64; ++i) { d->p[i] = esz_mask; }
        if (count & 63) { d->p[i] = MAKE_64BIT_MASK(0, count & 63) & esz_mask; }
    }
}
...
uint32_t HELPER(sve_while2l)(void* vd, uint32_t count, uint32_t pred_desc)
{
    ...
    if (count <= oprbits) { do_whilel(&d[0], esz_mask, count, oprbits); }
    else {
...
uint32_t HELPER(sve_whilecl)(void* vd, uint32_t count, uint32_t pred_desc)
{
    ...
    *d = (ARMPredicateReg){.p[0] = encode_pred_count(elements, count, esz, false)};
    return pred_count_test(elements, count, false);
}

/* D must be cleared on entry. */
static void do_whileg(ARMPredicateReg* d, uint64_t esz_mask, uint32_t count, uint32_t oprbits)
{
    tcg_debug_assert(count <= oprbits);
    if (count) {
        ...
        if (oprbits & 63) { d->p[i] = bits & MAKE_64BIT_MASK(0, oprbits & 63); }
    }
}
...
uint32_t HELPER(sve_while2g)(void* vd, uint32_t count, uint32_t pred_desc)
{
    ...
    count <<= esz;
    memset(d, 0, 2 * sizeof(*d));
    if (count <= oprbits) { do_whileg(&d[1], esz_mask, count, oprbits); }
    else {
...
uint32_t HELPER(sve_whilecg)(void* vd, uint32_t count, uint32_t pred_desc)
{
    ...
    *d = (ARMPredicateReg){.p[0] = encode_pred_count(elements, count, esz, true)};
...
#define DO_REDUCE(NAME, SUF, TYPE, H, FUNC, IDENT)                                                         \
    ...
    uint64_t helper_sve_##NAME##v_##SUF(void* vn, void* vg, float_status* status, uint32_t desc)           \
    {                                                                                                      \
        ...
        for (; i < maxsz; i += sizeof(TYPE)) { *(TYPE*)((void*)data + i) = ident; }                        \
        return FUNC##_reduce(data, status, maxsz / sizeof(TYPE));                                          \
    }                                                                                                      \
    void helper_sve2p1_##NAME##qv_##SUF(void* vd, void* vn, void* vg, float_status* status, uint32_t desc) \
    {                                                                                                      \
...
DO_REDUCE(ah_fmax, h, float16, H1_2, helper_vfp_ah_maxh, float16_chs(float16_infinity))
DO_REDUCE(ah_fmax, s, float32, H1_4, helper_vfp_ah_maxs, float32_chs(float32_infinity))
DO_REDUCE(ah_fmax, d, float64, H1_8, helper_vfp_ah_maxd, float64_chs(float64_infinity))

#undef DO_REDUCE

uint64_t HELPER(sve_fadda_h)(uint64_t nn, void* vm, void* vg, float_status* status, uint32_t desc)
{
...
uint64_t HELPER(sve_fadda_s)(uint64_t nn, void* vm, void* vg, float_status* status, uint32_t desc)
{
    intptr_t i = 0, opr_sz = simd_oprsz(desc);
    float32  result = nn;
...
uint64_t HELPER(sve_fadda_d)(uint64_t nn, void* vm, void* vg, float_status* status, uint32_t desc)
{
    ...
    for (i = 0; i < opr_sz; i++) {
        if (pg[H1(i)] & 1) { nn = float64_add(nn, m[i], status); }
    }
...
static inline float16 abd_h(float16 a, float16 b, float_status* s) { return float16_abs(float16_sub(a, b, s)); }

static inline float32 abd_s(float32 a, float32 b, float_status* s) { return float32_abs(float32_sub(a, b, s)); }

static inline float64 abd_d(float64 a, float64 b, float_status* s) { return float64_abs(float64_sub(a, b, s)); }
...
#define DO_ZPZS_FP(NAME, TYPE, H, OP)                                                                     \
    void HELPER(NAME)(void* vd, void* vn, void* vg, uint64_t scalar, float_status* status, uint32_t desc) \
    {                                                                                                     \
        intptr_t  i  = simd_oprsz(desc);                                                                  \
        uint64_t* g  = vg;                                                                                \
        TYPE      mm = scalar;                                                                            \
...
static inline float16 subr_h(float16 a, float16 b, float_status* s) { return float16_sub(b, a, s); }

static inline float32 subr_s(float32 a, float32 b, float_status* s) { return float32_sub(b, a, s); }

static inline float64 subr_d(float64 a, float64 b, float_status* s) { return float64_sub(b, a, s); }
...
#define DO_ZPZ_FP(NAME, TYPE, H, OP)                                                     \
    void HELPER(NAME)(void* vd, void* vn, void* vg, float_status* status, uint32_t desc) \
    {                                                                                    \
        intptr_t  i = simd_oprsz(desc);                                                  \
        uint64_t* g = vg;                                                                \
        do {                                                                             \
...
static int16_t do_float16_logb_as_int(float16 a, float_status* s)
{
    ...
    if (unlikely(exp == 0)) {
        ...
        if (frac == 0) { return INT16_MAX; /* infinity */ }
    }
    else {
        /* normal: exp - bias */
        return exp - 15;
...
static int32_t do_float32_logb_as_int(float32 a, float_status* s)
{
    ...
    if (unlikely(exp == 0)) {
        ...
        if (frac == 0) { return INT32_MAX; /* infinity */ }
    }
    else {
        /* normal: exp - bias */
        return exp - 127;
...
static int64_t do_float64_logb_as_int(float64 a, float_status* s)
{
    ...
    if (unlikely(exp == 0)) {
        ...
        if (frac == 0) { return INT64_MAX; /* infinity */ }
    }
    else {
        /* normal: exp - bias */
        return exp - 1023;
...
static void do_fmla_zpzzz_b16(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc,
...
void HELPER(sve_fmla_zpzzz_b16)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_b16(vd, vn, vm, va, vg, status, desc, 0, 0, 0); }

void HELPER(sve_fmls_zpzzz_b16)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_b16(vd, vn, vm, va, vg, status, desc, 0x8000, 0, 0); }

void HELPER(sve_fnmla_zpzzz_b16)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_b16(vd, vn, vm, va, vg, status, desc, 0x8000, 0x8000, 0); }

void HELPER(sve_fnmls_zpzzz_b16)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_b16(vd, vn, vm, va, vg, status, desc, 0, 0x8000, 0); }

void HELPER(sve_ah_fmls_zpzzz_b16)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status,
                                   uint32_t desc)
{ do_fmla_zpzzz_b16(vd, vn, vm, va, vg, status, desc, 0, 0, float_muladd_negate_product); }

void HELPER(sve_ah_fnmla_zpzzz_b16)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status,
                                    uint32_t desc)
{ do_fmla_zpzzz_b16(vd, vn, vm, va, vg, status, desc, 0, 0, float_muladd_negate_product | float_muladd_negate_c); }

void HELPER(sve_ah_fnmls_zpzzz_b16)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status,
                                    uint32_t desc)
{ do_fmla_zpzzz_b16(vd, vn, vm, va, vg, status, desc, 0, 0, float_muladd_negate_c); }

static void do_fmla_zpzzz_h(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc,
                            uint16_t neg1, uint16_t neg3, int flags)
{
    intptr_t  i = simd_oprsz(desc);
    uint64_t* g = vg;
...
void HELPER(sve_fmla_zpzzz_h)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_h(vd, vn, vm, va, vg, status, desc, 0, 0, 0); }

void HELPER(sve_fmls_zpzzz_h)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_h(vd, vn, vm, va, vg, status, desc, 0x8000, 0, 0); }

void HELPER(sve_fnmla_zpzzz_h)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_h(vd, vn, vm, va, vg, status, desc, 0x8000, 0x8000, 0); }

void HELPER(sve_fnmls_zpzzz_h)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_h(vd, vn, vm, va, vg, status, desc, 0, 0x8000, 0); }

void HELPER(sve_ah_fmls_zpzzz_h)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_h(vd, vn, vm, va, vg, status, desc, 0, 0, float_muladd_negate_product); }

void HELPER(sve_ah_fnmla_zpzzz_h)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_h(vd, vn, vm, va, vg, status, desc, 0, 0, float_muladd_negate_product | float_muladd_negate_c); }

void HELPER(sve_ah_fnmls_zpzzz_h)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_h(vd, vn, vm, va, vg, status, desc, 0, 0, float_muladd_negate_c); }

static void do_fmla_zpzzz_s(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc,
...
void HELPER(sve_fmla_zpzzz_s)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_s(vd, vn, vm, va, vg, status, desc, 0, 0, 0); }

void HELPER(sve_fmls_zpzzz_s)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_s(vd, vn, vm, va, vg, status, desc, 0x80000000, 0, 0); }

void HELPER(sve_fnmla_zpzzz_s)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_s(vd, vn, vm, va, vg, status, desc, 0x80000000, 0x80000000, 0); }

void HELPER(sve_fnmls_zpzzz_s)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_s(vd, vn, vm, va, vg, status, desc, 0, 0x80000000, 0); }

void HELPER(sve_ah_fmls_zpzzz_s)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_s(vd, vn, vm, va, vg, status, desc, 0, 0, float_muladd_negate_product); }

void HELPER(sve_ah_fnmla_zpzzz_s)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_s(vd, vn, vm, va, vg, status, desc, 0, 0, float_muladd_negate_product | float_muladd_negate_c); }

void HELPER(sve_ah_fnmls_zpzzz_s)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_s(vd, vn, vm, va, vg, status, desc, 0, 0, float_muladd_negate_c); }

static void do_fmla_zpzzz_d(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc,
...
void HELPER(sve_fmla_zpzzz_d)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_d(vd, vn, vm, va, vg, status, desc, 0, 0, 0); }

void HELPER(sve_fmls_zpzzz_d)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_d(vd, vn, vm, va, vg, status, desc, INT64_MIN, 0, 0); }

void HELPER(sve_fnmla_zpzzz_d)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_d(vd, vn, vm, va, vg, status, desc, INT64_MIN, INT64_MIN, 0); }

void HELPER(sve_fnmls_zpzzz_d)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_d(vd, vn, vm, va, vg, status, desc, 0, INT64_MIN, 0); }

void HELPER(sve_ah_fmls_zpzzz_d)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_d(vd, vn, vm, va, vg, status, desc, 0, 0, float_muladd_negate_product); }

void HELPER(sve_ah_fnmla_zpzzz_d)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_d(vd, vn, vm, va, vg, status, desc, 0, 0, float_muladd_negate_product | float_muladd_negate_c); }

void HELPER(sve_ah_fnmls_zpzzz_d)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{ do_fmla_zpzzz_d(vd, vn, vm, va, vg, status, desc, 0, 0, float_muladd_negate_c); }
...
#define DO_FPCMP_PPZZ_D(NAME, OP) DO_FPCMP_PPZZ(NAME##_d, float64, H1_8, OP)
...
#define DO_FCMUO(TYPE, X, Y, ST) TYPE##_compare_quiet(X, Y, ST) == float_relation_unordered
#define DO_FACGE(TYPE, X, Y, ST) TYPE##_compare(TYPE##_abs(Y), TYPE##_abs(X), ST) <= 0
#define DO_FACGT(TYPE, X, Y, ST) TYPE##_compare(TYPE##_abs(Y), TYPE##_abs(X), ST) < 0
...
#define DO_FPCMP_PPZ0_D(NAME, OP) DO_FPCMP_PPZ0(NAME##_d, float64, H1_8, OP)
...
void HELPER(sve_ftmad_h)(void* vd, void* vn, void* vm, float_status* s, uint32_t desc)
{
    ...
    for (i = 0; i < opr_sz; i++) {
        ...
        if (float16_is_neg(mm)) {
            if (fpcr_ah) { flags = float_muladd_negate_product; }
            else {
                mm = float16_abs(mm);
            }
            xx += 8;
...
void HELPER(sve_ftmad_s)(void* vd, void* vn, void* vm, float_status* s, uint32_t desc)
{
    static const float32 coeff[16] = {
        0x3f800000, 0xbe2aaaab, 0x3c088886, 0xb95008b9, 0x36369d6d, 0x00000000, 0x00000000, 0x00000000,
        0x3f800000, 0xbf000000, 0x3d2aaaa6, 0xbab60705, 0x37cd37cc, 0x00000000, 0x00000000, 0x00000000,
    };
...
void HELPER(sve_ftmad_d)(void* vd, void* vn, void* vm, float_status* s, uint32_t desc)
{
    static const float64 coeff[16] = {
        0x3ff0000000000000ull, 0xbfc5555555555543ull, 0x3f8111111110f30cull, 0xbf2a01a019b92fc6ull,
        0x3ec71de351f3d22bull, 0xbe5ae5e2b60f7b91ull, 0x3de5d8408868552full, 0x0000000000000000ull,
        0x3ff0000000000000ull, 0xbfe0000000000000ull, 0x3fa5555555555536ull, 0xbf56c16c16c13a0bull,
        0x3efa01a019b1e8d8ull, 0xbe927e4f7282f468ull, 0x3e21ee96d2641b13ull, 0xbda8f76380fbb401ull,
    };
...
void HELPER(sve_fcadd_h)(void* vd, void* vn, void* vm, void* vg, float_status* s, uint32_t desc)
{
    ...
    do {
        ...
        do {
            ...
            if (rot) { e3 = float16_maybe_ah_chs(e3, fpcr_ah); }
            else {
                e1 = float16_maybe_ah_chs(e1, fpcr_ah);
            }

            if (likely((pg >> (i & 63)) & 1)) { *(float16*)(vd + H1_2(i)) = float16_add(e0, e1, s); }
            if (likely((pg >> (j & 63)) & 1)) { *(float16*)(vd + H1_2(j)) = float16_add(e2, e3, s); }
        }
        while (i & 63);
    }
    while (i != 0);
...
void HELPER(sve_fcadd_s)(void* vd, void* vn, void* vm, void* vg, float_status* s, uint32_t desc)
{
    intptr_t  j, i = simd_oprsz(desc);
    uint64_t* g       = vg;
    bool      rot     = extract32(desc, SIMD_DATA_SHIFT, 1);
    ...
    do {
        ...
        do {
            ...
            if (rot) { e3 = float32_maybe_ah_chs(e3, fpcr_ah); }
            else {
                e1 = float32_maybe_ah_chs(e1, fpcr_ah);
            }

            if (likely((pg >> (i & 63)) & 1)) { *(float32*)(vd + H1_2(i)) = float32_add(e0, e1, s); }
            if (likely((pg >> (j & 63)) & 1)) { *(float32*)(vd + H1_2(j)) = float32_add(e2, e3, s); }
        }
        while (i & 63);
    }
    while (i != 0);
...
void HELPER(sve_fcadd_d)(void* vd, void* vn, void* vm, void* vg, float_status* s, uint32_t desc)
{
    intptr_t  j, i = simd_oprsz(desc);
    uint64_t* g       = vg;
    bool      rot     = extract32(desc, SIMD_DATA_SHIFT, 1);
    ...
    do {
        ...
        do {
            ...
            if (rot) { e3 = float64_maybe_ah_chs(e3, fpcr_ah); }
            else {
                e1 = float64_maybe_ah_chs(e1, fpcr_ah);
            }

            if (likely((pg >> (i & 63)) & 1)) { *(float64*)(vd + H1_2(i)) = float64_add(e0, e1, s); }
            if (likely((pg >> (j & 63)) & 1)) { *(float64*)(vd + H1_2(j)) = float64_add(e2, e3, s); }
        }
        while (i & 63);
    }
    while (i != 0);
...
void HELPER(sve_fcmla_zpzzz_h)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{
    intptr_t  j, i = simd_oprsz(desc);
...
void HELPER(sve_fcmla_zpzzz_s)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{
    intptr_t  j, i = simd_oprsz(desc);
    bool      flip      = extract32(desc, SIMD_DATA_SHIFT, 1);
    uint32_t  fpcr_ah   = extract32(desc, SIMD_DATA_SHIFT + 2, 1);
...
void HELPER(sve_fcmla_zpzzz_d)(void* vd, void* vn, void* vm, void* va, void* vg, float_status* status, uint32_t desc)
{
    intptr_t  j, i = simd_oprsz(desc);
    bool      flip      = extract32(desc, SIMD_DATA_SHIFT, 1);
    uint32_t  fpcr_ah   = extract32(desc, SIMD_DATA_SHIFT + 2, 1);
 ...
 */
static intptr_t find_next_active(uint64_t* vg, intptr_t reg_off, intptr_t reg_max, int esz)
{
...
bool sve_probe_page(SVEHostPage* info, bool nofault, CPUARMState* env, target_ulong addr, int mem_off,
                    MMUAccessType access_type, int mmu_idx, uintptr_t retaddr)
{
    ...
    flags       = probe_access_full(env, addr, 0, access_type, mmu_idx, nofault, &info->host, &full, retaddr);
    info->flags = flags;
 ...
 */
bool sve_cont_ldst_elements(SVEContLdSt* info, target_ulong addr, uint64_t* vg, intptr_t reg_max, int esz, int msize)
{
    ...
    do {
        ...
        if (pg) {
            ...
            if (reg_off_first < 0) { reg_off_first = i * 64 + ctz64(pg); }
        }
    }
    while (++i * 64 < reg_max);
     ...
     */
    if (elt_split != 0) { info->reg_off_last[0] = reg_off_split - esize; }
 ...
 */
bool sve_cont_ldst_pages(SVEContLdSt* info, SVEContFault fault, CPUARMState* env, target_ulong addr,
                         MMUAccessType access_type, uintptr_t retaddr)
{
    ...
    if (!sve_probe_page(&info->page[0], nofault, env, addr, mem_off, access_type, mmu_idx, retaddr)) {
        /* No work to be done. */
        return false;
    }
    ...
    have_work |= sve_probe_page(&info->page[1], nofault, env, addr, mem_off, access_type, mmu_idx, retaddr);
    return have_work;
}

void sve_cont_ldst_watchpoints(SVEContLdSt* info, CPUARMState* env, uint64_t* vg, target_ulong addr, int esize,
                               int msize, int wp_access, uintptr_t retaddr)
{
    intptr_t mem_off, reg_off, reg_last;
    int      flags0 = info->page[0].flags;
    int      flags1 = info->page[1].flags;

    if (likely(!((flags0 | flags1) & TLB_WATCHPOINT))) { return; }
    ...
    if (flags0 & TLB_WATCHPOINT) {
        ...
        while (reg_off <= reg_last) {
            ...
            do {
                if ((pg >> (reg_off & 63)) & 1) {
                    cpu_check_watchpoint(env_cpu(env), addr + mem_off, msize, info->page[0].attrs, wp_access, retaddr);
                }
                reg_off += esize;
                mem_off += msize;
            }
    ...
    if ((flags1 & TLB_WATCHPOINT) && mem_off >= 0) {
        ...
        do {
            ...
            do {
                if ((pg >> (reg_off & 63)) & 1) {
                    cpu_check_watchpoint(env_cpu(env), addr + mem_off, msize, info->page[1].attrs, wp_access, retaddr);
                }
                reg_off += esize;
                mem_off += msize;
            }
...
void sve_cont_ldst_mte_check(SVEContLdSt* info, CPUARMState* env, uint64_t* vg, target_ulong addr, int esize, int msize,
                             uint32_t mtedesc, uintptr_t ra)
{
    ...
    if (info->page[0].tagged) {
        ...
        if (reg_last < 0) { reg_last = info->reg_off_last[0]; }

        do {
            uint64_t pg = vg[reg_off >> 6];
            do {
                if ((pg >> (reg_off & 63)) & 1) { mte_check(env, mtedesc, addr, ra); }
                reg_off += esize;
                mem_off += msize;
            }
            while (reg_off <= reg_last && (reg_off & 63));
 ...
 */
static inline QEMU_ALWAYS_INLINE void sve_ldN_r(CPUARMState* env, uint64_t* vg, const target_ulong addr, uint32_t desc,
                                                const uintptr_t retaddr, const int esz, const int msz, const int N,
                                                uint32_t mtedesc, sve_ldst1_host_fn* host_fn, sve_ldst1_tlb_fn* tlb_fn)
{
    ...
    if (!sve_cont_ldst_elements(&info, addr, vg, reg_max, esz, N << msz)) {
        ...
        for (i = 0; i < N; ++i) { memset(&env->vfp.zregs[(rd + i) & 31], 0, reg_max); }
        return;
    }
    ...
    sve_cont_ldst_watchpoints(&info, env, vg, addr, 1 << esz, N << msz, BP_MEM_READ, retaddr);

    /*
     * Handle mte checks for all active elements.
     * Since TBI must be set for MTE, !mtedesc => !mte_active.
     */
    if (mtedesc) { sve_cont_ldst_mte_check(&info, env, vg, addr, 1 << esz, N << msz, mtedesc, retaddr); }
    ...
    if (unlikely(flags != 0)) {
        ...
        do {
            ...
            do {
                if ((pg >> (reg_off & 63)) & 1) {
                    for (i = 0; i < N; ++i) { tlb_fn(env, &scratch[i], reg_off, addr + mem_off + (i << msz), retaddr); }
                }
                reg_off += 1 << esz;
                mem_off += N << msz;
            }
        ...
        for (i = 0; i < N; ++i) { memcpy(&env->vfp.zregs[(rd + i) & 31], &scratch[i], reg_max); }
        return;
    }
    ...
    while (reg_off <= reg_last) {
        ...
        do {
            if ((pg >> (reg_off & 63)) & 1) {
                for (i = 0; i < N; ++i) {
                    host_fn(&env->vfp.zregs[(rd + i) & 31], reg_off, host + mem_off + (i << msz));
                }
            }
            reg_off += 1 << esz;
            mem_off += N << msz;
    ...
    if (unlikely(mem_off >= 0)) {
        ...
        for (i = 0; i < N; ++i) {
            tlb_fn(env, &env->vfp.zregs[(rd + i) & 31], reg_off, addr + mem_off + (i << msz), retaddr);
        }
    }
...
static inline QEMU_ALWAYS_INLINE void sve_ldN_r_mte(CPUARMState* env, uint64_t* vg, target_ulong addr, uint64_t desc,
                                                    ...
                                                    sve_ldst1_host_fn* host_fn, sve_ldst1_tlb_fn* tlb_fn)
{
    ...
    if (!tbi_check(mtedesc, bit55) || tcma_check(mtedesc, bit55, allocation_tag_from_addr(addr))) { mtedesc = 0; }

    sve_ldN_r(env, vg, addr, desc, ra, esz, msz, N, mtedesc, host_fn, tlb_fn);
}

#define DO_LD1_1(NAME, ESZ)                                                                             \
    void HELPER(sve_##NAME##_r)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)          \
    { sve_ldN_r(env, vg, addr, desc, GETPC(), ESZ, MO_8, 1, 0, sve_##NAME##_host, sve_##NAME##_tlb); }  \
    void HELPER(sve_##NAME##_r_mte)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)      \
    { sve_ldN_r_mte(env, vg, addr, desc, GETPC(), ESZ, MO_8, 1, sve_##NAME##_host, sve_##NAME##_tlb); }

#define DO_LD1_2(NAME, ESZ, MSZ)                                                                             \
    void HELPER(sve_##NAME##_le_r)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)            \
    { sve_ldN_r(env, vg, addr, desc, GETPC(), ESZ, MSZ, 1, 0, sve_##NAME##_le_host, sve_##NAME##_le_tlb); }  \
    void HELPER(sve_##NAME##_be_r)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)            \
    { sve_ldN_r(env, vg, addr, desc, GETPC(), ESZ, MSZ, 1, 0, sve_##NAME##_be_host, sve_##NAME##_be_tlb); }  \
    void HELPER(sve_##NAME##_le_r_mte)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)        \
    { sve_ldN_r_mte(env, vg, addr, desc, GETPC(), ESZ, MSZ, 1, sve_##NAME##_le_host, sve_##NAME##_le_tlb); } \
    void HELPER(sve_##NAME##_be_r_mte)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)        \
    { sve_ldN_r_mte(env, vg, addr, desc, GETPC(), ESZ, MSZ, 1, sve_##NAME##_be_host, sve_##NAME##_be_tlb); }
...
#define DO_LDN_1(N)                                                                                 \
    void HELPER(sve_ld##N##bb_r)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)     \
    { sve_ldN_r(env, vg, addr, desc, GETPC(), MO_8, MO_8, N, 0, sve_ld1bb_host, sve_ld1bb_tlb); }   \
    void HELPER(sve_ld##N##bb_r_mte)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc) \
    { sve_ldN_r_mte(env, vg, addr, desc, GETPC(), MO_8, MO_8, N, sve_ld1bb_host, sve_ld1bb_tlb); }

#define DO_LDN_2(N, SUFF, ESZ)                                                                                     \
    void HELPER(sve_ld##N##SUFF##_le_r)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)             \
    { sve_ldN_r(env, vg, addr, desc, GETPC(), ESZ, ESZ, N, 0, sve_ld1##SUFF##_le_host, sve_ld1##SUFF##_le_tlb); }  \
    void HELPER(sve_ld##N##SUFF##_be_r)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)             \
    { sve_ldN_r(env, vg, addr, desc, GETPC(), ESZ, ESZ, N, 0, sve_ld1##SUFF##_be_host, sve_ld1##SUFF##_be_tlb); }  \
    void HELPER(sve_ld##N##SUFF##_le_r_mte)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)         \
    { sve_ldN_r_mte(env, vg, addr, desc, GETPC(), ESZ, ESZ, N, sve_ld1##SUFF##_le_host, sve_ld1##SUFF##_le_tlb); } \
    void HELPER(sve_ld##N##SUFF##_be_r_mte)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)         \
    { sve_ldN_r_mte(env, vg, addr, desc, GETPC(), ESZ, ESZ, N, sve_ld1##SUFF##_be_host, sve_ld1##SUFF##_be_tlb); }
...
static void record_fault(CPUARMState* env, uintptr_t i, uintptr_t oprsz)
{
    ...
    for (; i < oprsz; i += 64) { ffr[i / 64] = 0; }
}

/*
 * Common helper for all contiguous no-fault and first-fault loads.
 */
static inline QEMU_ALWAYS_INLINE void sve_ldnfff1_r(CPUARMState* env, void* vg, const target_ulong addr, uint32_t desc,
                                                    const uintptr_t retaddr, uint32_t mtedesc, const int esz,
                                                    const int msz, const SVEContFault fault, sve_ldst1_host_fn* host_fn,
                                                    sve_ldst1_tlb_fn* tlb_fn)
{
     ...
     */
    if (!info.page[0].tagged) { mtedesc = 0; }

    if (fault == FAULT_FIRST) {
        /* Trapping mte check for the first-fault element.  */
        if (mtedesc) { mte_check(env, mtedesc, addr + mem_off, retaddr); }
        ...
        if (unlikely(mem_off == info.mem_off_split)) {
            ...
            if (unlikely(flags & TLB_WATCHPOINT)
                && (cpu_watchpoint_address_matches(env_cpu(env), addr + mem_off, 1 << msz) & BP_MEM_READ))
            {
                /* Watchpoint hit, see below. */
                goto do_fault;
            }
            if (mtedesc && !mte_probe(env, mtedesc, addr + mem_off)) { goto do_fault; }
            /*
             * Use the slow path for cross-page handling.
     ...
     */
    if (unlikely(flags & TLB_MMIO)) { goto do_fault; }
...
static inline QEMU_ALWAYS_INLINE void sve_ldnfff1_r_mte(CPUARMState* env, void* vg, target_ulong addr, uint64_t desc,
                                                        ...
                                                        sve_ldst1_tlb_fn* tlb_fn)
{
    ...
    sve_ldnfff1_r(env, vg, addr, desc, retaddr, mtedesc, esz, msz, fault, host_fn, tlb_fn);
}

#define DO_LDFF1_LDNF1_1(PART, ESZ)                                                                     \
    void HELPER(sve_ldff1##PART##_r)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)     \
    {                                                                                                   \
        sve_ldnfff1_r(env, vg, addr, desc, GETPC(), 0, ESZ, MO_8, FAULT_FIRST, sve_ld1##PART##_host,    \
                      sve_ld1##PART##_tlb);                                                             \
    }                                                                                                   \
    void HELPER(sve_ldnf1##PART##_r)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)     \
    {                                                                                                   \
        sve_ldnfff1_r(env, vg, addr, desc, GETPC(), 0, ESZ, MO_8, FAULT_NO, sve_ld1##PART##_host,       \
                      sve_ld1##PART##_tlb);                                                             \
    }                                                                                                   \
    void HELPER(sve_ldff1##PART##_r_mte)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc) \
    {                                                                                                   \
        sve_ldnfff1_r_mte(env, vg, addr, desc, GETPC(), ESZ, MO_8, FAULT_FIRST, sve_ld1##PART##_host,   \
                          sve_ld1##PART##_tlb);                                                         \
    }                                                                                                   \
    void HELPER(sve_ldnf1##PART##_r_mte)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc) \
    { sve_ldnfff1_r_mte(env, vg, addr, desc, GETPC(), ESZ, MO_8, FAULT_NO, sve_ld1##PART##_host, sve_ld1##PART##_tlb); }

#define DO_LDFF1_LDNF1_2(PART, ESZ, MSZ)                                                                   \
    void HELPER(sve_ldff1##PART##_le_r)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)     \
    {                                                                                                      \
        sve_ldnfff1_r(env, vg, addr, desc, GETPC(), 0, ESZ, MSZ, FAULT_FIRST, sve_ld1##PART##_le_host,     \
                      sve_ld1##PART##_le_tlb);                                                             \
    }                                                                                                      \
    void HELPER(sve_ldnf1##PART##_le_r)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)     \
    {                                                                                                      \
        sve_ldnfff1_r(env, vg, addr, desc, GETPC(), 0, ESZ, MSZ, FAULT_NO, sve_ld1##PART##_le_host,        \
                      sve_ld1##PART##_le_tlb);                                                             \
    }                                                                                                      \
    void HELPER(sve_ldff1##PART##_be_r)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)     \
    {                                                                                                      \
        sve_ldnfff1_r(env, vg, addr, desc, GETPC(), 0, ESZ, MSZ, FAULT_FIRST, sve_ld1##PART##_be_host,     \
                      sve_ld1##PART##_be_tlb);                                                             \
    }                                                                                                      \
    void HELPER(sve_ldnf1##PART##_be_r)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)     \
    {                                                                                                      \
        sve_ldnfff1_r(env, vg, addr, desc, GETPC(), 0, ESZ, MSZ, FAULT_NO, sve_ld1##PART##_be_host,        \
                      sve_ld1##PART##_be_tlb);                                                             \
    }                                                                                                      \
    void HELPER(sve_ldff1##PART##_le_r_mte)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc) \
    {                                                                                                      \
        sve_ldnfff1_r_mte(env, vg, addr, desc, GETPC(), ESZ, MSZ, FAULT_FIRST, sve_ld1##PART##_le_host,    \
                          sve_ld1##PART##_le_tlb);                                                         \
    }                                                                                                      \
    void HELPER(sve_ldnf1##PART##_le_r_mte)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc) \
    {                                                                                                      \
        sve_ldnfff1_r_mte(env, vg, addr, desc, GETPC(), ESZ, MSZ, FAULT_NO, sve_ld1##PART##_le_host,       \
                          sve_ld1##PART##_le_tlb);                                                         \
    }                                                                                                      \
    void HELPER(sve_ldff1##PART##_be_r_mte)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc) \
    {                                                                                                      \
        sve_ldnfff1_r_mte(env, vg, addr, desc, GETPC(), ESZ, MSZ, FAULT_FIRST, sve_ld1##PART##_be_host,    \
                          sve_ld1##PART##_be_tlb);                                                         \
    }                                                                                                      \
    void HELPER(sve_ldnf1##PART##_be_r_mte)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc) \
    {                                                                                                      \
        sve_ldnfff1_r_mte(env, vg, addr, desc, GETPC(), ESZ, MSZ, FAULT_NO, sve_ld1##PART##_be_host,       \
                          sve_ld1##PART##_be_tlb);                                                         \
    }
...
static inline QEMU_ALWAYS_INLINE void sve_stN_r(CPUARMState* env, uint64_t* vg, target_ulong addr, uint32_t desc,
                                                const uintptr_t retaddr, const int esz, const int msz, const int N,
                                                uint32_t mtedesc, sve_ldst1_host_fn* host_fn, sve_ldst1_tlb_fn* tlb_fn)
{
    const unsigned rd      = simd_data(desc);
    ...
    /* Handle watchpoints for all active elements. */
    sve_cont_ldst_watchpoints(&info, env, vg, addr, 1 << esz, N << msz, BP_MEM_WRITE, retaddr);
...
static inline QEMU_ALWAYS_INLINE void sve_stN_r_mte(CPUARMState* env, uint64_t* vg, target_ulong addr, uint64_t desc,
                                                    const uintptr_t ra, const int esz, const int msz, const int N,
                                                    sve_ldst1_host_fn* host_fn, sve_ldst1_tlb_fn* tlb_fn)
{
    uint32_t mtedesc = desc >> 32;
...
#define DO_STN_1(N, NAME, ESZ)                                                                                \
    void HELPER(sve_st##N##NAME##_r)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)           \
    { sve_stN_r(env, vg, addr, desc, GETPC(), ESZ, MO_8, N, 0, sve_st1##NAME##_host, sve_st1##NAME##_tlb); }  \
    void HELPER(sve_st##N##NAME##_r_mte)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)       \
    { sve_stN_r_mte(env, vg, addr, desc, GETPC(), ESZ, MO_8, N, sve_st1##NAME##_host, sve_st1##NAME##_tlb); }

#define DO_STN_2(N, NAME, ESZ, MSZ)                                                                                \
    void HELPER(sve_st##N##NAME##_le_r)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)             \
    { sve_stN_r(env, vg, addr, desc, GETPC(), ESZ, MSZ, N, 0, sve_st1##NAME##_le_host, sve_st1##NAME##_le_tlb); }  \
    void HELPER(sve_st##N##NAME##_be_r)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)             \
    { sve_stN_r(env, vg, addr, desc, GETPC(), ESZ, MSZ, N, 0, sve_st1##NAME##_be_host, sve_st1##NAME##_be_tlb); }  \
    void HELPER(sve_st##N##NAME##_le_r_mte)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)         \
    { sve_stN_r_mte(env, vg, addr, desc, GETPC(), ESZ, MSZ, N, sve_st1##NAME##_le_host, sve_st1##NAME##_le_tlb); } \
    void HELPER(sve_st##N##NAME##_be_r_mte)(CPUARMState * env, void* vg, target_ulong addr, uint64_t desc)         \
    { sve_stN_r_mte(env, vg, addr, desc, GETPC(), ESZ, MSZ, N, sve_st1##NAME##_be_host, sve_st1##NAME##_be_tlb); }
...
static target_ulong off_zsu_s(void* reg, intptr_t reg_ofs) { return *(uint32_t*)(reg + H1_4(reg_ofs)); }

static target_ulong off_zss_s(void* reg, intptr_t reg_ofs) { return *(int32_t*)(reg + H1_4(reg_ofs)); }

static target_ulong off_zsu_d(void* reg, intptr_t reg_ofs) { return (uint32_t)*(uint64_t*)(reg + reg_ofs); }

static target_ulong off_zss_d(void* reg, intptr_t reg_ofs) { return (int32_t)*(uint64_t*)(reg + reg_ofs); }

static target_ulong off_zd_d(void* reg, intptr_t reg_ofs) { return *(uint64_t*)(reg + reg_ofs); }

static inline QEMU_ALWAYS_INLINE void sve_ld1_z(CPUARMState* env, void* vd, uint64_t* vg, void* vm, target_ulong base,
    ...
    do {
        ...
        do {
            if (likely(pg & 1)) {
                ...
                sve_probe_page(&info, false, env, addr, 0, MMU_DATA_LOAD, mmu_idx, retaddr);

                if (likely(in_page >= msize)) {
                    if (unlikely(info.flags & TLB_WATCHPOINT)) {
                        cpu_check_watchpoint(env_cpu(env), addr, msize, info.attrs, BP_MEM_READ, retaddr);
                    }
                    if (mtedesc && info.tagged) { mte_check(env, mtedesc, addr, retaddr); }
                    if (unlikely(info.flags & TLB_MMIO)) { tlb_fn(env, &scratch, reg_off, addr, retaddr); }
                    else {
                    ...
                    sve_probe_page(&info2, false, env, addr + in_page, 0, MMU_DATA_LOAD, mmu_idx, retaddr);
...
static inline QEMU_ALWAYS_INLINE void sve_ld1_z_mte(CPUARMState* env, void* vd, uint64_t* vg, void* vm,
                                                    target_ulong base, uint64_t desc, uintptr_t retaddr, int esize,
                                                    int msize, zreg_off_fn* off_fn, sve_ldst1_host_fn* host_fn,
                                                    sve_ldst1_tlb_fn* tlb_fn)
{
    uint32_t mtedesc = desc >> 32;
     ...
     */
    sve_ld1_z(env, vd, vg, vm, base, desc, retaddr, mtedesc, esize, msize, off_fn, host_fn, tlb_fn);
}

#define DO_LD1_ZPZ_S(MEM, OFS, MSZ)                                                                            \
    void HELPER(sve_ld##MEM##_##OFS)(CPUARMState * env, void* vd, void* vg, void* vm, target_ulong base,       \
                                     uint64_t desc)                                                            \
    {                                                                                                          \
        sve_ld1_z(env, vd, vg, vm, base, desc, GETPC(), 0, 4, 1 << MSZ, off_##OFS##_s, sve_ld1##MEM##_host,    \
                  sve_ld1##MEM##_tlb);                                                                         \
    }                                                                                                          \
    void HELPER(sve_ld##MEM##_##OFS##_mte)(CPUARMState * env, void* vd, void* vg, void* vm, target_ulong base, \
                                           uint64_t desc)                                                      \
    {                                                                                                          \
        sve_ld1_z_mte(env, vd, vg, vm, base, desc, GETPC(), 4, 1 << MSZ, off_##OFS##_s, sve_ld1##MEM##_host,   \
                      sve_ld1##MEM##_tlb);                                                                     \
    }
...
#define DO_LD1_ZPZ_D(MEM, OFS, MSZ)                                                                            \
    void HELPER(sve_ld##MEM##_##OFS)(CPUARMState * env, void* vd, void* vg, void* vm, target_ulong base,       \
                                     uint64_t desc)                                                            \
    {                                                                                                          \
        sve_ld1_z(env, vd, vg, vm, base, desc, GETPC(), 0, 8, 1 << MSZ, off_##OFS##_d, sve_ld1##MEM##_host,    \
                  sve_ld1##MEM##_tlb);                                                                         \
    }                                                                                                          \
    void HELPER(sve_ld##MEM##_##OFS##_mte)(CPUARMState * env, void* vd, void* vg, void* vm, target_ulong base, \
                                           uint64_t desc)                                                      \
    {                                                                                                          \
        sve_ld1_z_mte(env, vd, vg, vm, base, desc, GETPC(), 8, 1 << MSZ, off_##OFS##_d, sve_ld1##MEM##_host,   \
                      sve_ld1##MEM##_tlb);                                                                     \
    }
...
#define DO_LD1_ZPZ_Q(MEM, OFS, MSZ)                                                                            \
    void HELPER(sve_ld##MEM##_##OFS)(CPUARMState * env, void* vd, void* vg, void* vm, target_ulong base,       \
                                     uint64_t desc)                                                            \
    {                                                                                                          \
        sve_ld1_z(env, vd, vg, vm, base, desc, GETPC(), 0, 16, 1 << MSZ, off_##OFS##_d, sve_ld1##MEM##_host,   \
                  sve_ld1##MEM##_tlb);                                                                         \
    }                                                                                                          \
    void HELPER(sve_ld##MEM##_##OFS##_mte)(CPUARMState * env, void* vd, void* vg, void* vm, target_ulong base, \
                                           uint64_t desc)                                                      \
    {                                                                                                          \
        sve_ld1_z_mte(env, vd, vg, vm, base, desc, GETPC(), 16, 1 << MSZ, off_##OFS##_d, sve_ld1##MEM##_host,  \
                      sve_ld1##MEM##_tlb);                                                                     \
    }
...
static inline QEMU_ALWAYS_INLINE void sve_ldff1_z(CPUARMState* env, void* vd, uint64_t* vg, void* vm, target_ulong base,
                                                  uint32_t desc, uintptr_t retaddr, uint32_t mtedesc, const int esz,
                                                  const int msz, zreg_off_fn* off_fn, sve_ldst1_host_fn* host_fn,
                                                  sve_ldst1_tlb_fn* tlb_fn)
{
    const int      mmu_idx = arm_env_mmu_index(env);
    const intptr_t reg_max = simd_oprsz(desc);
    ...
    if (mtedesc) { mte_check(env, mtedesc, addr, retaddr); }
    ...
    while (reg_off < reg_max) {
        ...
        do {
            if (likely((pg >> (reg_off & 63)) & 1)) {
                ...
                sve_probe_page(&info, true, env, addr, 0, MMU_DATA_LOAD, mmu_idx, retaddr);
                if (unlikely(info.flags & (TLB_INVALID_MASK | TLB_MMIO))) { goto fault; }
                if (unlikely(info.flags & TLB_WATCHPOINT)
                    && (cpu_watchpoint_address_matches(env_cpu(env), addr, msize) & BP_MEM_READ))
                {
                    goto fault;
                }
                if (mtedesc && info.tagged && !mte_probe(env, mtedesc, addr)) { goto fault; }
...
static inline QEMU_ALWAYS_INLINE void sve_ldff1_z_mte(CPUARMState* env, void* vd, uint64_t* vg, void* vm,
                                                      ...
                                                      sve_ldst1_host_fn* host_fn, sve_ldst1_tlb_fn* tlb_fn)
{
     ...
     * examine TBI + TCMA like we do for sve_ldN_r_mte().
     */
    sve_ldff1_z(env, vd, vg, vm, base, desc, retaddr, mtedesc, esz, msz, off_fn, host_fn, tlb_fn);
}

#define DO_LDFF1_ZPZ_S(MEM, OFS, MSZ)                                                                            \
    void HELPER(sve_ldff##MEM##_##OFS)(CPUARMState * env, void* vd, void* vg, void* vm, target_ulong base,       \
                                       uint64_t desc)                                                            \
    {                                                                                                            \
        sve_ldff1_z(env, vd, vg, vm, base, desc, GETPC(), 0, MO_32, MSZ, off_##OFS##_s, sve_ld1##MEM##_host,     \
                    sve_ld1##MEM##_tlb);                                                                         \
    }                                                                                                            \
    void HELPER(sve_ldff##MEM##_##OFS##_mte)(CPUARMState * env, void* vd, void* vg, void* vm, target_ulong base, \
                                             uint64_t desc)                                                      \
    {                                                                                                            \
        sve_ldff1_z_mte(env, vd, vg, vm, base, desc, GETPC(), MO_32, MSZ, off_##OFS##_s, sve_ld1##MEM##_host,    \
                        sve_ld1##MEM##_tlb);                                                                     \
    }
...
#define DO_LDFF1_ZPZ_D(MEM, OFS, MSZ)                                                                            \
    void HELPER(sve_ldff##MEM##_##OFS)(CPUARMState * env, void* vd, void* vg, void* vm, target_ulong base,       \
                                       uint64_t desc)                                                            \
    {                                                                                                            \
        sve_ldff1_z(env, vd, vg, vm, base, desc, GETPC(), 0, MO_64, MSZ, off_##OFS##_d, sve_ld1##MEM##_host,     \
                    sve_ld1##MEM##_tlb);                                                                         \
    }                                                                                                            \
    void HELPER(sve_ldff##MEM##_##OFS##_mte)(CPUARMState * env, void* vd, void* vg, void* vm, target_ulong base, \
                                             uint64_t desc)                                                      \
    {                                                                                                            \
        sve_ldff1_z_mte(env, vd, vg, vm, base, desc, GETPC(), MO_64, MSZ, off_##OFS##_d, sve_ld1##MEM##_host,    \
                        sve_ld1##MEM##_tlb);                                                                     \
    }
...
static inline QEMU_ALWAYS_INLINE void sve_st1_z(CPUARMState* env, void* vd, uint64_t* vg, void* vm, target_ulong base,
                                                uint32_t desc, uintptr_t retaddr, uint32_t mtedesc, int esize,
                                                int msize, zreg_off_fn* off_fn, sve_ldst1_host_fn* host_fn,
                                                sve_ldst1_tlb_fn* tlb_fn)
{
    ...
    do {
        ...
        do {
            ...
            if (likely((pg >> (reg_off & 63)) & 1)) {
                if (likely(in_page >= msize)) {
                    sve_probe_page(&info, false, env, addr, 0, MMU_DATA_STORE, mmu_idx, retaddr);
                    if (!(info.flags & TLB_MMIO)) { host[i] = info.host; }
                }
                else {
                    /*
                     * Element crosses the page boundary.
                     ...
                     */
                    sve_probe_page(&info, false, env, addr, 0, MMU_DATA_STORE, mmu_idx, retaddr);
                    sve_probe_page(&info2, false, env, addr + in_page, 0, MMU_DATA_STORE, mmu_idx, retaddr);
                    info.flags |= info2.flags;
                }

                if (unlikely(info.flags & TLB_WATCHPOINT)) {
                    cpu_check_watchpoint(env_cpu(env), addr, msize, info.attrs, BP_MEM_WRITE, retaddr);
                }
    ...
    do {
        void* h = host[i];
        if (likely(h != NULL)) { host_fn(vd, reg_off, h); }
...
static inline QEMU_ALWAYS_INLINE void sve_st1_z_mte(CPUARMState* env, void* vd, uint64_t* vg, void* vm,
                                                    target_ulong base, uint64_t desc, uintptr_t retaddr, int esize,
                                                    int msize, zreg_off_fn* off_fn, sve_ldst1_host_fn* host_fn,
                                                    sve_ldst1_tlb_fn* tlb_fn)
{
     ...
     * examine TBI + TCMA like we do for sve_ldN_r_mte().
     */
    sve_st1_z(env, vd, vg, vm, base, desc, retaddr, mtedesc, esize, msize, off_fn, host_fn, tlb_fn);
}

#define DO_ST1_ZPZ_S(MEM, OFS, MSZ)                                                                            \
    void HELPER(sve_st##MEM##_##OFS)(CPUARMState * env, void* vd, void* vg, void* vm, target_ulong base,       \
                                     uint64_t desc)                                                            \
    {                                                                                                          \
        sve_st1_z(env, vd, vg, vm, base, desc, GETPC(), 0, 4, 1 << MSZ, off_##OFS##_s, sve_st1##MEM##_host,    \
                  sve_st1##MEM##_tlb);                                                                         \
    }                                                                                                          \
    void HELPER(sve_st##MEM##_##OFS##_mte)(CPUARMState * env, void* vd, void* vg, void* vm, target_ulong base, \
                                           uint64_t desc)                                                      \
    {                                                                                                          \
        sve_st1_z_mte(env, vd, vg, vm, base, desc, GETPC(), 4, 1 << MSZ, off_##OFS##_s, sve_st1##MEM##_host,   \
                      sve_st1##MEM##_tlb);                                                                     \
    }
...
#define DO_ST1_ZPZ_D(MEM, OFS, MSZ)                                                                            \
    void HELPER(sve_st##MEM##_##OFS)(CPUARMState * env, void* vd, void* vg, void* vm, target_ulong base,       \
                                     uint64_t desc)                                                            \
    {                                                                                                          \
        sve_st1_z(env, vd, vg, vm, base, desc, GETPC(), 0, 8, 1 << MSZ, off_##OFS##_d, sve_st1##MEM##_host,    \
                  sve_st1##MEM##_tlb);                                                                         \
    }                                                                                                          \
    void HELPER(sve_st##MEM##_##OFS##_mte)(CPUARMState * env, void* vd, void* vg, void* vm, target_ulong base, \
                                           uint64_t desc)                                                      \
    {                                                                                                          \
        sve_st1_z_mte(env, vd, vg, vm, base, desc, GETPC(), 8, 1 << MSZ, off_##OFS##_d, sve_st1##MEM##_host,   \
                      sve_st1##MEM##_tlb);                                                                     \
    }
...
#define DO_ST1_ZPZ_Q(MEM, OFS, MSZ)                                                                            \
    void HELPER(sve_st##MEM##_##OFS)(CPUARMState * env, void* vd, void* vg, void* vm, target_ulong base,       \
                                     uint64_t desc)                                                            \
    {                                                                                                          \
        sve_st1_z(env, vd, vg, vm, base, desc, GETPC(), 0, 16, 1 << MSZ, off_##OFS##_d, sve_st1##MEM##_host,   \
                  sve_st1##MEM##_tlb);                                                                         \
    }                                                                                                          \
    void HELPER(sve_st##MEM##_##OFS##_mte)(CPUARMState * env, void* vd, void* vg, void* vm, target_ulong base, \
                                           uint64_t desc)                                                      \
    {                                                                                                          \
        sve_st1_z_mte(env, vd, vg, vm, base, desc, GETPC(), 16, 1 << MSZ, off_##OFS##_d, sve_st1##MEM##_host,  \
                      sve_st1##MEM##_tlb);                                                                     \
    }
...
static unsigned sve2p1_cont_ldst_elements(SVEContLdSt* info, vaddr addr, uint32_t png, intptr_t reg_max, int N,
                                          int v_esz)
{
     ...
     * It is useful as iteration bounds.
     */
    if (reg_off_split != 0) { info->reg_off_last[0] = ROUND_DOWN(reg_off_split - esize, b_stride); }
...
static void sve2p1_cont_ldst_watchpoints(SVEContLdSt* info, CPUARMState* env, target_ulong addr, unsigned estride,
                                         int esize, int wp_access, uintptr_t ra)
{
    ...
    if (flags0 & TLB_WATCHPOINT) {
        ...
        if (count_last < 0) { count_last = info->reg_off_last[0]; }
        do {
            cpu_check_watchpoint(env_cpu(env), addr + count_off, esize, info->page[0].attrs, wp_access, ra);
            count_off += estride;
        }
    ...
    if ((flags1 & TLB_WATCHPOINT) && count_off >= 0) {
        ...
        do {
            cpu_check_watchpoint(env_cpu(env), addr + count_off, esize, info->page[1].attrs, wp_access, ra);
            count_off += estride;
        }
        while (count_off <= count_last);
    }
...
static void sve2p1_cont_ldst_mte_check(SVEContLdSt* info, CPUARMState* env, target_ulong addr, unsigned estride,
                                       int esize, uint32_t mtedesc, uintptr_t ra)
{
    intptr_t count_off, count_last;
...
static inline QEMU_ALWAYS_INLINE void sve2p1_ld1_c(CPUARMState* env, ARMVectorReg* zd, const vaddr addr, uint32_t png,
                                                   uint64_t desc64, const uintptr_t ra, const MemOp esz,
                                                   sve_ldst1_host_fn* host_fn, sve_ldst1_tlb_fn* tlb_fn)
{
    ...
    if (estride == 0) {
        /* The entire predicate was false; no load occurs.  */
        for (unsigned n = 0; n < N; n++) { memset(zd + n * rstride, 0, reg_max); }
        return;
    }
    ...
    /* Handle watchpoints for all active elements. */
    sve2p1_cont_ldst_watchpoints(&info, env, addr, estride, esize, BP_MEM_READ, ra);

    /*
     * Handle mte checks for all active elements.
     * Since TBI must be set for MTE, !mtedesc => !mte_active.
     */
    if (mtedesc) { sve2p1_cont_ldst_mte_check(&info, env, estride, addr, esize, mtedesc, ra); }
    ...
    if (unlikely(flags != 0)) {
        ...
        for (unsigned n = 0; n < N; ++n) { memcpy(&zd[n * rstride], &scratch[n], reg_max); }
        return;
    }
...
void HELPER(sve2p1_ld1bb_c)(CPUARMState* env, void* vd, target_ulong addr, uint32_t png, uint64_t desc)
{ sve2p1_ld1_c(env, vd, addr, png, desc, GETPC(), MO_8, sve_ld1bb_host, sve_ld1bb_tlb); }

#define DO_LD1_2(NAME, ESZ)                                                                                        \
    void HELPER(sve2p1_##NAME##_le_c)(CPUARMState * env, void* vd, target_ulong addr, uint32_t png, uint64_t desc) \
    { sve2p1_ld1_c(env, vd, addr, png, desc, GETPC(), ESZ, sve_##NAME##_le_host, sve_##NAME##_le_tlb); }           \
    void HELPER(sve2p1_##NAME##_be_c)(CPUARMState * env, void* vd, target_ulong addr, uint32_t png, uint64_t desc) \
    { sve2p1_ld1_c(env, vd, addr, png, desc, GETPC(), ESZ, sve_##NAME##_be_host, sve_##NAME##_be_tlb); }
...
static inline QEMU_ALWAYS_INLINE void sve2p1_st1_c(CPUARMState* env, ARMVectorReg* zd, const vaddr addr, uint32_t png,
                                                   uint64_t desc64, const uintptr_t ra, const int esz,
                                                   sve_ldst1_host_fn* host_fn, sve_ldst1_tlb_fn* tlb_fn)
{
    uint32_t       mtedesc = desc64 >> 32;
    uint32_t       desc    = desc64;
    ...
    /* Handle watchpoints for all active elements. */
    sve2p1_cont_ldst_watchpoints(&info, env, addr, estride, esize, BP_MEM_WRITE, ra);
...
void HELPER(sve2p1_st1bb_c)(CPUARMState* env, void* vd, target_ulong addr, uint32_t png, uint64_t desc)
{ sve2p1_st1_c(env, vd, addr, png, desc, GETPC(), MO_8, sve_st1bb_host, sve_st1bb_tlb); }

#define DO_ST1_2(NAME, ESZ)                                                                                        \
    void HELPER(sve2p1_##NAME##_le_c)(CPUARMState * env, void* vd, target_ulong addr, uint32_t png, uint64_t desc) \
    { sve2p1_st1_c(env, vd, addr, png, desc, GETPC(), ESZ, sve_##NAME##_le_host, sve_##NAME##_le_tlb); }           \
    void HELPER(sve2p1_##NAME##_be_c)(CPUARMState * env, void* vd, target_ulong addr, uint32_t png, uint64_t desc) \
    { sve2p1_st1_c(env, vd, addr, png, desc, GETPC(), ESZ, sve_##NAME##_be_host, sve_##NAME##_be_tlb); }
...
void HELPER(sve2_eor3)(void* vd, void* vn, void* vm, void* vk, uint32_t desc)
{
    ...
    for (i = 0; i < opr_sz; ++i) { d[i] = n[i] ^ m[i] ^ k[i]; }
}
...
void HELPER(sve2_bsl1n)(void* vd, void* vn, void* vm, void* vk, uint32_t desc)
{
    ...
    for (i = 0; i < opr_sz; ++i) { d[i] = (~n[i] & k[i]) | (m[i] & ~k[i]); }
}
...
static inline uint32_t do_match(void* vd, void* vn, void* vm, void* vg, uint32_t desc, int esz, bool nmatch)
{
...
void HELPER(sve2_histcnt_s)(void* vd, void* vn, void* vm, void* vg, uint32_t desc)
{
    ARMVectorReg scratch;
    intptr_t     i, j;
    intptr_t     opr_sz = simd_oprsz(desc);
...
void HELPER(sve2_histcnt_d)(void* vd, void* vn, void* vm, void* vg, uint32_t desc)
{
    ARMVectorReg scratch;
    intptr_t     i, j;
    intptr_t     opr_sz = simd_oprsz(desc);
...
void HELPER(sve2_xar_s)(void* vd, void* vn, void* vm, uint32_t desc)
{
    ...
    for (i = 0; i < opr_sz; ++i) { d[i] = ror32(n[i] ^ m[i], shr); }
}

void HELPER(fmmla_s)(void* vd, void* vn, void* vm, void* va, float_status* status, uint32_t desc)
{
...
void HELPER(fmmla_d)(void* vd, void* vn, void* vm, void* va, float_status* status, uint32_t desc)
{
...
void HELPER(pext)(void* vd, uint32_t png, uint32_t desc)
{
    ...
    memset(d, 0, sizeof(*d));
    if (p.invert) {
        if (b_count <= 0) { do_whilel(vd, mask, vl, vl); }
        else if (b_count < vl) {
