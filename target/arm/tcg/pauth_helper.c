...
static uint64_t pac_sub(uint64_t i)
{
    ...
    for (b = 0; b < 64; b += 4) { o |= (uint64_t)sub[(i >> b) & 0xf] << b; }
    return o;
}
...
static uint64_t pac_sub1(uint64_t i)
{
    ...
    for (b = 0; b < 64; b += 4) { o |= (uint64_t)sub1[(i >> b) & 0xf] << b; }
    return o;
}
...
static uint64_t pac_inv_sub(uint64_t i)
{
    ...
    for (b = 0; b < 64; b += 4) { o |= (uint64_t)inv_sub[(i >> b) & 0xf] << b; }
    return o;
}
...
static uint64_t tweak_cell_rot(uint64_t cell) { return (cell >> 1) | (((cell ^ (cell >> 1)) & 1) << 3); }
...
static uint64_t tweak_cell_inv_rot(uint64_t cell) { return ((cell << 1) & 0xf) | ((cell & 1) ^ (cell >> 3)); }
...
static uint64_t pauth_computepac_architected(uint64_t data, uint64_t modifier, ARMPACKey key, bool isqarma3)
{
    ...
    for (i = 0; i <= iterations; ++i) {
        ...
        if (isqarma3) { workingval = pac_sub1(workingval); }
        else {
...
static uint64_t pauth_computepac_impdef(uint64_t data, uint64_t modifier, ARMPACKey key)
{ return qemu_xxhash64_4(data, modifier, key.lo, key.hi); }

static uint64_t pauth_computepac(CPUARMState* env, uint64_t data, uint64_t modifier, ARMPACKey key)
{
...
static uint64_t pauth_addpac(CPUARMState* env, uint64_t ptr, uint64_t modifier, ARMPACKey* key, bool data)
{
    ...
    if (param.tbi) { ext = sextract64(ptr, 55, 1); }
    else {
     ...
     */
    if (pauth_feature >= PauthFeat_2) { pac ^= ptr; }
    if (param.tbi) {
...
static uint64_t pauth_original_ptr(uint64_t ptr, ARMVAParameters param)
{
    ...
    if (extract64(ptr, 55, 1)) { return ptr | mask; }
    else {
        return ptr & ~mask;
    }
}

static G_NORETURN void pauth_fail_exception(CPUARMState* env, bool data, int keynumber, uintptr_t ra)
{ raise_exception_ra(env, EXCP_UDEF, syn_pacfail(data, keynumber), exception_target_el(env), ra); }

static uint64_t pauth_auth(CPUARMState* env, uint64_t ptr, uint64_t modifier, ARMPACKey* key, bool data, int keynumber,
                           uintptr_t ra, bool is_combined)
{
    ...
    if (pauth_feature >= PauthFeat_2) {
        ARMPauthFeature fault_feature = is_combined ? PauthFeat_FPACCOMBINED : PauthFeat_FPAC;
        uint64_t        result        = ptr ^ (pac & cmp_mask);

        if (pauth_feature >= fault_feature && ((result ^ sextract64(result, 55, 1)) & cmp_mask)) {
    ...
    if ((pac ^ ptr) & cmp_mask) {
        ...
        if (param.tbi) { return deposit64(orig_ptr, 53, 2, error_code); }
        else {
...
static void pauth_check_trap(CPUARMState* env, int el, uintptr_t ra)
{
    ...
    if (el < 3 && arm_feature(env, ARM_FEATURE_EL3)) {
        if (!(env->cp15.scr_el3 & SCR_API)) { pauth_trap(env, 3, ra); }
    }
}

static bool pauth_key_enabled(CPUARMState* env, int el, uint32_t bit)
{
    if (el > 0 && (env->cp15.apctl_el1 & APCTL_AppleMode)) { return true; }
...
uint64_t HELPER(pacia)(CPUARMState* env, uint64_t x, uint64_t y)
{
    ...
    if (!pauth_key_enabled(env, el, SCTLR_EnIA)) { return x; }
...
{
    int el = arm_current_el(env);
    if (!pauth_key_enabled(env, el, SCTLR_EnIB)) { return x; }
    pauth_check_trap(env, el, GETPC());
...
{
    int el = arm_current_el(env);
    if (!pauth_key_enabled(env, el, SCTLR_EnDA)) { return x; }
    pauth_check_trap(env, el, GETPC());
...
{
    int el = arm_current_el(env);
    if (!pauth_key_enabled(env, el, SCTLR_EnDB)) { return x; }
    pauth_check_trap(env, el, GETPC());
...
static uint64_t pauth_autia(CPUARMState* env, uint64_t x, uint64_t y, uintptr_t ra, bool is_combined)
{
    int el = arm_current_el(env);
    if (!pauth_key_enabled(env, el, SCTLR_EnIA)) { return x; }
...
uint64_t HELPER(autia)(CPUARMState* env, uint64_t x, uint64_t y) { return pauth_autia(env, x, y, GETPC(), false); }

uint64_t HELPER(autia_combined)(CPUARMState* env, uint64_t x, uint64_t y)
{ return pauth_autia(env, x, y, GETPC(), true); }

static uint64_t pauth_autib(CPUARMState* env, uint64_t x, uint64_t y, uintptr_t ra, bool is_combined)
{
    int el = arm_current_el(env);
    if (!pauth_key_enabled(env, el, SCTLR_EnIB)) { return x; }
    pauth_check_trap(env, el, ra);
...
uint64_t HELPER(autib)(CPUARMState* env, uint64_t x, uint64_t y) { return pauth_autib(env, x, y, GETPC(), false); }

uint64_t HELPER(autib_combined)(CPUARMState* env, uint64_t x, uint64_t y)
{ return pauth_autib(env, x, y, GETPC(), true); }

static uint64_t pauth_autda(CPUARMState* env, uint64_t x, uint64_t y, uintptr_t ra, bool is_combined)
{
    int el = arm_current_el(env);
    if (!pauth_key_enabled(env, el, SCTLR_EnDA)) { return x; }
    pauth_check_trap(env, el, ra);
...
uint64_t HELPER(autda)(CPUARMState* env, uint64_t x, uint64_t y) { return pauth_autda(env, x, y, GETPC(), false); }

uint64_t HELPER(autda_combined)(CPUARMState* env, uint64_t x, uint64_t y)
{ return pauth_autda(env, x, y, GETPC(), true); }

static uint64_t pauth_autdb(CPUARMState* env, uint64_t x, uint64_t y, uintptr_t ra, bool is_combined)
{
    int el = arm_current_el(env);
    if (!pauth_key_enabled(env, el, SCTLR_EnDB)) { return x; }
    pauth_check_trap(env, el, ra);
...
uint64_t HELPER(autdb)(CPUARMState* env, uint64_t x, uint64_t y) { return pauth_autdb(env, x, y, GETPC(), false); }

uint64_t HELPER(autdb_combined)(CPUARMState* env, uint64_t x, uint64_t y)
{ return pauth_autdb(env, x, y, GETPC(), true); }

uint64_t HELPER(xpaci)(CPUARMState* env, uint64_t a) { return pauth_strip(env, a, false); }

uint64_t HELPER(xpacd)(CPUARMState* env, uint64_t a) { return pauth_strip(env, a, true); }
