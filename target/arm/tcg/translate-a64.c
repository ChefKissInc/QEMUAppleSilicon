...
static int scale_by_log2_tag_granule(DisasContext* s, int x) { return x << LOG2_TAG_GRANULE; }
...
void a64_translate_init(void)
{
    ...
    for (i = 0; i < 32; i++) {
        cpu_X[i] = tcg_global_mem_new_i64(tcg_env, offsetof(CPUARMState, xregs[i]), regnames[i]);
    }

    cpu_exclusive_high = tcg_global_mem_new_i64(tcg_env, offsetof(CPUARMState, exclusive_high), "exclusive_high");
}
...
static void set_btype_raw(int val) { tcg_gen_st_i32(tcg_constant_i32(val), tcg_env, offsetof(CPUARMState, btype)); }
...
{
    assert(s->pc_save != -1);
    if (tb_cflags(s->base.tb) & CF_PCREL) { tcg_gen_addi_i64(dest, cpu_pc, (s->pc_curr - s->pc_save) + diff); }
    else {
 ...
 */
static void gen_top_byte_ignore(DisasContext* s, TCGv_i64 dst, TCGv_i64 src, int tbi)
{
    if (tbi == 0) {
...
static void gen_probe_access(DisasContext* s, TCGv_i64 ptr, MMUAccessType acc, int log2_size)
{
    gen_helper_probe_access(tcg_env, ptr, tcg_constant_i32(acc), tcg_constant_i32(get_mem_index(s)),
 ...
 */
static TCGv_i64 gen_mte_check1_mmuidx(DisasContext* s, TCGv_i64 addr, bool is_write, bool tag_checked, MemOp memop,
                                      bool is_unpriv, int core_idx)
{
    TCGv_i64 ret;
...
TCGv_i64 gen_mte_check1(DisasContext* s, TCGv_i64 addr, bool is_write, bool tag_checked, MemOp memop)
{ return gen_mte_check1_mmuidx(s, addr, is_write, tag_checked, memop, false, get_mem_index(s)); }

/*
 * For MTE, check multiple logical sequential accesses.
 */
TCGv_i64 gen_mte_checkN(DisasContext* s, TCGv_i64 addr, bool is_write, bool tag_checked, int total_size,
                        MemOp single_mop)
{
    TCGv_i64 ret;
 ...
 */
static void check_lse2_align(DisasContext* s, int rn, int imm, bool is_write, MemOp mop)
{
    TCGv_i32      tmp;
    TCGv_i64      addr;
    TCGLabel*     over_label;
    ...
    type = is_write ? MMU_DATA_STORE : MMU_DATA_LOAD, mmu_idx = get_mem_index(s);
    gen_helper_unaligned_access(tcg_env, addr, tcg_constant_i32(type), tcg_constant_i32(mmu_idx));
...
static MemOp check_atomic_align(DisasContext* s, int rn, MemOp mop)
{
     ...
     * be quadword aligned.
     */
    if (size == MO_128) { return finalize_memop_atom(s, MO_128 | MO_ALIGN, MO_ATOM_IFALIGN_PAIR); }
    if (dc_isar_feature(aa64_lse2, s)) { check_lse2_align(s, rn, 0, true, mop); }
    else {
        mop |= MO_ALIGN;
    }
...
static MemOp check_ordered_align(DisasContext* s, int rn, int imm, bool is_write, MemOp mop)
{
    MemOp size = mop & MO_SIZE;

    if (size == MO_8) { return mop; }
    if (size == MO_128) { return finalize_memop_atom(s, MO_128 | MO_ALIGN, MO_ATOM_IFALIGN_PAIR); }
    if (!dc_isar_feature(aa64_lse2, s)) { mop |= MO_ALIGN; }
    else if (!s->naa) {
...
static void gen_goto_tb(DisasContext* s, int n, int64_t diff)
{
    if (use_goto_tb(s, s->pc_curr + diff)) {
        ...
        gen_a64_update_pc(s, diff);
        if (s->ss_active) { gen_step_complete_exception(s); }
        else {
...
TCGv_i64 cpu_reg_sp(DisasContext* s, int reg) { return cpu_X[reg]; }
...
TCGv_i64 read_cpu_reg(DisasContext* s, int reg, int sf)
{
    ...
    if (reg != 31) {
        if (sf) { tcg_gen_mov_i64(v, cpu_X[reg]); }
        else {
 ...
 */
static inline int fp_reg_offset(DisasContext* s, int regno, MemOp size) { return vec_reg_offset(s, regno, 0, size); }

/* Offset of the high half of the 128 bit vector Qn */
static inline int fp_reg_hi_offset(DisasContext* s, int regno) { return vec_reg_offset(s, regno, 1, MO_64); }
 ...
 */
static void write_fp_dreg_merging(DisasContext* s, int reg, int mergereg, TCGv_i64 v)
{
    if (!s->fpcr_nep) {
     ...
     */
    tcg_gen_gvec_mov(MO_64, vec_full_reg_offset(s, reg), vec_full_reg_offset(s, mergereg), 16, vec_full_reg_size(s));
 ...
 */
static void write_fp_sreg_merging(DisasContext* s, int reg, int mergereg, TCGv_i32 v)
{
    if (!s->fpcr_nep) {
 ...
 */
static void write_fp_hreg_merging(DisasContext* s, int reg, int mergereg, TCGv_i32 v)
{
    if (!s->fpcr_nep) {
        write_fp_sreg(s, reg, v);
        return;
...
static void gen_gvec_fn2(DisasContext* s, bool is_q, int rd, int rn, GVecGen2Fn* gvec_fn, int vece)
{ gvec_fn(vece, vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn), is_q ? 16 : 8, vec_full_reg_size(s)); }

/* Expand a 2-operand + immediate AdvSIMD vector operation using
 * an expander function.
 */
static void gen_gvec_fn2i(DisasContext* s, bool is_q, int rd, int rn, int64_t imm, GVecGen2iFn* gvec_fn, int vece)
{ gvec_fn(vece, vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn), imm, is_q ? 16 : 8, vec_full_reg_size(s)); }

/* Expand a 3-operand AdvSIMD vector operation using an expander function.  */
static void gen_gvec_fn3(DisasContext* s, bool is_q, int rd, int rn, int rm, GVecGen3Fn* gvec_fn, int vece)
{
    gvec_fn(vece, vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn), vec_full_reg_offset(s, rm), is_q ? 16 : 8,
            vec_full_reg_size(s));
}

/* Expand a 4-operand AdvSIMD vector operation using an expander function.  */
static void gen_gvec_fn4(DisasContext* s, bool is_q, int rd, int rn, int rm, int rx, GVecGen4Fn* gvec_fn, int vece)
{
    gvec_fn(vece, vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn), vec_full_reg_offset(s, rm),
            vec_full_reg_offset(s, rx), is_q ? 16 : 8, vec_full_reg_size(s));
}

/* Expand a 2-operand operation using an out-of-line helper.  */
static void gen_gvec_op2_ool(DisasContext* s, bool is_q, int rd, int rn, int data, gen_helper_gvec_2* fn)
{
    tcg_gen_gvec_2_ool(vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn), is_q ? 16 : 8, vec_full_reg_size(s),
                       data, fn);
}

/* Expand a 3-operand operation using an out-of-line helper.  */
static void gen_gvec_op3_ool(DisasContext* s, bool is_q, int rd, int rn, int rm, int data, gen_helper_gvec_3* fn)
{
    tcg_gen_gvec_3_ool(vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn), vec_full_reg_offset(s, rm),
 ...
 */
static void gen_gvec_op3_fpst(DisasContext* s, bool is_q, int rd, int rn, int rm, ARMFPStatusFlavour fpsttype, int data,
                              gen_helper_gvec_3_ptr* fn)
{
    TCGv_ptr fpst = fpstatus_ptr(fpsttype);
    tcg_gen_gvec_3_ptr(vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn), vec_full_reg_offset(s, rm), fpst,
                       is_q ? 16 : 8, vec_full_reg_size(s), data, fn);
}

/* Expand a 4-operand operation using an out-of-line helper.  */
static void gen_gvec_op4_ool(DisasContext* s, bool is_q, int rd, int rn, int rm, int ra, int data,
                             gen_helper_gvec_4* fn)
{
    tcg_gen_gvec_4_ool(vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn), vec_full_reg_offset(s, rm),
                       vec_full_reg_offset(s, ra), is_q ? 16 : 8, vec_full_reg_size(s), data, fn);
}
 ...
 */
static void gen_gvec_op4_env(DisasContext* s, bool is_q, int rd, int rn, int rm, int ra, int data,
                             gen_helper_gvec_4_ptr* fn)
{
    tcg_gen_gvec_4_ptr(vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn), vec_full_reg_offset(s, rm),
                       vec_full_reg_offset(s, ra), tcg_env, is_q ? 16 : 8, vec_full_reg_size(s), data, fn);
}
 ...
 * an out-of-line helper.
 */
static void gen_gvec_op4_fpst(DisasContext* s, bool is_q, int rd, int rn, int rm, int ra, ARMFPStatusFlavour fpsttype,
                              int data, gen_helper_gvec_4_ptr* fn)
{
    TCGv_ptr fpst = fpstatus_ptr(fpsttype);
    tcg_gen_gvec_4_ptr(vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn), vec_full_reg_offset(s, rm),
                       vec_full_reg_offset(s, ra), fpst, is_q ? 16 : 8, vec_full_reg_size(s), data, fn);
}
...
static void gen_vfp_ah_negh(TCGv_i32 d, TCGv_i32 s)
{
    ...
    tcg_gen_movcond_i32(TCG_COND_GTU, d, abs_s, tcg_constant_i32(0x7c00), s, chs_s);
}
...
static void gen_vfp_ah_negs(TCGv_i32 d, TCGv_i32 s)
{
    ...
    tcg_gen_movcond_i32(TCG_COND_GTU, d, abs_s, tcg_constant_i32(0x7f800000UL), s, chs_s);
}
...
static void gen_vfp_ah_negd(TCGv_i64 d, TCGv_i64 s)
{
    ...
    tcg_gen_movcond_i64(TCG_COND_GTU, d, abs_s, tcg_constant_i64(0x7ff0000000000000ULL), s, chs_s);
}
...
static void gen_vfp_ah_absh(TCGv_i32 d, TCGv_i32 s)
{
    ...
    gen_vfp_absh(abs_s, s);
    tcg_gen_movcond_i32(TCG_COND_GTU, d, abs_s, tcg_constant_i32(0x7c00), s, abs_s);
}
...
static void gen_vfp_ah_abss(TCGv_i32 d, TCGv_i32 s)
{
    ...
    gen_vfp_abss(abs_s, s);
    tcg_gen_movcond_i32(TCG_COND_GTU, d, abs_s, tcg_constant_i32(0x7f800000UL), s, abs_s);
}
...
static void gen_vfp_ah_absd(TCGv_i64 d, TCGv_i64 s)
{
    ...
    gen_vfp_absd(abs_s, s);
    tcg_gen_movcond_i64(TCG_COND_GTU, d, abs_s, tcg_constant_i64(0x7ff0000000000000ULL), s, abs_s);
}

static void gen_vfp_maybe_ah_negh(DisasContext* dc, TCGv_i32 d, TCGv_i32 s)
{
    if (dc->fpcr_ah) { gen_vfp_ah_negh(d, s); }
    else {
        gen_vfp_negh(d, s);
    }
}
...
{
    if (dc->fpcr_ah) { gen_vfp_ah_negs(d, s); }
    else {
        gen_vfp_negs(d, s);
    }
}
...
{
    if (dc->fpcr_ah) { gen_vfp_ah_negd(d, s); }
    else {
        gen_vfp_negd(d, s);
    }
}
...
static void gen_adc(int sf, TCGv_i64 dest, TCGv_i64 t0, TCGv_i64 t1)
{
    ...
    if (!sf) { tcg_gen_ext32u_i64(dest, dest); }
}
 ...
 */
static void do_gpr_st_memidx(DisasContext* s, TCGv_i64 source, TCGv_i64 tcg_addr, MemOp memop, int memidx,
                             bool iss_valid, unsigned int iss_srt, bool iss_sf, bool iss_ar)
{
    ...
    if (iss_valid) {
        ...
        syn = syn_data_abort_with_iss(0, (memop & MO_SIZE), false, iss_srt, iss_sf, iss_ar, 0, 0, 0, 0, 0, false);
        disas_set_insn_syndrome(s, syn);
    }
}

static void do_gpr_st(DisasContext* s, TCGv_i64 source, TCGv_i64 tcg_addr, MemOp memop, bool iss_valid,
                      unsigned int iss_srt, bool iss_sf, bool iss_ar)
{ do_gpr_st_memidx(s, source, tcg_addr, memop, get_mem_index(s), iss_valid, iss_srt, iss_sf, iss_ar); }

/*
 * Load from memory to GPR register
 */
static void do_gpr_ld_memidx(DisasContext* s, TCGv_i64 dest, TCGv_i64 tcg_addr, MemOp memop, bool extend, int memidx,
                             bool iss_valid, unsigned int iss_srt, bool iss_sf, bool iss_ar)
{
    ...
    if (iss_valid) {
        ...
        syn = syn_data_abort_with_iss(0, (memop & MO_SIZE), (memop & MO_SIGN) != 0, iss_srt, iss_sf, iss_ar, 0, 0, 0, 0,
                                      0, false);
        disas_set_insn_syndrome(s, syn);
    }
}

static void do_gpr_ld(DisasContext* s, TCGv_i64 dest, TCGv_i64 tcg_addr, MemOp memop, bool extend, bool iss_valid,
                      unsigned int iss_srt, bool iss_sf, bool iss_ar)
{ do_gpr_ld_memidx(s, dest, tcg_addr, memop, extend, get_mem_index(s), iss_valid, iss_srt, iss_sf, iss_ar); }
...
static void do_fp_st(DisasContext* s, int srcidx, TCGv_i64 tcg_addr, MemOp mop)
{
    ...
    if ((mop & MO_SIZE) < MO_128) { tcg_gen_qemu_st_i64(tmplo, tcg_addr, get_mem_index(s), mop); }
    else {
...
static void do_fp_ld(DisasContext* s, int destidx, TCGv_i64 tcg_addr, MemOp mop)
{
    ...
    if ((mop & MO_SIZE) < MO_128) { tcg_gen_qemu_ld_i64(tmplo, tcg_addr, get_mem_index(s), mop); }
    else {
        TCGv_i128 t16 = tcg_temp_new_i128();
    ...
    if (tmphi) { tcg_gen_st_i64(tmphi, tcg_env, fp_reg_hi_offset(s, destidx)); }
...
static void read_vec_element(DisasContext* s, TCGv_i64 tcg_dest, int srcidx, int element, MemOp memop)
{
    int vect_off = vec_reg_offset(s, srcidx, element, memop & MO_SIZE);
    switch ((unsigned)memop) {
        case MO_8           : tcg_gen_ld8u_i64(tcg_dest, tcg_env, vect_off); break;
        case MO_16          : tcg_gen_ld16u_i64(tcg_dest, tcg_env, vect_off); break;
        case MO_32          : tcg_gen_ld32u_i64(tcg_dest, tcg_env, vect_off); break;
        case MO_8 | MO_SIGN : tcg_gen_ld8s_i64(tcg_dest, tcg_env, vect_off); break;
        case MO_16 | MO_SIGN: tcg_gen_ld16s_i64(tcg_dest, tcg_env, vect_off); break;
        case MO_32 | MO_SIGN: tcg_gen_ld32s_i64(tcg_dest, tcg_env, vect_off); break;
        case MO_64          :
        case MO_64 | MO_SIGN: tcg_gen_ld_i64(tcg_dest, tcg_env, vect_off); break;
        default             : tcg_debug_assert_not_reached();
    }
}

static void read_vec_element_i32(DisasContext* s, TCGv_i32 tcg_dest, int srcidx, int element, MemOp memop)
{
    int vect_off = vec_reg_offset(s, srcidx, element, memop & MO_SIZE);
    switch (memop) {
        case MO_8           : tcg_gen_ld8u_i32(tcg_dest, tcg_env, vect_off); break;
        case MO_16          : tcg_gen_ld16u_i32(tcg_dest, tcg_env, vect_off); break;
        case MO_8 | MO_SIGN : tcg_gen_ld8s_i32(tcg_dest, tcg_env, vect_off); break;
        case MO_16 | MO_SIGN: tcg_gen_ld16s_i32(tcg_dest, tcg_env, vect_off); break;
        case MO_32          :
        case MO_32 | MO_SIGN: tcg_gen_ld_i32(tcg_dest, tcg_env, vect_off); break;
        default             : tcg_debug_assert_not_reached();
    }
}

/* Set value of an element within a vector register */
static void write_vec_element(DisasContext* s, TCGv_i64 tcg_src, int destidx, int element, MemOp memop)
{
    int vect_off = vec_reg_offset(s, destidx, element, memop & MO_SIZE);
    switch (memop) {
        case MO_8 : tcg_gen_st8_i64(tcg_src, tcg_env, vect_off); break;
        case MO_16: tcg_gen_st16_i64(tcg_src, tcg_env, vect_off); break;
        case MO_32: tcg_gen_st32_i64(tcg_src, tcg_env, vect_off); break;
        case MO_64: tcg_gen_st_i64(tcg_src, tcg_env, vect_off); break;
        default   : tcg_debug_assert_not_reached();
    }
}

static void write_vec_element_i32(DisasContext* s, TCGv_i32 tcg_src, int destidx, int element, MemOp memop)
{
    int vect_off = vec_reg_offset(s, destidx, element, memop & MO_SIZE);
    switch (memop) {
        case MO_8 : tcg_gen_st8_i32(tcg_src, tcg_env, vect_off); break;
        case MO_16: tcg_gen_st16_i32(tcg_src, tcg_env, vect_off); break;
        case MO_32: tcg_gen_st_i32(tcg_src, tcg_env, vect_off); break;
        default   : tcg_debug_assert_not_reached();
    }
}
...
static void do_vec_ld(DisasContext* s, int destidx, int element, TCGv_i64 tcg_addr, MemOp mop)
{
    TCGv_i64 tcg_tmp = tcg_temp_new_i64();
...
static bool fp_access_check_only(DisasContext* s)
{
    if (s->fp_excp_el) {
        ...
        gen_exception_insn_el(s, 0, EXCP_UDEF, syn_fp_access_trap(1, 0xe, false, 0), s->fp_excp_el);
        return false;
    }
    s->fp_access_checked = 1;
    return true;
...
static bool nonstreaming_check(DisasContext* s)
{
    if (s->sme_trap_nonstreaming && s->is_nonstreaming) {
        gen_exception_insn(s, 0, EXCP_UDEF, syn_smetrap(SME_ET_Streaming, false));
        return false;
    }
    return true;
}

static bool fp_access_check(DisasContext* s) { return fp_access_check_only(s) && nonstreaming_check(s); }
...
static int fp_access_check_scalar_hsd(DisasContext* s, MemOp esz)
{
    switch (esz) {
        ...
        case MO_32: break;
        case MO_16:
            if (!dc_isar_feature(aa64_fp16, s)) { return -1; }
            break;
        default: return -1;
    }
...
bool sve_access_check(DisasContext* s)
{
    if (dc_isar_feature(aa64_sme, s)) {
        ...
        if (s->pstate_sm) { ret = sme_enabled_check(s); }
        ...
        if (ret) { ret = nonstreaming_check(s); }
...
continue_sve:
    if (s->sve_excp_el) {
        ...
        gen_exception_insn_el(s, 0, EXCP_UDEF, syn_sve_access_trap(), s->sve_excp_el);
...
bool sme_enabled_check(DisasContext* s)
{
     ...
     */
    if (s->sme_excp_el && (!s->fp_excp_el || s->sme_excp_el <= s->fp_excp_el)) {
...
{
    if (!sme_enabled_check(s)) { return false; }
    if (REG_FIELD_EX64(req, SVCR, SM) && !s->pstate_sm) {
        gen_exception_insn(s, 0, EXCP_UDEF, syn_smetrap(SME_ET_NotStreaming, false));
        return false;
    }
    if (REG_FIELD_EX64(req, SVCR, ZA) && !s->pstate_za) {
        gen_exception_insn(s, 0, EXCP_UDEF, syn_smetrap(SME_ET_InactiveZA, false));
        return false;
    }
    return true;
}
...
static bool do_gvec_op2_ool(DisasContext* s, arg_qrr_e* a, int data, gen_helper_gvec_2* fn)
{
    if (!a->q && a->esz == MO_64) { return false; }
    if (fp_access_check(s)) { gen_gvec_op2_ool(s, a->q, a->rd, a->rn, data, fn); }
    return true;
}

static bool do_gvec_op3_ool(DisasContext* s, arg_qrrr_e* a, int data, gen_helper_gvec_3* fn)
{
    if (!a->q && a->esz == MO_64) { return false; }
    if (fp_access_check(s)) { gen_gvec_op3_ool(s, a->q, a->rd, a->rn, a->rm, data, fn); }
    return true;
}
...
{
    if (!a->q && a->esz == MO_64) { return false; }
    if (fp_access_check(s)) { gen_gvec_fn3(s, a->q, a->rd, a->rn, a->rm, fn, a->esz); }
    return true;
}

static bool do_gvec_fn3_no64(DisasContext* s, arg_qrrr_e* a, GVecGen3Fn* fn)
{
    if (a->esz == MO_64) { return false; }
    if (fp_access_check(s)) { gen_gvec_fn3(s, a->q, a->rd, a->rn, a->rm, fn, a->esz); }
    return true;
}
...
{
    if (!a->q && a->esz == MO_64) { return false; }
    if (fp_access_check(s)) { gen_gvec_fn4(s, a->q, a->rd, a->rn, a->rm, a->ra, fn, a->esz); }
    return true;
}
 ...
 */
static void ext_and_shift_reg(TCGv_i64 tcg_out, TCGv_i64 tcg_in, int option, unsigned int shift)
{
...
static bool trans_CBZ(DisasContext* s, arg_cbz* a)
{
    ...
    tcg_gen_brcondi_i64(a->nz ? TCG_COND_NE : TCG_COND_EQ, tcg_cmp, 0, match.label);
    gen_goto_tb(s, 0, 4);
...
static bool trans_B_cond(DisasContext* s, arg_B_cond* a)
{
    ...
    if (a->c && !dc_isar_feature(aa64_hbc, s)) { return false; }
    reset_btype(s);
    if (a->cond < 0x0e) {
...
static TCGv_i64 auth_branch_target(DisasContext* s, TCGv_i64 dst, TCGv_i64 modifier, bool use_key_a)
{
    TCGv_i64 truedst;
    /*
     ...
     */
    if (!s->pauth_active) { return dst; }

    truedst = tcg_temp_new_i64();
    if (use_key_a) { gen_helper_autia_combined(truedst, tcg_env, dst, modifier); }
    else {
...
static bool trans_ERET(DisasContext* s, arg_ERET* a)
{
    ...
    if (s->guarded) { tcg_gen_ld_i64(dst, tcg_env, offsetof(CPUARMState, gxf.elr_gl[s->current_el])); }
    else {
...
static bool trans_NOP(DisasContext* s, arg_NOP* a) { return true; }
...
static bool trans_YIELD(DisasContext* s, arg_YIELD* a)
{
     ...
     */
    if (!(tb_cflags(s->base.tb) & CF_PARALLEL)) { s->base.is_jmp = DISAS_YIELD; }
    return true;
}
...
static bool trans_WFE(DisasContext* s, arg_WFI* a)
{
     ...
     * spin unnecessarily we would need to do something more involved.
     */
    if (!(tb_cflags(s->base.tb) & CF_PARALLEL)) { s->base.is_jmp = DISAS_WFE; }
    return true;
}

static bool trans_WFIT(DisasContext* s, arg_WFIT* a)
{
    if (!dc_isar_feature(aa64_wfxt, s)) { return false; }
...
{
    if (s->pauth_active) { gen_helper_xpaci(cpu_X[30], tcg_env, cpu_X[30]); }
    return true;
}

static bool trans_PACIA1716(DisasContext* s, arg_PACIA1716* a)
{
    if (s->pauth_active) { gen_helper_pacia(cpu_X[17], tcg_env, cpu_X[17], cpu_X[16]); }
    return true;
}

static bool trans_PACIB1716(DisasContext* s, arg_PACIB1716* a)
{
    if (s->pauth_active) { gen_helper_pacib(cpu_X[17], tcg_env, cpu_X[17], cpu_X[16]); }
    return true;
}

static bool trans_AUTIA1716(DisasContext* s, arg_AUTIA1716* a)
{
    if (s->pauth_active) { gen_helper_autia(cpu_X[17], tcg_env, cpu_X[17], cpu_X[16]); }
    return true;
}

static bool trans_AUTIB1716(DisasContext* s, arg_AUTIB1716* a)
{
    if (s->pauth_active) { gen_helper_autib(cpu_X[17], tcg_env, cpu_X[17], cpu_X[16]); }
    return true;
}
...
static bool trans_ESB(DisasContext* s, arg_ESB* a)
{
    ...
    if (dc_isar_feature(aa64_ras, s)) {
         ...
         */
        if (s->current_el <= 1 && arm_dc_feature(s, ARM_FEATURE_EL2)) { gen_helper_vesb(tcg_env); }
    }
    return true;
}
...
{
    if (s->pauth_active) { gen_helper_pacia(cpu_X[30], tcg_env, cpu_X[30], tcg_constant_i64(0)); }
    return true;
}

static bool trans_PACIASP(DisasContext* s, arg_PACIASP* a)
{
    if (s->pauth_active) { gen_helper_pacia(cpu_X[30], tcg_env, cpu_X[30], cpu_X[31]); }
    return true;
}

static bool trans_PACIBZ(DisasContext* s, arg_PACIBZ* a)
{
    if (s->pauth_active) { gen_helper_pacib(cpu_X[30], tcg_env, cpu_X[30], tcg_constant_i64(0)); }
    return true;
}

static bool trans_PACIBSP(DisasContext* s, arg_PACIBSP* a)
{
    if (s->pauth_active) { gen_helper_pacib(cpu_X[30], tcg_env, cpu_X[30], cpu_X[31]); }
    return true;
}

static bool trans_AUTIAZ(DisasContext* s, arg_AUTIAZ* a)
{
    if (s->pauth_active) { gen_helper_autia(cpu_X[30], tcg_env, cpu_X[30], tcg_constant_i64(0)); }
    return true;
}

static bool trans_AUTIASP(DisasContext* s, arg_AUTIASP* a)
{
    if (s->pauth_active) { gen_helper_autia(cpu_X[30], tcg_env, cpu_X[30], cpu_X[31]); }
    return true;
}

static bool trans_AUTIBZ(DisasContext* s, arg_AUTIBZ* a)
{
    if (s->pauth_active) { gen_helper_autib(cpu_X[30], tcg_env, cpu_X[30], tcg_constant_i64(0)); }
    return true;
}

static bool trans_AUTIBSP(DisasContext* s, arg_AUTIBSP* a)
{
    if (s->pauth_active) { gen_helper_autib(cpu_X[30], tcg_env, cpu_X[30], cpu_X[31]); }
    return true;
}
...
{
    if (!dc_isar_feature(aa64_sb, s)) { return false; }
    /*
...
{
    if (!dc_isar_feature(aa64_condm_4, s)) { return false; }
...
static bool trans_XAFLAG(DisasContext* s, arg_XAFLAG* a)
{
    ...
    if (!dc_isar_feature(aa64_condm_5, s)) { return false; }
...
{
    if (!dc_isar_feature(aa64_uao, s) || s->current_el == 0) { return false; }
    if (a->imm & 1) { set_pstate_bits(PSTATE_UAO); }
    else {
...
{
    if (!dc_isar_feature(aa64_pan, s) || s->current_el == 0) { return false; }
    if (a->imm & 1) { set_pstate_bits(PSTATE_PAN); }
    else {
...
{
    if (!dc_isar_feature(aa64_ssbs, s)) { return false; }
    if (a->imm & 1) { set_pstate_bits(PSTATE_SSBS); }
    else {
...
{
    if (!dc_isar_feature(aa64_dit, s)) { return false; }
    if (a->imm & 1) { set_pstate_bits(PSTATE_DIT); }
    else {
...
static bool trans_MSR_i_TCO(DisasContext* s, arg_i* a)
{
    if (dc_isar_feature(aa64_mte, s)) {
        ...
        if (a->imm & 1) { set_pstate_bits(PSTATE_TCO); }
        else {
...
{
    if (!dc_isar_feature(aa64_nmi, s) || s->current_el == 0) { return false; }

    if (a->imm == 0) { clear_pstate_bits(PSTATE_ALLINT); }
...
{
    if (!dc_isar_feature(aa64_sme, s) || a->mask == 0) { return false; }
    if (sme_access_check(s)) {
        ...
        if ((old ^ new) & a->mask) {
            ...
            gen_helper_set_svcr(tcg_env, tcg_constant_i32(new), tcg_constant_i32(a->mask));
            s->base.is_jmp = DISAS_TOO_MANY;
        }
    }
    return true;
...
static void gen_sysreg_undef(DisasContext* s, bool isread, uint8_t op0, uint8_t op1, uint8_t op2, uint8_t crn,
                             uint8_t crm, uint8_t rt)
{
    /*
    ...
    if (isread && dc_isar_feature(aa64_ids, s) && arm_cpreg_encoding_in_idspace(op0, op1, op2, crn, crm)) {
                       ...
                       unsigned int crn, unsigned int crm, unsigned int rt)
{
    ...
    if (crn == 11 || crn == 15) {
        ...
        switch (s->current_el) {
            case 0:
                if (dc_isar_feature(aa64_tidcp1, s)) { gen_helper_tidcp_el0(tcg_env, tcg_constant_i32(syndrome)); }
                break;
            case 1: gen_helper_tidcp_el1(tcg_env, tcg_constant_i32(syndrome)); break;
        }
    }
    ...
    if (s->nv2 && ri->nv2_redirect_offset) {
         ...
         */
        if (ri->nv2_redirect_offset & NV2_REDIR_NV1) { nv2_mem_redirect = s->nv1; }
    ...
    if (ri->accessfn || (ri->fgt && s->fgt_active)) {
        ...
        gen_helper_access_check_cp_reg(tcg_ri, tcg_env, tcg_constant_i32(key), tcg_constant_i32(syndrome),
                                       tcg_constant_i32(isread), tcg_constant_i32(s->fgt_active));
    }
    ...
    if (!skip_fp_access_checks) {
        if ((ri->type & ARM_CP_FPU) && !fp_access_check_only(s)) { return; }
    ...
    if (nv_redirect_reg) {
         ...
         */
        key = ENCODE_AA64_CP_REG(CP_REG_ARM64_SYSREG_CP, crn, crm, op0, 0, op2);
    ...
    if (nv2_mem_redirect) {
        ...
        tcg_gen_addi_i64(ptr, ptr, (ri->nv2_redirect_offset & ~NV2_REDIR_FLAG_MASK));
        tcg_rt = cpu_reg(s, rt);

        syn = syn_data_abort_vncr(0, !isread, 0);
        disas_set_insn_syndrome(s, syn);
        if (isread) { tcg_gen_qemu_ld_i64(tcg_rt, ptr, memidx, mop); }
        else {
    ...
    switch (ri->type & ARM_CP_SPECIAL_MASK) {
        case 0         : break;
        case ARM_CP_NOP: return;
        case ARM_CP_NZCV:
            tcg_rt = cpu_reg(s, rt);
            if (isread) { gen_get_nzcv(tcg_rt); }
            else {
                gen_set_nzcv(tcg_rt);
            }
            return;
        ...
        case ARM_CP_DC_ZVA:
            ...
            if (s->mte_active[0]) {
                ...
                gen_helper_mte_check_zva(tcg_rt, tcg_env, tcg_constant_i32(desc), cpu_reg(s, rt));
            }
            else {
                tcg_rt = cpu_reg(s, rt);
            }
    ...
    if (isread) {
        if (ri->type & ARM_CP_CONST) { tcg_gen_movi_i64(tcg_rt, ri->resetvalue); }
        else if (ri->readfn) {
...
static void gen_load_exclusive(DisasContext* s, int rt, int rt2, int rn, int size, bool is_pair)
{
    ...
    if (is_pair) {
        ...
        if (size == 2) {
            ...
            if (s->be_data == MO_LE) { tcg_gen_extr_i128_i64(cpu_exclusive_val, cpu_exclusive_high, t16); }
            else {
                tcg_gen_extr_i128_i64(cpu_exclusive_high, cpu_exclusive_val, t16);
            }
...
static void gen_store_exclusive(DisasContext* s, int rd, int rt, int rt2, int rn, int size, int is_pair)
{
     ...
     */
    memop = size + is_pair;
    if (memop == MO_128 || !dc_isar_feature(aa64_lse2, s)) { memop |= MO_ALIGN; }
    ...
    if (is_pair) {
        if (size == 2) {
            if (s->be_data == MO_LE) { tcg_gen_concat32_i64(tmp, cpu_reg(s, rt), cpu_reg(s, rt2)); }
            else {
                tcg_gen_concat32_i64(tmp, cpu_reg(s, rt2), cpu_reg(s, rt));
            }
            tcg_gen_atomic_cmpxchg_i64(tmp, cpu_exclusive_addr, cpu_exclusive_val, tmp, get_mem_index(s), memop);
            ...
            if (s->be_data == MO_LE) {
                ...
                tcg_gen_concat_i64_i128(c16, cpu_exclusive_high, cpu_exclusive_val);
            }

            tcg_gen_atomic_cmpxchg_i128(t16, cpu_exclusive_addr, c16, t16, get_mem_index(s), memop);

            a = tcg_temp_new_i64();
            b = tcg_temp_new_i64();
            if (s->be_data == MO_LE) { tcg_gen_extr_i128_i64(a, b, t16); }
            else {
        ...
        tcg_gen_atomic_cmpxchg_i64(tmp, cpu_exclusive_addr, cpu_exclusive_val, cpu_reg(s, rt), get_mem_index(s), memop);
        tcg_gen_setcond_i64(TCG_COND_NE, tmp, tmp, cpu_exclusive_val);
    }
...
static void gen_compare_and_swap(DisasContext* s, int rs, int rt, int rn, int size)
{
    ...
    if (rn == 31) { gen_check_sp_alignment(s); }
    memop      = check_atomic_align(s, rn, size);
    clean_addr = gen_mte_check1(s, cpu_reg_sp(s, rn), true, rn != 31, memop);
    tcg_gen_atomic_cmpxchg_i64(tcg_rs, clean_addr, tcg_rs, tcg_rt, memidx, memop);
}
...
static void gen_compare_and_swap_pair(DisasContext* s, int rs, int rt, int rn, int size)
{
    ...
    if (size == 2) {
        ...
        if (s->be_data == MO_LE) { tcg_gen_extr32_i64(s1, s2, cmp); }
        else {
        ...
        if (s->be_data == MO_LE) { tcg_gen_extr_i128_i64(s1, s2, cmp); }
        else {
...
static bool trans_STLR(DisasContext* s, arg_stlr* a)
{
     ...
     */
    if (!a->lasr && !dc_isar_feature(aa64_lor, s)) { return false; }
    /* Generate ISS for non-exclusive accesses including LASR.  */
    if (a->rn == 31) { gen_check_sp_alignment(s); }
    tcg_gen_mb(TCG_MO_ALL | TCG_BAR_STRL);
    memop      = check_ordered_align(s, a->rn, 0, true, a->sz);
    clean_addr = gen_mte_check1(s, cpu_reg_sp(s, a->rn), true, a->rn != 31, memop);
    do_gpr_st(s, cpu_reg(s, a->rt), clean_addr, memop, true, a->rt, iss_sf, a->lasr);
    return true;
}
...
static bool trans_LDAR(DisasContext* s, arg_stlr* a)
{
    ...
    clean_addr = gen_mte_check1(s, cpu_reg_sp(s, a->rn), false, a->rn != 31, memop);
    do_gpr_ld(s, cpu_reg(s, a->rt), clean_addr, memop, false, true, a->rt, iss_sf, a->lasr);
...
{
    if (!dc_isar_feature(aa64_lse, s)) { return false; }
    if (((a->rt | a->rs) & 1) != 0) { return false; }
...
static bool trans_LD_lit(DisasContext* s, arg_ldlit* a)
{
    ...
    do_gpr_ld(s, tcg_rt, clean_addr, memop, false, true, a->rt, iss_sf, false);
    return true;
}
...
static bool trans_LD_lit_v(DisasContext* s, arg_ldlit* a)
{
    ...
    if (!fp_access_check(s)) { return true; }
...
static void op_addr_ldstpair_pre(DisasContext* s, arg_ldstpair* a, TCGv_i64* clean_addr, TCGv_i64* dirty_addr,
                                 uint64_t offset, bool is_store, MemOp mop)
{
    if (a->rn == 31) { gen_check_sp_alignment(s); }

    *dirty_addr = read_cpu_reg_sp(s, a->rn, 1);
    if (!a->p) { tcg_gen_addi_i64(*dirty_addr, *dirty_addr, offset); }

    *clean_addr = gen_mte_checkN(s, *dirty_addr, is_store, (a->w || a->rn != 31), 2 << a->sz, mop);
}

static void op_addr_ldstpair_post(DisasContext* s, arg_ldstpair* a, TCGv_i64 dirty_addr, uint64_t offset)
{
    if (a->w) {
        if (a->p) { tcg_gen_addi_i64(dirty_addr, dirty_addr, offset); }
...
static bool trans_STP(DisasContext* s, arg_ldstpair* a)
{
     ...
     */
    mop = a->sz + 1;
    if (s->align_mem) { mop |= (a->sz == 2 ? MO_ALIGN_4 : MO_ALIGN_8); }
    mop = finalize_memop_pair(s, mop);
    if (a->sz == 2) {
        TCGv_i64 tmp = tcg_temp_new_i64();

        if (s->be_data == MO_LE) { tcg_gen_concat32_i64(tmp, tcg_rt, tcg_rt2); }
        else {
        ...
        if (s->be_data == MO_LE) { tcg_gen_concat_i64_i128(tmp, tcg_rt, tcg_rt2); }
        else {
...
static bool trans_LDP(DisasContext* s, arg_ldstpair* a)
{
    ...
    if (a->sz == 2) {
        ...
        if (s->be_data == MO_LE) { tcg_gen_extr_i128_i64(tcg_rt, tcg_rt2, tmp); }
        else {
...
static bool trans_STGP(DisasContext* s, arg_ldstpair* a)
{
    ...
    if (!dc_isar_feature(aa64_mte_insn_reg, s)) { return false; }
    ...
    if (s->ata[0]) {
        if (tb_cflags(s->base.tb) & CF_PARALLEL) { gen_helper_stg_parallel(tcg_env, dirty_addr, dirty_addr); }
        else {
...
static void op_addr_ldst_imm_pre(DisasContext* s, arg_ldst_imm* a, TCGv_i64* clean_addr, TCGv_i64* dirty_addr,
                                 uint64_t offset, bool is_store, MemOp mop)
{
    int memidx;
    ...
    *clean_addr = gen_mte_check1_mmuidx(s, *dirty_addr, is_store, a->w || a->rn != 31, mop, a->unpriv, memidx);
}

static void op_addr_ldst_imm_post(DisasContext* s, arg_ldst_imm* a, TCGv_i64 dirty_addr, uint64_t offset)
{
    if (a->w) {
        if (a->p) { tcg_gen_addi_i64(dirty_addr, dirty_addr, offset); }
        tcg_gen_mov_i64(cpu_reg_sp(s, a->rn), dirty_addr);
...
static bool trans_STR_i(DisasContext* s, arg_ldst_imm* a)
{
    ...
    do_gpr_st_memidx(s, tcg_rt, clean_addr, mop, memidx, iss_valid, a->rt, iss_sf, false);
...
static bool trans_LDR_i(DisasContext* s, arg_ldst_imm* a)
{
    ...
    do_gpr_ld_memidx(s, tcg_rt, clean_addr, mop, a->ext, memidx, iss_valid, a->rt, iss_sf, false);
    op_addr_ldst_imm_post(s, a, dirty_addr, a->imm);
    return true;
}
...
static void op_addr_ldst_pre(DisasContext* s, arg_ldst* a, TCGv_i64* clean_addr, TCGv_i64* dirty_addr, bool is_store,
                             MemOp memop)
{
    TCGv_i64 tcg_rm;
...
static bool trans_LDR(DisasContext* s, arg_ldst* a)
{
    ...
    if (extract32(a->opt, 1, 1) == 0) { return false; }

    memop = finalize_memop(s, a->sz + a->sign * MO_SIGN);
    op_addr_ldst_pre(s, a, &clean_addr, &dirty_addr, false, memop);
    tcg_rt = cpu_reg(s, a->rt);
    do_gpr_ld(s, tcg_rt, clean_addr, memop, a->ext, true, a->rt, iss_sf, false);
    return true;
}
...
static bool do_atomic_ld(DisasContext* s, arg_atomic* a, AtomicThreeOpFn* fn, int sign, bool invert)
{
    MemOp    mop = a->sz | sign;
    ...
    clean_addr = gen_mte_check1(s, cpu_reg_sp(s, a->rn), false, a->rn != 31, mop);
    tcg_rs     = read_cpu_reg(s, a->rs, true);
    tcg_rt     = cpu_reg(s, a->rt);
    if (invert) { tcg_gen_not_i64(tcg_rs, tcg_rs); }
    /*
    ...
    if (mop & MO_SIGN) {
        switch (a->sz) {
            case MO_8 : tcg_gen_ext8u_i64(tcg_rt, tcg_rt); break;
            case MO_16: tcg_gen_ext16u_i64(tcg_rt, tcg_rt); break;
            case MO_32: tcg_gen_ext32u_i64(tcg_rt, tcg_rt); break;
            case MO_64: break;
            default   : tcg_debug_assert_not_reached();
        }
    }
...
static bool do_atomic128_ld(DisasContext* s, arg_atomic128* a, Atomic128ThreeOpFn* fn, bool invert)
{
    MemOp     mop;
    int       rlo, rhi;
    ...
    if (a->rt == 31 || a->rt2 == 31 || a->rt == a->rt2) { return false; }
    if (a->rn == 31) { gen_check_sp_alignment(s); }
...
TRANS_FEAT(LDCLRP, aa64_lse128, do_atomic128_ld, a, tcg_gen_atomic_fetch_and_i128, true)
TRANS_FEAT(LDSETP, aa64_lse128, do_atomic128_ld, a, tcg_gen_atomic_fetch_or_i128, false)
TRANS_FEAT(SWPP, aa64_lse128, do_atomic128_ld, a, tcg_gen_atomic_xchg_i128, false)
...
static bool trans_LDAPR(DisasContext* s, arg_LDAPR* a)
{
    ...
    if (!dc_isar_feature(aa64_lse, s) || !dc_isar_feature(aa64_rcpc_8_3, s)) { return false; }
    if (a->rn == 31) { gen_check_sp_alignment(s); }
     ...
     */
    do_gpr_ld(s, cpu_reg(s, a->rt), clean_addr, mop, false, true, a->rt, iss_sf, true);
    tcg_gen_mb(TCG_MO_ALL | TCG_BAR_LDAQ);
    return true;
}
...
static bool trans_LDRA(DisasContext* s, arg_LDRA* a)
{
    ...
    if (s->pauth_active) {
        if (!a->m) { gen_helper_autda_combined(dirty_addr, tcg_env, dirty_addr, tcg_constant_i64(0)); }
        else {
            gen_helper_autdb_combined(dirty_addr, tcg_env, dirty_addr, tcg_constant_i64(0));
        }
    }
    ...
    clean_addr = gen_mte_check1(s, dirty_addr, false, a->w || a->rn != 31, memop);
    ...
    if (a->w) { tcg_gen_mov_i64(cpu_reg_sp(s, a->rn), dirty_addr); }
    return true;
}
...
static bool trans_LDAPR_i(DisasContext* s, arg_ldapr_stlr_i* a)
{
    ...
    if (!dc_isar_feature(aa64_rcpc_8_4, s)) { return false; }
     ...
     */
    do_gpr_ld(s, cpu_reg(s, a->rt), dirty_addr, mop, a->ext, true, a->rt, iss_sf, true);
    tcg_gen_mb(TCG_MO_ALL | TCG_BAR_LDAQ);
    return true;
}
...
static bool trans_LD_mult(DisasContext* s, arg_ldst_mult* a)
{
    ...
    if (size == 3 && !a->q && a->selem != 1) { return false; }
    if (!fp_access_check(s)) { return true; }
     ...
     */
    clean_addr = gen_mte_checkN(s, tcg_rn, false, a->p || a->rn != 31, total, finalize_memop_asimd(s, size));
    ...
    if (a->p) {
        if (a->rm == 31) { tcg_gen_addi_i64(tcg_rn, tcg_rn, total); }
        else {
...
static bool trans_ST_mult(DisasContext* s, arg_ldst_mult* a)
{
     ...
     * promote consecutive little-endian elements below.
     */
    clean_addr = gen_mte_checkN(s, tcg_rn, true, a->p || a->rn != 31, total, finalize_memop_asimd(s, size));
...
static bool trans_ST_single(DisasContext* s, arg_ldst_single* a)
{
    ...
    clean_addr = gen_mte_checkN(s, tcg_rn, true, a->p || a->rn != 31, total, mop);
...
static bool trans_LD_single(DisasContext* s, arg_ldst_single* a)
{
    ...
    mop        = finalize_memop_asimd(s, a->scale);
    clean_addr = gen_mte_checkN(s, tcg_rn, false, a->p || a->rn != 31, total, mop);
...
static bool trans_LD_single_repl(DisasContext* s, arg_LD_single_repl* a)
{
    ...
    for (xs = 0, rt = a->rt; xs < a->selem; xs++, rt = (rt + 1) % 32) {
        ...
        tcg_gen_gvec_dup_i64(a->scale, vec_full_reg_offset(s, rt), (a->q + 1) * 8, vec_full_reg_size(s), tcg_tmp);
        tcg_gen_add_i64(clean_addr, clean_addr, tcg_ebytes);
    }
...
static bool trans_STZGM(DisasContext* s, arg_ldst_tag* a)
{
    ...
    if (!dc_isar_feature(aa64_mte, s)) { return false; }
    if (s->current_el == 0) { return false; }
    ...
    if (s->ata[0]) { gen_helper_stzgm_tags(tcg_env, addr, tcg_rt); }
    /*
...
static bool trans_STGM(DisasContext* s, arg_ldst_tag* a)
{
    ...
    if (s->ata[0]) { gen_helper_stgm(tcg_env, addr, tcg_rt); }
    else {
...
static bool trans_LDGM(DisasContext* s, arg_ldst_tag* a)
{
    ...
    if (s->ata[0]) { gen_helper_ldgm(tcg_rt, tcg_env, addr); }
    else {
...
static bool trans_LDG(DisasContext* s, arg_ldst_tag* a)
{
    ...
    tcg_rt = cpu_reg(s, a->rt);
    if (s->ata[0]) { gen_helper_ldg(tcg_rt, tcg_env, addr, tcg_rt); }
    else {
        /*
...
static bool do_STG(DisasContext* s, arg_ldst_tag* a, bool is_zero, bool is_pair)
{
    ...
    if (!s->ata[0]) {
         ...
         */
        if (is_pair) { gen_helper_st2g_stub(tcg_env, addr); }
        else {
            gen_helper_stg_stub(tcg_env, addr);
        }
    }
    else if (tb_cflags(s->base.tb) & CF_PARALLEL) {
        if (is_pair) { gen_helper_st2g_parallel(tcg_env, addr, tcg_rt); }
        else {
            gen_helper_stg_parallel(tcg_env, addr, tcg_rt);
        }
    }
    else {
        if (is_pair) { gen_helper_st2g(tcg_env, addr, tcg_rt); }
        else {
...
static bool do_SET(DisasContext* s, arg_set* a, bool is_epilogue, bool is_setg, SetFn fn)
{
    int      memidx;
    uint32_t syndrome, desc = 0;

    if (is_setg && !dc_isar_feature(aa64_mte, s)) { return false; }
     ...
     */
    if (a->rs == a->rn || a->rs == a->rd || a->rn == a->rd || a->rd == 31 || a->rn == 31) { return false; }
     ...
     */
    syndrome = syn_mop(true, is_setg, (a->nontemp << 1) | a->unpriv, is_epilogue, false, true, a->rd, a->rs, a->rn);
...
static bool do_CPY(DisasContext* s, arg_cpy* a, bool is_epilogue, CpyFn fn)
{
     ...
     * (which we do in the helper function)
     */
    if (a->rs == a->rn || a->rs == a->rd || a->rn == a->rd || a->rd == 31 || a->rs == 31 || a->rn == 31) {
        return false;
    }
     ...
     * we pass wrong_option == false: helper function may set that bit.
     */
    syndrome = syn_mop(false, false, a->options, is_epilogue, false, true, a->rd, a->rs, a->rn);
     ...
     * than passing in an extra three integer arguments.
     */
    fn(tcg_env, tcg_constant_i32(syndrome), tcg_constant_i32(wdesc), tcg_constant_i32(rdesc));
    return true;
}
...
static bool gen_rri(DisasContext* s, arg_rri_sf* a, bool rd_sp, bool rn_sp, ArithTwoOp* fn)
{
...
static bool gen_add_sub_imm_with_tags(DisasContext* s, arg_rri_tag* a, bool sub_op)
{
    ...
    if (s->ata[0]) { gen_helper_addsubg(tcg_rd, tcg_env, tcg_rn, tcg_constant_i32(imm), tcg_constant_i32(a->uimm4)); }
    else {
...
static bool gen_rri_log(DisasContext* s, arg_rri_log* a, bool set_cc, void (*fn)(TCGv_i64, TCGv_i64, int64_t))
{
...
static bool trans_EXTR(DisasContext* s, arg_extract* a)
{
    ...
    if (unlikely(a->imm == 0)) {
         ...
         */
        if (a->sf) { tcg_gen_mov_i64(tcg_rd, cpu_reg(s, a->rm)); }
        else {
        ...
        if (a->sf) {
            ...
            if (a->rm == a->rn) { tcg_gen_rotri_i32(t0, t0, a->imm); }
            else {
...
static bool trans_TBL_TBX(DisasContext* s, arg_TBL_TBX* a)
{
    if (fp_access_check(s)) {
        ...
        tcg_gen_gvec_2_ptr(vec_full_reg_offset(s, a->rd), vec_full_reg_offset(s, a->rm), tcg_env, a->q ? 16 : 8,
                           vec_full_reg_size(s), (len << 6) | (a->tbx << 5) | a->rn, gen_helper_simd_tblx);
    }
    return true;
}
...
static bool do_simd_permute(DisasContext* s, arg_qrrr_e* a, simd_permute_idx_fn* fn, int part)
{
    MemOp    esz      = a->esz;
    ...
    for (int i = 0; i < elements; i++) {
        ...
        read_vec_element(s, tcg_ele, (idx & elements ? a->rm : a->rn), idx & (elements - 1), esz);

        w = (i << (esz + 3)) / 64;
        o = (i << (esz + 3)) % 64;
        if (o == 0) { tcg_gen_mov_i64(tcg_res[w], tcg_ele); }
        else {
            tcg_gen_deposit_i64(tcg_res[w], tcg_res[w], tcg_ele, o, 8 << esz);
        }
    }

    for (int i = a->q; i >= 0; --i) { write_vec_element(s, tcg_res[i], a->rd, i, MO_64); }
    clear_vec_high(s, a->q, a->rd);
    return true;
}

static int permute_load_uzp(int i, int part, int elements) { return 2 * i + part; }

TRANS(UZP1, do_simd_permute, a, permute_load_uzp, 0)
TRANS(UZP2, do_simd_permute, a, permute_load_uzp, 1)

static int permute_load_trn(int i, int part, int elements) { return (i & 1) * elements + (i & ~1) + part; }

TRANS(TRN1, do_simd_permute, a, permute_load_trn, 0)
TRANS(TRN2, do_simd_permute, a, permute_load_trn, 1)

static int permute_load_zip(int i, int part, int elements) { return (i & 1) * elements + ((part * elements + i) >> 1); }
...
{
    if (!dc_isar_feature(aa64_sm3, s)) { return false; }
    if (fp_access_check(s)) {
...
{
    if (fp_access_check(s)) { gen_gvec_op3_ool(s, true, a->rd, a->rn, a->rm, a->imm, fn); }
    return true;
}
...
{
    if (!dc_isar_feature(aa64_sha3, s)) { return false; }
    if (fp_access_check(s)) {
        gen_gvec_xar(MO_64, vec_full_reg_offset(s, a->rd), vec_full_reg_offset(s, a->rn), vec_full_reg_offset(s, a->rm),
                     a->imm, 16, vec_full_reg_size(s));
    }
    return true;
}
...
static bool trans_DUP_element_s(DisasContext* s, arg_DUP_element_s* a)
{
    ...
    if (!decode_esz_idx(a->imm, &esz, &idx)) { return false; }
    if (fp_access_check(s)) {
        /*
...
static bool trans_DUP_element_v(DisasContext* s, arg_DUP_element_v* a)
{
    ...
    if (esz == MO_64 && !a->q) { return false; }
    if (fp_access_check(s)) {
        tcg_gen_gvec_dup_mem(esz, vec_full_reg_offset(s, a->rd), vec_reg_offset(s, a->rn, idx, esz), a->q ? 16 : 8,
                             vec_full_reg_size(s));
    }
    return true;
}
...
static bool trans_DUP_general(DisasContext* s, arg_DUP_general* a)
{
    ...
    if (esz == MO_64 && !a->q) { return false; }
    if (fp_access_check(s)) {
        tcg_gen_gvec_dup_i64(esz, vec_full_reg_offset(s, a->rd), a->q ? 16 : 8, vec_full_reg_size(s),
                             cpu_reg(s, a->rn));
    }
    return true;
}
...
static bool do_smov_umov(DisasContext* s, arg_SMOV* a, MemOp is_signed)
{
    ...
    if (!decode_esz_idx(a->imm, &esz, &idx)) { return false; }
    if (is_signed) {
        if (esz == MO_64 || (esz == MO_32 && !a->q)) { return false; }
    }
    else {
        if (esz == MO_64 ? !a->q : a->q) { return false; }
    }
    if (fp_access_check(s)) {
        TCGv_i64 tcg_rd = cpu_reg(s, a->rd);
        read_vec_element(s, tcg_rd, a->rn, idx, esz | is_signed);
        if (is_signed && !a->q) { tcg_gen_ext32u_i64(tcg_rd, tcg_rd); }
    }
    return true;
}
...
static bool trans_INS_element(DisasContext* s, arg_INS_element* a)
{
    ...
    if (!decode_esz_idx(a->di, &esz, &didx)) { return false; }
    sidx = a->si >> esz;
    if (fp_access_check(s)) {
        TCGv_i64 tmp = tcg_temp_new_i64();
...
static bool do_fp3_scalar_with_fpsttype(DisasContext* s, arg_rrr_e* a, const FPScalar* f, int mergereg,
...
static bool do_fp3_scalar(DisasContext* s, arg_rrr_e* a, const FPScalar* f, int mergereg)
{ return do_fp3_scalar_with_fpsttype(s, a, f, mergereg, a->esz == MO_16 ? FPST_A64_F16 : FPST_A64); }

static bool do_fp3_scalar_ah_2fn(DisasContext* s, arg_rrr_e* a, const FPScalar* fnormal, const FPScalar* fah,
                                 int mergereg)
{ return do_fp3_scalar_with_fpsttype(s, a, s->fpcr_ah ? fah : fnormal, mergereg, select_ah_fpst(s, a->esz)); }

/* Some insns need to call different helpers when FPCR.AH == 1 */
static bool do_fp3_scalar_2fn(DisasContext* s, arg_rrr_e* a, const FPScalar* fnormal, const FPScalar* fah, int mergereg)
...
TRANS(FRECPS_s, do_fp3_scalar_ah_2fn, a, &f_scalar_frecps, &f_scalar_ah_frecps, a->rn)
...
TRANS(FRSQRTS_s, do_fp3_scalar_ah_2fn, a, &f_scalar_frsqrts, &f_scalar_ah_frsqrts, a->rn)

static bool do_fcmp0_s(DisasContext* s, arg_rr_e* a, const FPScalar* f, bool swap)
{
    switch (a->esz) {
        case MO_64:
            if (fp_access_check(s)) {
                ...
                if (swap) { f->gen_d(t0, t1, t0, fpstatus_ptr(FPST_A64)); }
                else {
        ...
        case MO_32:
            if (fp_access_check(s)) {
                ...
                if (swap) { f->gen_s(t0, t1, t0, fpstatus_ptr(FPST_A64)); }
                else {
        ...
        case MO_16:
            ...
            if (fp_access_check(s)) {
                TCGv_i32 t0 = read_fp_hreg(s, a->rn);
                TCGv_i32 t1 = tcg_constant_i32(0);
                if (swap) { f->gen_h(t0, t1, t0, fpstatus_ptr(FPST_A64_F16)); }
                else {
...
static bool do_satacc_s(DisasContext* s, arg_rrr_e* a, MemOp sgn_n, MemOp sgn_m,
                        ...
                        void (*gen_d)(TCGv_i64, TCGv_i64, TCGv_i64, TCGv_i64))
{
    ...
    if (esz == MO_64) { gen_d(t0, qc, t1, t2); }
    else {
...
static bool do_int3_scalar_d(DisasContext* s, arg_rrr_e* a, void (*fn)(TCGv_i64, TCGv_i64, TCGv_i64))
{
    if (fp_access_check(s)) {
...
static const ENVScalar2 f_scalar_sqshl = {
    {gen_helper_neon_qshl_s8, gen_helper_neon_qshl_s16, gen_helper_neon_qshl_s32},
    gen_helper_neon_qshl_s64,
};
TRANS(SQSHL_s, do_env_scalar2, a, &f_scalar_sqshl)

static const ENVScalar2 f_scalar_uqshl = {
    {gen_helper_neon_qshl_u8, gen_helper_neon_qshl_u16, gen_helper_neon_qshl_u32},
    gen_helper_neon_qshl_u64,
};
TRANS(UQSHL_s, do_env_scalar2, a, &f_scalar_uqshl)

static const ENVScalar2 f_scalar_sqrshl = {
    {gen_helper_neon_qrshl_s8, gen_helper_neon_qrshl_s16, gen_helper_neon_qrshl_s32},
    gen_helper_neon_qrshl_s64,
};
TRANS(SQRSHL_s, do_env_scalar2, a, &f_scalar_sqrshl)

static const ENVScalar2 f_scalar_uqrshl = {
    {gen_helper_neon_qrshl_u8, gen_helper_neon_qrshl_u16, gen_helper_neon_qrshl_u32},
    gen_helper_neon_qrshl_u64,
};
TRANS(UQRSHL_s, do_env_scalar2, a, &f_scalar_uqrshl)

static bool do_env_scalar2_hs(DisasContext* s, arg_rrr_e* a, const ENVScalar2* f)
{
    if (a->esz == MO_16 || a->esz == MO_32) { return do_env_scalar2(s, a, f); }
    return false;
}

static const ENVScalar2 f_scalar_sqdmulh = {{NULL, gen_helper_neon_qdmulh_s16, gen_helper_neon_qdmulh_s32}};
TRANS(SQDMULH_s, do_env_scalar2_hs, a, &f_scalar_sqdmulh)

static const ENVScalar2 f_scalar_sqrdmulh = {{NULL, gen_helper_neon_qrdmulh_s16, gen_helper_neon_qrdmulh_s32}};
...
static bool do_env_scalar3_hs(DisasContext* s, arg_rrr_e* a, const ENVScalar3* f)
{
    TCGv_i32 t0, t1, t2;

    if (a->esz != MO_16 && a->esz != MO_32) { return false; }
    if (!fp_access_check(s)) { return true; }
...
static const ENVScalar3 f_scalar_sqrdmlah = {{gen_helper_neon_qrdmlah_s16, gen_helper_neon_qrdmlah_s32}};
TRANS_FEAT(SQRDMLAH_s, aa64_rdm, do_env_scalar3_hs, a, &f_scalar_sqrdmlah)

static const ENVScalar3 f_scalar_sqrdmlsh = {{gen_helper_neon_qrdmlsh_s16, gen_helper_neon_qrdmlsh_s32}};
...
static bool do_fp3_vector_with_fpsttype(DisasContext* s, arg_qrrr_e* a, int data, gen_helper_gvec_3_ptr* const fns[3],
                                        ARMFPStatusFlavour fpsttype)
{
    MemOp esz   = a->esz;
    ...
    gen_gvec_op3_fpst(s, a->q, a->rd, a->rn, a->rm, fpsttype, data, fns[esz - 1]);
    return true;
}

static bool do_fp3_vector(DisasContext* s, arg_qrrr_e* a, int data, gen_helper_gvec_3_ptr* const fns[3])
{ return do_fp3_vector_with_fpsttype(s, a, data, fns, a->esz == MO_16 ? FPST_A64_F16 : FPST_A64); }

static bool do_fp3_vector_2fn(DisasContext* s, arg_qrrr_e* a, int data, gen_helper_gvec_3_ptr* const fnormal[3],
                              gen_helper_gvec_3_ptr* const fah[3])
{ return do_fp3_vector(s, a, data, s->fpcr_ah ? fah : fnormal); }

static bool do_fp3_vector_ah_2fn(DisasContext* s, arg_qrrr_e* a, int data, gen_helper_gvec_3_ptr* const fnormal[3],
                                 gen_helper_gvec_3_ptr* const fah[3])
{ return do_fp3_vector_with_fpsttype(s, a, data, s->fpcr_ah ? fah : fnormal, select_ah_fpst(s, a->esz)); }
...
static bool do_fmlal(DisasContext* s, arg_qrrr_e* a, bool is_s, bool is_2)
{
    if (fp_access_check(s)) {
        ...
        tcg_gen_gvec_3_ptr(vec_full_reg_offset(s, a->rd), vec_full_reg_offset(s, a->rn), vec_full_reg_offset(s, a->rm),
                           tcg_env, a->q ? 16 : 8, vec_full_reg_size(s), data, gen_helper_gvec_fmlal_a64);
    }
    return true;
}
...
{
    if (fp_access_check(s)) { gen_gvec_fn4(s, is_q, d, a, b, c, tcg_gen_gvec_bitsel, 0); }
    return true;
}
...
{
    if (a->esz == MO_64 && !a->q) { return false; }
    if (fp_access_check(s)) {
        tcg_gen_gvec_cmp(cond, a->esz, vec_full_reg_offset(s, a->rd), vec_full_reg_offset(s, a->rn),
                         vec_full_reg_offset(s, a->rm), a->q ? 16 : 8, vec_full_reg_size(s));
    }
    return true;
}
...
static bool do_dot_vector(DisasContext* s, arg_qrrr_e* a, gen_helper_gvec_4* fn)
{
    if (fp_access_check(s)) { gen_gvec_op4_ool(s, a->q, a->rd, a->rn, a->rm, a->rd, 0, fn); }
    return true;
}

static bool do_dot_vector_env(DisasContext* s, arg_qrrr_e* a, gen_helper_gvec_4_ptr* fn)
{
    if (fp_access_check(s)) { gen_gvec_op4_env(s, a->q, a->rd, a->rn, a->rm, a->rd, 0, fn); }
    return true;
}
...
{
    if (!dc_isar_feature(aa64_bf16, s)) { return false; }
    if (fp_access_check(s)) {
        /* Q bit selects BFMLALB vs BFMLALT. */
        gen_gvec_op4_fpst(s, true, a->rd, a->rn, a->rm, a->rd, s->fpcr_ah ? FPST_AH : FPST_A64, a->q,
 ...
 */
TRANS_FEAT(FCADD_90, aa64_fcma, do_fp3_vector, a, 0 | (s->fpcr_ah << 1), f_vector_fcadd)
TRANS_FEAT(FCADD_270, aa64_fcma, do_fp3_vector, a, 1 | (s->fpcr_ah << 1), f_vector_fcadd)
...
static bool trans_FCMLA_v(DisasContext* s, arg_FCMLA_v* a)
{
    ...
    if (!dc_isar_feature(aa64_fcma, s)) { return false; }

    check = fp_access_check_vector_hsd(s, a->q, a->esz);
    if (check <= 0) { return check == 0; }

    gen_gvec_op4_fpst(s, a->q, a->rd, a->rn, a->rm, a->rd, a->esz == MO_16 ? FPST_A64_F16 : FPST_A64,
...
static bool do_3op_widening(DisasContext* s, MemOp memop, int top, int rd, int rn, int rm, int idx,
                            NeonGenTwo64OpFn* fn, bool acc)
{
    ...
    if (esz >= MO_64) { return false; }
    if (!fp_access_check(s)) { return true; }

    if (idx >= 0) { read_vec_element(s, tcg_op2, rm, idx, memop); }
    ...
    for (int elt_fwd = 0; elt_fwd < half; ++elt_fwd) {
        ...
        if (idx < 0) { read_vec_element(s, tcg_op2, rm, elt + top_half, memop); }
        if (acc) { read_vec_element(s, tcg_op0, rd, elt, memop + 1); }
...
TRANS(SMULL_v, do_3op_widening, a->esz | MO_SIGN, a->q, a->rd, a->rn, a->rm, -1, tcg_gen_mul_i64, false)
TRANS(UMULL_v, do_3op_widening, a->esz, a->q, a->rd, a->rn, a->rm, -1, tcg_gen_mul_i64, false)
TRANS(SMLAL_v, do_3op_widening, a->esz | MO_SIGN, a->q, a->rd, a->rn, a->rm, -1, gen_muladd_i64, true)
TRANS(UMLAL_v, do_3op_widening, a->esz, a->q, a->rd, a->rn, a->rm, -1, gen_muladd_i64, true)
TRANS(SMLSL_v, do_3op_widening, a->esz | MO_SIGN, a->q, a->rd, a->rn, a->rm, -1, gen_mulsub_i64, true)
TRANS(UMLSL_v, do_3op_widening, a->esz, a->q, a->rd, a->rn, a->rm, -1, gen_mulsub_i64, true)

TRANS(SMULL_vi, do_3op_widening, a->esz | MO_SIGN, a->q, a->rd, a->rn, a->rm, a->idx, tcg_gen_mul_i64, false)
TRANS(UMULL_vi, do_3op_widening, a->esz, a->q, a->rd, a->rn, a->rm, a->idx, tcg_gen_mul_i64, false)
TRANS(SMLAL_vi, do_3op_widening, a->esz | MO_SIGN, a->q, a->rd, a->rn, a->rm, a->idx, gen_muladd_i64, true)
TRANS(UMLAL_vi, do_3op_widening, a->esz, a->q, a->rd, a->rn, a->rm, a->idx, gen_muladd_i64, true)
TRANS(SMLSL_vi, do_3op_widening, a->esz | MO_SIGN, a->q, a->rd, a->rn, a->rm, a->idx, gen_mulsub_i64, true)
TRANS(UMLSL_vi, do_3op_widening, a->esz, a->q, a->rd, a->rn, a->rm, a->idx, gen_mulsub_i64, true)
...
TRANS(SADDL_v, do_3op_widening, a->esz | MO_SIGN, a->q, a->rd, a->rn, a->rm, -1, tcg_gen_add_i64, false)
TRANS(UADDL_v, do_3op_widening, a->esz, a->q, a->rd, a->rn, a->rm, -1, tcg_gen_add_i64, false)
TRANS(SSUBL_v, do_3op_widening, a->esz | MO_SIGN, a->q, a->rd, a->rn, a->rm, -1, tcg_gen_sub_i64, false)
TRANS(USUBL_v, do_3op_widening, a->esz, a->q, a->rd, a->rn, a->rm, -1, tcg_gen_sub_i64, false)
TRANS(SABDL_v, do_3op_widening, a->esz | MO_SIGN, a->q, a->rd, a->rn, a->rm, -1, gen_sabd_i64, false)
TRANS(UABDL_v, do_3op_widening, a->esz, a->q, a->rd, a->rn, a->rm, -1, gen_uabd_i64, false)
TRANS(SABAL_v, do_3op_widening, a->esz | MO_SIGN, a->q, a->rd, a->rn, a->rm, -1, gen_saba_i64, true)
TRANS(UABAL_v, do_3op_widening, a->esz, a->q, a->rd, a->rn, a->rm, -1, gen_uaba_i64, true)
...
TRANS(SQDMULL_v, do_3op_widening, a->esz | MO_SIGN, a->q, a->rd, a->rn, a->rm, -1,
      a->esz == MO_16 ? gen_sqdmull_h : gen_sqdmull_s, false)
TRANS(SQDMLAL_v, do_3op_widening, a->esz | MO_SIGN, a->q, a->rd, a->rn, a->rm, -1,
      a->esz == MO_16 ? gen_sqdmlal_h : gen_sqdmlal_s, true)
TRANS(SQDMLSL_v, do_3op_widening, a->esz | MO_SIGN, a->q, a->rd, a->rn, a->rm, -1,
      a->esz == MO_16 ? gen_sqdmlsl_h : gen_sqdmlsl_s, true)

TRANS(SQDMULL_vi, do_3op_widening, a->esz | MO_SIGN, a->q, a->rd, a->rn, a->rm, a->idx,
      a->esz == MO_16 ? gen_sqdmull_h : gen_sqdmull_s, false)
TRANS(SQDMLAL_vi, do_3op_widening, a->esz | MO_SIGN, a->q, a->rd, a->rn, a->rm, a->idx,
      a->esz == MO_16 ? gen_sqdmlal_h : gen_sqdmlal_s, true)
TRANS(SQDMLSL_vi, do_3op_widening, a->esz | MO_SIGN, a->q, a->rd, a->rn, a->rm, a->idx,
      a->esz == MO_16 ? gen_sqdmlsl_h : gen_sqdmlsl_s, true)

static bool do_addsub_wide(DisasContext* s, arg_qrrr_e* a, MemOp sign, bool sub)
{
    ...
    for (int elt_fwd = 0; elt_fwd < half; ++elt_fwd) {
        ...
        if (sub) { tcg_gen_sub_i64(tcg_op0, tcg_op0, tcg_op1); }
        else {
...
static bool do_addsub_highnarrow(DisasContext* s, arg_qrrr_e* a, bool sub, bool round)
{
    TCGv_i64 tcg_op0, tcg_op1;
    MemOp    esz   = a->esz;
    int      half  = 8 >> esz;
    ...
    for (int elt_fwd = 0; elt_fwd < half; ++elt_fwd) {
        ...
        if (round) { tcg_gen_addi_i64(tcg_op0, tcg_op0, rbit); }
    ...
    switch (a->esz) {
        case MO_64:
            if (fp_access_check(s)) {
                ...
                if (neg) { gen_vfp_maybe_ah_negd(s, t1, t1); }
        ...
        case MO_32:
            if (fp_access_check(s)) {
                ...
                if (neg) { gen_vfp_maybe_ah_negs(s, t1, t1); }
        ...
        case MO_16:
            ...
            if (fp_access_check(s)) {
                ...
                if (neg) { gen_vfp_maybe_ah_negh(s, t1, t1); }
                gen_helper_advsimd_muladdh(t0, t1, t2, t0, fpstatus_ptr(FPST_A64_F16));
...
static bool do_env_scalar2_idx_hs(DisasContext* s, arg_rrx_e* a, const ENVScalar2* f)
{
    if (a->esz < MO_16 || a->esz > MO_32) { return false; }
    if (fp_access_check(s)) {
        TCGv_i32 t0 = tcg_temp_new_i32();
...
static bool do_env_scalar3_idx_hs(DisasContext* s, arg_rrx_e* a, const ENVScalar3* f)
{
    if (a->esz < MO_16 || a->esz > MO_32) { return false; }
    if (fp_access_check(s)) {
        TCGv_i32 t0 = tcg_temp_new_i32();
...
static bool do_scalar_muladd_widening_idx(DisasContext* s, arg_rrx_e* a, NeonGenTwo64OpFn* fn, bool acc)
{
    if (fp_access_check(s)) {
        TCGv_i64 t0 = tcg_temp_new_i64();
        TCGv_i64 t1 = tcg_temp_new_i64();
        ...
        if (acc) { read_vec_element(s, t0, a->rd, 0, a->esz + 1); }
...
TRANS(SQDMULL_si, do_scalar_muladd_widening_idx, a, a->esz == MO_16 ? gen_sqdmull_h : gen_sqdmull_s, false)
TRANS(SQDMLAL_si, do_scalar_muladd_widening_idx, a, a->esz == MO_16 ? gen_sqdmlal_h : gen_sqdmlal_s, true)
TRANS(SQDMLSL_si, do_scalar_muladd_widening_idx, a, a->esz == MO_16 ? gen_sqdmlsl_h : gen_sqdmlsl_s, true)

static bool do_fp3_vector_idx(DisasContext* s, arg_qrrx_e* a, gen_helper_gvec_3_ptr* const fns[3])
{
    MemOp esz   = a->esz;
    int   check = fp_access_check_vector_hsd(s, a->q, esz);
    ...
    gen_gvec_op3_fpst(s, a->q, a->rd, a->rn, a->rm, esz == MO_16 ? FPST_A64_F16 : FPST_A64, a->idx, fns[esz - 1]);
    return true;
}
...
static bool do_fmla_vector_idx(DisasContext* s, arg_qrrx_e* a, bool neg)
{
    static gen_helper_gvec_4_ptr* const fns[3][3] = {
        {gen_helper_gvec_fmla_idx_h, gen_helper_gvec_fmla_idx_s, gen_helper_gvec_fmla_idx_d},
        {gen_helper_gvec_fmls_idx_h, gen_helper_gvec_fmls_idx_s, gen_helper_gvec_fmls_idx_d},
        {gen_helper_gvec_ah_fmls_idx_h, gen_helper_gvec_ah_fmls_idx_s, gen_helper_gvec_ah_fmls_idx_d},
    };
    MemOp esz   = a->esz;
    int   check = fp_access_check_vector_hsd(s, a->q, esz);
    ...
    gen_gvec_op4_fpst(s, a->q, a->rd, a->rn, a->rm, a->rd, esz == MO_16 ? FPST_A64_F16 : FPST_A64, a->idx,
                      fns[neg ? 1 + s->fpcr_ah : 0][esz - 1]);
    return true;
}
...
static bool do_fmlal_idx(DisasContext* s, arg_qrrx_e* a, bool is_s, bool is_2)
{
    if (fp_access_check(s)) {
        ...
        tcg_gen_gvec_3_ptr(vec_full_reg_offset(s, a->rd), vec_full_reg_offset(s, a->rn), vec_full_reg_offset(s, a->rm),
                           tcg_env, a->q ? 16 : 8, vec_full_reg_size(s), data, gen_helper_gvec_fmlal_idx_a64);
    }
    return true;
}
...
static bool do_int3_vector_idx(DisasContext* s, arg_qrrx_e* a, gen_helper_gvec_3* const fns[2])
{
    assert(a->esz == MO_16 || a->esz == MO_32);
    if (fp_access_check(s)) { gen_gvec_op3_ool(s, a->q, a->rd, a->rn, a->rm, a->idx, fns[a->esz - 1]); }
    return true;
}
...
static bool do_mla_vector_idx(DisasContext* s, arg_qrrx_e* a, bool sub)
{
    ...
    assert(a->esz == MO_16 || a->esz == MO_32);
    if (fp_access_check(s)) { gen_gvec_op4_ool(s, a->q, a->rd, a->rn, a->rm, a->rd, a->idx, fns[a->esz - 1][sub]); }
    return true;
}
...
static bool do_int3_qc_vector_idx(DisasContext* s, arg_qrrx_e* a, gen_helper_gvec_4* const fns[2])
{
    assert(a->esz == MO_16 || a->esz == MO_32);
    if (fp_access_check(s)) {
        tcg_gen_gvec_4_ool(vec_full_reg_offset(s, a->rd), vec_full_reg_offset(s, a->rn), vec_full_reg_offset(s, a->rm),
                           offsetof(CPUARMState, vfp.qc), a->q ? 16 : 8, vec_full_reg_size(s), a->idx, fns[a->esz - 1]);
    }
    return true;
}
...
TRANS_FEAT(SQRDMLAH_vi, aa64_rdm, do_int3_qc_vector_idx, a, f_vector_idx_sqrdmlah)

static gen_helper_gvec_4* const f_vector_idx_sqrdmlsh[2] = {
    gen_helper_neon_sqrdmlsh_idx_h,
    gen_helper_neon_sqrdmlsh_idx_s,
};
TRANS_FEAT(SQRDMLSH_vi, aa64_rdm, do_int3_qc_vector_idx, a, f_vector_idx_sqrdmlsh)

static bool do_dot_vector_idx(DisasContext* s, arg_qrrx_e* a, gen_helper_gvec_4* fn)
{
    if (fp_access_check(s)) { gen_gvec_op4_ool(s, a->q, a->rd, a->rn, a->rm, a->rd, a->idx, fn); }
    return true;
}

static bool do_dot_vector_idx_env(DisasContext* s, arg_qrrx_e* a, gen_helper_gvec_4_ptr* fn)
{
    if (fp_access_check(s)) { gen_gvec_op4_env(s, a->q, a->rd, a->rn, a->rm, a->rd, a->idx, fn); }
    return true;
}
...
TRANS_FEAT(SUDOT_vi, aa64_i8mm, do_dot_vector_idx, a, gen_helper_gvec_sudot_idx_4b)
TRANS_FEAT(USDOT_vi, aa64_i8mm, do_dot_vector_idx, a, gen_helper_gvec_usdot_idx_4b)
TRANS_FEAT(BFDOT_vi, aa64_bf16, do_dot_vector_idx_env, a, gen_helper_gvec_bfdot_idx)
...
static bool trans_BFMLAL_vi(DisasContext* s, arg_qrrx_e* a)
{
    ...
    if (fp_access_check(s)) {
        /* Q bit selects BFMLALB vs BFMLALT. */
        gen_gvec_op4_fpst(s, true, a->rd, a->rn, a->rm, a->rd, s->fpcr_ah ? FPST_AH : FPST_A64, (a->idx << 1) | a->q,
...
static bool trans_FCMLA_vi(DisasContext* s, arg_FCMLA_vi* a)
{
    ...
    switch (a->esz) {
        case MO_16:
            ...
            break;
        case MO_32: fn = gen_helper_gvec_fcmlas_idx; break;
        default   : tcg_debug_assert_not_reached();
    }
    if (fp_access_check(s)) {
        gen_gvec_op4_fpst(s, a->q, a->rd, a->rn, a->rm, a->rd, a->esz == MO_16 ? FPST_A64_F16 : FPST_A64,
...
static bool do_fp3_scalar_pair_2fn(DisasContext* s, arg_rr_e* a, const FPScalar* fnormal, const FPScalar* fah)
...
static bool trans_FCSEL(DisasContext* s, arg_FCSEL* a)
{
    ...
    tcg_gen_movcond_i64(c.cond, t_true, c.value, tcg_constant_i64(0), t_true, t_false);
    ...
    switch (a->esz) {
        case MO_64:
            if (fp_access_check(s)) {
                ...
                if (neg_a) { gen_vfp_maybe_ah_negd(s, ta, ta); }
                if (neg_n) { gen_vfp_maybe_ah_negd(s, tn, tn); }
        ...
        case MO_32:
            if (fp_access_check(s)) {
                ...
                if (neg_a) { gen_vfp_maybe_ah_negs(s, ta, ta); }
                if (neg_n) { gen_vfp_maybe_ah_negs(s, tn, tn); }
                fpst = fpstatus_ptr(FPST_A64);
        ...
        case MO_16:
            ...
            if (fp_access_check(s)) {
                ...
                if (neg_a) { gen_vfp_maybe_ah_negh(s, ta, ta); }
                if (neg_n) { gen_vfp_maybe_ah_negh(s, tn, tn); }
...
static bool do_int_reduction(DisasContext* s, arg_qrr_e* a, bool widen, MemOp src_sign, NeonGenTwo64OpFn* fn)
{
 ...
 */
static TCGv_i32 do_reduction_op(DisasContext* s, int rn, MemOp esz, int ebase, int ecount, TCGv_ptr fpst,
...
static bool do_fp_reduction(DisasContext* s, arg_qrr_e* a, NeonGenTwoSingleOpFn* fnormal, NeonGenTwoSingleOpFn* fah)
{
    if (fp_access_check(s)) {
        MemOp    esz  = a->esz;
        int      elts = (a->q ? 16 : 8) >> esz;
        TCGv_ptr fpst = fpstatus_ptr(esz == MO_16 ? FPST_A64_F16 : FPST_A64);
        TCGv_i32 res  = do_reduction_op(s, a->rn, esz, 0, elts, fpst, s->fpcr_ah ? fah : fnormal);
        write_fp_sreg(s, a->rd, res);
    }
    return true;
}

TRANS_FEAT(FMAXNMV_h, aa64_fp16, do_fp_reduction, a, gen_helper_vfp_maxnumh, gen_helper_vfp_maxnumh)
TRANS_FEAT(FMINNMV_h, aa64_fp16, do_fp_reduction, a, gen_helper_vfp_minnumh, gen_helper_vfp_minnumh)
TRANS_FEAT(FMAXV_h, aa64_fp16, do_fp_reduction, a, gen_helper_vfp_maxh, gen_helper_vfp_ah_maxh)
TRANS_FEAT(FMINV_h, aa64_fp16, do_fp_reduction, a, gen_helper_vfp_minh, gen_helper_vfp_ah_minh)

TRANS(FMAXNMV_s, do_fp_reduction, a, gen_helper_vfp_maxnums, gen_helper_vfp_maxnums)
...
static void handle_fp_compare(DisasContext* s, int size, unsigned int rn, unsigned int rm, bool cmp_with_zero,
                              bool signal_all_nans)
{
    ...
    if (size == MO_64) {
        ...
        if (cmp_with_zero) { tcg_vm = tcg_constant_i64(0); }
        else {
            tcg_vm = read_fp_dreg(s, rm);
        }
        if (signal_all_nans) { gen_helper_vfp_cmped_a64(tcg_flags, tcg_vn, tcg_vm, fpst); }
        else {
        ...
        if (cmp_with_zero) { tcg_gen_movi_i32(tcg_vm, 0); }
        else {
        ...
        switch (size) {
            case MO_32:
                if (signal_all_nans) { gen_helper_vfp_cmpes_a64(tcg_flags, tcg_vn, tcg_vm, fpst); }
                else {
                    gen_helper_vfp_cmps_a64(tcg_flags, tcg_vn, tcg_vm, fpst);
                }
                break;
            case MO_16:
                if (signal_all_nans) { gen_helper_vfp_cmpeh_a64(tcg_flags, tcg_vn, tcg_vm, fpst); }
                else {
...
static bool trans_FCCMP(DisasContext* s, arg_FCCMP* a)
{
    ...
    if (label_continue) { gen_set_label(label_continue); }
    return true;
}
...
static bool trans_FMOVI_v_h(DisasContext* s, arg_FMOVI_v_h* a)
{
    if (!dc_isar_feature(aa64_fp16, s)) { return false; }
    if (fp_access_check(s)) {
        tcg_gen_gvec_dup_imm(MO_16, vec_full_reg_offset(s, a->rd), a->q ? 16 : 8, vec_full_reg_size(s),
                             vfp_expand_imm(MO_16, a->abcdefgh));
    }
    return true;
}

static void gen_movi(unsigned vece, uint32_t dofs, uint32_t aofs, int64_t c, uint32_t oprsz, uint32_t maxsz)
...
static bool trans_Vimm(DisasContext* s, arg_Vimm* a)
{
    ...
    if ((a->cmode & 1) && a->cmode < 12) {
        ...
        if (a->cmode == 15 && a->op == 1 && a->q == 0) { return false; }
        fn = gen_movi;
    }
...
{
    if (fp_access_check(s)) { gen_gvec_fn2i(s, a->q, a->rd, a->rn, a->imm, fn, a->esz); }
    return true;
}
...
static bool do_vec_shift_imm_wide(DisasContext* s, arg_qrri_e* a, bool is_u)
{
    ...
    for (int i = 0, elements = 8 >> esz; i < elements; i++) {
        if (is_u) { tcg_gen_extract_i64(tcg_rd, tcg_rn, i * esize, esize); }
        else {
...
{
    assert(shift >= 0 && shift <= 64);
    if (shift == 64) { tcg_gen_movi_i64(dst, 0); }
    else {
...
{
    assert(shift >= 0 && shift <= 64);
    if (shift == 0) { tcg_gen_mov_i64(dst, src); }
    else if (shift == 64) {
...
static bool do_vec_shift_imm_narrow(DisasContext* s, arg_qrri_e* a, WideShiftImmFn* const fns[3], MemOp sign)
{
    TCGv_i64        tcg_rn, tcg_rd;
    int             esz = a->esz;
    int             esize;
...
static bool do_scalar_shift_imm(DisasContext* s, arg_rri_e* a, WideShiftImmFn* fn, bool accumulate, MemOp sign)
{
    if (fp_access_check(s)) {
        ...
        if (accumulate) { read_vec_element(s, rd, a->rd, 0, a->esz | sign); }
        fn(rd, rn, a->imm);
...
static void trunc_i64_env_imm(TCGv_i64 d, TCGv_i64 s, int64_t i, NeonGenTwoOpEnvFn* fn)
{
...
static void gen_sqshli_b(TCGv_i64 d, TCGv_i64 s, int64_t i) { trunc_i64_env_imm(d, s, i, gen_helper_neon_qshl_s8); }

static void gen_sqshli_h(TCGv_i64 d, TCGv_i64 s, int64_t i) { trunc_i64_env_imm(d, s, i, gen_helper_neon_qshl_s16); }

static void gen_sqshli_s(TCGv_i64 d, TCGv_i64 s, int64_t i) { trunc_i64_env_imm(d, s, i, gen_helper_neon_qshl_s32); }

static void gen_sqshli_d(TCGv_i64 d, TCGv_i64 s, int64_t i)
{ gen_helper_neon_qshl_s64(d, tcg_env, s, tcg_constant_i64(i)); }

static void gen_uqshli_b(TCGv_i64 d, TCGv_i64 s, int64_t i) { trunc_i64_env_imm(d, s, i, gen_helper_neon_qshl_u8); }

static void gen_uqshli_h(TCGv_i64 d, TCGv_i64 s, int64_t i) { trunc_i64_env_imm(d, s, i, gen_helper_neon_qshl_u16); }

static void gen_uqshli_s(TCGv_i64 d, TCGv_i64 s, int64_t i) { trunc_i64_env_imm(d, s, i, gen_helper_neon_qshl_u32); }

static void gen_uqshli_d(TCGv_i64 d, TCGv_i64 s, int64_t i)
{ gen_helper_neon_qshl_u64(d, tcg_env, s, tcg_constant_i64(i)); }

static void gen_sqshlui_b(TCGv_i64 d, TCGv_i64 s, int64_t i) { trunc_i64_env_imm(d, s, i, gen_helper_neon_qshlu_s8); }

static void gen_sqshlui_h(TCGv_i64 d, TCGv_i64 s, int64_t i) { trunc_i64_env_imm(d, s, i, gen_helper_neon_qshlu_s16); }

static void gen_sqshlui_s(TCGv_i64 d, TCGv_i64 s, int64_t i) { trunc_i64_env_imm(d, s, i, gen_helper_neon_qshlu_s32); }

static void gen_sqshlui_d(TCGv_i64 d, TCGv_i64 s, int64_t i)
{ gen_helper_neon_qshlu_s64(d, tcg_env, s, tcg_constant_i64(i)); }

static WideShiftImmFn* const f_scalar_sqshli[] = {gen_sqshli_b, gen_sqshli_h, gen_sqshli_s, gen_sqshli_d};

static WideShiftImmFn* const f_scalar_uqshli[] = {gen_uqshli_b, gen_uqshli_h, gen_uqshli_s, gen_uqshli_d};

static WideShiftImmFn* const f_scalar_sqshlui[] = {gen_sqshlui_b, gen_sqshlui_h, gen_sqshlui_s, gen_sqshlui_d};
...
static bool do_scalar_shift_imm_narrow(DisasContext* s, arg_rri_e* a, WideShiftImmFn* const fns[3], MemOp sign,
                                       bool zext)
{
    MemOp esz = a->esz;
    ...
    if (fp_access_check(s)) {
        ...
        fns[esz](rd, rn, a->imm);
        if (zext) { tcg_gen_ext_i64(rd, rd, esz); }
        write_fp_dreg(s, a->rd, rd);
    }
    return true;
}
...
static bool do_div(DisasContext* s, arg_rrr_sf* a, bool is_signed)
{
    ...
    if (is_signed) { gen_helper_sdiv64(tcg_rd, tcg_n, tcg_m); }
    else {
 ...
 */
static void shift_reg(TCGv_i64 dst, TCGv_i64 src, int sf, enum a64_shift_type shift_type, TCGv_i64 shift_amount)
{
    switch (shift_type) {
        case A64_SHIFT_TYPE_LSL: tcg_gen_shl_i64(dst, src, shift_amount); break;
        case A64_SHIFT_TYPE_LSR: tcg_gen_shr_i64(dst, src, shift_amount); break;
        case A64_SHIFT_TYPE_ASR:
            if (!sf) { tcg_gen_ext32s_i64(dst, src); }
            tcg_gen_sar_i64(dst, sf ? src : dst, shift_amount);
            break;
        case A64_SHIFT_TYPE_ROR:
            if (sf) { tcg_gen_rotr_i64(dst, src, shift_amount); }
            else {
                TCGv_i32 t0, t1;
                t0 = tcg_temp_new_i32();
                t1 = tcg_temp_new_i32();
 ...
 */
static void shift_reg_imm(TCGv_i64 dst, TCGv_i64 src, int sf, enum a64_shift_type shift_type, unsigned int shift_i)
{
    assert(shift_i < (sf ? 64 : 32));

    if (shift_i == 0) { tcg_gen_mov_i64(dst, src); }
    else {
        shift_reg(dst, src, sf, shift_type, tcg_constant_i64(shift_i));
    }
}

static bool do_shift_reg(DisasContext* s, arg_rrr_sf* a, enum a64_shift_type shift_type)
{
...
static bool do_crc32(DisasContext* s, arg_rrr_e* a, bool crc32c)
{
    ...
    switch (a->esz) {
        ...
        case MO_32:
            ...
            break;
        case MO_64: tcg_val = cpu_reg(s, a->rm); break;
        default   : tcg_debug_assert_not_reached();
    }
    ...
    if (crc32c) { gen_helper_crc32c_64(tcg_rd, tcg_acc, tcg_val, tcg_bytes); }
    else {
...
static bool do_subp(DisasContext* s, arg_rrr* a, bool setflag)
{
    ...
    if (setflag) { gen_sub_CC(true, tcg_d, tcg_n, tcg_m); }
    else {
...
static bool trans_IRG(DisasContext* s, arg_rrr* a)
{
    if (dc_isar_feature(aa64_mte_insn_reg, s)) {
        ...
        if (s->ata[0]) { gen_helper_irg(tcg_rd, tcg_env, tcg_rn, cpu_reg(s, a->rm)); }
        else {
...
static void gen_rev_32(TCGv_i64 tcg_rd, TCGv_i64 tcg_rn) { tcg_gen_bswap32_i64(tcg_rd, tcg_rn, TCG_BSWAP_OZ); }
...
static void gen_clz64(TCGv_i64 tcg_rd, TCGv_i64 tcg_rn) { tcg_gen_clzi_i64(tcg_rd, tcg_rn, 64); }
...
static bool do_logic_reg(DisasContext* s, arg_logic_shift* a, ArithTwoOp* fn, ArithTwoOp* inv_fn, bool setflags)
{
    TCGv_i64 tcg_rd, tcg_rn, tcg_rm;

    if (!a->sf && (a->sa & (1 << 5))) { return false; }
    ...
    if (!a->sf) { tcg_gen_ext32u_i64(tcg_rd, tcg_rd); }
    if (setflags) { gen_logic_CC(a->sf, tcg_rd); }
    return true;
}
...
static bool trans_ORR_r(DisasContext* s, arg_logic_shift* a)
{
    ...
    if (a->sa == 0 && a->st == 0 && a->rn == 31) {
        ...
        if (a->n) {
            ...
            if (a->sf) { tcg_gen_mov_i64(tcg_rd, tcg_rm); }
            else {
...
static inline bool do_addsub_ext(DisasContext* s, arg_addsub_ext* a, bool sub_op, bool setflags)
{
    TCGv_i64 tcg_rm, tcg_rn, tcg_rd;

    if (a->sa > 4) { return false; }

    /* non-flag setting ops may use SP */
    if (!setflags) { tcg_rd = cpu_reg_sp(s, a->rd); }
    else {
        tcg_rd = cpu_reg(s, a->rd);
    }
    ...
    if (!setflags) {
        if (sub_op) { tcg_gen_sub_i64(tcg_rd, tcg_rn, tcg_rm); }
        else {
        ...
        if (sub_op) { gen_sub_CC(a->sf, tcg_rd, tcg_rn, tcg_rm); }
        else {
...
static inline bool do_addsub_reg(DisasContext* s, arg_addsub_shift* a, bool sub_op, bool setflags)
{
    TCGv_i64 tcg_rd, tcg_rn, tcg_rm;

    if (a->st == 3 || (!a->sf && (a->sa & 32))) { return false; }
...
static bool do_mulh(DisasContext* s, arg_rrr* a, void (*fn)(TCGv_i64, TCGv_i64, TCGv_i64, TCGv_i64))
{
...
static bool do_muladd(DisasContext* s, arg_rrrr* a, bool sf, bool is_sub, MemOp mop)
{
    TCGv_i64 tcg_rd = cpu_reg(s, a->rd);
    ...
    if (a->ra == 31 && !is_sub) {
        ...
        if (is_sub) { tcg_gen_sub_i64(tcg_rd, tcg_ra, tcg_tmp); }
        else {
            tcg_gen_add_i64(tcg_rd, tcg_ra, tcg_tmp);
        }
    }

    if (!sf) { tcg_gen_ext32u_i64(tcg_rd, tcg_rd); }
    return true;
}
...
static bool do_adc_sbc(DisasContext* s, arg_rrr_sf* a, bool is_sub, bool setflags)
{
    ...
    if (setflags) { gen_adc_CC(a->sf, tcg_rd, tcg_rn, tcg_y); }
    else {
...
static bool trans_CCMP(DisasContext* s, arg_CCMP* a)
{
    ...
    if (a->imm) { tcg_y = tcg_constant_i64(a->y); }
    else {
        tcg_y = cpu_reg(s, a->y);
    }
    tcg_rn = cpu_reg(s, a->rn);
    ...
    if (a->op) { gen_sub_CC(a->sf, tcg_tmp, tcg_rn, tcg_y); }
    else {
    ...
    if (nzcv & 8) { /* N */
        ...
        if (has_andc) { tcg_gen_andc_i32(cpu_NF, cpu_NF, tcg_t1); }
        else {
            tcg_gen_and_i32(cpu_NF, cpu_NF, tcg_t2);
        }
    }
    if (nzcv & 4) { /* Z */
        if (has_andc) { tcg_gen_andc_i32(cpu_ZF, cpu_ZF, tcg_t1); }
        else {
    ...
    if (nzcv & 2) { /* C */
        ...
        if (has_andc) { tcg_gen_andc_i32(cpu_CF, cpu_CF, tcg_t1); }
        else {
    ...
    if (nzcv & 1) { /* V */
        ...
        if (has_andc) { tcg_gen_andc_i32(cpu_VF, cpu_VF, tcg_t1); }
        else {
...
static bool trans_CSEL(DisasContext* s, arg_CSEL* a)
{
    ...
    if (a->rn == 31 && a->rm == 31 && (a->else_inc ^ a->else_inv)) {
        /* CSET & CSETM.  */
        if (a->else_inv) { tcg_gen_negsetcond_i64(tcg_invert_cond(c.cond), tcg_rd, c.value, zero); }
        else {
        ...
        if (a->else_inv && a->else_inc) { tcg_gen_neg_i64(t_false, t_false); }
        else if (a->else_inv) {
...
static bool do_fp1_scalar_int(DisasContext* s, arg_rr_e* a, const FPScalar1Int* f, bool merging)
{
    switch (a->esz) {
        case MO_64:
            if (fp_access_check(s)) {
                ...
                f->gen_d(t, t);
                if (merging) { write_fp_dreg_merging(s, a->rd, a->rd, t); }
                else {
        ...
        case MO_32:
            if (fp_access_check(s)) {
                ...
                f->gen_s(t, t);
                if (merging) { write_fp_sreg_merging(s, a->rd, a->rd, t); }
                else {
        ...
        case MO_16:
            ...
            if (fp_access_check(s)) {
                ...
                f->gen_h(t, t);
                if (merging) { write_fp_hreg_merging(s, a->rd, a->rd, t); }
                else {
                    write_fp_sreg(s, a->rd, t);
                }
            }
...
static bool do_fp1_scalar_int_2fn(DisasContext* s, arg_rr_e* a, const FPScalar1Int* fnormal, const FPScalar1Int* fah)
...
static bool do_fp1_scalar_with_fpsttype(DisasContext* s, arg_rr_e* a, const FPScalar1* f, int rmode,
                                        ARMFPStatusFlavour fpsttype)
{
    ...
    if (rmode >= 0) { tcg_rmode = gen_set_rmode(rmode, fpst); }
    ...
    if (rmode >= 0) { gen_restore_rmode(tcg_rmode, fpst); }
    return true;
}

static bool do_fp1_scalar(DisasContext* s, arg_rr_e* a, const FPScalar1* f, int rmode)
{ return do_fp1_scalar_with_fpsttype(s, a, f, rmode, a->esz == MO_16 ? FPST_A64_F16 : FPST_A64); }

static bool do_fp1_scalar_ah(DisasContext* s, arg_rr_e* a, const FPScalar1* f, int rmode)
...
TRANS_FEAT(FRINT32Z_s, aa64_frint, do_fp1_scalar, a, &f_scalar_frint32, FPROUNDING_ZERO)
...
TRANS_FEAT(FRINT64Z_s, aa64_frint, do_fp1_scalar, a, &f_scalar_frint64, FPROUNDING_ZERO)
...
TRANS(FRECPE_s, do_fp1_scalar_ah, a,
      s->fpcr_ah&& dc_isar_feature(aa64_rpres, s) ? &f_scalar_frecpe_rpres : &f_scalar_frecpe, -1)
...
TRANS(FRSQRTE_s, do_fp1_scalar_ah, a,
      s->fpcr_ah&& dc_isar_feature(aa64_rpres, s) ? &f_scalar_frsqrte_rpres : &f_scalar_frsqrte, -1)
...
static bool do_cvtf_scalar(DisasContext* s, MemOp esz, int rd, int shift, TCGv_i64 tcg_int, bool is_signed)
{
    ...
    switch (esz) {
        case MO_64:
            ...
            if (is_signed) { gen_helper_vfp_sqtod(tcg_double, tcg_int, tcg_shift, tcg_fpstatus); }
            else {
        ...
        case MO_32:
            ...
            if (is_signed) { gen_helper_vfp_sqtos(tcg_single, tcg_int, tcg_shift, tcg_fpstatus); }
            else {
        ...
        case MO_16:
            tcg_single = tcg_temp_new_i32();
            if (is_signed) { gen_helper_vfp_sqtoh(tcg_single, tcg_int, tcg_shift, tcg_fpstatus); }
            else {
...
static bool do_cvtf_g(DisasContext* s, arg_fcvt* a, bool is_signed)
{
    ...
    if (a->sf) { tcg_int = cpu_reg(s, a->rn); }
    else {
        tcg_int = read_cpu_reg(s, a->rn, true);
        if (is_signed) { tcg_gen_ext32s_i64(tcg_int, tcg_int); }
        else {
...
static void do_fcvt_scalar(DisasContext* s, MemOp out, MemOp esz, TCGv_i64 tcg_out, int shift, int rn,
                           ARMFPRounding rmode)
{
    TCGv_ptr tcg_fpstatus;
    ...
    switch (esz) {
        case MO_64:
            ...
            switch (out) {
                case MO_64 | MO_SIGN: gen_helper_vfp_tosqd(tcg_out, tcg_out, tcg_shift, tcg_fpstatus); break;
                case MO_64          : gen_helper_vfp_touqd(tcg_out, tcg_out, tcg_shift, tcg_fpstatus); break;
        ...
        case MO_32:
            ...
            switch (out) {
                case MO_64 | MO_SIGN: gen_helper_vfp_tosqs(tcg_out, tcg_single, tcg_shift, tcg_fpstatus); break;
                case MO_64          : gen_helper_vfp_touqs(tcg_out, tcg_single, tcg_shift, tcg_fpstatus); break;
                case MO_32 | MO_SIGN:
                    gen_helper_vfp_tosls(tcg_single, tcg_single, tcg_shift, tcg_fpstatus);
                    tcg_gen_extu_i32_i64(tcg_out, tcg_single);
                    break;
                case MO_32:
                    gen_helper_vfp_touls(tcg_single, tcg_single, tcg_shift, tcg_fpstatus);
                    tcg_gen_extu_i32_i64(tcg_out, tcg_single);
                    break;
                default: tcg_debug_assert_not_reached();
            }
        ...
        case MO_16:
            ...
            switch (out) {
                case MO_64 | MO_SIGN: gen_helper_vfp_tosqh(tcg_out, tcg_single, tcg_shift, tcg_fpstatus); break;
                case MO_64          : gen_helper_vfp_touqh(tcg_out, tcg_single, tcg_shift, tcg_fpstatus); break;
                case MO_32 | MO_SIGN:
                    gen_helper_vfp_toslh(tcg_single, tcg_single, tcg_shift, tcg_fpstatus);
                    tcg_gen_extu_i32_i64(tcg_out, tcg_single);
                    break;
                case MO_32:
                    gen_helper_vfp_toulh(tcg_single, tcg_single, tcg_shift, tcg_fpstatus);
                    tcg_gen_extu_i32_i64(tcg_out, tcg_single);
                    break;
                case MO_16 | MO_SIGN:
                    gen_helper_vfp_toshh(tcg_single, tcg_single, tcg_shift, tcg_fpstatus);
                    tcg_gen_extu_i32_i64(tcg_out, tcg_single);
                    break;
                case MO_16:
                    gen_helper_vfp_touhh(tcg_single, tcg_single, tcg_shift, tcg_fpstatus);
                    tcg_gen_extu_i32_i64(tcg_out, tcg_single);
                    break;
                default: tcg_debug_assert_not_reached();
            }
...
static bool do_fcvt_g(DisasContext* s, arg_fcvt* a, ARMFPRounding rmode, bool is_signed)
{
    ...
    do_fcvt_scalar(s, (a->sf ? MO_64 : MO_32) | (is_signed ? MO_SIGN : 0), a->esz, tcg_int, a->shift, a->rn, rmode);

    if (!a->sf) { tcg_gen_ext32u_i64(tcg_int, tcg_int); }
    return true;
}
 ...
 * size are always equal.
 */
static bool do_fcvt_f(DisasContext* s, arg_fcvt* a, ARMFPRounding rmode, bool is_signed)
{
    TCGv_i64 tcg_int;
    int      check = fp_access_check_scalar_hsd(s, a->esz);
    ...
    tcg_int = tcg_temp_new_i64();
    do_fcvt_scalar(s, a->esz | (is_signed ? MO_SIGN : 0), a->esz, tcg_int, a->shift, a->rn, rmode);

    if (!s->fpcr_nep) { clear_vec(s, a->rd); }
...
{
    if (!dc_isar_feature(aa64_jscvt, s)) { return false; }
    if (fp_access_check(s)) {
        TCGv_i64 t        = read_fp_dreg(s, a->rn);
...
static const ENVScalar1 f_scalar_sqabs = {
    {gen_helper_neon_qabs_s8, gen_helper_neon_qabs_s16, gen_helper_neon_qabs_s32},
...
static const ENVScalar1 f_scalar_sqneg = {
    {gen_helper_neon_qneg_s8, gen_helper_neon_qneg_s16, gen_helper_neon_qneg_s32},
...
static bool do_2misc_narrow_scalar(DisasContext* s, arg_rr_e* a, ArithOneOp* const fn[3])
{
    if (a->esz == MO_64) { return false; }
    if (fp_access_check(s)) {
        TCGv_i64 t = tcg_temp_new_i64();
...
#define WRAP_ENV(NAME)                                                                   \
    static void gen_##NAME(TCGv_i64 d, TCGv_i64 n) { gen_helper_##NAME(d, tcg_env, n); }
...
{
    if (!a->q && a->esz == MO_64) { return false; }
    if (fp_access_check(s)) { gen_gvec_fn2(s, a->q, a->rd, a->rn, fn, a->esz); }
    return true;
}
...
static bool do_2misc_narrow_vector(DisasContext* s, arg_qrr_e* a, ArithOneOp* const fn[3])
{
    if (a->esz == MO_64) { return false; }
    if (fp_access_check(s)) {
        TCGv_i64 t0 = tcg_temp_new_i64();
...
TRANS_FEAT(BFCVTN_v, aa64_bf16, do_2misc_narrow_vector, a, f_vector_bfcvtn[s->fpcr_ah])
...
static bool trans_SHLL_v(DisasContext* s, arg_qrr_e* a)
{
    ...
    for (pass = 0; pass < 2; pass++) { write_vec_element(s, tcg_res[pass], a->rd, pass, MO_64); }
    return true;
}
...
static bool do_fp1_vector(DisasContext* s, arg_qrr_e* a, const FPScalar1* f, int rmode)
{
    TCGv_i32 tcg_rmode = NULL;
    TCGv_ptr fpst;
    int      check = fp_access_check_vector_hsd(s, a->q, a->esz);
    ...
    if (a->esz == MO_64) {
        ...
        TCGv_i32 t32                                  = tcg_temp_new_i32();
        void     (*gen)(TCGv_i32, TCGv_i32, TCGv_ptr) = (a->esz == MO_16 ? f->gen_h : f->gen_s);
...
TRANS_FEAT(FRINT32Z_v, aa64_frint, do_fp1_vector, a, &f_scalar_frint32, FPROUNDING_ZERO)
TRANS_FEAT(FRINT32X_v, aa64_frint, do_fp1_vector, a, &f_scalar_frint32, -1)
TRANS_FEAT(FRINT64Z_v, aa64_frint, do_fp1_vector, a, &f_scalar_frint64, FPROUNDING_ZERO)
TRANS_FEAT(FRINT64X_v, aa64_frint, do_fp1_vector, a, &f_scalar_frint64, -1)

static bool do_gvec_op2_fpst_with_fpsttype(DisasContext* s, MemOp esz, bool is_q, int rd, int rn, int data,
                                           gen_helper_gvec_2_ptr* const fns[3], ARMFPStatusFlavour fpsttype)
{
    ...
    fpst = fpstatus_ptr(fpsttype);
    tcg_gen_gvec_2_ptr(vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn), fpst, is_q ? 16 : 8,
                       vec_full_reg_size(s), data, fns[esz - 1]);
    return true;
}

static bool do_gvec_op2_fpst(DisasContext* s, MemOp esz, bool is_q, int rd, int rn, int data,
                             gen_helper_gvec_2_ptr* const fns[3])
{ return do_gvec_op2_fpst_with_fpsttype(s, esz, is_q, rd, rn, data, fns, esz == MO_16 ? FPST_A64_F16 : FPST_A64); }

static bool do_gvec_op2_ah_fpst(DisasContext* s, MemOp esz, bool is_q, int rd, int rn, int data,
                                gen_helper_gvec_2_ptr* const fns[3])
{ return do_gvec_op2_fpst_with_fpsttype(s, esz, is_q, rd, rn, data, fns, select_ah_fpst(s, esz)); }
...
TRANS(SCVTF_vi, do_gvec_op2_fpst, a->esz, a->q, a->rd, a->rn, 0, f_scvtf_v)
...
TRANS(UCVTF_vi, do_gvec_op2_fpst, a->esz, a->q, a->rd, a->rn, 0, f_ucvtf_v)
TRANS(UCVTF_vf, do_gvec_op2_fpst, a->esz, a->q, a->rd, a->rn, a->shift, f_ucvtf_v)
...
TRANS(FCVTZS_vf, do_gvec_op2_fpst, a->esz, a->q, a->rd, a->rn, a->shift, f_fcvtzs_vf)
...
TRANS(FCVTZU_vf, do_gvec_op2_fpst, a->esz, a->q, a->rd, a->rn, a->shift, f_fcvtzu_vf)
...
TRANS(FCVTNS_vi, do_gvec_op2_fpst, a->esz, a->q, a->rd, a->rn, float_round_nearest_even, f_fcvt_s_vi)
TRANS(FCVTNU_vi, do_gvec_op2_fpst, a->esz, a->q, a->rd, a->rn, float_round_nearest_even, f_fcvt_u_vi)
TRANS(FCVTPS_vi, do_gvec_op2_fpst, a->esz, a->q, a->rd, a->rn, float_round_up, f_fcvt_s_vi)
TRANS(FCVTPU_vi, do_gvec_op2_fpst, a->esz, a->q, a->rd, a->rn, float_round_up, f_fcvt_u_vi)
TRANS(FCVTMS_vi, do_gvec_op2_fpst, a->esz, a->q, a->rd, a->rn, float_round_down, f_fcvt_s_vi)
TRANS(FCVTMU_vi, do_gvec_op2_fpst, a->esz, a->q, a->rd, a->rn, float_round_down, f_fcvt_u_vi)
TRANS(FCVTZS_vi, do_gvec_op2_fpst, a->esz, a->q, a->rd, a->rn, float_round_to_zero, f_fcvt_s_vi)
TRANS(FCVTZU_vi, do_gvec_op2_fpst, a->esz, a->q, a->rd, a->rn, float_round_to_zero, f_fcvt_u_vi)
TRANS(FCVTAS_vi, do_gvec_op2_fpst, a->esz, a->q, a->rd, a->rn, float_round_ties_away, f_fcvt_s_vi)
TRANS(FCVTAU_vi, do_gvec_op2_fpst, a->esz, a->q, a->rd, a->rn, float_round_ties_away, f_fcvt_u_vi)
...
static bool trans_FCVTL_v(DisasContext* s, arg_qrr_e* a)
{
    ...
    if (a->esz == MO_64) {
        ...
        for (pass = 0; pass < 4; pass++) {
            ...
            gen_helper_vfp_fcvt_f16_to_f32(tcg_res[pass], tcg_res[pass], fpst, ahp);
        }
        for (pass = 0; pass < 4; pass++) { write_vec_element_i32(s, tcg_res[pass], a->rd, pass, MO_32); }
    }
    clear_vec_high(s, true, a->rd);
    return true;
}

static bool trans_OK(DisasContext* s, arg_OK* a) { return true; }
...
static void aarch64_tr_translate_insn(DisasContextBase* dcbase, CPUState* cpu)
{
    ...
    if (dc_isar_feature(aa64_bti, s)) {
        if (s->base.num_insns == 1) {
             ...
             */
            if (s->btype != 0 && !btype_destination_ok(insn, s->bt, s->btype)) {
     ...
     */
    if (s->btype > 0 && s->base.is_jmp != DISAS_NORETURN) { reset_btype(s); }
}
    ...
    if (unlikely(dc->ss_active)) {
        ...
        switch (dc->base.is_jmp) {
            ...
            case DISAS_UPDATE_NOCHAIN:
                gen_a64_update_pc(dc, 4);
                /* fall through */
            case DISAS_JUMP    : tcg_gen_lookup_and_goto_ptr(); break;
            case DISAS_NORETURN:
            case DISAS_SWI     : break;
            case DISAS_WFE:
                gen_a64_update_pc(dc, 4);
