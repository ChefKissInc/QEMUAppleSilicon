...
{
    if (den == 0) { return 0; }
    if (num == LLONG_MIN && den == -1) { return LLONG_MIN; }
    return num / den;
}

uint64_t HELPER(rbit64)(uint64_t x) { return revbit64(x); }

void HELPER(msr_i_spsel)(CPUARMState* env, uint32_t imm) { update_spsel(env, imm); }

void HELPER(msr_set_allint_el1)(CPUARMState* env)
{
    /* ALLINT update to PSTATE. */
    if (arm_hcrx_el2_eff(env) & HCRX_TALLINT) {
        raise_exception_ra(env, EXCP_UDEF, syn_aa64_sysregtrap(0, 1, 0, 4, 1, 0x1f, 0), 2, GETPC());
    }
...
static void daif_check(CPUARMState* env, uint32_t op, uint32_t imm, uintptr_t ra)
{
    ...
    if (arm_current_el(env) == 0 && !(arm_sctlr(env, 0) & SCTLR_UMA)) {
        raise_exception_ra(env, EXCP_UDEF,
                           syn_aa64_sysregtrap(0, extract32(op, 0, 3), extract32(op, 3, 3), 4, imm, 0x1f, 0),
...
static inline uint32_t float_rel_to_flags(int res)
{
    ...
    switch (res) {
        ...
        default                      : flags = PSTATE_C | PSTATE_V; break;
    }
    return flags;
}
...
uint64_t HELPER(neon_ceq_f64)(float64 a, float64 b, float_status* fpst) { return -float64_eq_quiet(a, b, fpst); }

uint64_t HELPER(neon_cge_f64)(float64 a, float64 b, float_status* fpst) { return -float64_le(b, a, fpst); }

uint64_t HELPER(neon_cgt_f64)(float64 a, float64 b, float_status* fpst) { return -float64_lt(b, a, fpst); }
...
#define DO_RSQRTSF(NAME, CTYPE, FLOATTYPE, CHSFN)                               \
    ...
    {                                                                           \
        ...
        return FLOATTYPE##_muladd_scalbn(a, b, FLOATTYPE##_three, -1, 0, fpst); \
    }
...
uint32_t HELPER(frecpx_f16)(uint32_t a, float_status* fpst)
{
    ...
    if (float16_is_any_nan(a)) {
        ...
        if (float16_is_signaling_nan(a, fpst)) {
            ...
            if (!fpst->default_nan_mode) { nan = float16_silence_nan(a, fpst); }
        }
    ...
    if (exp == 0) { return make_float16(deposit32(sbit, 10, 5, 0x1e)); }
    else {
...
float32 HELPER(frecpx_f32)(float32 a, float_status* fpst)
{
    ...
    if (float32_is_any_nan(a)) {
        ...
        if (float32_is_signaling_nan(a, fpst)) {
            float_raise(float_flag_invalid, fpst);
            if (!fpst->default_nan_mode) { nan = float32_silence_nan(a, fpst); }
        }
    ...
    if (exp == 0) { return make_float32(sbit | (0xfe << 23)); }
    else {
...
float64 HELPER(frecpx_f64)(float64 a, float_status* fpst)
{
    ...
    if (float64_is_any_nan(a)) {
        ...
        if (float64_is_signaling_nan(a, fpst)) {
            float_raise(float_flag_invalid, fpst);
            if (!fpst->default_nan_mode) { nan = float64_silence_nan(a, fpst); }
        }
    ...
    if (exp == 0) { return make_float64(sbit | (0x7feULL << 52)); }
    else {
...
#define AH_MINMAX_HELPER(NAME, CTYPE, FLOATTYPE, MINMAX)                    \
    ...
    {                                                                       \
        ...
        a = FLOATTYPE##_squash_input_denormal(a, fpst);                     \
        b = FLOATTYPE##_squash_input_denormal(b, fpst);                     \
        if (FLOATTYPE##_is_zero(a) && FLOATTYPE##_is_zero(b)) { return b; } \
        if (FLOATTYPE##_is_any_nan(a) || FLOATTYPE##_is_any_nan(b)) {       \
            float_raise(float_flag_invalid, fpst);                          \
            return b;                                                       \
        }                                                                   \
...
#define ADVSIMD_HALFOP(name) \
    uint32_t ADVSIMD_HELPER(name, h)(uint32_t a, uint32_t b, float_status* fpst) { return float16_##name(a, b, fpst); }

#define ADVSIMD_TWOHALFOP(name)                                                           \
    uint32_t ADVSIMD_HELPER(name, 2h)(uint32_t two_a, uint32_t two_b, float_status* fpst) \
    {                                                                                     \
        float16  a1, a2, b1, b2;                                                          \
        uint32_t r1, r2;                                                                  \
...
uint32_t HELPER(advsimd_muladdh)(uint32_t a, uint32_t b, uint32_t c, float_status* fpst)
{ return float16_muladd(a, b, c, 0, fpst); }

uint32_t HELPER(advsimd_muladd2h)(uint32_t two_a, uint32_t two_b, uint32_t two_c, float_status* fpst)
{
...
/* round to integral */
uint32_t HELPER(advsimd_rinth_exact)(uint32_t x, float_status* fp_status) { return float16_round_to_int(x, fp_status); }
...
static int el_from_spsr(uint32_t spsr)
{
    ...
    if (spsr & PSTATE_nRW) {
        switch (spsr & CPSR_M) {
            ...
            case ARM_CPU_MODE_SYS: return 1;
...
void HELPER(exception_return)(CPUARMState* env, uint64_t new_pc)
{
    ...
    if (arm_is_guarded(env)) { spsr = env->gxf.spsr_gl[cur_el]; }
    else {
     ...
     */
    if (arm_generate_debug_exceptions(env)) { spsr &= ~PSTATE_SS; }
     ...
     */
    if (cur_el == 3 && new_el < 3 && (env->cp15.scr_el3 & (SCR_NS | SCR_NSE)) == SCR_NSE) { goto illegal_return; }
    ...
    if (!return_to_aa64) {
        ...
        if (spsr & CPSR_T) { env->regs[15] = new_pc & ~0x1; }
        else {
        ...
        if ((tbii >> extract64(new_pc, 55, 1)) & 1) {
            ...
            if (regime_has_2_ranges(core_to_aa64_mmu_idx(core_mmu_idx))) { new_pc = sextract64(new_pc, 0, 56); }
            else {
...
void HELPER(dc_zva)(CPUARMState* env, uint64_t vaddr_in)
{
    ...
    if (unlikely(!mem)) {
        ...
        if (unlikely(!mem)) {
             ...
             */
            for (int i = 0; i < blocklen; i++) { cpu_stb_mmuidx_ra(env, vaddr + i, 0, mmu_idx, ra); }
            return;
        }
    }
...
void HELPER(unaligned_access)(CPUARMState* env, uint64_t addr, uint32_t access_type, uint32_t mmu_idx)
{ arm_cpu_do_unaligned_access(env_cpu(env), addr, access_type, mmu_idx, GETPC()); }
...
static bool mops_enabled(CPUARMState* env)
{
    ...
    if (el < 2 && (arm_hcr_el2_eff(env) & (HCR_E2H | HCR_TGE)) != (HCR_E2H | HCR_TGE)
    ...
    if (el == 0) {
        if (!el_is_in_host(env, 0)) { return env->cp15.sctlr_el[1] & SCTLR_MSCEN; }
        else {
...
{
    if (!mops_enabled(env)) { raise_exception_ra(env, EXCP_UDEF, syn_uncategorized(), exception_target_el(env), ra); }
}
...
static int mops_mismatch_exception_target_el(CPUARMState* env)
{
    ...
    if (el > 1) { return el; }
    if (el == 0 && (arm_hcr_el2_eff(env) & HCR_TGE)) { return 2; }
    if (el == 1 && (arm_hcrx_el2_eff(env) & HCRX_MCE2)) { return 2; }
    return 1;
}
 ...
 */
static void check_mops_wrong_option(CPUARMState* env, uint32_t syndrome, uintptr_t ra)
{
    if (env->CF != 0) {
        syndrome |= 1 << 17; /* Set the wrong-option bit */
        raise_exception_ra(env, EXCP_UDEF, syndrome, mops_mismatch_exception_target_el(env), ra);
    }
}
 ...
 */
static uint64_t page_limit(uint64_t addr) { return TARGET_PAGE_ALIGN(addr + 1) - addr; }

/*
 * Return the number of bytes we can copy starting from addr and working
 * backwards without crossing a page boundary.
 */
static uint64_t page_limit_rev(uint64_t addr) { return (addr & ~TARGET_PAGE_MASK) + 1; }
 ...
 */
static uint64_t set_step(CPUARMState* env, uint64_t toaddr, uint64_t setsize, uint32_t data, int memidx,
 ...
 * the tags.
 */
static uint64_t set_step_tags(CPUARMState* env, uint64_t toaddr, uint64_t setsize, uint32_t data, int memidx,
                              uint32_t* mtedesc, uintptr_t ra)
{
    void* mem;
...
typedef uint64_t StepFn(CPUARMState* env, uint64_t toaddr, uint64_t setsize, uint32_t data, int memidx,
                        uint32_t* mtedesc, uintptr_t ra);

/* Extract register numbers from a MOPS exception syndrome value */
static int mops_destreg(uint32_t syndrome) { return extract32(syndrome, 10, 5); }

static int mops_srcreg(uint32_t syndrome) { return extract32(syndrome, 5, 5); }

static int mops_sizereg(uint32_t syndrome) { return extract32(syndrome, 0, 5); }
...
static bool mte_checks_needed(uint64_t ptr, uint32_t desc)
{
     ...
     */
    if (!tbi_check(desc, bit55)) { return false; }
    return !tcma_check(desc, bit55, allocation_tag_from_addr(ptr));
}

/* Take an exception if the SETG addr/size are not granule aligned */
static void check_setg_alignment(CPUARMState* env, uint64_t ptr, uint64_t size, uint32_t memidx, uintptr_t ra)
{
    if ((size != 0 && !QEMU_IS_ALIGNED(ptr, TAG_GRANULE)) || !QEMU_IS_ALIGNED(size, TAG_GRANULE)) {
        arm_cpu_do_unaligned_access(env_cpu(env), ptr, MMU_DATA_STORE, memidx, ra);
    }
}
 ...
 */
static void do_setp(CPUARMState* env, uint32_t syndrome, uint32_t mtedesc, StepFn* stepfn, bool is_setg, uintptr_t ra)
{
    ...
    if (setsize > INT64_MAX) {
        setsize = INT64_MAX;
        if (is_setg) { setsize &= ~0xf; }
    }

    if (unlikely(is_setg)) { check_setg_alignment(env, toaddr, setsize, memidx, ra); }
...
static void do_setm(CPUARMState* env, uint32_t syndrome, uint32_t mtedesc, StepFn* stepfn, bool is_setg, uintptr_t ra)
{
    ...
    while (stagesetsize > 0) {
        ...
        stagesetsize   -= step;
        env->xregs[rn]  = -setsize;
        if (stagesetsize > 0 && unlikely(cpu_loop_exit_requested(cs))) { cpu_loop_exit_restore(cs, ra); }
    }
}
...
static void do_sete(CPUARMState* env, uint32_t syndrome, uint32_t mtedesc, StepFn* stepfn, bool is_setg, uintptr_t ra)
{
 ...
 * for bytes other than the first.
 */
static uint64_t copy_step(CPUARMState* env, uint64_t toaddr, uint64_t fromaddr, uint64_t copysize, int wmemidx,
                          int rmemidx, uint32_t* wdesc, uint32_t* rdesc, uintptr_t ra)
{
    void* rmem;
    void* wmem;
    ...
    if (unlikely(!rmem || !wmem)) {
        uint8_t byte;
        if (rmem) { byte = *(uint8_t*)rmem; }
        else {
            byte = cpu_ldub_mmuidx_ra(env, fromaddr, rmemidx, ra);
        }
        if (wmem) { *(uint8_t*)wmem = byte; }
        else {
 ...
 */
static uint64_t copy_step_rev(CPUARMState* env, uint64_t toaddr, uint64_t fromaddr, uint64_t copysize, int wmemidx,
                              int rmemidx, uint32_t* wdesc, uint32_t* rdesc, uintptr_t ra)
{
    void* rmem;
    void* wmem;
 ...
 */
static void do_cpyp(CPUARMState* env, uint32_t syndrome, uint32_t wdesc, uint32_t rdesc, uint32_t move, uintptr_t ra)
{
    int      rd       = mops_destreg(syndrome);
    int      rs       = mops_srcreg(syndrome);
    int      rn       = mops_sizereg(syndrome);
    ...
    if (move) {
        ...
        if (fs < ts && fe > ts) { forwards = false; }
    }
    else {
        if (copysize > INT64_MAX) { copysize = INT64_MAX; }
    }

    if (!mte_checks_needed(fromaddr, rdesc)) { rdesc = 0; }
    if (!mte_checks_needed(toaddr, wdesc)) { wdesc = 0; }
    ...
    if (forwards) {
        ...
        while (stagecopysize) {
            ...
            step            = copy_step(env, toaddr, fromaddr, stagecopysize, wmemidx, rmemidx, &wdesc, &rdesc, ra);
            toaddr         += step;
            fromaddr       += step;
            copysize       -= step;
            stagecopysize  -= step;
        ...
        while (stagecopysize) {
            env->xregs[rn]  = copysize;
            step            = copy_step_rev(env, toaddr, fromaddr, stagecopysize, wmemidx, rmemidx, &wdesc, &rdesc, ra);
            copysize       -= step;
            stagecopysize  -= step;
            toaddr         -= step;
            fromaddr       -= step;
...
void HELPER(cpyp)(CPUARMState* env, uint32_t syndrome, uint32_t wdesc, uint32_t rdesc)
{ do_cpyp(env, syndrome, wdesc, rdesc, true, GETPC()); }

void HELPER(cpyfp)(CPUARMState* env, uint32_t syndrome, uint32_t wdesc, uint32_t rdesc)
{ do_cpyp(env, syndrome, wdesc, rdesc, false, GETPC()); }

static void do_cpym(CPUARMState* env, uint32_t syndrome, uint32_t wdesc, uint32_t rdesc, uint32_t move, uintptr_t ra)
{
    ...
    if (move) { forwards = (int64_t)env->xregs[rn] < 0; }
    ...
    if (forwards) {
        while (copysize >= TARGET_PAGE_SIZE) {
            step            = copy_step(env, toaddr, fromaddr, copysize, wmemidx, rmemidx, &wdesc, &rdesc, ra);
            toaddr         += step;
            fromaddr       += step;
            copysize       -= step;
            env->xregs[rn]  = -copysize;
            if (copysize >= TARGET_PAGE_SIZE && unlikely(cpu_loop_exit_requested(cs))) {
        ...
        while (copysize >= TARGET_PAGE_SIZE) {
            step            = copy_step_rev(env, toaddr, fromaddr, copysize, wmemidx, rmemidx, &wdesc, &rdesc, ra);
            toaddr         -= step;
            fromaddr       -= step;
            copysize       -= step;
            env->xregs[rn]  = copysize;
...
void HELPER(cpym)(CPUARMState* env, uint32_t syndrome, uint32_t wdesc, uint32_t rdesc)
{ do_cpym(env, syndrome, wdesc, rdesc, true, GETPC()); }

void HELPER(cpyfm)(CPUARMState* env, uint32_t syndrome, uint32_t wdesc, uint32_t rdesc)
{ do_cpym(env, syndrome, wdesc, rdesc, false, GETPC()); }

static void do_cpye(CPUARMState* env, uint32_t syndrome, uint32_t wdesc, uint32_t rdesc, uint32_t move, uintptr_t ra)
{
    /* Epilogue: do the last partial page */
    int      rd       = mops_destreg(syndrome);
    int      rs       = mops_srcreg(syndrome);
...
void HELPER(cpye)(CPUARMState* env, uint32_t syndrome, uint32_t wdesc, uint32_t rdesc)
{ do_cpye(env, syndrome, wdesc, rdesc, true, GETPC()); }

void HELPER(cpyfe)(CPUARMState* env, uint32_t syndrome, uint32_t wdesc, uint32_t rdesc)
...
static bool is_guarded_page(CPUARMState* env, target_ulong addr, uintptr_t ra)
{
    ...
    int              flags   = probe_access_full(env, addr, 0, MMU_INST_FETCH, mmu_idx, false, &host, &full, ra);
...
void HELPER(guarded_page_check)(CPUARMState* env)
{
    ...
    if (is_guarded_page(env, env->pc, 0)) {
        raise_exception(env, EXCP_UDEF, syn_btitrap(env->btype), exception_target_el(env));
    }
}
...
uint64_t HELPER(wkdmc)(CPUARMState* env, uint64_t vaddr_in, uint64_t vaddr_out)
{
    ...
    if (TARGET_PAGE_BITS < 10 || TARGET_PAGE_BITS > 14) { return -1; }
    ...
    if (in_mem == NULL || out_mem == NULL) { return -1; }
...
uint64_t HELPER(wkdmd)(CPUARMState* env, uint64_t vaddr_in, uint64_t vaddr_out)
{
    ...
    if (TARGET_PAGE_BITS < 10 || TARGET_PAGE_BITS > 14) { return 0x3000; }
    ...
    if (in_mem == NULL || out_mem == NULL) { return 0x3000; }

    if (WKdm_decompress(in_mem + in_offset, out_mem, csize)) { return 0; }
