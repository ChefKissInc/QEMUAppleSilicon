...
static uint32_t cho(uint32_t x, uint32_t y, uint32_t z) { return (x & (y ^ z)) ^ z; }

static uint32_t par(uint32_t x, uint32_t y, uint32_t z) { return x ^ y ^ z; }

static uint32_t maj(uint32_t x, uint32_t y, uint32_t z) { return (x & y) | ((x | y) & z); }
...
static uint32_t do_sha1c(union CRYPTO_STATE* d) { return cho(CR_ST_WORD(*d, 1), CR_ST_WORD(*d, 2), CR_ST_WORD(*d, 3)); }

void HELPER(crypto_sha1c)(void* vd, void* vn, void* vm, uint32_t desc) { crypto_sha1_3reg(vd, vn, vm, desc, do_sha1c); }

static uint32_t do_sha1p(union CRYPTO_STATE* d) { return par(CR_ST_WORD(*d, 1), CR_ST_WORD(*d, 2), CR_ST_WORD(*d, 3)); }

void HELPER(crypto_sha1p)(void* vd, void* vn, void* vm, uint32_t desc) { crypto_sha1_3reg(vd, vn, vm, desc, do_sha1p); }

static uint32_t do_sha1m(union CRYPTO_STATE* d) { return maj(CR_ST_WORD(*d, 1), CR_ST_WORD(*d, 2), CR_ST_WORD(*d, 3)); }

void HELPER(crypto_sha1m)(void* vd, void* vn, void* vm, uint32_t desc) { crypto_sha1_3reg(vd, vn, vm, desc, do_sha1m); }
...
static uint32_t S0(uint32_t x) { return ror32(x, 2) ^ ror32(x, 13) ^ ror32(x, 22); }

static uint32_t S1(uint32_t x) { return ror32(x, 6) ^ ror32(x, 11) ^ ror32(x, 25); }

static uint32_t s0(uint32_t x) { return ror32(x, 7) ^ ror32(x, 18) ^ (x >> 3); }

static uint32_t s1(uint32_t x) { return ror32(x, 17) ^ ror32(x, 19) ^ (x >> 10); }
...
static uint64_t cho512(uint64_t x, uint64_t y, uint64_t z) { return (x & (y ^ z)) ^ z; }

static uint64_t maj512(uint64_t x, uint64_t y, uint64_t z) { return (x & y) | ((x | y) & z); }

static uint64_t S0_512(uint64_t x) { return ror64(x, 28) ^ ror64(x, 34) ^ ror64(x, 39); }

static uint64_t S1_512(uint64_t x) { return ror64(x, 14) ^ ror64(x, 18) ^ ror64(x, 41); }

static uint64_t s0_512(uint64_t x) { return ror64(x, 1) ^ ror64(x, 8) ^ (x >> 7); }

static uint64_t s1_512(uint64_t x) { return ror64(x, 19) ^ ror64(x, 61) ^ (x >> 6); }
...
static inline void QEMU_ALWAYS_INLINE crypto_sm3tt(uint64_t* rd, uint64_t* rn, uint64_t* rm, uint32_t desc,
                                                   uint32_t opcode)
{
    union CRYPTO_STATE d    = {.l = {rd[0], rd[1]}};
    union CRYPTO_STATE n    = {.l = {rn[0], rn[1]}};
...
#define DO_SM3TT(NAME, OPCODE)                                                                                 \
    void HELPER(NAME)(void* vd, void* vn, void* vm, uint32_t desc) { crypto_sm3tt(vd, vn, vm, desc, OPCODE); }
...
void HELPER(crypto_sm4e)(void* vd, void* vn, void* vm, uint32_t desc)
{
    ...
    for (i = 0; i < opr_sz; i += 16) { do_crypto_sm4e(vd + i, vn + i, vm + i); }
    clear_tail(vd, opr_sz, simd_maxsz(desc));
}
...
void HELPER(crypto_sm4ekey)(void* vd, void* vn, void* vm, uint32_t desc)
{
    ...
    for (i = 0; i < opr_sz; i += 16) { do_crypto_sm4ekey(vd + i, vn + i, vm + i); }
    clear_tail(vd, opr_sz, simd_maxsz(desc));
}
...
void HELPER(crypto_rax1)(void* vd, void* vn, void* vm, uint32_t desc)
{
    ...
    for (i = 0; i < opr_sz / 8; ++i) { d[i] = n[i] ^ rol64(m[i], 1); }
    clear_tail(vd, opr_sz, simd_maxsz(desc));
}
