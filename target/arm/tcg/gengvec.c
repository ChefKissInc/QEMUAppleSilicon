...
static void gen_gvec_fn3_qc(uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz, uint32_t max_sz,
...
void gen_gvec_sqdmulh_qc(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz,
                         uint32_t max_sz)
{
    static gen_helper_gvec_3_ptr* const fns[2] = {gen_helper_neon_sqdmulh_h, gen_helper_neon_sqdmulh_s};
...
void gen_gvec_sqrdmulh_qc(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz,
                          uint32_t max_sz)
{
    static gen_helper_gvec_3_ptr* const fns[2] = {gen_helper_neon_sqrdmulh_h, gen_helper_neon_sqrdmulh_s};
    tcg_debug_assert(vece >= 1 && vece <= 2);
    gen_gvec_fn3_qc(rd_ofs, rn_ofs, rm_ofs, opr_sz, max_sz, fns[vece - 1]);
}
...
void gen_gvec_sqrdmlah_qc(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz,
                          uint32_t max_sz)
{
    static gen_helper_gvec_3_ptr* const fns[2] = {gen_helper_gvec_qrdmlah_s16, gen_helper_gvec_qrdmlah_s32};
    tcg_debug_assert(vece >= 1 && vece <= 2);
    gen_gvec_fn3_qc(rd_ofs, rn_ofs, rm_ofs, opr_sz, max_sz, fns[vece - 1]);
}
...
void gen_gvec_sqrdmlsh_qc(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz,
                          uint32_t max_sz)
{
    static gen_helper_gvec_3_ptr* const fns[2] = {gen_helper_gvec_qrdmlsh_s16, gen_helper_gvec_qrdmlsh_s32};
    tcg_debug_assert(vece >= 1 && vece <= 2);
    gen_gvec_fn3_qc(rd_ofs, rn_ofs, rm_ofs, opr_sz, max_sz, fns[vece - 1]);
}

#define GEN_CMP0(NAME, COND)                                                           \
    void NAME(unsigned vece, uint32_t d, uint32_t m, uint32_t opr_sz, uint32_t max_sz) \
...
void gen_gvec_ushr(unsigned vece, uint32_t rd_ofs, uint32_t rm_ofs, int64_t shift, uint32_t opr_sz, uint32_t max_sz)
{
    ...
    if (shift >= (8 << vece)) { tcg_gen_gvec_dup_imm(vece, rd_ofs, opr_sz, max_sz, 0); }
    else {
...
{
    static const TCGOpcode vecop_list[] = {INDEX_op_sari_vec, INDEX_op_add_vec, 0};
...
{
    static const TCGOpcode vecop_list[] = {INDEX_op_shri_vec, INDEX_op_add_vec, 0};
    static const GVecGen2i ops[4]       = {
        {
            .fni8      = gen_usra8_i64,
            .fniv      = gen_usra_vec,
     ...
     */
    if (shift < (8 << vece)) { tcg_gen_gvec_2i(rd_ofs, rm_ofs, opr_sz, max_sz, shift, &ops[vece]); }
    else {
...
{
    static const TCGOpcode vecop_list[] = {INDEX_op_shri_vec, INDEX_op_sari_vec, INDEX_op_add_vec, 0};
    static const GVecGen2i ops[4]       = {
        {.fni8    = gen_srshr8_i64,
         .fniv    = gen_srshr_vec,
...
static void gen_ursra8_i64(TCGv_i64 d, TCGv_i64 a, int64_t sh)
{
    ...
    if (sh == 8) { tcg_gen_vec_shr8i_i64(t, a, 7); }
    else {
...
static void gen_ursra16_i64(TCGv_i64 d, TCGv_i64 a, int64_t sh)
{
    ...
    if (sh == 16) { tcg_gen_vec_shr16i_i64(t, a, 15); }
    else {
...
static void gen_ursra32_i32(TCGv_i32 d, TCGv_i32 a, int32_t sh)
{
    ...
    if (sh == 32) { tcg_gen_shri_i32(t, a, 31); }
    else {
...
static void gen_ursra64_i64(TCGv_i64 d, TCGv_i64 a, int64_t sh)
{
    ...
    if (sh == 64) { tcg_gen_shri_i64(t, a, 63); }
    else {
...
static void gen_ursra_vec(unsigned vece, TCGv_vec d, TCGv_vec a, int64_t sh)
{
    ...
    if (sh == (8 << vece)) { tcg_gen_shri_vec(vece, t, a, sh - 1); }
    else {
...
void gen_gvec_sli(unsigned vece, uint32_t rd_ofs, uint32_t rm_ofs, int64_t shift, uint32_t opr_sz, uint32_t max_sz)
{
    ...
    if (shift == 0) { tcg_gen_gvec_mov(vece, rd_ofs, rm_ofs, opr_sz, max_sz); }
    else {
        tcg_gen_gvec_2i(rd_ofs, rm_ofs, opr_sz, max_sz, shift, &ops[vece]);
    }
}
...
{
    static const TCGOpcode vecop_list[] = {INDEX_op_mul_vec, INDEX_op_add_vec, 0};
    static const GVecGen3  ops[4]       = {
        {.fni4 = gen_mla8_i32, .fniv = gen_mla_vec, .load_dest = true, .opt_opc = vecop_list, .vece = MO_8},
        {.fni4 = gen_mla16_i32, .fniv = gen_mla_vec, .load_dest = true, .opt_opc = vecop_list, .vece = MO_16},
        {.fni4 = gen_mla32_i32, .fniv = gen_mla_vec, .load_dest = true, .opt_opc = vecop_list, .vece = MO_32},
        {.fni8       = gen_mla64_i64,
         .fniv       = gen_mla_vec,
         .prefer_i64 = TCG_TARGET_REG_BITS == 64,
         .load_dest  = true,
...
{
    static const TCGOpcode vecop_list[] = {INDEX_op_mul_vec, INDEX_op_sub_vec, 0};
    static const GVecGen3  ops[4]       = {
        {.fni4 = gen_mls8_i32, .fniv = gen_mls_vec, .load_dest = true, .opt_opc = vecop_list, .vece = MO_8},
        {.fni4 = gen_mls16_i32, .fniv = gen_mls_vec, .load_dest = true, .opt_opc = vecop_list, .vece = MO_16},
        {.fni4 = gen_mls32_i32, .fniv = gen_mls_vec, .load_dest = true, .opt_opc = vecop_list, .vece = MO_32},
        {.fni8       = gen_mls64_i64,
         .fniv       = gen_mls_vec,
         .prefer_i64 = TCG_TARGET_REG_BITS == 64,
         .load_dest  = true,
...
static void gen_cmtst_i32(TCGv_i32 d, TCGv_i32 a, TCGv_i32 b) { tcg_gen_negsetcond_i32(TCG_COND_TSTNE, d, a, b); }

void gen_cmtst_i64(TCGv_i64 d, TCGv_i64 a, TCGv_i64 b) { tcg_gen_negsetcond_i64(TCG_COND_TSTNE, d, a, b); }
...
void gen_gvec_cmtst(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz, uint32_t max_sz)
{
    ...
    static const GVecGen3  ops[4]       = {
        {.fni4 = gen_helper_neon_tst_u8, .fniv = gen_cmtst_vec, .opt_opc = vecop_list, .vece = MO_8},
        {.fni4 = gen_helper_neon_tst_u16, .fniv = gen_cmtst_vec, .opt_opc = vecop_list, .vece = MO_16},
        {.fni4 = gen_cmtst_i32, .fniv = gen_cmtst_vec, .opt_opc = vecop_list, .vece = MO_32},
        {.fni8       = gen_cmtst_i64,
         .fniv       = gen_cmtst_vec,
         .prefer_i64 = TCG_TARGET_REG_BITS == 64,
         .opt_opc    = vecop_list,
...
static void gen_ushl_vec(unsigned vece, TCGv_vec dst, TCGv_vec src, TCGv_vec shift)
{
    ...
    if (vece == MO_8) { tcg_gen_mov_vec(lsh, shift); }
    else {
...
{
    static const TCGOpcode vecop_list[] = {INDEX_op_neg_vec, INDEX_op_shlv_vec, INDEX_op_shrv_vec, INDEX_op_cmpsel_vec,
                                           0};
    static const GVecGen3  ops[4]       = {
        {.fniv = gen_ushl_vec, .fno = gen_helper_gvec_ushl_b, .opt_opc = vecop_list, .vece = MO_8},
        {.fniv = gen_ushl_vec, .fno = gen_helper_gvec_ushl_h, .opt_opc = vecop_list, .vece = MO_16},
        {.fni4 = gen_ushl_i32, .fniv = gen_ushl_vec, .opt_opc = vecop_list, .vece = MO_32},
        {.fni8 = gen_ushl_i64, .fniv = gen_ushl_vec, .opt_opc = vecop_list, .vece = MO_64},
    };
    tcg_gen_gvec_3(rd_ofs, rn_ofs, rm_ofs, opr_sz, max_sz, &ops[vece]);
}
...
static void gen_sshl_vec(unsigned vece, TCGv_vec dst, TCGv_vec src, TCGv_vec shift)
{
    TCGv_vec lval = tcg_temp_new_vec_matching(dst);
    TCGv_vec rval = tcg_temp_new_vec_matching(dst);
    TCGv_vec lsh  = tcg_temp_new_vec_matching(dst);
    ...
    if (vece == MO_8) { tcg_gen_cmpsel_vec(TCG_COND_LT, vece, dst, lsh, zero, rval, lval); }
    else {
...
{
    static const TCGOpcode vecop_list[] = {INDEX_op_neg_vec,  INDEX_op_umin_vec,   INDEX_op_shlv_vec,
                                           INDEX_op_sarv_vec, INDEX_op_cmpsel_vec, 0};
    static const GVecGen3  ops[4]       = {
        {.fniv = gen_sshl_vec, .fno = gen_helper_gvec_sshl_b, .opt_opc = vecop_list, .vece = MO_8},
        {.fniv = gen_sshl_vec, .fno = gen_helper_gvec_sshl_h, .opt_opc = vecop_list, .vece = MO_16},
        {.fni4 = gen_sshl_i32, .fniv = gen_sshl_vec, .opt_opc = vecop_list, .vece = MO_32},
        {.fni8 = gen_sshl_i64, .fniv = gen_sshl_vec, .opt_opc = vecop_list, .vece = MO_64},
    };
    tcg_gen_gvec_3(rd_ofs, rn_ofs, rm_ofs, opr_sz, max_sz, &ops[vece]);
}
...
void gen_neon_sqrshl(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz, uint32_t max_sz)
{
    static gen_helper_gvec_3_ptr* const fns[] = {
        gen_helper_neon_sqrshl_b,
...
void gen_neon_uqrshl(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz, uint32_t max_sz)
{
    static gen_helper_gvec_3_ptr* const fns[] = {
        gen_helper_neon_uqrshl_b,
        gen_helper_neon_uqrshl_h,
        gen_helper_neon_uqrshl_s,
...
void gen_neon_sqshli(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, int64_t c, uint32_t opr_sz, uint32_t max_sz)
{
    static gen_helper_gvec_2_ptr* const fns[] = {
        gen_helper_neon_sqshli_b,
        gen_helper_neon_sqshli_h,
        gen_helper_neon_sqshli_s,
...
{
    static gen_helper_gvec_2_ptr* const fns[] = {
        gen_helper_neon_uqshli_b,
        gen_helper_neon_uqshli_h,
        gen_helper_neon_uqshli_s,
...
{
    static gen_helper_gvec_2_ptr* const fns[] = {
        gen_helper_neon_sqshlui_b,
        gen_helper_neon_sqshlui_h,
        gen_helper_neon_sqshlui_s,
...
void gen_uqadd_d(TCGv_i64 res, TCGv_i64 qc, TCGv_i64 a, TCGv_i64 b)
{
    ...
    tcg_gen_movcond_i64(TCG_COND_LTU, res, t, a, tcg_constant_i64(UINT64_MAX), t);
    tcg_gen_xor_i64(t, t, res);
    tcg_gen_or_i64(qc, qc, t);
}

static void gen_uqadd_vec(unsigned vece, TCGv_vec t, TCGv_vec qc, TCGv_vec a, TCGv_vec b)
{
...
void gen_gvec_uqadd_qc(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz,
                       uint32_t max_sz)
{
    static const TCGOpcode vecop_list[] = {INDEX_op_usadd_vec, INDEX_op_add_vec, 0};
    ...
    tcg_debug_assert(opr_sz <= sizeof_field(CPUARMState, vfp.qc));
    tcg_gen_gvec_4(rd_ofs, offsetof(CPUARMState, vfp.qc), rn_ofs, rm_ofs, opr_sz, max_sz, &ops[vece]);
}
...
static void gen_sqadd_vec(unsigned vece, TCGv_vec t, TCGv_vec qc, TCGv_vec a, TCGv_vec b)
{
    TCGv_vec x = tcg_temp_new_vec_matching(t);
    tcg_gen_add_vec(vece, x, a, b);
...
void gen_gvec_sqadd_qc(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz,
                       uint32_t max_sz)
{
    static const TCGOpcode vecop_list[] = {INDEX_op_ssadd_vec, INDEX_op_add_vec, 0};
    static const GVecGen4  ops[4]       = {
        {.fniv       = gen_sqadd_vec,
...
static void gen_uqsub_vec(unsigned vece, TCGv_vec t, TCGv_vec qc, TCGv_vec a, TCGv_vec b)
{
    TCGv_vec x = tcg_temp_new_vec_matching(t);
...
void gen_gvec_uqsub_qc(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz,
                       uint32_t max_sz)
{
    static const TCGOpcode vecop_list[] = {INDEX_op_ussub_vec, INDEX_op_sub_vec, 0};
    static const GVecGen4  ops[4]       = {
        {.fniv       = gen_uqsub_vec,
...
static void gen_sqsub_vec(unsigned vece, TCGv_vec t, TCGv_vec qc, TCGv_vec a, TCGv_vec b)
{
    TCGv_vec x = tcg_temp_new_vec_matching(t);
    tcg_gen_sub_vec(vece, x, a, b);
...
void gen_gvec_sqsub_qc(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz,
                       uint32_t max_sz)
{
    static const TCGOpcode vecop_list[] = {INDEX_op_sssub_vec, INDEX_op_sub_vec, 0};
    static const GVecGen4  ops[4]       = {
        {.fniv       = gen_sqsub_vec,
...
{
    static const TCGOpcode vecop_list[] = {INDEX_op_sub_vec, INDEX_op_smin_vec, INDEX_op_smax_vec, 0};
    static const GVecGen3  ops[4]       = {
        {.fniv = gen_sabd_vec, .fno = gen_helper_gvec_sabd_b, .opt_opc = vecop_list, .vece = MO_8},
        {.fniv = gen_sabd_vec, .fno = gen_helper_gvec_sabd_h, .opt_opc = vecop_list, .vece = MO_16},
        {.fni4    = gen_sabd_i32,
         .fniv    = gen_sabd_vec,
...
{
    static const TCGOpcode vecop_list[] = {INDEX_op_sub_vec, INDEX_op_umin_vec, INDEX_op_umax_vec, 0};
    static const GVecGen3  ops[4]       = {
        {.fniv = gen_uabd_vec, .fno = gen_helper_gvec_uabd_b, .opt_opc = vecop_list, .vece = MO_8},
        {.fniv = gen_uabd_vec, .fno = gen_helper_gvec_uabd_h, .opt_opc = vecop_list, .vece = MO_16},
        {.fni4    = gen_uabd_i32,
         .fniv    = gen_uabd_vec,
...
{
    static const TCGOpcode vecop_list[] = {INDEX_op_sub_vec, INDEX_op_add_vec, INDEX_op_smin_vec, INDEX_op_smax_vec, 0};
    static const GVecGen3  ops[4]       = {
        {.fniv = gen_saba_vec, .fno = gen_helper_gvec_saba_b, .opt_opc = vecop_list, .load_dest = true, .vece = MO_8},
        {.fniv = gen_saba_vec, .fno = gen_helper_gvec_saba_h, .opt_opc = vecop_list, .load_dest = true, .vece = MO_16},
        {.fni4      = gen_saba_i32,
         .fniv      = gen_saba_vec,
...
{
    static const TCGOpcode vecop_list[] = {INDEX_op_sub_vec, INDEX_op_add_vec, INDEX_op_umin_vec, INDEX_op_umax_vec, 0};
    static const GVecGen3  ops[4]       = {
        {.fniv = gen_uaba_vec, .fno = gen_helper_gvec_uaba_b, .opt_opc = vecop_list, .load_dest = true, .vece = MO_8},
        {.fniv = gen_uaba_vec, .fno = gen_helper_gvec_uaba_h, .opt_opc = vecop_list, .load_dest = true, .vece = MO_16},
        {.fni4      = gen_uaba_i32,
         .fniv      = gen_uaba_vec,
...
void gen_gvec_shadd(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz, uint32_t max_sz)
{
    ...
    static const GVecGen3  g[]          = {
        {.fni8 = gen_shadd8_i64, .fniv = gen_shadd_vec, .opt_opc = vecop_list, .vece = MO_8},
        {.fni8 = gen_shadd16_i64, .fniv = gen_shadd_vec, .opt_opc = vecop_list, .vece = MO_16},
        {.fni4 = gen_shadd_i32, .fniv = gen_shadd_vec, .opt_opc = vecop_list, .vece = MO_32},
    };
    tcg_debug_assert(vece <= MO_32);
...
void gen_gvec_uhadd(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz, uint32_t max_sz)
{
    static const TCGOpcode vecop_list[] = {INDEX_op_shri_vec, INDEX_op_add_vec, 0};
    static const GVecGen3  g[]          = {
        {.fni8 = gen_uhadd8_i64, .fniv = gen_uhadd_vec, .opt_opc = vecop_list, .vece = MO_8},
        {.fni8 = gen_uhadd16_i64, .fniv = gen_uhadd_vec, .opt_opc = vecop_list, .vece = MO_16},
        {.fni4 = gen_uhadd_i32, .fniv = gen_uhadd_vec, .opt_opc = vecop_list, .vece = MO_32},
    };
    tcg_debug_assert(vece <= MO_32);
    tcg_gen_gvec_3(rd_ofs, rn_ofs, rm_ofs, opr_sz, max_sz, &g[vece]);
}
...
{
    static const TCGOpcode vecop_list[] = {INDEX_op_sari_vec, INDEX_op_sub_vec, 0};
    static const GVecGen3  g[4]         = {
        {.fni8 = gen_shsub8_i64, .fniv = gen_shsub_vec, .opt_opc = vecop_list, .vece = MO_8},
        {.fni8 = gen_shsub16_i64, .fniv = gen_shsub_vec, .opt_opc = vecop_list, .vece = MO_16},
        {.fni4 = gen_shsub_i32, .fniv = gen_shsub_vec, .opt_opc = vecop_list, .vece = MO_32},
    };
    assert(vece <= MO_32);
    tcg_gen_gvec_3(rd_ofs, rn_ofs, rm_ofs, opr_sz, max_sz, &g[vece]);
}
...
{
    static const TCGOpcode vecop_list[] = {INDEX_op_shri_vec, INDEX_op_sub_vec, 0};
    static const GVecGen3  g[4]         = {
        {.fni8 = gen_uhsub8_i64, .fniv = gen_uhsub_vec, .opt_opc = vecop_list, .vece = MO_8},
        {.fni8 = gen_uhsub16_i64, .fniv = gen_uhsub_vec, .opt_opc = vecop_list, .vece = MO_16},
        {.fni4 = gen_uhsub_i32, .fniv = gen_uhsub_vec, .opt_opc = vecop_list, .vece = MO_32},
    };
    assert(vece <= MO_32);
    tcg_gen_gvec_3(rd_ofs, rn_ofs, rm_ofs, opr_sz, max_sz, &g[vece]);
}
...
void gen_gvec_srhadd(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz, uint32_t max_sz)
{
    static const TCGOpcode vecop_list[] = {INDEX_op_sari_vec, INDEX_op_add_vec, 0};
    static const GVecGen3  g[]          = {
        {.fni8 = gen_srhadd8_i64, .fniv = gen_srhadd_vec, .opt_opc = vecop_list, .vece = MO_8},
        {.fni8 = gen_srhadd16_i64, .fniv = gen_srhadd_vec, .opt_opc = vecop_list, .vece = MO_16},
        {.fni4 = gen_srhadd_i32, .fniv = gen_srhadd_vec, .opt_opc = vecop_list, .vece = MO_32},
    };
    assert(vece <= MO_32);
    tcg_gen_gvec_3(rd_ofs, rn_ofs, rm_ofs, opr_sz, max_sz, &g[vece]);
}
...
void gen_gvec_urhadd(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz, uint32_t max_sz)
{
    static const TCGOpcode vecop_list[] = {INDEX_op_shri_vec, INDEX_op_add_vec, 0};
    static const GVecGen3  g[]          = {
        {.fni8 = gen_urhadd8_i64, .fniv = gen_urhadd_vec, .opt_opc = vecop_list, .vece = MO_8},
        {.fni8 = gen_urhadd16_i64, .fniv = gen_urhadd_vec, .opt_opc = vecop_list, .vece = MO_16},
        {.fni4 = gen_urhadd_i32, .fniv = gen_urhadd_vec, .opt_opc = vecop_list, .vece = MO_32},
    };
    assert(vece <= MO_32);
    tcg_gen_gvec_3(rd_ofs, rn_ofs, rm_ofs, opr_sz, max_sz, &g[vece]);
}
...
void gen_gvec_cls(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t opr_sz, uint32_t max_sz)
{
    static const GVecGen2 g[] = {
        {.fni4 = gen_helper_neon_cls_s8, .vece = MO_8},
        {.fni4 = gen_helper_neon_cls_s16, .vece = MO_16},
        {.fni4 = tcg_gen_clrsb_i32, .vece = MO_32},
    };
    assert(vece <= MO_32);
    tcg_gen_gvec_2(rd_ofs, rn_ofs, opr_sz, max_sz, &g[vece]);
}

static void gen_clz32_i32(TCGv_i32 d, TCGv_i32 n) { tcg_gen_clzi_i32(d, n, 32); }

void gen_gvec_clz(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t opr_sz, uint32_t max_sz)
{
    static const GVecGen2 g[] = {
        {.fni4 = gen_helper_neon_clz_u8, .vece = MO_8},
        {.fni4 = gen_helper_neon_clz_u16, .vece = MO_16},
        {.fni4 = gen_clz32_i32, .vece = MO_32},
    };
    assert(vece <= MO_32);
    tcg_gen_gvec_2(rd_ofs, rn_ofs, opr_sz, max_sz, &g[vece]);
}
...
{
    assert(vece == MO_8);
    tcg_gen_gvec_2_ool(rd_ofs, rn_ofs, opr_sz, max_sz, 0, gen_helper_gvec_cnt_b);
}

void gen_gvec_rbit(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t opr_sz, uint32_t max_sz)
{
    assert(vece == MO_8);
    tcg_gen_gvec_2_ool(rd_ofs, rn_ofs, opr_sz, max_sz, 0, gen_helper_gvec_rbit_b);
}
...
{
    static const GVecGen2 g = {.fni8       = gen_bswap32_i64,
                               .fni4       = tcg_gen_bswap32_i32,
                               .prefer_i64 = TCG_TARGET_REG_BITS == 64,
                               .vece       = MO_32};

    switch (vece) {
        case MO_16: tcg_gen_gvec_rotli(MO_32, rd_ofs, rn_ofs, 16, opr_sz, max_sz); break;
        case MO_8 : tcg_gen_gvec_2(rd_ofs, rn_ofs, opr_sz, max_sz, &g); break;
...
{
    static const GVecGen2 g[] = {
        {.fni8 = tcg_gen_bswap64_i64, .vece = MO_64},
        {.fni8 = tcg_gen_hswap_i64, .vece = MO_64},
    };

    switch (vece) {
        case MO_32: tcg_gen_gvec_rotli(MO_64, rd_ofs, rn_ofs, 32, opr_sz, max_sz); break;
        case MO_8 :
        case MO_16: tcg_gen_gvec_2(rd_ofs, rn_ofs, opr_sz, max_sz, &g[vece]); break;
        default   : assert_not_reached();
    }
}
...
{
    static const TCGOpcode vecop_list[] = {INDEX_op_sari_vec, INDEX_op_shli_vec, INDEX_op_add_vec, 0};
    static const GVecGen2  g[]          = {
        {.fniv = gen_saddlp_vec, .fni8 = gen_helper_neon_addlp_s8, .opt_opc = vecop_list, .vece = MO_16},
        {.fniv = gen_saddlp_vec, .fni8 = gen_helper_neon_addlp_s16, .opt_opc = vecop_list, .vece = MO_32},
        {.fniv = gen_saddlp_vec, .fni8 = gen_saddlp_s_i64, .opt_opc = vecop_list, .vece = MO_64},
    };
    assert(vece <= MO_32);
    tcg_gen_gvec_2(rd_ofs, rn_ofs, opr_sz, max_sz, &g[vece]);
}
...
void gen_gvec_sadalp(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t opr_sz, uint32_t max_sz)
{
    static const TCGOpcode vecop_list[] = {INDEX_op_sari_vec, INDEX_op_shli_vec, INDEX_op_add_vec, 0};
    static const GVecGen2  g[]          = {
        {.fniv = gen_sadalp_vec, .fni8 = gen_sadalp_b_i64, .opt_opc = vecop_list, .load_dest = true, .vece = MO_16},
        {.fniv = gen_sadalp_vec, .fni8 = gen_sadalp_h_i64, .opt_opc = vecop_list, .load_dest = true, .vece = MO_32},
        {.fniv = gen_sadalp_vec, .fni8 = gen_sadalp_s_i64, .opt_opc = vecop_list, .load_dest = true, .vece = MO_64},
    };
    assert(vece <= MO_32);
    tcg_gen_gvec_2(rd_ofs, rn_ofs, opr_sz, max_sz, &g[vece]);
}
...
void gen_gvec_uaddlp(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t opr_sz, uint32_t max_sz)
{
    static const TCGOpcode vecop_list[] = {INDEX_op_shri_vec, INDEX_op_add_vec, 0};
    static const GVecGen2  g[]          = {
        {.fniv = gen_uaddlp_vec, .fni8 = gen_uaddlp_b_i64, .opt_opc = vecop_list, .vece = MO_16},
        {.fniv = gen_uaddlp_vec, .fni8 = gen_uaddlp_h_i64, .opt_opc = vecop_list, .vece = MO_32},
        {.fniv = gen_uaddlp_vec, .fni8 = gen_uaddlp_s_i64, .opt_opc = vecop_list, .vece = MO_64},
    };
    assert(vece <= MO_32);
    tcg_gen_gvec_2(rd_ofs, rn_ofs, opr_sz, max_sz, &g[vece]);
}
...
void gen_gvec_uadalp(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t opr_sz, uint32_t max_sz)
{
    static const TCGOpcode vecop_list[] = {INDEX_op_shri_vec, INDEX_op_add_vec, 0};
    static const GVecGen2  g[]          = {
        {.fniv = gen_uadalp_vec, .fni8 = gen_uadalp_b_i64, .load_dest = true, .opt_opc = vecop_list, .vece = MO_16},
        {.fniv = gen_uadalp_vec, .fni8 = gen_uadalp_h_i64, .load_dest = true, .opt_opc = vecop_list, .vece = MO_32},
        {.fniv = gen_uadalp_vec, .fni8 = gen_uadalp_s_i64, .load_dest = true, .opt_opc = vecop_list, .vece = MO_64},
    };
    assert(vece <= MO_32);
    tcg_gen_gvec_2(rd_ofs, rn_ofs, opr_sz, max_sz, &g[vece]);
}
...
{
    assert(vece == MO_32);
    tcg_gen_gvec_2_ool(rd_ofs, rn_ofs, opr_sz, max_sz, 0, gen_helper_gvec_urecpe_s);
}

void gen_gvec_ursqrte(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t opr_sz, uint32_t max_sz)
{
    assert(vece == MO_32);
    tcg_gen_gvec_2_ool(rd_ofs, rn_ofs, opr_sz, max_sz, 0, gen_helper_gvec_ursqrte_s);
}
