...
#define DEF(NAME, OP0, OP1, CRN, CRM, OP2) [NAME##_IDX] = SYS_##NAME,
    ...
    if (vq_map != 0) {
        ...
        if (cpu->sve_max_vq && max_vq > cpu->sve_max_vq) {
            ...
            error_append_hint(errp,
                              ...
                              max_vq * 128, cpu->sve_max_vq, cpu->sve_max_vq * 128);
            return;
        }
 ...
 */
static void cpu_arm_get_vq(Object* obj, Visitor* v, const char* name, void* opaque, Error** errp)
{
    ...
    if (sve ? !cpu_isar_feature(aa64_sve, cpu) : !cpu_isar_feature(aa64_sme, cpu)) { value = false; }
    else {
...
static void cpu_arm_set_vq(Object* obj, Visitor* v, const char* name, void* opaque, Error** errp)
{
    ARMVQMap* vq_map = opaque;
    uint32_t  vq     = atoi(&name[3]) / 128;
    bool      value;
...
{
    ARMCPU* cpu = ARM_CPU(obj);
    return cpu_isar_feature(aa64_sme, cpu) && cpu_isar_feature(aa64_sme_fa64, cpu);
}
...
void aarch64_add_sve_properties(Object* obj)
{
    ...
    for (vq = 1; vq <= ARM_MAX_VQ; ++vq) {
        ...
        object_property_add(obj, name, "bool", cpu_arm_get_vq, cpu_arm_set_vq, NULL, &cpu->sve_vq);
    }
}
...
void aarch64_add_sme_properties(Object* obj)
{
    ...
    object_property_add_bool(obj, "sme_fa64", cpu_arm_get_sme_fa64, cpu_arm_set_sme_fa64);

    for (vq = 1; vq <= ARM_MAX_VQ; vq <<= 1) {
        char name[8];
        snprintf(name, sizeof(name), "sme%d", vq * 128);
        object_property_add(obj, name, "bool", cpu_arm_get_vq, cpu_arm_set_vq, NULL, &cpu->sme_vq);
    }
}
...
void arm_cpu_pauth_finalize(ARMCPU* cpu, Error** errp)
{
    ...
    if (kvm_enabled() || hvf_enabled()) {
        ...
        if (cpu->prop_pauth) {
            if (features == 0) {
                error_setg(errp,
                           "'pauth' feature not supported by "
                           "%s on this host",
                           current_accel_name());
            }
            return;
        ...
        if (cpu->prop_pauth) {
            if ((cpu->prop_pauth_impdef && cpu->prop_pauth_qarma3) || (cpu->prop_pauth_impdef && cpu->prop_pauth_qarma5)
                || (cpu->prop_pauth_qarma3 && cpu->prop_pauth_qarma5))
            {
                error_setg(errp, "cannot enable pauth-impdef, pauth-qarma3 and "
        ...
        else if (cpu->prop_pauth_impdef || cpu->prop_pauth_qarma3 || cpu->prop_pauth_qarma5) {
...
static const Property arm_cpu_pauth_property = DEFINE_PROP_BOOL("pauth", ARMCPU, prop_pauth, true);
...
static void aarch64_max_initfn(Object* obj)
{
    ...
    if (tcg_enabled()) { aarch64_a57_initfn(obj); }

    /* '-cpu max' for TCG: we currently do this as "A57 with extra things" */
    if (tcg_enabled()) { aarch64_max_tcg_initfn(obj); }
}
...
static void aarch64_apple_gxf_initfn(Object* obj)
{
    ...
    SET_IDREG(&cpu->isar, ID_AA64ISAR1,
              REG_FIELD_DP64(GET_IDREG(&cpu->isar, ID_AA64ISAR1), ID_AA64ISAR1, APA, PauthFeat_2));
...
static void aarch64_cpu_register_types(void)
{
    ...
    for (i = 0; i < ARRAY_SIZE(aarch64_cpus); ++i) { arm_cpu_register(&aarch64_cpus[i]); }
}
