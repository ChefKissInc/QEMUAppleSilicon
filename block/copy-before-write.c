...
static int coroutine_fn GRAPH_RDLOCK cbw_co_preadv(BlockDriverState* bs, int64_t offset, int64_t bytes,
 ...
 */
static coroutine_fn int cbw_do_copy_before_write(BlockDriverState* bs, uint64_t offset, uint64_t bytes,
                                                 BdrvRequestFlags flags)
{
    ...
    if (flags & BDRV_REQ_WRITE_UNCHANGED) { return 0; }

    if (s->snapshot_error) { return 0; }
    ...
    ret = block_copy(s->bcs, off, end - off, s->cbw_timeout_ns, block_copy_cb, bs);
    if (ret < 0 && s->on_cbw_error == ON_CBW_ERROR_BREAK_GUEST_WRITE) { return ret; }

    WITH_QEMU_LOCK_GUARD(&s->lock)
    {
        if (ret < 0) {
            assert(s->on_cbw_error == ON_CBW_ERROR_BREAK_SNAPSHOT);
            if (!s->snapshot_error) { s->snapshot_error = ret; }
        }
        else {
...
static int coroutine_fn GRAPH_RDLOCK cbw_co_pdiscard(BlockDriverState* bs, int64_t offset, int64_t bytes)
{
...
static int coroutine_fn GRAPH_RDLOCK cbw_co_pwrite_zeroes(BlockDriverState* bs, int64_t offset, int64_t bytes,
                                                          BdrvRequestFlags flags)
{
...
static coroutine_fn GRAPH_RDLOCK int cbw_co_pwritev(BlockDriverState* bs, int64_t offset, int64_t bytes,
                                                    QEMUIOVector* qiov, BdrvRequestFlags flags)
{
    int ret = cbw_do_copy_before_write(bs, offset, bytes, flags);
    if (ret < 0) { return ret; }
 ...
 */
static BlockReq* coroutine_fn GRAPH_RDLOCK cbw_snapshot_read_lock(BlockDriverState* bs, int64_t offset, int64_t bytes,
...
static int coroutine_fn GRAPH_RDLOCK cbw_co_preadv_snapshot(BlockDriverState* bs, int64_t offset, int64_t bytes,
                                                            QEMUIOVector* qiov, size_t qiov_offset)
{
    ...
    while (bytes) {
        ...
        if (!req) { return -EACCES; }

        ret = bdrv_co_preadv_part(file, offset, cur_bytes, qiov, qiov_offset, 0);
...
static int coroutine_fn GRAPH_RDLOCK cbw_co_snapshot_block_status(BlockDriverState* bs, unsigned int mode,
...
static int coroutine_fn GRAPH_RDLOCK cbw_co_pdiscard_snapshot(BlockDriverState* bs, int64_t offset, int64_t bytes)
{
    BDRVCopyBeforeWriteState* s              = bs->opaque;
    ...
    if (aligned_end <= aligned_offset) { return 0; }
    aligned_bytes = aligned_end - aligned_offset;

    WITH_QEMU_LOCK_GUARD(&s->lock) { bdrv_reset_dirty_bitmap(s->access_bitmap, aligned_offset, aligned_bytes); }
...
{ pstrcpy(bs->exact_filename, sizeof(bs->exact_filename), bs->file->bs->filename); }

static void GRAPH_RDLOCK cbw_child_perm(BlockDriverState* bs, BdrvChild* c, BdrvChildRole role,
        ...
        *nperm   = BLK_PERM_WRITE;
        ...
        if (!QLIST_EMPTY(&bs->parents)) {
            ...
            *nperm = *nperm | BLK_PERM_CONSISTENT_READ;
            if (s->discard_source) { *nperm = *nperm | BLK_PERM_WRITE; }
...
static int cbw_open(BlockDriverState* bs, QDict* options, int flags, Error** errp)
{
    ERRP_GUARD();
    BDRVCopyBeforeWriteState* s      = bs->opaque;
    ...
    s->target = bdrv_open_child(NULL, options, "target", bs, &child_of_bds, BDRV_CHILD_DATA, false, errp);
    if (!s->target) { return -EINVAL; }

    GRAPH_RDLOCK_GUARD_MAINLOOP();

    if (opts->bitmap) {
        bitmap = block_dirty_bitmap_lookup(opts->bitmap->node, opts->bitmap->name, NULL, errp);
        if (!bitmap) { return -EINVAL; }
    }
    s->on_cbw_error   = opts->has_on_cbw_error ? opts->on_cbw_error : ON_CBW_ERROR_BREAK_GUEST_WRITE;
    s->cbw_timeout_ns = opts->has_cbw_timeout ? opts->cbw_timeout * NANOSECONDS_PER_SECOND : 0;
    ...
    s->bcs = block_copy_state_new(bs->file, s->target, bs, bitmap, flags & BDRV_O_CBW_DISCARD_SOURCE,
    ...
    if (!s->done_bitmap) { return -EINVAL; }
    bdrv_disable_dirty_bitmap(s->done_bitmap);

    /* s->access_bitmap starts equal to bcs bitmap */
    s->access_bitmap = bdrv_create_dirty_bitmap(bs, cluster_size, NULL, errp);
    if (!s->access_bitmap) { return -EINVAL; }
    bdrv_disable_dirty_bitmap(s->access_bitmap);
    bdrv_dirty_bitmap_merge_internal(s->access_bitmap, block_copy_dirty_bitmap(s->bcs), NULL, true);
...
BlockDriverState* bdrv_cbw_append(BlockDriverState* source, BlockDriverState* target, const char* filter_node_name,
                                  bool discard_source, uint64_t min_cluster_size, BlockCopyState** bcs,
                                  OnCbwError on_cbw_error, Error** errp)
{
    ...
    if (filter_node_name) { qdict_put_str(opts, "node-name", filter_node_name); }
...
static void cbw_init(void) { bdrv_register(&bdrv_cbw_filter); }
