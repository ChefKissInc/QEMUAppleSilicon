...
static QLIST_HEAD(, BlockExport) block_exports = QLIST_HEAD_INITIALIZER(block_exports);
...
BlockExport* blk_exp_find(const char* id)
{
    ...
    QLIST_FOREACH (exp, &block_exports, next) {
        if (strcmp(id, exp->id) == 0) { return exp; }
    }
...
static const BlockExportDriver* blk_exp_find_driver(BlockExportType type)
{
    ...
    for (i = 0; i < ARRAY_SIZE(blk_exp_drivers); i++) {
        if (blk_exp_drivers[i]->type == type) { return blk_exp_drivers[i]; }
    }
    return NULL;
}
...
BlockExport* blk_exp_add(BlockExportOptions* export, Error** errp)
{
    ...
    if (!export->has_writable) { export->writable = false; }
    ...
    if (export->writable) { perm |= BLK_PERM_WRITE; }

    blk = blk_new(ctx, perm, BLK_PERM_ALL);

    if (!fixed_iothread) { blk_set_allow_aio_context_change(blk, true); }
    if (allow_inactive) { blk_set_force_allow_inactivate(blk); }

    ret = blk_insert_bs(blk, bs, errp);
    if (ret < 0) { goto fail; }

    if (!export->has_writethrough) { export->writethrough = false; }
...
void blk_exp_unref(BlockExport* exp)
{
    ...
    if (qatomic_fetch_dec(&exp->refcount) == 1) {
        ...
        aio_bh_schedule_oneshot(qemu_get_aio_context(), blk_exp_delete_bh, exp);
    }
}
...
static bool blk_exp_has_type(BlockExportType type)
{
    ...
    if (type == BLOCK_EXPORT_TYPE__MAX) { return !QLIST_EMPTY(&block_exports); }

    QLIST_FOREACH (exp, &block_exports, next) {
        if (exp->drv->type == type) { return true; }
    }
...
void blk_exp_close_all_type(BlockExportType type)
{
    ...
    QLIST_FOREACH_SAFE (exp, &block_exports, next, next) {
        if (type != BLOCK_EXPORT_TYPE__MAX && exp->drv->type != type) { continue; }
...
void blk_exp_close_all(void) { blk_exp_close_all_type(BLOCK_EXPORT_TYPE__MAX); }
...
void qmp_block_export_del(const char* id, bool has_mode, BlockExportRemoveMode mode, Error** errp)
{
    ...
    if (!has_mode) { mode = BLOCK_EXPORT_REMOVE_MODE_SAFE; }
    if (mode == BLOCK_EXPORT_REMOVE_MODE_SAFE && qatomic_read(&exp->refcount) > 1) {
