...
/* Called with s->bounce_lock held */
static bool blkio_do_alloc_bounce_buffer(BDRVBlkioState* s, BlkioBounceBuf* bounce, int64_t bytes)
{
    ...
    if (bytes > space) { return false; }
    if (prev) { QLIST_INSERT_AFTER(prev, bounce, next); }
    else {
...
static int coroutine_fn blkio_alloc_bounce_buffer(BDRVBlkioState* s, BlkioBounceBuf* bounce, int64_t bytes)
{
    /*
    ...
    if (!qemu_co_queue_empty(&s->bounce_available)) {
        qemu_co_queue_wait_flags(&s->bounce_available, &s->bounce_lock, wait_flags);
...
static void coroutine_fn blkio_free_bounce_buffer(BDRVBlkioState* s, BlkioBounceBuf* bounce)
{
    QEMU_LOCK_GUARD(&s->bounce_lock);
...
static void blkio_completion_fd_read(void* opaque)
{
    ...
    while (true) {
        ...
        WITH_QEMU_LOCK_GUARD(&s->blkio_lock) { ret = blkioq_do_io(s->blkioq, &completion, 0, 1, NULL); }
        if (ret != 1) { break; }
...
static bool blkio_completion_fd_poll(void* opaque)
{
    ...
    if (s->poll_completion.user_data != NULL) { return true; }

    WITH_QEMU_LOCK_GUARD(&s->blkio_lock) { ret = blkioq_do_io(s->blkioq, &s->poll_completion, 0, 1, NULL); }
    return ret == 1;
}

static void blkio_completion_fd_poll_ready(void* opaque) { blkio_completion_fd_read(opaque); }

static void blkio_attach_aio_context(BlockDriverState* bs, AioContext* new_context)
{
    BDRVBlkioState* s = bs->opaque;

    aio_set_fd_handler(new_context, s->completion_fd, blkio_completion_fd_read, NULL, blkio_completion_fd_poll,
...
static void blkio_detach_aio_context(BlockDriverState* bs)
{
    ...
    aio_set_fd_handler(bdrv_get_aio_context(bs), s->completion_fd, NULL, NULL, NULL, NULL, NULL);
}
...
static void blkio_deferred_fn(void* opaque)
{
    ...
    WITH_QEMU_LOCK_GUARD(&s->blkio_lock) { blkioq_do_io(s->blkioq, NULL, 0, 0, NULL); }
}
...
static int coroutine_fn blkio_co_pdiscard(BlockDriverState* bs, int64_t offset, int64_t bytes)
{
    BDRVBlkioState* s   = bs->opaque;
    BlkioCoData     cod = {
    ...
    WITH_QEMU_LOCK_GUARD(&s->blkio_lock) { blkioq_discard(s->blkioq, offset, bytes, &cod, 0); }
...
static int coroutine_fn blkio_co_preadv(BlockDriverState* bs, int64_t offset, int64_t bytes, QEMUIOVector* qiov,
                                        BdrvRequestFlags flags)
{
    ...
    BDRVBlkioState* s                 = bs->opaque;
    bool            use_bounce_buffer = s->needs_mem_regions && !(flags & BDRV_REQ_REGISTERED_BUF);
    ...
    WITH_QEMU_LOCK_GUARD(&s->blkio_lock) { blkioq_readv(s->blkioq, offset, iov, iovcnt, &cod, 0); }

    blkio_submit_io(bs);
    qemu_coroutine_yield();

    if (use_bounce_buffer) {
        if (cod.ret == 0) { qemu_iovec_from_buf(qiov, 0, bounce.buf.iov_base, bounce.buf.iov_len); }
...
static int coroutine_fn blkio_co_pwritev(BlockDriverState* bs, int64_t offset, int64_t bytes, QEMUIOVector* qiov,
                                         BdrvRequestFlags flags)
{
    ...
    WITH_QEMU_LOCK_GUARD(&s->blkio_lock) { blkioq_writev(s->blkioq, offset, iov, iovcnt, &cod, blkio_flags); }

    blkio_submit_io(bs);
    qemu_coroutine_yield();

    if (use_bounce_buffer) { blkio_free_bounce_buffer(s, &bounce); }
...
static int coroutine_fn blkio_co_flush(BlockDriverState* bs)
{
    ...
    WITH_QEMU_LOCK_GUARD(&s->blkio_lock) { blkioq_flush(s->blkioq, &cod, 0); }
...
static int coroutine_fn blkio_co_pwrite_zeroes(BlockDriverState* bs, int64_t offset, int64_t bytes,
                                               BdrvRequestFlags flags)
{
    BDRVBlkioState* s   = bs->opaque;
    BlkioCoData     cod = {
    ...
    if (flags & BDRV_REQ_FUA) { blkio_flags |= BLKIO_REQ_FUA; }
    if (!(flags & BDRV_REQ_MAY_UNMAP)) { blkio_flags |= BLKIO_REQ_NO_UNMAP; }
    if (flags & BDRV_REQ_NO_FALLBACK) { blkio_flags |= BLKIO_REQ_NO_FALLBACK; }

    WITH_QEMU_LOCK_GUARD(&s->blkio_lock) { blkioq_write_zeroes(s->blkioq, offset, bytes, &cod, blkio_flags); }
 ...
 */
static BlkioMemRegionResult blkio_mem_region_from_host(BlockDriverState* bs, void* host, size_t size,
                                                       struct blkio_mem_region* region, Error** errp)
{
    BDRVBlkioState* s         = bs->opaque;
    int             fd        = -1;
    ...
    if (s->needs_mem_region_fd) {
        ...
        if (ram_block) { fd = qemu_ram_get_fd(ram_block); }
        if (fd == -1) {
            /*
...
static bool blkio_register_buf(BlockDriverState* bs, void* host, size_t size, Error** errp)
{
    BDRVBlkioState*         s = bs->opaque;
     ...
     */
    if (!s->needs_mem_regions && s->may_pin_mem_regions) { return true; }

    region_result = blkio_mem_region_from_host(bs, host, size, &region, errp);
    if (region_result == BMRR_SKIP) { return true; }
    else if (region_result != BMRR_OK) {
        return false;
    }

    WITH_QEMU_LOCK_GUARD(&s->blkio_lock) { ret = blkio_map_mem_region(s->blkio, &region); }

    if (ret < 0) {
        error_setg(errp, "Failed to add blkio mem region %p with size %zu: %s", host, size, blkio_get_error_msg());
        return false;
    }
    return true;
}
...
static void blkio_unregister_buf(BlockDriverState* bs, void* host, size_t size)
{
    ...
    if (!s->needs_mem_regions && s->may_pin_mem_regions) { return; }

    if (blkio_mem_region_from_host(bs, host, size, &region, NULL) != BMRR_OK) { return; }

    WITH_QEMU_LOCK_GUARD(&s->blkio_lock) { blkio_unmap_mem_region(s->blkio, &region); }
}

static int blkio_io_uring_connect(BlockDriverState* bs, QDict* options, int flags, Error** errp)
{
    ...
    if (ret < 0) {
        error_setg_errno(errp, -ret, "failed to set path: %s", blkio_get_error_msg());
        return ret;
    }
    ...
    if (flags & BDRV_O_NOCACHE) {
        ...
        if (ret < 0) {
            error_setg_errno(errp, -ret, "failed to set direct: %s", blkio_get_error_msg());
            return ret;
        }
    }
    ...
    if (ret < 0) {
        error_setg_errno(errp, -ret, "blkio_connect failed: %s", blkio_get_error_msg());
        return ret;
    }
...
static int blkio_nvme_io_uring_connect(BlockDriverState* bs, QDict* options, int flags, Error** errp)
{
...
static int blkio_virtio_blk_connect(BlockDriverState* bs, QDict* options, int flags, Error** errp)
{
    const char*     path         = qdict_get_try_str(options, "path");
    BDRVBlkioState* s            = bs->opaque;
    ...
    if (blkio_set_int(s->blkio, "fd", -1) == 0) { fd_supported = true; }
    ...
    if (fd_supported && ret == -EINVAL) {
        ...
        if (ret < 0) {
            error_setg_errno(errp, -ret, "failed to set fd: %s", blkio_get_error_msg());
            return ret;
        }
...
static int blkio_open(BlockDriverState* bs, QDict* options, int flags, Error** errp)
{
    ...
    if (ret < 0) {
        error_setg_errno(errp, -ret, "blkio_create failed: %s", blkio_get_error_msg());
        return ret;
    }
    ...
    if (!(flags & BDRV_O_RDWR)) {
        ...
        if (ret < 0) {
            error_setg_errno(errp, -ret, "failed to set read-only: %s", blkio_get_error_msg());
            blkio_destroy(&s->blkio);
            return ret;
        }
    }

    if (strcmp(blkio_driver, "io_uring") == 0) { ret = blkio_io_uring_connect(bs, options, flags, errp); }
    ...
    ret = blkio_get_bool(s->blkio, "needs-mem-regions", &s->needs_mem_regions);
    if (ret < 0) {
        error_setg_errno(errp, -ret, "failed to get needs-mem-regions: %s", blkio_get_error_msg());
        blkio_destroy(&s->blkio);
        return ret;
    }

    ret = blkio_get_bool(s->blkio, "needs-mem-region-fd", &s->needs_mem_region_fd);
    if (ret < 0) {
        error_setg_errno(errp, -ret, "failed to get needs-mem-region-fd: %s", blkio_get_error_msg());
        blkio_destroy(&s->blkio);
        return ret;
    }

    ret = blkio_get_uint64(s->blkio, "mem-region-alignment", &s->mem_region_alignment);
    if (ret < 0) {
        error_setg_errno(errp, -ret, "failed to get mem-region-alignment: %s", blkio_get_error_msg());
        blkio_destroy(&s->blkio);
        return ret;
    }

    ret = blkio_get_bool(s->blkio, "may-pin-mem-regions", &s->may_pin_mem_regions);
    if (ret < 0) {
    ...
    if (ret < 0) {
        error_setg_errno(errp, -ret, "blkio_start failed: %s", blkio_get_error_msg());
        blkio_destroy(&s->blkio);
        if (s->may_pin_mem_regions) { ram_block_discard_disable(false); }
        return ret;
    }
...
static int64_t coroutine_fn blkio_co_getlength(BlockDriverState* bs)
{
    ...
    WITH_QEMU_LOCK_GUARD(&s->blkio_lock) { ret = blkio_get_uint64(s->blkio, "capacity", &capacity); }
    if (ret < 0) { return -ret; }

    return capacity;
}

static int coroutine_fn blkio_truncate(BlockDriverState* bs, int64_t offset, bool exact, PreallocMode prealloc,
...
static int coroutine_fn blkio_co_get_info(BlockDriverState* bs, BlockDriverInfo* bdi) { return 0; }
...
static void blkio_refresh_limits(BlockDriverState* bs, Error** errp)
{
    ...
    if (ret < 0) {
        error_setg_errno(errp, -ret, "failed to get \"request-alignment\": %s", blkio_get_error_msg());
        return;
    }
    bs->bl.request_alignment = value;
    if (bs->bl.request_alignment < 1 || bs->bl.request_alignment >= INT_MAX || !is_power_of_2(bs->bl.request_alignment))
    {
        error_setg(errp,
    ...
    if (ret < 0) {
        error_setg_errno(errp, -ret, "failed to get \"optimal-io-size\": %s", blkio_get_error_msg());
        return;
    }
    bs->bl.opt_transfer = value;
    if (bs->bl.opt_transfer > INT_MAX || (bs->bl.opt_transfer % bs->bl.request_alignment)) {
        error_setg(errp,
                   "invalid \"optimal-io-size\" value %" PRIu32 ", must "
                   "be a multiple of %" PRIu32,
    ...
    if (ret < 0) {
        error_setg_errno(errp, -ret, "failed to get \"max-transfer\": %s", blkio_get_error_msg());
        return;
    }
    ...
    {
        error_setg(errp,
                   "invalid \"max-transfer\" value %" PRIu32 ", must be "
                   "a multiple of %" PRIu32 " and %" PRIu32 " (if non-zero)",
                   bs->bl.max_transfer, bs->bl.request_alignment, bs->bl.opt_transfer);
        return;
    }

    ret = blkio_get_int(s->blkio, "buf-alignment", &value);
    if (ret < 0) {
        error_setg_errno(errp, -ret, "failed to get \"buf-alignment\": %s", blkio_get_error_msg());
        return;
    }
    if (value < 1) {
        error_setg(errp,
    ...
    if (ret < 0) {
        error_setg_errno(errp, -ret, "failed to get \"optimal-buf-alignment\": %s", blkio_get_error_msg());
        return;
    }
    if (value < 1) {
        error_setg(errp,
    ...
    if (ret < 0) {
        error_setg_errno(errp, -ret, "failed to get \"max-segments\": %s", blkio_get_error_msg());
        return;
    }
    if (value < 1) {
        error_setg(errp, "invalid \"max-segments\" value %d, must be positive", value);
        return;
    }
    bs->bl.max_iov = value;
}
...
#define BLKIO_DRIVER_COMMON                                                                                          \
    .instance_size = sizeof(BDRVBlkioState), .bdrv_open = blkio_open, .bdrv_close = blkio_close,                     \
    .bdrv_co_getlength = blkio_co_getlength, .bdrv_co_truncate = blkio_truncate,                                     \
    .bdrv_co_get_info = blkio_co_get_info, .bdrv_attach_aio_context = blkio_attach_aio_context,                      \
    .bdrv_detach_aio_context = blkio_detach_aio_context, .bdrv_co_pdiscard = blkio_co_pdiscard,                      \
    .bdrv_co_preadv = blkio_co_preadv, .bdrv_co_pwritev = blkio_co_pwritev, .bdrv_co_flush_to_disk = blkio_co_flush, \
    .bdrv_co_pwrite_zeroes = blkio_co_pwrite_zeroes, .bdrv_refresh_limits = blkio_refresh_limits,                    \
    .bdrv_register_buf = blkio_register_buf, .bdrv_unregister_buf = blkio_unregister_buf,
...
static BlockDriver bdrv_io_uring = {.format_name         = "io_uring",
                                    .protocol_name       = "io_uring",
                                    .bdrv_needs_filename = true,
                                    BLKIO_DRIVER_COMMON};

static BlockDriver bdrv_nvme_io_uring = {.format_name   = "nvme-io_uring",
