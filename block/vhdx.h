...
#define VHDX_HEADER_SIZE                                 \
    (4 * KiB) /* although the vhdx_header struct in disk \
 ...
 */
#define VHDX_REGION_ENTRY_REQUIRED         \
    0x01 /* if set, parser must understand \
...
#define VHDX_META_FLAGS_IS_VIRTUAL_DISK   \
    0x02 /* virtual disk metadata if set, \
            otherwise file metadata */
#define VHDX_META_FLAGS_IS_REQUIRED    \
    0x04 /* parse must understand this \
...
#define VHDX_PARAMS_LEAVE_BLOCKS_ALLOCED                           \
    0x01                            /* Do not change any blocks to \
...
int GRAPH_RDLOCK vhdx_update_headers(BlockDriverState* bs, BDRVVHDXState* s, bool rw, MSGUID* log_guid);
...
int GRAPH_RDLOCK vhdx_parse_log(BlockDriverState* bs, BDRVVHDXState* s, bool* flushed, Error** errp);

int coroutine_fn GRAPH_RDLOCK vhdx_log_write_and_flush(BlockDriverState* bs, BDRVVHDXState* s, void* data,
...
int GRAPH_RDLOCK vhdx_user_visible_write(BlockDriverState* bs, BDRVVHDXState* s);
