...
static const MSGUID metadata_guid = {.data1 = 0x8b7ca206,
                                     .data2 = 0x4790,
                                     .data3 = 0x4b9a,
                                     .data4 = {0xb8, 0xfe, 0x57, 0x5f, 0x05, 0x0f, 0x88, 0x6e}};
...
static const MSGUID phys_sector_guid = {.data1 = 0xcda348c7,
                                        .data2 = 0x445d,
                                        .data3 = 0x4471,
                                        .data4 = {0x9c, 0xc9, 0xe9, 0x88, 0x52, 0x51, 0xc5, 0x56}};

static const MSGUID parent_locator_guid = {.data1 = 0xa8d35f2d,
                                           .data2 = 0xb30b,
                                           .data3 = 0x454d,
                                           .data4 = {0xab, 0xf7, 0xd3, 0xd8, 0x48, 0x34, 0xab, 0x0c}};
...
static const MSGUID parent_vhdx_guid
    __attribute__((unused)) = {.data1 = 0xb04aefb7,
                               .data2 = 0xd19e,
                               .data3 = 0x4a81,
                               .data4 = {0xb7, 0x89, 0x25, 0xb8, 0xe9, 0x44, 0x59, 0x13}};
...
#define META_ALL_PRESENT                                                                 \
    (META_FILE_PARAMETER_PRESENT | META_VIRTUAL_DISK_SIZE_PRESENT | META_PAGE_83_PRESENT \
     | META_LOGICAL_SECTOR_SIZE_PRESENT | META_PHYS_SECTOR_SIZE_PRESENT)
...
uint32_t vhdx_checksum_calc(uint32_t crc, uint8_t* buf, size_t size, int crc_offset)
{
    uint32_t crc_new;
    uint32_t crc_orig;
    assert(buf != NULL);
    ...
    if (crc_offset > 0) { memcpy(buf + crc_offset, &crc_orig, sizeof(crc_orig)); }
...
static void vhdx_region_register(BDRVVHDXState* s, uint64_t start, uint64_t length)
{
    VHDXRegionEntry* r;
...
{
    if (buf_size >= 8 && !memcmp(buf, "vhdxfile", 8)) { return 100; }
    return 0;
}
 ...
 */
static int vhdx_write_header(BdrvChild* file, VHDXHeader* hdr, uint64_t offset, bool read)
{
    ...
    vhdx_update_checksum(buffer, VHDX_HEADER_SIZE, offsetof(VHDXHeader, checksum));
 ...
 */
static int GRAPH_RDLOCK vhdx_update_header(BlockDriverState* bs, BDRVVHDXState* s, bool generate_data_write_guid,
                                           MSGUID* log_guid)
{
    int      ret           = 0;
    int      hdr_idx       = 0;
    ...
    if (generate_data_write_guid) { vhdx_guid_generate(&inactive_header->data_write_guid); }

    /* update the log guid if present */
    if (log_guid) { inactive_header->log_guid = *log_guid; }
 ...
 */
int vhdx_update_headers(BlockDriverState* bs, BDRVVHDXState* s, bool generate_data_write_guid, MSGUID* log_guid)
{
    int ret;
...
static void GRAPH_RDLOCK vhdx_parse_header(BlockDriverState* bs, BDRVVHDXState* s, Error** errp)
{
    int         ret;
    VHDXHeader* header1;
    VHDXHeader* header2;
     ...
     * region */
    ret = bdrv_pread(bs->file, VHDX_HEADER1_OFFSET, VHDX_HEADER_SIZE, buffer, 0);
    if (ret < 0) { goto fail; }
    ...
    if (vhdx_checksum_is_valid(buffer, VHDX_HEADER_SIZE, 4)) {
        ...
        if (header1->signature == VHDX_HEADER_SIGNATURE && header1->version == 1) {
            h1_seq   = header1->sequence_number;
            h1_valid = true;
        }
    }

    ret = bdrv_pread(bs->file, VHDX_HEADER2_OFFSET, VHDX_HEADER_SIZE, buffer, 0);
    if (ret < 0) { goto fail; }
    /* copy over just the relevant portion that we need */
    ...
    if (vhdx_checksum_is_valid(buffer, VHDX_HEADER_SIZE, 4)) {
        ...
        if (header2->signature == VHDX_HEADER_SIGNATURE && header2->version == 1) {
    ...
    if (h1_valid && !h2_valid) { s->curr_header = 0; }
         ...
         * invalid */
        if (h1_seq > h2_seq) { s->curr_header = 0; }
            ...
            if (!memcmp(header1, header2, sizeof(VHDXHeader))) { s->curr_header = 0; }
            else {
                goto fail;
            }
        }
    ...
    vhdx_region_register(s, s->headers[s->curr_header]->log_offset, s->headers[s->curr_header]->log_length);
    goto exit;
...
static int GRAPH_RDLOCK vhdx_open_region_tables(BlockDriverState* bs, BDRVVHDXState* s)
{
    int                  ret = 0;
    uint8_t*             buffer;
    int                  offset = 0;
    ...
    ret = bdrv_pread(bs->file, VHDX_REGION_TABLE_OFFSET, VHDX_HEADER_BLOCK_SIZE, buffer, 0);
    if (ret < 0) { goto fail; }
 ...
 */
static int GRAPH_RDLOCK vhdx_parse_metadata(BlockDriverState* bs, BDRVVHDXState* s)
{
    int                    ret = 0;
    uint8_t*               buffer;
    int                    offset = 0;
    ...
    ret = bdrv_pread(bs->file, s->metadata_rt.file_offset, VHDX_METADATA_TABLE_MAX_SIZE, buffer, 0);
    if (ret < 0) { goto exit; }
    ...
    if ((s->metadata_hdr.entry_count * sizeof(md_entry)) > (VHDX_METADATA_TABLE_MAX_SIZE - offset)) {
        ret = -EINVAL;
        goto exit;
    }
    ...
    for (i = 0; i < s->metadata_hdr.entry_count; i++) {
        ...
        if (guid_eq(md_entry.item_id, logical_sector_guid)) {
            if (s->metadata_entries.present & META_LOGICAL_SECTOR_SIZE_PRESENT) {
                ret = -EINVAL;
                goto exit;
            }
    ...
    ret = bdrv_pread(bs->file, s->metadata_entries.file_parameters_entry.offset + s->metadata_rt.file_offset,
                     sizeof(s->params), &s->params, 0);
    ...
    ret = bdrv_pread(bs->file, s->metadata_entries.virtual_disk_size_entry.offset + s->metadata_rt.file_offset,
                     sizeof(uint64_t), &s->virtual_disk_size, 0);
    if (ret < 0) { goto exit; }
    ret = bdrv_pread(bs->file, s->metadata_entries.logical_sector_size_entry.offset + s->metadata_rt.file_offset,
                     sizeof(uint32_t), &s->logical_sector_size, 0);
    if (ret < 0) { goto exit; }
    ret = bdrv_pread(bs->file, s->metadata_entries.phys_sector_size_entry.offset + s->metadata_rt.file_offset,
                     sizeof(uint32_t), &s->physical_sector_size, 0);
    if (ret < 0) { goto exit; }
    ...
    if (s->params.block_size < VHDX_BLOCK_SIZE_MIN || s->params.block_size > VHDX_BLOCK_SIZE_MAX) {
        ret = -EINVAL;
        goto exit;
    }
...
static void vhdx_calc_bat_entries(BDRVVHDXState* s)
{
    ...
    if (s->parent_entries) { s->bat_entries = bitmap_blocks_cnt * (s->chunk_ratio + 1); }
    else {
        s->bat_entries = data_blocks_cnt + ((data_blocks_cnt - 1) >> s->chunk_ratio_bits);
    }
}

static int coroutine_mixed_fn GRAPH_RDLOCK vhdx_check_bat_entries(BlockDriverState* bs, int* errcnt)
{
    ...
    for (i = 0; i < s->bat_entries; i++) {
        if ((s->bat[i] & VHDX_BAT_STATE_BIT_MASK) == PAYLOAD_BLOCK_FULLY_PRESENT) {
             ...
             */
            uint32_t block_length = MIN(s->block_size, bs->total_sectors * BDRV_SECTOR_SIZE - i * s->block_size);
            /*
            ...
            if (offset >= image_file_size) {
                error_report("VHDX BAT entry %" PRIu64 " start offset %" PRIu64 " points after end of file (%" PRIi64
                             "). Image"
                ...
                error_report("VHDX BAT entry %" PRIu64 " end offset %" PRIu64 " points after end of file (%" PRIi64
                             "). Image"
                             " has probably been truncated.",
...
static int vhdx_open(BlockDriverState* bs, QDict* options, int flags, Error** errp)
{
    BDRVVHDXState* s   = bs->opaque;
    int            ret = 0;
    uint32_t       i;
    ...
    for (i = 0; i < s->bat_entries; i++) { s->bat[i] = le64_to_cpu(s->bat[i]); }
...
static int vhdx_reopen_prepare(BDRVReopenState* state, BlockReopenQueue* queue, Error** errp) { return 0; }

/*
 * Perform sector to block offset translations, to get various
 * sector and file offsets into the image.  See VHDXSectorInfo
 */
static void vhdx_block_translate(BDRVVHDXState* s, int64_t sector_num, int nb_sectors, VHDXSectorInfo* sinfo)
{
    ...
    if (sinfo->sectors_avail > nb_sectors) { sinfo->sectors_avail = nb_sectors; }
...
static int coroutine_fn vhdx_co_get_info(BlockDriverState* bs, BlockDriverInfo* bdi)
{
    BDRVVHDXState* s = bs->opaque;
...
static int coroutine_fn GRAPH_RDLOCK vhdx_co_readv(BlockDriverState* bs, int64_t sector_num, int nb_sectors,
                                                   QEMUIOVector* qiov)
{
    BDRVVHDXState* s   = bs->opaque;
    int            ret = 0;
        ...
        if (s->params.data_bits & VHDX_PARAMS_HAS_PARENT) {
            ...
            switch (s->bat[sinfo.bat_idx] & VHDX_BAT_STATE_BIT_MASK) {
                ...
                case PAYLOAD_BLOCK_FULLY_PRESENT:
                    ...
                    ret = bdrv_co_preadv(bs->file, sinfo.file_offset, sinfo.sectors_avail * BDRV_SECTOR_SIZE, &hd_qiov,
                                         0);
                    qemu_co_mutex_lock(&s->lock);
                    if (ret < 0) { goto exit; }
                    break;
 ...
 */
static int coroutine_fn GRAPH_RDLOCK vhdx_allocate_block(BlockDriverState* bs, BDRVVHDXState* s, uint64_t* new_offset,
                                                         bool* need_zero)
{
    int64_t current_len;
    ...
    *new_offset = ROUND_UP(*new_offset, 1 * MiB);
    if (*new_offset > INT64_MAX) { return -EINVAL; }

    if (*need_zero) {
        int ret;

        ret = bdrv_co_truncate(bs->file, *new_offset + s->block_size, false, PREALLOC_MODE_OFF, BDRV_REQ_ZERO_WRITE,
                               NULL);
        if (ret != -ENOTSUP) {
            *need_zero = false;
            return ret;
    ...
    return bdrv_co_truncate(bs->file, *new_offset + s->block_size, false, PREALLOC_MODE_OFF, 0, NULL);
}

/*
 * Update the BAT table entry with the new file offset, and the new entry
 * state */
static void vhdx_update_bat_table_entry(BlockDriverState* bs, BDRVVHDXState* s, VHDXSectorInfo* sinfo,
                                        uint64_t* bat_entry_le, uint64_t* bat_offset, int state)
{
    /* The BAT entry is a uint64, with 44 bits for the file offset in units of
     * 1MB, and 3 bits for the block state. */
    if ((state == PAYLOAD_BLOCK_ZERO) || (state == PAYLOAD_BLOCK_UNDEFINED) || (state == PAYLOAD_BLOCK_NOT_PRESENT)
...
static int coroutine_fn GRAPH_RDLOCK vhdx_co_writev(BlockDriverState* bs, int64_t sector_num, int nb_sectors,
        ...
        if (s->params.data_bits & VHDX_PARAMS_HAS_PARENT) {
            ...
            switch (bat_state) {
                ...
                case PAYLOAD_BLOCK_UNDEFINED:
                    ...
                    ret              = vhdx_allocate_block(bs, s, &sinfo.file_offset, &use_zero_buffers);
                    if (ret < 0) { goto exit; }
                    /*
                    ...
                    vhdx_update_bat_table_entry(bs, s, &sinfo, &bat_entry, &bat_entry_offset,
                     ...
                     */
                    if (!use_zero_buffers) { sinfo.file_offset += sinfo.block_offset; }
                    else {
                        ...
                        if (sinfo.block_offset) {
                            ...
                            qemu_iovec_concat_iov(&hd_qiov, &iov1, 1, 0, iov1.iov_len);
                        ...
                        if ((sinfo.bytes_avail - sinfo.block_offset) < s->block_size) {
                            iov2.iov_len  = s->block_size - (sinfo.bytes_avail + sinfo.block_offset);
                            iov2.iov_base = qemu_blockalign(bs, iov2.iov_len);
                            memset(iov2.iov_base, 0, iov2.iov_len);
                            qemu_iovec_concat_iov(&hd_qiov, &iov2, 1, 0, iov2.iov_len);
                ...
                case PAYLOAD_BLOCK_FULLY_PRESENT:
                    ...
                    if (!use_zero_buffers) { qemu_iovec_concat(&hd_qiov, qiov, bytes_done, sinfo.bytes_avail); }
                    /* block exists, so we can just overwrite it */
                    qemu_co_mutex_unlock(&s->lock);
                    ret =
                        bdrv_co_pwritev(bs->file, sinfo.file_offset, sectors_to_write * BDRV_SECTOR_SIZE, &hd_qiov, 0);
                    qemu_co_mutex_lock(&s->lock);
                    if (ret < 0) { goto error_bat_restore; }
                    break;
                case PAYLOAD_BLOCK_PARTIALLY_PRESENT:
                    /* we don't yet support difference files, fall through
                     * to error */
            ...
            if (bat_update) {
                ...
                ret = vhdx_log_write_and_flush(bs, s, &bat_entry, sizeof(VHDXBatEntry), bat_entry_offset);
                if (ret < 0) { goto exit; }
            }
...
error_bat_restore:
    if (bat_update) {
        ...
        vhdx_update_bat_table_entry(bs, s, &sinfo, &bat_entry, &bat_entry_offset, bat_state);
    }
exit:
 ...
 * the active header
 */
static int coroutine_fn GRAPH_UNLOCKED vhdx_create_new_headers(BlockBackend* blk, uint64_t image_size,
                                                               uint32_t log_size)
{
...
#define VHDX_METADATA_ENTRY_BUFFER_SIZE                                                      \
    (sizeof(VHDXFileParameters) + sizeof(VHDXVirtualDiskSize) + sizeof(VHDXPage83Data)       \
     + sizeof(VHDXVirtualDiskLogicalSectorSize) + sizeof(VHDXVirtualDiskPhysicalSectorSize))
 ...
 */
static int coroutine_fn vhdx_create_new_metadata(BlockBackend* blk, uint64_t image_size, uint32_t block_size,
                                                 uint32_t sector_size, uint64_t metadata_offset, VHDXImageType type)
{
    ...
    md_table_entry[1].data_bits |= VHDX_META_FLAGS_IS_REQUIRED | VHDX_META_FLAGS_IS_VIRTUAL_DISK;
    ...
    md_table_entry[2].data_bits |= VHDX_META_FLAGS_IS_REQUIRED | VHDX_META_FLAGS_IS_VIRTUAL_DISK;
    ...
    md_table_entry[3].data_bits |= VHDX_META_FLAGS_IS_REQUIRED | VHDX_META_FLAGS_IS_VIRTUAL_DISK;
    ...
    md_table_entry[4].data_bits |= VHDX_META_FLAGS_IS_REQUIRED | VHDX_META_FLAGS_IS_VIRTUAL_DISK;
    vhdx_metadata_entry_le_export(&md_table_entry[4]);

    ret = blk_co_pwrite(blk, metadata_offset, VHDX_HEADER_BLOCK_SIZE, buffer, 0);
    if (ret < 0) { goto exit; }

    ret = blk_co_pwrite(blk, metadata_offset + (64 * KiB), VHDX_METADATA_ENTRY_BUFFER_SIZE, entry_buffer, 0);
    if (ret < 0) { goto exit; }
 ...
 */
static int coroutine_fn GRAPH_UNLOCKED vhdx_create_bat(BlockBackend* blk, BDRVVHDXState* s, uint64_t image_size,
                                                       VHDXImageType type, bool use_zero_blocks, uint64_t file_offset,
                                                       uint32_t length, Error** errp)
{
    ...
    if (type == VHDX_TYPE_DYNAMIC) {
        ...
        ret = blk_co_truncate(blk, data_file_offset, false, PREALLOC_MODE_OFF, 0, errp);
        if (ret < 0) { goto exit; }
    }
    else if (type == VHDX_TYPE_FIXED) {
        ret = blk_co_truncate(blk, data_file_offset + image_size, false, PREALLOC_MODE_OFF, 0, errp);
        if (ret < 0) { goto exit; }
    }
    else {
    ...
    if (type == VHDX_TYPE_FIXED || use_zero_blocks || has_zero_init == 0) {
        ...
        while (sector_num < total_sectors) {
            ...
            sinfo.file_offset = data_file_offset + (sector_num << s->logical_sector_size_bits);
            sinfo.file_offset = ROUND_UP(sinfo.file_offset, MiB);
            vhdx_update_bat_table_entry(blk_bs(blk), s, &sinfo, &unused, &unused, block_state);
 ...
 * created.
 */
static int coroutine_fn GRAPH_UNLOCKED vhdx_create_new_region_table(BlockBackend* blk, uint64_t image_size,
                                                                    ...
                                                                    Error** errp)
{
    ...
    s->chunk_ratio = (VHDX_MAX_SECTORS_PER_BLOCK) * (uint64_t)sector_size / (uint64_t)block_size;
    ...
    rt_metadata->file_offset = ROUND_UP(rt_bat->file_offset + rt_bat->length, MiB);
    ...
    *metadata_offset         = rt_metadata->file_offset;
    ...
    vhdx_update_checksum(buffer, VHDX_HEADER_BLOCK_SIZE, offsetof(VHDXRegionTableHeader, checksum));

    /* The region table gives us the data we need to create the BAT,
     * so do that now */
    ret = vhdx_create_bat(blk, s, image_size, type, use_zero_blocks, bat_file_offset, bat_length, errp);
    if (ret < 0) { goto exit; }

    /* Now write out the region headers to disk */
    ret = blk_co_pwrite(blk, VHDX_REGION_TABLE_OFFSET, VHDX_HEADER_BLOCK_SIZE, buffer, 0);
    if (ret < 0) {
        error_setg_errno(errp, -ret, "Failed to write first region table");
        goto exit;
    }

    ret = blk_co_pwrite(blk, VHDX_REGION_TABLE2_OFFSET, VHDX_HEADER_BLOCK_SIZE, buffer, 0);
    if (ret < 0) {
 ...
 *   1MB
 */
static int coroutine_fn GRAPH_UNLOCKED vhdx_co_create(BlockdevCreateOptions* opts, Error** errp)
{
    ...
    if (!vhdx_opts->has_log_size) { log_size = DEFAULT_LOG_SIZE; }
    else {
    ...
    if (!vhdx_opts->has_block_state_zero) { use_zero_blocks = true; }
    else {
        use_zero_blocks = vhdx_opts->block_state_zero;
    }

    if (!vhdx_opts->has_subformat) { vhdx_opts->subformat = BLOCKDEV_VHDX_SUBFORMAT_DYNAMIC; }

    switch (vhdx_opts->subformat) {
        case BLOCKDEV_VHDX_SUBFORMAT_DYNAMIC: image_type = VHDX_TYPE_DYNAMIC; break;
        case BLOCKDEV_VHDX_SUBFORMAT_FIXED  : image_type = VHDX_TYPE_FIXED; break;
        default                             : assert_not_reached();
    }

    /* These are pretty arbitrary, and mainly designed to keep the BAT
     * size reasonable to load into RAM */
    if (vhdx_opts->has_block_size) { block_size = vhdx_opts->block_size; }
    else {
        if (image_size > 32 * TiB) { block_size = 64 * MiB; }
    ...
    if (block_size > VHDX_BLOCK_SIZE_MAX) {
        error_setg(errp, "Block size must not exceed %" PRId64, VHDX_BLOCK_SIZE_MAX);
        return -EINVAL;
    }
    ...
    creator   = g_utf8_to_utf16("QEMU v" QEMU_VERSION, -1, NULL, &creator_items, NULL);
    signature = cpu_to_le64(VHDX_FILE_SIGNATURE);
    ret       = blk_co_pwrite(blk, VHDX_FILE_ID_OFFSET, sizeof(signature), &signature, 0);
    if (ret < 0) {
    ...
    if (creator) {
        ret =
            blk_co_pwrite(blk, VHDX_FILE_ID_OFFSET + sizeof(signature), creator_items * sizeof(gunichar2), creator, 0);
        if (ret < 0) {
    ...
    ret = vhdx_create_new_region_table(blk, image_size, block_size, 512, log_size, use_zero_blocks, image_type,
                                       &metadata_offset, errp);
    if (ret < 0) { goto delete_and_exit; }

    /* Creates (H) */
    ret = vhdx_create_new_metadata(blk, image_size, block_size, 512, metadata_offset, image_type);
    if (ret < 0) {
...
static int coroutine_fn GRAPH_UNLOCKED vhdx_co_create_opts(BlockDriver* drv, const char* filename, QemuOpts* opts,
                                                           Error** errp)
{
    ...
    create_options->u.vhdx.size = ROUND_UP(create_options->u.vhdx.size, BDRV_SECTOR_SIZE);

    if (create_options->u.vhdx.has_log_size) {
        create_options->u.vhdx.log_size = ROUND_UP(create_options->u.vhdx.log_size, MiB);
    }
    if (create_options->u.vhdx.has_block_size) {
        create_options->u.vhdx.block_size = ROUND_UP(create_options->u.vhdx.block_size, MiB);

        if (create_options->u.vhdx.block_size == 0) { create_options->u.vhdx.has_block_size = false; }
 ...
 * for us to do here
 */
static int coroutine_fn GRAPH_RDLOCK vhdx_co_check(BlockDriverState* bs, BdrvCheckResult* result, BdrvCheckMode fix)
{
    BDRVVHDXState* s = bs->opaque;

    if (s->log_replayed_on_open) { result->corruptions_fixed++; }
...
static QemuOptsList vhdx_create_opts = {
    ...
    .desc = {{.name = BLOCK_OPT_SIZE, .type = QEMU_OPT_SIZE, .help = "Virtual disk size; max of 64TB."},
