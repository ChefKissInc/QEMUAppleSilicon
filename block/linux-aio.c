...
static inline ssize_t io_event_ret(struct io_event* ev) { return (ssize_t)(((uint64_t)ev->res2 << 32) | ev->res); }
...
static void qemu_laio_process_completion(struct qemu_laiocb* laiocb)
{
    ...
    ret = laiocb->ret;
    if (ret != -ECANCELED) {
        if (ret == laiocb->nbytes) { ret = 0; }
        else if (ret >= 0) {
            /* Short reads mean EOF, pad with zeros. */
            if (laiocb->is_read) { qemu_iovec_memset(laiocb->qiov, ret, 0, laiocb->qiov->size - ret); }
            else {
                ret = -ENOSPC;
            }
        }
    ...
    if (!qemu_coroutine_entered(laiocb->co)) { aio_co_wake(laiocb->co); }
}
 ...
 */
static inline unsigned int io_getevents_peek(io_context_t ctx, struct io_event** events)
{
...
static inline void io_getevents_commit(io_context_t ctx, unsigned int nr)
{
    ...
    if (nr) { ring->head = (ring->head + nr) % ring->nr; }
}
 ...
 */
static inline unsigned int io_getevents_advance_and_peek(io_context_t ctx, struct io_event** events, unsigned int nr)
{
...
static void qemu_laio_process_completions(LinuxAioState* s)
{
    ...
    while ((s->event_max = io_getevents_advance_and_peek(s->ctx, &events, s->event_idx))) {
...
static void qemu_laio_process_completions_and_submit(LinuxAioState* s)
{
    ...
    if (!QSIMPLEQ_EMPTY(&s->io_q.pending)) { ioq_submit(s); }
}
...
static void qemu_laio_completion_cb(EventNotifier* e)
{
    ...
    if (event_notifier_test_and_clear(&s->e)) { qemu_laio_process_completions_and_submit(s); }
}
...
static void ioq_submit(LinuxAioState* s)
{
    ...
    do {
        if (s->io_q.in_flight >= MAX_EVENTS) { break; }
        len = 0;
        QSIMPLEQ_FOREACH (aiocb, &s->io_q.pending, next) {
            iocbs[len++] = &aiocb->iocb;
            if (s->io_q.in_flight + len >= MAX_EVENTS) { break; }
        }
...
static void laio_deferred_fn(void* opaque)
{
    ...
    if (!s->io_q.blocked && !QSIMPLEQ_EMPTY(&s->io_q.pending)) { ioq_submit(s); }
}

static int laio_do_submit(int fd, struct qemu_laiocb* laiocb, off_t offset, int type, BdrvRequestFlags flags,
                          uint64_t dev_max_batch)
{
...
#endif
        break;
        case QEMU_AIO_ZONE_APPEND: io_prep_pwritev(iocbs, fd, qiov->iov, qiov->niov, offset); break;
        case QEMU_AIO_READ       : io_prep_preadv(iocbs, fd, qiov->iov, qiov->niov, offset); break;
        case QEMU_AIO_FLUSH      : io_prep_fdsync(iocbs, fd); break;
        /* Currently Linux kernel does not support other operations */
        default: fprintf(stderr, "%s: invalid AIO request type 0x%x.\n", __func__, type); return -EIO;
    }
    ...
    s->io_q.in_queue++;
    if (!s->io_q.blocked) {
        if (s->io_q.in_queue >= laio_max_batch(s, dev_max_batch)) { ioq_submit(s); }
        else {
...
int coroutine_fn laio_co_submit(int fd, uint64_t offset, QEMUIOVector* qiov, int type, BdrvRequestFlags flags,
                                uint64_t dev_max_batch)
{
    int                ret;
    ...
    if (laiocb.ret == -EINPROGRESS) { qemu_coroutine_yield(); }
    return laiocb.ret;
}
...
void laio_attach_aio_context(LinuxAioState* s, AioContext* new_context)
{
    ...
    aio_set_event_notifier(new_context, &s->e, qemu_laio_completion_cb, qemu_laio_poll_cb, qemu_laio_poll_ready);
}
...
void laio_cleanup(LinuxAioState* s)
{
    ...
    if (io_destroy(s->ctx) != 0) { fprintf(stderr, "%s: destroy AIO context %p failed\n", __func__, &s->ctx); }
    g_free(s);
}
