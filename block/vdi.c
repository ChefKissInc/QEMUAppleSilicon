...
#define logout(fmt, ...)                                                               \
    do {                                                                               \
        if (VDI_DEBUG) { fprintf(stderr, "vdi\t%-24s" fmt, __func__, ##__VA_ARGS__); } \
    }                                                                                  \
    while (0)
...
#define VDI_BLOCKS_IN_IMAGE_MAX ((unsigned)((INT_MAX + 1u - BDRV_SECTOR_SIZE) / sizeof(uint32_t)))
...
static int coroutine_fn vdi_co_check(BlockDriverState* bs, BdrvCheckResult* res, BdrvCheckMode fix)
{
    ...
    for (block = 0; block < s->header.blocks_in_image; block++) {
        ...
        if (VDI_IS_ALLOCATED(bmap_entry)) {
            if (bmap_entry < s->header.blocks_in_image) {
                blocks_allocated++;
                if (!VDI_IS_ALLOCATED(bmap[bmap_entry])) { bmap[bmap_entry] = bmap_entry; }
                else {
...
static int coroutine_fn vdi_co_get_info(BlockDriverState* bs, BlockDriverInfo* bdi)
{
...
static int vdi_open(BlockDriverState* bs, QDict* options, int flags, Error** errp)
{
    ...
    if (VDI_DEBUG) { vdi_header_print(&header); }

    if (header.disk_size > VDI_DISK_SIZE_MAX) {
        error_setg(errp, "Unsupported VDI image size (size is 0x%" PRIx64 ", max supported is 0x%" PRIx64 ")",
    ...
    if (header.signature != VDI_SIGNATURE) {
        error_setg(errp, "Image not in VDI format (bad signature %08" PRIx32 ")", header.signature);
        ret = -EINVAL;
        goto fail;
    }
    else if (header.version != VDI_VERSION_1_1) {
        error_setg(errp, "unsupported VDI image (version %" PRIu32 ".%" PRIu32 ")", header.version >> 16,
        ...
        error_setg(errp, "unsupported VDI image (unaligned data offset 0x%" PRIx32 ")", header.offset_data);
        ret = -ENOTSUP;
        goto fail;
    }
    else if (header.sector_size != SECTOR_SIZE) {
        error_setg(errp, "unsupported VDI image (sector size %" PRIu32 " is not %llu)", header.sector_size,
                   SECTOR_SIZE);
        ret = -ENOTSUP;
        goto fail;
    }
    else if (header.block_size != DEFAULT_CLUSTER_SIZE) {
        error_setg(errp, "unsupported VDI image (block size %" PRIu32 " is not %" PRIu32 ")", header.block_size,
                   DEFAULT_CLUSTER_SIZE);
        ret = -ENOTSUP;
        goto fail;
    }
    else if (header.disk_size > (uint64_t)header.blocks_in_image * header.block_size) {
        error_setg(errp,
    ...
    ret = bdrv_pread(bs->file, header.offset_bmap, bmap_size * SECTOR_SIZE, s->bmap, 0);
    if (ret < 0) { goto fail_free_bmap; }
...
static int vdi_reopen_prepare(BDRVReopenState* state, BlockReopenQueue* queue, Error** errp) { return 0; }

static int coroutine_fn GRAPH_RDLOCK vdi_co_block_status(BlockDriverState* bs, unsigned int mode, int64_t offset,
                                                         ...
                                                         BlockDriverState** file)
{
    ...
    *pnum  = MIN(s->block_size - index_in_block, bytes);
    ...
    if (!result) { return BDRV_BLOCK_ZERO; }

    *map  = s->header.offset_data + (uint64_t)bmap_entry * s->block_size + index_in_block;
    *file = bs->file->bs;
    return BDRV_BLOCK_DATA | BDRV_BLOCK_OFFSET_VALID
           | (s->header.image_type == VDI_TYPE_STATIC ? BDRV_BLOCK_RECURSE : 0);
}

static int coroutine_fn GRAPH_RDLOCK vdi_co_preadv(BlockDriverState* bs, int64_t offset, int64_t bytes,
                                                   QEMUIOVector* qiov, BdrvRequestFlags flags)
{
    ...
    while (ret >= 0 && bytes > 0) {
        ...
        if (!VDI_IS_ALLOCATED(bmap_entry)) {
            ...
            uint64_t data_offset = s->header.offset_data + (uint64_t)bmap_entry * s->block_size + offset_in_block;

            qemu_iovec_reset(&local_qiov);
            qemu_iovec_concat(&local_qiov, qiov, bytes_done, n_bytes);

            ret = bdrv_co_preadv(bs->file, data_offset, n_bytes, &local_qiov, 0);
        }
        logout("%u bytes read\n", n_bytes);
...
static int coroutine_fn GRAPH_RDLOCK vdi_co_pwritev(BlockDriverState* bs, int64_t offset, int64_t bytes,
                                                    QEMUIOVector* qiov, BdrvRequestFlags flags)
{
    BDRVVdiState* s = bs->opaque;
    QEMUIOVector  local_qiov;
    ...
    while (ret >= 0 && bytes > 0) {
        ...
        if (!VDI_IS_ALLOCATED(bmap_entry)) {
            ...
            s->header.blocks_allocated++;
            data_offset = s->header.offset_data + (uint64_t)bmap_entry * s->block_size;
            if (block == NULL) {
            ...
            qemu_iovec_to_buf(qiov, bytes_done, block + offset_in_block, n_bytes);
            memset(block + offset_in_block + n_bytes, 0, s->block_size - n_bytes - offset_in_block);

            /* Write the new block under CoRwLock write-side protection,
             * so this full-cluster write does not overlap a partial write
             * of the same cluster, issued from the "else" branch.
             */
            ret = bdrv_co_pwrite(bs->file, data_offset, s->block_size, block, 0);
            qemu_co_rwlock_unlock(&s->bmap_lock);
        }
        else {
        nonallocating_write:
            data_offset = s->header.offset_data + (uint64_t)bmap_entry * s->block_size + offset_in_block;
            qemu_co_rwlock_unlock(&s->bmap_lock);

            qemu_iovec_reset(&local_qiov);
            qemu_iovec_concat(&local_qiov, qiov, bytes_done, n_bytes);

            ret = bdrv_co_pwritev(bs->file, data_offset, n_bytes, &local_qiov, 0);
        }
    ...
    if (block) {
        ...
        *header = s->header;
        ...
        ret = bdrv_co_pwrite(bs->file, bmap_offset * SECTOR_SIZE, n_sectors * SECTOR_SIZE, base, 0);
    }

    return ret;
}

static int coroutine_fn GRAPH_UNLOCKED vdi_co_do_create(BlockdevCreateOptions* create_options, size_t block_size,
                                                        Error** errp)
{
    ERRP_GUARD();
    ...
    if (!vdi_opts->has_preallocation) { vdi_opts->preallocation = PREALLOC_MODE_OFF; }
    switch (vdi_opts->preallocation) {
        case PREALLOC_MODE_OFF     : image_type = VDI_TYPE_DYNAMIC; break;
        case PREALLOC_MODE_METADATA: image_type = VDI_TYPE_STATIC; break;
        default                    : error_setg(errp, "Preallocation mode not supported for vdi"); return -EINVAL;
    }
    ...
    if (block_size != DEFAULT_CLUSTER_SIZE) {
        ret = -ENOTSUP;
        error_setg(errp, "A non-default cluster size is not supported in this build");
        goto exit;
    }
#endif
    ...
    if (bytes > VDI_DISK_SIZE_MAX) {
        ret = -ENOTSUP;
        error_setg(errp, "Unsupported VDI image size (size is 0x%" PRIx64 ", max supported is 0x%" PRIx64 ")", bytes,
                   VDI_DISK_SIZE_MAX);
        goto exit;
    }
    ...
    blk = blk_co_new_with_bs(bs_file, BLK_PERM_WRITE | BLK_PERM_RESIZE, BLK_PERM_ALL, errp);
    if (!blk) {
        ret = -EPERM;
        goto exit;
    }
    ...
    if (image_type == VDI_TYPE_STATIC) { header.blocks_allocated = blocks; }
    ...
    if (image_type == VDI_TYPE_STATIC) {
        ret = blk_co_truncate(blk, offset + blocks * block_size, false, PREALLOC_MODE_OFF, 0, errp);
        if (ret < 0) {
...
static int coroutine_fn GRAPH_UNLOCKED vdi_co_create(BlockdevCreateOptions* create_options, Error** errp)
{ return vdi_co_do_create(create_options, DEFAULT_CLUSTER_SIZE, errp); }

static int coroutine_fn GRAPH_UNLOCKED vdi_co_create_opts(BlockDriver* drv, const char* filename, QemuOpts* opts,
                                                          Error** errp)
{
    QDict*                 qdict          = NULL;
    ...
    block_size = qemu_opt_get_size_del(opts, BLOCK_OPT_CLUSTER_SIZE, DEFAULT_CLUSTER_SIZE);
    if (block_size < BDRV_SECTOR_SIZE || block_size > UINT32_MAX || !is_power_of_2(block_size)) {
        error_setg(errp, "Invalid cluster size");
        ret = -EINVAL;
        goto done;
    }
#endif
    if (qemu_opt_get_bool_del(opts, BLOCK_OPT_STATIC, false)) { is_static = true; }
    ...
    bs_file = bdrv_co_open(filename, NULL, NULL, BDRV_O_RDWR | BDRV_O_RESIZE | BDRV_O_PROTOCOL, errp);
    if (!bs_file) {
        ret = -EIO;
        goto done;
    }
    ...
    if (is_static) { qdict_put_str(qdict, "preallocation", "metadata"); }
    ...
    assert(create_options->driver == BLOCKDEV_DRIVER_VDI);
    create_options->u.vdi.size = ROUND_UP(create_options->u.vdi.size, BDRV_SECTOR_SIZE);
...
static int GRAPH_RDLOCK vdi_has_zero_init(BlockDriverState* bs)
{
    ...
    if (s->header.image_type == VDI_TYPE_STATIC) { return bdrv_has_zero_init(bs->file->bs); }
    else {
        return 1;
    }
}
