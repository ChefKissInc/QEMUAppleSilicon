...
void cpu_list_lock(void) { qemu_mutex_lock(&qemu_cpu_list_lock); }

void cpu_list_unlock(void) { qemu_mutex_unlock(&qemu_cpu_list_lock); }
...
int cpu_get_free_index(void)
{
    ...
    CPU_FOREACH (some_cpu) {
        if (some_cpu->cpu_index >= max_cpu_index) { max_cpu_index = some_cpu->cpu_index + 1; }
    }
...
unsigned int cpu_list_generation_id_get(void) { return cpu_list_generation_id; }
...
CPUState* qemu_get_cpu(int index)
{
    ...
    CPU_FOREACH (cpu) {
        if (cpu->cpu_index == index) { return cpu; }
    }
...
{
    while (pending_cpus) { qemu_cond_wait(&exclusive_resume, &qemu_cpu_list_lock); }
}
...
void start_exclusive(void)
{
    ...
    while (pending_cpus > 1) { qemu_cond_wait(&exclusive_cond, &qemu_cpu_list_lock); }
...
{
    current_cpu->exclusive_context_count--;
    if (current_cpu->exclusive_context_count) { return; }
...
void cpu_exec_end(CPUState* cpu)
{
    ...
    if (unlikely(qatomic_read(&pending_cpus))) {
        ...
        if (cpu->has_waiter) {
            ...
            if (pending_cpus == 1) { qemu_cond_signal(&exclusive_cond); }
        }
    }
}
...
int cpu_breakpoint_insert(CPUState* cpu, vaddr pc, int flags, CPUBreakpoint** breakpoint)
{
    ...
    if (cpu->cc->gdb_adjust_breakpoint) { pc = cpu->cc->gdb_adjust_breakpoint(cpu, pc); }
    ...
    if (flags & BP_GDB) { QTAILQ_INSERT_HEAD(&cpu->breakpoints, bp, entry); }
    else {
        QTAILQ_INSERT_TAIL(&cpu->breakpoints, bp, entry);
    }

    if (breakpoint) { *breakpoint = bp; }
...
void cpu_breakpoint_remove_all(CPUState* cpu, int mask)
{
    ...
    QTAILQ_FOREACH_SAFE (bp, &cpu->breakpoints, entry, next) {
        if (bp->flags & mask) { cpu_breakpoint_remove_by_ref(cpu, bp); }
    }
}
