...
static void dma_blk_unmap(DMAAIOCB* dbs)
{
    ...
    for (i = 0; i < dbs->iov.niov; ++i) {
        dma_memory_unmap(dbs->sg->as, dbs->iov.iov[i].iov_base, dbs->iov.iov[i].iov_len, dbs->dir,
...
static void dma_complete(DMAAIOCB* dbs, int ret)
{
    ...
    assert(!dbs->acb && !dbs->bh);
    dma_blk_unmap(dbs);
    if (dbs->common.cb) { dbs->common.cb(dbs->common.opaque, ret); }
...
static void dma_aio_cancel(BlockAIOCB* acb)
{
    ...
    if (dbs->common.cb) { dbs->common.cb(dbs->common.opaque, -ECANCELED); }
}
...
static BlockAIOCB* dma_blk_read_io_func(int64_t offset, QEMUIOVector* iov, BlockCompletionFunc* cb, void* cb_opaque,
                                        void* opaque)
{
...
BlockAIOCB* dma_blk_read(BlockBackend* blk, QEMUSGList* sg, uint64_t offset, uint32_t align,
                         void (*cb)(void* opaque, int ret), void* opaque)
{ return dma_blk_io(sg, offset, align, dma_blk_read_io_func, blk, cb, opaque, DMA_DIRECTION_FROM_DEVICE); }

static BlockAIOCB* dma_blk_write_io_func(int64_t offset, QEMUIOVector* iov, BlockCompletionFunc* cb, void* cb_opaque,
                                         void* opaque)
{
    BlockBackend* blk = opaque;
...
BlockAIOCB* dma_blk_write(BlockBackend* blk, QEMUSGList* sg, uint64_t offset, uint32_t align,
                          void (*cb)(void* opaque, int ret), void* opaque)
{ return dma_blk_io(sg, offset, align, dma_blk_write_io_func, blk, cb, opaque, DMA_DIRECTION_TO_DEVICE); }

static MemTxResult dma_buf_rw(void* buf, dma_addr_t len, dma_addr_t* residual, QEMUSGList* sg, DMADirection dir,
                              MemTxAttrs attrs)
{
    uint8_t*    ptr = buf;
    ...
    if (residual) { *residual = xresidual; }
    return res;
}
...
uint64_t dma_aligned_pow2_mask(uint64_t start, uint64_t end, int max_addr_bits)
{
    ...
    if (max_addr_bits != 64) { max_mask = (1ULL << max_addr_bits) - 1; }
    ...
    if (alignment_mask <= size_mask) {
        ...
        if (addr_mask == UINT64_MAX) { return UINT64_MAX; }
