...
#define REQUIRE_CLONE_FLAG(flag)                                                           \
    const struct scmp_arg_cmp clone_arg##flag[] = {                                        \
        {.arg = CLONE_FLAGS_ARG, .op = SCMP_CMP_MASKED_EQ, .datum_a = flag, .datum_b = 0}}

#define FORBID_CLONE_FLAG(flag)                                                               \
    const struct scmp_arg_cmp clone_arg##flag[] = {                                           \
        {.arg = CLONE_FLAGS_ARG, .op = SCMP_CMP_MASKED_EQ, .datum_a = flag, .datum_b = flag}}

#define RULE_CLONE_FLAG(flag)                                                                                      \
    {SCMP_SYS(clone), QEMU_SECCOMP_SET_SPAWN, ARRAY_SIZE(clone_arg##flag), clone_arg##flag, SCMP_ACT_ERRNO(EPERM)}

/* If no CLONE_* flags are set, except CSIGNAL, deny */
const struct scmp_arg_cmp clone_arg_none[] = {
    {.arg = CLONE_FLAGS_ARG, .op = SCMP_CMP_MASKED_EQ, .datum_a = ~(CSIGNAL), .datum_b = 0}};
...
static const struct QemuSeccompSyscall denylist[] = {
    ...
    {SCMP_SYS(reboot), QEMU_SECCOMP_SET_DEFAULT, 0, NULL, SCMP_ACT_TRAP},
    ...
    {SCMP_SYS(syslog), QEMU_SECCOMP_SET_DEFAULT, 0, NULL, SCMP_ACT_TRAP},
    ...
    {SCMP_SYS(gtty), QEMU_SECCOMP_SET_DEFAULT, 0, NULL, SCMP_ACT_TRAP},
    {SCMP_SYS(lock), QEMU_SECCOMP_SET_DEFAULT, 0, NULL, SCMP_ACT_TRAP},
    {SCMP_SYS(mpx), QEMU_SECCOMP_SET_DEFAULT, 0, NULL, SCMP_ACT_TRAP},
    {SCMP_SYS(prof), QEMU_SECCOMP_SET_DEFAULT, 0, NULL, SCMP_ACT_TRAP},
    {SCMP_SYS(profil), QEMU_SECCOMP_SET_DEFAULT, 0, NULL, SCMP_ACT_TRAP},
    {SCMP_SYS(putpmsg), QEMU_SECCOMP_SET_DEFAULT, 0, NULL, SCMP_ACT_TRAP},
    ...
    {SCMP_SYS(tuxcall), QEMU_SECCOMP_SET_DEFAULT, 0, NULL, SCMP_ACT_TRAP},
    ...
    /* obsolete */
    {SCMP_SYS(readdir), QEMU_SECCOMP_SET_OBSOLETE, 0, NULL, SCMP_ACT_TRAP},
    ...
    {SCMP_SYS(get_kernel_syms), QEMU_SECCOMP_SET_OBSOLETE, 0, NULL, SCMP_ACT_TRAP},
    ...
    {SCMP_SYS(ustat), QEMU_SECCOMP_SET_OBSOLETE, 0, NULL, SCMP_ACT_TRAP},
    /* privileged */
    {SCMP_SYS(setuid), QEMU_SECCOMP_SET_PRIVILEGED, 0, NULL, SCMP_ACT_TRAP},
    ...
    /* spawn */
    {SCMP_SYS(fork), QEMU_SECCOMP_SET_SPAWN, 0, NULL, SCMP_ACT_ERRNO(EPERM)},
    {SCMP_SYS(vfork), QEMU_SECCOMP_SET_SPAWN, 0, NULL, SCMP_ACT_ERRNO(EPERM)},
    {SCMP_SYS(execve), QEMU_SECCOMP_SET_SPAWN, 0, NULL, SCMP_ACT_ERRNO(EPERM)},
    {SCMP_SYS(clone), QEMU_SECCOMP_SET_SPAWN, ARRAY_SIZE(clone_arg_none), clone_arg_none, SCMP_ACT_ERRNO(EPERM)},
...
#ifdef __SNR_clone3
    {SCMP_SYS(clone3), QEMU_SECCOMP_SET_SPAWN, 0, NULL, SCMP_ACT_ERRNO(ENOSYS)},
#endif
    ...
    /* resource control */
    {SCMP_SYS(setpriority), QEMU_SECCOMP_SET_RESOURCECTL, 0, NULL, SCMP_ACT_ERRNO(EPERM)},
    {SCMP_SYS(sched_setparam), QEMU_SECCOMP_SET_RESOURCECTL, 0, NULL, SCMP_ACT_ERRNO(EPERM)},
    {SCMP_SYS(sched_setscheduler), QEMU_SECCOMP_SET_RESOURCECTL, ARRAY_SIZE(sched_setscheduler_arg),
     sched_setscheduler_arg, SCMP_ACT_ERRNO(EPERM)},
    {SCMP_SYS(sched_setaffinity), QEMU_SECCOMP_SET_RESOURCECTL, 0, NULL, SCMP_ACT_ERRNO(EPERM)},
};

static inline __attribute__((unused)) int qemu_seccomp(unsigned int operation, unsigned int flags, void* args)
{
#ifdef __NR_seccomp
...
{
#if defined(SECCOMP_GET_ACTION_AVAIL) && defined(SCMP_ACT_KILL_PROCESS) && defined(SECCOMP_RET_KILL_PROCESS)
    if (action == SCMP_ACT_TRAP) {
        static int kill_process = -1;
        if (kill_process == -1) {
            uint32_t testaction = SECCOMP_RET_KILL_PROCESS;

            if (qemu_seccomp(SECCOMP_GET_ACTION_AVAIL, 0, &testaction) == 0) { kill_process = 1; }
            else {
                kill_process = 0;
            }
        }
        if (kill_process == 1) { return SCMP_ACT_KILL_PROCESS; }
    }
#endif
    return action;
}
...
static int seccomp_start(uint32_t seccomp_opts, Error** errp)
{
    ...
    if (rc != 0) {
        error_setg_errno(errp, -rc, "failed to set seccomp rawrc attribute");
        goto seccomp_return;
    }
#endif
    ...
    if (rc != 0) {
        error_setg_errno(errp, -rc, "failed to set seccomp thread synchronization");
        goto seccomp_return;
    }

    for (i = 0; i < ARRAY_SIZE(denylist); i++) {
        uint32_t action;
        if (!(seccomp_opts & denylist[i].set)) { continue; }

        action = qemu_seccomp_update_action(denylist[i].action);
        rc     = seccomp_rule_add_array(ctx, action, denylist[i].num, denylist[i].narg, denylist[i].arg_cmp);
        if (rc < 0) {
            error_setg_errno(errp, -rc, "failed to add seccomp denylist rules");
            goto seccomp_return;
        }
    }

    rc = seccomp_load(ctx);
    if (rc < 0) { error_setg_errno(errp, -rc, "failed to load seccomp syscall filter in kernel"); }
...
int parse_sandbox(void* opaque, QemuOpts* opts, Error** errp)
{
    if (qemu_opt_get_bool(opts, "enable", false)) {
        ...
        if (value) {
            if (g_str_equal(value, "allow")) { seccomp_opts &= ~QEMU_SECCOMP_SET_OBSOLETE; }
        ...
        if (value) {
            if (g_str_equal(value, "deny")) { seccomp_opts |= QEMU_SECCOMP_SET_PRIVILEGED; }
        ...
        if (value) {
            if (g_str_equal(value, "deny")) { seccomp_opts |= QEMU_SECCOMP_SET_SPAWN; }
            else if (g_str_equal(value, "allow")) {
                /* default value */
            }
            else {
        ...
        if (value) {
            if (g_str_equal(value, "deny")) { seccomp_opts |= QEMU_SECCOMP_SET_RESOURCECTL; }
            else if (g_str_equal(value, "allow")) {
                /* default value */
            }
            else {
        ...
        if (seccomp_start(seccomp_opts, errp) < 0) { return -1; }
    }
...
static void seccomp_register(void)
{
    ...
    check = qemu_seccomp(SECCOMP_SET_MODE_FILTER, SECCOMP_FILTER_FLAG_TSYNC, NULL);
    if (check < 0 && errno == EFAULT) { add = true; }
#endif

    if (add) { qemu_add_opts(&qemu_sandbox_opts); }
}
