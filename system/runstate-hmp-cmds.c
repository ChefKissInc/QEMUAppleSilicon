...
void hmp_one_insn_per_tb(Monitor* mon, const QDict* qdict)
{
    ...
    if (!object_property_find(OBJECT(accel), "one-insn-per-tb")) {
        monitor_printf(mon, "This accelerator does not support setting one-insn-per-tb\n");
        return;
    }

    if (!option || !strcmp(option, "on")) { newval = true; }
    ...
    object_property_set_bool(OBJECT(accel), "one-insn-per-tb", newval, &error_abort);
}
...
void watchdog_action_completion(ReadLineState* rs, int nb_args, const char* str)
{
    ...
    for (i = 0; i < WATCHDOG_ACTION__MAX; i++) { readline_add_completion_of(rs, str, WatchdogAction_str(i)); }
}
