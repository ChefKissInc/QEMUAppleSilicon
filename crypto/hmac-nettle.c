...
typedef void (*qcrypto_nettle_hmac_setkey)(void* ctx, size_t key_length, const uint8_t* key);

typedef void (*qcrypto_nettle_hmac_update)(void* ctx, size_t length, const uint8_t* data);

typedef void (*qcrypto_nettle_hmac_digest)(void* ctx, size_t length, uint8_t* digest);
...
{
    if (alg < G_N_ELEMENTS(qcrypto_hmac_alg_map) && qcrypto_hmac_alg_map[alg].setkey != NULL) { return true; }
...
void* qcrypto_hmac_ctx_new(QCryptoHashAlgo alg, const uint8_t* key, size_t nkey, Error** errp)
{
    ...
    if (!qcrypto_hmac_supports(alg)) {
        error_setg(errp, "Unsupported hmac algorithm %s", QCryptoHashAlgo_str(alg));
        return NULL;
    }
...
static void qcrypto_nettle_hmac_ctx_free(QCryptoHmac* hmac)
{
    QCryptoHmacNettle* ctx;
...
static int qcrypto_nettle_hmac_bytesv(QCryptoHmac* hmac, const struct iovec* iov, size_t niov, uint8_t** result,
                                      size_t* resultlen, Error** errp)
{
    ...
    if (resultlen == NULL) { return 0; }
    else if (*resultlen == 0) {
        *resultlen = qcrypto_hmac_alg_map[hmac->alg].len;
        *result    = g_new0(uint8_t, *resultlen);
    }
    else if (*resultlen != qcrypto_hmac_alg_map[hmac->alg].len) {
        error_setg(errp, "Result buffer size %zu is smaller than hash %zu", *resultlen,
                   qcrypto_hmac_alg_map[hmac->alg].len);
        return -1;
    }
