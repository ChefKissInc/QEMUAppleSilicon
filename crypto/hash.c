                        ...
                        Error** errp)
{
    ...
    if (qcrypto_hash_updatev(ctx, iov, niov, errp) < 0 || qcrypto_hash_finalize_bytes(ctx, result, resultlen, errp) < 0)
    {
        return -1;
    }
...
QCryptoHash* qcrypto_hash_new(QCryptoHashAlgo alg, Error** errp)
{
    ...
    if (!qcrypto_hash_supports(alg)) {
        error_setg(errp, "Unsupported hash algorithm %s", QCryptoHashAlgo_str(alg));
        return NULL;
    }
...
int qcrypto_hash_finalize_base64(QCryptoHash* hash, char** base64, Error** errp)
{
    ...
    ret = qcrypto_hash_finalize_bytes(hash, &result, &resultlen, errp);
    if (ret == 0) { *base64 = g_base64_encode(result, resultlen); }
...
int qcrypto_hash_digestv(QCryptoHashAlgo alg, const struct iovec* iov, size_t niov, char** digest, Error** errp)
{
    ...
    if (qcrypto_hash_updatev(ctx, iov, niov, errp) < 0 || qcrypto_hash_finalize_digest(ctx, digest, errp) < 0) {
        return -1;
    }
...
int qcrypto_hash_base64v(QCryptoHashAlgo alg, const struct iovec* iov, size_t niov, char** base64, Error** errp)
{
    ...
    if (qcrypto_hash_updatev(ctx, iov, niov, errp) < 0 || qcrypto_hash_finalize_base64(ctx, base64, errp) < 0) {
        return -1;
    }
