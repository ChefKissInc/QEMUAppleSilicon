...
{
#ifdef _WIN32
    if (!CryptAcquireContext(&hCryptProv, NULL, NULL, PROV_RSA_FULL, CRYPT_SILENT | CRYPT_VERIFYCONTEXT)) {
        error_setg_win32(errp, GetLastError(), "Unable to create cryptographic provider");
        return -1;
    }
#else
    #ifdef CONFIG_GETRANDOM
    ...
    if (fd == -1 && errno == ENOENT) { fd = open("/dev/random", O_RDONLY | O_CLOEXEC); }
    if (fd < 0) {
...
int qcrypto_random_bytes(void* buf, size_t buflen, Error** errp)
{
    ...
    if (!CryptGenRandom(hCryptProv, buflen, buf)) {
        error_setg_win32(errp, GetLastError(), "Unable to read random bytes");
        return -1;
    }
#else
    #ifdef CONFIG_GETRANDOM
