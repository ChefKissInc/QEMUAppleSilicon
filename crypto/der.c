...
#define QCRYPTO_DER_TAG(class, enc, val)                                                      \
    (((class) << QCRYPTO_DER_TAG_CLASS_SHIFT) | ((enc) << QCRYPTO_DER_TAG_ENC_SHIFT) | (val))
 ...
 */
static void qcrypto_der_encode_length(size_t src_len, uint8_t* dst, size_t* dst_len)
{
    ...
    if (src_len < QCRYPTO_DER_SHORT_LEN_MASK) {
        ...
        *dst_len    = 1;
        ...
        for (length_bytes = 1; max_length < src_len; length_bytes++) { max_length = (max_length << 8) + max_length; }
    ...
    *dst++ = header_byte;
    ...
    for (; length_bytes > 0; length_bytes--) { *dst++ = ((src_len >> (length_bytes - 1) * 8) & 0xFF); }
}

static uint8_t qcrypto_der_peek_byte(const uint8_t** data, size_t* dlen) { return **data; }

static void qcrypto_der_cut_nbytes(const uint8_t** data, size_t* dlen, size_t nbytes)
{
    *data += nbytes;
    *dlen -= nbytes;
}
...
static int qcrypto_der_invoke_callback(QCryptoDERDecodeCb cb, void* ctx, const uint8_t* value, size_t vlen,
                                       Error** errp)
{
    if (!cb) { return 0; }
...
static int qcrypto_der_extract_definite_data(const uint8_t** data, size_t* dlen, QCryptoDERDecodeCb cb, void* ctx,
                                             Error** errp)
{
    const uint8_t* value;
    size_t         vlen       = 0;
    ...
    if (!(byte_count & QCRYPTO_DER_SHORT_LEN_MASK)) {
        ...
        if (qcrypto_der_invoke_callback(cb, ctx, value, vlen, errp) != 0) { return -1; }
        return vlen;
    }
    ...
    if (byte_count > sizeof(size_t)) {
        error_setg(errp, "Invalid byte count of content length: %u", byte_count);
        return -1;
    }
...
static int qcrypto_der_extract_data(const uint8_t** data, size_t* dlen, QCryptoDERDecodeCb cb, void* ctx, Error** errp)
{
    uint8_t val;
    if (*dlen < 1) {
...
static int qcrypto_der_decode_tlv(const uint8_t expected_tag, const uint8_t** data, size_t* dlen, QCryptoDERDecodeCb cb,
                                  void* ctx, Error** errp)
{
    ...
    if (tag != expected_tag) {
        error_setg(errp, "Unexpected tag: expected: %u, actual: %u", expected_tag, tag);
        goto error;
    }

    data_length = qcrypto_der_extract_data(data, dlen, cb, ctx, errp);
    if (data_length < 0) { goto error; }
    return data_length;
...
int qcrypto_der_decode_int(const uint8_t** data, size_t* dlen, QCryptoDERDecodeCb cb, void* ctx, Error** errp)
{
    const uint8_t tag = QCRYPTO_DER_TAG(QCRYPTO_DER_TAG_CLASS_UNIV, QCRYPTO_DER_TAG_ENC_PRIM, QCRYPTO_DER_TYPE_TAG_INT);
    return qcrypto_der_decode_tlv(tag, data, dlen, cb, ctx, errp);
}

int qcrypto_der_decode_seq(const uint8_t** data, size_t* dlen, QCryptoDERDecodeCb cb, void* ctx, Error** errp)
{
    uint8_t tag = QCRYPTO_DER_TAG(QCRYPTO_DER_TAG_CLASS_UNIV, QCRYPTO_DER_TAG_ENC_CONS, QCRYPTO_DER_TYPE_TAG_SEQ);
    return qcrypto_der_decode_tlv(tag, data, dlen, cb, ctx, errp);
}

int qcrypto_der_decode_octet_str(const uint8_t** data, size_t* dlen, QCryptoDERDecodeCb cb, void* ctx, Error** errp)
{
    uint8_t tag = QCRYPTO_DER_TAG(QCRYPTO_DER_TAG_CLASS_UNIV, QCRYPTO_DER_TAG_ENC_PRIM, QCRYPTO_DER_TYPE_TAG_OCT_STR);
    return qcrypto_der_decode_tlv(tag, data, dlen, cb, ctx, errp);
}

int qcrypto_der_decode_bit_str(const uint8_t** data, size_t* dlen, QCryptoDERDecodeCb cb, void* ctx, Error** errp)
{
    uint8_t tag = QCRYPTO_DER_TAG(QCRYPTO_DER_TAG_CLASS_UNIV, QCRYPTO_DER_TAG_ENC_PRIM, QCRYPTO_DER_TYPE_TAG_BIT_STR);
    return qcrypto_der_decode_tlv(tag, data, dlen, cb, ctx, errp);
}

int qcrypto_der_decode_oid(const uint8_t** data, size_t* dlen, QCryptoDERDecodeCb cb, void* ctx, Error** errp)
{
    uint8_t tag = QCRYPTO_DER_TAG(QCRYPTO_DER_TAG_CLASS_UNIV, QCRYPTO_DER_TAG_ENC_PRIM, QCRYPTO_DER_TYPE_TAG_OID);
    return qcrypto_der_decode_tlv(tag, data, dlen, cb, ctx, errp);
}

int qcrypto_der_decode_ctx_tag(const uint8_t** data, size_t* dlen, int tag_id, QCryptoDERDecodeCb cb, void* ctx,
                               Error** errp)
{
    uint8_t tag = QCRYPTO_DER_TAG(QCRYPTO_DER_TAG_CLASS_CONT, QCRYPTO_DER_TAG_ENC_CONS, tag_id);
    return qcrypto_der_decode_tlv(tag, data, dlen, cb, ctx, errp);
}

static void qcrypto_der_encode_prim(QCryptoEncodeContext* ctx, uint8_t tag, const uint8_t* data, size_t dlen)
{
...
static void qcrypto_der_encode_cons_begin(QCryptoEncodeContext* ctx, uint8_t tag)
{
    QCryptoDerEncodeNode* node = g_new0(QCryptoDerEncodeNode, 1);
...
void qcrypto_der_encode_seq_end(QCryptoEncodeContext* ctx) { qcrypto_der_encode_cons_end(ctx); }
...
{
    uint8_t tag = QCRYPTO_DER_TAG(QCRYPTO_DER_TAG_CLASS_UNIV, QCRYPTO_DER_TAG_ENC_PRIM, QCRYPTO_DER_TYPE_TAG_INT);
    qcrypto_der_encode_prim(ctx, tag, src, src_len);
}

void qcrypto_der_encode_null(QCryptoEncodeContext* ctx)
{
    uint8_t tag = QCRYPTO_DER_TAG(QCRYPTO_DER_TAG_CLASS_UNIV, QCRYPTO_DER_TAG_ENC_PRIM, QCRYPTO_DER_TYPE_TAG_NULL);
...
size_t qcrypto_der_encode_ctx_buffer_len(QCryptoEncodeContext* ctx) { return ctx->root.dlen; }
...
void qcrypto_der_encode_ctx_flush_and_free(QCryptoEncodeContext* ctx, uint8_t* dst)
{
    ...
    for (prev = &ctx->root; (node = prev->next) && (prev->next = node->next, 1);) {
        /* Tag */
        *dst++ = node->tag;
