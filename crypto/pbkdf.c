...
static int qcrypto_pbkdf2_get_thread_cpu(unsigned long long* val_ms, Error** errp)
{
#ifdef _WIN32
    ...
    *val_ms = ((info.user_time.seconds * 1000ll) + (info.user_time.microseconds / 1000));
    return 0;
    ...
    *val_ms = ((ru.ru_utime.tv_sec * 1000ll) + (ru.ru_utime.tv_usec / 1000));
    return 0;
#else
    *val_ms = 0;
...
static void* threaded_qcrypto_pbkdf2_count_iters(void* data)
{
    ...
    while (1) {
        if (qcrypto_pbkdf2_get_thread_cpu(&start_ms, errp) < 0) { goto cleanup; }
        if (qcrypto_pbkdf2(hash, key, nkey, salt, nsalt, iterations, out, nout, errp) < 0) { goto cleanup; }
        if (qcrypto_pbkdf2_get_thread_cpu(&end_ms, errp) < 0) { goto cleanup; }
         ...
         * that point.
         */
        if (scaled > 5 && delta_ms == 0) { /* sanity check */
...
{
    CountItersData data = {hash, key, nkey, salt, nsalt, nout, 0, errp};
    QemuThread     thread;

    qemu_thread_create(&thread, "pbkdf2", threaded_qcrypto_pbkdf2_count_iters, &data, QEMU_THREAD_JOINABLE);
