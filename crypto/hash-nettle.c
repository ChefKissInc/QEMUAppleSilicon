...
typedef void (*qcrypto_nettle_write)(void* ctx, size_t len, const uint8_t* buf);
...
{
    if (alg < G_N_ELEMENTS(qcrypto_hash_alg_map) && qcrypto_hash_alg_map[alg].init != NULL) { return true; }
    return false;
}

static QCryptoHash* qcrypto_nettle_hash_new(QCryptoHashAlgo alg, Error** errp)
{
    QCryptoHash* hash;
...
static void qcrypto_nettle_hash_free(QCryptoHash* hash)
{
...
static int qcrypto_nettle_hash_update(QCryptoHash* hash, const struct iovec* iov, size_t niov, Error** errp)
{
    union qcrypto_hash_ctx* ctx = hash->opaque;

    for (int i = 0; i < niov; i++) { qcrypto_hash_alg_map[hash->alg].write(ctx, iov[i].iov_len, iov[i].iov_base); }

    return 0;
}

static int qcrypto_nettle_hash_finalize(QCryptoHash* hash, uint8_t** result, size_t* result_len, Error** errp)
{
    union qcrypto_hash_ctx* ctx = hash->opaque;
    ...
    if (*result_len == 0) {
        ...
        *result     = g_new(uint8_t, *result_len);
        ...
        error_setg(errp, "Result buffer size %zu is smaller than hash %d", *result_len, ret);
        return -1;
    }
