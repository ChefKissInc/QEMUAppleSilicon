...
static void hexdump(const char* buf, int len, void (*trace_fn)(size_t ofs, const char* text))
{
...
int gdb_put_packet_binary(const char* buf, int len, bool dump)
{
    ...
    for (;;) {
        ...
        if (gdb_got_immediate_ack()) { break; }
    }
    return 0;
}
    ...
    while (len--) {
        ...
        switch (c) {
            ...
            case '}':
                ...
                break;
            default: g_string_append_c(buf, c); break;
        }
    }
}
...
GDBProcess* gdb_get_process(uint32_t pid)
{
    ...
    for (i = 0; i < gdbserver_state.process_num; i++) {
        if (gdbserver_state.processes[i].pid == pid) { return &gdbserver_state.processes[i]; }
    }
...
static CPUState* find_cpu(uint32_t thread_id)
{
    ...
    CPU_FOREACH (cpu) {
        if (gdb_get_cpu_index(cpu) == thread_id) { return cpu; }
    }
...
CPUState* gdb_get_first_cpu_in_process(GDBProcess* process)
{
    ...
    CPU_FOREACH (cpu) {
        if (gdb_get_cpu_pid(cpu) == process->pid) { return cpu; }
    }
...
static CPUState* gdb_next_cpu_in_process(CPUState* cpu)
{
    ...
    while (cpu) {
        if (gdb_get_cpu_pid(cpu) == pid) { break; }
...
static CPUState* gdb_next_attached_cpu(CPUState* cpu)
{
    ...
    while (cpu) {
        if (gdb_get_cpu_process(cpu)->attached) { break; }
...
CPUState* gdb_first_attached_cpu(void)
{
    ...
    if (!process->attached) { return gdb_next_attached_cpu(cpu); }
...
static CPUState* gdb_get_cpu(uint32_t pid, uint32_t tid)
{
    ...
    if (!pid && !tid) {
        ...
        if (!process->attached) { return NULL; }
        ...
        if (pid && process->pid != pid) { return NULL; }
...
static const char* get_feature_xml(const char* p, const char** newp, GDBProcess* process)
{
    ...
    *newp            = term + 1;
    ...
    if (strncmp(p, "target.xml", len) == 0) {
        if (!process->target_xml) {
            ...
            if (cpu->cc->gdb_arch_name) {
                g_ptr_array_add(
                    xml, g_markup_printf_escaped("<architecture>%s</architecture>", cpu->cc->gdb_arch_name(cpu)));
            }
    ...
    for (guint i = 0; i < cpu->gdb_regs->len; i++) {
        r = &g_array_index(cpu->gdb_regs, GDBRegisterState, i);
        if (strncmp(p, r->feature->xmlname, len) == 0) { return r->feature->xml; }
    }
...
void gdb_feature_builder_init(GDBFeatureBuilder* builder, GDBFeature* feature, const char* name, const char* xmlname,
                              int base_reg)
{
    char* header = g_markup_printf_escaped("<?xml version=\"1.0\"?>"
...
{
    if (builder->regs->len <= regnum) { g_ptr_array_set_size(builder->regs, regnum + 1); }

    builder->regs->pdata[regnum] = (gpointer*)name;

    if (group) {
        gdb_feature_builder_append_tag(builder,
                                       "<reg name=\"%s\" bitsize=\"%d\" regnum=\"%d\" type=\"%s\" group=\"%s\"/>", name,
                                       bitsize, builder->base_reg + regnum, type, group);
    }
    else {
        gdb_feature_builder_append_tag(builder, "<reg name=\"%s\" bitsize=\"%d\" regnum=\"%d\" type=\"%s\"/>", name,
                                       bitsize, builder->base_reg + regnum, type);
    }
}
...
void gdb_feature_builder_end(const GDBFeatureBuilder* builder)
{
    ...
    for (guint i = 0; i < builder->xml->len - 2; i++) { g_free(g_ptr_array_index(builder->xml, i)); }
...
const GDBFeature* gdb_find_static_feature(const char* xmlname)
{
    ...
    for (feature = gdb_static_features; feature->xmlname; feature++) {
        if (!strcmp(feature->xmlname, xmlname)) { return feature; }
    }
...
GArray* gdb_get_register_list(CPUState* cpu)
{
    ...
    if (!cpu->gdb_regs) { return results; }
...
int gdb_read_register(CPUState* cpu, GByteArray* buf, int reg)
{
    ...
    if (reg < cpu->cc->gdb_num_core_regs) { return cpu->cc->gdb_read_register(cpu, buf, reg); }
...
int gdb_write_register(CPUState* cpu, uint8_t* mem_buf, int reg)
{
    ...
    if (reg < cpu->cc->gdb_num_core_regs) { return cpu->cc->gdb_write_register(cpu, mem_buf, reg); }
...
static const char* gdb_get_core_xml_file(CPUState* cpu)
{
     ...
     */
    if (cc->gdb_get_core_xml_file) { return cc->gdb_get_core_xml_file(cpu); }
...
void gdb_init_cpu(CPUState* cpu)
{
    ...
    if (cc->gdb_num_core_regs) { cpu->gdb_num_regs = cpu->gdb_num_g_regs = cc->gdb_num_core_regs; }
}
...
static GDBThreadIdKind read_thread_id(const char* buf, const char** end_buf, uint32_t* pid, uint32_t* tid)
{
    ...
    if (*buf == 'p') {
        ...
        if (ret) { return GDB_READ_THREAD_ERR; }
    ...
    *end_buf = buf;
    ...
    if (p == -1) { return GDB_ALL_PROCESSES; }

    if (pid) { *pid = p; }

    if (t == -1) { return GDB_ALL_THREADS; }
...
static int gdb_handle_vcont(const char* p)
{
    ...
    CPU_FOREACH (cpu) { newstates[cpu->cpu_index] = 1; }
    ...
    while (*p) {
        ...
        switch (kind) {
            case GDB_READ_THREAD_ERR: return -EINVAL;
            ...
            case GDB_ALL_THREADS:
                ...
                if (!process->attached) { return -EINVAL; }
     ...
     */
    if (target_count > 0) { gdbserver_state.c_cpu = last_target; }
...
static const char* cmd_next_param(const char* param, const char delimiter)
{
    ...
    if (delimiter == '?') { delimiters = all_delimiters; }
    else if (delimiter == '0') {
...
static int cmd_parse_params(const char* data, const char* schema, GArray* params)
{
    ...
    while (curr_schema[0] && curr_schema[1] && *curr_data) {
        ...
        switch (curr_schema[0]) {
            case 'l':
                if (qemu_strtoul(curr_data, &curr_data, 16, &this_param.val_ul)) { return -EINVAL; }
                curr_data = cmd_next_param(curr_data, curr_schema[1]);
                g_array_append_val(params, this_param);
                break;
            case 'L':
                if (qemu_strtou64(curr_data, &curr_data, 16, (uint64_t*)&this_param.val_ull)) { return -EINVAL; }
                curr_data = cmd_next_param(curr_data, curr_schema[1]);
                g_array_append_val(params, this_param);
                break;
            case 's':
            ...
            case 't':
                this_param.thread_id.kind =
                    read_thread_id(curr_data, &curr_data, &this_param.thread_id.pid, &this_param.thread_id.tid);
                curr_data = cmd_next_param(curr_data, curr_schema[1]);
                g_array_append_val(params, this_param);
                break;
            case '?': curr_data = cmd_next_param(curr_data, curr_schema[1]); break;
...
static bool process_string_cmd(const char* data, const GdbCmdParseEntry* cmds, int num_cmds)
{
    int i;
    ...
    for (i = 0; i < num_cmds; i++) {
        ...
        if ((cmd->cmd_startswith && !startswith(data, cmd->cmd)) || (!cmd->cmd_startswith && strcmp(cmd->cmd, data))) {
            continue;
        }

        if (cmd->schema) {
            if (cmd_parse_params(&data[strlen(cmd->cmd)], cmd->schema, params)) { return false; }
        }

        if (cmd->need_cpu_context) { user_ctx = (void*)gdbserver_state.g_cpu; }
...
static void run_cmd_parser(const char* data, const GdbCmdParseEntry* cmd)
{
    ...
    if (!process_string_cmd(data, cmd, 1)) { gdb_put_packet(""); }
}
...
static void handle_detach(GArray* params, void* user_ctx)
{
    ...
    if (pid == gdb_get_cpu_pid(gdbserver_state.c_cpu)) { gdbserver_state.c_cpu = gdb_first_attached_cpu(); }

    if (pid == gdb_get_cpu_pid(gdbserver_state.g_cpu)) { gdbserver_state.g_cpu = gdb_first_attached_cpu(); }
...
{
    if (params->len) { gdb_set_cpu_pc(gdb_get_cmd_param(params, 0)->val_ull); }
...
static void handle_cont_with_sig(GArray* params, void* user_ctx)
{
     ...
     */
    if (params->len) { signal = gdb_get_cmd_param(params, 0)->val_ul; }

    gdbserver_state.signal = gdb_signal_to_target(signal);
    if (gdbserver_state.signal == -1) { gdbserver_state.signal = 0; }
    gdb_continue();
}
...
static void handle_set_thread(GArray* params, void* user_ctx)
{
    ...
    switch (gdb_get_cmd_param(params, 0)->opcode) {
        ...
        case 'g':
            ...
            gdb_put_packet("OK");
            break;
        default: gdb_put_packet("E22"); break;
    }
}
...
static void handle_write_mem(GArray* params, void* user_ctx)
{
    ...
    if (gdb_get_cmd_param(params, 1)->val_ull > strlen(gdb_get_cmd_param(params, 2)->data) / 2) {
        gdb_put_packet("E22");
        return;
    }
...
static void handle_v_cont(GArray* params, void* user_ctx)
{
    ...
    if ((res == -EINVAL) || (res == -ERANGE)) { gdb_put_packet("E22"); }
    else if (res) {
        gdb_put_packet("");
    }
}
...
static const GdbCmdParseEntry gdb_v_commands_table[] = {
    ...
    {.handler = handle_v_cont_query, .cmd = "Cont?", .cmd_startswith = true},
    {.handler = handle_v_cont, .cmd = "Cont", .cmd_startswith = true, .allow_stop_reply = true, .schema = "s0"},
    {.handler = handle_v_attach, .cmd = "Attach;", .cmd_startswith = true, .allow_stop_reply = true, .schema = "l0"},
    {.handler = handle_v_kill, .cmd = "Kill;", .cmd_startswith = true},
};
...
static void handle_query_thread_extra(GArray* params, void* user_ctx)
{
    ...
    if (gdbserver_state.multiprocess && (gdbserver_state.process_num > 1)) {
        ...
        g_string_printf(rs, "%s %s [%s]", cpu_model, cpu_name, cpu->halted ? "halted " : "running");
    }
    else {
...
void gdb_extend_qsupported_features(char* qflags)
{
    ...
    else if (!g_strv_contains((const gchar* const*)extra_query_flags, qflags)) {
        int len                = g_strv_length(extra_query_flags);
        extra_query_flags      = g_realloc_n(extra_query_flags, len + 2, sizeof(char*));
...
static void handle_query_supported(GArray* params, void* user_ctx)
{
    ...
    if (gdb_get_core_xml_file(first_cpu)) { g_string_append(gdbserver_state.str_buf, ";qXfer:features:read+"); }

    if (params->len) {
        const char* gdb_supported = gdb_get_cmd_param(params, 0)->data;

        if (strstr(gdb_supported, "multiprocess+")) { gdbserver_state.multiprocess = true; }
    }
    ...
    if (extra_query_flags) {
        ...
        for (int i = 0; i < extras; i++) { g_string_append(gdbserver_state.str_buf, extra_query_flags[i]); }
    }
...
static void handle_query_xfer_features(GArray* params, void* user_ctx)
{
    ...
    if (len > (MAX_PACKET_LENGTH - 5) / 2) { len = (MAX_PACKET_LENGTH - 5) / 2; }
...
{
    if (!table) { table = g_ptr_array_new(); }

    for (int i = 0; i < extensions->len; i++) {
        gpointer entry = g_ptr_array_index(extensions, i);
        if (!g_ptr_array_find(table, entry, NULL)) { g_ptr_array_add(table, entry); }
    }
...
static bool process_extended_table(GPtrArray* table, const char* data)
{
    for (int i = 0; i < table->len; i++) {
        ...
        if (process_string_cmd(data, entry, 1)) { return true; }
    }
    return false;
}
...
static const GdbCmdParseEntry gdb_gen_query_table[] = {
    ...
    {.handler = handle_query_thread_extra, .cmd = "ThreadExtraInfo,", .cmd_startswith = true, .schema = "t0"},
    {.handler = gdb_handle_query_rcmd, .cmd = "Rcmd,", .cmd_startswith = true, .schema = "s0"},
    {.handler = handle_query_supported, .cmd = "Supported:", .cmd_startswith = true, .schema = "s0"},
    {.handler = handle_query_supported, .cmd = "Supported", .schema = "s0"},
    {.handler = handle_query_xfer_features, .cmd = "Xfer:features:read:", .cmd_startswith = true, .schema = "s:l,l0"},
    {.handler = gdb_handle_query_attached, .cmd = "Attached:", .cmd_startswith = true},
    {
...
static const GdbCmdParseEntry gdb_gen_set_table[] = {
    /* Order is important if has same prefix */
    {.handler = handle_set_qemu_sstep, .cmd = "qemu.sstep:", .cmd_startswith = true, .schema = "l0"},
    {.handler = gdb_handle_set_qemu_phy_mem_mode, .cmd = "qemu.PhyMemMode:", .cmd_startswith = true, .schema = "l0"},
};
...
static void handle_gen_query(GArray* params, void* user_ctx)
{
    ...
    if (process_string_cmd(data, gdb_gen_query_set_common_table, ARRAY_SIZE(gdb_gen_query_set_common_table))) {
        return;
    }

    if (process_string_cmd(data, gdb_gen_query_table, ARRAY_SIZE(gdb_gen_query_table))) { return; }

    if (extended_query_table && process_extended_table(extended_query_table, data)) { return; }
...
static void handle_gen_set(GArray* params, void* user_ctx)
{
    ...
    if (process_string_cmd(data, gdb_gen_set_table, ARRAY_SIZE(gdb_gen_set_table))) { return; }

    if (extended_set_table && process_extended_table(extended_set_table, data)) { return; }
...
static int gdb_handle_packet(const char* line_buf)
{
    ...
    switch (line_buf[0]) {
        ...
        case 'c': {
            static const GdbCmdParseEntry continue_cmd_desc = {.handler          = handle_continue,
                                                               .cmd              = "c",
                                                               .cmd_startswith   = true,
                                                               .allow_stop_reply = true,
                                                               .schema           = "L0"};
        ...
        case 'C': {
            static const GdbCmdParseEntry cont_with_sig_cmd_desc = {.handler          = handle_cont_with_sig,
                                                                    .cmd              = "C",
                                                                    .cmd_startswith   = true,
                                                                    .allow_stop_reply = true,
                                                                    .schema           = "l0"};
        ...
        case 'v': {
            static const GdbCmdParseEntry v_cmd_desc = {.handler        = handle_v_commands,
                                                        .cmd            = "v",
                                                        .cmd_startswith = true,
                                                        .schema         = "s0"};
        ...
        case 'k':
            ...
            break;
        case 'D': {
            static const GdbCmdParseEntry detach_cmd_desc = {.handler        = handle_detach,
                                                             .cmd            = "D",
                                                             .cmd_startswith = true,
                                                             .schema         = "?.l0"};
        ...
        case 's': {
            static const GdbCmdParseEntry step_cmd_desc = {.handler          = handle_step,
                                                           .cmd              = "s",
                                                           .cmd_startswith   = true,
                                                           .allow_stop_reply = true,
                                                           .schema           = "L0"};
        ...
        case 'b': {
            static const GdbCmdParseEntry backward_cmd_desc = {.handler          = handle_backward,
                                                               .cmd              = "b",
                                                               .cmd_startswith   = true,
                                                               .allow_stop_reply = true,
                                                               .schema           = "o0"};
        ...
        case 'G': {
            static const GdbCmdParseEntry write_all_regs_cmd_desc = {.handler        = handle_write_all_regs,
                                                                     .cmd            = "G",
                                                                     .cmd_startswith = true,
                                                                     .schema         = "s0"};
        ...
        case 'm': {
            static const GdbCmdParseEntry read_mem_cmd_desc = {.handler        = handle_read_mem,
                                                               .cmd            = "m",
                                                               .cmd_startswith = true,
                                                               .schema         = "L,L0"};
        ...
        case 'M': {
            static const GdbCmdParseEntry write_mem_cmd_desc = {.handler        = handle_write_mem,
                                                                .cmd            = "M",
                                                                .cmd_startswith = true,
                                                                .schema         = "L,L:s0"};
        ...
        case 'p': {
            static const GdbCmdParseEntry get_reg_cmd_desc = {.handler        = handle_get_reg,
                                                              .cmd            = "p",
                                                              .cmd_startswith = true,
                                                              .schema         = "L0"};
        ...
        case 'P': {
            static const GdbCmdParseEntry set_reg_cmd_desc = {.handler        = handle_set_reg,
                                                              .cmd            = "P",
                                                              .cmd_startswith = true,
                                                              .schema         = "L?s0"};
        ...
        case 'Z': {
            static const GdbCmdParseEntry insert_bp_cmd_desc = {.handler        = handle_insert_bp,
                                                                .cmd            = "Z",
                                                                .cmd_startswith = true,
                                                                .schema         = "l?L?L0"};
        ...
        case 'z': {
            static const GdbCmdParseEntry remove_bp_cmd_desc = {.handler        = handle_remove_bp,
                                                                .cmd            = "z",
                                                                .cmd_startswith = true,
                                                                .schema         = "l?L?L0"};
        ...
        case 'H': {
            static const GdbCmdParseEntry set_thread_cmd_desc = {.handler        = handle_set_thread,
                                                                 .cmd            = "H",
                                                                 .cmd_startswith = true,
                                                                 .schema         = "o.t0"};
        ...
        case 'T': {
            static const GdbCmdParseEntry thread_alive_cmd_desc = {.handler        = handle_thread_alive,
                                                                   .cmd            = "T",
                                                                   .cmd_startswith = true,
                                                                   .schema         = "t0"};
        ...
        case 'q': {
            static const GdbCmdParseEntry gen_query_cmd_desc = {.handler        = handle_gen_query,
                                                                .cmd            = "q",
                                                                .cmd_startswith = true,
                                                                .schema         = "s0"};
        ...
        case 'Q': {
            static const GdbCmdParseEntry gen_set_cmd_desc = {.handler        = handle_gen_set,
                                                              .cmd            = "Q",
                                                              .cmd_startswith = true,
                                                              .schema         = "s0"};
    ...
    if (cmd_parser) { run_cmd_parser(line_buf, cmd_parser); }
...
void gdb_read_byte(uint8_t ch)
{
    ...
    if (gdbserver_state.last_packet->len) {
        ...
        if (ch == '+' || ch == '$') { g_byte_array_set_size(gdbserver_state.last_packet, 0); }
        if (ch != '$') { return; }
    }
...
void gdb_create_default_process(GDBState* s)
{
    ...
    if (gdbserver_state.process_num) { pid = s->processes[s->process_num - 1].pid; }
    else {
        pid = 0;
    }
