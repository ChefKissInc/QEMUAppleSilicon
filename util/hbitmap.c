...
static unsigned long hbitmap_iter_skip_words(HBitmapIter* hbi)
{
    ...
    if (i == 0 && cur == (1UL << (BITS_PER_LONG - 1))) { return 0; }
...
{
    unsigned long cur = hbi->cur[HBITMAP_LEVELS - 1] & hbi->hb->levels[HBITMAP_LEVELS - 1][hbi->pos];
    int64_t       item;
...
void hbitmap_iter_init(HBitmapIter* hbi, const HBitmap* hb, uint64_t first)
{
    ...
    for (i = HBITMAP_LEVELS; i-- > 0;) {
         ...
         */
        if (i != HBITMAP_LEVELS - 1) { hbi->cur[i] &= ~(1UL << bit); }
    }
}
...
int64_t hbitmap_next_dirty(const HBitmap* hb, int64_t start, int64_t count)
{
    ...
    if (start >= hb->orig_size || count == 0) { return -1; }
    ...
    if (first_dirty_off < 0 || first_dirty_off >= end) { return -1; }
...
int64_t hbitmap_next_zero(const HBitmap* hb, int64_t start, int64_t count)
{
    ...
    end_bit = count > hb->orig_size - start ? hb->size : ((start + count - 1) >> hb->granularity) + 1;
...
bool hbitmap_empty(const HBitmap* hb) { return hb->count == 0; }

int hbitmap_granularity(const HBitmap* hb) { return hb->granularity; }

uint64_t hbitmap_count(const HBitmap* hb) { return hb->count << hb->granularity; }
...
static uint64_t hb_count_between(HBitmap* hb, uint64_t start, uint64_t last)
{
    ...
    for (;;) {
        ...
        if (pos >= (end >> BITS_PER_LEVEL)) { break; }
        count += ctpopl(cur);
    }
...
static bool hb_set_between(HBitmap* hb, int level, uint64_t start, uint64_t last)
{
     ...
     * the one above.
     */
    if (level > 0 && changed) { hb_set_between(hb, level - 1, pos, lastpos); }
    return changed;
}
...
void hbitmap_set(HBitmap* hb, uint64_t start, uint64_t count)
{
    ...
    trace_hbitmap_set(hb, start, count, start >> hb->granularity, last >> hb->granularity);
    ...
    if (hb_set_between(hb, HBITMAP_LEVELS - 1, first, last) && hb->meta) { hbitmap_set(hb->meta, start, count); }
}
...
/* The recursive workhorse (the depth is limited to HBITMAP_LEVELS)...
 * Returns true if at least one bit is changed. */
static bool hb_reset_between(HBitmap* hb, int level, uint64_t start, uint64_t last)
{
    size_t pos     = start >> BITS_PER_LEVEL;
    size_t lastpos = last >> BITS_PER_LEVEL;
    bool   changed = false;
    ...
    if (i < lastpos) {
         ...
         */
        if (hb_reset_elem(&hb->levels[level][i], start, next - 1)) { changed = true; }
        else {
            pos++;
        }
    ...
    if (hb_reset_elem(&hb->levels[level][i], start, last)) { changed = true; }
    else {
        lastpos--;
    }

    if (level > 0 && changed) { hb_reset_between(hb, level - 1, pos, lastpos); }
...
void hbitmap_reset(HBitmap* hb, uint64_t start, uint64_t count)
{
    ...
    trace_hbitmap_reset(hb, start, count, start >> hb->granularity, last >> hb->granularity);
    ...
    if (hb_reset_between(hb, HBITMAP_LEVELS - 1, first, last) && hb->meta) { hbitmap_set(hb->meta, start, count); }
}
...
void hbitmap_reset_all(HBitmap* hb)
{
    ...
    for (i = HBITMAP_LEVELS; --i >= 1;) { memset(hb->levels[i], 0, hb->sizes[i] * sizeof(unsigned long)); }
 ...
 */
static void serialization_chunk(const HBitmap* hb, uint64_t start, uint64_t count, unsigned long** first_el,
                                uint64_t* el_count)
{
    uint64_t last = start + count - 1;
    ...
    if ((last >> hb->granularity) != hb->size - 1) { assert((count & (gran - 1)) == 0); }
...
void hbitmap_serialize_part(const HBitmap* hb, uint8_t* buf, uint64_t start, uint64_t count)
{
    ...
    while (cur != end) {
        unsigned long el = (BITS_PER_LONG == 32 ? cpu_to_le32(*cur) : cpu_to_le64(*cur));
...
void hbitmap_deserialize_part(HBitmap* hb, uint8_t* buf, uint64_t start, uint64_t count, bool finish)
{
    ...
    while (cur != end) {
        ...
        if (BITS_PER_LONG == 32) { le32_to_cpus((uint32_t*)cur); }
        else {
    ...
    if (finish) { hbitmap_deserialize_finish(hb); }
}
    ...
    for (lev = HBITMAP_LEVELS - 1; lev-- > 0;) {
        ...
        for (i = 0; i < prev_size; ++i) {
            if (bitmap->levels[lev + 1][i]) {
                bitmap->levels[lev][i >> BITS_PER_LEVEL] |= 1UL << (i & (BITS_PER_LONG - 1));
            }
        }
    }
...
void hbitmap_free(HBitmap* hb)
{
    unsigned i;
    assert(!hb->meta);
    for (i = HBITMAP_LEVELS; i-- > 0;) { g_free(hb->levels[i]); }
    g_free(hb);
}
...
void hbitmap_truncate(HBitmap* hb, uint64_t size)
{
    ...
    for (i = HBITMAP_LEVELS; i-- > 0;) {
        ...
        if (hb->sizes[i] == size) { break; }
        old           = hb->sizes[i];
        hb->sizes[i]  = size;
        hb->levels[i] = g_renew(unsigned long, hb->levels[i], size);
        if (!shrink) { memset(&hb->levels[i][old], 0x00, (size - old) * sizeof(*hb->levels[i])); }
    }
    if (hb->meta) { hbitmap_truncate(hb->meta, hb->size << hb->granularity); }
}
...
static void hbitmap_sparse_merge(HBitmap* dst, const HBitmap* src)
{
    ...
    for (offset = 0; hbitmap_next_dirty_area(src, offset, src->orig_size, INT64_MAX, &offset, &count); offset += count)
    {
...
void hbitmap_merge(const HBitmap* a, const HBitmap* b, HBitmap* result)
{
    ...
    if ((!hbitmap_count(a) && result == b) || (!hbitmap_count(b) && result == a)) { return; }
    ...
    if (a->granularity != b->granularity) {
        if ((a != result) && (b != result)) { hbitmap_reset_all(result); }
        if (a != result) { hbitmap_sparse_merge(result, a); }
        if (b != result) { hbitmap_sparse_merge(result, b); }
        return;
    }
    ...
    for (i = HBITMAP_LEVELS - 1; i >= 0; i--) {
        for (j = 0; j < a->sizes[i]; j++) { result->levels[i][j] = a->levels[i][j] | b->levels[i][j]; }
    }
