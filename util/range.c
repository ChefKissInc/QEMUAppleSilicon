...
int range_compare(Range* a, Range* b)
{
    ...
    if (b->lob && b->lob - 1 > a->upb) { return -1; }
    if (a->lob && a->lob - 1 > b->upb) { return 1; }
    return 0;
}
...
static inline GList* append_new_range(GList* list, uint64_t lob, uint64_t upb)
{
...
void range_inverse_array(GList* in, GList** rev, uint64_t low, uint64_t high)
{
    ...
    for (l = in; l && range_upb(l->data) < low; l = l->next) { continue; }
    ...
    if (range_lob(r) > low) { out = append_new_range(out, low, MIN(range_lob(r) - 1, high)); }

    /* insert a range in between each original range until we reach high */
    for (; l->next; l = l->next) {
        r  = (Range*)l->data;
        rn = (Range*)l->next->data;
        if (range_lob(r) >= high) { goto exit; }
        if (range_compare(r, rn)) { out = append_new_range(out, range_upb(r) + 1, MIN(range_lob(rn) - 1, high)); }
    }
    ...
    if (range_upb(r) < high) { out = append_new_range(out, range_upb(r) + 1, high); }
exit:
    *rev = out;
}
