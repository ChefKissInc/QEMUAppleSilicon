...
void qemu_uuid_generate(QemuUUID* uuid)
{
    ...
    for (i = 0; i < 4; ++i) { tmp[i] = g_random_int(); }
...
int qemu_uuid_is_equal(const QemuUUID* lhv, const QemuUUID* rhv) { return memcmp(lhv, rhv, sizeof(QemuUUID)) == 0; }

void qemu_uuid_unparse(const QemuUUID* uuid, char* out)
{
    const unsigned char* uu = &uuid->data[0];
    snprintf(out, UUID_STR_LEN, UUID_FMT, uu[0], uu[1], uu[2], uu[3], uu[4], uu[5], uu[6], uu[7], uu[8], uu[9], uu[10],
...
{
    const unsigned char* uu = &uuid->data[0];
    return g_strdup_printf(UUID_FMT, uu[0], uu[1], uu[2], uu[3], uu[4], uu[5], uu[6], uu[7], uu[8], uu[9], uu[10],
                           uu[11], uu[12], uu[13], uu[14], uu[15]);
}
...
int qemu_uuid_parse(const char* str, QemuUUID* uuid)
{
    ...
    if (!qemu_uuid_is_valid(str)) { return -1; }

    ret = sscanf(str, UUID_FMT, &uu[0], &uu[1], &uu[2], &uu[3], &uu[4], &uu[5], &uu[6], &uu[7], &uu[8], &uu[9], &uu[10],
                 &uu[11], &uu[12], &uu[13], &uu[14], &uu[15]);
...
uint32_t qemu_uuid_hash(const void* uuid)
{
    ...
    for (i = 0; i < ARRAY_SIZE(qid->data); i++) { h = (h << 5) + h + qid->data[i]; }
