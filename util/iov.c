...
size_t iov_size(const struct iovec* iov, const unsigned int iov_cnt)
{
    ...
    len = 0;
    for (i = 0; i < iov_cnt; i++) { len += iov[i].iov_len; }
    return len;
}

/* helper function for iov_send_recv() */
static ssize_t do_send_recv(int sockfd, int flags, struct iovec* iov, unsigned iov_cnt, bool do_send)
{
#ifdef CONFIG_POSIX
    ssize_t       ret;
    struct msghdr msg;
    ...
    do {
        ret = do_send ? sendmsg(sockfd, &msg, flags) : recvmsg(sockfd, &msg, flags);
    }
    ...
    ssize_t  off = 0;
    while (i < iov_cnt) {
        ssize_t r = do_send ? send(sockfd, iov[i].iov_base + off, iov[i].iov_len - off, flags) :
...
ssize_t iov_send_recv(int sockfd, const struct iovec* _iov, unsigned iov_cnt, size_t offset, size_t bytes, bool do_send)
{ return iov_send_recv_with_flags(sockfd, 0, _iov, iov_cnt, offset, bytes, do_send); }

ssize_t iov_send_recv_with_flags(int sockfd, int sockflags, const struct iovec* _iov, unsigned iov_cnt, size_t offset,
                                 size_t bytes, bool do_send)
{
    ...
    while (bytes > 0) {
        ...
        for (niov = 0; niov < iov_cnt && offset >= iov[niov].iov_len; ++niov) { offset -= iov[niov].iov_len; }
        ...
        tail = bytes;
        for (niov = 0; niov < iov_cnt && iov[niov].iov_len <= tail; ++niov) { tail -= iov[niov].iov_len; }
        if (tail) {
        ...
        if (ret < 0) {
            assert(errno != EINTR);
            g_free(local_iov);
            if (errno == EAGAIN && total > 0) { return total; }
            return -1;
        }
...
void iov_hexdump(const struct iovec* iov, const unsigned int iov_cnt, FILE* fp, const char* prefix, size_t limit)
{
    ...
    for (v = 0; v < iov_cnt; v++) { size += iov[v].iov_len; }
...
unsigned iov_copy(struct iovec* dst_iov, unsigned int dst_iov_cnt, const struct iovec* iov, unsigned int iov_cnt,
...
void qemu_iovec_init_external(QEMUIOVector* qiov, struct iovec* iov, int niov)
{
    ...
    qiov->nalloc = -1;
    qiov->size   = 0;
    for (i = 0; i < niov; i++) { qiov->size += iov[i].iov_len; }
}
 ...
 */
size_t qemu_iovec_concat_iov(QEMUIOVector* dst, struct iovec* src_iov, unsigned int src_cnt, size_t soffset,
                             size_t sbytes)
{
    int    i;
    size_t done;
 ...
 */
static struct iovec* iov_skip_offset(struct iovec* iov, size_t offset, size_t* remaining_offset)
{
 ...
 */
struct iovec* qemu_iovec_slice(QEMUIOVector* qiov, size_t offset, size_t len, size_t* head, size_t* tail, int* niov)
{
...
bool qemu_iovec_is_zero(QEMUIOVector* qiov, size_t offset, size_t bytes)
{
    ...
    while (bytes) {
        ...
        if (!buffer_is_zero(base, len)) { return false; }
...
void qemu_iovec_init_slice(QEMUIOVector* qiov, QEMUIOVector* source, size_t offset, size_t len)
{
    ...
    slice_iov = qemu_iovec_slice(source, offset, len, &slice_head, &slice_tail, &slice_niov);
    if (slice_niov == 1) { qemu_iovec_init_buf(qiov, slice_iov[0].iov_base + slice_head, len); }
    else {
...
{
    if (qiov->nalloc != -1) { g_free(qiov->iov); }
...
ssize_t qemu_iovec_compare(QEMUIOVector* a, QEMUIOVector* b)
{
    ...
    for (i = 0; i < a->niov; i++) {
        ...
        while (len < a->iov[i].iov_len && *p++ == *q++) { len++; }

        offset += len;

        if (len != a->iov[i].iov_len) { return offset; }
    }
    return -1;
}
...
static int sortelem_cmp_src_base(const void* a, const void* b)
{
    ...
    /* Don't overflow */
    if (elem_a->src_iov->iov_base < elem_b->src_iov->iov_base) { return -1; }
...
void qemu_iovec_clone(QEMUIOVector* dest, const QEMUIOVector* src, void* buf)
{
    ...
    for (i = 0; i < src->niov; i++) {
        ...
        /* Detect overlap */
        if (last_end && last_end > cur->iov_base) { rewind = last_end - cur->iov_base; }
    ...
    for (i = 0; i < src->niov; i++) { qemu_iovec_add(dest, sortelems[i].dest_base, src->iov[i].iov_len); }
}

void iov_discard_undo(IOVDiscardUndo* undo)
{
    /* Restore original iovec if it was modified */
    if (undo->modified_iov) { *undo->modified_iov = undo->orig; }
}

size_t iov_discard_front_undoable(struct iovec** iov, unsigned int* iov_cnt, size_t bytes, IOVDiscardUndo* undo)
{
    size_t        total = 0;
    struct iovec* cur;

    if (undo) { undo->modified_iov = NULL; }
...
size_t iov_discard_back_undoable(struct iovec* iov, unsigned int* iov_cnt, size_t bytes, IOVDiscardUndo* undo)
{
    size_t        total = 0;
    struct iovec* cur;
