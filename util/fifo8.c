...
void fifo8_destroy(Fifo8* fifo) { g_free(fifo->data); }
...
void fifo8_push_all(Fifo8* fifo, const uint8_t* data, uint32_t num)
{
    ...
    if (start + num <= fifo->capacity) { memcpy(&fifo->data[start], data, num); }
    else {
...
static const uint8_t* fifo8_peekpop_bufptr(Fifo8* fifo, uint32_t max, uint32_t skip, uint32_t* numptr, bool do_pop)
{
    uint8_t* ret;
    uint32_t num, head;
...
static uint32_t fifo8_peekpop_buf(Fifo8* fifo, uint8_t* dest, uint32_t destlen, bool do_pop)
{
    const uint8_t* buf;
    uint32_t       n1, n2 = 0;
    uint32_t       len;
...
bool fifo8_is_empty(const Fifo8* fifo) { return (fifo->num == 0); }

bool fifo8_is_full(const Fifo8* fifo) { return (fifo->num == fifo->capacity); }

uint32_t fifo8_num_free(const Fifo8* fifo) { return fifo->capacity - fifo->num; }

uint32_t fifo8_num_used(const Fifo8* fifo) { return fifo->num; }
