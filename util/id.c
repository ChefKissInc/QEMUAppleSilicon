...
bool id_wellformed(const char* id)
{
    ...
    if (!qemu_isalpha(id[0])) { return false; }
    for (i = 1; id[i]; i++) {
        if (!qemu_isalnum(id[i]) && !strchr("-._", id[i])) { return false; }
    }
    return true;
}
...
char* id_generate(IdSubSystems id)
{
    ...
    return g_strdup_printf("%c%s%" PRIu64 "%02" PRId32, ID_SPECIAL_CHAR, id_subsys_str[id], id_counters[id]++, rnd);
}
