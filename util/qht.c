...
{
    if (ht->mode & QHT_MODE_RAW_MUTEXES) { qemu_mutex_lock__raw(&ht->lock); }
    else {
...
{
    if (ht->mode & QHT_MODE_RAW_MUTEXES) { return qemu_mutex_trylock__raw(&(ht)->lock); }
    return qemu_mutex_trylock(&(ht)->lock);
}

/* this inline is not really necessary, but it helps keep code consistent */
static inline void qht_unlock(struct qht* ht) { qemu_mutex_unlock(&ht->lock); }
...
static void qht_map_debug__all_locked(struct qht_map* map)
{
    ...
    for (i = 0; i < map->n_buckets; i++) { qht_bucket_debug__locked(&map->buckets[i]); }
}
#else
...
static inline size_t qht_elems_to_buckets(size_t n_elems) { return pow2ceil(n_elems / QHT_BUCKET_ENTRIES); }
 ...
 */
static inline void qht_do_if_first_in_stripe(struct qht_map* map, struct qht_bucket* b, void (*func)(QemuSpin* spin))
{
#ifdef CONFIG_TSAN
...
static inline void qht_bucket_lock_do(struct qht_map* map, struct qht_bucket* b, void (*func)(QemuSpin* lock))
{
#ifdef CONFIG_TSAN
    unsigned long bucket_idx = b - map->buckets;
    unsigned long lock_idx   = bucket_idx & (QHT_TSAN_BUCKET_LOCKS - 1);
...
static inline void qht_bucket_lock(struct qht_map* map, struct qht_bucket* b)
{ qht_bucket_lock_do(map, b, qemu_spin_lock); }

static inline void qht_bucket_unlock(struct qht_map* map, struct qht_bucket* b)
...
static inline struct qht_bucket* qht_map_to_bucket(const struct qht_map* map, uint32_t hash)
 ...
 */
static inline bool qht_map_is_stale__locked(const struct qht* ht, const struct qht_map* map) { return map != ht->map; }
 ...
 */
static inline void qht_map_lock_buckets__no_stale(struct qht* ht, struct qht_map** pmap)
{
    struct qht_map* map;
 ...
 * Note: callers cannot have ht->lock held.
 */
static inline struct qht_bucket* qht_bucket_lock__no_stale(struct qht* ht, uint32_t hash, struct qht_map** pmap)
{
    struct qht_bucket* b;
    struct qht_map*    map;
...
{ return qatomic_read(&map->n_added_buckets) > map->n_added_buckets_threshold; }

static inline void qht_chain_destroy(struct qht_map* map, struct qht_bucket* head)
{
...
static void qht_map_destroy(struct qht_map* map)
{
    ...
    for (i = 0; i < map->n_buckets; i++) { qht_chain_destroy(map, &map->buckets[i]); }
...
static struct qht_map* qht_map_create(size_t n_buckets)
{
    ...
    map->n_added_buckets_threshold = n_buckets / QHT_NR_ADDED_BUCKETS_THRESHOLD_DIV;

    /* let tiny hash tables to at least add one non-head bucket */
    if (unlikely(map->n_added_buckets_threshold == 0)) { map->n_added_buckets_threshold = 1; }

    map->buckets = qemu_memalign(QHT_BUCKET_ALIGN, sizeof(*map->buckets) * n_buckets);
    for (i = 0; i < n_buckets; i++) { qht_head_init(map, &map->buckets[i]); }
    return map;
}
...
static void qht_bucket_reset__locked(struct qht_bucket* head)
{
    ...
    do {
        for (i = 0; i < QHT_BUCKET_ENTRIES; i++) {
            if (b->pointers[i] == NULL) { goto done; }
...
static void qht_map_reset__all_locked(struct qht_map* map)
{
    ...
    for (i = 0; i < map->n_buckets; i++) { qht_bucket_reset__locked(&map->buckets[i]); }
...
static inline void qht_do_resize(struct qht* ht, struct qht_map* new) { qht_do_resize_reset(ht, new, false); }

static inline void qht_do_resize_and_reset(struct qht* ht, struct qht_map* new) { qht_do_resize_reset(ht, new, true); }
...
bool qht_reset_size(struct qht* ht, size_t n_elems)
{
    ...
    qht_lock(ht);
    map = ht->map;
    if (n_buckets != map->n_buckets) { new = qht_map_create(n_buckets); }
...
static inline void* qht_do_lookup(const struct qht_bucket* head, qht_lookup_func_t func, const void* userp,
                                  uint32_t hash)
{
    ...
    do {
        for (i = 0; i < QHT_BUCKET_ENTRIES; i++) {
            if (qatomic_read(&b->hashes[i]) == hash) {
                ...
                if (likely(p) && likely(func(p, userp))) { return p; }
            }
        }
...
static __attribute__((noinline)) void* qht_lookup__slowpath(const struct qht_bucket* b, qht_lookup_func_t func,
...
void* qht_lookup_custom(const struct qht* ht, const void* userp, uint32_t hash, qht_lookup_func_t func)
{
    ...
    version = seqlock_read_begin(&b->sequence);
    ret     = qht_do_lookup(b, func, userp, hash);
    if (likely(!seqlock_read_retry(&b->sequence, version))) { return ret; }
    /*
 ...
 */
static void* qht_insert__locked(const struct qht* ht, struct qht_map* map, struct qht_bucket* head, void* p,
                                uint32_t hash, bool* needs_resize)
{
    struct qht_bucket* b    = head;
    ...
    do {
        for (i = 0; i < QHT_BUCKET_ENTRIES; i++) {
            if (b->pointers[i]) {
                if (unlikely(b->hashes[i] == hash && ht->cmp(b->pointers[i], p))) { return b->pointers[i]; }
            }
            else {
                goto found;
            }
    ...
    if (unlikely(qht_map_needs_resize(map)) && needs_resize) { *needs_resize = true; }

found:
    /* found an empty key: acquire the seqlock and write */
    seqlock_write_begin(&head->sequence);
    if (new) { qatomic_rcu_set(&prev->next, b); }
...
bool qht_insert(struct qht* ht, void* p, uint32_t hash, void** existing)
{
    ...
    if (unlikely(needs_resize) && ht->mode & QHT_MODE_AUTO_RESIZE) { qht_grow_maybe(ht); }
    if (likely(prev == NULL)) { return true; }
    if (existing) { *existing = prev; }
    return false;
}
...
static void qht_entry_move(struct qht_bucket* to, int i, struct qht_bucket* from, int j)
{
...
static inline void qht_bucket_remove_entry(struct qht_bucket* orig, int pos)
{
    ...
    do {
        for (i = 0; i < QHT_BUCKET_ENTRIES; i++) {
            if (b->pointers[i]) { continue; }
            if (i > 0) { return qht_entry_move(orig, pos, b, i - 1); }
...
static inline bool qht_remove__locked(struct qht_bucket* head, const void* p, uint32_t hash)
{
    struct qht_bucket* b = head;
    int                i;
    ...
    do {
        for (i = 0; i < QHT_BUCKET_ENTRIES; i++) {
            ...
            if (unlikely(q == NULL)) { return false; }
            if (q == p) {
...
static inline void qht_bucket_iter(struct qht_bucket* head, const struct qht_iter* iter, void* userp)
{
    struct qht_bucket* b = head;
    int                i;
    ...
    do {
        for (i = 0; i < QHT_BUCKET_ENTRIES; i++) {
            if (b->pointers[i] == NULL) { return; }
            switch (iter->type) {
                case QHT_ITER_VOID: iter->f.retvoid(b->pointers[i], b->hashes[i], userp); break;
                case QHT_ITER_RM:
...
static inline void qht_map_iter__all_locked(struct qht_map* map, const struct qht_iter* iter, void* userp)
{
    size_t i;

    for (i = 0; i < map->n_buckets; i++) { qht_bucket_iter(&map->buckets[i], iter, userp); }
}

static inline void do_qht_iter(struct qht* ht, const struct qht_iter* iter, void* userp)
{
    struct qht_map* map;
...
static void qht_do_resize_reset(struct qht* ht, struct qht_map* new, bool reset)
{
    ...
    if (reset) { qht_map_reset__all_locked(old); }
...
void qht_statistics_init(const struct qht* ht, struct qht_stats* stats)
{
    ...
    for (i = 0; i < map->n_buckets; i++) {
        ...
        do {
            ...
            do {
                for (j = 0; j < QHT_BUCKET_ENTRIES; j++) {
                    if (qatomic_read(&b->pointers[j]) == NULL) { break; }
                    entries++;
                }
                buckets++;
                b = qatomic_rcu_read(&b->next);
        ...
        if (entries) {
            ...
            qdist_inc(&stats->occupancy, (double)entries / QHT_BUCKET_ENTRIES / buckets);
