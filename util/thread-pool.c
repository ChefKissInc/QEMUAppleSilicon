...
static void* worker_thread(void* opaque)
{
    ...
    while (pool->cur_threads <= pool->max_threads) {
        ...
        if (QTAILQ_EMPTY(&pool->request_list)) {
            ...
            pool->idle_threads--;
            if (ret == 0 && QTAILQ_EMPTY(&pool->request_list) && pool->cur_threads > pool->min_threads) {
...
static void spawn_thread(ThreadPoolAio* pool)
{
     ...
     */
    if (!pool->pending_threads) { qemu_bh_schedule(pool->new_thread_bh); }
}
...
restart:
    QLIST_FOREACH_SAFE (elem, &pool->head, all, next) {
        if (elem->state != THREAD_DONE) { continue; }

        trace_thread_pool_complete_aio(pool, elem, elem->common.opaque, elem->ret);
...
BlockAIOCB* thread_pool_submit_aio(ThreadPoolFunc* func, void* arg, BlockCompletionFunc* cb, void* opaque)
{
    ThreadPoolElementAio* req;
    ...
    qemu_mutex_lock(&pool->lock);
    if (pool->idle_threads == 0 && pool->cur_threads < pool->max_threads) { spawn_thread(pool); }
...
void thread_pool_update_params(ThreadPoolAio* pool, AioContext* ctx)
{
     ...
     */
    for (int i = pool->cur_threads; i < pool->min_threads; i++) { spawn_thread(pool); }

    for (int i = pool->cur_threads; i > pool->max_threads; i--) { qemu_cond_signal(&pool->request_cond); }
...
void thread_pool_free_aio(ThreadPoolAio* pool)
{
    ...
    while (pool->cur_threads > 0) { qemu_cond_wait(&pool->worker_stopped, &pool->lock); }
...
static void thread_pool_func(gpointer data, gpointer user_data)
{
    ...
    if (el->opaque_destroy) { el->opaque_destroy(el->opaque); }
    ...
    if (pool->cur_work == 0) { qemu_cond_signal(&pool->all_finished_cond); }
}
...
void thread_pool_submit(ThreadPool* pool, ThreadPoolFunc* func, void* opaque, GDestroyNotify opaque_destroy)
{
    ...
    WITH_QEMU_LOCK_GUARD(&pool->cur_work_lock) { pool->cur_work++; }
...
void thread_pool_submit_immediate(ThreadPool* pool, ThreadPoolFunc* func, void* opaque, GDestroyNotify opaque_destroy)
{
...
void thread_pool_wait(ThreadPool* pool)
{
    ...
    while (pool->cur_work > 0) { qemu_cond_wait(&pool->all_finished_cond, &pool->cur_work_lock); }
}
