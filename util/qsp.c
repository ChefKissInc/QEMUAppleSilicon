...
QemuRecMutexTrylockFunc qemu_rec_mutex_trylock_func = qemu_rec_mutex_trylock_impl;
 ...
 */
static inline uint32_t do_qsp_callsite_hash(const QSPCallSite* callsite, uint64_t ab)
{
...
static inline uint32_t qsp_callsite_hash(const QSPCallSite* callsite) { return do_qsp_callsite_hash(callsite, 0); }
...
static uint32_t qsp_entry_no_thread_hash(const QSPEntry* entry) { return do_qsp_entry_hash(entry, 0); }
...
static bool qsp_callsite_cmp(const void* ap, const void* bp)
{
    ...
    return a == b
           || (a->obj == b->obj && a->line == b->line && a->type == b->type
...
static bool qsp_callsite_no_obj_cmp(const void* ap, const void* bp)
{
    ...
    return a == b || (a->line == b->line && a->type == b->type && (a->file == b->file || !strcmp(a->file, b->file)));
}
...
static bool qsp_entry_cmp(const void* ap, const void* bp)
{
    ...
    return a->thread_ptr == b->thread_ptr && qsp_callsite_cmp(a->callsite, b->callsite);
}
...
static void qsp_do_init(void)
{
    ...
    qht_init(&qsp_ht, qsp_entry_cmp, QSP_INITIAL_SIZE, QHT_MODE_AUTO_RESIZE | QHT_MODE_RAW_MUTEXES);
    qht_init(&qsp_callsite_ht, qsp_callsite_cmp, QSP_INITIAL_SIZE, QHT_MODE_AUTO_RESIZE | QHT_MODE_RAW_MUTEXES);
}
...
static __attribute__((noinline)) void qsp_init__slowpath(void)
{
    if (qatomic_cmpxchg(&qsp_initializing, false, true) == false) {
        ...
        while (!qatomic_read(&qsp_initialized)) { cpu_relax(); }
    }
}
...
{
    if (likely(qatomic_read(&qsp_initialized))) { return; }
    qsp_init__slowpath();
}
...
static QSPEntry* qsp_entry_create(struct qht* ht, const QSPEntry* entry, uint32_t hash)
{
    QSPEntry* e;
    void*     existing = NULL;
...
static QSPEntry* qsp_entry_find(struct qht* ht, const QSPEntry* entry, uint32_t hash)
{
    QSPEntry* e;

    e = qht_lookup(ht, entry, hash);
    if (e == NULL) { e = qsp_entry_create(ht, entry, hash); }
    return e;
}
 ...
 */
static QSPEntry* qsp_entry_get(const void* obj, const char* file, int line, enum QSPType type)
{
    QSPCallSite callsite = {
        .obj  = obj,
        .file = file,
...
static inline void do_qsp_entry_record(QSPEntry* e, int64_t delta, bool acq)
{
    ...
    if (acq) { qatomic_set_u64(&e->n_acqs, e->n_acqs + 1); }
}

static inline void qsp_entry_record(QSPEntry* e, int64_t delta) { do_qsp_entry_record(e, delta, true); }
...
QSP_GEN_VOID(QemuRecMutex, QSP_REC_MUTEX, qsp_rec_mutex_lock, qemu_rec_mutex_lock_impl)
QSP_GEN_RET1(QemuRecMutex, QSP_REC_MUTEX, qsp_rec_mutex_trylock, qemu_rec_mutex_trylock_impl)

#undef QSP_GEN_RET1
#undef QSP_GEN_VOID

static void qsp_cond_wait(QemuCond* cond, QemuMutex* mutex, const char* file, int line)
{
    QSPEntry* e;
    int64_t   t0, t1;
...
static bool qsp_cond_timedwait(QemuCond* cond, QemuMutex* mutex, int ms, const char* file, int line)
{
    QSPEntry* e;
    int64_t   t0, t1;
    bool      ret;
...
bool qsp_is_enabled(void) { return qatomic_read(&qemu_mutex_lock_func) == qsp_mutex_lock; }
...
static void qsp_diff(struct qht* orig, struct qht* new) { qht_iter(orig, qsp_iter_diff, new); }
...
static void qsp_ht_delete(void* p, uint32_t h, void* htp) { g_free(p); }
...
static void qsp_mktree(GTree* tree, bool callsite_coalesce)
{
    ...
    WITH_RCU_READ_LOCK_GUARD()
    {
        ...
        qht_init(&ht, qsp_entry_no_thread_cmp, QSP_INITIAL_SIZE, QHT_MODE_AUTO_RESIZE | QHT_MODE_RAW_MUTEXES);
        qht_iter(&qsp_ht, qsp_aggregate, &ht);

        /* compute the difference wrt the snapshot, if any */
        if (snap) { qsp_diff(&snap->ht, &ht); }
    }
...
static char* qsp_at(const QSPCallSite* callsite)
{
    ...
    if (unlikely(strlen(callsite->file) < qsp_qemu_path_len)) { shortened = callsite->file; }
    else {
...
static gboolean qsp_tree_report(gpointer key, gpointer value, gpointer udata)
{
    ...
    if (report->n_entries == report->max_n_entries) { return TRUE; }
...
static void pr_report(const QSPReport* rep)
{
    ...
    for (i = 0; i < rep->n_entries; i++) {
        ...
        if (e->n_objs > 1) { g_string_append_printf(s, "[%12u]", e->n_objs); }
        else {
            g_string_append_printf(s, "%14p", e->obj);
        }
        g_string_append_printf(s, "  %s%*s  %13.5f  %12" PRIu64 "  %12.2f\n", e->callsite_at,
                               callsite_len - (int)strlen(e->callsite_at), "", e->time_s, e->n_acqs, e->ns_avg * 1e-3);
        qemu_printf("%s", s->str);
...
void qsp_reset(void)
{
    ...
    qht_init(&new->ht, qsp_entry_cmp, QSP_INITIAL_SIZE, QHT_MODE_AUTO_RESIZE | QHT_MODE_RAW_MUTEXES);
    ...
    if (old) { call_rcu(old, qsp_snapshot_destroy, rcu); }
}
