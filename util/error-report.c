...
static void print_loc(void)
{
    ...
    switch (cur_loc->kind) {
        ...
        case LOC_FILE:
            ...
            if (cur_loc->num) { error_printf("%d:", cur_loc->num); }
            error_printf(" ");
            break;
        default: error_printf("%s", sep);
    }
}
...
static void vreport(report_type type, const char* fmt, va_list ap)
{
    ...
    if (error_with_guestname && error_guest_name && !monitor_cur()) { error_printf("%s ", error_guest_name); }

    print_loc();

    switch (type) {
        case REPORT_TYPE_ERROR  : break;
        case REPORT_TYPE_WARNING: error_printf("warning: "); break;
        case REPORT_TYPE_INFO   : error_printf("info: "); break;
    }
 ...
 */
void error_vreport(const char* fmt, va_list ap) { vreport(REPORT_TYPE_ERROR, fmt, ap); }
 ...
 * Prepend the current location and append a newline.
 */
void warn_vreport(const char* fmt, va_list ap) { vreport(REPORT_TYPE_WARNING, fmt, ap); }
 ...
 * Prepend the current location and append a newline.
 */
void info_vreport(const char* fmt, va_list ap) { vreport(REPORT_TYPE_INFO, fmt, ap); }
...
static void qemu_log_func(const gchar* log_domain, GLogLevelFlags log_level, const gchar* message, gpointer user_data)
{
    switch (log_level & G_LOG_LEVEL_MASK) {
        ...
        case G_LOG_LEVEL_INFO:
             ...
             * messages
             */
            if (qemu_glog_domains == NULL) { break; }
            if (strcmp(qemu_glog_domains, "all") != 0 && (log_domain == NULL || !strstr(qemu_glog_domains, log_domain)))
            {
                break;
            }
            /* Fall through */
        case G_LOG_LEVEL_MESSAGE : info_report("%s%s%s", log_domain ?: "", log_domain ? ": " : "", message); break;
        case G_LOG_LEVEL_WARNING : warn_report("%s%s%s", log_domain ?: "", log_domain ? ": " : "", message); break;
        case G_LOG_LEVEL_CRITICAL:
        case G_LOG_LEVEL_ERROR   : error_report("%s%s%s", log_domain ?: "", log_domain ? ": " : "", message); break;
    }
}
