...
static inline void stat64_rdlock(Stat64* s)
{
    ...
    while (qatomic_read(&s->lock) & 1) { cpu_relax(); }
}

static inline void stat64_rdunlock(Stat64* s) { qatomic_sub(&s->lock, 2); }

static inline bool stat64_wrtrylock(Stat64* s) { return qatomic_cmpxchg(&s->lock, 0, 1) == 0; }

static inline void stat64_wrunlock(Stat64* s) { qatomic_dec(&s->lock); }
...
{
    while (!stat64_wrtrylock(s)) { cpu_relax(); }
