...
{
    unsigned left = CPUINFO_ZBA | CPUINFO_ZBB | CPUINFO_ZBS | CPUINFO_ZICOND | CPUINFO_ZVE64X;
    ...
    info |= CPUINFO_ZICOND;
#endif
#if defined(__riscv_arch_test) && (defined(__riscv_vector) || defined(__riscv_zve64x))
    info |= CPUINFO_ZVE64X;
#endif
    left &= ~info;
    ...
    if (left) {
        ...
        if (syscall(__NR_riscv_hwprobe, &pair, 1, 0, NULL, 0) == 0 && pair.key >= 0) {
    ...
    if (left) {
        ...
        if (left & CPUINFO_ZBS) {
            ...
            got_sigill = 0;
            asm volatile(".insn r 0x33, 5, 0x24, zero, zero, zero" : : : "memory");
