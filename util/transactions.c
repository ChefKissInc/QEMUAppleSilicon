...
void tran_add(Transaction* tran, TransactionActionDrv* drv, void* opaque)
{
    ...
    *act = (TransactionAction){.drv = drv, .opaque = opaque};
...
void tran_abort(Transaction* tran)
{
    ...
    QSLIST_FOREACH (act, &tran->actions, entry) {
        if (act->drv->abort) { act->drv->abort(act->opaque); }
    }

    QSLIST_FOREACH_SAFE (act, &tran->actions, entry, next) {
        if (act->drv->clean) { act->drv->clean(act->opaque); }
...
void tran_commit(Transaction* tran)
{
    ...
    QSLIST_FOREACH (act, &tran->actions, entry) {
        if (act->drv->commit) { act->drv->commit(act->opaque); }
    }
