    ...
    while (true) {
        ...
        switch (tc->thread_cmd) {
            ...
            case TC_CMD_NEW: {
                ...
                qemu_thread_create(cmd_new->thread, cmd_new->name, cmd_new->start_routine, cmd_new->arg, cmd_new->mode);
                tc->thread_cmd      = TC_CMD_NONE;
...
static void thread_context_set_cpu_affinity(Object* obj, Visitor* v, const char* name, void* opaque, Error** errp)
{
    ...
    if (!visit_type_uint16List(v, name, &host_cpus, errp)) { return; }
    ...
    for (l = host_cpus; l; l = l->next) { nbits = MAX(nbits, l->value + 1); }
    bitmap = bitmap_new(nbits);
    for (l = host_cpus; l; l = l->next) { set_bit(l->value, bitmap); }
    ...
    if (tc->thread_id != -1) {
        ...
        if (ret) { error_setg(errp, "Setting CPU affinity failed: %s", strerror(ret)); }
    }
    else {
...
static void thread_context_get_cpu_affinity(Object* obj, Visitor* v, const char* name, void* opaque, Error** errp)
{
...
static void thread_context_get_thread_id(Object* obj, Visitor* v, const char* name, void* opaque, Error** errp)
{
    ThreadContext* tc    = THREAD_CONTEXT(obj);
...
static void thread_context_instance_complete(UserCreatable* uc, Error** errp)
{
    ...
    thread_name = g_strdup_printf("TC %s", object_get_canonical_path_component(OBJECT(uc)));
    qemu_thread_create(&tc->thread, thread_name, thread_context_run, tc, QEMU_THREAD_JOINABLE);
    g_free(thread_name);

    /* Wait until initialization of the thread is done. */
    while (tc->thread_id == -1) { qemu_sem_wait(&tc->sem); }

    if (tc->init_cpu_bitmap) {
        ret = qemu_thread_set_affinity(&tc->thread, tc->init_cpu_bitmap, tc->init_cpu_nbits);
        if (ret) { error_setg(errp, "Setting CPU affinity failed: %s", strerror(ret)); }
...
static void thread_context_class_init(ObjectClass* oc, const void* data)
{
    ...
    object_class_property_add(oc, "thread-id", "int", thread_context_get_thread_id, NULL, NULL, NULL);
    object_class_property_add(oc, "cpu-affinity", "int", thread_context_get_cpu_affinity,
                                                                           ...
                                                                           int mode)
{
    ...
    while (tc->thread_cmd != TC_CMD_NONE) { qemu_sem_wait(&tc->sem); }
