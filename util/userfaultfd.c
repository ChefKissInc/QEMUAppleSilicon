...
int uffd_open(int flags)
{
    ...
    if (open_mode == UFFD_UNINITIALIZED) {
        ...
        if (uffd_dev >= 0) { open_mode = UFFD_USE_DEV_PATH; }
        else {
 ...
 */
int uffd_register_memory(int uffd_fd, void* addr, uint64_t length, uint64_t mode, uint64_t* ioctls)
{
    ...
    if (ioctls) { *ioctls = uffd_register.ioctls; }
 ...
 */
int uffd_change_protection(int uffd_fd, void* addr, uint64_t length, bool wp, bool dont_wake)
{
    ...
    if (ioctl(uffd_fd, UFFDIO_WRITEPROTECT, &uffd_writeprotect)) {
        error_report("uffd_change_protection() failed: addr=%p len=%" PRIu64 " mode=%" PRIx64 " errno=%i", addr, length,
 ...
 */
int uffd_copy_page(int uffd_fd, void* dst_addr, void* src_addr, uint64_t length, bool dont_wake)
{
    ...
    if (ioctl(uffd_fd, UFFDIO_COPY, &uffd_copy)) {
        int e = errno;
        error_report("uffd_copy_page() failed: dst_addr=%p src_addr=%p length=%" PRIu64 " mode=%" PRIx64 " errno=%i",
                     dst_addr, src_addr, length, (uint64_t)uffd_copy.mode, e);
        return -e;
    }
...
int uffd_zero_page(int uffd_fd, void* addr, uint64_t length, bool dont_wake)
{
    ...
    if (ioctl(uffd_fd, UFFDIO_ZEROPAGE, &uffd_zeropage)) {
        int e = errno;
        error_report("uffd_zero_page() failed: addr=%p length=%" PRIu64 " mode=%" PRIx64 " errno=%i", addr, length,
...
int uffd_wakeup(int uffd_fd, void* addr, uint64_t length)
{
    ...
    if (ioctl(uffd_fd, UFFDIO_WAKE, &uffd_range)) {
        int e = errno;
        error_report("uffd_wakeup() failed: addr=%p length=%" PRIu64 " errno=%i", addr, length, e);
        return -e;
    }
