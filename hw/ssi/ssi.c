...
DeviceState* ssi_get_cs(SSIBus* bus, uint8_t cs_index)
{
    ...
    QTAILQ_FOREACH (kid, &b->children, sibling) {
        ...
        if (kid_ssi->cs_index == cs_index) { return kid->child; }
    }
...
static bool ssi_bus_check_address(BusState* b, DeviceState* dev, Error** errp)
{
    ...
    if (ssi_get_cs(SSI_BUS(b), s->cs_index)) {
        error_setg(errp, "CS index '0x%x' in use by a %s device", s->cs_index, object_get_typename(OBJECT(dev)));
        return false;
    }
...
static void ssi_cs_default(void* opaque, int n, int level)
{
    ...
    assert(n == 0);
    if (s->cs != cs) {
        if (s->spc->set_cs) { s->spc->set_cs(s, cs); }
    }
    s->cs = cs;
}
...
static uint32_t ssi_transfer_raw_default(SSIPeripheral* dev, uint32_t val)
{
    ...
    if ((dev->cs && ssc->cs_polarity == SSI_CS_HIGH) || (!dev->cs && ssc->cs_polarity == SSI_CS_LOW)
...
static void ssi_peripheral_realize(DeviceState* dev, Error** errp)
{
    ...
    if (ssc->transfer_raw == ssi_transfer_raw_default && ssc->cs_polarity != SSI_CS_NONE) {
...
static void ssi_peripheral_class_init(ObjectClass* klass, const void* data)
{
    ...
    if (!ssc->transfer_raw) { ssc->transfer_raw = ssi_transfer_raw_default; }
