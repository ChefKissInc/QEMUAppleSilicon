...
static const AppleUartReg apple_uart_regs[] = {
    {"ULCON", ULCON, 0x00000000},   {"UCON", UCON, 0x00000000},         {"UFCON", UFCON, 0x00000000},
    {"UMCON", UMCON, 0x00000000},   {"UTRSTAT", UTRSTAT, 0x00000006},   {"UERSTAT", UERSTAT, 0x00000000}, /* RO */
    ...
    {"UBRDIV", UBRDIV, 0x00000000}, {"UFRACVAL", UFRACVAL, 0x00000000},
};
...
static const char* apple_uart_regname(hwaddr offset)
{
    ...
    for (i = 0; i < ARRAY_SIZE(apple_uart_regs); i++) {
        if (offset == apple_uart_regs[i].offset) { return apple_uart_regs[i].name; }
    }
...
static void apple_uart_update_irq(AppleUartState* s)
{
    ...
    if (s->reg[I_(UFCON)] & UFCON_FIFO_ENABLE) {
        ...
        if (s->reg[I_(UCON)] & UCON_TXTHRESH_ENA) {
            mask |= UTRSTAT_Tx_THRESH | UTRSTAT_Tx_EMPTY | UTRSTAT_Tx_BUFFER_EMPTY;
            if (count <= apple_uart_Tx_FIFO_trigger_level(s) && !(s->last_irq & UTRSTAT_Tx_THRESH)) {
        ...
        if (s->reg[I_(UCON)] & UCON_RXTIMEOUT_ENA) { mask |= UTRSTAT_Rx_TIMEOUT; }
    }
...
static void apple_uart_timeout_int(void* opaque)
{
    ...
    if ((s->reg[I_(UTRSTAT)] & UTRSTAT_Rx_BUFFER_DATA_READY) || (s->reg[I_(UCON)] & UCON_RXTIMEOUT_ENA)) {
...
static void apple_uart_update_parameters(AppleUartState* s)
{
    ...
    trace_apple_uart_update_params(s->channel, speed, parity, data_bits, stop_bits, s->wordtime);
}
...
static void apple_uart_write(void* opaque, hwaddr offset, uint64_t val, unsigned size)
{
    AppleUartState* s = opaque;
    uint8_t         ch;
    ...
    switch (offset) {
        ...
        case UFSTAT:
        case UMSTAT:
        case URXH  : trace_apple_uart_ro_write(s->channel, apple_uart_regname(offset), offset); break;
        case UCON  : apple_uart_update_irq(s); break;
        case UMCON :
...
static uint64_t apple_uart_read(void* opaque, hwaddr offset, unsigned size)
{
    ...
    switch (offset) {
        case UERSTAT: /* Read Only */
            ...
            s->reg[I_(UERSTAT)] = 0;
            trace_apple_uart_read(s->channel, offset, apple_uart_regname(offset), res);
            return res;
        case UFSTAT: /* Read Only */
            s->reg[I_(UFSTAT)] = fifo8_num_used(&s->rx) & 0xf;
            if (fifo8_num_free(&s->rx) == 0) { s->reg[I_(UFSTAT)] |= UFSTAT_Rx_FIFO_FULL; }
            trace_apple_uart_read(s->channel, offset, apple_uart_regname(offset), s->reg[I_(UFSTAT)]);
        ...
        case URXH:
            if (s->reg[I_(UFCON)] & UFCON_FIFO_ENABLE) {
                if (fifo8_num_used(&s->rx)) {
                    ...
                    if (fifo8_is_empty(&s->rx)) { s->reg[I_(UTRSTAT)] &= ~UTRSTAT_Rx_BUFFER_DATA_READY; }
                    else {
            ...
            trace_apple_uart_read(s->channel, offset, apple_uart_regname(offset), res);
            return res;
        case UTXH: trace_apple_uart_wo_read(s->channel, apple_uart_regname(offset), offset); break;
        default:
            trace_apple_uart_read(s->channel, offset, apple_uart_regname(offset), s->reg[I_(offset)]);
...
static int apple_uart_can_receive(void* opaque)
{
    ...
    if (s->reg[I_(UFCON)] & UFCON_FIFO_ENABLE) { return fifo8_num_free(&s->rx); }
    else {
...
static void apple_uart_init(Object* obj)
{
    ...
    /* memory mapping */
    memory_region_init_io(&s->iomem, obj, &apple_uart_ops, s, TYPE_APPLE_UART, APPLE_UART_REGS_MEM_SIZE);
...
static void apple_uart_register(void) { type_register_static(&apple_uart_info); }
