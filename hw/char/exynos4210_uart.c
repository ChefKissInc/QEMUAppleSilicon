...
static const Exynos4210UartReg exynos4210_uart_regs[] = {
    {"ULCON", ULCON, 0x00000000},     {"UCON", UCON, 0x00003000},         {"UFCON", UFCON, 0x00000000},
    {"UMCON", UMCON, 0x00000000},     {"UTRSTAT", UTRSTAT, 0x00000006}, /* RO */
    ...
    {"UBRDIV", UBRDIV, 0x00000000},   {"UFRACVAL", UFRACVAL, 0x00000000}, {"UINTP", UINTP, 0x00000000},
    {"UINTSP", UINTSP, 0x00000000},   {"UINTM", UINTM, 0x00000000},
};
...
static const char* exynos4210_uart_regname(hwaddr offset)
{
    ...
    for (i = 0; i < ARRAY_SIZE(exynos4210_uart_regs); i++) {
        if (offset == exynos4210_uart_regs[i].offset) { return exynos4210_uart_regs[i].name; }
    }
...
{
    if (q->sp < q->rp) { return q->size - q->rp + q->sp; }

    return q->sp - q->rp;
}

static int fifo_empty_elements_number(const Exynos4210UartFIFO* q) { return q->size - fifo_elements_number(q); }
...
static uint32_t exynos4210_uart_FIFO_trigger_level(uint32_t channel, uint32_t reg)
{
    uint32_t level;

    switch (channel) {
        case 0: level = reg * 32; break;
        case 1:
        case 4: level = reg * 8; break;
        case 2:
        case 3: level = reg * 2; break;
        default:
            level = 0;
...
static uint32_t exynos4210_uart_Tx_FIFO_trigger_level(const Exynos4210UartState* s)
{
    ...
    reg = (s->reg[I_(UFCON)] & UFCON_Tx_FIFO_TRIGGER_LEVEL) >> UFCON_Tx_FIFO_TRIGGER_LEVEL_SHIFT;

    return exynos4210_uart_FIFO_trigger_level(s->channel, reg);
}

static uint32_t exynos4210_uart_Rx_FIFO_trigger_level(const Exynos4210UartState* s)
{
    uint32_t reg;

    reg = ((s->reg[I_(UFCON)] & UFCON_Rx_FIFO_TRIGGER_LEVEL) >> UFCON_Rx_FIFO_TRIGGER_LEVEL_SHIFT) + 1;
...
static void exynos4210_uart_update_irq(Exynos4210UartState* s)
{
    ...
    if (s->reg[I_(UFCON)] & UFCON_FIFO_ENABLE) {
        uint32_t count = (s->reg[I_(UFSTAT)] & UFSTAT_Tx_FIFO_COUNT) >> UFSTAT_Tx_FIFO_COUNT_SHIFT;

        if (count <= exynos4210_uart_Tx_FIFO_trigger_level(s)) { s->reg[I_(UINTSP)] |= UINTSP_TXD; }
        ...
        if ((count && !(s->reg[I_(UCON)] & 0x80)) || count >= exynos4210_uart_Rx_FIFO_trigger_level(s)) {
...
static void exynos4210_uart_timeout_int(void* opaque)
{
    ...
    trace_exynos_uart_rx_timeout(s->channel, s->reg[I_(UTRSTAT)], s->reg[I_(UINTSP)]);

    if ((s->reg[I_(UTRSTAT)] & UTRSTAT_Rx_BUFFER_DATA_READY) || (s->reg[I_(UCON)] & (1 << 11))) {
        s->reg[I_(UINTSP)]  |= UINTSP_RXD;
...
static void exynos4210_uart_update_parameters(Exynos4210UartState* s)
{
    ...
    if (s->reg[I_(UBRDIV)] == 0) { return; }

    if (s->reg[I_(ULCON)] & 0x20) {
        if (s->reg[I_(ULCON)] & 0x28) { parity = 'E'; }
        else {
            parity = 'O';
        }
    }
    ...
    if (s->reg[I_(ULCON)] & 0x4) { stop_bits = 2; }
    else {
        stop_bits = 1;
    }
    ...
    speed = uclk_rate / ((16 * (s->reg[I_(UBRDIV)]) & 0xffff) + (s->reg[I_(UFRACVAL)] & 0x7) + 16);
    ...
    trace_exynos_uart_update_params(s->channel, speed, parity, data_bits, stop_bits, s->wordtime);
}
...
static void exynos4210_uart_rx_timeout_set(Exynos4210UartState* s)
{
    if (s->reg[I_(UCON)] & 0x80) {
        ...
        timer_mod(s->fifo_timeout_timer, qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL) + timeout);
    }
    else {
        timer_del(s->fifo_timeout_timer);
    }
...
static void exynos4210_uart_write(void* opaque, hwaddr offset, uint64_t val, unsigned size)
{
...
static uint64_t exynos4210_uart_read(void* opaque, hwaddr offset, unsigned size)
{
    Exynos4210UartState* s = (Exynos4210UartState*)opaque;
    uint32_t             res;
    ...
    switch (offset) {
        case UERSTAT: /* Read Only */
            ...
            s->reg[I_(UERSTAT)] = 0;
            trace_exynos_uart_read(s->channel, offset, exynos4210_uart_regname(offset), res);
            return res;
        case UFSTAT: /* Read Only */
            ...
            trace_exynos_uart_read(s->channel, offset, exynos4210_uart_regname(offset), s->reg[I_(UFSTAT)]);
        ...
        case URXH:
            if (s->reg[I_(UFCON)] & UFCON_FIFO_ENABLE) {
                if (fifo_elements_number(&s->rx)) {
                    ...
                    if (!fifo_elements_number(&s->rx)) { s->reg[I_(UTRSTAT)] &= ~UTRSTAT_Rx_BUFFER_DATA_READY; }
                    else {
            ...
            trace_exynos_uart_read(s->channel, offset, exynos4210_uart_regname(offset), res);
            return res;
        case UTXH: trace_exynos_uart_wo_read(s->channel, exynos4210_uart_regname(offset), offset); break;
        default:
            trace_exynos_uart_read(s->channel, offset, exynos4210_uart_regname(offset), s->reg[I_(offset)]);
            return s->reg[I_(offset)];
    }

    trace_exynos_uart_read(s->channel, offset, exynos4210_uart_regname(offset), 0);
    return 0;
}
...
static int exynos4210_uart_can_receive(void* opaque)
{
    ...
    if (s->reg[I_(UFCON)] & UFCON_FIFO_ENABLE) { return fifo_empty_elements_number(&s->rx); }
    else {
...
static void exynos4210_uart_receive(void* opaque, const uint8_t* buf, int size)
{
    ...
    if (s->reg[I_(UFCON)] & UFCON_FIFO_ENABLE) {
        ...
        for (i = 0; i < size; i++) { fifo_store(&s->rx, buf[i]); }
...
static void exynos4210_uart_reset(DeviceState* dev)
{
    ...
    for (i = 0; i < ARRAY_SIZE(exynos4210_uart_regs); i++) {
        s->reg[I_(exynos4210_uart_regs[i].offset)] = exynos4210_uart_regs[i].reset_value;
    }
...
DeviceState* exynos4210_uart_create(hwaddr addr, int fifo_size, int channel, Chardev* chr, qemu_irq irq)
{
    ...
    if (addr != (hwaddr)-1) { sysbus_mmio_map(bus, 0, addr); }
...
static void exynos4210_uart_init(Object* obj)
{
    ...
    /* memory mapping */
    memory_region_init_io(&s->iomem, obj, &exynos4210_uart_ops, s, "exynos4210.uart", EXYNOS4210_UART_REGS_MEM_SIZE);
...
static void exynos4210_uart_realize(DeviceState* dev, Error** errp)
{
    ...
    s->fifo_timeout_timer = timer_new_ns(QEMU_CLOCK_VIRTUAL, exynos4210_uart_timeout_int, s);

    qemu_chr_fe_set_handlers(&s->chr, exynos4210_uart_can_receive, exynos4210_uart_receive, exynos4210_uart_event, NULL,
                             s, NULL, true);
}
...
static void exynos4210_uart_register(void) { type_register_static(&exynos4210_uart_info); }
