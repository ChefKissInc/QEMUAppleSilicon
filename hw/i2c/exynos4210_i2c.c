...
#define I2CSTAT_START_BUSY      (1 << 5)
    ...
    #define DPRINT(fmt, args...)                       \
        do {                                           \
            fprintf(stderr, "QEMU I2C: " fmt, ##args); \
        }                                              \
        while (0)
...
{
    switch (offset) {
        case I2CCON_ADDR : return "I2CCON";
        case I2CSTAT_ADDR: return "I2CSTAT";
        case I2CADD_ADDR : return "I2CADD";
        case I2CDS_ADDR  : return "I2CDS";
        case I2CLC_ADDR  : return "I2CLC";
        default          : return "[?]";
    }
}
...
static void exynos4210_i2c_data_send(void* opaque)
{
    ...
    s->i2cstat  &= ~I2CSTAT_LAST_BIT;
    s->scl_free  = false;
    if (i2c_send(s->bus, s->i2cds) < 0 && (s->i2ccon & I2CCON_ACK_GEN)) { s->i2cstat |= I2CSTAT_LAST_BIT; }
    exynos4210_i2c_raise_interrupt(s);
}

static uint64_t exynos4210_i2c_read(void* opaque, hwaddr offset, unsigned size)
{
    Exynos4210I2CState* s = (Exynos4210I2CState*)opaque;
    uint8_t             value;

    switch (offset) {
        case I2CCON_ADDR : value = s->i2ccon; break;
        case I2CSTAT_ADDR: value = s->i2cstat; break;
        case I2CADD_ADDR : value = s->i2cadd; break;
        case I2CDS_ADDR:
            value       = s->i2cds;
            s->scl_free = true;
            if (EXYNOS4_I2C_MODE(s->i2cstat) == I2CMODE_MASTER_Rx && (s->i2cstat & I2CSTAT_START_BUSY)
                && !(s->i2ccon & I2CCON_INT_PEND))
            {
                exynos4210_i2c_data_receive(s);
            }
            break;
        case I2CLC_ADDR: value = s->i2clc; break;
        default:
            value = 0;
    ...
    DPRINT("read %s [0x%02x] -> 0x%02x\n", exynos4_i2c_get_regname(offset), (unsigned int)offset, value);
    return value;
}

static void exynos4210_i2c_write(void* opaque, hwaddr offset, uint64_t value, unsigned size)
{
    Exynos4210I2CState* s = (Exynos4210I2CState*)opaque;
    uint8_t             v = value & 0xff;

    DPRINT("write %s [0x%02x] <- 0x%02x\n", exynos4_i2c_get_regname(offset), (unsigned int)offset, v);
    ...
    switch (offset) {
        case I2CCON_ADDR:
            ...
            if ((s->i2ccon & I2CCON_INT_PEND) && !(v & I2CCON_INT_PEND)) {
                ...
                if (!(s->i2ccon & I2CCON_INTRS_EN)) { s->i2cstat &= ~I2CSTAT_START_BUSY; }

                if (s->i2cstat & I2CSTAT_START_BUSY) {
                    if (s->scl_free) {
                        if (EXYNOS4_I2C_MODE(s->i2cstat) == I2CMODE_MASTER_Tx) { exynos4210_i2c_data_send(s); }
            ...
            break;
        case I2CSTAT_ADDR:
            s->i2cstat = (s->i2cstat & I2CSTAT_START_BUSY) | (v & ~I2CSTAT_START_BUSY);
            ...
            if (!I2C_IN_MASTER_MODE(s->i2cstat)) { break; }
            ...
            if (v & I2CSTAT_START_BUSY) {
                ...
                if (i2c_start_transfer(s->bus, s->i2cds >> 1, s->i2cds & 0x1) && (s->i2ccon & I2CCON_ACK_GEN)) {
                    s->i2cstat |= I2CSTAT_LAST_BIT;
                }
                else if (EXYNOS4_I2C_MODE(s->i2cstat) == I2CMODE_MASTER_Rx) {
                    exynos4210_i2c_data_receive(s);
                ...
                if (!(s->i2ccon & I2CCON_INT_PEND)) { s->i2cstat &= ~I2CSTAT_START_BUSY; }
                s->scl_free = true;
            }
            break;
        case I2CADD_ADDR:
            if ((s->i2cstat & I2CSTAT_OUTPUT_EN) == 0) { s->i2cadd = v; }
            break;
        case I2CDS_ADDR:
            if (s->i2cstat & I2CSTAT_OUTPUT_EN) {
                s->i2cds    = v;
                s->scl_free = true;
                if (EXYNOS4_I2C_MODE(s->i2cstat) == I2CMODE_MASTER_Tx && (s->i2cstat & I2CSTAT_START_BUSY)
                    && !(s->i2ccon & I2CCON_INT_PEND))
                {
            ...
            break;
        case I2CLC_ADDR: s->i2clc = v; break;
        default        : DPRINT("ERROR: Bad write offset 0x%x\n", (unsigned int)offset); break;
    }
}
...
static void exynos4210_i2c_init(Object* obj)
{
    ...
    memory_region_init_io(&s->iomem, obj, &exynos4210_i2c_ops, s, TYPE_EXYNOS4_I2C, EXYNOS4_I2C_MEM_SIZE);
