...
{
    bool level = false;
    if (REG(s, REG_SMSTA) & REG(s, REG_IMASK)) { level = true; }
    else {
        level = false;
    }
    if (level != s->last_irq) {
...
static void apple_i2c_reg_write(void* opaque, hwaddr addr, uint64_t data, unsigned size)
{
    ...
    switch (addr) {
        case REG_MTXFIFO: {
            ...
            if ((value & kMTXFIFOStart)) {
                if (kMTXFIFOData(value) & 1) { s->is_recv = true; }
                else {
                    s->is_recv = false;
                }
                ...
                if (value & kMTXFIFORead) {
                    ...
                    while (len--) { fifo8_push(&s->rx_fifo, i2c_recv(s->bus)); }
                    if (kMTXFIFOData(value) > 0) {
                        REG(s, REG_SMSTA) |= (kSMSTAmrne);
                        if (kMTXFIFOData(value) >= kRDCOUNT(REG(s, REG_RDCOUNT))) { REG(s, REG_SMSTA) |= (kSMSTAmrf); }
                    }
                }
                else {
                    if (s->is_recv) {
        ...
        case REG_SMSTA:
            ...
            iflg  = true;
            break;
        case REG_IMASK: iflg = true; break;
        case REG_CTL:
            if (value & kCTLMRR) { fifo8_reset(&s->rx_fifo); }
            value = 0;
            break;
        default: break;
    }
    ...
    *mmio = value;
    if (iflg) { apple_i2c_update_irq(s); }
}
...
static uint64_t apple_i2c_reg_read(void* opaque, hwaddr addr, unsigned size)
{
    ...
    switch (addr) {
        ...
        case REG_MCNT:
            ...
            break;
        case REG_VERSION: value = 2; break;
        default         : break;
    }

#ifdef DEBUG_APPLE_I2C
    qemu_log_mask(LOG_UNIMP, "%s: reg READ @ 0x" HWADDR_FMT_plx " value: 0x%x\n", DEVICE(s)->id, addr, value);
#endif
    return value;
}
...
SysBusDevice* apple_i2c_create(const char* name)
{
    ...
    memory_region_init_io(&s->iomem, OBJECT(dev), &i2c_reg_ops, s, dev->id, MMIO_SIZE);
