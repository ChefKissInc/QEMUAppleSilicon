...
static void generate_timings(Timings* timings, uint32_t refresh_rate, uint32_t xres, uint32_t yres)
{
    ...
    timings->clock = ((uint64_t)refresh_rate * (xres + timings->xblank) * (yres + timings->yblank)) / 10000000;
}
...
static void edid_fill_modes(uint8_t* edid, uint8_t* xtra3, uint8_t* dta, uint32_t maxx, uint32_t maxy)
{
    ...
    for (i = 0; i < ARRAY_SIZE(modes); i++) {
        ...
        if ((maxx && mode->xres > maxx) || (maxy && mode->yres > maxy)) { continue; }

        if (mode->byte) { edid[mode->byte] |= (1 << mode->bit); }
        else if (std < 54) {
        ...
        if (dta && mode->dta) { edid_ext_dta_mode(dta, mode->dta); }
    }
...
static void edid_checksum(uint8_t* edid, size_t len)
{
    ...
    for (i = 0; i < len; i++) { sum += edid[i]; }
    sum &= 0xff;
    if (sum) { edid[len] = 0x100 - sum; }
}

static uint8_t* edid_desc_next(uint8_t* edid, uint8_t* dta, uint8_t* desc)
{
    if (desc == NULL) { return NULL; }
    if (desc + 18 + 18 < edid + 127) { return desc + 18; }
    if (dta) {
        if (desc < edid + 127) { return dta + dta[2]; }
        if (desc + 18 + 18 < dta + 127) { return desc + 18; }
    }
    return NULL;
}
...
static void edid_desc_text(uint8_t* desc, uint8_t type, const char* text)
{
    size_t len;
...
static void edid_desc_dummy(uint8_t* desc) { edid_desc_type(desc, 0x10); }

static void edid_desc_timing(uint8_t* desc, const Timings* timings, uint32_t xres, uint32_t yres, uint32_t xmm,
                             uint32_t ymm)
{
    ...
    desc[4] = (((xres & 0xf00) >> 4) | ((timings->xblank & 0xf00) >> 8));

    desc[5] = yres & 0xff;
    desc[6] = timings->yblank & 0xff;
    desc[7] = (((yres & 0xf00) >> 4) | ((timings->yblank & 0xf00) >> 8));

    desc[8] = timings->xfront & 0xff;
    desc[9] = timings->xsync & 0xff;

    desc[10] = (((timings->yfront & 0x00f) << 4) | ((timings->ysync & 0x00f) << 0));
    desc[11] = (((timings->xfront & 0x300) >> 2) | ((timings->xsync & 0x300) >> 4) | ((timings->yfront & 0x030) >> 2)
                | ((timings->ysync & 0x030) >> 4));

    desc[12] = xmm & 0xff;
    desc[13] = ymm & 0xff;
    desc[14] = (((xmm & 0xf00) >> 4) | ((ymm & 0xf00) >> 8));

    desc[17] = 0x18;
}

static uint32_t edid_to_10bit(float value) { return (uint32_t)(value * 1024 + 0.5); }

static void edid_colorspace(uint8_t* edid, float rx, float ry, float gx, float gy, float bx, float by, float wx,
                            float wy)
{
    ...
    edid[25] = (((red_x & 0x03) << 6) | ((red_y & 0x03) << 4) | ((green_x & 0x03) << 2) | ((green_y & 0x03) << 0));
    edid[26] = (((blue_x & 0x03) << 6) | ((blue_y & 0x03) << 4) | ((white_x & 0x03) << 2) | ((white_y & 0x03) << 0));
    edid[27] = red_x >> 2;
    edid[28] = red_y >> 2;
    edid[29] = green_x >> 2;
    edid[30] = green_y >> 2;
...
static uint32_t qemu_edid_dpi_from_mm(uint32_t mm, uint32_t res) { return res * 254 / 10 / mm; }

uint32_t qemu_edid_dpi_to_mm(uint32_t dpi, uint32_t res) { return res * 254 / 10 / dpi; }
...
static void qemu_displayid_generate(uint8_t* did, const Timings* timings, uint32_t xres, uint32_t yres, uint32_t xmm,
                                    uint32_t ymm)
{
    did[0] = 0x70; /* display id extension */
    did[1] = 0x13; /* version 1.3 */
...
void qemu_edid_generate(uint8_t* edid, size_t size, qemu_edid_info* info)
{
    ...
    if (!info->vendor || strlen(info->vendor) != 3) { info->vendor = "RHT"; }
    if (!info->name) { info->name = "QEMU Monitor"; }
    if (!info->prefx) { info->prefx = 1280; }
    if (!info->prefy) { info->prefy = 800; }
    ...
    if (info->prefx >= 4096 || info->prefy >= 4096 || timings.clock >= 65536) { large_screen = 1; }
    ...
    uint16_t vendor_id = ((((info->vendor[0] - '@') & 0x1f) << 10) | (((info->vendor[1] - '@') & 0x1f) << 5)
    ...
    edid_colorspace(edid, 0.6400, 0.3300, /* red   */
                    0.3000, 0.6000,       /* green */
                    0.1500, 0.0600,       /* blue  */
    ...
    if (!large_screen) {
        ...
        edid_desc_timing(desc, &timings, info->prefx, info->prefy, width_mm, height_mm);
    ...
    if (did && large_screen) { qemu_displayid_generate(did, &timings, info->prefx, info->prefy, width_mm, height_mm); }

    /* =============== finish up =============== */

    edid_checksum(edid, 127);
    if (dta) { edid_checksum(dta, 127); }
    if (did) { edid_checksum(did, 127); }
}
