...
static uint64_t dpcd_read(void* opaque, hwaddr offset, unsigned size)
{
    ...
    if (offset < DPCD_READABLE_AREA) { ret = e->dpcd_info[offset]; }
    else {
...
static void dpcd_write(void* opaque, hwaddr offset, uint64_t value, unsigned size)
{
    DPCDState* e = DPCD(opaque);

    trace_dpcd_write(offset, value);
    if (offset < DPCD_READABLE_AREA) { e->dpcd_info[offset] = value; }
    else {
        qemu_log_mask(LOG_GUEST_ERROR, "dpcd: Bad offset 0x%" HWADDR_PRIX "\n", offset);
    }
}
...
static void dpcd_reset_enter(Object* obj, ResetType type)
{
    ...
    s->dpcd_info[DPCD_LANE0_1_STATUS] = DPCD_LANE0_CR_DONE | DPCD_LANE0_CHANNEL_EQ_DONE | DPCD_LANE0_SYMBOL_LOCKED
                                        | DPCD_LANE1_CR_DONE | DPCD_LANE1_CHANNEL_EQ_DONE | DPCD_LANE1_SYMBOL_LOCKED;
    s->dpcd_info[DPCD_LANE2_3_STATUS] = DPCD_LANE2_CR_DONE | DPCD_LANE2_CHANNEL_EQ_DONE | DPCD_LANE2_SYMBOL_LOCKED
