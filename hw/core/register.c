...
static inline void register_write_val(RegisterInfo* reg, uint64_t val)
{
    ...
    switch (reg->data_size) {
        case 1 : *(uint8_t*)reg->data = val; break;
        case 2 : *(uint16_t*)reg->data = val; break;
        case 4 : *(uint32_t*)reg->data = val; break;
        case 8 : *(uint64_t*)reg->data = val; break;
...
{
    switch (reg->data_size) {
        case 1 : return *(uint8_t*)reg->data;
        case 2 : return *(uint16_t*)reg->data;
        case 4 : return *(uint32_t*)reg->data;
        case 8 : return *(uint64_t*)reg->data;
        default: assert_not_reached();
    }
...
void register_write(RegisterInfo* reg, uint64_t val, uint64_t we, const char* prefix, bool debug)
{
    ...
    if (ac->pre_write) { new_val = ac->pre_write(reg, new_val); }

    if (debug) { qemu_log("%s:%s: write of value 0x%" PRIx64 "\n", prefix, ac->name, new_val); }

    register_write_val(reg, new_val);

    if (ac->post_write) { ac->post_write(reg, new_val); }
}
...
uint64_t register_read(RegisterInfo* reg, uint64_t re, const char* prefix, bool debug)
{
    ...
    if (debug) { qemu_log("%s:%s: read of value 0x%" PRIx64 "\n", prefix, ac->name, ret); }
...
void register_reset(RegisterInfo* reg)
{
    ...
    if (!reg->data || !reg->access) { return; }

    ac = reg->access;

    register_write_val(reg, reg->access->reset);

    if (ac->post_write) { ac->post_write(reg, reg->access->reset); }
}
...
void register_write_memory(void* opaque, hwaddr addr, uint64_t value, unsigned size)
{
    ...
    register_write(reg, value, we, reg_array->prefix, reg_array->debug);
}
...
uint64_t register_read_memory(void* opaque, hwaddr addr, unsigned size)
{
    ...
    read_val = register_read(reg, re, reg_array->prefix, reg_array->debug);

    return extract64(read_val, 0, size * 8);
}

static RegisterInfoArray* register_init_block(DeviceState* owner, const RegisterAccessInfo* rae, int num,
                                              RegisterInfo* ri, void* data, const MemoryRegionOps* ops,
                                              bool debug_enabled, uint64_t memory_size, size_t data_size_bits)
{
    ...
    memory_region_init_io(&r_array->mem, OBJECT(owner), ops, r_array, device_prefix, memory_size);
...
{ return register_init_block(owner, rae, num, ri, (void*)data, ops, debug_enabled, memory_size, 8); }
...
RegisterInfoArray* register_init_block32(DeviceState* owner, const RegisterAccessInfo* rae, int num, RegisterInfo* ri,
                                         ...
                                         uint64_t memory_size)
{ return register_init_block(owner, rae, num, ri, (void*)data, ops, debug_enabled, memory_size, 32); }
...
RegisterInfoArray* register_init_block64(DeviceState* owner, const RegisterAccessInfo* rae, int num, RegisterInfo* ri,
                                         ...
                                         uint64_t memory_size)
{ return register_init_block(owner, rae, num, ri, (void*)data, ops, debug_enabled, memory_size, 64); }
...
static void register_register_types(void) { type_register_static(&register_info); }
