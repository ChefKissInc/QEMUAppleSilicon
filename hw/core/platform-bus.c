 ...
 */
int platform_bus_get_irqn(PlatformBusDevice* pbus, SysBusDevice* sbdev, int n)
{
    qemu_irq sbirq = sysbus_get_connected_irq(sbdev, n);
    int      i;

    for (i = 0; i < pbus->num_irqs; i++) {
        if (pbus->irqs[i] == sbirq) { return i; }
    }
 ...
 */
hwaddr platform_bus_get_mmio_addr(PlatformBusDevice* pbus, SysBusDevice* sbdev, int n)
{
    ...
    parent_mr = object_property_get_link(OBJECT(sbdev_mr), "container", &error_abort);
...
static void platform_bus_count_irqs(SysBusDevice* sbdev, void* opaque)
{
    ...
    for (n = 0;; n++) {
        if (!sysbus_has_irq(sbdev, n)) { break; }
...
static void platform_bus_map_irq(PlatformBusDevice* pbus, SysBusDevice* sbdev, int n)
{
...
static void platform_bus_map_mmio(PlatformBusDevice* pbus, SysBusDevice* sbdev, int n)
{
    MemoryRegion* sbdev_mr  = sysbus_mmio_get_region(sbdev, n);
    ...
    if (!found_region) {
        error_report("Platform Bus: Can not fit MMIO region of size %" PRIx64, size);
        exit(1);
    }
...
void platform_bus_link_device(PlatformBusDevice* pbus, SysBusDevice* sbdev)
{
    ...
    for (i = 0; sysbus_has_irq(sbdev, i); i++) { platform_bus_map_irq(pbus, sbdev, i); }

    for (i = 0; sysbus_has_mmio(sbdev, i); i++) { platform_bus_map_mmio(pbus, sbdev, i); }
}
...
static void platform_bus_realize(DeviceState* dev, Error** errp)
{
    ...
    memory_region_init(&pbus->mmio, OBJECT(dev), "platform bus", pbus->mmio_size);
    sysbus_init_mmio(d, &pbus->mmio);

    pbus->used_irqs = bitmap_new(pbus->num_irqs);
    pbus->irqs      = g_new0(qemu_irq, pbus->num_irqs);
    for (i = 0; i < pbus->num_irqs; i++) { sysbus_init_irq(d, &pbus->irqs[i]); }
