...
CPUState* cpu_by_arch_id(int64_t id)
{
    ...
    CPU_FOREACH (cpu) {
        if (cpu->cc->get_arch_id(cpu) == id) { return cpu; }
    }
    return NULL;
}
...
static int cpu_common_gdb_read_register(CPUState* cpu, GByteArray* buf, int reg) { return 0; }

static int cpu_common_gdb_write_register(CPUState* cpu, uint8_t* buf, int reg) { return 0; }
...
ObjectClass* cpu_class_by_name(const char* typename, const char* cpu_model)
{
    ...
    if (object_class_dynamic_cast(oc, typename) && !object_class_is_abstract(oc)) { return oc; }
...
char* cpu_model_from_type(const char* typename)
{
    ...
    if (!object_class_by_name(typename)) { return NULL; }

    if (g_str_has_suffix(typename, suffix)) { return g_strndup(typename, strlen(typename) - strlen(suffix)); }
...
{
    if (!accel_cpu_common_realize(cpu, errp)) { return false; }
...
static void cpu_common_realizefn(DeviceState* dev, Error** errp)
{
    ...
    if (object_dynamic_cast(machine, TYPE_MACHINE)) {
        ...
        if (mc) { cpu->ignore_memory_transaction_failures = mc->ignore_memory_transaction_failures; }
    }
...
static void cpu_common_finalize(Object* obj)
{
    ...
    if (cpu->gdb_regs) { g_array_free(cpu->gdb_regs, TRUE); }
