...
static void enable_trace_buffer(AppleSEPState* s)
{
    ...
    if (s->chip_id == 0x8015) { sepos_phys_base = SEPOS_PHYS_BASE_T8015; }
...
static void pmgr_base_reg_write(void* opaque, hwaddr addr, uint64_t data, unsigned size)
{
    ...
    switch (addr) {
        ...
        case 0x8000:
            ...
            if ((data & 1) != 0) { s->pmgr_fuse_changer_bit0_was_set = true; }
            if ((data & 2) != 0) { s->pmgr_fuse_changer_bit1_was_set = true; }
...
static void sep_manual_timer(void* opaque)
{
    ...
    WITH_QEMU_LOCK_GUARD(&s->manual_timer_lock)
    {
        ...
        apple_a7iop_interrupt_status_push(s->mailbox, INTERRUPT_SEP_MANUAL_TIMER);
...
static void key_fcfg_reg_write(void* opaque, hwaddr addr, uint64_t data, unsigned size)
{
    ...
    switch (addr) {
        ...
        case 0xc:
            ...
            goto jump_default;
        case 0x10:
            if (data == 0x1) { ((uint32_t*)s->key_base_regs)[0x00 / 4] = BIT(31) | BIT(0); }
    ...
    switch (addr) {
        case 0x14: {
            ...
            key_fcfg_offset_0x14_index_limit =
                sizeof(s->key_fcfg_offset_0x14_values) / sizeof(s->key_fcfg_offset_0x14_values[0]);
...
static void aess_keywrap_uid(AppleAESSState* s, uint8_t* in, uint8_t* out, QCryptoCipherAlgo cipher_alg, uint32_t cmd,
...
static void aess_base_reg_write(void* opaque, hwaddr addr, uint64_t data, unsigned size)
{
    AppleAESSState* s         = opaque;
    AppleSEPState*  sep       = s->sep;
...
static void aesh_base_reg_write(void* opaque, hwaddr addr, uint64_t data, unsigned size)
{
    AppleAESHState* s   = opaque;
    AppleSEPState*  sep = s->sep;
    ...
    switch (addr) {
        ...
        case 0x8:
            ...
            if (data == 0x41A7 && (s->chip_id >= 0x8015)) {
                ...
                AddressSpace* nsas = &address_space_memory;
                address_space_write(nsas, s->sep_fw_addr, MEMTXATTRS_UNSPECIFIED, s->fw_data, s->sep_fw_size);
#endif
                // g_free(sep_fw);
            }
#if 1
...
#define LVL3_BASE_COPYFROM 0x24090C000ULL
        ...
        case 0x0:
            ...
            if (data == 0xDEADBEE0) { qemu_irq_lower(qdev_get_gpio_in((DeviceState*)s->cpu, ARM_CPU_IRQ)); }
            if (data == 0xDEADBEE1) { qemu_irq_lower(qdev_get_gpio_in((DeviceState*)s->cpu, ARM_CPU_FIQ)); }
            if (data == 0xDEADBEE2) { qemu_irq_lower(qdev_get_gpio_in((DeviceState*)s->cpu, ARM_CPU_VIRQ)); }
            if (data == 0xDEADBEE3) { qemu_irq_lower(qdev_get_gpio_in((DeviceState*)s->cpu, ARM_CPU_VFIQ)); }

            if (data == 0xDEADBEE4) { qemu_irq_raise(qdev_get_gpio_in((DeviceState*)s->cpu, ARM_CPU_IRQ)); }
            if (data == 0xDEADBEE5) { qemu_irq_raise(qdev_get_gpio_in((DeviceState*)s->cpu, ARM_CPU_FIQ)); }
            if (data == 0xDEADBEE6) { qemu_irq_raise(qdev_get_gpio_in((DeviceState*)s->cpu, ARM_CPU_VIRQ)); }
            if (data == 0xDEADBEE7) { qemu_irq_raise(qdev_get_gpio_in((DeviceState*)s->cpu, ARM_CPU_VFIQ)); }
            if (data == 0xCAFE1334) {
                uint32_t i = 0;
...
static void apple_sep_iop_start(AppleA7IOP* s)
{
    ...
    if (sep->modern) { async_safe_run_on_cpu(CPU(sep->cpu), apple_sep_cpu_moni_jump, RUN_ON_CPU_HOST_PTR(sep)); }
}
                                   ...
                                   uint32_t chip_id)
{
    ...
    memory_region_init_io(&s->pmgr_base_mr, OBJECT(dev), &pmgr_base_reg_ops, s, "sep.pmgr_base", PMGR_BASE_REG_SIZE);
    sysbus_init_mmio(sbd, &s->pmgr_base_mr);
    memory_region_init_io(&s->trng_regs_mr, OBJECT(dev), &trng_regs_reg_ops, &s->trng_state, "sep.trng_regs",
                          TRNG_REGS_REG_SIZE);
    sysbus_init_mmio(sbd, &s->trng_regs_mr);
    memory_region_init_io(&s->key_base_mr, OBJECT(dev), &key_base_reg_ops, s, "sep.key_base", KEY_BASE_REG_SIZE);
    sysbus_init_mmio(sbd, &s->key_base_mr);
    memory_region_init_io(&s->key_fkey_mr, OBJECT(dev), &key_fkey_reg_ops, s, "sep.key_fkey", KEY_FKEY_REG_SIZE_T8015);
    sysbus_init_mmio(sbd, &s->key_fkey_mr);
    memory_region_init_io(&s->key_fcfg_mr, OBJECT(dev), &key_fcfg_reg_ops, s, "sep.key_fcfg", KEY_FCFG_REG_SIZE_T8020);
    sysbus_init_mmio(sbd, &s->key_fcfg_mr);
    memory_region_init_io(&s->moni_base_mr, OBJECT(dev), &moni_base_reg_ops, s, "sep.moni_base", MONI_BASE_REG_SIZE);
    sysbus_init_mmio(sbd, &s->moni_base_mr);
    memory_region_init_io(&s->moni_thrm_mr, OBJECT(dev), &moni_thrm_reg_ops, s, "sep.moni_thrm", MONI_THRM_REG_SIZE);
    sysbus_init_mmio(sbd, &s->moni_thrm_mr);
    memory_region_init_io(&s->eisp_base_mr, OBJECT(dev), &eisp_base_reg_ops, s, "sep.eisp_base", EISP_BASE_REG_SIZE);
    sysbus_init_mmio(sbd, &s->eisp_base_mr);
    memory_region_init_io(&s->eisp_hmac_mr, OBJECT(dev), &eisp_hmac_reg_ops, s, "sep.eisp_hmac", EISP_HMAC_REG_SIZE);
    sysbus_init_mmio(sbd, &s->eisp_hmac_mr);
    memory_region_init_io(&s->aess_base_mr, OBJECT(dev), &aess_base_reg_ops, &s->aess_state, "sep.aess_base",
                          AESS_BASE_REG_SIZE);
    sysbus_init_mmio(sbd, &s->aess_base_mr);
    // at least >= t8015 have this (aesh), according to their seprom's, but I'm
    // not sure about s8000
    memory_region_init_io(&s->aesh_base_mr, OBJECT(dev), &aesh_base_reg_ops, &s->aesh_state, "sep.aesh_base",
                          AESH_BASE_REG_SIZE);
    sysbus_init_mmio(sbd, &s->aesh_base_mr);
    memory_region_init_io(&s->aesc_base_mr, OBJECT(dev), &aesc_base_reg_ops, s, "sep.aesc_base", AESC_BASE_REG_SIZE);
    sysbus_init_mmio(sbd, &s->aesc_base_mr);
    memory_region_init_io(&s->pka_base_mr, OBJECT(dev), &pka_base_reg_ops, &s->pka_state, "sep.pka_base",
                          PKA_BASE_REG_SIZE);
    sysbus_init_mmio(sbd, &s->pka_base_mr);
    memory_region_init_io(&s->pka_tmm_mr, OBJECT(dev), &pka_tmm_reg_ops, s, "sep.pka_tmm", PKA_TMM_REG_SIZE);
    sysbus_init_mmio(sbd, &s->pka_tmm_mr);
    memory_region_init_io(&s->misc0_mr, OBJECT(dev), &misc0_reg_ops, s, "sep.misc0", MISC0_REG_SIZE);
    sysbus_init_mmio(sbd, &s->misc0_mr);
    memory_region_init_io(&s->misc1_mr, OBJECT(dev), &misc1_reg_ops, s, "sep.misc1", MISC1_REG_SIZE);
    sysbus_init_mmio(sbd, &s->misc1_mr);
    memory_region_init_io(&s->misc2_mr, OBJECT(dev), &misc2_reg_ops, s, "sep.misc2", MISC2_REG_SIZE);
    ...
    memory_region_init_io(&s->boot_monitor_mr, OBJECT(dev), &boot_monitor_reg_ops, s, "sep.boot_monitor",
                          BOOT_MONITOR_REG_SIZE);
    sysbus_init_mmio(sbd, &s->boot_monitor_mr);
#ifdef SEP_ENABLE_TRACE_BUFFER
    // TODO: Let's think about something for T8015
    memory_region_init_io(&s->debug_trace_mr, OBJECT(dev), &debug_trace_reg_ops, s, "sep.debug_trace",
    ...
    s->aess_state.command_bh = aio_bh_new_guarded(qemu_get_aio_context(), aess_handle_cmd_bh, &s->aess_state,
                                                  &DEVICE(s)->mem_reentrancy_guard);
    s->aesh_state.command_bh = aio_bh_new_guarded(qemu_get_aio_context(), aesh_handle_cmd_bh, &s->aesh_state,
                                                  &DEVICE(s)->mem_reentrancy_guard);
    s->pka_state.command_bh  = aio_bh_new_guarded(qemu_get_aio_context(), pka_handle_cmd_bh, &s->pka_state,
...
static int apple_ssc_event(I2CSlave* s, enum i2c_event event)
{
    ...
    switch (event) {
        case I2C_START_SEND: DPRINTF("apple_ssc_event: I2C_START_SEND\n"); break;
        case I2C_FINISH    : DPRINTF("apple_ssc_event: I2C_FINISH\n");
#if 1
...
#endif
            break;
        case I2C_START_RECV: DPRINTF("apple_ssc_event: I2C_START_RECV\n"); break;
        case I2C_NACK      : DPRINTF("apple_ssc_event: I2C_NACK\n"); break;
        default            : return -1;
    }
    return 0;
}
...
#else
static bool is_keyslot_valid(struct AppleSSCState* ssc_state, uint16_t kbkdf_index)
#endif
{
    bool ret;
...
static int aes_ccm_crypt(struct AppleSSCState* ssc_state, uint16_t kbkdf_index, uint8_t* prefix, int payload_len,
...
static int generate_ec_priv(struct AppleSSCState* ssc_state, const char* priv, struct ecc_scalar* ecc_key,
...
static int generate_kbkdf_keys(struct AppleSSCState* ssc_state, struct ecc_scalar* ecc_key,
...
static void aes_keys_from_sp_key(struct AppleSSCState* ssc_state, uint16_t kbkdf_index, uint8_t* prefix,
...
static void answer_cmd_0x0_init1(struct AppleSSCState* ssc_state, uint8_t* request, uint8_t* response)
{
    DPRINTF("%s: entered function\n", __func__);
...
static void answer_cmd_0x1_connect_sp(struct AppleSSCState* ssc_state, uint8_t* request, uint8_t* response)
{
    DPRINTF("%s: entered function\n", __func__);
...
static void answer_cmd_0x2_disconnect_sp(struct AppleSSCState* ssc_state, uint8_t* request, uint8_t* response)
{
    DPRINTF("%s: entered function\n", __func__);
...
static void answer_cmd_0x3_metadata_write(struct AppleSSCState* ssc_state, uint8_t* request, uint8_t* response)
{
    DPRINTF("%s: entered function\n", __func__);
...
static void answer_cmd_0x4_metadata_data_read(struct AppleSSCState* ssc_state, uint8_t* request, uint8_t* response)
{
    DPRINTF("%s: entered function\n", __func__);
...
static void answer_cmd_0x5_metadata_data_write(struct AppleSSCState* ssc_state, uint8_t* request, uint8_t* response)
{
    DPRINTF("%s: entered function\n", __func__);
...
static void answer_cmd_0x6_metadata_read(struct AppleSSCState* ssc_state, uint8_t* request, uint8_t* response)
{
    DPRINTF("%s: entered function\n", __func__);
...
static void answer_cmd_0x7_init0(struct AppleSSCState* ssc_state, uint8_t* request, uint8_t* response)
{
    struct ecc_point ecc_pub;
    DPRINTF("%s: entered function\n", __func__);
...
static void answer_cmd_0x8_sleep(struct AppleSSCState* ssc_state, uint8_t* request, uint8_t* response)
{
    DPRINTF("%s: entered function\n", __func__);
    do_response_prefix(request, response, SSC_RESPONSE_FLAG_OK);
...
static void answer_cmd_0x9_panic(struct AppleSSCState* ssc_state, uint8_t* request, uint8_t* response)
{
    DPRINTF("%s: entered function\n", __func__);
...
static int apple_ssc_tx(I2CSlave* i2c, uint8_t data)
{
    ...
    if (ssc->req_cur >= sizeof(ssc->req_cmd)) {
        qemu_log_mask(LOG_GUEST_ERROR, "apple_ssc_tx: ssc->req_cur too high 0x%02x\n", ssc->req_cur);
        return 0;
    }
...
{
    AppleSSCState* ssc;
    AppleI2CState* i2c          = APPLE_I2C(object_property_get_link(OBJECT(machine), "sep_i2c", &error_fatal));
...
static void apple_ssc_register_types(void) { type_register_static(&apple_ssc_type_info); }
