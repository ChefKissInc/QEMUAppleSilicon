...
static void apple_sep_sim_handle_control_msg(AppleSEPSimState* s, SEPMessage* msg)
{
    ...
    switch (msg->op) {
        ...
        case CONTROL_OP_SET_OOL_IN_ADDR:
            ...
            qemu_log_mask(LOG_GUEST_ERROR, "EP_CONTROL: SET_OOL_IN_ADDR (%d, 0x%" PRIx64 ")\n", set_ool_msg->id,
                          (uint64_t)set_ool_msg->data << 12);
            apple_sep_sim_set_ool_in_addr(s, set_ool_msg->id, (uint64_t)set_ool_msg->data << 12);
            apple_sep_sim_control_send_ack(s, msg, 0, 0);
            break;
        case CONTROL_OP_SET_OOL_OUT_ADDR:
            set_ool_msg = (SetOOLMessage*)msg;
            qemu_log_mask(LOG_GUEST_ERROR, "EP_CONTROL: SET_OOL_OUT_ADDR (%d, 0x%" PRIx64 ")\n", set_ool_msg->id,
                          (uint64_t)set_ool_msg->data << 12);
            apple_sep_sim_set_ool_out_addr(s, set_ool_msg->id, (uint64_t)set_ool_msg->data << 12);
            apple_sep_sim_control_send_ack(s, msg, 0, 0);
            break;
        case CONTROL_OP_SET_OOL_IN_SIZE:
            set_ool_msg = (SetOOLMessage*)msg;
            qemu_log_mask(LOG_GUEST_ERROR, "EP_CONTROL: SET_OOL_IN_SIZE (%d, 0x%X)\n", set_ool_msg->id,
                          set_ool_msg->data);
        ...
        case CONTROL_OP_SET_OOL_OUT_SIZE:
            set_ool_msg = (SetOOLMessage*)msg;
            qemu_log_mask(LOG_GUEST_ERROR, "EP_CONTROL: SET_OOL_OUT_SIZE (%d, 0x%X)\n", set_ool_msg->id,
                          set_ool_msg->data);
        ...
        case CONTROL_OP_ERASE_INSTALL: {
            ...
            if (dma_memory_write(s->dma_as, s->ool_state[EP_ART_STORAGE].out_addr, data, data_len,
                                 MEMTXATTRS_UNSPECIFIED)
                != MEMTX_OK)
            {
                qemu_log_mask(LOG_GUEST_ERROR, "EP_XART_STORAGE: Failed to write ART to OOL");
            }
            apple_sep_sim_send_message(s, EP_ART_STORAGE, 0, ART_STORAGE_OP_INCOMING, 0, 0);
            break;
        }
        default: qemu_log_mask(LOG_GUEST_ERROR, "EP_CONTROL: Unknown opcode %d\n", msg->op); break;
    }
}
...
static const uint8_t apple_sep_sim_eps[] = {
    EP_CONTROL,    EP_LOGGER,   EP_ART_STORAGE, EP_ART_REQUESTS, EP_SECURE_CREDENTIALS,
...
static void apple_sep_sim_advertise_eps(AppleSEPSimState* s)
{
    ...
    for (i = 0; i < ARRAY_SIZE(apple_sep_sim_eps); i++) {
        ...
        memcpy(&ool_advert_msg->ool_info, s->ool_info + apple_sep_sim_eps[i], sizeof(AppleSEPSimOOLInfo));
        apple_a7iop_send_ap(a7iop, msg);
    }
}
...
static void apple_sep_sim_handle_bootstrap_msg(AppleSEPSimState* s, SEPMessage* msg)
{
    ...
    switch (msg->op) {
        case BOOTSTRAP_OP_PING:
            ...
            apple_sep_sim_send_message(s, EP_BOOTSTRAP, msg->tag, BOOTSTRAP_OP_PING_ACK, 0, 0);
            break;
        case BOOTSTRAP_OP_GET_STATUS:
            qemu_log_mask(LOG_GUEST_ERROR, "EP_BOOTSTRAP: GET_STATUS\n");

            apple_sep_sim_send_message(s, EP_BOOTSTRAP, msg->tag, BOOTSTRAP_OP_STATUS_REPLY, 0, s->status);
            break;
        case BOOTSTRAP_OP_GENERATE_NONCE:
            ...
            apple_sep_sim_send_message(s, EP_BOOTSTRAP, msg->tag, BOOTSTRAP_OP_NONCE_GENERATED, 0, 0xa0);
            break;
        case BOOTSTRAP_OP_GET_NONCE_WORD:
            ...
            apple_sep_sim_send_message(s, EP_BOOTSTRAP, msg->tag, BOOTSTRAP_OP_NONCE_WORD_REPLY, msg->param, randval);
            break;
        case BOOTSTRAP_OP_BOOT_TZ0:
            ...
            apple_sep_sim_send_message(s, EP_BOOTSTRAP, msg->tag, BOOTSTRAP_OP_TZ0_ACCEPTED, 0, 0);
            break;
        case BOOTSTRAP_OP_BOOT_IMG4: {
            ...
            apple_sep_sim_send_message(s, EP_BOOTSTRAP, msg->tag, BOOTSTRAP_OP_IMG4_ACCEPTED, 0, 0);
            apple_sep_sim_send_message(s, EP_CONTROL, 0, CONTROL_OP_NOTIFY_ALIVE, 0, 0);
        ...
        case BOOTSTRAP_OP_OPCODE_16:
            ...
            qemu_guest_getrandom(&randval, sizeof(randval), NULL);
            apple_sep_sim_send_message(s, EP_BOOTSTRAP, msg->tag, BOOTSTRAP_OP_OPCODE_16_RESPONSE, 0, randval);
            break;
        default: qemu_log_mask(LOG_GUEST_ERROR, "EP_BOOTSTRAP: Unknown opcode %d\n", msg->op); break;
    }
}

static uint8_t* apple_sep_sim_gen_sks_hash(uint8_t* buf, const uint32_t msg_size)
{
    ...
    uint8_t* hash     = NULL;
    size_t   hash_len = 0;
    int      ret = qcrypto_hash_bytesv(QCRYPTO_HASH_ALGO_SHA256, iov, ARRAY_SIZE(iov), &hash, &hash_len, &error_fatal);
                                                 ...
                                                 const uint32_t resp_size)
{
    ...
    dma_memory_write(s->dma_as, s->ool_state[EP_KEYSTORE].out_addr, resp_buf, resp_size, MEMTXATTRS_UNSPECIFIED);
...
static void apple_sep_sim_handle_keystore_msg(AppleSEPSimState* s, KeystoreMessage* msg)
{
    ...
    dma_memory_read(s->dma_as, s->ool_state[EP_KEYSTORE].in_addr, msg_buf, msg->size, MEMTXATTRS_UNSPECIFIED);
    ...
    switch (msg_code) {
        case 0x01: {
            ...
            memcpy(resp_hdr->ipc_digest, msg_hdr->ipc_digest, sizeof(resp_hdr->ipc_digest));
        ...
        case 0x02: {
            ...
            qemu_log_mask(LOG_GUEST_ERROR, "SEP KeyStore // Copy Keybag 0x%X 0x%" PRIx64 " 0x%X\n", *word0, *lword,
                          *word1);
        ...
        case 0x08: {
            ...
            // apple_sep_sim_keystore_send_ipc_resp(s, msg, resp_buf, resp_size);
            // g_free(resp_buf);
            apple_sep_sim_send_message(s, msg->ep, msg->tag | KEYSTORE_MSG_TAG_REPLY, msg->id, 0, 0);
            break;
        }
        case 0x0A: {
        ...
        case 0x19: {
            ...
            qemu_log_mask(LOG_GUEST_ERROR, "SEP KeyStore // Get Device State 0x%X 0x%" PRIx64 " 0x%X 0x%X\n", *word0,
                          *lword, *word1, *word2);
        ...
        case 0x1B: {
            ...
            qemu_log_mask(LOG_GUEST_ERROR, "SEP KeyStore // Client Terminate (0x%X)\n", *selector);
        ...
        default: {
            ...
            dma_memory_write(s->dma_as, s->ool_state[EP_KEYSTORE].out_addr, msg_buf, msg->size, MEMTXATTRS_UNSPECIFIED);
            apple_sep_sim_send_message(s, msg->ep, msg->tag | KEYSTORE_MSG_TAG_REPLY, msg->id, 0,
                                       (uint32_t)msg->size << 16);
            break;
        }
    }
    g_free(msg_buf);
...
AppleSEPSimState* apple_sep_sim_from_node(AppleDTNode* node, bool modern)
{
    ...
    apple_a7iop_init(a7iop, "SEP", reg[1], modern ? APPLE_A7IOP_V4 : APPLE_A7IOP_V2, NULL,
...
static void apple_sep_sim_class_init(ObjectClass* klass, const void* data)
{
    ...
    resettable_class_set_parent_phases(rc, NULL, apple_sep_sim_reset_hold, NULL, &sc->parent_phases);

    device_class_set_parent_realize(dc, apple_sep_sim_realize, &sc->parent_realize);
