...
static GICv3CPUState* icc_cs_from_env(CPUARMState* env) { return env->gicv3state; }
...
static inline int icv_min_vbpr(GICv3CPUState* cs) { return 7 - cs->vprebits; }
...
static uint32_t ich_lr_vintid(uint64_t lr) { return extract64(lr, ICH_LR_EL2_VINTID_SHIFT, ICH_LR_EL2_VINTID_LENGTH); }

static uint32_t ich_lr_pintid(uint64_t lr) { return extract64(lr, ICH_LR_EL2_PINTID_SHIFT, ICH_LR_EL2_PINTID_LENGTH); }

static uint32_t ich_lr_prio(uint64_t lr)
{ return extract64(lr, ICH_LR_EL2_PRIORITY_SHIFT, ICH_LR_EL2_PRIORITY_LENGTH); }

static int ich_lr_state(uint64_t lr) { return extract64(lr, ICH_LR_EL2_STATE_SHIFT, ICH_LR_EL2_STATE_LENGTH); }
...
static bool icv_access(CPUARMState* env, int hcr_flags)
{
    ...
    return flagmatch && arm_current_el(env) == 1 && !arm_is_secure_below_el3(env);
}
...
static int read_vbpr(GICv3CPUState* cs, int grp)
{
     ...
     */
    if (grp == GICV3_G0) { return extract64(cs->ich_vmcr_el2, ICH_VMCR_EL2_VBPR0_SHIFT, ICH_VMCR_EL2_VBPR0_LENGTH); }
    else {
        return extract64(cs->ich_vmcr_el2, ICH_VMCR_EL2_VBPR1_SHIFT, ICH_VMCR_EL2_VBPR1_LENGTH);
    }
}
...
static void write_vbpr(GICv3CPUState* cs, int grp, int value)
{
    ...
    if (grp == GICV3_G0) {
        cs->ich_vmcr_el2 = deposit64(cs->ich_vmcr_el2, ICH_VMCR_EL2_VBPR0_SHIFT, ICH_VMCR_EL2_VBPR0_LENGTH, value);
    }
    else {
        cs->ich_vmcr_el2 = deposit64(cs->ich_vmcr_el2, ICH_VMCR_EL2_VBPR1_SHIFT, ICH_VMCR_EL2_VBPR1_LENGTH, value);
    }
}
...
static int ich_highest_active_virt_prio(GICv3CPUState* cs)
{
    ...
    if (cs->ich_apr[GICV3_G1NS][0] & ICV_AP1R_EL1_NMI) { return 0x0; }

    for (i = 0; i < aprmax; i++) {
        uint32_t apr = cs->ich_apr[GICV3_G0][i] | cs->ich_apr[GICV3_G1NS][i];
...
static int hppvi_index(GICv3CPUState* cs)
{
    ...
    for (i = 0; i < cs->num_list_regs; i++) {
        ...
        if (lr & ICH_LR_EL2_GROUP) {
            if (!(cs->ich_vmcr_el2 & ICH_VMCR_EL2_VENG1)) { continue; }
        }
        else {
            if (!(cs->ich_vmcr_el2 & ICH_VMCR_EL2_VENG0)) { continue; }
        }
    ...
    if (cs->hppvlpi.prio < prio && !arm_is_secure(env)) {
        if (cs->hppvlpi.grp == GICV3_G0) {
            if (cs->ich_vmcr_el2 & ICH_VMCR_EL2_VENG0) { return HPPVI_INDEX_VLPI; }
        }
        else {
            if (cs->ich_vmcr_el2 & ICH_VMCR_EL2_VENG1) { return HPPVI_INDEX_VLPI; }
        }
    }
...
static uint32_t icv_gprio_mask(GICv3CPUState* cs, int group)
{
    ...
    if (group == GICV3_G1NS && cs->ich_vmcr_el2 & ICH_VMCR_EL2_VCBPR) { group = GICV3_G0; }
...
static bool icv_hppi_can_preempt(GICv3CPUState* cs, uint64_t lr)
{
    ...
    vpmr   = extract64(cs->ich_vmcr_el2, ICH_VMCR_EL2_VPMR_SHIFT, ICH_VMCR_EL2_VPMR_LENGTH);
     ...
     */
    if ((prio & mask) < (rprio & mask)) { return true; }

    if ((prio & mask) == (rprio & mask) && is_nmi && !(cs->ich_apr[GICV3_G1NS][0] & ICV_AP1R_EL1_NMI)) { return true; }
...
static bool icv_hppvlpi_can_preempt(GICv3CPUState* cs)
{
     ...
     * group priority is sufficient (the subpriorities are not considered).
     */
    if ((cs->hppvlpi.prio & mask) < (rprio & mask)) { return true; }

    return false;
}

static uint32_t eoi_maintenance_interrupt_state(GICv3CPUState* cs, uint32_t* misr)
{
    ...
    for (i = 0; i < cs->num_list_regs; i++) {
        ...
        if ((lr & (ICH_LR_EL2_STATE_MASK | ICH_LR_EL2_HW | ICH_LR_EL2_EOI)) == ICH_LR_EL2_EOI) { value |= (1 << i); }
        if ((lr & ICH_LR_EL2_STATE_MASK)) { validcount++; }
        if (ich_lr_state(lr) == ICH_LR_EL2_STATE_PENDING) { seenpending = true; }
    }

    if (misr) {
        if (validcount < 2 && (cs->ich_hcr_el2 & ICH_HCR_EL2_UIE)) { *misr |= ICH_MISR_EL2_U; }
        if (!seenpending && (cs->ich_hcr_el2 & ICH_HCR_EL2_NPIE)) { *misr |= ICH_MISR_EL2_NP; }
        if (value) { *misr |= ICH_MISR_EL2_EOI; }
    }
    return value;
}
...
static uint32_t maintenance_interrupt_state(GICv3CPUState* cs)
{
    ...
    if ((cs->ich_hcr_el2 & ICH_HCR_EL2_VGRP0EIE) && (cs->ich_vmcr_el2 & ICH_VMCR_EL2_VENG0)) {
        value |= ICH_MISR_EL2_VGRP0E;
    }

    if ((cs->ich_hcr_el2 & ICH_HCR_EL2_VGRP0DIE) && !(cs->ich_vmcr_el2 & ICH_VMCR_EL2_VENG1)) {
        value |= ICH_MISR_EL2_VGRP0D;
    }
    if ((cs->ich_hcr_el2 & ICH_HCR_EL2_VGRP1EIE) && (cs->ich_vmcr_el2 & ICH_VMCR_EL2_VENG1)) {
        value |= ICH_MISR_EL2_VGRP1E;
    }

    if ((cs->ich_hcr_el2 & ICH_HCR_EL2_VGRP1DIE) && !(cs->ich_vmcr_el2 & ICH_VMCR_EL2_VENG1)) {
...
void gicv3_cpuif_virt_irq_fiq_update(GICv3CPUState* cs)
{
    ...
    idx = hppvi_index(cs);
    trace_gicv3_cpuif_virt_update(gicv3_redist_affid(cs), idx, cs->hppvlpi.irq, cs->hppvlpi.grp, cs->hppvlpi.prio);
...
static void gicv3_cpuif_virt_update(GICv3CPUState* cs)
{
    ...
    if ((cs->ich_hcr_el2 & ICH_HCR_EL2_EN) && maintenance_interrupt_state(cs) != 0) { maintlevel = 1; }
...
static void icv_ap_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    GICv3CPUState* cs    = icc_cs_from_env(env);
    int            regno = ri->opc2 & 3;
    int            grp   = (ri->crm & 1) ? GICV3_G1NS : GICV3_G0;
    ...
    if (cs->nmi_support) { cs->ich_apr[grp][regno] = value & (0xFFFFFFFFU | ICV_AP1R_EL1_NMI); }
    else {
...
static void icv_bpr_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    GICv3CPUState* cs  = icc_cs_from_env(env);
    int            grp = (ri->crm == 8) ? GICV3_G0 : GICV3_G1NS;

    trace_gicv3_icv_bpr_write(ri->crm == 8 ? 0 : 1, gicv3_redist_affid(cs), value);
...
static uint64_t icv_pmr_read(CPUARMState* env, const ARMCPRegInfo* ri)
{
    ...
    value = extract64(cs->ich_vmcr_el2, ICH_VMCR_EL2_VPMR_SHIFT, ICH_VMCR_EL2_VPMR_LENGTH);

    trace_gicv3_icv_pmr_read(gicv3_redist_affid(cs), value);
    return value;
}

static void icv_pmr_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    GICv3CPUState* cs = icc_cs_from_env(env);
    ...
    cs->ich_vmcr_el2 = deposit64(cs->ich_vmcr_el2, ICH_VMCR_EL2_VPMR_SHIFT, ICH_VMCR_EL2_VPMR_LENGTH, value);
...
static uint64_t icv_igrpen_read(CPUARMState* env, const ARMCPRegInfo* ri)
{
    ...
    trace_gicv3_icv_igrpen_read(ri->opc2 & 1 ? 1 : 0, gicv3_redist_affid(cs), value);
    return value;
}

static void icv_igrpen_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    GICv3CPUState* cs = icc_cs_from_env(env);
    int            enbit;

    trace_gicv3_icv_igrpen_write(ri->opc2 & 1 ? 1 : 0, gicv3_redist_affid(cs), value);
...
static uint64_t icv_ctlr_read(CPUARMState* env, const ARMCPRegInfo* ri)
{
     ...
     */
    value = ICC_CTLR_EL1_A3V | (1 << ICC_CTLR_EL1_IDBITS_SHIFT) | ((cs->vpribits - 1) << ICC_CTLR_EL1_PRIBITS_SHIFT);

    if (cs->ich_vmcr_el2 & ICH_VMCR_EL2_VEOIM) { value |= ICC_CTLR_EL1_EOIMODE; }

    if (cs->ich_vmcr_el2 & ICH_VMCR_EL2_VCBPR) { value |= ICC_CTLR_EL1_CBPR; }

    trace_gicv3_icv_ctlr_read(gicv3_redist_affid(cs), value);
    return value;
}

static void icv_ctlr_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    GICv3CPUState* cs = icc_cs_from_env(env);

    trace_gicv3_icv_ctlr_write(gicv3_redist_affid(cs), value);

    cs->ich_vmcr_el2 = deposit64(cs->ich_vmcr_el2, ICH_VMCR_EL2_VCBPR_SHIFT, 1, value & ICC_CTLR_EL1_CBPR ? 1 : 0);
    cs->ich_vmcr_el2 = deposit64(cs->ich_vmcr_el2, ICH_VMCR_EL2_VEOIM_SHIFT, 1, value & ICC_CTLR_EL1_EOIMODE ? 1 : 0);
...
static uint64_t icv_rpr_read(CPUARMState* env, const ARMCPRegInfo* ri)
{
    ...
    if (cs->ich_apr[GICV3_G1NS][0] & ICV_AP1R_EL1_NMI) { prio |= ICV_RPR_EL1_NMI; }
...
static uint64_t icv_hppir_read(CPUARMState* env, const ARMCPRegInfo* ri)
{
    ...
    if (idx == HPPVI_INDEX_VLPI) {
        if (cs->hppvlpi.grp == grp) { value = cs->hppvlpi.irq; }
    }
    else if (idx >= 0) {
        uint64_t lr      = cs->ich_lr_el2[idx];
        int      thisgrp = (lr & ICH_LR_EL2_GROUP) ? GICV3_G1NS : GICV3_G0;

        if (grp == thisgrp) { value = ich_lr_vintid(lr); }
    }

    trace_gicv3_icv_hppir_read(ri->crm == 8 ? 0 : 1, gicv3_redist_affid(cs), value);
    return value;
}
...
static void icv_activate_irq(GICv3CPUState* cs, int idx, int grp)
{
    ...
    if (nmi) { cs->ich_apr[grp][regno] |= ICV_AP1R_EL1_NMI; }
    else {
...
static uint64_t icv_iar_read(CPUARMState* env, const ARMCPRegInfo* ri)
{
    ...
    if (idx == HPPVI_INDEX_VLPI) {
        ...
        if (thisgrp == grp && icv_hppi_can_preempt(cs, lr)) {
            ...
            if (!gicv3_intid_is_special(intid)) {
                if (!nmi) { icv_activate_irq(cs, idx, grp); }
                else {
                    intid = INTID_NMI;
                }
            }
    ...
    trace_gicv3_icv_iar_read(ri->crm == 8 ? 0 : 1, gicv3_redist_affid(cs), intid);
...
static uint64_t icv_nmiar1_read(CPUARMState* env, const ARMCPRegInfo* ri)
{
    ...
    if (idx >= 0 && idx != HPPVI_INDEX_VLPI) {
        ...
        if ((thisgrp == GICV3_G1NS) && icv_hppi_can_preempt(cs, lr)) {
            intid = ich_lr_vintid(lr);
            if (!gicv3_intid_is_special(intid)) {
                if (lr & ICH_LR_EL2_NMI) { icv_activate_irq(cs, idx, GICV3_G1NS); }
                else {
...
static inline int icc_min_bpr_ns(GICv3CPUState* cs) { return icc_min_bpr(cs) + 1; }
...
static int icc_highest_active_prio(GICv3CPUState* cs)
{
    ...
    if (cs->nmi_support) {
         ...
         */
        if (cs->icc_apr[GICV3_G1][0] & ICC_AP1R_EL1_NMI) { return 0; }
        if (cs->icc_apr[GICV3_G1NS][0] & ICC_AP1R_EL1_NMI) { return (cs->gic->gicd_ctlr & GICD_CTLR_DS) ? 0 : 0x80; }
    }

    for (i = 0; i < icc_num_aprs(cs); i++) {
        uint32_t apr = cs->icc_apr[GICV3_G0][i] | cs->icc_apr[GICV3_G1][i] | cs->icc_apr[GICV3_G1NS][i];
...
static uint32_t icc_gprio_mask(GICv3CPUState* cs, int group)
{
    ...
    if ((group == GICV3_G1 && cs->icc_ctlr_el1[GICV3_S] & ICC_CTLR_EL1_CBPR)
        || (group == GICV3_G1NS && cs->icc_ctlr_el1[GICV3_NS] & ICC_CTLR_EL1_CBPR))
    {
        group = GICV3_G0;
    }
...
static bool icc_hppi_can_preempt(GICv3CPUState* cs)
{
    ...
    if (cs->hppi.nmi) {
        if (!(cs->gic->gicd_ctlr & GICD_CTLR_DS) && cs->hppi.grp == GICV3_G1NS) {
            if (cs->icc_pmr_el1 < 0x80) { return false; }
            if (arm_is_secure(env) && cs->icc_pmr_el1 == 0x80) { return false; }
        }
    }
    ...
    if (cs->hppi.nmi && (cs->hppi.prio & mask) == (rprio & mask)) {
        if (!(cs->icc_apr[cs->hppi.grp][0] & ICC_AP1R_EL1_NMI)) { return true; }
    }
...
void gicv3_cpuif_update(GICv3CPUState* cs)
{
    ...
    trace_gicv3_cpuif_update(gicv3_redist_affid(cs), cs->hppi.irq, cs->hppi.grp, cs->hppi.prio);
...
static uint64_t icc_pmr_read(CPUARMState* env, const ARMCPRegInfo* ri)
{
    ...
    if (icv_access(env, HCR_FMO | HCR_IMO)) { return icv_pmr_read(env, ri); }

    if (arm_feature(env, ARM_FEATURE_EL3) && !arm_is_secure(env) && (env->cp15.scr_el3 & SCR_FIQ)) {
...
static void icc_pmr_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    GICv3CPUState* cs = icc_cs_from_env(env);

    if (icv_access(env, HCR_FMO | HCR_IMO)) { return icv_pmr_write(env, ri, value); }
...
static void icc_activate_irq(GICv3CPUState* cs, int irq)
{
    ...
    if (nmi) { cs->icc_apr[cs->hppi.grp][regno] |= ICC_AP1R_EL1_NMI; }
    else {
...
static uint64_t icc_hppir0_value(GICv3CPUState* cs, CPUARMState* env)
{
    ...
    if (icc_no_enabled_hppi(cs)) { return INTID_SPURIOUS; }
     ...
     * is always zero.)
     */
    irq_is_secure = (!(cs->gic->gicd_ctlr & GICD_CTLR_DS) && (cs->hppi.grp != GICV3_G1NS));

    if (cs->hppi.grp != GICV3_G0 && !arm_is_el3_or_mon(env)) { return INTID_SPURIOUS; }
...
static uint64_t icc_iar0_read(CPUARMState* env, const ARMCPRegInfo* ri)
{
    ...
    if (!icc_hppi_can_preempt(cs)) { intid = INTID_SPURIOUS; }
    else {
        intid = icc_hppir0_value(cs, env);
    }

    if (!gicv3_intid_is_special(intid)) { icc_activate_irq(cs, intid); }
...
static uint64_t icc_iar1_read(CPUARMState* env, const ARMCPRegInfo* ri)
{
    ...
    if (icv_access(env, HCR_IMO)) { return icv_iar_read(env, ri); }
    ...
    if (!gicv3_intid_is_special(intid)) {
        if (cs->hppi.nmi && env->cp15.sctlr_el[el] & SCTLR_NMI) { intid = INTID_NMI; }
        else {
...
static uint64_t icc_nmiar1_read(CPUARMState* env, const ARMCPRegInfo* ri)
{
    ...
    if (icv_access(env, HCR_IMO)) { return icv_nmiar1_read(env, ri); }
    ...
    if (!gicv3_intid_is_special(intid)) {
        if (!cs->hppi.nmi) { intid = INTID_SPURIOUS; }
        else {
            icc_activate_irq(cs, intid);
        }
    }
...
static bool icc_eoi_split(CPUARMState* env, GICv3CPUState* cs)
{
     ...
     */
    if (arm_is_el3_or_mon(env)) { return cs->icc_ctlr_el3 & ICC_CTLR_EL3_EOIMODE_EL3; }
    if (arm_is_secure_below_el3(env)) { return cs->icc_ctlr_el1[GICV3_S] & ICC_CTLR_EL1_EOIMODE; }
    else {
...
static int icc_highest_active_group(GICv3CPUState* cs)
{
    ...
    if (cs->nmi_support) {
        if (cs->icc_apr[GICV3_G1][0] & ICC_AP1R_EL1_NMI) { return GICV3_G1; }
        if (cs->icc_apr[GICV3_G1NS][0] & ICC_AP1R_EL1_NMI) { return GICV3_G1NS; }
    }
    ...
    for (i = 0; i < ARRAY_SIZE(cs->icc_apr[0]); i++) {
        ...
        if (g1nsctz < g0ctz && g1nsctz < g1ctz) { return GICV3_G1NS; }
        if (g1ctz < g0ctz) { return GICV3_G1; }
        if (g0ctz < 32) { return GICV3_G0; }
    }
...
static int icv_find_active(GICv3CPUState* cs, int irq)
{
    ...
    for (i = 0; i < cs->num_list_regs; i++) {
        ...
        if ((lr & ICH_LR_EL2_STATE_ACTIVE_BIT) && ich_lr_vintid(lr) == irq) { return i; }
    }
...
static void icv_deactivate_irq(GICv3CPUState* cs, int idx)
{
    ...
    if (lr & ICH_LR_EL2_HW) {
        ...
        if (pirq < INTID_SECURE) { icc_deactivate_irq(cs, pirq); }
    }
...
static void icv_increment_eoicount(GICv3CPUState* cs)
{
    ...
    int eoicount = extract64(cs->ich_hcr_el2, ICH_HCR_EL2_EOICOUNT_SHIFT, ICH_HCR_EL2_EOICOUNT_LENGTH);

    cs->ich_hcr_el2 = deposit64(cs->ich_hcr_el2, ICH_HCR_EL2_EOICOUNT_SHIFT, ICH_HCR_EL2_EOICOUNT_LENGTH, eoicount + 1);
}
...
static void icv_dir_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    ...
    if (!icv_eoi_split(env, cs)) { return; }
...
static void icv_eoir_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    /* End of Interrupt */
    GICv3CPUState* cs  = icc_cs_from_env(env);
    int            irq = value & 0xffffff;
    ...
    trace_gicv3_icv_eoir_write(ri->crm == 8 ? 0 : 1, gicv3_redist_affid(cs), value);

    if (gicv3_intid_is_special(irq)) { return; }
    ...
    if (idx < 0) {
         ...
         */
        if (irq < GICV3_LPI_INTID_START) { icv_increment_eoicount(cs); }
    }
    else {
        uint64_t lr       = cs->ich_lr_el2[idx];
        int      thisgrp  = (lr & ICH_LR_EL2_GROUP) ? GICV3_G1NS : GICV3_G0;
...
static void icc_eoir_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    /* End of Interrupt */
    GICv3CPUState* cs  = icc_cs_from_env(env);
    int            irq = value & 0xffffff;
    ...
    trace_gicv3_icc_eoir_write(is_eoir0 ? 0 : 1, gicv3_redist_affid(cs), value);

    if ((irq >= cs->gic->num_irq) && !(cs->gic->lpi_enable && (irq >= GICV3_LPI_INTID_START))) {
    ...
    switch (grp) {
        case GICV3_G0:
            if (!is_eoir0) { return; }
            if (!(cs->gic->gicd_ctlr & GICD_CTLR_DS) && arm_feature(env, ARM_FEATURE_EL3) && !arm_is_secure(env)) {
                return;
            }
            break;
        case GICV3_G1:
            if (is_eoir0) { return; }
            if (!arm_is_secure(env)) { return; }
            break;
        case GICV3_G1NS:
            if (is_eoir0) { return; }
            if (!arm_is_el3_or_mon(env) && arm_is_secure(env)) { return; }
            break;
        default: qemu_log_mask(LOG_GUEST_ERROR, "%s: IRQ %d isn't active\n", __func__, irq); return;
    }
...
static uint64_t icc_hppir0_read(CPUARMState* env, const ARMCPRegInfo* ri)
{
    ...
    if (icv_access(env, HCR_FMO)) { return icv_hppir_read(env, ri); }
...
static uint64_t icc_hppir1_read(CPUARMState* env, const ARMCPRegInfo* ri)
{
    ...
    if (icv_access(env, HCR_IMO)) { return icv_hppir_read(env, ri); }
...
static uint64_t icc_bpr_read(CPUARMState* env, const ARMCPRegInfo* ri)
{
    ...
    if (icv_access(env, grp == GICV3_G0 ? HCR_FMO : HCR_IMO)) { return icv_bpr_read(env, ri); }

    if (grp == GICV3_G1 && gicv3_use_ns_bank(env)) { grp = GICV3_G1NS; }

    if (grp == GICV3_G1 && !arm_is_el3_or_mon(env) && (cs->icc_ctlr_el1[GICV3_S] & ICC_CTLR_EL1_CBPR)) {
    ...
    if (grp == GICV3_G1NS && arm_current_el(env) < 3 && (cs->icc_ctlr_el1[GICV3_NS] & ICC_CTLR_EL1_CBPR)) {
...
static void icc_bpr_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    GICv3CPUState* cs  = icc_cs_from_env(env);
    int            grp = (ri->crm == 8) ? GICV3_G0 : GICV3_G1;
    uint64_t       minval;
    ...
    trace_gicv3_icc_bpr_write(ri->crm == 8 ? 0 : 1, gicv3_redist_affid(cs), value);
...
static uint64_t icc_ap_read(CPUARMState* env, const ARMCPRegInfo* ri)
{
    ...
    if (icv_access(env, grp == GICV3_G0 ? HCR_FMO : HCR_IMO)) { return icv_ap_read(env, ri); }
...
static void icc_ap_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    GICv3CPUState* cs = icc_cs_from_env(env);
     ...
     */
    if (grp == GICV3_G1NS && regno < 2 && arm_feature(env, ARM_FEATURE_EL3)) { return; }

    if (cs->nmi_support) { cs->icc_apr[grp][regno] = value & (0xFFFFFFFFU | ICC_AP1R_EL1_NMI); }
    else {
...
static void icc_dir_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    /* Deactivate interrupt */
    GICv3CPUState* cs  = icc_cs_from_env(env);
    int            irq = value & 0xffffff;
    ...
    if (!icc_eoi_split(env, cs)) { return; }
    ...
    switch (arm_current_el(env)) {
        case 3: break;
        case 2:
            if (single_sec_state && irq_is_grp0 && !route_fiq_to_el3) { break; }
            if (!irq_is_secure && !irq_is_grp0 && !route_irq_to_el3) { break; }
            return;
        case 1:
            if (!arm_is_secure_below_el3(env)) {
                if (single_sec_state && irq_is_grp0 && !route_fiq_to_el3 && !route_fiq_to_el2) { break; }
                if (!irq_is_secure && !irq_is_grp0 && !route_irq_to_el3 && !route_irq_to_el2) { break; }
            }
            else {
                if (irq_is_grp0 && !route_fiq_to_el3) { break; }
                if (!irq_is_grp0 && (!irq_is_secure || !single_sec_state) && !route_irq_to_el3) { break; }
            }
            return;
        default: assert_not_reached();
    }
...
static uint64_t icc_rpr_read(CPUARMState* env, const ARMCPRegInfo* ri)
{
    ...
    if (icv_access(env, HCR_FMO | HCR_IMO)) { return icv_rpr_read(env, ri); }
    ...
    if (cs->nmi_support) {
        ...
        if (arm_feature(env, ARM_FEATURE_EL3) && !arm_is_secure(env)) {
            if (cs->icc_apr[GICV3_G1NS][0] & ICC_AP1R_EL1_NMI) { prio |= ICC_RPR_EL1_NMI; }
        }
        else {
            if (cs->icc_apr[GICV3_G1NS][0] & ICC_AP1R_EL1_NMI) { prio |= ICC_RPR_EL1_NSNMI; }
            if (cs->icc_apr[GICV3_G1][0] & ICC_AP1R_EL1_NMI) { prio |= ICC_RPR_EL1_NMI; }
        }
    }
...
static void icc_generate_sgi(CPUARMState* env, GICv3CPUState* cs, uint64_t value, int grp, bool ns)
{
    GICv3State* s = cs->gic;

    /* Extract Aff3/Aff2/Aff1 and shift into the bottom 24 bits */
    uint64_t aff        = extract64(value, 48, 8) << 16 | extract64(value, 32, 8) << 8 | extract64(value, 16, 8);
    ...
    trace_gicv3_icc_generate_sgi(gicv3_redist_affid(cs), irq, irm, aff, targetlist);
    ...
    for (i = 0; i < s->num_cpu; i++) {
        ...
        if (irm) {
            ...
            if (ocs->gicr_typer >> 40 != aff) { continue; }
            aff0 = extract64(ocs->gicr_typer, 32, 8);
            if (aff0 > 15 || extract32(targetlist, aff0, 1) == 0) { continue; }
        }
...
static void icc_sgi0r_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
...
static void icc_sgi1r_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
...
static void icc_asgi1r_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
...
static uint64_t icc_igrpen_read(CPUARMState* env, const ARMCPRegInfo* ri)
{
    ...
    if (icv_access(env, grp == GICV3_G0 ? HCR_FMO : HCR_IMO)) { return icv_igrpen_read(env, ri); }

    if (grp == GICV3_G1 && gicv3_use_ns_bank(env)) { grp = GICV3_G1NS; }

    value = cs->icc_igrpen[grp];
    trace_gicv3_icc_igrpen_read(ri->opc2 & 1 ? 1 : 0, gicv3_redist_affid(cs), value);
    return value;
}

static void icc_igrpen_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    GICv3CPUState* cs  = icc_cs_from_env(env);
    int            grp = ri->opc2 & 1 ? GICV3_G1 : GICV3_G0;
    ...
    trace_gicv3_icc_igrpen_write(ri->opc2 & 1 ? 1 : 0, gicv3_redist_affid(cs), value);
...
static void icc_igrpen1_el3_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    GICv3CPUState* cs = icc_cs_from_env(env);
...
static uint64_t icc_ctlr_el1_read(CPUARMState* env, const ARMCPRegInfo* ri)
{
    ...
    if (icv_access(env, HCR_FMO | HCR_IMO)) { return icv_ctlr_read(env, ri); }
...
static void icc_ctlr_el1_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    GICv3CPUState* cs   = icc_cs_from_env(env);
    int            bank = gicv3_use_ns_bank(env) ? GICV3_NS : GICV3_S;
    uint64_t       mask;
     ...
     */
    if (arm_feature(env, ARM_FEATURE_EL3) && ((cs->gic->gicd_ctlr & GICD_CTLR_DS) == 0)) {
...
static uint64_t icc_ctlr_el3_read(CPUARMState* env, const ARMCPRegInfo* ri)
{
    ...
    if (cs->icc_ctlr_el1[GICV3_NS] & ICC_CTLR_EL1_EOIMODE) { value |= ICC_CTLR_EL3_EOIMODE_EL1NS; }
    if (cs->icc_ctlr_el1[GICV3_NS] & ICC_CTLR_EL1_CBPR) { value |= ICC_CTLR_EL3_CBPR_EL1NS; }
    if (cs->icc_ctlr_el1[GICV3_NS] & ICC_CTLR_EL1_EOIMODE) { value |= ICC_CTLR_EL3_EOIMODE_EL1S; }
    if (cs->icc_ctlr_el1[GICV3_NS] & ICC_CTLR_EL1_CBPR) { value |= ICC_CTLR_EL3_CBPR_EL1S; }

    trace_gicv3_icc_ctlr_el3_read(gicv3_redist_affid(cs), value);
    return value;
}

static void icc_ctlr_el3_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    GICv3CPUState* cs = icc_cs_from_env(env);
    uint64_t       mask;
    ...
    if (value & ICC_CTLR_EL3_EOIMODE_EL1NS) { cs->icc_ctlr_el1[GICV3_NS] |= ICC_CTLR_EL1_EOIMODE; }
    if (value & ICC_CTLR_EL3_CBPR_EL1NS) { cs->icc_ctlr_el1[GICV3_NS] |= ICC_CTLR_EL1_CBPR; }

    cs->icc_ctlr_el1[GICV3_S] &= ~(ICC_CTLR_EL1_CBPR | ICC_CTLR_EL1_EOIMODE);
    if (value & ICC_CTLR_EL3_EOIMODE_EL1S) { cs->icc_ctlr_el1[GICV3_S] |= ICC_CTLR_EL1_EOIMODE; }
    if (value & ICC_CTLR_EL3_CBPR_EL1S) { cs->icc_ctlr_el1[GICV3_S] |= ICC_CTLR_EL1_CBPR; }
...
static CPAccessResult gicv3_irqfiq_access(CPUARMState* env, const ARMCPRegInfo* ri, bool isread)
{
    CPAccessResult r  = CP_ACCESS_OK;
    GICv3CPUState* cs = icc_cs_from_env(env);
    int            el = arm_current_el(env);

    if ((cs->ich_hcr_el2 & ICH_HCR_EL2_TC) && el == 1 && !arm_is_secure_below_el3(env)) {
    ...
    if ((env->cp15.scr_el3 & (SCR_FIQ | SCR_IRQ)) == (SCR_FIQ | SCR_IRQ)) {
        switch (el) {
            case 1:
                ...
                if ((arm_hcr_el2_eff(env) & (HCR_IMO | HCR_FMO)) == 0) { r = CP_ACCESS_TRAP_EL3; }
                break;
            case 2: r = CP_ACCESS_TRAP_EL3; break;
            case 3:
                if (!arm_is_el3_or_mon(env)) { r = CP_ACCESS_TRAP_EL3; }
                break;
            default: assert_not_reached();
        }
    }
...
static CPAccessResult gicv3_dir_access(CPUARMState* env, const ARMCPRegInfo* ri, bool isread)
{
    GICv3CPUState* cs = icc_cs_from_env(env);

    if ((cs->ich_hcr_el2 & ICH_HCR_EL2_TDIR) && arm_current_el(env) == 1 && !arm_is_secure_below_el3(env)) {
        /* Takes priority over a possible EL3 trap */
        return CP_ACCESS_TRAP_EL2;
    }
...
static CPAccessResult gicv3_sgi_access(CPUARMState* env, const ARMCPRegInfo* ri, bool isread)
{
    if (arm_current_el(env) == 1 && (arm_hcr_el2_eff(env) & (HCR_IMO | HCR_FMO)) != 0) {
        /* Takes priority over a possible EL3 trap */
        return CP_ACCESS_TRAP_EL2;
    }
...
static CPAccessResult gicv3_fiq_access(CPUARMState* env, const ARMCPRegInfo* ri, bool isread)
{
    CPAccessResult r  = CP_ACCESS_OK;
    GICv3CPUState* cs = icc_cs_from_env(env);
    int            el = arm_current_el(env);

    if ((cs->ich_hcr_el2 & ICH_HCR_EL2_TALL0) && el == 1 && !arm_is_secure_below_el3(env)) {
        /* Takes priority over a possible EL3 trap */
        return CP_ACCESS_TRAP_EL2;
    }
    ...
    if (env->cp15.scr_el3 & SCR_FIQ) {
        switch (el) {
            case 1:
                if ((arm_hcr_el2_eff(env) & HCR_FMO) == 0) { r = CP_ACCESS_TRAP_EL3; }
                break;
            case 2: r = CP_ACCESS_TRAP_EL3; break;
            case 3:
                if (!arm_is_el3_or_mon(env)) { r = CP_ACCESS_TRAP_EL3; }
...
static CPAccessResult gicv3_irq_access(CPUARMState* env, const ARMCPRegInfo* ri, bool isread)
{
    CPAccessResult r  = CP_ACCESS_OK;
    GICv3CPUState* cs = icc_cs_from_env(env);
    int            el = arm_current_el(env);

    if ((cs->ich_hcr_el2 & ICH_HCR_EL2_TALL1) && el == 1 && !arm_is_secure_below_el3(env)) {
        /* Takes priority over a possible EL3 trap */
        return CP_ACCESS_TRAP_EL2;
    }
    ...
    if (env->cp15.scr_el3 & SCR_IRQ) {
        switch (el) {
            case 1:
                if ((arm_hcr_el2_eff(env) & HCR_IMO) == 0) { r = CP_ACCESS_TRAP_EL3; }
                break;
            case 2: r = CP_ACCESS_TRAP_EL3; break;
            case 3:
                if (!arm_is_el3_or_mon(env)) { r = CP_ACCESS_TRAP_EL3; }
...
static void icc_reset(CPUARMState* env, const ARMCPRegInfo* ri)
{
    ...
    cs->icc_ctlr_el1[GICV3_S] =
        ICC_CTLR_EL1_A3V | (1 << ICC_CTLR_EL1_IDBITS_SHIFT) | ((cs->pribits - 1) << ICC_CTLR_EL1_PRIBITS_SHIFT);
    cs->icc_ctlr_el1[GICV3_NS] =
        ICC_CTLR_EL1_A3V | (1 << ICC_CTLR_EL1_IDBITS_SHIFT) | ((cs->pribits - 1) << ICC_CTLR_EL1_PRIBITS_SHIFT);
    cs->icc_pmr_el1         = 0;
    ...
    cs->icc_ctlr_el3 = ICC_CTLR_EL3_NDS | ICC_CTLR_EL3_A3V | (1 << ICC_CTLR_EL3_IDBITS_SHIFT)
                       | ((cs->pribits - 1) << ICC_CTLR_EL3_PRIBITS_SHIFT);

    memset(cs->ich_apr, 0, sizeof(cs->ich_apr));
    cs->ich_hcr_el2 = 0;
    memset(cs->ich_lr_el2, 0, sizeof(cs->ich_lr_el2));
    cs->ich_vmcr_el2 = ICH_VMCR_EL2_VFIQEN | ((icv_min_vbpr(cs) + 1) << ICH_VMCR_EL2_VBPR1_SHIFT)
...
static void ich_ap_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    GICv3CPUState* cs    = icc_cs_from_env(env);
    int            regno = ri->opc2 & 3;
    int            grp   = (ri->crm & 1) ? GICV3_G1NS : GICV3_G0;
...
static void ich_hcr_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    GICv3CPUState* cs = icc_cs_from_env(env);

    trace_gicv3_ich_hcr_write(gicv3_redist_affid(cs), value);

    value &= ICH_HCR_EL2_EN | ICH_HCR_EL2_UIE | ICH_HCR_EL2_LRENPIE | ICH_HCR_EL2_NPIE | ICH_HCR_EL2_VGRP0EIE
             | ICH_HCR_EL2_VGRP0DIE | ICH_HCR_EL2_VGRP1EIE | ICH_HCR_EL2_VGRP1DIE | ICH_HCR_EL2_TC | ICH_HCR_EL2_TALL0
             | ICH_HCR_EL2_TALL1 | ICH_HCR_EL2_TSEI | ICH_HCR_EL2_TDIR | ICH_HCR_EL2_EOICOUNT_MASK;
...
static void ich_vmcr_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    GICv3CPUState* cs = icc_cs_from_env(env);

    trace_gicv3_ich_vmcr_write(gicv3_redist_affid(cs), value);

    value &= ICH_VMCR_EL2_VENG0 | ICH_VMCR_EL2_VENG1 | ICH_VMCR_EL2_VCBPR | ICH_VMCR_EL2_VEOIM | ICH_VMCR_EL2_VBPR1_MASK
...
static void ich_lr_write(CPUARMState* env, const ARMCPRegInfo* ri, uint64_t value)
{
    GICv3CPUState* cs    = icc_cs_from_env(env);
    int            regno = ri->opc2 | ((ri->crm & 1) << 3);
    ...
    if (cs->vpribits < 8) { value = deposit64(value, ICH_LR_EL2_PRIORITY_SHIFT, 8 - cs->vpribits, 0); }

    /* Enforce RES0 bit in NMI field when FEAT_GICv3_NMI is not implemented */
    if (!cs->nmi_support) { value &= ~ICH_LR_EL2_NMI; }
...
static uint64_t ich_vtr_read(CPUARMState* env, const ARMCPRegInfo* ri)
{
    ...
    value = ((cs->num_list_regs - 1) << ICH_VTR_EL2_LISTREGS_SHIFT) | ICH_VTR_EL2_TDS | ICH_VTR_EL2_A3V
            | (1 << ICH_VTR_EL2_IDBITS_SHIFT) | ((cs->vprebits - 1) << ICH_VTR_EL2_PREBITS_SHIFT)
            | ((cs->vpribits - 1) << ICH_VTR_EL2_PRIBITS_SHIFT);

    if (cs->gic->revision < 4) { value |= ICH_VTR_EL2_NV4; }
...
static uint64_t ich_elrsr_read(CPUARMState* env, const ARMCPRegInfo* ri)
{
    ...
    for (i = 0; i < cs->num_list_regs; i++) {
        ...
        if ((lr & ICH_LR_EL2_STATE_MASK) == 0 && ((lr & ICH_LR_EL2_HW) != 0 || (lr & ICH_LR_EL2_EOI) == 0)) {
            value |= (1 << i);
        }
    }
...
void gicv3_init_cpuif(GICv3State* s)
{
    ...
    for (i = 0; i < s->num_cpu; i++) {
         ...
         */
        if (s->force_8bit_prio) { cs->pribits = 8; }
        else {
         ...
         */
        if (cs->prebits >= 6) { define_arm_cp_regs(cpu, gicv3_cpuif_icc_apxr1_reginfo); }
        if (cs->prebits == 7) { define_arm_cp_regs(cpu, gicv3_cpuif_icc_apxr23_reginfo); }
        ...
        if (arm_feature(&cpu->env, ARM_FEATURE_EL2)) {
            ...
            if (cs->vprebits >= 6) { define_arm_cp_regs(cpu, gicv3_cpuif_ich_apxr1_reginfo); }
            if (cs->vprebits == 7) { define_arm_cp_regs(cpu, gicv3_cpuif_ich_apxr23_reginfo); }
        }
        if (tcg_enabled()) {
            /*
