...
#define DPRINTF(fmt, ...)                                                             \
    do {                                                                              \
        if (DEBUG_GIC_GATE) { fprintf(stderr, "%s: " fmt, __func__, ##__VA_ARGS__); } \
    }                                                                                 \
    while (0)

static const uint8_t gic_id_11mpcore[] = {0x00, 0x00, 0x00, 0x00, 0x90, 0x13, 0x04, 0x00, 0x0d, 0xf0, 0x05, 0xb1};

static const uint8_t gic_id_gicv1[] = {0x04, 0x00, 0x00, 0x00, 0x90, 0xb3, 0x1b, 0x00, 0x0d, 0xf0, 0x05, 0xb1};

static const uint8_t gic_id_gicv2[] = {0x04, 0x00, 0x00, 0x00, 0x90, 0xb4, 0x2b, 0x00, 0x0d, 0xf0, 0x05, 0xb1};

static inline int gic_get_current_cpu(GICState* s)
{
    if (s->num_cpu > 1) { return current_cpu->cpu_index - s->first_cpu_index; }
    return 0;
}

static inline int gic_get_current_vcpu(GICState* s) { return gic_get_current_cpu(s) + GIC_NCPU; }

/* Return true if this GIC config has interrupt groups, which is
 * true if we're a GICv2, or a GICv1 with the security extensions.
 */
static inline bool gic_has_groups(GICState* s) { return s->revision == 2 || s->security_extn; }
...
static inline void gic_get_best_irq(GICState* s, int cpu, int* best_irq, int* best_prio, int* group)
{
    int irq;
    int cm = 1 << cpu;
    ...
    *best_prio = 0x100;
    ...
    for (irq = 0; irq < s->num_irq; irq++) {
        if (GIC_DIST_TEST_ENABLED(irq, cm) && gic_test_pending(s, irq, cm) && (!GIC_DIST_TEST_ACTIVE(irq, cm))
    ...
    if (*best_irq < 1023) { *group = GIC_DIST_TEST_GROUP(*best_irq, cm); }
}

static inline void gic_get_best_virq(GICState* s, int cpu, int* best_irq, int* best_prio, int* group)
{
    int lr_idx = 0;
 ...
 *   groups.
 */
static inline bool gic_irq_signaling_enabled(GICState* s, int cpu, bool virt, int group_mask)
{
    int cpu_iface = virt ? (cpu + GIC_NCPU) : cpu;

    if (!virt && !(s->ctlr & group_mask)) { return false; }

    if (virt && !(s->h_hcr[cpu] & R_GICH_HCR_EN_MASK)) { return false; }

    if (!(s->cpu_ctlr[cpu_iface] & group_mask)) { return false; }
...
static inline void gic_update_internal(GICState* s, bool virt)
{
    ...
    for (cpu = 0; cpu < s->num_cpu; cpu++) {
        ...
        if (!gic_irq_signaling_enabled(s, cpu, virt, GICD_CTLR_EN_GRP0 | GICD_CTLR_EN_GRP1)) {
            qemu_irq_lower(irq_lines[cpu]);
            qemu_irq_lower(fiq_lines[cpu]);
            continue;
        }

        if (virt) { gic_get_best_virq(s, cpu, &best_irq, &best_prio, &group); }
        else {
            gic_get_best_irq(s, cpu, &best_irq, &best_prio, &group);
        }

        if (best_irq != 1023) {
            trace_gic_update_bestirq(virt ? "vcpu" : "cpu", cpu, best_irq, best_prio, s->priority_mask[cpu_iface],
        ...
        if (best_prio < s->priority_mask[cpu_iface]) {
            ...
            if (best_prio < s->running_priority[cpu_iface]) {
                if (gic_irq_signaling_enabled(s, cpu, virt, 1 << group)) {
                    if (group == 0 && s->cpu_ctlr[cpu_iface] & GICC_CTLR_FIQ_EN) {
                        DPRINTF("Raised pending FIQ %d (cpu %d)\n", best_irq, cpu_iface);
                        fiq_level = 1;
                        trace_gic_update_set_irq(cpu, virt ? "vfiq" : "fiq", fiq_level);
                    }
                    else {
                        DPRINTF("Raised pending IRQ %d (cpu %d)\n", best_irq, cpu_iface);
                        irq_level = 1;
                        trace_gic_update_set_irq(cpu, virt ? "virq" : "irq", irq_level);
                    }
                }
            }
        }
...
static void gic_update(GICState* s) { gic_update_internal(s, false); }

/* Return true if this LR is empty, i.e. the corresponding bit
 * in ELRSR is set.
 */
static inline bool gic_lr_entry_is_free(uint32_t entry)
{ return (GICH_LR_STATE(entry) == GICH_LR_STATE_INVALID) && (GICH_LR_HW(entry) || !GICH_LR_EOI(entry)); }
...
static inline void gic_extract_lr_info(GICState* s, int cpu, int* num_eoi, int* num_valid, int* num_pending)
{
    int lr_idx;
    ...
    *num_pending = 0;
    ...
    for (lr_idx = 0; lr_idx < s->num_lrs; lr_idx++) {
        ...
        if (gic_lr_entry_is_eoi(*entry)) { (*num_eoi)++; }

        if (GICH_LR_STATE(*entry) != GICH_LR_STATE_INVALID) { (*num_valid)++; }

        if (GICH_LR_STATE(*entry) == GICH_LR_STATE_PENDING) { (*num_pending)++; }
    }
}
...
static void gic_compute_misr(GICState* s, int cpu)
{
    ...
    /* EOI */
    if (num_eoi) { value |= R_GICH_MISR_EOI_MASK; }

    /* U: true if only 0 or 1 LR entry is valid */
    if ((s->h_hcr[cpu] & R_GICH_HCR_UIE_MASK) && (num_valid < 2)) { value |= R_GICH_MISR_U_MASK; }

    /* LRENP: EOICount is not 0 */
    if ((s->h_hcr[cpu] & R_GICH_HCR_LRENPIE_MASK) && ((s->h_hcr[cpu] & R_GICH_HCR_EOICount_MASK) != 0)) {
        value |= R_GICH_MISR_LRENP_MASK;
    }

    /* NP: no pending interrupts */
    if ((s->h_hcr[cpu] & R_GICH_HCR_NPIE_MASK) && (num_pending == 0)) { value |= R_GICH_MISR_NP_MASK; }

    /* VGrp0E: group0 virq signaling enabled */
    if ((s->h_hcr[cpu] & R_GICH_HCR_VGRP0EIE_MASK) && (s->cpu_ctlr[vcpu] & GICC_CTLR_EN_GRP0)) {
        value |= R_GICH_MISR_VGrp0E_MASK;
    }

    /* VGrp0D: group0 virq signaling disabled */
    if ((s->h_hcr[cpu] & R_GICH_HCR_VGRP0DIE_MASK) && !(s->cpu_ctlr[vcpu] & GICC_CTLR_EN_GRP0)) {
        value |= R_GICH_MISR_VGrp0D_MASK;
    }

    /* VGrp1E: group1 virq signaling enabled */
    if ((s->h_hcr[cpu] & R_GICH_HCR_VGRP1EIE_MASK) && (s->cpu_ctlr[vcpu] & GICC_CTLR_EN_GRP1)) {
        value |= R_GICH_MISR_VGrp1E_MASK;
    }

    /* VGrp1D: group1 virq signaling disabled */
    if ((s->h_hcr[cpu] & R_GICH_HCR_VGRP1DIE_MASK) && !(s->cpu_ctlr[vcpu] & GICC_CTLR_EN_GRP1)) {
...
static void gic_set_irq_11mpcore(GICState* s, int irq, int level, int cm, int target)
{
    if (level) {
...
static void gic_set_irq_generic(GICState* s, int irq, int level, int cm, int target)
{
    if (level) {
        GIC_DIST_SET_LEVEL(irq, cm);
        DPRINTF("Set %d pending mask %x\n", irq, target);
        if (GIC_DIST_TEST_EDGE_TRIGGER(irq)) { GIC_DIST_SET_PENDING(irq, target); }
    }
    else {
        GIC_DIST_CLEAR_LEVEL(irq, cm);
    }
...
static void gic_set_irq(void* opaque, int irq, int level)
{
    ...
    if (level == GIC_DIST_TEST_LEVEL(irq, cm)) { return; }

    if (s->revision == REV_11MPCORE) { gic_set_irq_11mpcore(s, irq, level, cm, target); }
    else {
...
static uint16_t gic_get_current_pending_irq(GICState* s, int cpu, MemTxAttrs attrs)
{
...
static int gic_get_group_priority(GICState* s, int cpu, int irq)
{
    ...
    if (gic_has_groups(s) && !(s->cpu_ctlr[cpu] & GICC_CTLR_CBPR) && gic_test_group(s, irq, cpu)) {
        bpr = s->abpr[cpu] - 1;
        assert(bpr >= 0);
    }
    else {
...
static int gic_get_prio_from_apr_bits(GICState* s, int cpu)
{
    ...
    if (gic_is_vcpu(cpu)) {
        ...
        if (apr) { return ctz32(apr) << (GIC_VIRT_MIN_BPR + 1); }
        else {
            return 0x100;
        }
    }
...
static inline uint32_t gic_clear_pending_sgi(GICState* s, int irq, int cpu)
{
    ...
    if (!gic_is_vcpu(cpu)) {
        ...
        if (s->sgi_pending[irq][cpu] == 0) { gic_clear_pending(s, irq, cpu); }
        ret = irq | ((src & 0x7) << 10);
    }
    else {
...
uint32_t gic_acknowledge_irq(GICState* s, int cpu, MemTxAttrs attrs)
{
    ...
    trace_gic_acknowledge_irq(gic_is_vcpu(cpu) ? "vcpu" : "cpu", gic_get_vcpu_real_id(cpu), irq);
    ...
    if (s->revision == REV_11MPCORE) {
        ...
        if (irq < GIC_NR_SGIS) { ret = gic_clear_pending_sgi(s, irq, cpu); }
        else {
            gic_clear_pending(s, irq, cpu);
            ret = irq;
        }
    ...
    if (gic_is_vcpu(cpu)) { gic_update_virt(s); }
    else {
        gic_update(s);
    }
    DPRINTF("ACK %d\n", irq);
...
static uint32_t gic_fullprio_mask(GICState* s, int cpu)
{
    ...
    if (gic_is_vcpu(cpu)) { priBits = GIC_VIRT_MAX_GROUP_PRIO_BITS; }
    else {
...
void gic_dist_set_priority(GICState* s, int cpu, int irq, uint8_t val, MemTxAttrs attrs)
{
    if (s->security_extn && !attrs.secure) {
        if (!GIC_DIST_TEST_GROUP(irq, (1 << cpu))) { return; /* Ignore Non-secure access of Group0 IRQ */ }
        val = 0x80 | (val >> 1); /* Non-secure view */
    }

    val &= gic_fullprio_mask(s, cpu);

    if (irq < GIC_INTERNAL) { s->priority1[irq][cpu] = val; }
    else {
        s->priority2[(irq)-GIC_INTERNAL] = val;
    }
}

static uint32_t gic_dist_get_priority(GICState* s, int cpu, int irq, MemTxAttrs attrs)
{
...
static void gic_set_priority_mask(GICState* s, int cpu, uint8_t pmask, MemTxAttrs attrs)
{
...
static void gic_set_cpu_control(GICState* s, int cpu, uint32_t value, MemTxAttrs attrs)
{
    uint32_t mask;
    ...
    if (gic_cpu_ns_access(s, cpu, attrs)) {
        ...
        if (s->revision == 2) { mask |= GICC_CTLR_EOIMODE_NS; }
        s->cpu_ctlr[cpu] &= ~mask;
        s->cpu_ctlr[cpu] |= (value << 1) & mask;
    }
    else {
        if (s->revision == 2) { mask = s->security_extn ? GICC_CTLR_V2_S_MASK : GICC_CTLR_V2_MASK; }
        else {
    ...
    DPRINTF("CPU Interface %d: Group0 Interrupts %sabled, "
            ...
            cpu, (s->cpu_ctlr[cpu] & GICC_CTLR_EN_GRP0) ? "En" : "Dis",
...
static bool gic_eoi_split(GICState* s, int cpu, MemTxAttrs attrs)
{
    ...
    if (gic_cpu_ns_access(s, cpu, attrs)) { return s->cpu_ctlr[cpu] & GICC_CTLR_EOIMODE_NS; }
...
static void gic_deactivate_irq(GICState* s, int cpu, int irq, MemTxAttrs attrs)
{
    ...
    if (!gic_eoi_split(s, cpu, attrs)) {
        ...
        qemu_log_mask(LOG_GUEST_ERROR, "gic_deactivate_irq: GICC_DIR write when EOIMode clear");
        return;
    }
...
static void gic_complete_irq(GICState* s, int cpu, int irq, MemTxAttrs attrs)
{
    ...
    if (s->running_priority[cpu] == 0x100) { return; /* No active IRQ.  */ }

    if (s->revision == REV_11MPCORE) {
        /* Mark level triggered interrupts as pending if they are still
           raised.  */
        if (!GIC_DIST_TEST_EDGE_TRIGGER(irq) && GIC_DIST_TEST_ENABLED(irq, cm) && GIC_DIST_TEST_LEVEL(irq, cm)
    ...
    if (!gic_eoi_split(s, cpu, attrs)) { gic_clear_active(s, irq, cpu); }
    gic_update(s);
}
...
static uint8_t gic_dist_readb(void* opaque, hwaddr offset, MemTxAttrs attrs)
{
    ...
    if (offset < 0x100) {
        ...
        if (offset >= 0x80) {
            ...
            if (!(s->security_extn && !attrs.secure) && gic_has_groups(s)) {
                ...
                for (i = 0; i < 8; i++) {
                    if (GIC_DIST_TEST_GROUP(irq + i, cm)) { res |= (1 << i); }
                }
            }
            return res;
        }
        ...
        res = 0;
        for (i = 0; i < 8; i++) {
            if (s->security_extn && !attrs.secure && !GIC_DIST_TEST_GROUP(irq + i, 1 << cpu)) {
                continue; /* Ignore Non-secure access of Group0 IRQ */
            }

            if (GIC_DIST_TEST_ENABLED(irq + i, cm)) { res |= (1 << i); }
        }
    }
        ...
        for (i = 0; i < 8; i++) {
            ...
            if (gic_test_pending(s, irq + i, mask)) { res |= (1 << i); }
        }
    }
    else if (offset < 0x400) {
        /* Interrupt Set/Clear Active.  */
        if (offset < 0x380) { irq = (offset - 0x300) * 8; }
        ...
        for (i = 0; i < 8; i++) {
            ...
            if (GIC_DIST_TEST_ACTIVE(irq + i, mask)) { res |= (1 << i); }
        }
    }
        ...
        if (s->num_cpu == 1 && s->revision != REV_11MPCORE) {
            ...
            irq = (offset - 0x800);
            if (irq >= s->num_irq) { goto bad_reg; }
            if (irq < 29 && s->revision == REV_11MPCORE) { res = 0; }
        ...
        for (i = 0; i < 4; i++) {
            ...
            if (GIC_DIST_TEST_MODEL(irq + i)) { res |= (1 << (i * 2)); }
            if (GIC_DIST_TEST_EDGE_TRIGGER(irq + i)) { res |= (2 << (i * 2)); }
        }
    }
        ...
        if (s->revision == REV_11MPCORE) { goto bad_reg; }
        ...
        if (s->security_extn && !attrs.secure && !GIC_DIST_TEST_GROUP(irq, 1 << cpu)) {
        ...
        if (offset & 3) { res = 0; }
            ...
            switch (s->revision) {
                case REV_11MPCORE: res = gic_id_11mpcore[(offset - 0xfd0) >> 2]; break;
                case 1           : res = gic_id_gicv1[(offset - 0xfd0) >> 2]; break;
                case 2           : res = gic_id_gicv2[(offset - 0xfd0) >> 2]; break;
                default          : res = 0;
            }
        }
    }
    ...
    return res;
bad_reg:
    qemu_log_mask(LOG_GUEST_ERROR, "gic_dist_readb: Bad offset %x\n", (int)offset);
    return 0;
}

static MemTxResult gic_dist_read(void* opaque, hwaddr offset, uint64_t* data, unsigned size, MemTxAttrs attrs)
{
    switch (size) {
        case 1: *data = gic_dist_readb(opaque, offset, attrs); break;
        case 2:
...
static void gic_dist_writeb(void* opaque, hwaddr offset, uint32_t value, MemTxAttrs attrs)
{
    GICState* s = (GICState*)opaque;
    int       irq;
    int       i;
    ...
    if (offset < 0x100) {
        ...
        for (i = 0; i < 8; i++) {
            if (value & (1 << i)) {
                int mask = (irq < GIC_INTERNAL) ? (1 << cpu) : GIC_DIST_TARGET(irq + i);
                int cm   = (irq < GIC_INTERNAL) ? (1 << cpu) : ALL_CPU_MASK;
                 ...
                 */
                if (s->revision == REV_11MPCORE && GIC_DIST_TEST_LEVEL(irq + i, mask)
        ...
        for (i = 0; i < 8; i++) {
            ...
            /* ??? This currently clears the pending bit for all CPUs, even
               ...
               correct behavior.  */
            if (value & (1 << i)) { GIC_DIST_CLEAR_PENDING(irq + i, ALL_CPU_MASK); }
        }
    }
    else if (offset < 0x380) {
        /* Interrupt Set Active.  */
        if (s->revision != 2) { goto bad_reg; }
        ...
        for (i = 0; i < 8; i++) {
            ...
            if (value & (1 << i)) { GIC_DIST_SET_ACTIVE(irq + i, cm); }
        }
    }
    else if (offset < 0x400) {
        ...
        for (i = 0; i < 8; i++) {
            ...
            if (value & (1 << i)) { GIC_DIST_CLEAR_ACTIVE(irq + i, cm); }
        }
    }
    else if (offset < 0x800) {
        /* Interrupt Priority.  */
        ...
        if (s->num_cpu != 1 || s->revision == REV_11MPCORE) {
            irq = (offset - 0x800);
            if (irq >= s->num_irq) { goto bad_reg; }
            if (irq < 29 && s->revision == REV_11MPCORE) { value = 0; }
            else if (irq < GIC_INTERNAL) {
                value = ALL_CPU_MASK;
            }
        ...
        for (i = 0; i < 4; i++) {
            ...
            if (s->revision == REV_11MPCORE) {
                if (value & (1 << (i * 2))) { GIC_DIST_SET_MODEL(irq + i); }
                else {
                    GIC_DIST_CLEAR_MODEL(irq + i);
                }
            }
            if (value & (2 << (i * 2))) { GIC_DIST_SET_EDGE_TRIGGER(irq + i); }
            else {
        ...
        if (!s->security_extn || attrs.secure || GIC_DIST_TEST_GROUP(irq, 1 << cpu)) {
            ...
            if (s->sgi_pending[irq][cpu] == 0) { GIC_DIST_CLEAR_PENDING(irq, 1 << cpu); }
        }
    }
    else if (offset < 0xf30) {
        /* GICD_SPENDSGIRn */
    ...
    return;
bad_reg:
    qemu_log_mask(LOG_GUEST_ERROR, "gic_dist_writeb: Bad offset %x\n", (int)offset);
}

static void gic_dist_writew(void* opaque, hwaddr offset, uint32_t value, MemTxAttrs attrs)
{
    gic_dist_writeb(opaque, offset, value & 0xff, attrs);
    gic_dist_writeb(opaque, offset + 1, value >> 8, attrs);
}

static void gic_dist_writel(void* opaque, hwaddr offset, uint32_t value, MemTxAttrs attrs)
{
    GICState* s = (GICState*)opaque;
    if (offset == 0xf00) {
        int cpu;
        ...
        irq = value & 0xf;
        switch ((value >> 24) & 3) {
            case 0: mask = (value >> 16) & ALL_CPU_MASK; break;
            case 1: mask = ALL_CPU_MASK ^ (1 << cpu); break;
            case 2: mask = 1 << cpu; break;
            default:
...
static MemTxResult gic_dist_write(void* opaque, hwaddr offset, uint64_t data, unsigned size, MemTxAttrs attrs)
{
    trace_gic_dist_write(offset, size, data);

    switch (size) {
        case 1 : gic_dist_writeb(opaque, offset, data, attrs); return MEMTX_OK;
        case 2 : gic_dist_writew(opaque, offset, data, attrs); return MEMTX_OK;
        case 4 : gic_dist_writel(opaque, offset, data, attrs); return MEMTX_OK;
        default: return MEMTX_ERROR;
    }
}
...
static inline uint32_t gic_apr_ns_view(GICState* s, int cpu, int regno)
{
    ...
    switch (GIC_MIN_BPR) {
        case 0:
            if (regno < 2) { return s->nsapr[regno + 2][cpu]; }
            break;
        case 1:
            if (regno == 0) { return s->nsapr[regno + 1][cpu]; }
            break;
        case 2:
            if (regno == 0) { return extract32(s->nsapr[0][cpu], 16, 16); }
            break;
        case 3:
            if (regno == 0) { return extract32(s->nsapr[0][cpu], 8, 8); }
            break;
...
static inline void gic_apr_write_ns_view(GICState* s, int cpu, int regno, uint32_t value)
{
    /* Write the Nonsecure view of GICC_APR<regno>. */
    switch (GIC_MIN_BPR) {
        case 0:
            if (regno < 2) { s->nsapr[regno + 2][cpu] = value; }
            break;
        case 1:
            if (regno == 0) { s->nsapr[regno + 1][cpu] = value; }
            break;
        case 2:
            if (regno == 0) { s->nsapr[0][cpu] = deposit32(s->nsapr[0][cpu], 16, 16, value); }
            break;
        case 3:
            if (regno == 0) { s->nsapr[0][cpu] = deposit32(s->nsapr[0][cpu], 8, 8, value); }
            break;
        default: assert_not_reached();
    }
}

static MemTxResult gic_cpu_read(GICState* s, int cpu, int offset, uint64_t* data, MemTxAttrs attrs)
{
    switch (offset) {
        case 0x00: /* Control */ *data = gic_get_cpu_control(s, cpu, attrs); break;
        case 0x04: /* Priority mask */ *data = gic_get_priority_mask(s, cpu, attrs); break;
        case 0x08: /* Binary Point */
            ...
            break;
        case 0x0c: /* Acknowledge */ *data = gic_acknowledge_irq(s, cpu, attrs); break;
        case 0x14: /* Running Priority */ *data = gic_get_running_priority(s, cpu, attrs); break;
        case 0x18: /* Highest Pending Interrupt */ *data = gic_get_current_pending_irq(s, cpu, attrs); break;
        case 0x1c: /* Aliased Binary Point */
             ...
             */
            if (!gic_has_groups(s) || (gic_cpu_ns_access(s, cpu, attrs))) { *data = 0; }
            else {
                *data = s->abpr[cpu];
            }
            break;
        ...
        case 0xdc: {
            ...
            if (regno >= nr_aprs || s->revision != 2) { *data = 0; }
        ...
        case 0xec: {
            ...
            if (regno >= GIC_NR_APRS || s->revision != 2 || !gic_has_groups(s) || gic_cpu_ns_access(s, cpu, attrs)
                || gic_is_vcpu(cpu))
            {
                *data = 0;
            }
        ...
        case 0xfc:
            ...
            break;
        default:
            qemu_log_mask(LOG_GUEST_ERROR, "gic_cpu_read: Bad offset %x\n", (int)offset);
            *data = 0;
            break;
    }

    trace_gic_cpu_read(gic_is_vcpu(cpu) ? "vcpu" : "cpu", gic_get_vcpu_real_id(cpu), offset, *data);
    return MEMTX_OK;
}

static MemTxResult gic_cpu_write(GICState* s, int cpu, int offset, uint32_t value, MemTxAttrs attrs)
{
    trace_gic_cpu_write(gic_is_vcpu(cpu) ? "vcpu" : "cpu", gic_get_vcpu_real_id(cpu), offset, value);

    switch (offset) {
        case 0x00: /* Control */ gic_set_cpu_control(s, cpu, value, attrs); break;
        case 0x04: /* Priority mask */ gic_set_priority_mask(s, cpu, value, attrs); break;
        case 0x08: /* Binary Point */
            if (gic_cpu_ns_access(s, cpu, attrs)) {
                if (s->cpu_ctlr[cpu] & GICC_CTLR_CBPR) {
                    /* WI when CBPR is 1 */
            ...
            break;
        case 0x10: /* End Of Interrupt */ gic_complete_irq(s, cpu, value & 0x3ff, attrs); return MEMTX_OK;
        case 0x1c: /* Aliased Binary Point */
        ...
        case 0xdc: {
            ...
            if (regno >= nr_aprs || s->revision != 2) { return MEMTX_OK; }
            if (gic_is_vcpu(cpu)) { s->h_apr[gic_get_vcpu_real_id(cpu)] = value; }
            else if (gic_cpu_ns_access(s, cpu, attrs)) {
                /* NS view of GICC_APR<n> is the top half of GIC_NSAPR<n> */
        ...
        case 0xec: {
            ...
            if (regno >= GIC_NR_APRS || s->revision != 2) { return MEMTX_OK; }
            if (gic_is_vcpu(cpu)) { return MEMTX_OK; }
            if (!gic_has_groups(s) || (gic_cpu_ns_access(s, cpu, attrs))) { return MEMTX_OK; }
...
static MemTxResult gic_thiscpu_read(void* opaque, hwaddr addr, uint64_t* data, unsigned size, MemTxAttrs attrs)
{
    GICState* s = (GICState*)opaque;
    return gic_cpu_read(s, gic_get_current_cpu(s), addr, data, attrs);
}

static MemTxResult gic_thiscpu_write(void* opaque, hwaddr addr, uint64_t value, unsigned size, MemTxAttrs attrs)
{
    GICState* s = (GICState*)opaque;
 ...
 */
static MemTxResult gic_do_cpu_read(void* opaque, hwaddr addr, uint64_t* data, unsigned size, MemTxAttrs attrs)
{
...
static MemTxResult gic_do_cpu_write(void* opaque, hwaddr addr, uint64_t value, unsigned size, MemTxAttrs attrs)
{
    GICState** backref = (GICState**)opaque;
    GICState*  s       = *backref;
    int        id      = (backref - s->backref);
...
static MemTxResult gic_thisvcpu_read(void* opaque, hwaddr addr, uint64_t* data, unsigned size, MemTxAttrs attrs)
{
    GICState* s = (GICState*)opaque;
...
static MemTxResult gic_thisvcpu_write(void* opaque, hwaddr addr, uint64_t value, unsigned size, MemTxAttrs attrs)
{
    GICState* s = (GICState*)opaque;
...
static uint32_t gic_compute_eisr(GICState* s, int cpu, int lr_start)
{
    ...
    for (lr_idx = lr_start; lr_idx < s->num_lrs; lr_idx++) {
        uint32_t* entry = &s->h_lr[lr_idx][cpu];
        ret             = deposit32(ret, lr_idx - lr_start, 1, gic_lr_entry_is_eoi(*entry));
    }
...
static uint32_t gic_compute_elrsr(GICState* s, int cpu, int lr_start)
{
    ...
    for (lr_idx = lr_start; lr_idx < s->num_lrs; lr_idx++) {
        uint32_t* entry = &s->h_lr[lr_idx][cpu];
        ret             = deposit32(ret, lr_idx - lr_start, 1, gic_lr_entry_is_free(*entry));
    }
...
static MemTxResult gic_hyp_read(void* opaque, int cpu, hwaddr addr, uint64_t* data, MemTxAttrs attrs)
{
    GICState* s    = ARM_GIC(opaque);
    int       vcpu = cpu + GIC_NCPU;

    switch (addr) {
        case A_GICH_HCR: /* Hypervisor Control */ *data = s->h_hcr[cpu]; break;

        case A_GICH_VTR: /* VGIC Type */
            *data = REG_FIELD_DP32(0, GICH_VTR, ListRegs, s->num_lrs - 1);
            *data = REG_FIELD_DP32(*data, GICH_VTR, PREbits, GIC_VIRT_MAX_GROUP_PRIO_BITS - 1);
            *data = REG_FIELD_DP32(*data, GICH_VTR, PRIbits, (7 - GIC_VIRT_MIN_BPR) - 1);
            break;

        case A_GICH_VMCR: /* Virtual Machine Control */
            *data = REG_FIELD_DP32(0, GICH_VMCR, VMCCtlr, extract32(s->cpu_ctlr[vcpu], 0, 10));
            *data = REG_FIELD_DP32(*data, GICH_VMCR, VMABP, s->abpr[vcpu]);
            *data = REG_FIELD_DP32(*data, GICH_VMCR, VMBP, s->bpr[vcpu]);
            *data = REG_FIELD_DP32(*data, GICH_VMCR, VMPriMask, extract32(s->priority_mask[vcpu], 3, 5));
            break;

        case A_GICH_MISR: /* Maintenance Interrupt Status */ *data = s->h_misr[cpu]; break;

        case A_GICH_EISR0: /* End of Interrupt Status 0 and 1 */
        case A_GICH_EISR1: *data = gic_compute_eisr(s, cpu, (addr - A_GICH_EISR0) * 8); break;

        case A_GICH_ELRSR0: /* Empty List Status 0 and 1 */
        case A_GICH_ELRSR1: *data = gic_compute_elrsr(s, cpu, (addr - A_GICH_ELRSR0) * 8); break;

        case A_GICH_APR: /* Active Priorities */ *data = s->h_apr[cpu]; break;

        case A_GICH_LR0 ... A_GICH_LR63: /* List Registers */
        {
            int lr_idx = (addr - A_GICH_LR0) / 4;

            if (lr_idx > s->num_lrs) { *data = 0; }
            else {
        ...
        default: qemu_log_mask(LOG_GUEST_ERROR, "gic_hyp_read: Bad offset %" HWADDR_PRIx "\n", addr); return MEMTX_OK;
    }
...
static MemTxResult gic_hyp_write(void* opaque, int cpu, hwaddr addr, uint64_t value, MemTxAttrs attrs)
{
    GICState* s    = ARM_GIC(opaque);
    int       vcpu = cpu + GIC_NCPU;
    ...
    switch (addr) {
        case A_GICH_HCR: /* Hypervisor Control */ s->h_hcr[cpu] = value & GICH_HCR_MASK; break;

        case A_GICH_VMCR: /* Virtual Machine Control */ gic_vmcr_write(s, value, attrs); break;
        ...
        case A_GICH_LR0 ... A_GICH_LR63: /* List Registers */
        {
            ...
            if (lr_idx > s->num_lrs) { return MEMTX_OK; }
        ...
        default: qemu_log_mask(LOG_GUEST_ERROR, "gic_hyp_write: Bad offset %" HWADDR_PRIx "\n", addr); return MEMTX_OK;
    }
...
static MemTxResult gic_thiscpu_hyp_read(void* opaque, hwaddr addr, uint64_t* data, unsigned size, MemTxAttrs attrs)
{
    GICState* s = (GICState*)opaque;
...
static MemTxResult gic_thiscpu_hyp_write(void* opaque, hwaddr addr, uint64_t value, unsigned size, MemTxAttrs attrs)
{
    GICState* s = (GICState*)opaque;
...
static MemTxResult gic_do_hyp_read(void* opaque, hwaddr addr, uint64_t* data, unsigned size, MemTxAttrs attrs)
{
    GICState** backref = (GICState**)opaque;
    GICState*  s       = *backref;
    int        id      = (backref - s->backref);
...
static MemTxResult gic_do_hyp_write(void* opaque, hwaddr addr, uint64_t value, unsigned size, MemTxAttrs attrs)
{
    GICState** backref = (GICState**)opaque;
    GICState*  s       = *backref;
    int        id      = (backref - s->backref);
...
static void arm_gic_realize(DeviceState* dev, Error** errp)
{
    ...
    if (s->n_prio_bits > GIC_MAX_PRIORITY_BITS
        || (s->virt_extn ? s->n_prio_bits < GIC_VIRT_MAX_GROUP_PRIO_BITS : s->n_prio_bits < GIC_MIN_PRIORITY_BITS))
    {
        error_setg(errp,
                   "num-priority-bits cannot be greater than %d"
                   " or less than %d",
                   GIC_MAX_PRIORITY_BITS, s->virt_extn ? GIC_VIRT_MAX_GROUP_PRIO_BITS : GIC_MIN_PRIORITY_BITS);
        return;
    }
    ...
    if (s->virt_extn) {
        for (i = 0; i < s->num_cpu; i++) {
            memory_region_init_io(&s->vifaceiomem[i + 1], OBJECT(s), &gic_viface_ops, &s->backref[i], "gic_viface",
                                  0x200);
...
static void arm_gic_register_types(void) { type_register_static(&arm_gic_info); }
