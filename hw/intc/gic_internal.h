...
#define GIC_DIST_CLEAR_EDGE_TRIGGER(irq) (s->irq_state[irq].edge_trigger = false)
#define GIC_DIST_TEST_EDGE_TRIGGER(irq)  (s->irq_state[irq].edge_trigger)
#define GIC_DIST_GET_PRIORITY(irq, cpu)                                                    \
...
REG32(GICH_HCR, 0x0)
REG_FIELD(GICH_HCR, EN, 0, 1)
REG_FIELD(GICH_HCR, UIE, 1, 1)
REG_FIELD(GICH_HCR, LRENPIE, 2, 1)
REG_FIELD(GICH_HCR, NPIE, 3, 1)
REG_FIELD(GICH_HCR, VGRP0EIE, 4, 1)
REG_FIELD(GICH_HCR, VGRP0DIE, 5, 1)
REG_FIELD(GICH_HCR, VGRP1EIE, 6, 1)
REG_FIELD(GICH_HCR, VGRP1DIE, 7, 1)
REG_FIELD(GICH_HCR, EOICount, 27, 5)

#define GICH_HCR_MASK                                                                                            \
    (R_GICH_HCR_EN_MASK | R_GICH_HCR_UIE_MASK | R_GICH_HCR_LRENPIE_MASK | R_GICH_HCR_NPIE_MASK                   \
     | R_GICH_HCR_VGRP0EIE_MASK | R_GICH_HCR_VGRP0DIE_MASK | R_GICH_HCR_VGRP1EIE_MASK | R_GICH_HCR_VGRP1DIE_MASK \
     | R_GICH_HCR_EOICount_MASK)

REG32(GICH_VTR, 0x4)
REG_FIELD(GICH_VTR, ListRegs, 0, 6)
REG_FIELD(GICH_VTR, PREbits, 26, 3)
REG_FIELD(GICH_VTR, PRIbits, 29, 3)

REG32(GICH_VMCR, 0x8)
REG_FIELD(GICH_VMCR, VMCCtlr, 0, 10)
REG_FIELD(GICH_VMCR, VMABP, 18, 3)
REG_FIELD(GICH_VMCR, VMBP, 21, 3)
REG_FIELD(GICH_VMCR, VMPriMask, 27, 5)

REG32(GICH_MISR, 0x10)
REG_FIELD(GICH_MISR, EOI, 0, 1)
REG_FIELD(GICH_MISR, U, 1, 1)
REG_FIELD(GICH_MISR, LRENP, 2, 1)
REG_FIELD(GICH_MISR, NP, 3, 1)
REG_FIELD(GICH_MISR, VGrp0E, 4, 1)
REG_FIELD(GICH_MISR, VGrp0D, 5, 1)
REG_FIELD(GICH_MISR, VGrp1E, 6, 1)
REG_FIELD(GICH_MISR, VGrp1D, 7, 1)
...
REG_FIELD(GICH_LR0, VirtualID, 0, 10)
REG_FIELD(GICH_LR0, PhysicalID, 10, 10)
REG_FIELD(GICH_LR0, CPUID, 10, 3)
REG_FIELD(GICH_LR0, EOI, 19, 1)
REG_FIELD(GICH_LR0, Priority, 23, 5)
REG_FIELD(GICH_LR0, State, 28, 2)
REG_FIELD(GICH_LR0, Grp1, 30, 1)
REG_FIELD(GICH_LR0, HW, 31, 1)

/* Last LR register */
REG32(GICH_LR63, 0x1fc)

#define GICH_LR_MASK                                                                                      \
    (R_GICH_LR0_VirtualID_MASK | R_GICH_LR0_PhysicalID_MASK | R_GICH_LR0_CPUID_MASK | R_GICH_LR0_EOI_MASK \
     | R_GICH_LR0_Priority_MASK | R_GICH_LR0_State_MASK | R_GICH_LR0_Grp1_MASK | R_GICH_LR0_HW_MASK)
...
#define GICH_LR_VIRT_ID(entry)  (REG_FIELD_EX32(entry, GICH_LR0, VirtualID))
#define GICH_LR_PHYS_ID(entry)  (REG_FIELD_EX32(entry, GICH_LR0, PhysicalID))
#define GICH_LR_CPUID(entry)    (REG_FIELD_EX32(entry, GICH_LR0, CPUID))
#define GICH_LR_EOI(entry)      (REG_FIELD_EX32(entry, GICH_LR0, EOI))
#define GICH_LR_PRIORITY(entry) (REG_FIELD_EX32(entry, GICH_LR0, Priority) << 3)
#define GICH_LR_STATE(entry)    (REG_FIELD_EX32(entry, GICH_LR0, State))
#define GICH_LR_GROUP(entry)    (REG_FIELD_EX32(entry, GICH_LR0, Grp1))
#define GICH_LR_HW(entry)       (REG_FIELD_EX32(entry, GICH_LR0, HW))

#define GICH_LR_CLEAR_PENDING(entry) ((entry) &= ~(GICH_LR_STATE_PENDING << R_GICH_LR0_State_SHIFT))
#define GICH_LR_SET_ACTIVE(entry)    ((entry) |= (GICH_LR_STATE_ACTIVE << R_GICH_LR0_State_SHIFT))
...
{
    if (s->revision == REV_11MPCORE) { return s->irq_state[irq].pending & cm; }
    else {
         ...
         */
        return (s->irq_state[irq].pending & cm) || (!GIC_DIST_TEST_EDGE_TRIGGER(irq) && GIC_DIST_TEST_LEVEL(irq, cm));
    }
}

static inline bool gic_is_vcpu(int cpu) { return cpu >= GIC_NCPU; }

static inline int gic_get_vcpu_real_id(int cpu) { return (cpu >= GIC_NCPU) ? (cpu - GIC_NCPU) : cpu; }
...
static inline bool gic_virq_is_valid(GICState* s, int irq, int vcpu)
{
    ...
    for (lr_idx = 0; lr_idx < s->num_lrs; lr_idx++) {
        ...
        if ((GICH_LR_VIRT_ID(*entry) == irq) && (GICH_LR_STATE(*entry) & GICH_LR_STATE_ACTIVE)) { return true; }
    }
...
static inline uint32_t* gic_get_lr_entry(GICState* s, int irq, int vcpu)
{
    ...
    for (lr_idx = 0; lr_idx < s->num_lrs; lr_idx++) {
        ...
        if ((GICH_LR_VIRT_ID(*entry) == irq) && (GICH_LR_STATE(*entry) != GICH_LR_STATE_INVALID)) { return entry; }
    }
...
static inline void gic_clear_pending(GICState* s, int irq, int cpu)
{
    if (gic_is_vcpu(cpu)) {
         ...
         */
        GIC_DIST_CLEAR_PENDING(irq, GIC_DIST_TEST_MODEL(irq) ? ALL_CPU_MASK : (1 << cpu));
    }
}
