...
DECLARE_OBJ_CHECKERS(GICv3ITSState, GICv3ITSClass, ARM_GICV3_ITS, TYPE_ARM_GICV3_ITS)
...
static bool its_feature_virtual(GICv3ITSState* s) { return s->typer & R_GITS_TYPER_VIRTUAL_MASK; }

static inline bool intid_in_lpi_range(uint32_t id)
{ return id >= GICV3_LPI_INTID_START && id < (1 << (GICD_TYPER_IDBITS + 1)); }
...
static uint64_t table_entry_addr(GICv3ITSState* s, TableDesc* td, uint32_t idx, MemTxResult* res)
{
    /*
    ...
    *res = MEMTX_OK;
    ...
    l2 = address_space_ldq_le(as, td->base_addr + (l2idx * L1TABLE_ENTRY_SIZE), MEMTXATTRS_UNSPECIFIED, res);
    if (*res != MEMTX_OK) { return -1; }
    if (!(l2 & L2_TABLE_VALID_MASK)) { return -1; }
 ...
 * access error.
 */
static bool update_ite(GICv3ITSState* s, uint32_t eventid, const DTEntry* dte, const ITEntry* ite)
{
    AddressSpace* as      = &s->gicv3->dma_as;
    MemTxResult   res     = MEMTX_OK;
    ...
    trace_gicv3_its_ite_write(dte->ittaddr, eventid, ite->valid, ite->inttype, ite->intid, ite->icid, ite->vpeid,
                              ite->doorbell);
 ...
 * the error code.
 */
static MemTxResult get_ite(GICv3ITSState* s, uint32_t eventid, const DTEntry* dte, ITEntry* ite)
{
    AddressSpace* as  = &s->gicv3->dma_as;
    MemTxResult   res = MEMTX_OK;
    uint64_t      itel;
    ...
    trace_gicv3_its_ite_read(dte->ittaddr, eventid, ite->valid, ite->inttype, ite->intid, ite->icid, ite->vpeid,
                             ite->doorbell);
    return MEMTX_OK;
}
...
out:
    if (res != MEMTX_OK) { trace_gicv3_its_dte_read_fault(devid); }
    else {
...
static MemTxResult get_vte(GICv3ITSState* s, uint32_t vpeid, VTEntry* vte)
{
    ...
    trace_gicv3_its_vte_read(vpeid, vte->valid, vte->vptsize, vte->vptaddr, vte->rdbase);
    return res;
}
 ...
 */
static ItsCmdResult lookup_ite(GICv3ITSState* s, const char* who, uint32_t devid, uint32_t eventid, ITEntry* ite,
                               DTEntry* dte)
{
    uint64_t num_eventids;

    if (devid >= s->dt.num_entries) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid command attributes: devid %d>=%d", who, devid, s->dt.num_entries);
        return CMD_CONTINUE;
    }

    if (get_dte(s, devid, dte) != MEMTX_OK) { return CMD_STALL; }
    if (!dte->valid) {
    ...
    if (eventid >= num_eventids) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid command attributes: eventid %d >= %" PRId64 "\n", who, eventid,
                      num_eventids);
        return CMD_CONTINUE;
    }

    if (get_ite(s, eventid, dte, ite) != MEMTX_OK) { return CMD_STALL; }

    if (!ite->valid) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid command attributes: invalid ITE\n", who);
        return CMD_CONTINUE;
    }
 ...
 * and should indicate the name of the calling function or similar.
 */
static ItsCmdResult lookup_cte(GICv3ITSState* s, const char* who, uint32_t icid, CTEntry* cte)
{
    if (icid >= s->ct.num_entries) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid ICID 0x%x\n", who, icid);
        return CMD_CONTINUE;
    }
    if (get_cte(s, icid, cte) != MEMTX_OK) { return CMD_STALL; }
    if (!cte->valid) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid CTE\n", who);
        return CMD_CONTINUE;
    }
    if (cte->rdbase >= s->gicv3->num_cpu) { return CMD_CONTINUE; }
    return CMD_CONTINUE_OK;
}
 ...
 * and should indicate the name of the calling function or similar.
 */
static ItsCmdResult lookup_vte(GICv3ITSState* s, const char* who, uint32_t vpeid, VTEntry* vte)
{
    ...
    if (get_vte(s, vpeid, vte) != MEMTX_OK) { return CMD_STALL; }
    if (!vte->valid) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid VTE for VPEID 0x%x\n", who, vpeid);
        return CMD_CONTINUE;
    }

    if (vte->rdbase >= s->gicv3->num_cpu) { return CMD_CONTINUE; }
    return CMD_CONTINUE_OK;
}

static ItsCmdResult process_its_cmd_phys(GICv3ITSState* s, const ITEntry* ite, int irqlevel)
{
    CTEntry      cte = {};
    ItsCmdResult cmdres;

    cmdres = lookup_cte(s, __func__, ite->icid, &cte);
    if (cmdres != CMD_CONTINUE_OK) { return cmdres; }
    gicv3_redist_process_lpi(&s->gicv3->cpu[cte.rdbase], ite->intid, irqlevel);
    return CMD_CONTINUE_OK;
}

static ItsCmdResult process_its_cmd_virt(GICv3ITSState* s, const ITEntry* ite, int irqlevel)
{
    VTEntry      vte = {};
    ItsCmdResult cmdres;
    ...
    if (!intid_in_lpi_range(ite->intid) || ite->intid >= (1ULL << (vte.vptsize + 1))) {
     ...
     * redistributor code
     */
    gicv3_redist_process_vlpi(&s->gicv3->cpu[vte.rdbase], ite->intid, vte.vptaddr << 16, ite->doorbell, irqlevel);
    return CMD_CONTINUE_OK;
}
 ...
 */
static ItsCmdResult do_process_its_cmd(GICv3ITSState* s, uint32_t devid, uint32_t eventid, ItsCmdType cmd)
{
    DTEntry      dte = {};
    ITEntry      ite = {};
    ItsCmdResult cmdres;
    ...
    switch (ite.inttype) {
        case ITE_INTTYPE_PHYSICAL: cmdres = process_its_cmd_phys(s, &ite, irqlevel); break;
        case ITE_INTTYPE_VIRTUAL:
            if (!its_feature_virtual(s)) {
                /* Can't happen unless guest is illegally writing to table memory */
                qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid type %d in ITE (table corrupted?)\n", __func__,
                              ite.inttype);
                return CMD_CONTINUE;
            }
...
static ItsCmdResult process_its_cmd(GICv3ITSState* s, const uint64_t* cmdpkt, ItsCmdType cmd)
{
    ...
    switch (cmd) {
        case INTERRUPT: trace_gicv3_its_cmd_int(devid, eventid); break;
        case CLEAR    : trace_gicv3_its_cmd_clear(devid, eventid); break;
        case DISCARD  : trace_gicv3_its_cmd_discard(devid, eventid); break;
        default       : assert_not_reached();
    }
    return do_process_its_cmd(s, devid, eventid, cmd);
}

static ItsCmdResult process_mapti(GICv3ITSState* s, const uint64_t* cmdpkt, bool ignore_pInt)
{
    uint32_t devid, eventid;
    uint32_t pIntid = 0;
    uint64_t num_eventids;
    ...
    if (devid >= s->dt.num_entries) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid command attributes: devid %d>=%d", __func__, devid,
                      s->dt.num_entries);
        return CMD_CONTINUE;
    }
    ...
    if (icid >= s->ct.num_entries) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid ICID 0x%x >= 0x%x\n", __func__, icid, s->ct.num_entries);
        return CMD_CONTINUE;
    }

    if (!dte.valid) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: no valid DTE for devid 0x%x\n", __func__, devid);
        return CMD_CONTINUE;
    }

    if (eventid >= num_eventids) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid event ID 0x%x >= 0x%" PRIx64 "\n", __func__, eventid, num_eventids);
        return CMD_CONTINUE;
    }

    if (!intid_in_lpi_range(pIntid)) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid interrupt ID 0x%x\n", __func__, pIntid);
        return CMD_CONTINUE;
    }
...
static ItsCmdResult process_vmapti(GICv3ITSState* s, const uint64_t* cmdpkt, bool ignore_vintid)
{
    ...
    if (!its_feature_virtual(s)) { return CMD_CONTINUE; }
    ...
    if (devid >= s->dt.num_entries) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid DeviceID 0x%x (must be less than 0x%x)\n", __func__, devid,
                      s->dt.num_entries);
        return CMD_CONTINUE;
    }
    ...
    if (!dte.valid) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: no entry in device table for DeviceID 0x%x\n", __func__, devid);
        return CMD_CONTINUE;
    }
    ...
    if (!intid_in_lpi_range(vintid)) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: VIntID 0x%x not a valid LPI\n", __func__, vintid);
        return CMD_CONTINUE;
    }
    if (!valid_doorbell(doorbell)) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: Doorbell %d not 1023 and not a valid LPI\n", __func__, doorbell);
        return CMD_CONTINUE;
    }
    if (vpeid >= s->vpet.num_entries) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: VPEID 0x%x out of range (must be less than 0x%x)\n", __func__, vpeid,
                      s->vpet.num_entries);
        return CMD_CONTINUE;
    }
    /* add ite entry to interrupt translation table */
...
static ItsCmdResult process_mapc(GICv3ITSState* s, const uint64_t* cmdpkt)
{
    ...
    if (cte.valid && cte.rdbase >= s->gicv3->num_cpu) {
        qemu_log_mask(LOG_GUEST_ERROR, "ITS MAPC: invalid RDBASE %u\n", cte.rdbase);
        return CMD_CONTINUE;
    }
...
static ItsCmdResult process_mapd(GICv3ITSState* s, const uint64_t* cmdpkt)
{
    ...
    if (devid >= s->dt.num_entries) {
        qemu_log_mask(LOG_GUEST_ERROR, "ITS MAPD: invalid device ID field 0x%x >= 0x%x\n", devid, s->dt.num_entries);
        return CMD_CONTINUE;
    }

    if (dte.size > REG_FIELD_EX64(s->typer, GITS_TYPER, IDBITS)) {
        qemu_log_mask(LOG_GUEST_ERROR, "ITS MAPD: invalid size %d\n", dte.size);
        return CMD_CONTINUE;
    }
...
static ItsCmdResult process_movall(GICv3ITSState* s, const uint64_t* cmdpkt)
{
    ...
    if (rd1 >= s->gicv3->num_cpu) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: RDBASE1 %" PRId64 " out of range (must be less than %d)\n", __func__, rd1,
                      s->gicv3->num_cpu);
        return CMD_CONTINUE;
    }
    if (rd2 >= s->gicv3->num_cpu) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: RDBASE2 %" PRId64 " out of range (must be less than %d)\n", __func__, rd2,
                      s->gicv3->num_cpu);
        return CMD_CONTINUE;
    }
...
static ItsCmdResult process_movi(GICv3ITSState* s, const uint64_t* cmdpkt)
{
    ...
    if (old_cte.rdbase != new_cte.rdbase) {
        ...
        gicv3_redist_mov_lpi(&s->gicv3->cpu[old_cte.rdbase], &s->gicv3->cpu[new_cte.rdbase], old_ite.intid);
    }
...
static bool update_vte(GICv3ITSState* s, uint32_t vpeid, const VTEntry* vte)
{
    ...
    trace_gicv3_its_vte_write(vpeid, vte->valid, vte->vptsize, vte->vptaddr, vte->rdbase);
...
static ItsCmdResult process_vmapp(GICv3ITSState* s, const uint64_t* cmdpkt)
{
    ...
    trace_gicv3_its_cmd_vmapp(vpeid, vte.rdbase, vte.valid, vte.vptaddr, vte.vptsize);
    ...
    if (vte.vptsize > REG_FIELD_EX64(s->typer, GITS_TYPER, IDBITS)) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid VPT_size 0x%x\n", __func__, vte.vptsize);
        return CMD_CONTINUE;
    }

    if (vte.valid && vte.rdbase >= s->gicv3->num_cpu) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid rdbase 0x%x\n", __func__, vte.rdbase);
        return CMD_CONTINUE;
    }
...
static void vmovp_callback(gpointer data, gpointer opaque)
{
    ...
    switch (cmdres) {
        case CMD_STALL: cbdata->result = CMD_STALL; return;
        case CMD_CONTINUE:
            if (cbdata->result != CMD_STALL) { cbdata->result = CMD_CONTINUE; }
            return;
        case CMD_CONTINUE_OK: break;
    }

    vte.rdbase = cbdata->rdbase;
    if (!update_vte(s, cbdata->vpeid, &vte)) { cbdata->result = CMD_STALL; }
}
...
static ItsCmdResult process_vmovp(GICv3ITSState* s, const uint64_t* cmdpkt)
{
    ...
    if (cbdata.rdbase >= s->gicv3->num_cpu) { return CMD_CONTINUE; }
...
static ItsCmdResult process_vmovi(GICv3ITSState* s, const uint64_t* cmdpkt)
{
    ...
    if (doorbell_valid && !valid_doorbell(doorbell)) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid doorbell 0x%x\n", __func__, doorbell);
        return CMD_CONTINUE;
    }
    ...
    if (!intid_in_lpi_range(ite.intid) || ite.intid >= (1ULL << (old_vte.vptsize + 1))
        || ite.intid >= (1ULL << (new_vte.vptsize + 1)))
    {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: ITE intid 0x%x out of range\n", __func__, ite.intid);
        return CMD_CONTINUE;
    }

    ite.vpeid = vpeid;
    if (doorbell_valid) { ite.doorbell = doorbell; }
    ...
    if (old_vte.vptaddr != new_vte.vptaddr) {
        gicv3_redist_mov_vlpi(&s->gicv3->cpu[old_vte.rdbase], old_vte.vptaddr << 16, &s->gicv3->cpu[new_vte.rdbase],
                              new_vte.vptaddr << 16, ite.intid, ite.doorbell);
    }
...
static ItsCmdResult process_inv(GICv3ITSState* s, const uint64_t* cmdpkt)
{
    ...
    switch (ite.inttype) {
        ...
        case ITE_INTTYPE_VIRTUAL:
            ...
            gicv3_redist_inv_vlpi(&s->gicv3->cpu[vte.rdbase], ite.intid, vte.vptaddr << 16);
            break;
        default: assert_not_reached();
    }
...
static void process_cmdq(GICv3ITSState* s)
{
    ...
    if (!(s->ctlr & R_GITS_CTLR_ENABLED_MASK)) { return; }
    ...
    while (wr_offset != rd_offset) {
        ...
        hostmem = address_space_map(as, s->cq.base_addr + cq_offset, &buflen, false, MEMTXATTRS_UNSPECIFIED);
        if (!hostmem || buflen != GITS_CMDQ_ENTRY_SIZE) {
            if (hostmem) { address_space_unmap(as, hostmem, buflen, false, 0); }
            s->creadr = REG_FIELD_DP64(s->creadr, GITS_CREADR, STALLED, 1);
            qemu_log_mask(LOG_GUEST_ERROR, "%s: could not read command at 0x%" PRIx64 "\n", __func__,
                          s->cq.base_addr + cq_offset);
            break;
        }
        for (i = 0; i < ARRAY_SIZE(cmdpkt); i++) { cmdpkt[i] = ldq_le_p(hostmem + i * sizeof(uint64_t)); }
        ...
        switch (cmd) {
            case GITS_CMD_INT  : result = process_its_cmd(s, cmdpkt, INTERRUPT); break;
            case GITS_CMD_CLEAR: result = process_its_cmd(s, cmdpkt, CLEAR); break;
            case GITS_CMD_SYNC:
                /*
            ...
            case GITS_CMD_VSYNC:
                ...
                break;
            case GITS_CMD_MAPD   : result = process_mapd(s, cmdpkt); break;
            case GITS_CMD_MAPC   : result = process_mapc(s, cmdpkt); break;
            case GITS_CMD_MAPTI  : result = process_mapti(s, cmdpkt, false); break;
            case GITS_CMD_MAPI   : result = process_mapti(s, cmdpkt, true); break;
            case GITS_CMD_DISCARD: result = process_its_cmd(s, cmdpkt, DISCARD); break;
            case GITS_CMD_INV    : result = process_inv(s, cmdpkt); break;
            case GITS_CMD_INVALL:
                ...
                for (i = 0; i < s->gicv3->num_cpu; i++) { gicv3_redist_update_lpi(&s->gicv3->cpu[i]); }
                break;
            case GITS_CMD_MOVI   : result = process_movi(s, cmdpkt); break;
            case GITS_CMD_MOVALL : result = process_movall(s, cmdpkt); break;
            case GITS_CMD_VMAPTI : result = process_vmapti(s, cmdpkt, false); break;
            case GITS_CMD_VMAPI  : result = process_vmapti(s, cmdpkt, true); break;
            case GITS_CMD_VMAPP  : result = process_vmapp(s, cmdpkt); break;
            case GITS_CMD_VMOVP  : result = process_vmovp(s, cmdpkt); break;
            case GITS_CMD_VMOVI  : result = process_vmovi(s, cmdpkt); break;
            case GITS_CMD_VINVALL: result = process_vinvall(s, cmdpkt); break;
            default              : trace_gicv3_its_cmd_unknown(cmd); break;
        }
        if (result != CMD_STALL) {
            ...
            /* CMD_STALL */
            s->creadr = REG_FIELD_DP64(s->creadr, GITS_CREADR, STALLED, 1);
            qemu_log_mask(LOG_GUEST_ERROR, "%s: 0x%x cmd processing failed, stalling\n", __func__, cmd);
            break;
        }
    }
}
...
static void extract_table_params(GICv3ITSState* s)
{
    ...
    for (int i = 0; i < 8; i++) {
        ...
        switch (page_sz_type) {
            case 0: page_sz = GITS_PAGE_SIZE_4K; break;

            case 1: page_sz = GITS_PAGE_SIZE_16K; break;

            case 2:
            case 3: page_sz = GITS_PAGE_SIZE_64K; break;
         ...
         */
        if (!REG_FIELD_EX64(value, GITS_BASER, VALID)) { continue; }
        td->page_sz   = page_sz;
        ...
        if (!td->indirect) { td->num_entries = (num_pages * page_sz) / td->entry_sz; }
        else {
            td->num_entries = (((num_pages * page_sz) / L1TABLE_ENTRY_SIZE) * (page_sz / td->entry_sz));
        }
...
static void extract_cmdq_params(GICv3ITSState* s)
{
    ...
    if (REG_FIELD_EX64(value, GITS_CBASER, VALID)) {
        s->cq.num_entries   = (num_pages * GITS_PAGE_SIZE_4K) / GITS_CMDQ_ENTRY_SIZE;
        s->cq.base_addr     = REG_FIELD_EX64(value, GITS_CBASER, PHYADDR);
        s->cq.base_addr   <<= R_GITS_CBASER_PHYADDR_SHIFT;
    }
}

static MemTxResult gicv3_its_translation_read(void* opaque, hwaddr offset, uint64_t* data, unsigned size,
                                              MemTxAttrs attrs)
{
    /*
...
static MemTxResult gicv3_its_translation_write(void* opaque, hwaddr offset, uint64_t data, unsigned size,
                                               MemTxAttrs attrs)
{
    ...
    switch (offset) {
        case GITS_TRANSLATER:
            if (s->ctlr & R_GITS_CTLR_ENABLED_MASK) { result = do_process_its_cmd(s, attrs.requester_id, data, NONE); }
            break;
        default: break;
    }

    if (result) { return MEMTX_OK; }
    else {
        return MEMTX_ERROR;
    }
}

static bool its_writel(GICv3ITSState* s, hwaddr offset, uint64_t value, MemTxAttrs attrs)
{
    bool result = true;
    int  index;
    ...
    switch (offset) {
        ...
        case GITS_CBASER + 4:
            ...
            break;
        case GITS_CWRITER:
            s->cwriter = deposit64(s->cwriter, 0, 32, (value & ~R_GITS_CWRITER_RETRY_MASK));
            if (s->cwriter != s->creadr) { process_cmdq(s); }
            break;
        case GITS_CWRITER + 4: s->cwriter = deposit64(s->cwriter, 32, 32, value); break;
        case GITS_CREADR:
            if (s->gicv3->gicd_ctlr & GICD_CTLR_DS) {
                s->creadr = deposit64(s->creadr, 0, 32, (value & ~R_GITS_CREADR_STALLED_MASK));
            }
            else {
            ...
            break;
        case GITS_CREADR + 4:
            if (s->gicv3->gicd_ctlr & GICD_CTLR_DS) { s->creadr = deposit64(s->creadr, 32, 32, value); }
            else {
                /* RO register, ignore the write */
                qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid guest write to RO register at offset " HWADDR_FMT_plx "\n",
                              __func__, offset);
...
static bool its_readl(GICv3ITSState* s, hwaddr offset, uint64_t* data, MemTxAttrs attrs)
{
    bool result = true;
    int  index;

    switch (offset) {
        case GITS_CTLR: *data = s->ctlr; break;
        case GITS_IIDR: *data = gicv3_iidr(); break;
        case GITS_IDREGS ... GITS_IDREGS + 0x2f:
            /* ID registers */
            *data = gicv3_idreg(s->gicv3, offset - GITS_IDREGS, GICV3_PIDR0_ITS);
            break;
        case GITS_TYPER      : *data = extract64(s->typer, 0, 32); break;
        case GITS_TYPER + 4  : *data = extract64(s->typer, 32, 32); break;
        case GITS_CBASER     : *data = extract64(s->cbaser, 0, 32); break;
        case GITS_CBASER + 4 : *data = extract64(s->cbaser, 32, 32); break;
        case GITS_CREADR     : *data = extract64(s->creadr, 0, 32); break;
        case GITS_CREADR + 4 : *data = extract64(s->creadr, 32, 32); break;
        case GITS_CWRITER    : *data = extract64(s->cwriter, 0, 32); break;
        case GITS_CWRITER + 4: *data = extract64(s->cwriter, 32, 32); break;
        case GITS_BASER ... GITS_BASER + 0x3f:
            index = (offset - GITS_BASER) / 8;
            if (offset & 7) { *data = extract64(s->baser[index], 32, 32); }
            else {
...
static bool its_writell(GICv3ITSState* s, hwaddr offset, uint64_t value, MemTxAttrs attrs)
{
    bool result = true;
    int  index;
    ...
    switch (offset) {
        ...
        case GITS_CWRITER:
            ...
            break;
        case GITS_CREADR:
            if (s->gicv3->gicd_ctlr & GICD_CTLR_DS) { s->creadr = value & ~R_GITS_CREADR_STALLED_MASK; }
            else {
                /* RO register, ignore the write */
                qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid guest write to RO register at offset " HWADDR_FMT_plx "\n",
                              __func__, offset);
...
static bool its_readll(GICv3ITSState* s, hwaddr offset, uint64_t* data, MemTxAttrs attrs)
{
    bool result = true;
    int  index;

    switch (offset) {
        case GITS_TYPER: *data = s->typer; break;
        case GITS_BASER ... GITS_BASER + 0x3f:
            index = (offset - GITS_BASER) / 8;
            *data = s->baser[index];
            break;
        case GITS_CBASER : *data = s->cbaser; break;
        case GITS_CREADR : *data = s->creadr; break;
        case GITS_CWRITER: *data = s->cwriter; break;
        default          : result = false; break;
    }
    return result;
}

static MemTxResult gicv3_its_read(void* opaque, hwaddr offset, uint64_t* data, unsigned size, MemTxAttrs attrs)
{
    GICv3ITSState* s = (GICv3ITSState*)opaque;
    bool           result;

    switch (size) {
        case 4 : result = its_readl(s, offset, data, attrs); break;
        case 8 : result = its_readll(s, offset, data, attrs); break;
        default: result = false; break;
    }

    if (!result) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid guest read at offset " HWADDR_FMT_plx " size %u\n", __func__,
                      offset, size);
...
static MemTxResult gicv3_its_write(void* opaque, hwaddr offset, uint64_t data, unsigned size, MemTxAttrs attrs)
{
    GICv3ITSState* s = (GICv3ITSState*)opaque;
    bool           result;

    switch (size) {
        case 4 : result = its_writel(s, offset, data, attrs); break;
        case 8 : result = its_writell(s, offset, data, attrs); break;
        default: result = false; break;
    }

    if (!result) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid guest write at offset " HWADDR_FMT_plx " size %u\n", __func__,
                      offset, size);
...
static void gicv3_arm_its_realize(DeviceState* dev, Error** errp)
{
    ...
    s->typer = REG_FIELD_DP64(s->typer, GITS_TYPER, ITT_ENTRY_SIZE, ITS_ITT_ENTRY_SIZE - 1);
...
static void gicv3_its_reset_hold(Object* obj, ResetType type)
{
     ...
     */
    s->baser[0] = REG_FIELD_DP64(s->baser[0], GITS_BASER, TYPE, GITS_BASER_TYPE_DEVICE);
    s->baser[0] = REG_FIELD_DP64(s->baser[0], GITS_BASER, PAGESIZE, GITS_BASER_PAGESIZE_64K);
    s->baser[0] = REG_FIELD_DP64(s->baser[0], GITS_BASER, ENTRYSIZE, GITS_DTE_SIZE - 1);

    s->baser[1] = REG_FIELD_DP64(s->baser[1], GITS_BASER, TYPE, GITS_BASER_TYPE_COLLECTION);
    s->baser[1] = REG_FIELD_DP64(s->baser[1], GITS_BASER, PAGESIZE, GITS_BASER_PAGESIZE_64K);
    s->baser[1] = REG_FIELD_DP64(s->baser[1], GITS_BASER, ENTRYSIZE, GITS_CTE_SIZE - 1);

    if (its_feature_virtual(s)) {
        s->baser[2] = REG_FIELD_DP64(s->baser[2], GITS_BASER, TYPE, GITS_BASER_TYPE_VPE);
        s->baser[2] = REG_FIELD_DP64(s->baser[2], GITS_BASER, PAGESIZE, GITS_BASER_PAGESIZE_64K);
        s->baser[2] = REG_FIELD_DP64(s->baser[2], GITS_BASER, ENTRYSIZE, GITS_VPE_SIZE - 1);
    }
}
...
static const Property gicv3_its_props[] = {
    DEFINE_PROP_LINK("parent-gicv3", GICv3ITSState, gicv3, "arm-gicv3", GICv3State*),
};
...
static void gicv3_its_class_init(ObjectClass* klass, const void* data)
{
    ...
    resettable_class_set_parent_phases(rc, NULL, gicv3_its_reset_hold, NULL, &ic->parent_phases);
...
static void gicv3_its_register_types(void) { type_register_static(&gicv3_its_info); }
