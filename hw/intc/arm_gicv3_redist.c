...
static void gicr_write_bitmap_reg(GICv3CPUState* cs, MemTxAttrs attrs, uint32_t* reg, uint32_t val)
{
...
static void gicr_write_set_bitmap_reg(GICv3CPUState* cs, MemTxAttrs attrs, uint32_t* reg, uint32_t val)
{
...
static void gicr_write_clear_bitmap_reg(GICv3CPUState* cs, MemTxAttrs attrs, uint32_t* reg, uint32_t val)
{
...
static uint32_t gicr_read_bitmap_reg(GICv3CPUState* cs, MemTxAttrs attrs, uint32_t reg)
{
...
static bool vcpu_resident(GICv3CPUState* cs, uint64_t vptaddr)
{
     ...
     */
    if (!REG_FIELD_EX64(cs->gicr_vpendbaser, GICR_VPENDBASER, VALID)) { return false; }
 ...
 */
static void update_for_one_lpi(GICv3CPUState* cs, int irq, uint64_t ctbase, bool ds, PendingIrq* hpp)
{
    uint8_t lpite;
    uint8_t prio;

    address_space_read(&cs->gic->dma_as, ctbase + ((irq - GICV3_LPI_INTID_START) * sizeof(lpite)),
                       MEMTXATTRS_UNSPECIFIED, &lpite, sizeof(lpite));

    if (!(lpite & LPI_CTE_ENABLED)) { return; }

    if (ds) { prio = lpite & LPI_PRIORITY_MASK; }
    else {
 ...
 */
static void update_for_all_lpis(GICv3CPUState* cs, uint64_t ptbase, uint64_t ctbase, unsigned ptsizebits, bool ds,
                                PendingIrq* hpp)
{
 ...
 */
static bool set_pending_table_bit(GICv3CPUState* cs, uint64_t ptbase, int irq, bool level)
{
    AddressSpace* as   = &cs->gic->dma_as;
...
static uint8_t gicr_read_ipriorityr(GICv3CPUState* cs, MemTxAttrs attrs, int irq)
{
...
static void gicr_write_ipriorityr(GICv3CPUState* cs, MemTxAttrs attrs, int irq, uint8_t value)
{
...
static void gicr_write_vpendbaser(GICv3CPUState* cs, uint64_t newval)
{
     ...
     */
    newval &= R_GICR_VPENDBASER_INNERCACHE_MASK | R_GICR_VPENDBASER_SHAREABILITY_MASK | R_GICR_VPENDBASER_PHYADDR_MASK
              | R_GICR_VPENDBASER_OUTERCACHE_MASK | R_GICR_VPENDBASER_PENDINGLAST_MASK | R_GICR_VPENDBASER_IDAI_MASK
...
static MemTxResult gicr_readb(GICv3CPUState* cs, hwaddr offset, uint64_t* data, MemTxAttrs attrs)
{
    switch (offset) {
...
static MemTxResult gicr_writeb(GICv3CPUState* cs, hwaddr offset, uint64_t value, MemTxAttrs attrs)
{
    switch (offset) {
        case GICR_IPRIORITYR ... GICR_IPRIORITYR + 0x1f:
...
static MemTxResult gicr_readl(GICv3CPUState* cs, hwaddr offset, uint64_t* data, MemTxAttrs attrs)
{
    switch (offset) {
        case GICR_CTLR     : *data = cs->gicr_ctlr; return MEMTX_OK;
        case GICR_IIDR     : *data = gicv3_iidr(); return MEMTX_OK;
        case GICR_TYPER    : *data = extract64(cs->gicr_typer, 0, 32); return MEMTX_OK;
        case GICR_TYPER + 4: *data = extract64(cs->gicr_typer, 32, 32); return MEMTX_OK;
        case GICR_STATUSR:
            ...
            *data = 0;
            return MEMTX_OK;
        case GICR_WAKER        : *data = cs->gicr_waker; return MEMTX_OK;
        case GICR_PROPBASER    : *data = extract64(cs->gicr_propbaser, 0, 32); return MEMTX_OK;
        case GICR_PROPBASER + 4: *data = extract64(cs->gicr_propbaser, 32, 32); return MEMTX_OK;
        case GICR_PENDBASER    : *data = extract64(cs->gicr_pendbaser, 0, 32); return MEMTX_OK;
        case GICR_PENDBASER + 4: *data = extract64(cs->gicr_pendbaser, 32, 32); return MEMTX_OK;
        case GICR_IGROUPR0:
            if (!attrs.secure && !(cs->gic->gicd_ctlr & GICD_CTLR_DS)) {
                *data = 0;
                return MEMTX_OK;
        ...
        case GICR_ICENABLER0: *data = gicr_read_bitmap_reg(cs, attrs, cs->gicr_ienabler0); return MEMTX_OK;
        case GICR_ISPENDR0  :
        case GICR_ICPENDR0  : {
        ...
        case GICR_INMIR0:
            *data = cs->gic->nmi_support ? gicr_read_bitmap_reg(cs, attrs, cs->gicr_inmir0) : 0;
            return MEMTX_OK;
        case GICR_ICFGR0:
        case GICR_ICFGR1: {
        ...
        case GICR_IDREGS ... GICR_IDREGS + 0x2f:
            *data = gicv3_idreg(cs->gic, offset - GICR_IDREGS, GICV3_PIDR0_REDIST);
             ...
             */
        case GICR_VPROPBASER    : *data = extract64(cs->gicr_vpropbaser, 0, 32); return MEMTX_OK;
        case GICR_VPROPBASER + 4: *data = extract64(cs->gicr_vpropbaser, 32, 32); return MEMTX_OK;
        case GICR_VPENDBASER    : *data = extract64(cs->gicr_vpendbaser, 0, 32); return MEMTX_OK;
        case GICR_VPENDBASER + 4: *data = extract64(cs->gicr_vpendbaser, 32, 32); return MEMTX_OK;
        default                 : return MEMTX_ERROR;
    }
}

static MemTxResult gicr_writel(GICv3CPUState* cs, hwaddr offset, uint64_t value, MemTxAttrs attrs)
{
    switch (offset) {
        case GICR_CTLR:
        ...
        case GICR_WAKER:
            ...
            if (value & GICR_WAKER_ProcessorSleep) { value |= GICR_WAKER_ChildrenAsleep; }
            cs->gicr_waker = value;
            return MEMTX_OK;
        case GICR_PROPBASER    : cs->gicr_propbaser = deposit64(cs->gicr_propbaser, 0, 32, value); return MEMTX_OK;
        case GICR_PROPBASER + 4: cs->gicr_propbaser = deposit64(cs->gicr_propbaser, 32, 32, value); return MEMTX_OK;
        case GICR_PENDBASER    : cs->gicr_pendbaser = deposit64(cs->gicr_pendbaser, 0, 32, value); return MEMTX_OK;
        case GICR_PENDBASER + 4: cs->gicr_pendbaser = deposit64(cs->gicr_pendbaser, 32, 32, value); return MEMTX_OK;
        case GICR_IGROUPR0:
            if (!attrs.secure && !(cs->gic->gicd_ctlr & GICD_CTLR_DS)) { return MEMTX_OK; }
        ...
        case GICR_IPRIORITYR ... GICR_IPRIORITYR + 0x1f: {
            ...
            for (i = irq; i < irq + 4; i++, value >>= 8) { gicr_write_ipriorityr(cs, attrs, i, value); }
            gicv3_redist_update(cs);
            return MEMTX_OK;
        }
        case GICR_INMIR0:
            if (cs->gic->nmi_support) { gicr_write_bitmap_reg(cs, attrs, &cs->gicr_inmir0, value); }
            return MEMTX_OK;
...
static MemTxResult gicr_readll(GICv3CPUState* cs, hwaddr offset, uint64_t* data, MemTxAttrs attrs)
{
    switch (offset) {
        case GICR_TYPER    : *data = cs->gicr_typer; return MEMTX_OK;
        case GICR_PROPBASER: *data = cs->gicr_propbaser; return MEMTX_OK;
        case GICR_PENDBASER:
            *data = cs->gicr_pendbaser;
             ...
             * prevent pre-v4 GIC from passing us offsets this high.
             */
        case GICR_VPROPBASER: *data = cs->gicr_vpropbaser; return MEMTX_OK;
        case GICR_VPENDBASER: *data = cs->gicr_vpendbaser; return MEMTX_OK;
        default             : return MEMTX_ERROR;
    }
}

static MemTxResult gicr_writell(GICv3CPUState* cs, hwaddr offset, uint64_t value, MemTxAttrs attrs)
{
    switch (offset) {
        case GICR_PROPBASER: cs->gicr_propbaser = value; return MEMTX_OK;
        case GICR_PENDBASER: cs->gicr_pendbaser = value; return MEMTX_OK;
        case GICR_TYPER:
             ...
             * prevent pre-v4 GIC from passing us offsets this high.
             */
        case GICR_VPROPBASER: cs->gicr_vpropbaser = value; return MEMTX_OK;
        case GICR_VPENDBASER: gicr_write_vpendbaser(cs, value); return MEMTX_OK;
        default             : return MEMTX_ERROR;
    }
}

MemTxResult gicv3_redist_read(void* opaque, hwaddr offset, uint64_t* data, unsigned size, MemTxAttrs attrs)
{
    ...
    switch (size) {
        case 1 : r = gicr_readb(cs, offset, data, attrs); break;
        case 4 : r = gicr_readl(cs, offset, data, attrs); break;
        case 8 : r = gicr_readll(cs, offset, data, attrs); break;
        default: r = MEMTX_ERROR; break;
    }

    if (r != MEMTX_OK) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid guest read at offset " HWADDR_FMT_plx " size %u\n", __func__,
                      offset, size);
        trace_gicv3_redist_badread(gicv3_redist_affid(cs), offset, size, attrs.secure);
        ...
        *data = 0;
        ...
        trace_gicv3_redist_read(gicv3_redist_affid(cs), offset, *data, size, attrs.secure);
    }
    return r;
}

MemTxResult gicv3_redist_write(void* opaque, hwaddr offset, uint64_t data, unsigned size, MemTxAttrs attrs)
{
    GICv3RedistRegion* region = opaque;
    GICv3State*        s      = region->gic;
    GICv3CPUState*     cs;
    ...
    switch (size) {
        case 1 : r = gicr_writeb(cs, offset, data, attrs); break;
        case 4 : r = gicr_writel(cs, offset, data, attrs); break;
        case 8 : r = gicr_writell(cs, offset, data, attrs); break;
        default: r = MEMTX_ERROR; break;
    }
    ...
    if (r != MEMTX_OK) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid guest write at offset " HWADDR_FMT_plx " size %u\n", __func__,
                      offset, size);
        trace_gicv3_redist_badwrite(gicv3_redist_affid(cs), offset, data, size, attrs.secure);
        /* The spec requires that reserved registers are RAZ/WI;
         * so use MEMTX_ERROR returns from leaf functions as a way to
         * trigger the guest-error logging but don't return it to
         * the caller, or we'll cause a spurious guest data abort.
        ...
        trace_gicv3_redist_write(gicv3_redist_affid(cs), offset, data, size, attrs.secure);
    }
    return r;
}
...
static void gicv3_redist_check_lpi_priority(GICv3CPUState* cs, int irq)
{
    ...
    update_for_one_lpi(cs, irq, lpict_baddr, cs->gic->gicd_ctlr & GICD_CTLR_DS, &cs->hpplpi);
}
...
void gicv3_redist_update_lpi_only(GICv3CPUState* cs)
{
    ...
    idbits = MIN(REG_FIELD_EX64(cs->gicr_propbaser, GICR_PROPBASER, IDBITS), GICD_TYPER_IDBITS);

    if (!(cs->gicr_ctlr & GICR_CTLR_ENABLE_LPIS)) { return; }

    lpipt_baddr = cs->gicr_pendbaser & R_GICR_PENDBASER_PHYADDR_MASK;
    lpict_baddr = cs->gicr_propbaser & R_GICR_PROPBASER_PHYADDR_MASK;

    update_for_all_lpis(cs, lpipt_baddr, lpict_baddr, idbits, cs->gic->gicd_ctlr & GICD_CTLR_DS, &cs->hpplpi);
}
...
void gicv3_redist_lpi_pending(GICv3CPUState* cs, int irq, int level)
{
    ...
    if (level) {
        ...
        if (irq == cs->hpplpi.irq) { gicv3_redist_update_lpi(cs); }
    }
}
...
void gicv3_redist_process_lpi(GICv3CPUState* cs, int irq, int level)
{
    ...
    if (!(cs->gicr_ctlr & GICR_CTLR_ENABLE_LPIS) || (irq > (1ULL << (idbits + 1)) - 1) || irq < GICV3_LPI_INTID_START) {
        return;
    }
...
void gicv3_redist_mov_lpi(GICv3CPUState* src, GICv3CPUState* dest, int irq)
{
    ...
    if (!(src->gicr_ctlr & GICR_CTLR_ENABLE_LPIS) || !(dest->gicr_ctlr & GICR_CTLR_ENABLE_LPIS)) { return; }

    idbits = MIN(REG_FIELD_EX64(src->gicr_propbaser, GICR_PROPBASER, IDBITS), GICD_TYPER_IDBITS);
    idbits = MIN(REG_FIELD_EX64(dest->gicr_propbaser, GICR_PROPBASER, IDBITS), idbits);

    pendt_size = 1ULL << (idbits + 1);
    if ((irq / 8) >= pendt_size) { return; }
...
void gicv3_redist_movall_lpis(GICv3CPUState* src, GICv3CPUState* dest)
{
    ...
    for (i = GICV3_LPI_INTID_START / 8; i < pendt_size / 8; i++) {
        ...
        address_space_read(as, src_baddr + i, MEMTXATTRS_UNSPECIFIED, &src_pend, sizeof(src_pend));
        if (!src_pend) { continue; }
        address_space_read(as, dest_baddr + i, MEMTXATTRS_UNSPECIFIED, &dest_pend, sizeof(dest_pend));
        dest_pend |= src_pend;
        src_pend   = 0;
        address_space_write(as, src_baddr + i, MEMTXATTRS_UNSPECIFIED, &src_pend, sizeof(src_pend));
        address_space_write(as, dest_baddr + i, MEMTXATTRS_UNSPECIFIED, &dest_pend, sizeof(dest_pend));
    }
...
void gicv3_redist_vlpi_pending(GICv3CPUState* cs, int irq, int level)
{
    ...
    if (set_pending_table_bit(cs, vptbase, irq, level)) {
        if (level) {
            ...
            if (irq == cs->hppvlpi.irq) { gicv3_redist_update_vlpi(cs); }
        }
    }
}

void gicv3_redist_process_vlpi(GICv3CPUState* cs, int irq, uint64_t vptaddr, int doorbell, int level)
{
    bool     bit_changed;
    ...
    if (!resident && level && doorbell != INTID_SPURIOUS && (cs->gicr_ctlr & GICR_CTLR_ENABLE_LPIS)) {
        /* vCPU is not currently resident: ring the doorbell */
        gicv3_redist_process_lpi(cs, doorbell, 1);
    }
}

void gicv3_redist_mov_vlpi(GICv3CPUState* src, uint64_t src_vptaddr, GICv3CPUState* dest, uint64_t dest_vptaddr,
...
void gicv3_redist_set_irq(GICv3CPUState* cs, int irq, int level)
{
    ...
    if (level == extract32(cs->level, irq, 1)) { return; }
    ...
    if (level) {
        ...
        if (extract32(cs->edge_trigger, irq, 1)) { cs->gicr_ipendr0 = deposit32(cs->gicr_ipendr0, irq, 1, 1); }
    }
...
void gicv3_redist_send_sgi(GICv3CPUState* cs, int grp, int irq, bool ns)
{
    ...
    if (ns && !(cs->gic->gicd_ctlr & GICD_CTLR_DS)) {
        ...
        if ((irqgrp == GICV3_G0 && nsaccess < 1) || (irqgrp == GICV3_G1 && nsaccess < 2)) { return; }
    }
