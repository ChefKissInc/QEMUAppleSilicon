...
static uint64_t gicv2m_read(void* opaque, hwaddr offset, unsigned size)
{
    ARMGICv2mState* s = (ARMGICv2mState*)opaque;
    uint32_t        val;
    ...
    switch (offset) {
        ...
        case V2M_IIDR0 ... V2M_IIDR11:
             ...
             */
            return 0;
        default: qemu_log_mask(LOG_GUEST_ERROR, "gicv2m_read: Bad offset %x\n", (int)offset); return 0;
    }
}

static void gicv2m_write(void* opaque, hwaddr offset, uint64_t value, unsigned size)
{
    ARMGICv2mState* s = (ARMGICv2mState*)opaque;
    ...
    switch (offset) {
        case V2M_MSI_SETSPI_NS: {
            ...
            if (spi >= 0 && spi < s->num_spi) { gicv2m_set_irq(s, spi); }
            return;
        }
        default: qemu_log_mask(LOG_GUEST_ERROR, "gicv2m_write: Bad offset %x\n", (int)offset);
    }
}
...
static void gicv2m_realize(DeviceState* dev, Error** errp)
{
    ...
    if (s->num_spi > GICV2M_NUM_SPI_MAX) {
        error_setg(errp, "requested %u SPIs exceeds GICv2m frame maximum %d", s->num_spi, GICV2M_NUM_SPI_MAX);
        return;
    }

    if (s->base_spi + 32 > 1020 - s->num_spi) {
        error_setg(errp, "requested base SPI %u+%u exceeds max. number 1020", s->base_spi + 32, s->num_spi);
        return;
    }

    for (i = 0; i < s->num_spi; i++) { sysbus_init_irq(SYS_BUS_DEVICE(dev), &s->spi[i]); }
...
static void gicv2m_register_types(void) { type_register_static(&gicv2m_info); }
