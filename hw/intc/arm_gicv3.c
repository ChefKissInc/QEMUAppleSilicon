...
static bool irqbetter(GICv3CPUState* cs, int irq, uint8_t prio, bool nmi)
{
     ...
     * relies on).
     */
    if (prio != cs->hppi.prio) { return prio < cs->hppi.prio; }
     ...
     */
    if (nmi != cs->hppi.nmi) { return nmi; }

    /* If multiple pending interrupts have the same priority then it is an
     * IMPDEF choice which of them to signal to the CPU. We choose to
     * signal the one with the lowest interrupt number.
     */
    if (irq <= cs->hppi.irq) { return true; }
    return false;
}
...
static uint32_t gicd_int_pending(GICv3State* s, int irq)
{
    ...
    if (s->gicd_ctlr & GICD_CTLR_DS) { grpmod = 0; }

    grpmask = 0;
    if (s->gicd_ctlr & GICD_CTLR_EN_GRP1NS) { grpmask |= group; }
    if (s->gicd_ctlr & GICD_CTLR_EN_GRP1S) { grpmask |= (~group & grpmod); }
    if (s->gicd_ctlr & GICD_CTLR_EN_GRP0) { grpmask |= (~group & ~grpmod); }
    pend &= grpmask;
...
static uint32_t gicr_int_pending(GICv3CPUState* cs)
{
    ...
    if (cs->gic->gicd_ctlr & GICD_CTLR_DS) { grpmod = 0; }
    else {
        grpmod = cs->gicr_igrpmodr0;
    }

    grpmask = 0;
    if (cs->gic->gicd_ctlr & GICD_CTLR_EN_GRP1NS) { grpmask |= cs->gicr_igroupr0; }
    if (cs->gic->gicd_ctlr & GICD_CTLR_EN_GRP1S) { grpmask |= (~cs->gicr_igroupr0 & grpmod); }
    if (cs->gic->gicd_ctlr & GICD_CTLR_EN_GRP0) { grpmask |= (~cs->gicr_igroupr0 & ~grpmod); }
    pend &= grpmask;

    return pend;
}

static bool gicv3_get_priority(GICv3CPUState* cs, bool is_redist, int irq, uint8_t* prio)
{
    uint32_t nmi = 0x0;

    if (is_redist) { nmi = extract32(cs->gicr_inmir0, irq, 1); }
    else {
    ...
    if (is_redist) { *prio = cs->gicr_ipriorityr[irq]; }
    else {
...
static void gicv3_redist_update_noirqset(GICv3CPUState* cs)
{
    ...
    if (seenbetter) { cs->hppi.grp = gicv3_irq_group(cs->gic, cs, cs->hppi.irq); }

    if ((cs->gicr_ctlr & GICR_CTLR_ENABLE_LPIS) && cs->gic->lpi_enable && (cs->gic->gicd_ctlr & GICD_CTLR_EN_GRP1NS)
        && (cs->hpplpi.prio != 0xff))
    {
     ...
     */
    if (!seenbetter && cs->hppi.prio != 0xff && (cs->hppi.irq < GIC_INTERNAL || cs->hppi.irq >= GICV3_LPI_INTID_START))
    {
...
static void gicv3_update_noirqset(GICv3State* s, int start, int len)
{
    ...
    for (i = 0; i < s->num_cpu; i++) { s->cpu[i].seenbetter = false; }
    ...
    for (i = 0; i < s->num_cpu; i++) {
        ...
        if (cs->seenbetter) { cs->hppi.grp = gicv3_irq_group(cs->gic, cs, cs->hppi.irq); }

        if (!cs->seenbetter && cs->hppi.prio != 0xff && cs->hppi.irq >= start && cs->hppi.irq < start + len) {
...
void gicv3_update(GICv3State* s, int start, int len)
{
    ...
    for (i = 0; i < s->num_cpu; i++) { gicv3_cpuif_update(&s->cpu[i]); }
}
...
void gicv3_full_update_noirqset(GICv3State* s)
{
    ...
    for (i = 0; i < s->num_cpu; i++) { gicv3_redist_update_noirqset(&s->cpu[i]); }
}
...
static void arm_gicv3_post_load(GICv3State* s)
{
     ...
     */
    for (i = 0; i < s->num_cpu; i++) { gicv3_redist_update_lpi_only(&s->cpu[i]); }
    gicv3_full_update_noirqset(s);
...
static void arm_gicv3_register_types(void) { type_register_static(&arm_gicv3_info); }
