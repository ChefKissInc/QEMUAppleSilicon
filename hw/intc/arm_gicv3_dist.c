...
static uint32_t mask_group_and_nsacr(GICv3State* s, MemTxAttrs attrs, maskfn* maskfn, int irq)
{
...
static int gicd_ns_access(GICv3State* s, int irq)
{
     ...
     * specified interrupt.
     */
    if (irq < GIC_INTERNAL || irq >= s->num_irq) { return 0; }
    return extract32(s->gicd_nsacr[irq / 16], (irq % 16) * 2, 2);
}

static void gicd_write_bitmap_reg(GICv3State* s, MemTxAttrs attrs, uint32_t* bmp, maskfn* maskfn, int offset,
                                  uint32_t val)
{
    /*
...
static void gicd_write_set_bitmap_reg(GICv3State* s, MemTxAttrs attrs, uint32_t* bmp, maskfn* maskfn, int offset,
                                      uint32_t val)
{
...
static void gicd_write_clear_bitmap_reg(GICv3State* s, MemTxAttrs attrs, uint32_t* bmp, maskfn* maskfn, int offset,
                                        uint32_t val)
{
...
static uint32_t gicd_read_bitmap_reg(GICv3State* s, MemTxAttrs attrs, uint32_t* bmp, maskfn* maskfn, int offset)
{
...
static void gicd_write_ipriorityr(GICv3State* s, MemTxAttrs attrs, int irq, uint8_t value)
{
...
static uint64_t gicd_read_irouter(GICv3State* s, MemTxAttrs attrs, int irq)
{
    ...
    if (!attrs.secure && !(s->gicd_ctlr & GICD_CTLR_DS)) {
        ...
        if (!gicv3_gicd_group_test(s, irq)) {
            if (gicd_ns_access(s, irq) != 3) { return 0; }
        }
    }
...
static void gicd_write_irouter(GICv3State* s, MemTxAttrs attrs, int irq, uint64_t val)
{
    ...
    if (!attrs.secure && !(s->gicd_ctlr & GICD_CTLR_DS)) {
        /* RAZ/WI for NS accesses to secure interrupts */
        if (!gicv3_gicd_group_test(s, irq)) {
            if (gicd_ns_access(s, irq) != 3) { return; }
        }
    }
...
static bool gicd_readb(GICv3State* s, hwaddr offset, uint64_t* data, MemTxAttrs attrs)
{
...
static bool gicd_writeb(GICv3State* s, hwaddr offset, uint64_t value, MemTxAttrs attrs)
{
    /* Most GICv3 distributor registers do not support byte accesses. */
    switch (offset) {
        case GICD_CPENDSGIR ... GICD_CPENDSGIR + 0xf:
        ...
        case GICD_IPRIORITYR ... GICD_IPRIORITYR + 0x3ff: {
            ...
            if (irq < GIC_INTERNAL || irq >= s->num_irq) { return true; }
...
static bool gicd_readw(GICv3State* s, hwaddr offset, uint64_t* data, MemTxAttrs attrs)
{
...
static bool gicd_writew(GICv3State* s, hwaddr offset, uint64_t value, MemTxAttrs attrs)
{
    /* Only GICD_SETSPI_NSR, GICD_CLRSPI_NSR, GICD_SETSPI_SR and GICD_SETSPI_NSR
     * support 16 bit accesses, and those registers are all part of the
     * optional message-based SPI feature which this GIC does not currently
...
static bool gicd_readl(GICv3State* s, hwaddr offset, uint64_t* data, MemTxAttrs attrs)
{
    ...
    switch (offset) {
        case GICD_CTLR:
            if (!attrs.secure && !(s->gicd_ctlr & GICD_CTLR_DS)) {
                 ...
                 */
                *data = s->gicd_ctlr & (GICD_CTLR_ARE_S | GICD_CTLR_EN_GRP1NS | GICD_CTLR_RWP);
            }
            else {
                *data = s->gicd_ctlr;
            }
        ...
        case GICD_TYPER: {
            ...
            *data = (1 << 25) | (1 << 24) | (dvis << 18) | (sec_extn << 10) | (s->nmi_support << GICD_TYPER_NMI_SHIFT)
                    | (s->lpi_enable << GICD_TYPER_LPIS_SHIFT) | (0xf << 19) | itlinesnumber;
            return true;
        }
        case GICD_IIDR:
        ...
        case GICD_ISENABLER ... GICD_ISENABLER + 0x7f:
            *data = gicd_read_bitmap_reg(s, attrs, s->enabled, NULL, offset - GICD_ISENABLER);
            return true;
        case GICD_ICENABLER ... GICD_ICENABLER + 0x7f:
            *data = gicd_read_bitmap_reg(s, attrs, s->enabled, NULL, offset - GICD_ICENABLER);
            return true;
        case GICD_ISPENDR ... GICD_ISPENDR + 0x7f:
            *data = gicd_read_bitmap_reg(s, attrs, s->pending, mask_nsacr_ge1, offset - GICD_ISPENDR);
            return true;
        case GICD_ICPENDR ... GICD_ICPENDR + 0x7f:
            *data = gicd_read_bitmap_reg(s, attrs, s->pending, mask_nsacr_ge2, offset - GICD_ICPENDR);
            return true;
        case GICD_ISACTIVER ... GICD_ISACTIVER + 0x7f:
            *data = gicd_read_bitmap_reg(s, attrs, s->active, mask_nsacr_ge2, offset - GICD_ISACTIVER);
            return true;
        case GICD_ICACTIVER ... GICD_ICACTIVER + 0x7f:
            *data = gicd_read_bitmap_reg(s, attrs, s->active, mask_nsacr_ge2, offset - GICD_ICACTIVER);
            return true;
        case GICD_IPRIORITYR ... GICD_IPRIORITYR + 0x3ff: {
        ...
        case GICD_INMIR ... GICD_INMIR + 0x7f:
            *data = (!s->nmi_support) ? 0 : gicd_read_bitmap_reg(s, attrs, s->nmi, NULL, offset - GICD_INMIR);
            return true;
        ...
        case GICD_SGIR:
            ...
            qemu_log_mask(LOG_GUEST_ERROR, "%s: invalid guest read from WO register at offset " HWADDR_FMT_plx "\n",
                          __func__, offset);
            *data = 0;
            return true;
        default: return false;
...
static bool gicd_writel(GICv3State* s, hwaddr offset, uint64_t value, MemTxAttrs attrs)
{
    ...
    switch (offset) {
        ...
        case GICD_IGROUPR ... GICD_IGROUPR + 0x7f: {
            ...
            if (!attrs.secure && !(s->gicd_ctlr & GICD_CTLR_DS)) { return true; }
            /* RAZ/WI for SGIs, PPIs, unimplemented irqs */
            irq = (offset - GICD_IGROUPR) * 8;
            if (irq < GIC_INTERNAL || irq >= s->num_irq) { return true; }
        ...
        case GICD_ISENABLER ... GICD_ISENABLER + 0x7f:
            gicd_write_set_bitmap_reg(s, attrs, s->enabled, NULL, offset - GICD_ISENABLER, value);
            return true;
        case GICD_ICENABLER ... GICD_ICENABLER + 0x7f:
            gicd_write_clear_bitmap_reg(s, attrs, s->enabled, NULL, offset - GICD_ICENABLER, value);
            return true;
        case GICD_ISPENDR ... GICD_ISPENDR + 0x7f:
            gicd_write_set_bitmap_reg(s, attrs, s->pending, mask_nsacr_ge1, offset - GICD_ISPENDR, value);
            return true;
        case GICD_ICPENDR ... GICD_ICPENDR + 0x7f:
            gicd_write_clear_bitmap_reg(s, attrs, s->pending, mask_nsacr_ge2, offset - GICD_ICPENDR, value);
            return true;
        case GICD_ISACTIVER ... GICD_ISACTIVER + 0x7f:
            gicd_write_set_bitmap_reg(s, attrs, s->active, NULL, offset - GICD_ISACTIVER, value);
            return true;
        case GICD_ICACTIVER ... GICD_ICACTIVER + 0x7f:
            gicd_write_clear_bitmap_reg(s, attrs, s->active, NULL, offset - GICD_ICACTIVER, value);
            return true;
        case GICD_IPRIORITYR ... GICD_IPRIORITYR + 0x3ff: {
            int i, irq = offset - GICD_IPRIORITYR;

            if (irq < GIC_INTERNAL || irq + 3 >= s->num_irq) { return true; }

            for (i = irq; i < irq + 4; i++, value >>= 8) { gicd_write_ipriorityr(s, attrs, i, value); }
        ...
        case GICD_SPENDSGIR ... GICD_SPENDSGIR + 0xf:
            ...
            return true;
        case GICD_INMIR ... GICD_INMIR + 0x7f:
            if (s->nmi_support) { gicd_write_bitmap_reg(s, attrs, s->nmi, NULL, offset - GICD_INMIR, value); }
            return true;
        case GICD_IROUTER ... GICD_IROUTER + 0x1fdf: {
            uint64_t r;
            int      irq = (offset - GICD_IROUTER) / 8;
...
static bool gicd_writeq(GICv3State* s, hwaddr offset, uint64_t value, MemTxAttrs attrs)
{
...
static bool gicd_readq(GICv3State* s, hwaddr offset, uint64_t* data, MemTxAttrs attrs)
{
    /* Our only 64-bit registers are GICD_IROUTER<n> */
    int irq;
...
MemTxResult gicv3_dist_read(void* opaque, hwaddr offset, uint64_t* data, unsigned size, MemTxAttrs attrs)
{
    ...
    switch (size) {
        case 1 : r = gicd_readb(s, offset, data, attrs); break;
        case 2 : r = gicd_readw(s, offset, data, attrs); break;
        case 4 : r = gicd_readl(s, offset, data, attrs); break;
        case 8 : r = gicd_readq(s, offset, data, attrs); break;
...
MemTxResult gicv3_dist_write(void* opaque, hwaddr offset, uint64_t data, unsigned size, MemTxAttrs attrs)
{
    GICv3State* s = (GICv3State*)opaque;
    bool        r;

    switch (size) {
        case 1 : r = gicd_writeb(s, offset, data, attrs); break;
        case 2 : r = gicd_writew(s, offset, data, attrs); break;
        case 4 : r = gicd_writel(s, offset, data, attrs); break;
        case 8 : r = gicd_writeq(s, offset, data, attrs); break;
        default: r = false; break;
    }
...
void gicv3_dist_set_irq(GICv3State* s, int irq, int level)
{
    ...
    if (level == gicv3_gicd_level_test(s, irq)) { return; }
    ...
    if (level) {
        ...
        if (gicv3_gicd_edge_trigger_test(s, irq)) { gicv3_gicd_pending_set(s, irq); }
    }
