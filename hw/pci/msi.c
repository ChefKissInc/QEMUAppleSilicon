...
static inline uint8_t msi_cap_sizeof(uint16_t flags)
{
    switch (flags & (PCI_MSI_FLAGS_MASKBIT | PCI_MSI_FLAGS_64BIT)) {
        case PCI_MSI_FLAGS_MASKBIT | PCI_MSI_FLAGS_64BIT: return PCI_MSI_64M_SIZEOF;
...
#ifdef MSI_DEBUG
    #define MSI_DPRINTF(fmt, ...) fprintf(stderr, "%s:%d " fmt, __func__, __LINE__, ##__VA_ARGS__)
#else
    #define MSI_DPRINTF(fmt, ...) \
        do { }                    \
        while (0)
#endif
#define MSI_DEV_PRINTF(dev, fmt, ...) MSI_DPRINTF("%s:%x " fmt, (dev)->name, (dev)->devfn, ##__VA_ARGS__)

static inline unsigned int msi_nr_vectors(uint16_t flags)
{ return 1U << ((flags & PCI_MSI_FLAGS_QSIZE) >> ctz32(PCI_MSI_FLAGS_QSIZE)); }

static inline uint8_t msi_flags_off(const PCIDevice* dev) { return dev->msi_cap + PCI_MSI_FLAGS; }

static inline uint8_t msi_address_lo_off(const PCIDevice* dev) { return dev->msi_cap + PCI_MSI_ADDRESS_LO; }

static inline uint8_t msi_address_hi_off(const PCIDevice* dev) { return dev->msi_cap + PCI_MSI_ADDRESS_HI; }
...
void msi_set_message(PCIDevice* dev, MSIMessage msg)
{
    ...
    if (msi64bit) { pci_set_quad(dev->config + msi_address_lo_off(dev), msg.address); }
    else {
...
static MSIMessage msi_prepare_message(PCIDevice* dev, unsigned int vector)
{
    ...
    if (msi64bit) { msg.address = pci_get_quad(dev->config + msi_address_lo_off(dev)); }
    else {
...
MSIMessage msi_get_message(PCIDevice* dev, unsigned int vector) { return dev->msi_prepare_message(dev, vector); }

bool msi_enabled(const PCIDevice* dev)
{ return msi_present(dev) && (pci_get_word(dev->config + msi_flags_off(dev)) & PCI_MSI_FLAGS_ENABLE); }
 ...
 */
int msi_init(struct PCIDevice* dev, uint8_t offset, unsigned int nr_vectors, bool msi64bit, bool msi_per_vector_mask,
             Error** errp)
{
    ...
    MSI_DEV_PRINTF(dev, "init offset: 0x%" PRIx8 " vector: %" PRId8 " 64bit %d mask %d\n", offset, nr_vectors, msi64bit,
                   msi_per_vector_mask);
    ...
    pci_set_word(dev->wmask + msi_flags_off(dev), PCI_MSI_FLAGS_QSIZE | PCI_MSI_FLAGS_ENABLE);
    pci_set_long(dev->wmask + msi_address_lo_off(dev), PCI_MSI_ADDRESS_LO_MASK);
    if (msi64bit) { pci_set_long(dev->wmask + msi_address_hi_off(dev), 0xffffffff); }
    pci_set_word(dev->wmask + msi_data_off(dev, msi64bit), 0xffff);

    if (msi_per_vector_mask) {
        /* Make mask bits 0 to nr_vectors - 1 writable. */
        pci_set_long(dev->wmask + msi_mask_off(dev, msi64bit), 0xffffffff >> (PCI_MSI_VECTORS_MAX - nr_vectors));
    }
...
void msi_reset(PCIDevice* dev)
{
    ...
    if (msi64bit) { pci_set_long(dev->config + msi_address_hi_off(dev), 0); }
...
bool msi_is_masked(const PCIDevice* dev, unsigned int vector)
{
    ...
    mask = pci_get_long(dev->config + msi_mask_off(dev, flags & PCI_MSI_FLAGS_64BIT));
...
void msi_set_mask(PCIDevice* dev, int vector, bool mask, Error** errp)
{
    ...
    if (vector >= PCI_MSI_VECTORS_MAX) {
        error_setg(errp, "msi: vector %d not allocated. max vector is %d", vector, (PCI_MSI_VECTORS_MAX - 1));
        return;
    }
...
void msi_notify(PCIDevice* dev, unsigned int vector)
{
    ...
    if (msi_is_masked(dev, vector)) {
        ...
        pci_long_test_and_set_mask(dev->config + msi_pending_off(dev, msi64bit), 1U << vector);
...
void msi_send_message(PCIDevice* dev, MSIMessage msg) { dev->msi_trigger(dev, msg); }
...
void msi_write_config(PCIDevice* dev, uint32_t addr, uint32_t val, int len)
{
    ...
    if (!msi_present(dev) || !ranges_overlap(addr, len, dev->msi_cap, msi_cap_sizeof(flags))) { return; }

#ifdef MSI_DEBUG
    MSI_DEV_PRINTF(dev, "addr 0x%" PRIx32 " val 0x%" PRIx32 " len %d\n", addr, val, len);
    MSI_DEV_PRINTF(dev, "ctrl: 0x%" PRIx16 " address: 0x%" PRIx32, flags,
                   pci_get_long(dev->config + msi_address_lo_off(dev)));
    if (msi64bit) { fprintf(stderr, " address-hi: 0x%" PRIx32, pci_get_long(dev->config + msi_address_hi_off(dev))); }
    fprintf(stderr, " data: 0x%" PRIx16, pci_get_word(dev->config + msi_data_off(dev, msi64bit)));
    if (flags & PCI_MSI_FLAGS_MASKBIT) {
    ...
    if (!(flags & PCI_MSI_FLAGS_ENABLE)) { return; }
     ...
     */
    log_num_vecs = (flags & PCI_MSI_FLAGS_QSIZE) >> ctz32(PCI_MSI_FLAGS_QSIZE);
    log_max_vecs = (flags & PCI_MSI_FLAGS_QMASK) >> ctz32(PCI_MSI_FLAGS_QMASK);
    ...
    for (vector = 0; vector < nr_vectors; ++vector) {
        if (msi_is_masked(dev, vector) || !(pending & (1U << vector))) { continue; }

        pci_long_test_and_clear_mask(dev->config + msi_pending_off(dev, msi64bit), 1U << vector);
        msi_notify(dev, vector);
    }
}
