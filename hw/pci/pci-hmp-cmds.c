...
static void hmp_info_pci_device(Monitor* mon, const PciDeviceInfo* dev)
{
    ...
    if (dev->class_info->desc) { monitor_puts(mon, dev->class_info->desc); }
    else {
    ...
    if (dev->pci_bridge) {
        ...
        monitor_printf(mon, "      memory range [0x%08" PRIx64 ", 0x%08" PRIx64 "]\n",
                       dev->pci_bridge->bus->memory_range->base, dev->pci_bridge->bus->memory_range->limit);

        monitor_printf(mon,
                       "      prefetchable memory range "
                       "[0x%08" PRIx64 ", 0x%08" PRIx64 "]\n",
                       dev->pci_bridge->bus->prefetchable_range->base, dev->pci_bridge->bus->prefetchable_range->limit);
    }
    ...
    for (region = dev->regions; region; region = region->next) {
        ...
        if (!strcmp(region->value->type, "io")) {
            ...
            if (addr != PCI_BAR_UNMAPPED) {
                monitor_printf(mon, "%d bit%s memory at 0x%08" PRIx64 " [0x%08" PRIx64 "]\n",
    ...
    if (dev->pci_bridge) {
        if (dev->pci_bridge->has_devices) {
            ...
            for (cdev = dev->pci_bridge->devices; cdev; cdev = cdev->next) { hmp_info_pci_device(mon, cdev->value); }
        }
    }
}
...
void hmp_info_pci(Monitor* mon, const QDict* qdict)
{
    ...
    for (info = info_list; info; info = info->next) {
        ...
        for (dev = info->value->devices; dev; dev = dev->next) { hmp_info_pci_device(mon, dev->value); }
    }
...
void pcibus_dev_print(Monitor* mon, DeviceState* dev, int indent)
{
    ...
    if (desc->desc) { snprintf(ctxt, sizeof(ctxt), "%s", desc->desc); }
    else {
    ...
    monitor_printf(mon,
                   ...
                   indent, "", ctxt, pci_dev_bus_num(d), PCI_SLOT(d->devfn), PCI_FUNC(d->devfn),
                   pci_get_word(d->config + PCI_VENDOR_ID), pci_get_word(d->config + PCI_DEVICE_ID),
                   pci_get_word(d->config + PCI_SUBSYSTEM_VENDOR_ID), pci_get_word(d->config + PCI_SUBSYSTEM_ID));
    for (i = 0; i < PCI_NUM_REGIONS; i++) {
        r = &d->io_regions[i];
        if (!r->size) { continue; }
        monitor_printf(mon, "%*sbar %d: %s at 0x%" FMT_PCIBUS " [0x%" FMT_PCIBUS "]\n", indent, "", i,
                       r->type & PCI_BASE_ADDRESS_SPACE_IO ? "i/o" : "mem", r->addr, r->addr + r->size - 1);
    }
}
...
void hmp_pcie_aer_inject_error(Monitor* mon, const QDict* qdict)
{
    ...
    aer_err.flags = 0;
    if (correctable) { aer_err.flags |= PCIE_AER_ERR_IS_CORRECTABLE; }
    if (qdict_get_try_bool(qdict, "advisory_non_fatal", false)) { aer_err.flags |= PCIE_AER_ERR_MAYBE_ADVISORY; }
    if (qdict_haskey(qdict, "header0")) { aer_err.flags |= PCIE_AER_ERR_HEADER_VALID; }
    if (qdict_haskey(qdict, "prefix0")) { aer_err.flags |= PCIE_AER_ERR_TLP_PREFIX_PRESENT; }
