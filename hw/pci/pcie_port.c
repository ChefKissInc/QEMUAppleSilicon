...
void pcie_port_init_reg(PCIDevice* d)
{
    ...
    pci_word_test_and_clear_mask(d->wmask + PCI_BRIDGE_CONTROL,
                                 PCI_BRIDGE_CTL_MASTER_ABORT | PCI_BRIDGE_CTL_FAST_BACK | PCI_BRIDGE_CTL_DISCARD
                                     | PCI_BRIDGE_CTL_SEC_DISCARD | PCI_BRIDGE_CTL_DISCARD_STATUS
...
static struct PCIEChassis* pcie_chassis_find(uint8_t chassis_number)
{
    ...
    QLIST_FOREACH (c, &chassis, next) {
        if (c->number == chassis_number) { break; }
    }
    return c;
}
...
static PCIESlot* pcie_chassis_find_slot_with_chassis(struct PCIEChassis* c, uint8_t slot)
{
    PCIESlot* s;
...
int pcie_chassis_add_slot(struct PCIESlot* slot)
{
    ...
    if (!c) { return -ENODEV; }
    if (pcie_chassis_find_slot_with_chassis(c, slot->slot)) { return -EBUSY; }
...
static const Property pcie_port_props[] = {
    ...
    DEFINE_PROP_UINT16("aer_log_max", PCIEPort, parent_obj.parent_obj.exp.aer_log.log_max, PCIE_AER_LOG_MAX_DEFAULT),
};
...
PCIDevice* pcie_find_port_by_pn(PCIBus* bus, uint8_t pn)
{
    ...
    for (devfn = 0; devfn < ARRAY_SIZE(bus->devices); devfn++) {
        ...
        if (!d || !pci_is_express(d) || !d->exp.exp_cap) { continue; }

        if (!object_dynamic_cast(OBJECT(d), TYPE_PCIE_PORT)) { continue; }

        port = PCIE_PORT(d);
        if (port->port == pn) { return d; }
    }
...
PCIDevice* pcie_find_port_first(PCIBus* bus)
{
    ...
    for (devfn = 0; devfn < ARRAY_SIZE(bus->devices); devfn++) {
        ...
        if (object_dynamic_cast(OBJECT(d), TYPE_PCIE_PORT)) { return d; }
    }
...
int pcie_count_ds_ports(PCIBus* bus)
{
    ...
    for (devfn = 0; devfn < ARRAY_SIZE(bus->devices); devfn++) {
        ...
        if (!d || !pci_is_express(d) || !d->exp.exp_cap) { continue; }
        if (object_dynamic_cast(OBJECT(d), TYPE_PCIE_PORT)) { dsp_count++; }
    }
    return dsp_count;
}

static bool pcie_slot_is_hotpluggable_bus(HotplugHandler* plug_handler, BusState* bus)
{
