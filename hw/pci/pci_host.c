...
// #define DEBUG_PCI
    ...
    #define PCI_DPRINTF(fmt, ...)                         \
        do {                                              \
            printf("pci_host_data: " fmt, ##__VA_ARGS__); \
        }                                                 \
        while (0)
#else
    #define PCI_DPRINTF(fmt, ...)
...
{
    if ((*limit > PCI_CONFIG_SPACE_SIZE) && !pci_bus_allows_extended_config_space(bus)) {
...
static bool is_pci_dev_ejected(PCIDevice* pci_dev)
{
     ...
     */
    return pci_dev && pci_dev->partially_hotplugged && !pci_dev->qdev.pending_deleted_event;
}

void pci_host_config_write_common(PCIDevice* pci_dev, uint32_t addr, uint32_t limit, uint32_t val, uint32_t len)
{
     ...
     */
    if ((pci_dev->qdev.hotplugged && !pci_get_function_0(pci_dev)) || !pci_dev->enabled || is_pci_dev_ejected(pci_dev))
    {
        return;
    }

    trace_pci_cfg_write(pci_dev->name, pci_dev_bus_num(pci_dev), PCI_SLOT(pci_dev->devfn), PCI_FUNC(pci_dev->devfn),
                        addr, val);
...
uint32_t pci_host_config_read_common(PCIDevice* pci_dev, uint32_t addr, uint32_t limit, uint32_t len)
{
    ...
    trace_pci_cfg_read(pci_dev->name, pci_dev_bus_num(pci_dev), PCI_SLOT(pci_dev->devfn), PCI_FUNC(pci_dev->devfn),
                       addr, ret);
...
void pci_data_write(PCIBus* s, uint32_t addr, uint32_t val, unsigned len)
{
    ...
    if (!pci_dev) {
        trace_pci_cfg_write("empty", extract32(addr, 16, 8), extract32(addr, 11, 5), extract32(addr, 8, 3), config_addr,
                            val);
        return;
    }
...
uint32_t pci_data_read(PCIBus* s, uint32_t addr, unsigned len)
{
    ...
    if (!pci_dev) {
        trace_pci_cfg_read("empty", extract32(addr, 16, 8), extract32(addr, 11, 5), extract32(addr, 8, 3), config_addr,
                           ~0x0);
        return ~0x0;
    }

    return pci_host_config_read_common(pci_dev, config_addr, PCI_CONFIG_SPACE_SIZE, len);
}
...
static void pci_host_data_write(void* opaque, hwaddr addr, uint64_t val, unsigned len)
{
    ...
    if (s->config_reg & (1u << 31)) { pci_data_write(s->bus, s->config_reg | (addr & 3), val, len); }
}
...
static uint64_t pci_host_data_read(void* opaque, hwaddr addr, unsigned len)
{
    ...
    if (!(s->config_reg & (1U << 31))) { return 0xffffffff; }
