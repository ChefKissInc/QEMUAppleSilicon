...
int slotid_cap_init(PCIDevice* d, int nslots, uint8_t chassis, unsigned offset, Error** errp)
{
    int cap;
    ...
    cap = pci_add_capability(d, PCI_CAP_ID_SLOTID, offset, SLOTID_CAP_LENGTH, errp);
    if (cap < 0) { return cap; }
    /* We make each chassis unique, this way each bridge is First in Chassis */
    d->config[cap + PCI_SID_ESR]        = PCI_SID_ESR_FIC | (nslots << SLOTID_NSLOTS_SHIFT);
