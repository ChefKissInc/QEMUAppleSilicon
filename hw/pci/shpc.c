...
#define SHPC_SLOT_STATE_SHIFT ctz32(SHPC_SLOT_STATE_MASK)
...
#define SHPC_SLOT_PWR_LED_SHIFT  ctz32(SHPC_SLOT_PWR_LED_MASK)
#define SHPC_SLOT_ATTN_LED_MASK  0x30
#define SHPC_SLOT_ATTN_LED_SHIFT ctz32(SHPC_SLOT_ATTN_LED_MASK)
...
{
    switch (value) {
        case SHPC_LED_ON   : return "on";
        case SHPC_LED_BLINK: return "blink";
        case SHPC_LED_OFF  : return "off";
...
{
    switch (value) {
        case SHPC_STATE_PWRONLY : return "power-only";
        case SHPC_STATE_ENABLED : return "enabled";
        case SHPC_STATE_DISABLED: return "disabled";
        default                 : return "invalid";
    }
}
...
static void shpc_set_status(SHPCDevice* shpc, int slot, uint8_t value, uint16_t msk)
{
    uint8_t* status = shpc->config + SHPC_SLOT_STATUS(slot);
...
static void shpc_interrupt_update(PCIDevice* d)
{
    ...
    for (slot = 0; slot < shpc->nslots; ++slot) {
        ...
        if (event & ~disable) { int_locator |= mask; }
    }
    serr_int = pci_get_long(shpc->config + SHPC_SERR_INT);
    if ((serr_int & SHPC_CMD_DETECTED) && !(serr_int & SHPC_CMD_INT_DIS)) { int_locator |= SHPC_INT_COMMAND; }
    pci_set_long(shpc->config + SHPC_INT_LOCATOR, int_locator);
    level = (!(serr_int & SHPC_INT_DIS) && int_locator) ? 1 : 0;
    if (msi_enabled(d) && shpc->msi_requested != level) { msi_notify(d, 0); }
    else {
        pci_set_irq(d, level);
    }
...
static void shpc_set_sec_bus_speed(SHPCDevice* shpc, uint8_t speed)
{
    switch (speed) {
        case SHPC_SEC_BUS_33:
            ...
            break;
        default: pci_word_test_and_set_mask(shpc->config + SHPC_CMD_STATUS, SHPC_CMD_STATUS_INVALID_MODE);
    }
}
...
void shpc_reset(PCIDevice* d)
{
    ...
    pci_set_word(shpc->config + SHPC_PHYS_SLOT,
                 SHPC_IDX_TO_PHYSICAL(0) | SHPC_PHYS_NUM_UP | SHPC_PHYS_MRL | SHPC_PHYS_BUTTON);
    pci_set_long(shpc->config + SHPC_SERR_INT, SHPC_INT_DIS | SHPC_SERR_DIS | SHPC_CMD_INT_DIS | SHPC_ARB_SERR_DIS);
    ...
    for (i = 0; i < shpc->nslots; ++i) {
        pci_set_byte(shpc->config + SHPC_SLOT_EVENT_SERR_INT_DIS(d, i),
                     SHPC_SLOT_EVENT_PRESENCE | SHPC_SLOT_EVENT_ISOLATED_FAULT | SHPC_SLOT_EVENT_BUTTON
                         | SHPC_SLOT_EVENT_MRL | SHPC_SLOT_EVENT_CONNECTED_FAULT | SHPC_SLOT_EVENT_MRL_SERR_DIS
        ...
        if (shpc->sec_bus->devices[PCI_DEVFN(SHPC_IDX_TO_PCI(i), 0)]) {
            ...
            shpc_set_status(shpc, i, SHPC_SLOT_STATUS_PRSNT_7_5W, SHPC_SLOT_STATUS_PRSNT_MASK);
            ...
            shpc_set_status(shpc, i, SHPC_SLOT_STATUS_PRSNT_EMPTY, SHPC_SLOT_STATUS_PRSNT_MASK);
...
{ pci_word_test_and_set_mask(shpc->config + SHPC_CMD_STATUS, SHPC_CMD_STATUS_INVALID_CMD); }
...
static void shpc_free_devices_in_slot(SHPCDevice* shpc, int slot)
{
    ...
    for (devfn = PCI_DEVFN(pci_slot, 0); devfn <= PCI_DEVFN(pci_slot, PCI_FUNC_MAX - 1); ++devfn) {
        PCIDevice* affected_dev = shpc->sec_bus->devices[devfn];
        if (affected_dev) {
            hotplug_ctrl = qdev_get_hotplug_handler(DEVICE(affected_dev));
            hotplug_handler_unplug(hotplug_ctrl, DEVICE(affected_dev), &error_abort);
...
static void shpc_slot_command(PCIDevice* d, uint8_t target, uint8_t state, uint8_t power, uint8_t attn)
{
    SHPCDevice* shpc      = d->shpc;
    ...
    if (power == SHPC_LED_NO) { power = old_power; }
    else {
        /* TODO: send event to monitor */
        shpc_set_status(shpc, slot, power, SHPC_SLOT_PWR_LED_MASK);
    }

    if (attn == SHPC_LED_NO) { attn = old_attn; }
    else {
        /* TODO: send event to monitor */
        shpc_set_status(shpc, slot, attn, SHPC_SLOT_ATTN_LED_MASK);
    }

    if (state == SHPC_STATE_NO) { state = old_state; }
    else {
    ...
    if (trace_event_get_state_backends(TRACE_SHPC_SLOT_COMMAND)) {
        ...
        int          pci_slot = SHPC_IDX_TO_PCI(slot);
        DeviceState* child    = DEVICE(shpc->sec_bus->devices[PCI_DEVFN(pci_slot, 0)]);

        trace_shpc_slot_command(parent->canonical_path, pci_slot, child ? child->canonical_path : "no-child",
                                shpc_led_state_to_str(old_power), shpc_led_state_to_str(power),
                                shpc_led_state_to_str(old_attn), shpc_led_state_to_str(attn),
                                shpc_slot_state_to_str(old_state), shpc_slot_state_to_str(state));
    }

    if (!shpc_slot_is_off(old_state, old_power, old_attn) && shpc_slot_is_off(state, power, attn)) {
        shpc_free_devices_in_slot(shpc, slot);
        shpc_set_status(shpc, slot, 1, SHPC_SLOT_STATUS_MRL_OPEN);
        shpc_set_status(shpc, slot, SHPC_SLOT_STATUS_PRSNT_EMPTY, SHPC_SLOT_STATUS_PRSNT_MASK);
        shpc->config[SHPC_SLOT_EVENT_LATCH(slot)] |= SHPC_SLOT_EVENT_MRL | SHPC_SLOT_EVENT_PRESENCE;
    }
}
...
static void shpc_command(PCIDevice* d)
{
    ...
    pci_word_test_and_clear_mask(shpc->config + SHPC_CMD_STATUS, SHPC_CMD_STATUS_BUSY | SHPC_CMD_STATUS_MRL_OPEN
        ...
        case 0x48:
            ...
            for (i = 0; i < shpc->nslots; ++i) {
                if (!(shpc_get_status(shpc, i, SHPC_SLOT_STATUS_MRL_OPEN))) {
                    shpc_slot_command(d, i + SHPC_CMD_TRGT_MIN, SHPC_STATE_PWRONLY, SHPC_LED_ON, SHPC_LED_NO);
                }
                else {
                    shpc_slot_command(d, i + SHPC_CMD_TRGT_MIN, SHPC_STATE_NO, SHPC_LED_OFF, SHPC_LED_NO);
                }
            }
            break;
        case 0x49:
            ...
            for (i = 0; i < shpc->nslots; ++i) {
                if (!(shpc_get_status(shpc, i, SHPC_SLOT_STATUS_MRL_OPEN))) {
                    shpc_slot_command(d, i + SHPC_CMD_TRGT_MIN, SHPC_STATE_ENABLED, SHPC_LED_ON, SHPC_LED_NO);
                }
                else {
                    shpc_slot_command(d, i + SHPC_CMD_TRGT_MIN, SHPC_STATE_NO, SHPC_LED_OFF, SHPC_LED_NO);
                }
            ...
            break;
        default: shpc_invalid_command(shpc); break;
    }
done:
...
static void shpc_write(PCIDevice* d, unsigned addr, uint64_t val, int l)
{
    ...
    if (ranges_overlap(addr, l, SHPC_CMD_CODE, 2)) { shpc_command(d); }
    shpc_interrupt_update(d);
}
...
{
    uint64_t val = 0x0;
    if (addr >= SHPC_SIZEOF(d)) { return val; }
    l = MIN(l, SHPC_SIZEOF(d) - addr);
...
static int shpc_cap_add_config(PCIDevice* d, Error** errp)
{
    uint8_t* config;
    int      config_offset;
    config_offset = pci_add_capability(d, PCI_CAP_ID_SHPC, 0, SHPC_CAP_LENGTH, errp);
    if (config_offset < 0) { return config_offset; }
...
static uint64_t shpc_mmio_read(void* opaque, hwaddr addr, unsigned size) { return shpc_read(opaque, addr, size); }
...
static bool shpc_device_get_slot(PCIDevice* affected_dev, int* slot, SHPCDevice* shpc, Error** errp)
{
    ...
    *slot        = SHPC_PCI_TO_IDX(pci_slot);
    ...
    if (pci_slot < SHPC_IDX_TO_PCI(0) || *slot >= shpc->nslots) {
        error_setg(errp,
                   ...
                   pci_slot, SHPC_IDX_TO_PCI(0), SHPC_IDX_TO_PCI(shpc->nslots) - 1);
        return false;
    }
...
void shpc_device_plug_cb(HotplugHandler* hotplug_dev, DeviceState* dev, Error** errp)
{
    PCIDevice*  pci_hotplug_dev = PCI_DEVICE(hotplug_dev);
    SHPCDevice* shpc            = pci_hotplug_dev->shpc;
    int         slot;

    if (!shpc_device_get_slot(PCI_DEVICE(dev), &slot, shpc, errp)) { return; }
    ...
    if (!dev->hotplugged) {
        ...
        shpc_set_status(shpc, slot, SHPC_SLOT_STATUS_PRSNT_7_5W, SHPC_SLOT_STATUS_PRSNT_MASK);
        return;
    }
    ...
    if (shpc_get_status(shpc, slot, SHPC_SLOT_STATUS_MRL_OPEN)) {
        ...
        shpc->config[SHPC_SLOT_EVENT_LATCH(slot)] |=
            SHPC_SLOT_EVENT_BUTTON | SHPC_SLOT_EVENT_MRL | SHPC_SLOT_EVENT_PRESENCE;
    }
    else {
...
void shpc_device_unplug_cb(HotplugHandler* hotplug_dev, DeviceState* dev, Error** errp) { qdev_unrealize(dev); }

void shpc_device_unplug_request_cb(HotplugHandler* hotplug_dev, DeviceState* dev, Error** errp)
{
    PCIDevice*  pci_hotplug_dev = PCI_DEVICE(hotplug_dev);
    SHPCDevice* shpc            = pci_hotplug_dev->shpc;
    uint8_t     state;
...
int shpc_init(PCIDevice* d, PCIBus* sec_bus, MemoryRegion* bar, unsigned offset, Error** errp)
{
    int         i, ret;
    ...
    if (nslots < SHPC_MIN_SLOTS) { return 0; }
    if (nslots > SHPC_MAX_SLOTS || SHPC_IDX_TO_PCI(nslots) > PCI_SLOT_MAX) {
    ...
    pci_set_byte(shpc->wmask + SHPC_CMD_TRGT, SHPC_CMD_TRGT_MAX);
    pci_set_long(shpc->wmask + SHPC_SERR_INT, SHPC_INT_DIS | SHPC_SERR_DIS | SHPC_CMD_INT_DIS | SHPC_ARB_SERR_DIS);
    pci_set_long(shpc->w1cmask + SHPC_SERR_INT, SHPC_CMD_DETECTED | SHPC_ARB_DETECTED);
    for (i = 0; i < nslots; ++i) {
        pci_set_byte(shpc->wmask + SHPC_SLOT_EVENT_SERR_INT_DIS(d, i),
                     SHPC_SLOT_EVENT_PRESENCE | SHPC_SLOT_EVENT_ISOLATED_FAULT | SHPC_SLOT_EVENT_BUTTON
                         | SHPC_SLOT_EVENT_MRL | SHPC_SLOT_EVENT_CONNECTED_FAULT | SHPC_SLOT_EVENT_MRL_SERR_DIS
                         | SHPC_SLOT_EVENT_CONNECTED_FAULT_SERR_DIS);
        pci_set_byte(shpc->w1cmask + SHPC_SLOT_EVENT_LATCH(i), SHPC_SLOT_EVENT_PRESENCE | SHPC_SLOT_EVENT_ISOLATED_FAULT
                                                                   | SHPC_SLOT_EVENT_BUTTON | SHPC_SLOT_EVENT_MRL
                                                                   | SHPC_SLOT_EVENT_CONNECTED_FAULT);
    }

    /* TODO: init cmask */
    memory_region_init_io(&shpc->mmio, OBJECT(d), &shpc_mmio_ops, d, "shpc-mmio", SHPC_SIZEOF(d));
...
int shpc_bar_size(PCIDevice* d) { return pow2roundup32(SHPC_SLOT_REG(SHPC_MAX_SLOTS)); }
...
{
    if (!ranges_overlap(addr, l, d->shpc->cap, SHPC_CAP_LENGTH)) { return; }
    if (ranges_overlap(addr, l, d->shpc->cap + SHPC_CAP_DWORD_DATA, 4)) {
        unsigned dword_data;
        dword_data = pci_get_long(d->shpc->config + d->shpc->cap + SHPC_CAP_DWORD_DATA);
