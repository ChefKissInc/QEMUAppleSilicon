...
static void pcie_pci_bridge_realize(PCIDevice* d, Error** errp)
{
    ...
    if (pcie_br->msi != ON_OFF_AUTO_OFF) {
        ...
        if (rc < 0) {
            assert(rc == -ENOTSUP);
            if (pcie_br->msi != ON_OFF_AUTO_ON) { error_free(local_err); }
            else {
    ...
    pci_register_bar(d, 0, PCI_BASE_ADDRESS_SPACE_MEMORY | PCI_BASE_ADDRESS_MEM_TYPE_64, &pcie_br->shpc_bar);
    return;
...
static void pciepci_register(void) { type_register_static(&pcie_pci_bridge_info); }
