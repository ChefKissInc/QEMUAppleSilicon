...
static const uint32_t nvme_cse_acs_default[256] = {
    ...
    [NVME_ADM_CMD_NS_ATTACHMENT]  = NVME_CMD_EFF_CSUPP | NVME_CMD_EFF_NIC | NVME_CMD_EFF_CCC,
...
static uint16_t nvme_sqid(NvmeRequest* req) { return le16_to_cpu(req->sq->sqid); }
...
static inline bool nvme_ph_valid(NvmeNamespace* ns, uint16_t ph) { return ph < ns->fdp.nphs; }

static inline bool nvme_rg_valid(NvmeEnduranceGroup* endgrp, uint16_t rg) { return rg < endgrp->fdp.nrg; }
...
static inline bool nvme_parse_pid(NvmeNamespace* ns, uint16_t pid, uint16_t* ph, uint16_t* rg)
{
...
static void nvme_assign_zone_state(NvmeNamespace* ns, NvmeZone* zone, NvmeZoneState state)
{
    if (QTAILQ_IN_USE(zone, entry)) {
        switch (nvme_get_zone_state(zone)) {
            case NVME_ZONE_STATE_EXPLICITLY_OPEN: QTAILQ_REMOVE(&ns->exp_open_zones, zone, entry); break;
            case NVME_ZONE_STATE_IMPLICITLY_OPEN: QTAILQ_REMOVE(&ns->imp_open_zones, zone, entry); break;
            case NVME_ZONE_STATE_CLOSED         : QTAILQ_REMOVE(&ns->closed_zones, zone, entry); break;
            case NVME_ZONE_STATE_FULL           : QTAILQ_REMOVE(&ns->full_zones, zone, entry);
            default                             :;
        }
    }
    ...
    switch (state) {
        case NVME_ZONE_STATE_EXPLICITLY_OPEN: QTAILQ_INSERT_TAIL(&ns->exp_open_zones, zone, entry); break;
        case NVME_ZONE_STATE_IMPLICITLY_OPEN: QTAILQ_INSERT_TAIL(&ns->imp_open_zones, zone, entry); break;
        case NVME_ZONE_STATE_CLOSED         : QTAILQ_INSERT_TAIL(&ns->closed_zones, zone, entry); break;
        case NVME_ZONE_STATE_FULL           : QTAILQ_INSERT_TAIL(&ns->full_zones, zone, entry);
        case NVME_ZONE_STATE_READ_ONLY      : break;
        default                             : zone->d.za = 0;
    }
}

static uint16_t nvme_zns_check_resources(NvmeNamespace* ns, uint32_t act, uint32_t opn, uint32_t zrwa)
{
    if (ns->params.max_active_zones != 0 && ns->nr_active_zones + act > ns->params.max_active_zones) {
        trace_pci_nvme_err_insuff_active_res(ns->params.max_active_zones);
        return NVME_ZONE_TOO_MANY_ACTIVE | NVME_DNR;
    }

    if (ns->params.max_open_zones != 0 && ns->nr_open_zones + opn > ns->params.max_open_zones) {
        trace_pci_nvme_err_insuff_open_res(ns->params.max_open_zones);
        return NVME_ZONE_TOO_MANY_OPEN | NVME_DNR;
    }

    if (zrwa > ns->zns.numzrwa) { return NVME_NOZRWA | NVME_DNR; }
...
static NvmeFdpEvent* nvme_fdp_alloc_event(NvmeCtrl* n, NvmeFdpEventBuffer* ebuf)
{
    ...
    if (unlikely(ebuf->next == NVME_FDP_MAX_EVENTS)) { ebuf->next = 0; }
    if (is_full) { ebuf->start = ebuf->next; }
    else {
        ebuf->nelems++;
    }
...
static bool nvme_update_ruh(NvmeCtrl* n, NvmeNamespace* ns, uint16_t pid)
{
    ...
    if (!nvme_parse_pid(ns, pid, &ph, &rg)) { return false; }
...
{ return nsid && (nsid == NVME_NSID_BROADCAST || nsid <= NVME_MAX_NAMESPACES); }
...
static void nvme_inc_sq_head(NvmeSQueue* sq) { sq->head = (sq->head + 1) % sq->size; }

static uint8_t nvme_cq_full(NvmeCQueue* cq) { return (cq->tail + 1) % cq->size == cq->head; }

static uint8_t nvme_sq_empty(NvmeSQueue* sq) { return sq->head == sq->tail; }
...
static void nvme_irq_check(NvmeCtrl* n)
{
    ...
    if (msix_enabled(pci)) { return; }

    if (~intms & n->irq_status) { pci_irq_assert(pci); }
    else {
...
{
    if (cq->irq_enabled) {
        if (msix_enabled(&n->parent_obj)) { return; }
        else {
            assert(cq->vector < 32);
            if (!n->cq_pending) { n->irq_status &= ~(1 << cq->vector); }
            nvme_irq_check(n);
        }
    }
}
...
{
    if (!(sg->flags & NVME_SG_ALLOC)) { return; }

    if (sg->flags & NVME_SG_DMA) { qemu_sglist_destroy(&sg->qsg); }
    else {
 ...
 */
static void nvme_sg_split(NvmeSg* sg, NvmeNamespace* ns, NvmeSg* data, NvmeSg* mdata)
{
    NvmeSg*  dst = data;
    ...
    while (sg_len) {
        ...
        if (dst) {
            if (dma) { qemu_sglist_add(&dst->qsg, sg->qsg.sg[sg_idx].base + offset, trans_len); }
            else {
                qemu_iovec_add(&dst->iov, sg->iov.iov[sg_idx].iov_base + offset, trans_len);
            }
        }
...
static uint16_t nvme_map_addr_cmb(NvmeCtrl* n, QEMUIOVector* iov, hwaddr addr, size_t len)
{
    if (!len) { return NVME_SUCCESS; }

    trace_pci_nvme_map_addr_cmb(addr, len);

    if (!nvme_addr_is_cmb(n, addr) || !nvme_addr_is_cmb(n, addr + len - 1)) { return NVME_DATA_TRAS_ERROR; }
...
static uint16_t nvme_map_addr_pmr(NvmeCtrl* n, QEMUIOVector* iov, hwaddr addr, size_t len)
{
    if (!len) { return NVME_SUCCESS; }

    if (!nvme_addr_is_pmr(n, addr) || !nvme_addr_is_pmr(n, addr + len - 1)) { return NVME_DATA_TRAS_ERROR; }
...
static uint16_t nvme_map_addr(NvmeCtrl* n, NvmeSg* sg, hwaddr addr, size_t len)
{
    ...
    if (nvme_addr_is_iomem(n, addr)) { return NVME_DATA_TRAS_ERROR; }

    if (nvme_addr_is_cmb(n, addr)) { cmb = true; }
    else if (nvme_addr_is_pmr(n, addr)) {
        pmr = true;
    }

    if (cmb || pmr) {
        if (sg->flags & NVME_SG_DMA) { return NVME_INVALID_USE_OF_CMB | NVME_DNR; }

        if (sg->iov.niov + 1 > IOV_MAX) { goto max_mappings_exceeded; }

        if (cmb) { return nvme_map_addr_cmb(n, &sg->iov, addr, len); }
        else {
    ...
    if (sg->qsg.nsg + 1 > IOV_MAX) { goto max_mappings_exceeded; }
...
max_mappings_exceeded:
    NVME_GUEST_ERR(pci_nvme_ub_too_many_mappings, "number of mappings exceed 1024");
...
static uint16_t nvme_map_prp(NvmeCtrl* n, NvmeSg* sg, uint64_t prp1, uint64_t prp2, uint32_t len)
{
        ...
        if (len > n->page_size) {
            ...
            while (len != 0) {
                ...
                if (i == nents - 1 && len > n->page_size) {
                    ...
                    ret       = nvme_addr_read(n, prp_ent, (void*)prp_list, prp_trans);
                    if (ret) {
 ...
 */
static uint16_t nvme_map_sgl_data(NvmeCtrl* n, NvmeSg* sg, NvmeSglDescriptor* segment, uint64_t nsgld, size_t* len,
                                  NvmeCmd* cmd)
{
    ...
    for (int i = 0; i < nsgld; i++) {
        ...
        switch (type) {
            case NVME_SGL_DESCR_TYPE_DATA_BLOCK  : break;
            case NVME_SGL_DESCR_TYPE_SEGMENT     :
            case NVME_SGL_DESCR_TYPE_LAST_SEGMENT: return NVME_INVALID_NUM_SGL_DESCRS | NVME_DNR;
            default                              : return NVME_SGL_DESCR_TYPE_INVALID | NVME_DNR;
        }
        ...
        if (*len == 0) {
            ...
            if (sgls & NVME_CTRL_SGLS_EXCESS_LENGTH) { break; }
        ...
        if (UINT64_MAX - addr < dlen) { return NVME_DATA_SGL_LEN_INVALID | NVME_DNR; }
...
static uint16_t nvme_map_sgl(NvmeCtrl* n, NvmeSg* sg, NvmeSglDescriptor sgl, size_t len, NvmeCmd* cmd)
{
    /*
...
#define SEG_CHUNK_SIZE 256
    ...
    for (;;) {
        ...
        if (!seg_len || seg_len & 0xf) { return NVME_INVALID_SGL_SEG_DESCR | NVME_DNR; }

        if (UINT64_MAX - addr < seg_len) { return NVME_DATA_SGL_LEN_INVALID | NVME_DNR; }
        ...
        while (nsgld > SEG_CHUNK_SIZE) {
            ...
            status = nvme_map_sgl_data(n, sg, segment, SEG_CHUNK_SIZE, &len, cmd);
            if (status) { goto unmap; }
...
uint16_t nvme_map_dptr(NvmeCtrl* n, NvmeSg* sg, size_t len, NvmeCmd* cmd)
{
    ...
    switch (NVME_CMD_FLAGS_PSDT(cmd->flags)) {
        ...
        case NVME_PSDT_SGL_MPTR_SGL       : return nvme_map_sgl(n, sg, cmd->dptr.sgl, len, cmd);
        default                           : return NVME_INVALID_FIELD;
    }
}

static uint16_t nvme_map_mptr(NvmeCtrl* n, NvmeSg* sg, size_t len, NvmeCmd* cmd)
{
    ...
    if (psdt == NVME_PSDT_SGL_MPTR_SGL) {
        ...
        if (nvme_addr_read(n, mptr, &sgl, sizeof(sgl))) { return NVME_DATA_TRAS_ERROR; }

        status = nvme_map_sgl(n, sg, sgl, len, cmd);
        if (status && (status & 0x7ff) == NVME_DATA_SGL_LEN_INVALID) { status = NVME_MD_SGL_LEN_INVALID | NVME_DNR; }
...
static uint16_t nvme_map_data(NvmeCtrl* n, uint32_t nlb, NvmeRequest* req)
{
    ...
    if (nvme_ns_ext(ns) && !(pi && pract && ns->lbaf.ms == nvme_pi_tuple_size(ns))) {
        NvmeSg sg;
...
static uint16_t nvme_tx_interleaved(NvmeCtrl* n, NvmeSg* sg, uint8_t* ptr, uint32_t len, uint32_t bytes,
                                    int32_t skip_bytes, int64_t offset, NvmeTxDirection dir)
{
    hwaddr   addr;
    ...
    while (len) {
        ...
        if (dma) { addr = sg->qsg.sg[sg_idx].base + offset; }
        else {
            addr = (hwaddr)(uintptr_t)sg->iov.iov[sg_idx].iov_base + offset;
        }

        if (dir == NVME_TX_DIRECTION_TO_DEVICE) { ret = nvme_addr_read(n, addr, ptr, trans_len); }
        else {
            ret = nvme_addr_write(n, addr, ptr, trans_len);
        }

        if (ret) { return NVME_DATA_TRAS_ERROR; }
...
static uint16_t nvme_tx(NvmeCtrl* n, NvmeSg* sg, void* ptr, uint32_t len, NvmeTxDirection dir)
{
    assert(sg->flags & NVME_SG_ALLOC);
    ...
    if (sg->flags & NVME_SG_DMA) {
        ...
        if (dir == NVME_TX_DIRECTION_TO_DEVICE) { dma_buf_write(ptr, len, &residual, &sg->qsg, attrs); }
        else {
        ...
        if (dir == NVME_TX_DIRECTION_TO_DEVICE) { bytes = qemu_iovec_to_buf(&sg->iov, 0, ptr, len); }
        else {
...
static inline uint16_t nvme_c2h(NvmeCtrl* n, void* ptr, uint32_t len, NvmeRequest* req)
{
    uint16_t status;
...
static inline uint16_t nvme_h2c(NvmeCtrl* n, void* ptr, uint32_t len, NvmeRequest* req)
{
    uint16_t status;
...
uint16_t nvme_bounce_data(NvmeCtrl* n, void* ptr, uint32_t len, NvmeTxDirection dir, NvmeRequest* req)
{
    ...
    if (nvme_ns_ext(ns) && !(pi && pract && ns->lbaf.ms == nvme_pi_tuple_size(ns))) {
        return nvme_tx_interleaved(n, &req->sg, ptr, len, ns->lbasz, ns->lbaf.ms, 0, dir);
    }
...
uint16_t nvme_bounce_mdata(NvmeCtrl* n, void* ptr, uint32_t len, NvmeTxDirection dir, NvmeRequest* req)
{
    ...
    if (nvme_ns_ext(ns)) { return nvme_tx_interleaved(n, &req->sg, ptr, len, ns->lbaf.ms, ns->lbasz, ns->lbasz, dir); }
...
static inline void nvme_blk_read(BlockBackend* blk, int64_t offset, uint32_t align, BlockCompletionFunc* cb,
                                 NvmeRequest* req)
{
    assert(req->sg.flags & NVME_SG_ALLOC);

    if (req->sg.flags & NVME_SG_DMA) { req->aiocb = dma_blk_read(blk, &req->sg.qsg, offset, align, cb, req); }
    else {
        req->aiocb = blk_aio_preadv(blk, offset, &req->sg.iov, 0, cb, req);
    }
}

static inline void nvme_blk_write(BlockBackend* blk, int64_t offset, uint32_t align, BlockCompletionFunc* cb,
                                  NvmeRequest* req)
{
    assert(req->sg.flags & NVME_SG_ALLOC);

    if (req->sg.flags & NVME_SG_DMA) { req->aiocb = dma_blk_write(blk, &req->sg.qsg, offset, align, cb, req); }
    else {
...
static void nvme_update_cq_eventidx(const NvmeCQueue* cq)
{
    ...
    stl_le_pci_dma(&cq->ctrl->parent_obj, cq->ei_addr, cq->head, MEMTXATTRS_UNSPECIFIED);
}

static void nvme_update_cq_head(NvmeCQueue* cq)
{
    ldl_le_pci_dma(&cq->ctrl->parent_obj, cq->db_addr, &cq->head, MEMTXATTRS_UNSPECIFIED);
...
static void nvme_post_cqes(void* opaque)
{
    ...
    QTAILQ_FOREACH_SAFE (req, &cq->req_list, entry, next) {
        ...
        ret              = pci_dma_write(&n->parent_obj, addr, (void*)&req->cqe, sizeof(req->cqe));
        if (ret) {
        ...
        if (QTAILQ_EMPTY(&sq->req_list) && !nvme_sq_empty(sq)) { qemu_bh_schedule(sq->bh); }

        QTAILQ_INSERT_TAIL(&sq->req_list, req, entry);
    }
    if (cq->tail != cq->head) {
        if (cq->irq_enabled && !pending) { n->cq_pending++; }
...
static void nvme_enqueue_req_completion(NvmeCQueue* cq, NvmeRequest* req)
{
    ...
    trace_pci_nvme_enqueue_req_completion(nvme_cid(req), cq->cqid, le32_to_cpu(req->cqe.result),
                                          le32_to_cpu(req->cqe.dw1), req->status);

    if (req->status) { trace_pci_nvme_err_req_status(nvme_cid(req), nvme_nsid(req->ns), req->status, req->cmd.opcode); }
...
static void nvme_process_aers(void* opaque)
{
    ...
    QTAILQ_FOREACH_SAFE (event, &n->aer_queue, entry, next) {
        ...
        trace_pci_nvme_aer_post_cqe(result->event_type, result->event_info, result->log_page);

        nvme_enqueue_req_completion(&n->admin_cq, req);
    }
}

static void nvme_enqueue_event(NvmeCtrl* n, uint8_t event_type, uint8_t event_info, uint8_t log_page)
{
...
static void nvme_smart_event(NvmeCtrl* n, uint8_t event)
{
    ...
    if (!(NVME_AEC_SMART(n->features.async_config) & event)) { return; }

    switch (event) {
        case NVME_SMART_SPARE                : aer_info = NVME_AER_INFO_SMART_SPARE_THRESH; break;
        case NVME_SMART_TEMPERATURE          : aer_info = NVME_AER_INFO_SMART_TEMP_THRESH; break;
        case NVME_SMART_RELIABILITY          :
        case NVME_SMART_MEDIA_READ_ONLY      :
        case NVME_SMART_FAILED_VOLATILE_MEDIA:
        case NVME_SMART_PMR_UNRELIABLE       : aer_info = NVME_AER_INFO_SMART_RELIABILITY; break;
        default                              : return;
    }
...
static inline uint16_t nvme_check_bounds(NvmeNamespace* ns, uint64_t slba, uint32_t nlb)
{
...
static int nvme_block_status_all(NvmeNamespace* ns, uint64_t slba, uint32_t nlb, int flags)
{
    ...
    do {
        ...
        trace_pci_nvme_block_status(offset, bytes, pnum, ret, !!(ret & BDRV_BLOCK_ZERO));

        if (!(ret & flags)) { return 1; }
...
static uint16_t nvme_check_dulbe(NvmeNamespace* ns, uint64_t slba, uint32_t nlb)
{
    int    ret;
    Error* err = NULL;
...
static inline NvmeZone* nvme_get_zone_by_slba(NvmeNamespace* ns, uint64_t slba)
{
    ...
    if (zone_idx >= ns->num_zones) { return NULL; }
...
static uint16_t nvme_check_zone_state_for_write(NvmeZone* zone)
{
    ...
    switch (nvme_get_zone_state(zone)) {
        ...
        case NVME_ZONE_STATE_FULL           : trace_pci_nvme_err_zone_is_full(zslba); return NVME_ZONE_FULL;
        case NVME_ZONE_STATE_OFFLINE        : trace_pci_nvme_err_zone_is_offline(zslba); return NVME_ZONE_OFFLINE;
        case NVME_ZONE_STATE_READ_ONLY      : trace_pci_nvme_err_zone_is_read_only(zslba); return NVME_ZONE_READ_ONLY;
...
static uint16_t nvme_check_zone_write(NvmeNamespace* ns, NvmeZone* zone, uint64_t slba, uint32_t nlb)
{
...
static uint16_t nvme_check_zone_read(NvmeNamespace* ns, uint64_t slba, uint32_t nlb)
{
    NvmeZone* zone;
    uint64_t  bndry, end;
    uint16_t  status;
    ...
    if (status) { ; }
        ...
        if (!ns->params.cross_zone_read) { status = NVME_ZONE_BOUNDARY_ERROR; }
        else {
            /*
...
{
    switch (nvme_get_zone_state(zone)) {
        case NVME_ZONE_STATE_FULL: return NVME_SUCCESS;
        ...
        case NVME_ZONE_STATE_CLOSED:
            ...
            if (zone->d.za & NVME_ZA_ZRWA_VALID) {
                ...
                if (ns->params.numzrwa) { ns->zns.numzrwa++; }
            }

            /* fallthrough */
        case NVME_ZONE_STATE_EMPTY: nvme_assign_zone_state(ns, zone, NVME_ZONE_STATE_FULL); return NVME_SUCCESS;

        default: return NVME_ZONE_INVAL_TRANSITION;
    }
}
...
static uint16_t nvme_zrm_reset(NvmeNamespace* ns, NvmeZone* zone)
{
    switch (nvme_get_zone_state(zone)) {
        ...
        case NVME_ZONE_STATE_FULL:
            ...
            /* fallthrough */
        case NVME_ZONE_STATE_EMPTY: return NVME_SUCCESS;
...
static void nvme_zrm_auto_transition_zone(NvmeNamespace* ns)
{
    ...
    if (ns->params.max_open_zones && ns->nr_open_zones == ns->params.max_open_zones) {
...
static uint16_t nvme_zrm_open_flags(NvmeCtrl* n, NvmeNamespace* ns, NvmeZone* zone, int flags)
{
    int      act = 0;
    uint16_t status;
    ...
    switch (nvme_get_zone_state(zone)) {
        ...
        case NVME_ZONE_STATE_CLOSED:
            if (n->params.auto_transition_zones) { nvme_zrm_auto_transition_zone(ns); }
            status = nvme_zns_check_resources(ns, act, 1, (flags & NVME_ZRM_ZRWA) ? 1 : 0);
            if (status) { return status; }

            if (act) { nvme_aor_inc_active(ns); }
        ...
        case NVME_ZONE_STATE_IMPLICITLY_OPEN:
            if (flags & NVME_ZRM_AUTO) { return NVME_SUCCESS; }
...
static inline uint16_t nvme_zrm_auto(NvmeCtrl* n, NvmeNamespace* ns, NvmeZone* zone)
{ return nvme_zrm_open_flags(n, ns, zone, NVME_ZRM_AUTO); }

static void nvme_advance_zone_wp(NvmeNamespace* ns, NvmeZone* zone, uint32_t nlb)
{
    zone->d.wp += nlb;

    if (zone->d.wp == nvme_zone_wr_boundary(zone)) { nvme_zrm_finish(ns, zone); }
}

static void nvme_zoned_zrwa_implicit_flush(NvmeNamespace* ns, NvmeZone* zone, uint32_t nlbc)
{
...
static void nvme_finalize_zoned_write(NvmeNamespace* ns, NvmeRequest* req)
{
    ...
    if (zone->d.za & NVME_ZA_ZRWA_VALID) {
        ...
        if (elba > ezrwa) { nvme_zoned_zrwa_implicit_flush(ns, zone, elba - ezrwa); }
...
static inline bool nvme_is_write(NvmeRequest* req)
{
    ...
    return rw->opcode == NVME_CMD_WRITE || rw->opcode == NVME_CMD_ZONE_APPEND || rw->opcode == NVME_CMD_WRITE_ZEROES;
}
...
static void nvme_misc_cb(void* opaque, int ret)
{
    ...
    if (ret) {
        if (!req->status) { req->status = NVME_INTERNAL_DEV_ERROR; }
...
void nvme_rw_complete_cb(void* opaque, int ret)
{
    ...
    if (ns->params.zoned && nvme_is_write(req)) { nvme_finalize_zoned_write(ns, req); }
...
static void nvme_rw_cb(void* opaque, int ret)
{
    ...
    if (ns->lbaf.ms) {
        ...
        if (req->cmd.opcode == NVME_CMD_WRITE_ZEROES) {
            ...
            req->aiocb = blk_aio_pwrite_zeroes(blk, offset, mlen, BDRV_REQ_MAY_UNMAP, nvme_rw_complete_cb, req);
            return;
        }
        ...
        if (nvme_ns_ext(ns) || req->cmd.mptr) {
            ...
            if (req->cmd.opcode == NVME_CMD_READ) { return nvme_blk_read(blk, offset, 1, nvme_rw_complete_cb, req); }
...
static void nvme_verify_cb(void* opaque, int ret)
{
    ...
    if (NVME_ID_NS_DPS_TYPE(ns->id_ns.dps)) {
        status = nvme_dif_mangle_mdata(ns, ctx->mdata.bounce, ctx->mdata.iov.size, slba);
        if (status) {
            req->status = status;
            goto out;
        }

        req->status = nvme_dif_check(ns, ctx->data.bounce, ctx->data.iov.size, ctx->mdata.bounce, ctx->mdata.iov.size,
...
static void nvme_verify_mdata_in_cb(void* opaque, int ret)
{
    ...
    req->aiocb = blk_aio_preadv(blk, offset, &ctx->mdata.iov, 0, nvme_verify_cb, ctx);
    return;
...
static void nvme_compare_mdata_cb(void* opaque, int ret)
{
    ...
    status = nvme_bounce_mdata(n, buf, ctx->mdata.iov.size, NVME_TX_DIRECTION_TO_DEVICE, req);
    if (status) {
        req->status = status;
        goto out;
    }
    ...
    if (NVME_ID_NS_DPS_TYPE(ns->id_ns.dps)) {
        ...
        status = nvme_dif_check(ns, ctx->data.bounce, ctx->data.iov.size, ctx->mdata.bounce, ctx->mdata.iov.size,
                                prinfo, slba, apptag, appmask, &reftag);
        if (status) {
            req->status = status;
            goto out;
         ...
         * tuple.
         */
        if (!(ns->id_ns.dps & NVME_ID_NS_DPS_FIRST_EIGHT)) { pil = ns->lbaf.ms - nvme_pi_tuple_size(ns); }
...
static void nvme_compare_data_cb(void* opaque, int ret)
{
    ...
    status = nvme_bounce_data(n, buf, ctx->data.iov.size, NVME_TX_DIRECTION_TO_DEVICE, req);
    if (status) {
        req->status = status;
        goto out;
    }
    ...
    if (ns->lbaf.ms) {
        ...
        req->aiocb = blk_aio_preadv(blk, offset, &ctx->mdata.iov, 0, nvme_compare_mdata_cb, req);
        return;
    }
...
static void nvme_dsm_md_cb(void* opaque, int ret)
{
    ...
    if (ret < 0 || iocb->ret < 0 || !ns->lbaf.ms) { goto done; }
    ...
    iocb->aiocb = blk_aio_pwrite_zeroes(ns->blkconf.blk, nvme_moff(ns, slba), nvme_m2b(ns, nlb), BDRV_REQ_MAY_UNMAP,
                                        nvme_dsm_cb, iocb);
    return;
...
next:
    ...
    iocb->aiocb = blk_aio_pdiscard(ns->blkconf.blk, nvme_l2b(ns, slba), nvme_l2b(ns, nlb), nvme_dsm_md_cb, iocb);
    return;
...
static uint16_t nvme_dsm(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    if (attr & NVME_DSMGMT_AD) {
        NvmeDSMAIOCB* iocb = blk_aio_get(&nvme_dsm_aiocb_info, ns->blkconf.blk, nvme_misc_cb, req);
        ...
        status = nvme_h2c(n, (uint8_t*)iocb->range, sizeof(NvmeDsmRange) * nr, req);
        if (status) {
            g_free(iocb->range);
            qemu_aio_unref(iocb);
...
static uint16_t nvme_verify(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    if (NVME_ID_NS_DPS_TYPE(ns->id_ns.dps)) {
        ...
        if (prinfo & NVME_PRINFO_PRACT) { return NVME_INVALID_PROT_INFO | NVME_DNR; }
    }

    if (nvme_ns_ext(ns) && !(NVME_ID_CTRL_CTRATT_MEM(n->id_ctrl.ctratt))) { data_len += nvme_m2b(ns, nlb); }

    if (data_len > (n->page_size << n->params.vsl)) { return NVME_INVALID_FIELD | NVME_DNR; }
    ...
    block_acct_start(blk_get_stats(blk), &req->acct, ctx->data.iov.size, BLOCK_ACCT_READ);

    req->aiocb = blk_aio_preadv(ns->blkconf.blk, offset, &ctx->data.iov, 0, nvme_verify_mdata_in_cb, ctx);
    return NVME_NO_COMPLETE;
}
...
static void nvme_copy_done(NvmeCopyAIOCB* iocb)
{
    ...
    if (iocb->idx != iocb->nr) { req->cqe.result = cpu_to_le32(iocb->idx); }
...
static void nvme_copy_source_range_parse_format0_2(void* ranges, int idx, uint64_t* slba, uint32_t* nlb,
                                                   uint32_t* snsid, uint16_t* apptag, uint16_t* appmask,
                                                   uint64_t* reftag)
{
    NvmeCopySourceRangeFormat0_2* _ranges = ranges;

    if (snsid) { *snsid = le32_to_cpu(_ranges[idx].sparams); }

    if (slba) { *slba = le64_to_cpu(_ranges[idx].slba); }

    if (nlb) { *nlb = le16_to_cpu(_ranges[idx].nlb) + 1; }

    if (apptag) { *apptag = le16_to_cpu(_ranges[idx].apptag); }

    if (appmask) { *appmask = le16_to_cpu(_ranges[idx].appmask); }

    if (reftag) { *reftag = le32_to_cpu(_ranges[idx].reftag); }
}

static void nvme_copy_source_range_parse_format1_3(void* ranges, int idx, uint64_t* slba, uint32_t* nlb,
                                                   uint32_t* snsid, uint16_t* apptag, uint16_t* appmask,
                                                   uint64_t* reftag)
{
...
static void nvme_copy_source_range_parse(void* ranges, int idx, uint8_t format, uint64_t* slba, uint32_t* nlb,
                                         uint32_t* snsid, uint16_t* apptag, uint16_t* appmask, uint64_t* reftag)
{
    switch (format) {
        case NVME_COPY_FORMAT_0:
        case NVME_COPY_FORMAT_2:
            nvme_copy_source_range_parse_format0_2(ranges, idx, slba, nlb, snsid, apptag, appmask, reftag);
            break;
...
static inline uint16_t nvme_check_copy_mcl(NvmeNamespace* ns, NvmeCopyAIOCB* iocb, uint16_t nr)
{
    uint32_t copy_len = 0;

    for (int idx = 0; idx < nr; idx++) {
        uint32_t nlb;
        nvme_copy_source_range_parse(iocb->ranges, idx, iocb->format, NULL, &nlb, NULL, NULL, NULL, NULL);
        copy_len += nlb;
    }
    iocb->tcl = copy_len;
    if (copy_len > ns->id_ns.mcl) { return NVME_CMD_SIZE_LIMIT | NVME_DNR; }
...
static void nvme_copy_out_completed_cb(void* opaque, int ret)
{
    ...
    nvme_copy_source_range_parse(iocb->ranges, iocb->idx, iocb->format, NULL, &nlb, NULL, NULL, NULL, NULL);
    ...
    if (dns->params.zoned) { nvme_advance_zone_wp(dns, iocb->zone, nlb); }
...
static void nvme_copy_out_cb(void* opaque, int ret)
{
    ...
    if (ret < 0 || iocb->ret < 0 || !dns->lbaf.ms) { goto out; }
    ...
    iocb->aiocb =
        blk_aio_pwritev(dns->blkconf.blk, nvme_moff(dns, iocb->slba), &iocb->iov, 0, nvme_copy_out_completed_cb, iocb);
...
static void nvme_copy_in_completed_cb(void* opaque, int ret)
{
    ...
    nvme_copy_source_range_parse(iocb->ranges, iocb->idx, iocb->format, &slba, &nlb, NULL, &apptag, &appmask, &reftag);
    ...
    if (NVME_ID_NS_DPS_TYPE(sns->id_ns.dps)) {
        ...
        status = nvme_dif_check(sns, iocb->bounce, len, mbounce, mlen, prinfor, slba, apptag, appmask, &reftag);
        if (status) { goto invalid; }
    }
    ...
    if (NVME_ID_NS_DPS_TYPE(dns->id_ns.dps)) {
        ...
        if (prinfow & NVME_PRINFO_PRACT) {
            ...
            nvme_dif_pract_generate_dif(dns, iocb->bounce, len, mbounce, mlen, apptag, &iocb->reftag);
        }
        else {
            status = nvme_dif_check(dns, iocb->bounce, len, mbounce, mlen, prinfow, iocb->slba, apptag, appmask,
                                    &iocb->reftag);
            if (status) { goto invalid; }
        }
    }
    ...
    if (dns->params.zoned) {
        ...
        if (!(iocb->zone->d.za & NVME_ZA_ZRWA_VALID)) { iocb->zone->w_ptr += nlb; }
    }

    qemu_iovec_reset(&iocb->iov);
    qemu_iovec_add(&iocb->iov, iocb->bounce, len);

    block_acct_start(blk_get_stats(dns->blkconf.blk), &iocb->acct.write, 0, BLOCK_ACCT_WRITE);

    iocb->aiocb = blk_aio_pwritev(dns->blkconf.blk, nvme_l2b(dns, iocb->slba), &iocb->iov, 0, nvme_copy_out_cb, iocb);
...
static void nvme_copy_in_cb(void* opaque, int ret)
{
    ...
    if (ret < 0 || iocb->ret < 0 || !sns->lbaf.ms) { goto out; }

    nvme_copy_source_range_parse(iocb->ranges, iocb->idx, iocb->format, &slba, &nlb, NULL, NULL, NULL, NULL);

    qemu_iovec_reset(&iocb->iov);
    qemu_iovec_add(&iocb->iov, iocb->bounce + nvme_l2b(sns, nlb), nvme_m2b(sns, nlb));

    iocb->aiocb =
        blk_aio_preadv(sns->blkconf.blk, nvme_moff(sns, slba), &iocb->iov, 0, nvme_copy_in_completed_cb, iocb);
    return;
...
static inline bool nvme_csi_supports_copy(uint8_t csi) { return csi == NVME_CSI_NVM || csi == NVME_CSI_ZONED; }

static inline bool nvme_copy_ns_format_match(NvmeNamespace* sns, NvmeNamespace* dns)
{ return sns->lbaf.ds == dns->lbaf.ds && sns->lbaf.ms == dns->lbaf.ms; }

static bool nvme_copy_matching_ns_format(NvmeNamespace* sns, NvmeNamespace* dns, bool pi_enable)
{
    if (!nvme_csi_supports_copy(sns->csi) || !nvme_csi_supports_copy(dns->csi)) { return false; }

    if (!pi_enable && !nvme_copy_ns_format_match(sns, dns)) { return false; }

    if (pi_enable && (!nvme_copy_ns_format_match(sns, dns) || sns->id_ns.dps != dns->id_ns.dps)) { return false; }

    return true;
}

static inline bool nvme_copy_corresp_pi_match(NvmeNamespace* sns, NvmeNamespace* dns)
{ return sns->lbaf.ms == 0 && ((dns->lbaf.ms == 8 && dns->pif == 0) || (dns->lbaf.ms == 16 && dns->pif == 1)); }

static bool nvme_copy_corresp_pi_format(NvmeNamespace* sns, NvmeNamespace* dns, bool sns_pi_en)
{
    if (!nvme_csi_supports_copy(sns->csi) || !nvme_csi_supports_copy(dns->csi)) { return false; }

    if (!sns_pi_en && !nvme_copy_corresp_pi_match(sns, dns)) { return false; }

    if (sns_pi_en && !nvme_copy_corresp_pi_match(dns, sns)) { return false; }
...
static void nvme_do_copy(NvmeCopyAIOCB* iocb)
{
    ...
    if (iocb->format == 2 || iocb->format == 3) {
        nvme_copy_source_range_parse(iocb->ranges, iocb->idx, iocb->format, &slba, &nlb, &snsid, NULL, NULL, NULL);
        if (snsid != dnsid) {
            if (snsid == NVME_NSID_BROADCAST || !nvme_nsid_valid(iocb->n, snsid)) {
            ...
            if (((slba + nlb) > iocb->slba) && ((slba + nlb) < (iocb->slba + iocb->tcl))) {
        ...
        && ((prinfor & NVME_PRINFO_PRACT) != (prinfow & NVME_PRINFO_PRACT)))
    {
        ...
        if (!NVME_ID_NS_DPS_TYPE(sns->id_ns.dps) && !NVME_ID_NS_DPS_TYPE(dns->id_ns.dps)) {
        ...
        if (NVME_ID_NS_DPS_TYPE(sns->id_ns.dps) && NVME_ID_NS_DPS_TYPE(dns->id_ns.dps)) {
            if ((prinfor & NVME_PRINFO_PRACT) != (prinfow & NVME_PRINFO_PRACT)) {
                status = NVME_CMD_INCOMP_NS_OR_FMT | NVME_DNR;
                goto invalid;
            }
            else {
    ...
    g_free(iocb->bounce);
    iocb->bounce = g_malloc_n(le16_to_cpu(sns->id_ns.mssrl), sns->lbasz + sns->lbaf.ms);

    qemu_iovec_reset(&iocb->iov);
    qemu_iovec_add(&iocb->iov, iocb->bounce, len);

    block_acct_start(blk_get_stats(sns->blkconf.blk), &iocb->acct.read, 0, BLOCK_ACCT_READ);

    iocb->aiocb = blk_aio_preadv(sns->blkconf.blk, nvme_l2b(sns, slba), &iocb->iov, 0, nvme_copy_in_cb, iocb);
    return;
...
static uint16_t nvme_copy(NvmeCtrl* n, NvmeRequest* req)
{
    NvmeNamespace* ns     = req->ns;
    NvmeCopyCmd*   copy   = (NvmeCopyCmd*)&req->cmd;
    NvmeCopyAIOCB* iocb   = blk_aio_get(&nvme_copy_aiocb_info, ns->blkconf.blk, nvme_misc_cb, req);
    ...
    if (!(n->id_ctrl.ocfs & (1 << format)) || ((format == 2 || format == 3) && !(n->features.hbs.cdfe & (1 << format))))
    {
    ...
    if ((ns->pif == 0x0 && (format != 0x0 && format != 0x2)) || (ns->pif != 0x0 && (format != 0x1 && format != 0x3))) {
        status = NVME_INVALID_FORMAT | NVME_DNR;
        goto invalid;
    }

    if (ns->pif) { len = sizeof(NvmeCopySourceRangeFormat1_3); }
...
static uint16_t nvme_compare(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    if (nvme_ns_ext(ns)) { len += nvme_m2b(ns, nlb); }

    if (NVME_ID_CTRL_CTRATT_MEM(n->id_ctrl.ctratt)) { status = nvme_check_mdts(n, data_len); }
    else {
    ...
    block_acct_start(blk_get_stats(blk), &req->acct, data_len, BLOCK_ACCT_READ);
    req->aiocb = blk_aio_preadv(blk, offset, &ctx->data.iov, 0, nvme_compare_data_cb, req);
...
static uint16_t nvme_read(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    if (nvme_ns_ext(ns) && !(NVME_ID_CTRL_CTRATT_MEM(n->id_ctrl.ctratt))) {
        ...
        if (NVME_ID_NS_DPS_TYPE(ns->id_ns.dps)) {
            ...
            if (pract && ns->lbaf.ms == nvme_pi_tuple_size(ns)) { mapped_size = data_size; }
        }
    }
    ...
    if (NVME_ID_NS_DPS_TYPE(ns->id_ns.dps)) { return nvme_dif_rw(n, req); }
...
static void nvme_do_write_fdp(NvmeCtrl* n, NvmeRequest* req, uint64_t slba, uint32_t nlb)
{
    NvmeNamespace*   ns        = req->ns;
    NvmeRwCmd*       rw        = (NvmeRwCmd*)&req->cmd;
    uint64_t         data_size = nvme_l2b(ns, nlb);
...
static uint16_t nvme_do_write(NvmeCtrl* n, NvmeRequest* req, bool append, bool wrz)
{
    NvmeRwCmd*       rw          = (NvmeRwCmd*)&req->cmd;
    NvmeNamespace*   ns          = req->ns;
    uint64_t         slba        = le64_to_cpu(rw->slba);
    ...
    if (nvme_ns_ext(ns) && !(NVME_ID_CTRL_CTRATT_MEM(n->id_ctrl.ctratt))) {
        ...
        if (NVME_ID_NS_DPS_TYPE(ns->id_ns.dps)) {
            ...
            if (pract && ns->lbaf.ms == nvme_pi_tuple_size(ns)) { mapped_size -= nvme_m2b(ns, nlb); }
        }
    }

    trace_pci_nvme_write(nvme_cid(req), nvme_io_opc_str(rw->opcode), nvme_nsid(ns), nlb, mapped_size, slba);
    ...
    if (ns->params.zoned) {
        ...
        if (append) {
            ...
            if (unlikely(zone->d.za & NVME_ZA_ZRWA_VALID)) { return NVME_INVALID_ZONE_OP | NVME_DNR; }
            ...
            switch (NVME_ID_NS_DPS_TYPE(ns->id_ns.dps)) {
                case NVME_ID_NS_DPS_TYPE_1:
                    if (!piremap) { return NVME_INVALID_PROT_INFO | NVME_DNR; }
        ...
        if (!(zone->d.za & NVME_ZA_ZRWA_VALID)) { zone->w_ptr += nlb; }
    }
    ...
    if (!wrz) {
        ...
        req->aiocb = blk_aio_pwrite_zeroes(blk, data_offset, data_size, BDRV_REQ_MAY_UNMAP, nvme_rw_cb, req);
    }
...
static inline uint16_t nvme_write(NvmeCtrl* n, NvmeRequest* req) { return nvme_do_write(n, req, false, false); }

static inline uint16_t nvme_write_zeroes(NvmeCtrl* n, NvmeRequest* req) { return nvme_do_write(n, req, false, true); }

static inline uint16_t nvme_zone_append(NvmeCtrl* n, NvmeRequest* req) { return nvme_do_write(n, req, true, false); }

static uint16_t nvme_get_mgmt_zone_slba_idx(NvmeNamespace* ns, NvmeCmd* c, uint64_t* slba, uint32_t* zone_idx)
{
...
static uint16_t nvme_open_zone(NvmeNamespace* ns, NvmeZone* zone, NvmeZoneState state, NvmeRequest* req)
{
    ...
    if (cmd->zsflags & NVME_ZSFLAG_ZRWA_ALLOC) {
        ...
        if (!(ozcs & NVME_ID_NS_ZONED_OZCS_ZRWASUP)) { return NVME_INVALID_ZONE_OP | NVME_DNR; }

        if (zone->w_ptr % ns->zns.zrwafg) { return NVME_NOZRWA | NVME_DNR; }
...
static uint16_t nvme_close_zone(NvmeNamespace* ns, NvmeZone* zone, NvmeZoneState state, NvmeRequest* req)
{ return nvme_zrm_close(ns, zone); }

static uint16_t nvme_finish_zone(NvmeNamespace* ns, NvmeZone* zone, NvmeZoneState state, NvmeRequest* req)
{ return nvme_zrm_finish(ns, zone); }

static uint16_t nvme_offline_zone(NvmeNamespace* ns, NvmeZone* zone, NvmeZoneState state, NvmeRequest* req)
{
    switch (state) {
        case NVME_ZONE_STATE_READ_ONLY:
            nvme_assign_zone_state(ns, zone, NVME_ZONE_STATE_OFFLINE);
            /* fall through */
        case NVME_ZONE_STATE_OFFLINE: return NVME_SUCCESS;
        default                     : return NVME_ZONE_INVAL_TRANSITION;
    }
}
...
static uint16_t nvme_bulk_proc_zone(NvmeNamespace* ns, NvmeZone* zone, enum NvmeZoneProcessingMask proc_mask,
                                    op_handler_t op_hndlr, NvmeRequest* req)
{
    ...
    switch (zs) {
        case NVME_ZONE_STATE_IMPLICITLY_OPEN:
        case NVME_ZONE_STATE_EXPLICITLY_OPEN: proc_zone = proc_mask & NVME_PROC_OPENED_ZONES; break;
        case NVME_ZONE_STATE_CLOSED         : proc_zone = proc_mask & NVME_PROC_CLOSED_ZONES; break;
        case NVME_ZONE_STATE_READ_ONLY      : proc_zone = proc_mask & NVME_PROC_READ_ONLY_ZONES; break;
        case NVME_ZONE_STATE_FULL           : proc_zone = proc_mask & NVME_PROC_FULL_ZONES; break;
        default                             : proc_zone = false;
    }

    if (proc_zone) { status = op_hndlr(ns, zone, zs, req); }

    return status;
}

static uint16_t nvme_do_zone_op(NvmeNamespace* ns, NvmeZone* zone, enum NvmeZoneProcessingMask proc_mask,
                                op_handler_t op_hndlr, NvmeRequest* req)
{
    NvmeZone* next;
    uint16_t  status = NVME_SUCCESS;
    ...
    if (!proc_mask) { status = op_hndlr(ns, zone, nvme_get_zone_state(zone), req); }
    else {
        if (proc_mask & NVME_PROC_CLOSED_ZONES) {
            QTAILQ_FOREACH_SAFE (zone, &ns->closed_zones, entry, next) {
                status = nvme_bulk_proc_zone(ns, zone, proc_mask, op_hndlr, req);
                if (status && status != NVME_NO_COMPLETE) { goto out; }
            }
        }
...
static void nvme_zone_reset_epilogue_cb(void* opaque, int ret)
{
    ...
    if (ret < 0 || iocb->ret < 0 || !ns->lbaf.ms) { goto out; }

    moff  = nvme_moff(ns, iocb->zone->d.zslba);
    count = nvme_m2b(ns, ns->zone_size);

    iocb->aiocb = blk_aio_pwrite_zeroes(ns->blkconf.blk, moff, count, BDRV_REQ_MAY_UNMAP, nvme_zone_reset_cb, iocb);
    return;
...
static void nvme_zone_reset_cb(void* opaque, int ret)
{
    ...
    while (iocb->idx < ns->num_zones) {
        ...
        iocb->aiocb = blk_aio_pwrite_zeroes(ns->blkconf.blk, nvme_l2b(ns, zone->d.zslba), nvme_l2b(ns, ns->zone_size),
                                            BDRV_REQ_MAY_UNMAP, nvme_zone_reset_epilogue_cb, iocb);
        return;
    }
...
static uint16_t nvme_zone_mgmt_send_zrwa_flush(NvmeCtrl* n, NvmeZone* zone, uint64_t elba, NvmeRequest* req)
{
    NvmeNamespace* ns   = req->ns;
    uint16_t       ozcs = le16_to_cpu(ns->id_ns_zoned->ozcs);
    ...
    if (!(zone->d.za & NVME_ZA_ZRWA_VALID)) { return NVME_INVALID_FIELD | NVME_DNR; }

    if (elba < wp || elba > wp + ns->zns.zrwas) { return NVME_ZONE_BOUNDARY_ERROR | NVME_DNR; }

    if (nlb % ns->zns.zrwafg) { return NVME_INVALID_FIELD | NVME_DNR; }
...
static uint16_t nvme_zone_mgmt_send(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    switch (action) {
        ...
        case NVME_ZONE_ACTION_RESET:
            ...
            iocb = blk_aio_get(&nvme_zone_reset_aiocb_info, ns->blkconf.blk, nvme_misc_cb, req);
        ...
        case NVME_ZONE_ACTION_OFFLINE:
            if (all) { proc_mask = NVME_PROC_READ_ONLY_ZONES; }
        ...
        case NVME_ZONE_ACTION_SET_ZD_EXT:
            ...
            if (all || !ns->params.zd_extension_size) { return NVME_INVALID_FIELD | NVME_DNR; }
        ...
        case NVME_ZONE_ACTION_ZRWA_FLUSH:
            if (all) { return NVME_INVALID_FIELD | NVME_DNR; }

            return nvme_zone_mgmt_send_zrwa_flush(n, zone, slba, req);

        default: trace_pci_nvme_err_invalid_mgmt_action(action); status = NVME_INVALID_FIELD;
    }

    if (status == NVME_ZONE_INVAL_TRANSITION) {
        trace_pci_nvme_err_invalid_zone_state_transition(action, slba, zone->d.za);
    }
    if (status) { status |= NVME_DNR; }
...
static bool nvme_zone_matches_filter(uint32_t zafs, NvmeZone* zl)
{
    ...
    switch (zafs) {
        case NVME_ZONE_REPORT_ALL            : return true;
        case NVME_ZONE_REPORT_EMPTY          : return zs == NVME_ZONE_STATE_EMPTY;
        case NVME_ZONE_REPORT_IMPLICITLY_OPEN: return zs == NVME_ZONE_STATE_IMPLICITLY_OPEN;
        case NVME_ZONE_REPORT_EXPLICITLY_OPEN: return zs == NVME_ZONE_STATE_EXPLICITLY_OPEN;
        case NVME_ZONE_REPORT_CLOSED         : return zs == NVME_ZONE_STATE_CLOSED;
        case NVME_ZONE_REPORT_FULL           : return zs == NVME_ZONE_STATE_FULL;
        case NVME_ZONE_REPORT_READ_ONLY      : return zs == NVME_ZONE_STATE_READ_ONLY;
        case NVME_ZONE_REPORT_OFFLINE        : return zs == NVME_ZONE_STATE_OFFLINE;
...
static uint16_t nvme_zone_mgmt_recv(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    zra = dw13 & 0xff;
    if (zra != NVME_ZONE_REPORT && zra != NVME_ZONE_REPORT_EXTENDED) { return NVME_INVALID_FIELD | NVME_DNR; }
    if (zra == NVME_ZONE_REPORT_EXTENDED && !ns->params.zd_extension_size) { return NVME_INVALID_FIELD | NVME_DNR; }

    zrasf = (dw13 >> 8) & 0xff;
    if (zrasf > NVME_ZONE_REPORT_OFFLINE) { return NVME_INVALID_FIELD | NVME_DNR; }

    if (data_size < sizeof(NvmeZoneReportHeader)) { return NVME_INVALID_FIELD | NVME_DNR; }
    ...
    if (zra == NVME_ZONE_REPORT_EXTENDED) { zone_entry_sz += ns->params.zd_extension_size; }
    ...
    for (i = zone_idx; i < ns->num_zones; i++) {
        if (partial && nr_zones >= max_zones) { break; }
        if (nvme_zone_matches_filter(zrasf, zone++)) { nr_zones++; }
    }
    header           = buf;
    ...
    for (; zone_idx < ns->num_zones && max_zones > 0; zone_idx++) {
        ...
        if (nvme_zone_matches_filter(zrasf, zone)) {
            ...
            if (nvme_wp_is_valid(zone)) { z->wp = cpu_to_le64(zone->d.wp); }
            else {
            ...
            if (zra == NVME_ZONE_REPORT_EXTENDED) {
                if (zone->d.za & NVME_ZA_ZD_EXT_VALID) {
                    memcpy(buf_p, nvme_get_zd_extension(ns, zone_idx), ns->params.zd_extension_size);
                }
...
static uint16_t nvme_io_mgmt_recv_ruhs(NvmeCtrl* n, NvmeRequest* req, size_t len)
{
    NvmeNamespace*      ns = req->ns;
    ...
    if (!n->subsys) { return NVME_INVALID_FIELD | NVME_DNR; }

    if (ns->params.nsid == 0 || ns->params.nsid == 0xffffffff) { return NVME_INVALID_NSID | NVME_DNR; }

    if (!n->subsys->endgrp.fdp.enabled) { return NVME_FDP_DISABLED | NVME_DNR; }
...
static uint16_t nvme_io_mgmt_recv(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    switch (mo) {
        case NVME_IOMR_MO_NOP       : return 0;
        case NVME_IOMR_MO_RUH_STATUS: return nvme_io_mgmt_recv_ruhs(n, req, len);
...
static uint16_t nvme_io_mgmt_send_ruh_update(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    if (!ns->endgrp || !ns->endgrp->fdp.enabled) { return NVME_FDP_DISABLED | NVME_DNR; }

    maxnpid = n->subsys->endgrp.fdp.nrg * n->subsys->endgrp.fdp.nruh;

    if (unlikely(npid >= MIN(NVME_FDP_MAXPIDS, maxnpid))) { return NVME_INVALID_FIELD | NVME_DNR; }
    ...
    for (; i < npid; i++) {
        if (!nvme_update_ruh(n, ns, pids[i])) { return NVME_INVALID_FIELD | NVME_DNR; }
    }
...
static uint16_t nvme_io_mgmt_send(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    switch (mo) {
        case NVME_IOMS_MO_NOP       : return 0;
        case NVME_IOMS_MO_RUH_UPDATE: return nvme_io_mgmt_send_ruh_update(n, req);
        default                     : return NVME_INVALID_FIELD | NVME_DNR;
    };
}
...
static uint16_t __nvme_io_cmd_nvm(NvmeCtrl* n, NvmeRequest* req)
{
    switch (req->cmd.opcode) {
        ...
        case NVME_CMD_REPRIORITIZE: return NVME_SUCCESS;
    }
...
static uint16_t nvme_io_cmd_zoned(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    switch (req->cmd.opcode) {
        case NVME_CMD_ZONE_APPEND   : return nvme_zone_append(n, req);
        case NVME_CMD_ZONE_MGMT_SEND: return nvme_zone_mgmt_send(n, req);
        case NVME_CMD_ZONE_MGMT_RECV: return nvme_zone_mgmt_recv(n, req);
    }
...
static uint16_t nvme_io_cmd(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    trace_pci_nvme_io_cmd(nvme_cid(req), nsid, nvme_sqid(req), req->cmd.opcode, nvme_io_opc_str(req->cmd.opcode));
    ...
    if (req->cmd.opcode == NVME_CMD_FLUSH) { return nvme_flush(n, req); }

    if (!nvme_nsid_valid(n, nsid) || nsid == NVME_NSID_BROADCAST) { return NVME_INVALID_NSID | NVME_DNR; }

    ns = nvme_ns(n, nsid);
    if (unlikely(!ns)) { return NVME_INVALID_FIELD | NVME_DNR; }

    if (ns->status) { return ns->status; }

    if (NVME_CMD_FLAGS_FUSE(req->cmd.flags)) { return NVME_INVALID_FIELD; }

    req->ns = ns;

    switch (ns->csi) {
        case NVME_CSI_NVM  : return nvme_io_cmd_nvm(n, req);
        case NVME_CSI_ZONED: return nvme_io_cmd_zoned(n, req);
    }
...
static void nvme_cq_notifier(EventNotifier* e)
{
    ...
    if (cq->tail == cq->head) {
        if (cq->irq_enabled) { n->cq_pending--; }
...
static int nvme_init_cq_ioeventfd(NvmeCQueue* cq)
{
    ...
    memory_region_add_eventfd(&n->iomem, 0x1000 + offset, 4, false, 0, &cq->notifier);
...
static int nvme_init_sq_ioeventfd(NvmeSQueue* sq)
{
    ...
    memory_region_add_eventfd(&n->iomem, 0x1000 + offset, 4, false, 0, &sq->notifier);
...
static void nvme_free_sq(NvmeSQueue* sq, NvmeCtrl* n)
{
    ...
    if (sq->ioeventfd_enabled) {
        memory_region_del_eventfd(&n->iomem, 0x1000 + offset, 4, false, 0, &sq->notifier);
        event_notifier_set_handler(&sq->notifier, NULL);
        event_notifier_cleanup(&sq->notifier);
    }
    g_free(sq->io_req);
    if (sq->sqid) { g_free(sq); }
}
                         ...
                         uint32_t entry_count)
{
    ...
    if (n->dbbuf_enabled) {
        ...
        if (n->params.ioeventfd && sq->sqid != 0) {
            if (!nvme_init_sq_ioeventfd(sq)) { sq->ioeventfd_enabled = true; }
        }
    }
...
static uint16_t nvme_ocp_extended_smart_info(NvmeCtrl* n, uint8_t rae, uint32_t buf_len, uint64_t off, NvmeRequest* req)
{
    ...
    if (off >= sizeof(smart_l)) { return NVME_INVALID_FIELD | NVME_DNR; }

    /* accumulate all stats from all namespaces */
    for (int i = 1; i <= NVME_MAX_NAMESPACES; i++) {
        ns = nvme_ns(n, i);
        if (ns) { nvme_set_blk_stats(ns, &stats); }
    }
    ...
    static const uint8_t guid[16] = {0xC5, 0xAF, 0x10, 0x28, 0xEA, 0xBF, 0xF2, 0xA4,
                                     0x9C, 0x4F, 0x6F, 0x7C, 0xC9, 0x14, 0xD5, 0xAF};
    memcpy(smart_l.log_page_guid, guid, sizeof(smart_l.log_page_guid));

    if (!rae) { nvme_clear_events(n, NVME_AER_TYPE_SMART); }

    trans_len = MIN(sizeof(smart_l) - off, buf_len);
    return nvme_c2h(n, (uint8_t*)&smart_l + off, trans_len, req);
}

static uint16_t nvme_smart_info(NvmeCtrl* n, uint8_t rae, uint32_t buf_len, uint64_t off, NvmeRequest* req)
{
    uint32_t          nsid  = le32_to_cpu(req->cmd.nsid);
    struct nvme_stats stats = {0};
    ...
    if (off >= sizeof(smart)) { return NVME_INVALID_FIELD | NVME_DNR; }

    if (nsid != 0xffffffff) {
        ns = nvme_ns(n, nsid);
        if (!ns) { return NVME_INVALID_NSID | NVME_DNR; }
    ...
    if ((n->temperature >= n->features.temp_thresh_hi) || (n->temperature <= n->features.temp_thresh_low)) {
        smart.critical_warning |= NVME_SMART_TEMPERATURE;
    }

    current_ms              = qemu_clock_get_ms(QEMU_CLOCK_VIRTUAL);
    smart.power_on_hours[0] = cpu_to_le64((((current_ms - n->starttime_ms) / 1000) / 60) / 60);
...
static uint16_t nvme_endgrp_info(NvmeCtrl* n, uint8_t rae, uint32_t buf_len, uint64_t off, NvmeRequest* req)
{
    ...
    if (!n->subsys || endgrpid != 0x1) { return NVME_INVALID_FIELD | NVME_DNR; }

    if (off >= sizeof(info)) { return NVME_INVALID_FIELD | NVME_DNR; }
    ...
    info.data_units_read[0]     = cpu_to_le64(DIV_ROUND_UP(stats.units_read / 1000000000, 1000000000));
    info.data_units_written[0]  = cpu_to_le64(DIV_ROUND_UP(stats.units_written / 1000000000, 1000000000));
    info.media_units_written[0] = cpu_to_le64(DIV_ROUND_UP(stats.units_written / 1000000000, 1000000000));
...
static uint16_t nvme_fw_log_info(NvmeCtrl* n, uint32_t buf_len, uint64_t off, NvmeRequest* req)
{
    uint32_t          trans_len;
    ...
    if (off >= sizeof(fw_log)) { return NVME_INVALID_FIELD | NVME_DNR; }
...
static uint16_t nvme_error_info(NvmeCtrl* n, uint8_t rae, uint32_t buf_len, uint64_t off, NvmeRequest* req)
{
    uint32_t     trans_len;
    NvmeErrorLog errlog;

    if (off >= sizeof(errlog)) { return NVME_INVALID_FIELD | NVME_DNR; }

    if (!rae) { nvme_clear_events(n, NVME_AER_TYPE_ERROR); }
...
static uint16_t nvme_changed_nslist(NvmeCtrl* n, uint8_t rae, uint32_t buf_len, uint64_t off, NvmeRequest* req)
{
    ...
    while ((nsid = find_first_bit(n->changed_nsids, NVME_CHANGED_NSID_SIZE)) != NVME_CHANGED_NSID_SIZE) {
        /*
     ...
     */
    if (nslist[0] == 0xffffffff) { bitmap_zero(n->changed_nsids, NVME_CHANGED_NSID_SIZE); }

    if (!rae) { nvme_clear_events(n, NVME_AER_TYPE_NOTICE); }

    return nvme_c2h(n, ((uint8_t*)nslist) + off, trans_len, req);
}

static uint16_t nvme_cmd_effects(NvmeCtrl* n, uint8_t csi, uint32_t buf_len, uint64_t off, NvmeRequest* req)
{
    NvmeEffectsLog  log  = {};
    ...
    switch (NVME_CC_CSS(ldl_le_p(&n->bar.cc))) {
        case NVME_CC_CSS_NVM: iocs = n->cse.iocs.nvm; break;

        case NVME_CC_CSS_ALL:
            switch (csi) {
                case NVME_CSI_NVM  : iocs = n->cse.iocs.nvm; break;
                case NVME_CSI_ZONED: iocs = n->cse.iocs.zoned; break;
            }
    ...
    if (iocs) { memcpy(log.iocs, iocs, sizeof(log.iocs)); }
...
static uint16_t nvme_vendor_specific_log(NvmeCtrl* n, uint8_t rae, uint32_t buf_len, uint64_t off, NvmeRequest* req,
                                         uint8_t lid)
{
    switch (lid) {
        case NVME_OCP_EXTENDED_SMART_INFO:
            if (n->params.ocp) { return nvme_ocp_extended_smart_info(n, rae, buf_len, off, req); }
            break;
...
static uint16_t nvme_fdp_confs(NvmeCtrl* n, uint32_t endgrpid, uint32_t buf_len, uint64_t off, NvmeRequest* req)
{
    ...
    if (endgrpid != 1 || !n->subsys) { return NVME_INVALID_FIELD | NVME_DNR; }

    endgrp = &n->subsys->endgrp;

    if (endgrp->fdp.enabled) { nruh = endgrp->fdp.nruh; }
    else {
        nruh = 1;
    }
    ...
    if (off >= log_size) { return NVME_INVALID_FIELD | NVME_DNR; }
...
static uint16_t nvme_fdp_ruh_usage(NvmeCtrl* n, uint32_t endgrpid, uint32_t dw10, uint32_t dw12, uint32_t buf_len,
                                   uint64_t off, NvmeRequest* req)
{
    ...
    if (!endgrp->fdp.enabled) { return NVME_FDP_DISABLED | NVME_DNR; }
    ...
    for (i = 0; i < endgrp->fdp.nruh; i++, ruhud++, ruh++) { ruhud->ruha = ruh->ruha; }

    return nvme_c2h(n, (uint8_t*)buf + off, trans_len, req);
}

static uint16_t nvme_fdp_stats(NvmeCtrl* n, uint32_t endgrpid, uint32_t buf_len, uint64_t off, NvmeRequest* req)
{
    NvmeEnduranceGroup* endgrp;
    NvmeFdpStatsLog     log = {};
    uint32_t            trans_len;

    if (off >= sizeof(NvmeFdpStatsLog)) { return NVME_INVALID_FIELD | NVME_DNR; }
...
static uint16_t nvme_fdp_events(NvmeCtrl* n, uint32_t endgrpid, uint32_t buf_len, uint64_t off, NvmeRequest* req)
{
    NvmeEnduranceGroup*          endgrp;
    NvmeCmd*                     cmd         = &req->cmd;
    ...
    if (host_events) { ebuf = &endgrp->fdp.host_events; }
    else {
...
static uint16_t nvme_get_log(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    if (off & 0x3) { return NVME_INVALID_FIELD | NVME_DNR; }
    ...
    switch (lid) {
        case NVME_LOG_ERROR_INFO                          : return nvme_error_info(n, rae, len, off, req);
        case NVME_LOG_SMART_INFO                          : return nvme_smart_info(n, rae, len, off, req);
        case NVME_LOG_FW_SLOT_INFO                        : return nvme_fw_log_info(n, len, off, req);
        case NVME_LOG_VENDOR_START ... NVME_LOG_VENDOR_END: return nvme_vendor_specific_log(n, rae, len, off, req, lid);
        case NVME_LOG_CHANGED_NSLIST                      : return nvme_changed_nslist(n, rae, len, off, req);
...
static void nvme_free_cq(NvmeCQueue* cq, NvmeCtrl* n)
{
    ...
    if (cq->ioeventfd_enabled) {
        memory_region_del_eventfd(&n->iomem, 0x1000 + offset, 4, false, 0, &cq->notifier);
        event_notifier_set_handler(&cq->notifier, NULL);
        event_notifier_cleanup(&cq->notifier);
    }
    if (msix_enabled(pci) && cq->irq_enabled) { msix_vector_unuse(pci, cq->vector); }
    if (cq->cqid) { g_free(cq); }
}
...
static uint16_t nvme_del_cq(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    if (cq->irq_enabled && cq->tail != cq->head) { n->cq_pending--; }
...
static void nvme_init_cq(NvmeCQueue* cq, NvmeCtrl* n, uint64_t dma_addr, uint16_t cqid, uint16_t vector, uint16_t size,
                         uint16_t irq_enabled)
{
    PCIDevice* pci = &n->parent_obj;

    if (msix_enabled(pci) && irq_enabled) { msix_vector_use(pci, vector); }
    ...
    if (n->dbbuf_enabled) {
        ...
        if (n->params.ioeventfd && cqid != 0) {
            if (!nvme_init_cq_ioeventfd(cq)) { cq->ioeventfd_enabled = true; }
        }
    }
    n->cq[cqid] = cq;
...
static uint16_t nvme_create_cq(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    trace_pci_nvme_create_cq(prp1, cqid, vector, qsize, qflags, NVME_CQ_FLAGS_IEN(qflags) != 0);
...
static uint16_t nvme_identify_ctrl_csi(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    switch (c->csi) {
        ...
        case NVME_CSI_ZONED: ((NvmeIdCtrlZoned*)&id)->zasl = n->params.zasl; break;
...
static uint16_t nvme_identify_ns(NvmeCtrl* n, NvmeRequest* req, bool active)
{
    ...
    if (unlikely(!ns)) {
        if (!active) {
            ...
            if (!ns) { return nvme_rpt_empty_id_struct(n, req); }
        }
        else {
    ...
    if (active || ns->csi == NVME_CSI_NVM) { return nvme_c2h(n, (uint8_t*)&ns->id_ns, sizeof(NvmeIdNs), req); }

    return NVME_INVALID_IOCS | NVME_DNR;
}

static uint16_t nvme_identify_ctrl_list(NvmeCtrl* n, NvmeRequest* req, bool attached)
{
    NvmeIdentify*  c                               = (NvmeIdentify*)&req->cmd;
    uint32_t       nsid                            = le32_to_cpu(c->nsid);
    ...
    if (attached) {
        if (nsid == NVME_NSID_BROADCAST) { return NVME_INVALID_FIELD | NVME_DNR; }

        ns = nvme_subsys_ns(n->subsys, nsid);
        if (!ns) { return NVME_INVALID_FIELD | NVME_DNR; }
    }
    ...
    for (cntlid = min_id; cntlid < ARRAY_SIZE(n->subsys->ctrls); cntlid++) {
        ...
        if (attached && !nvme_ns(ctrl, nsid)) { continue; }
...
static uint16_t nvme_identify_ns_csi(NvmeCtrl* n, NvmeRequest* req, bool active)
{
    NvmeNamespace* ns;
    NvmeIdentify*  c    = (NvmeIdentify*)&req->cmd;
    uint32_t       nsid = le32_to_cpu(c->nsid);
    ...
    if (c->csi == NVME_CSI_NVM) { return nvme_c2h(n, (uint8_t*)&ns->id_ns_nvm, sizeof(NvmeIdNsNvm), req); }
    else if (c->csi == NVME_CSI_ZONED && ns->csi == NVME_CSI_ZONED) {
        return nvme_c2h(n, (uint8_t*)ns->id_ns_zoned, sizeof(NvmeIdNsZoned), req);
    }

    return NVME_INVALID_FIELD | NVME_DNR;
}

static uint16_t nvme_identify_nslist(NvmeCtrl* n, NvmeRequest* req, bool active)
{
    NvmeNamespace*   ns;
    NvmeIdentify*    c                             = (NvmeIdentify*)&req->cmd;
     ...
     */
    if (min_nsid >= NVME_NSID_BROADCAST - 1) { return NVME_INVALID_NSID | NVME_DNR; }
    ...
    for (i = 1; i <= NVME_MAX_NAMESPACES; i++) {
        ...
        if (ns->params.nsid <= min_nsid) { continue; }
        list_ptr[j++] = cpu_to_le32(ns->params.nsid);
        if (j == data_len / sizeof(uint32_t)) { break; }
    }

    return nvme_c2h(n, list, data_len, req);
}

static uint16_t nvme_identify_nslist_csi(NvmeCtrl* n, NvmeRequest* req, bool active)
{
    NvmeNamespace*   ns;
    NvmeIdentify*    c                             = (NvmeIdentify*)&req->cmd;
    uint32_t         min_nsid                      = le32_to_cpu(c->nsid);
    ...
    if (c->csi != NVME_CSI_NVM && c->csi != NVME_CSI_ZONED) { return NVME_INVALID_FIELD | NVME_DNR; }
    ...
    for (i = 1; i <= NVME_MAX_NAMESPACES; i++) {
        ...
        if (ns->params.nsid <= min_nsid || c->csi != ns->csi) { continue; }
        list_ptr[j++] = cpu_to_le32(ns->params.nsid);
        if (j == data_len / sizeof(uint32_t)) { break; }
    }
...
static uint16_t nvme_identify(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    trace_pci_nvme_identify(nvme_cid(req), c->cns, le16_to_cpu(c->ctrlid), c->csi);

    switch (c->cns) {
        case NVME_ID_CNS_NS                   : return nvme_identify_ns(n, req, true);
        case NVME_ID_CNS_NS_PRESENT           : return nvme_identify_ns(n, req, false);
        case NVME_ID_CNS_NS_ATTACHED_CTRL_LIST: return nvme_identify_ctrl_list(n, req, true);
        case NVME_ID_CNS_CTRL_LIST            : return nvme_identify_ctrl_list(n, req, false);
        case NVME_ID_CNS_PRIMARY_CTRL_CAP     : return nvme_identify_pri_ctrl_cap(n, req);
        case NVME_ID_CNS_CS_NS                : return nvme_identify_ns_csi(n, req, true);
        case NVME_ID_CNS_CS_IND_NS            : return nvme_identify_ns_ind(n, req, false);
        case NVME_ID_CNS_CS_IND_NS_ALLOCATED  : return nvme_identify_ns_ind(n, req, true);
        case NVME_ID_CNS_CS_NS_PRESENT        : return nvme_identify_ns_csi(n, req, false);
        case NVME_ID_CNS_CTRL                 : return nvme_identify_ctrl(n, req);
        case NVME_ID_CNS_CS_CTRL              : return nvme_identify_ctrl_csi(n, req);
        case NVME_ID_CNS_NS_ACTIVE_LIST       : return nvme_identify_nslist(n, req, true);
        case NVME_ID_CNS_NS_PRESENT_LIST      : return nvme_identify_nslist(n, req, false);
        case NVME_ID_CNS_CS_NS_ACTIVE_LIST    : return nvme_identify_nslist_csi(n, req, true);
        case NVME_ID_CNS_ENDURANCE_GROUP_LIST : return nvme_endurance_group_list(n, req);
...
static uint16_t nvme_abort(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    req->cqe.result = 1;
    if (nvme_check_sqid(n, sqid)) { return NVME_INVALID_FIELD | NVME_DNR; }

    if (sqid == 0) {
        for (i = 0; i < n->outstanding_aers; i++) {
            NvmeRequest* re = n->aer_reqs[i];
            if (re->cqe.cid == cid) {
                memmove(n->aer_reqs + i, n->aer_reqs + i + 1, (n->outstanding_aers - i - 1) * sizeof(NvmeRequest*));
                n->outstanding_aers--;
...
static int nvme_get_feature_fdp(NvmeCtrl* n, uint32_t endgrpid, uint32_t* result)
{
    *result = 0;

    if (!n->subsys || !n->subsys->endgrp.fdp.enabled) { return NVME_INVALID_FIELD | NVME_DNR; }
...
static uint16_t nvme_get_feature_fdp_events(NvmeCtrl* n, NvmeNamespace* ns, NvmeRequest* req, uint32_t* result)
{
    NvmeCmd*                      cmd   = &req->cmd;
    ...
    if (!n->subsys || !n->subsys->endgrp.fdp.enabled) { return NVME_FDP_DISABLED | NVME_DNR; }

    if (!nvme_ph_valid(ns, ph)) { return NVME_INVALID_FIELD | NVME_DNR; }
    ...
    if (unlikely(noet == 0)) { return NVME_INVALID_FIELD | NVME_DNR; }
...
static uint16_t nvme_get_feature(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    if (!nvme_feature_support[fid]) { return NVME_INVALID_FIELD | NVME_DNR; }
    ...
    if (nvme_feature_cap[fid] & NVME_FEAT_CAP_NS) {
        ...
        if (!nvme_ns(n, nsid)) { return NVME_INVALID_FIELD | NVME_DNR; }
    }

    switch (sel) {
        case NVME_GETFEAT_SELECT_CURRENT: break;
        case NVME_GETFEAT_SELECT_SAVED:
            /* no features are saveable by the controller; fallthrough */
        case NVME_GETFEAT_SELECT_DEFAULT: goto defaults;
        case NVME_GETFEAT_SELECT_CAP    : result = nvme_feature_cap[fid]; goto out;
    }
    ...
    switch (fid) {
        case NVME_TEMPERATURE_THRESHOLD:
             ...
             */
            if (NVME_TEMP_TMPSEL(dw11) != NVME_TEMP_TMPSEL_COMPOSITE) { goto out; }

            switch (NVME_TEMP_THSEL(dw11)) {
                case NVME_TEMP_THSEL_OVER : result = n->features.temp_thresh_hi; goto out;
                case NVME_TEMP_THSEL_UNDER: result = n->features.temp_thresh_low; goto out;
            }

            return NVME_INVALID_FIELD | NVME_DNR;
        case NVME_ERROR_RECOVERY:
            if (!nvme_nsid_valid(n, nsid)) { return NVME_INVALID_NSID | NVME_DNR; }
        ...
        case NVME_VOLATILE_WRITE_CACHE:
            ...
            goto out;
        case NVME_ASYNCHRONOUS_EVENT_CONF: result = n->features.async_config; goto out;
        case NVME_TIMESTAMP              : return nvme_get_feature_timestamp(n, req);
        ...
        case NVME_MSP_TYPE:
            ...
            //  it.
            goto out;
        case NVME_HOST_BEHAVIOR_SUPPORT: return nvme_c2h(n, (uint8_t*)&n->features.hbs, sizeof(n->features.hbs), req);
        case NVME_FDP_MODE:
            endgrpid = dw11 & 0xff;

            if (endgrpid != 0x1) { return NVME_INVALID_FIELD | NVME_DNR; }
...
defaults:
    switch (fid) {
        case NVME_TEMPERATURE_THRESHOLD:
            ...
            if (NVME_TEMP_TMPSEL(dw11) != NVME_TEMP_TMPSEL_COMPOSITE) { break; }

            if (NVME_TEMP_THSEL(dw11) == NVME_TEMP_THSEL_OVER) { result = NVME_TEMPERATURE_WARNING; }
        ...
        case NVME_INTERRUPT_VECTOR_CONF:
            iv = dw11 & 0xffff;
            if (iv >= n->conf_ioqpairs + 1) { return NVME_INVALID_FIELD | NVME_DNR; }

            result = iv;
            if (iv == n->admin_cq.vector) { result |= NVME_INTVC_NOCOALESCING; }
            break;
        case NVME_FDP_MODE:
            endgrpid = dw11 & 0xff;
        ...
        case NVME_WRITE_ATOMICITY: result = n->dn; break;
        default                  : result = nvme_feature_default[fid]; break;
    }
...
static uint16_t nvme_set_feature_fdp_events(NvmeCtrl* n, NvmeNamespace* ns, NvmeRequest* req)
{
    NvmeCmd*            cmd   = &req->cmd;
    uint32_t            cdw11 = le32_to_cpu(cmd->cdw11);
    uint16_t            ph    = cdw11 & 0xffff;
    ...
    for (i = 0; i < noet; i++) { event_mask |= (1 << nvme_fdp_evf_shifts[events[i]]); }

    if (enable) { ruh->event_filter |= event_mask; }
    else {
...
static uint16_t nvme_set_feature(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    if (save && !(nvme_feature_cap[fid] & NVME_FEAT_CAP_SAVE)) { return NVME_FID_NOT_SAVEABLE | NVME_DNR; }
    ...
    if (!(nvme_feature_cap[fid] & NVME_FEAT_CAP_CHANGE)) { return NVME_FEAT_NOT_CHANGEABLE | NVME_DNR; }
    ...
    switch (fid) {
        case NVME_TEMPERATURE_THRESHOLD:
            ...
            switch (NVME_TEMP_THSEL(dw11)) {
                case NVME_TEMP_THSEL_OVER : n->features.temp_thresh_hi = NVME_TEMP_TMPTH(dw11); break;
                case NVME_TEMP_THSEL_UNDER: n->features.temp_thresh_low = NVME_TEMP_TMPTH(dw11); break;
                default                   : return NVME_INVALID_FIELD | NVME_DNR;
            }
        ...
        case NVME_ERROR_RECOVERY:
            if (nsid == NVME_NSID_BROADCAST) {
                for (i = 1; i <= NVME_MAX_NAMESPACES; i++) {
                    ...
                    if (NVME_ID_NS_NSFEAT_DULBE(ns->id_ns.nsfeat)) { ns->features.err_rec = dw11; }
                }
        ...
        case NVME_VOLATILE_WRITE_CACHE:
            for (i = 1; i <= NVME_MAX_NAMESPACES; i++) {
                ...
                if (!(dw11 & 0x1) && blk_enable_write_cache(ns->blkconf.blk)) { blk_flush(ns->blkconf.blk); }
        ...
        case NVME_NUMBER_OF_QUEUES:
            if (n->qs_created) { return NVME_CMD_SEQ_ERROR | NVME_DNR; }
            ...
            trace_pci_nvme_setfeat_numq((dw11 & 0xffff) + 1, ((dw11 >> 16) & 0xffff) + 1, n->conf_ioqpairs,
                                        n->conf_ioqpairs);
            req->cqe.result = cpu_to_le32((n->conf_ioqpairs - 1) | ((n->conf_ioqpairs - 1) << 16));
            break;
        case NVME_ASYNCHRONOUS_EVENT_CONF: n->features.async_config = dw11; break;
        case NVME_TIMESTAMP              : return nvme_set_feature_timestamp(n, req);
        case NVME_HOST_BEHAVIOR_SUPPORT:
            status = nvme_h2c(n, (uint8_t*)&n->features.hbs, sizeof(n->features.hbs), req);
            if (status) { return status; }
            ...
            for (i = 1; i <= NVME_MAX_NAMESPACES; i++) {
                ...
                if (!n->features.hbs.lbafee) { ns->id_ns.nlbaf = MIN(ns->id_ns.nlbaf, 15); }
            }
        ...
        case NVME_FDP_EVENTS: return nvme_set_feature_fdp_events(n, ns, req);
        case NVME_WRITE_ATOMICITY:

            n->dn = 0x1 & dw11;

            if (n->dn) { atomic->atomic_max_write_size = le16_to_cpu(id->awupf) + 1; }
            else {
                atomic->atomic_max_write_size = le16_to_cpu(id->awun) + 1;
            }

            if (atomic->atomic_max_write_size == 1) { atomic->atomic_writes = 0; }
            else {
                atomic->atomic_writes = 1;
            }
            break;
        default: return NVME_FEAT_NOT_CHANGEABLE | NVME_DNR;
    }
    return NVME_SUCCESS;
}
...
static uint16_t nvme_aer(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    if (!QTAILQ_EMPTY(&n->aer_queue)) { nvme_process_aers(n); }
...
{
    if (ns) {
        n->dmrsl = MIN_NON_ZERO(n->dmrsl, BDRV_REQUEST_MAX_BYTES / nvme_l2b(ns, 1));
...
static bool nvme_csi_supported(NvmeCtrl* n, uint8_t csi)
{
    ...
    switch (csi) {
        case NVME_CSI_NVM: return true;

        case NVME_CSI_ZONED: cc = ldl_le_p(&n->bar.cc); return NVME_CC_CSS(cc) == NVME_CC_CSS_ALL;
    }
...
static uint16_t nvme_ns_attachment(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    if (!*nr_ids) { return NVME_NS_CTRL_LIST_INVALID | NVME_DNR; }

    *nr_ids = MIN(*nr_ids, NVME_CONTROLLER_LIST_SIZE - 1);
    for (i = 0; i < *nr_ids; i++) {
        ctrl = nvme_subsys_ctrl(n->subsys, ids[i]);
        if (!ctrl) { return NVME_NS_CTRL_LIST_INVALID | NVME_DNR; }

        switch (sel) {
            case NVME_NS_ATTACHMENT_ATTACH:
                if (nvme_ns(ctrl, nsid)) { return NVME_NS_ALREADY_ATTACHED | NVME_DNR; }

                if (ns->attached && !ns->params.shared) { return NVME_NS_PRIVATE | NVME_DNR; }

                if (!nvme_csi_supported(ctrl, ns->csi)) { return NVME_IOCS_NOT_SUPPORTED | NVME_DNR; }
            ...
            case NVME_NS_ATTACHMENT_DETACH:
                if (!nvme_ns(ctrl, nsid)) { return NVME_NS_NOT_ATTACHED | NVME_DNR; }
        ...
        if (!test_and_set_bit(nsid, ctrl->changed_nsids)) {
            nvme_enqueue_event(ctrl, NVME_AER_TYPE_NOTICE, NVME_AER_INFO_NOTICE_NS_ATTR_CHANGED,
...
static void nvme_format_set(NvmeNamespace* ns, uint8_t lbaf, uint8_t mset, uint8_t pi, uint8_t pil)
{
...
static void nvme_format_ns_cb(void* opaque, int ret)
{
    ...
    if (iocb->offset < ns->size) {
        ...
        iocb->aiocb =
            blk_aio_pwrite_zeroes(ns->blkconf.blk, iocb->offset, bytes, BDRV_REQ_MAY_UNMAP, nvme_format_ns_cb, iocb);
...
{
    if (ns->params.zoned) { return NVME_INVALID_FORMAT | NVME_DNR; }

    if (lbaf > ns->id_ns.nlbaf) { return NVME_INVALID_FORMAT | NVME_DNR; }

    if (pi && (ns->id_ns.lbaf[lbaf].ms < nvme_pi_tuple_size(ns))) { return NVME_INVALID_FORMAT | NVME_DNR; }

    if (pi && pi > NVME_ID_NS_DPS_TYPE_3) { return NVME_INVALID_FIELD | NVME_DNR; }
...
static uint16_t nvme_format(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    if (n->features.hbs.lbafee) { iocb->lbaf |= lbafu << 4; }
...
static void nvme_create_ns_ns_cb(void* opaque, int ret)
{
    ...
    if (iocb->offset < zero_size) {
        ...
        iocb->aiocb =
            blk_aio_pwrite_zeroes(ns->blkconf.blk, iocb->offset, bytes, BDRV_REQ_MAY_UNMAP, nvme_create_ns_ns_cb, iocb);
...
static uint16_t nvme_tunnel(NvmeCtrl* n, NvmeRequest* req) { return NVME_SUCCESS; }

static uint16_t nvme_maybe_bfh_ofw(NvmeCtrl* n, NvmeRequest* req) { return NVME_SUCCESS; }

static uint16_t nvme_download_fw(NvmeCtrl* n, NvmeRequest* req) { return NVME_SUCCESS; }

static uint16_t nvme_activate_fw(NvmeCtrl* n, NvmeRequest* req) { return NVME_SUCCESS; }
...
static uint16_t nvme_dbbuf_config(NvmeCtrl* n, const NvmeRequest* req)
{
    ...
    if (dbs_addr & (n->page_size - 1) || eis_addr & (n->page_size - 1)) { return NVME_INVALID_FIELD | NVME_DNR; }
...
static uint16_t nvme_directive_send(NvmeCtrl* n, NvmeRequest* req) { return NVME_INVALID_FIELD | NVME_DNR; }
...
static uint16_t nvme_directive_receive(NvmeCtrl* n, NvmeRequest* req)
{
    ...
    if (nsid == NVME_NSID_BROADCAST || dtype != NVME_DIRECTIVE_IDENTIFY || doper != NVME_DIRECTIVE_RETURN_PARAMS) {
        return NVME_INVALID_FIELD | NVME_DNR;
    }
...
{
    trace_pci_nvme_admin_cmd(nvme_cid(req), nvme_sqid(req), req->cmd.opcode, nvme_adm_opc_str(req->cmd.opcode));
    ...
    if (NVME_CMD_FLAGS_PSDT(req->cmd.flags) != NVME_PSDT_PRP) { return NVME_INVALID_FIELD | NVME_DNR; }
    ...
    switch (req->cmd.opcode) {
        ...
        case NVME_ADM_CMD_TUNNEL        : return nvme_tunnel(n, req);
        ...
        case NVME_ADM_CMD_DIRECTIVE_SEND: return nvme_directive_send(n, req);
        case NVME_ADM_CMD_DIRECTIVE_RECV: return nvme_directive_receive(n, req);
        case NVME_ADM_CMD_CREATE_NS     : return nvme_create_ns(n, req);
        case NVME_ADM_CMD_MAYBE_BFH_OFW : return nvme_maybe_bfh_ofw(n, req);
        case NVME_ADM_CMD_DOWNLOAD_FW   : return nvme_download_fw(n, req);
        case NVME_ADM_CMD_ACTIVATE_FW   : return nvme_activate_fw(n, req);
        default                         : assert_not_reached();
    }
...
static void nvme_update_sq_eventidx(const NvmeSQueue* sq)
{
    ...
    stl_le_pci_dma(&sq->ctrl->parent_obj, sq->ei_addr, sq->tail, MEMTXATTRS_UNSPECIFIED);
}

static void nvme_update_sq_tail(NvmeSQueue* sq)
{
    ldl_le_pci_dma(&sq->ctrl->parent_obj, sq->db_addr, &sq->tail, MEMTXATTRS_UNSPECIFIED);
...
static int nvme_atomic_write_check(NvmeCtrl* n, NvmeCmd* cmd, NvmeAtomic* atomic)
{
    ...
    if ((cmd->opcode == NVME_CMD_READ)
        || ((cmd->opcode == NVME_CMD_WRITE) && ((rw->nlb + 1) > atomic->atomic_max_write_size)))
    {
        cmd_atomic_wr = false;
    }
    ...
    for (i = 1; i < n->params.max_ioqpairs + 1; i++) {
        ...
        QTAILQ_FOREACH (req, &sq->out_req_list, entry) {
                ...
                && (cmd->nsid == req->ns->params.nsid))
            {
                ...
                if (cmd_atomic_wr) {
                    if ((elba >= req_slba) && (slba <= req_elba)) { return NVME_ATOMIC_NO_START; }
                }
                else {
                    if (req->atomic_write && ((elba >= req_slba) && (slba <= req_elba))) {
...
static void nvme_process_sq(void* opaque)
{
    ...
    if (n->dbbuf_enabled) { nvme_update_sq_tail(sq); }
    ...
    while (!(nvme_sq_empty(sq) || QTAILQ_EMPTY(&sq->req_list))) {
        ...
        if (sq->sqid && atomic) {
            ...
            switch (ret) {
                case NVME_ATOMIC_NO_START       : qemu_bh_schedule(sq->bh); return;
                case NVME_ATOMIC_START_ATOMIC   : cmd_is_atomic = true; break;
        ...
        if (sq->sqid && atomic) { req->atomic_write = cmd_is_atomic; }
...
static void nvme_update_msixcap_ts(PCIDevice* pci_dev, uint32_t table_size)
{
    ...
    if (!msix_present(pci_dev)) { return; }

    assert(table_size > 0 && table_size <= pci_dev->msix_entries_nr);

    config = pci_dev->config + pci_dev->msix_cap;
    pci_set_word_by_mask(config + PCI_MSIX_FLAGS, PCI_MSIX_FLAGS_QSIZE, table_size - 1);
}
...
static void nvme_ctrl_reset(NvmeCtrl* n, NvmeResetType rst)
{
    ...
    for (i = 0; i < n->params.max_ioqpairs + 1; i++) {
        if (n->sq[i] != NULL) { nvme_free_sq(n->sq[i], n); }
    }
    for (i = 0; i < n->params.max_ioqpairs + 1; i++) {
        if (n->cq[i] != NULL) { nvme_free_cq(n->cq[i], n); }
    }
...
static void nvme_ctrl_shutdown(NvmeCtrl* n)
{
    ...
    if (n->pmr.dev) { memory_region_msync(&n->pmr.dev->mr, 0, n->pmr.dev->size); }
...
static int nvme_start_ctrl(NvmeCtrl* n)
{
    ...
    if (unlikely(NVME_CC_MPS(cc) < NVME_CAP_MPSMIN(cap))) {
        trace_pci_nvme_err_startfail_page_too_small(NVME_CC_MPS(cc), NVME_CAP_MPSMIN(cap));
        return -1;
    }
    if (unlikely(NVME_CC_MPS(cc) > NVME_CAP_MPSMAX(cap))) {
        trace_pci_nvme_err_startfail_page_too_large(NVME_CC_MPS(cc), NVME_CAP_MPSMAX(cap));
        return -1;
    }
    ...
    for (int i = 1; i <= NVME_MAX_NAMESPACES; i++) {
        ...
        if (!ns || (!ns->params.shared && ns->ctrl != n)) { continue; }

        if (nvme_csi_supported(n, ns->csi) && !ns->params.detached) {
            if (!ns->attached || ns->params.shared) { nvme_attach_ns(n, ns); }
        }
    }
    ...
    switch (offset) {
        case NVME_REG_INTMS:
            if (unlikely(msix_enabled(pci))) {
                NVME_GUEST_ERR(pci_nvme_ub_mmiowr_intmask_with_msix, "undefined access to interrupt mask set"
        ...
        case NVME_REG_INTMC:
            if (unlikely(msix_enabled(pci))) {
                NVME_GUEST_ERR(pci_nvme_ub_mmiowr_intmask_with_msix, "undefined access to interrupt mask clr"
                                                                     " when MSI-X is enabled");
                /* should be ignored, fall through for now */
            }
            intms &= ~data;
        ...
        case NVME_REG_CSTS:
            if (data & (1 << 4)) {
                NVME_GUEST_ERR(pci_nvme_ub_mmiowr_ssreset_w1c_unsupported, "attempted to W1C CSTS.NSSRO"
                                                                           " but CAP.NSSRS is zero (not supported)");
            }
            else if (data != 0) {
                NVME_GUEST_ERR(pci_nvme_ub_mmiowr_ro_csts, "attempted to set a read only bit"
                                                           " of controller status");
            }
            break;
        case NVME_REG_NSSR:
            if (data == 0x4e564d65) { trace_pci_nvme_ub_mmiowr_ssreset_unsupported(); }
            else {
        ...
        case NVME_REG_CMBLOC:
            NVME_GUEST_ERR(pci_nvme_ub_mmiowr_cmbloc_reserved, "invalid write to reserved CMBLOC"
                                                               " when CMBSZ is zero, ignored");
            return;
        case NVME_REG_CMBSZ:
            NVME_GUEST_ERR(pci_nvme_ub_mmiowr_cmbsz_readonly, "invalid write to read only CMBSZ, ignored");
            return;
        case NVME_REG_CMBMSC:
            ...
            return;
        case NVME_REG_CMBMSC + 4: stl_le_p((uint8_t*)&n->bar.cmbmsc + 4, data); return;

        case NVME_REG_PMRCAP:
            NVME_GUEST_ERR(pci_nvme_ub_mmiowr_pmrcap_readonly, "invalid write to PMRCAP register, ignored");
            return;
        case NVME_REG_PMRCTL:
            if (!NVME_CAP_PMRS(cap)) { return; }
        ...
        case NVME_REG_PMRSTS:
            NVME_GUEST_ERR(pci_nvme_ub_mmiowr_pmrsts_readonly, "invalid write to PMRSTS register, ignored");
            return;
        case NVME_REG_PMREBS:
            NVME_GUEST_ERR(pci_nvme_ub_mmiowr_pmrebs_readonly, "invalid write to PMREBS register, ignored");
            return;
        case NVME_REG_PMRSWTP:
            NVME_GUEST_ERR(pci_nvme_ub_mmiowr_pmrswtp_readonly, "invalid write to PMRSWTP register, ignored");
            return;
        case NVME_REG_PMRMSCL:
            ...
            if (NVME_PMRMSCL_CMSE(data)) {
                ...
                hwaddr   cba     = pmrmscu << 32 | (NVME_PMRMSCL_CBA(data) << PMRMSCL_CBA_SHIFT);
...
static uint64_t nvme_mmio_read(void* opaque, hwaddr addr, unsigned size)
{
     ...
     */
    if (addr == NVME_REG_PMRSTS && (NVME_PMRCAP_PMRWBM(ldl_le_p(&n->bar.pmrcap)) & 0x02)) {
...
static void nvme_process_db(NvmeCtrl* n, hwaddr addr, int val)
{
    ...
    if (((addr - 0x1000) >> 2) & 1) {
        ...
        if (unlikely(nvme_check_cqid(n, qid))) {
             ...
             */
            if (n->outstanding_aers) {
                nvme_enqueue_event(n, NVME_AER_TYPE_ERROR, NVME_AER_INFO_ERR_INVALID_DB_REGISTER, NVME_LOG_ERROR_INFO);
            }
        ...
        if (unlikely(new_head >= cq->size)) {
            ...
            if (n->outstanding_aers) {
                nvme_enqueue_event(n, NVME_AER_TYPE_ERROR, NVME_AER_INFO_ERR_INVALID_DB_VALUE, NVME_LOG_ERROR_INFO);
            }
        ...
        if (nvme_cq_full(cq)) { qemu_bh_schedule(cq->bh); }

        cq->head = new_head;
        if (!qid && n->dbbuf_enabled) { stl_le_pci_dma(pci, cq->db_addr, cq->head, MEMTXATTRS_UNSPECIFIED); }
...
static void nvme_mmio_write(void* opaque, hwaddr addr, uint64_t data, unsigned size)
{
    ...
    if (addr < sizeof(n->bar)) { nvme_write_bar(n, addr, data, size); }
    else {
...
static bool nvme_check_params(NvmeCtrl* n, Error** errp)
{
    ...
    if (params->max_ioqpairs < 1 || params->max_ioqpairs > NVME_MAX_IOQPAIRS) {
        error_setg(errp, "max_ioqpairs must be between 1 and %d", NVME_MAX_IOQPAIRS);
        return false;
    }

    if (params->msix_qsize < 1 || params->msix_qsize > PCI_MSIX_FLAGS_QSIZE + 1) {
        error_setg(errp, "msix_qsize must be between 1 and %d", PCI_MSIX_FLAGS_QSIZE + 1);
        return false;
    }
...
static void nvme_init_state(NvmeCtrl* n)
{
    ...
    if (id->awun || id->awupf) {
        if (id->awupf > id->awun) { id->awupf = 0; }

        if (n->dn) { atomic->atomic_max_write_size = id->awupf + 1; }
        else {
...
static void nvme_init_cmb(NvmeCtrl* n, PCIDevice* pci_dev)
{
    ...
    memory_region_init_io(&n->cmb.mem, OBJECT(n), &nvme_cmb_ops, n, "nvme-cmb", cmb_size);
...
static uint64_t nvme_mbar_size(unsigned total_queues, unsigned total_irqs, unsigned* msix_table_offset,
                               unsigned* msix_pba_offset)
{
    ...
    if (total_irqs == 0) { goto out; }

    bar_size = QEMU_ALIGN_UP(bar_size, 4 * KiB);

    if (msix_table_offset) { *msix_table_offset = bar_size; }

    msix_table_size  = PCI_MSIX_ENTRY_SIZE * total_irqs;
    bar_size        += msix_table_size;
    bar_size         = QEMU_ALIGN_UP(bar_size, 4 * KiB);

    if (msix_pba_offset) { *msix_pba_offset = bar_size; }
...
static int nvme_add_pm_capability(PCIDevice* pci_dev, uint8_t offset)
{
    ...
    pci_set_word(pci_dev->config + offset + PCI_PM_PMC, PCI_PM_CAP_VER_1_2);
    pci_set_word(pci_dev->config + offset + PCI_PM_CTRL, PCI_PM_CTRL_NO_SOFT_RESET);
    pci_set_word(pci_dev->wmask + offset + PCI_PM_CTRL, PCI_PM_CTRL_STATE_MASK);
...
static bool nvme_init_pci(NvmeCtrl* n, PCIDevice* pci_dev, Error** errp)
{
    ...
    if (n->params.msix_exclusive_bar) {
        ...
        memory_region_init_io(&n->iomem, OBJECT(n), &nvme_mmio_ops, n, "nvme", bar_size);
        ...
        bar_size   = nvme_mbar_size(n->params.max_ioqpairs + 1, nr_vectors, &msix_table_offset, &msix_pba_offset);
        ...
        ret = msix_init(pci_dev, nr_vectors, &n->bar0, 0, msix_table_offset, &n->bar0, 0, msix_pba_offset, 0, errp);
    }
    ...
    if (n->params.cmb_size_mb) { nvme_init_cmb(n, pci_dev); }

    if (n->pmr.dev) { nvme_init_pmr(n, pci_dev); }
...
static void nvme_init_subnqn(NvmeCtrl* n)
{
    ...
    if (!subsys) { snprintf((char*)id->subnqn, sizeof(id->subnqn), "nqn.2019-08.org.qemu:%s", n->params.serial); }
    else {
...
static void nvme_init_ctrl(NvmeCtrl* n, PCIDevice* pci_dev)
{
    ...
    if (n->params.ctratt.mem) { ctratt |= NVME_CTRATT_MEM; }
    ...
    id->oncs = cpu_to_le16(NVME_ONCS_WRITE_ZEROES | NVME_ONCS_TIMESTAMP | NVME_ONCS_FEATURES | NVME_ONCS_DSM
                           | NVME_ONCS_COMPARE | NVME_ONCS_COPY | NVME_ONCS_NVMCSA | NVME_ONCS_NVMAFC);
    ...
    id->ocfs = cpu_to_le16(NVME_OCFS_COPY_FORMAT_0 | NVME_OCFS_COPY_FORMAT_1 | NVME_OCFS_COPY_FORMAT_2
                           | NVME_OCFS_COPY_FORMAT_3);
    id->sgls = cpu_to_le32(NVME_CTRL_SGLS_SUPPORT_NO_ALIGN | NVME_CTRL_SGLS_MPTR_SGL);
...
static int nvme_init_subsys(NvmeCtrl* n, Error** errp)
{
    ...
    if (!n->subsys) {
        ...
        if (!qdev_realize(dev, NULL, errp)) { return -1; }
...
static void nvme_realize(PCIDevice* pci_dev, Error** errp)
{
    ...
    if (!nvme_check_params(n, errp)) { return; }

    qbus_init(&n->bus, sizeof(NvmeBus), TYPE_NVME_BUS, dev, dev->id);

    if (nvme_init_subsys(n, errp)) { return; }
    nvme_init_state(n);
    if (!nvme_init_pci(n, pci_dev, errp)) { return; }
    ...
    if (n->namespace.blkconf.blk) {
        ...
        if (nvme_ns_setup(ns, errp)) { return; }
...
static void nvme_exit(PCIDevice* pci_dev)
{
    ...
    if (n->params.cmb_size_mb) { g_free(n->cmb.buf); }

    if (n->pmr.dev) { host_memory_backend_set_mapped(n->pmr.dev, false); }

    if (n->params.msix_exclusive_bar) { msix_uninit_exclusive_bar(pci_dev); }
    else {
...
static const Property nvme_props[] = {
    ...
    DEFINE_PROP_LINK("pmrdev", NvmeCtrl, pmr.dev, TYPE_MEMORY_BACKEND, HostMemoryBackend*),
    DEFINE_PROP_LINK("subsys", NvmeCtrl, subsys, TYPE_NVME_SUBSYS, NvmeSubsystem*),
    ...
    DEFINE_PROP_BOOL("zoned.auto_transition", NvmeCtrl, params.auto_transition_zones, true),
    DEFINE_PROP_BOOL("msix-exclusive-bar", NvmeCtrl, params.msix_exclusive_bar, false),
...
static void nvme_get_smart_warning(Object* obj, Visitor* v, const char* name, void* opaque, Error** errp)
{
    NvmeCtrl* n     = NVME(obj);
...
static void nvme_set_smart_warning(Object* obj, Visitor* v, const char* name, void* opaque, Error** errp)
{
    NvmeCtrl* n = NVME(obj);
    ...
    cap = NVME_SMART_SPARE | NVME_SMART_TEMPERATURE | NVME_SMART_RELIABILITY | NVME_SMART_MEDIA_READ_ONLY
          | NVME_SMART_FAILED_VOLATILE_MEDIA;
    if (NVME_CAP_PMRS(ldq_le_p(&n->bar.cap))) { cap |= NVME_SMART_PMR_UNRELIABLE; }

    if ((value & cap) != value) {
        error_setg(errp, "unsupported smart critical warning bits: 0x%x", value & ~cap);
        return;
    }
    ...
    for (index = 0; index < NVME_SMART_WARN_MAX; index++) {
        event = 1 << index;
        if (value & ~old_value & event) { nvme_smart_event(n, event); }
    }
}
...
static void nvme_pci_write_config(PCIDevice* dev, uint32_t address, uint32_t val, int len)
{
...
static void nvme_instance_init(Object* obj)
{
    ...
    device_add_bootindex_property(obj, &n->namespace.blkconf.bootindex, "bootindex", "/namespace@1,0", DEVICE(obj));

    object_property_add(obj, "smart_critical_warning", "uint8", nvme_get_smart_warning, nvme_set_smart_warning, NULL,
                        NULL);
}
