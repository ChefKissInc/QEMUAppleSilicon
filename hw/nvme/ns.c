...
void nvme_ns_init_format(NvmeNamespace* ns)
{
    ...
    if (ret >= 0 && bdi.cluster_size > ns->blkconf.discard_granularity) { npdg = bdi.cluster_size / ns->lbasz; }
...
static int nvme_ns_init(NvmeNamespace* ns, Error** errp)
{
    ...
    ++ns_count;
    if (!ns->params.eui64 && ns->params.eui64_default) { ns->params.eui64 = ns_count + NVME_EUI64_DEFAULT; }
    ...
    if (ms && ns->params.mset) { id_ns->flbas |= NVME_ID_NS_FLBAS_EXTENDED; }

    id_ns->dpc = 0x1f;
    id_ns->dps = ns->params.pi;
    if (ns->params.pi && ns->params.pil) { id_ns->dps |= NVME_ID_NS_DPS_FIRST_EIGHT; }

    ns->pif = ns->params.pif;

    static const NvmeLBAF defaults[16] = {
        [0] = {.ds = 9},  [1] = {.ds = 9, .ms = 8},  [2] = {.ds = 9, .ms = 16},  [3] = {.ds = 9, .ms = 64},
        [4] = {.ds = 12}, [5] = {.ds = 12, .ms = 8}, [6] = {.ds = 12, .ms = 16}, [7] = {.ds = 12, .ms = 64},
    };
...
static int nvme_ns_init_blk(NvmeNamespace* ns, Error** errp)
{
    ...
    if (!blkconf_blocksizes(&ns->blkconf, errp)) { return -1; }

    read_only = !blk_supports_write_perm(ns->blkconf.blk);
    if (!blkconf_apply_backend_options(&ns->blkconf, read_only, false, errp)) { return -1; }

    if (ns->blkconf.discard_granularity == -1) {
        ns->blkconf.discard_granularity = MAX(ns->blkconf.logical_block_size, MIN_DISCARD_GRANULARITY);
    }
...
static int nvme_ns_zoned_check_calc_geometry(NvmeNamespace* ns, Error** errp)
{
    ...
    if (ns->params.zone_size_bs) { zone_size = ns->params.zone_size_bs; }
    else {
        zone_size = NVME_DEFAULT_ZONE_SIZE;
    }
    if (ns->params.zone_cap_bs) { zone_cap = ns->params.zone_cap_bs; }
    else {
        zone_cap = zone_size;
    }
    ...
    if (zone_size < ns->lbasz) {
        error_setg(errp,
                   "zone size %" PRIu64 "B too small, "
                   "must be at least %zuB",
                   zone_size, ns->lbasz);
        return -1;
    }
    if (zone_cap < ns->lbasz) {
        error_setg(errp,
                   "zone capacity %" PRIu64 "B too small, "
                   "must be at least %zuB",
                   zone_cap, ns->lbasz);
        return -1;
    }
...
static void nvme_ns_zoned_init_state(NvmeNamespace* ns)
{
    ...
    if (ns->params.zd_extension_size) { ns->zd_extensions = g_malloc0(ns->params.zd_extension_size * ns->num_zones); }
    ...
    for (i = 0; i < ns->num_zones; i++, zone++) {
        if (start + zone_size > capacity) { zone_size = capacity - start; }
    ...
    ns->zone_size_log2 = 0;
    if (is_power_of_2(ns->zone_size)) { ns->zone_size_log2 = 63 - clz64(ns->zone_size); }
}
...
static void nvme_ns_init_zoned(NvmeNamespace* ns)
{
    ...
    id_ns_z->zoc  = 0;
    id_ns_z->ozcs = ns->params.cross_zone_read ? NVME_ID_NS_ZONED_OZCS_RAZB : 0x00;

    for (i = 0; i <= ns->id_ns.nlbaf; i++) {
        id_ns_z->lbafe[i].zsze = cpu_to_le64(ns->zone_size);
        id_ns_z->lbafe[i].zdes = ns->params.zd_extension_size >> 6; /* Units of 64B */
    }

    if (ns->params.zrwas) {
        ns->zns.numzrwa = ns->params.numzrwa ? ns->params.numzrwa : ns->num_zones;
...
static NvmeRuHandle* nvme_find_ruh_by_attr(NvmeEnduranceGroup* endgrp, uint8_t ruha, uint16_t* ruhid)
{
...
static bool nvme_ns_init_fdp(NvmeNamespace* ns, Error** errp)
{
    ...
    if (!ns->params.fdp.ruhs) {
        ...
        if (!ruh) {
            ...
            for (uint16_t rg = 0; rg < endgrp->fdp.nrg; rg++) { ruh->rus[rg].ruamw = ruh->ruamw; }
        }
    ...
    for (unsigned int i = 0; i < ns->fdp.nphs; i++) {
        for (unsigned int j = i + 1; j < ns->fdp.nphs; j++) {
            if (ruhids[i] == ruhids[j]) {
                error_setg(errp, "duplicate reclaim unit handle identifier: %u", ruhids[i]);
                return false;
            }
        }
    }
...
static int nvme_ns_check_constraints(NvmeNamespace* ns, Error** errp)
{
    ...
    if (ns->params.pi) {
        ...
        switch (ns->params.pif) {
            case NVME_PI_GUARD_16: pi_size = 8; break;
            case NVME_PI_GUARD_64: pi_size = 16; break;
            default              : error_setg(errp, "invalid 'pif'"); return -1;
        }

        if (ns->params.ms < pi_size) {
            error_setg(errp,
                       "at least %u bytes of metadata required to "
                       "enable protection information",
                       pi_size);
            return -1;
        }
    }
    ...
    if (ns->params.nsid > NVME_MAX_NAMESPACES) {
        error_setg(errp, "invalid namespace id (must be between 0 and %d)", NVME_MAX_NAMESPACES);
        return -1;
    }
    ...
    if (ns->params.zoned) {
        if (ns->params.max_active_zones) {
            ...
            if (!ns->params.max_open_zones) { ns->params.max_open_zones = ns->params.max_active_zones; }
        }
        ...
        if (ns->params.zrwas) {
            ...
            if (ns->params.zrwafg == -1) { ns->params.zrwafg = ns->blkconf.logical_block_size; }
            ...
            if (ns->params.zrwas % ns->params.zrwafg) {
                error_setg(errp,
                           ...
                           "%" PRIu64 ")",
                           ns->params.zrwas, ns->params.zrwafg);
                return -1;
            }
...
{
    if (nvme_ns_check_constraints(ns, errp)) { return -1; }

    if (nvme_ns_init_blk(ns, errp)) { return -1; }

    if (nvme_ns_init(ns, errp)) { return -1; }
    if (ns->params.zoned) {
        if (nvme_ns_zoned_check_calc_geometry(ns, errp) != 0) { return -1; }
        nvme_ns_init_zoned(ns);
    }

    if (ns->endgrp && ns->endgrp->fdp.enabled) {
        if (!nvme_ns_init_fdp(ns, errp)) { return -1; }
    }

    return 0;
}

void nvme_ns_drain(NvmeNamespace* ns) { blk_drain(ns->blkconf.blk); }

void nvme_ns_shutdown(NvmeNamespace* ns)
{
    blk_flush(ns->blkconf.blk);
    if (ns->params.zoned) { nvme_zoned_ns_shutdown(ns); }
}
...
void nvme_ns_cleanup(NvmeNamespace* ns)
{
    ...
    if (ns->endgrp && ns->endgrp->fdp.enabled) { g_free(ns->fdp.phs); }
}
...
static void nvme_ns_realize(DeviceState* dev, Error** errp)
{
    ...
    if (!qdev_set_parent_bus(dev, &subsys->bus.parent_bus, errp)) { return; }
    ns->subsys = subsys;
    ...
    if (!nsid) {
        for (i = 1; i <= NVME_MAX_NAMESPACES; i++) {
            if (nvme_subsys_ns(subsys, i)) { continue; }
    ...
    if (n->params.is_apple_ans && ns->id_ns.nstype == 1) {
        ...
        // int chip_die_index = 0x; // only used when vendor_index == 1 && density_index == 1, has weird encoding in
        // id_standard_high
...
static const Property nvme_ns_props[] = {
    ...
    DEFINE_PROP_SIZE("zoned.zone_size", NvmeNamespace, params.zone_size_bs, NVME_DEFAULT_ZONE_SIZE),
    DEFINE_PROP_SIZE("zoned.zone_capacity", NvmeNamespace, params.zone_cap_bs, 0),
    DEFINE_PROP_BOOL("zoned.cross_read", NvmeNamespace, params.cross_zone_read, false),
    DEFINE_PROP_UINT32("zoned.max_active", NvmeNamespace, params.max_active_zones, 0),
    DEFINE_PROP_UINT32("zoned.max_open", NvmeNamespace, params.max_open_zones, 0),
    DEFINE_PROP_UINT32("zoned.descr_ext_size", NvmeNamespace, params.zd_extension_size, 0),
    ...
    DEFINE_PROP_BOOL("eui64-default", NvmeNamespace, params.eui64_default, false),
