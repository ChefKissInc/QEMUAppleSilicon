...
static bool nvme_nguid_is_valid(const char* str)
{
    ...
    for (i = 0; i < strlen(str); i++) {
        ...
        if (c == NGUID_SEPARATOR) {
             ...
             * back "--".
             */
            if ((i > 0) && (str[i - 1] != NGUID_SEPARATOR) && (digit_count % 2) == 0) { continue; }
        }
        return false;
    }
    /*
...
static int nvme_nguid_parse(const char* str, NvmeNGUID* nguid)
{
    ...
    if (!nvme_nguid_is_valid(str)) { return -1; }

    for (i = 0; i < NGUID_LEN; i++) {
        ret = sscanf(ptr, "%02hhx", &id[i]);
        if (ret != 1) { return -1; }
        ptr += 2;
        if (*ptr == NGUID_SEPARATOR) { ptr++; }
    }
...
static void nvme_nguid_stringify(const NvmeNGUID* nguid, char* out)
{
    ...
    snprintf(out, NGUID_STR_LEN, NGUID_FMT, id[0], id[1], id[2], id[3], id[4], id[5], id[6], id[7], id[8], id[9],
             id[10], id[11], id[12], id[13], id[14], id[15]);
}

static void get_nguid(Object* obj, Visitor* v, const char* name, void* opaque, Error** errp)
{
...
static void set_nguid(Object* obj, Visitor* v, const char* name, void* opaque, Error** errp)
{
    const Property* prop  = opaque;
    NvmeNGUID*      nguid = object_field_prop_ptr(obj, prop);
    char*           str;
    ...
    if (!strcmp(str, NGUID_VALUE_AUTO)) { nvme_nguid_generate(nguid); }
...
const PropertyInfo qdev_prop_nguid = {
    .type        = "str",
    .description = "NGUID or \"" NGUID_VALUE_AUTO "\" for random value",
    .get         = get_nguid,
    .set         = set_nguid,
};
