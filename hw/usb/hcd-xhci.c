...
#define get_field(data, field) (((data) >> field##_SHIFT) & field##_MASK)
...
static const char* TRBCCode_names[] = {
    ...
    [CC_MAX_EXIT_LATENCY_TOO_LARGE_ERROR] = "CC_MAX_EXIT_LATENCY_TOO_LARGE_ERROR",
...
static const char* ep_state_names[] = {
    [EP_DISABLED] = "disabled", [EP_RUNNING] = "running", [EP_HALTED] = "halted",
    [EP_STOPPED] = "stopped",   [EP_ERROR] = "error",
};

static const char* lookup_name(uint32_t index, const char** list, uint32_t llen)
{
    if (index >= llen || list[index] == NULL) { return "???"; }
    return list[index];
}
...
bool xhci_get_flag(XHCIState* xhci, enum xhci_flags bit) { return xhci->flags & (1 << bit); }

void xhci_set_flag(XHCIState* xhci, enum xhci_flags bit) { xhci->flags |= (1 << bit); }
...
{
    if (sizeof(dma_addr_t) == 4) { return low; }
    else {
...
{
    if (sizeof(dma_addr_t) == 4) { return addr & 0xffffffff; }
    else {
        return addr;
    }
}
...
static inline void xhci_dma_read_u32s(XHCIState* xhci, dma_addr_t addr, uint32_t* buf, size_t len)
{
    ...
    if (dma_memory_read(xhci->as, addr, buf, len, MEMTXATTRS_UNSPECIFIED) != MEMTX_OK) {
    ...
    for (i = 0; i < (len / sizeof(uint32_t)); i++) { buf[i] = le32_to_cpu(buf[i]); }
}

static inline void xhci_dma_write_u32s(XHCIState* xhci, dma_addr_t addr, const uint32_t* buf, size_t len)
{
    int      i;
    uint32_t tmp[5];
    ...
    for (i = 0; i < n; i++) { tmp[i] = cpu_to_le32(buf[i]); }
    if (dma_memory_write(xhci->as, addr, tmp, len, MEMTXATTRS_UNSPECIFIED) != MEMTX_OK) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: DMA memory access failed!\n", __func__);
        xhci_die(xhci);
        return;
    }
...
static void xhci_intr_update(XHCIState* xhci, int v)
{
    ...
    if (v == 0) {
        if (xhci->intr[0].iman & IMAN_IP && xhci->intr[0].iman & IMAN_IE && xhci->usbcmd & USBCMD_INTE) { level = 1; }
        if (xhci->intr_raise) {
            if (xhci->intr_raise(xhci, 0, level)) { xhci->intr[0].iman &= ~IMAN_IP; }
        }
    }
    if (xhci->intr_update) { xhci->intr_update(xhci, v, xhci->intr[v].iman & IMAN_IE); }
}
...
static void xhci_intr_raise(XHCIState* xhci, int v)
{
    ...
    if (pending) { return; }
    if (!(xhci->intr[v].iman & IMAN_IE)) { return; }

    if (!(xhci->usbcmd & USBCMD_INTE)) { return; }
    if (xhci->intr_raise) {
        if (xhci->intr_raise(xhci, v, true)) { xhci->intr[v].iman &= ~IMAN_IP; }
    }
}

static inline int xhci_running(XHCIState* xhci) { return !(xhci->usbsts & USBSTS_HCH); }
...
static void xhci_write_event(XHCIState* xhci, XHCIEvent* event, int v)
{
    ...
    if (intr->er_pcs) { ev_trb.control |= TRB_C; }
    ev_trb.control = cpu_to_le32(ev_trb.control);

    trace_usb_xhci_queue_event(v, intr->er_ep_idx, trb_name(&ev_trb), event_name(event), ev_trb.parameter,
                               ev_trb.status, ev_trb.control);

    addr = intr->er_start + TRB_SIZE * intr->er_ep_idx;
    if (dma_memory_write(xhci->as, addr, &ev_trb, TRB_SIZE, MEMTXATTRS_UNSPECIFIED) != MEMTX_OK) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: DMA memory access failed!\n", __func__);
        xhci_die(xhci);
    }
...
static void xhci_event(XHCIState* xhci, XHCIEvent* event, int v)
{
    ...
    if (xhci->numintrs == 1 || (xhci->intr_mapping_supported && !xhci->intr_mapping_supported(xhci))) { v = 0; }
...
static TRBType xhci_ring_fetch(XHCIState* xhci, XHCIRing* ring, XHCITRB* trb, dma_addr_t* addr)
{
    uint32_t link_cnt = 0;

    while (1) {
        TRBType type;
        if (dma_memory_read(xhci->as, ring->dequeue, trb, TRB_SIZE, MEMTXATTRS_UNSPECIFIED) != MEMTX_OK) {
            qemu_log_mask(LOG_GUEST_ERROR, "%s: DMA memory access failed!\n", __func__);
            return 0;
        }
        ...
        if ((trb->control & TRB_C) != ring->ccs) { return 0; }

        type = TRB_TYPE(*trb);

        if (type != TR_LINK) {
            if (addr) { *addr = ring->dequeue; }
            ...
            if (trb->control & TRB_LK_TC) { ring->ccs = !ring->ccs; }
        }
    }
}
...
static int xhci_ring_chain_length(XHCIState* xhci, const XHCIRing* ring)
{
    ...
    do {
        ...
        if ((trb.control & TRB_C) != ccs) { return -length; }

        type = TRB_TYPE(trb);

        if (type == TR_LINK) {
            if (++link_cnt > TRB_LINK_LIMIT) { return -length; }
            dequeue = xhci_mask64(trb.parameter);
            if (trb.control & TRB_LK_TC) { ccs = !ccs; }
            continue;
        }
        ...
        if (type == TR_SETUP) { control_td_set = 1; }
        else if (type == TR_STATUS) {
            control_td_set = 0;
        }

        if (!control_td_set && !(trb.control & TRB_TR_CH)) { return length; }
...
static void xhci_er_reset(XHCIState* xhci, int v)
{
    ...
    if (dma_memory_read(xhci->as, erstba, &seg, sizeof(seg), MEMTXATTRS_UNSPECIFIED) != MEMTX_OK) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: DMA memory access failed!\n", __func__);
        xhci_die(xhci);
        return;
    }
...
static void xhci_reset_streams(XHCIEPContext* epctx)
{
    ...
    for (i = 0; i < epctx->nr_pstreams; i++) { epctx->pstreams[i].sct = -1; }
}
...
static int xhci_epmask_to_eps_with_streams(XHCIState* xhci, unsigned int slotid, uint32_t epmask,
                                           XHCIEPContext** epctxs, USBEndpoint** eps)
{
    XHCISlot*      slot;
    XHCIEPContext* epctx;
    USBEndpoint*   ep;
    ...
    for (i = 2, j = 0; i <= 31; i++) {
        if (!(epmask & (1u << i))) { continue; }

        epctx = slot->eps[i - 1];
        ep    = xhci_epid_to_usbep(epctx);
        if (!epctx || !epctx->nr_pstreams || !ep) { continue; }

        if (epctxs) { epctxs[j] = epctx; }
        eps[j++] = ep;
    }
    return j;
}
...
static void xhci_free_device_streams(XHCIState* xhci, unsigned int slotid, uint32_t epmask)
{
    ...
    if (nr_eps) { usb_device_free_streams(eps[0]->dev, eps, nr_eps); }
}
...
static TRBCCode xhci_alloc_device_streams(XHCIState* xhci, unsigned int slotid, uint32_t epmask)
{
     ...
     * can handle.
     */
    if (req_nr_streams > dev_max_streams) { req_nr_streams = dev_max_streams; }
...
static void xhci_init_epctx(XHCIEPContext* epctx, dma_addr_t pctx, uint32_t* ctx)
{
    ...
    if (epctx->max_pstreams) { xhci_alloc_streams(epctx, dequeue); }
    else {
...
static TRBCCode xhci_enable_ep(XHCIState* xhci, unsigned int slotid, unsigned int epid, dma_addr_t pctx, uint32_t* ctx)
{
    ...
    slot = &xhci->slots[slotid - 1];
    if (slot->eps[epid - 1]) { xhci_disable_ep(xhci, slotid, epid); }
...
static XHCITransfer* xhci_ep_alloc_xfer(XHCIEPContext* epctx, uint32_t length)
{
    ...
    if (epctx->xfer_count >= limit) { return NULL; }
...
static int xhci_ep_nuke_xfers(XHCIState* xhci, unsigned int slotid, unsigned int epid, TRBCCode report)
{
    ...
    for (;;) {
        ...
        if (killed) { report = 0; /* Only report once */ }
        xhci_ep_free_xfer(xfer);
    }

    ep = xhci_epid_to_usbep(epctx);
    if (ep) { usb_device_ep_stopped(ep->dev, ep); }
    return killed;
}
...
static TRBCCode xhci_disable_ep(XHCIState* xhci, unsigned int slotid, unsigned int epid)
{
    ...
    if (epctx->nr_pstreams) { xhci_free_streams(epctx); }

    /* only touch guest RAM if we're not resetting the HC */
    if (xhci->dcbaap_low || xhci->dcbaap_high) { xhci_set_ep_state(xhci, epctx, NULL, EP_DISABLED); }
...
static TRBCCode xhci_stop_ep(XHCIState* xhci, unsigned int slotid, unsigned int epid)
{
    ...
    if (epctx->nr_pstreams) { xhci_reset_streams(epctx); }
...
static TRBCCode xhci_reset_ep(XHCIState* xhci, unsigned int slotid, unsigned int epid)
{
    ...
    if (!xhci->slots[slotid - 1].uport || !xhci->slots[slotid - 1].uport->dev
...
static TRBCCode xhci_set_ep_dequeue(XHCIState* xhci, unsigned int slotid, unsigned int epid, unsigned int streamid,
                                    uint64_t pdequeue)
{
    XHCISlot*          slot;
    XHCIEPContext*     epctx;
...
static int xhci_xfer_create_sgl(XHCITransfer* xfer, int in_xfer)
{
    ...
    for (i = 0; i < xfer->trb_count; i++) {
        ...
        if (trb->control & TRB_TR_IOC) { xfer->int_req = true; }
    ...
    for (i = 0; i < xfer->trb_count; i++) {
        ...
        switch (TRB_TYPE(*trb)) {
            ...
            case TR_ISOCH:
                ...
                if (chunk > left) {
                    chunk = left;
                    if (xfer->status == CC_SUCCESS) { shortpkt = 1; }
                }
                left  -= chunk;
                edtla += chunk;
                break;
        ...
        if (!reported
            && ((trb->control & TRB_TR_IOC) || (shortpkt && (trb->control & TRB_TR_ISP))
                || (xfer->status != CC_SUCCESS && left == 0)))
        {
            ...
            event.flags  = 0;
            event.ptr    = trb->addr;
            if (xfer->status == CC_SUCCESS) { event.ccode = shortpkt ? CC_SHORT_PACKET : CC_SUCCESS; }
            else {
            ...
            reported = 1;
            if (xfer->status != CC_SUCCESS) { return; }
        }
...
static int xhci_setup_packet(XHCITransfer* xfer)
{
    ...
    if (xfer->packet.ep) { ep = xfer->packet.ep; }
    else {
...
static int xhci_try_complete_packet(XHCITransfer* xfer)
{
    ...
    switch (xfer->packet.status) {
        ...
        case USB_RET_BABBLE:
            ...
            xhci_stall_ep(xfer);
            break;
        default: DPRINTF("%s: FIXME: status = %d\n", __func__, xfer->packet.status); FIXME("unhandled USB_RET_*");
    }
    return 0;
}
...
static int xhci_fire_ctl_transfer(XHCIState* xhci, XHCITransfer* xfer)
{
    ...
    if (TRB_TYPE(*trb_status) == TR_EVDATA && xfer->trb_count > 2) { trb_status--; }
    ...
    if (xhci_setup_packet(xfer) < 0) { return -1; }
...
static void xhci_calc_iso_kick(XHCIState* xhci, XHCITransfer* xfer, XHCIEPContext* epctx, uint64_t mfindex)
{
    if (xfer->trbs[0].control & TRB_TR_SIA) {
        ...
        if (xfer->mfindex_kick + 0x100 < mfindex) { xfer->mfindex_kick += 0x4000; }
    }
}
...
static int xhci_submit(XHCIState* xhci, XHCITransfer* xfer, XHCIEPContext* epctx)
{
    ...
    switch (epctx->type) {
        ...
        case ET_INTR_IN:
            ...
            if (xfer->running_retry) { return -1; }
            break;
        case ET_BULK_OUT:
        case ET_BULK_IN:
            xfer->pkts       = 0;
        ...
        case ET_ISO_IN:
            ...
            if (xfer->running_retry) { return -1; }
            break;
        default: trace_usb_xhci_unimplemented("endpoint type", epctx->type); return -1;
    }
...
{
    return (xhci->slots[slotid - 1].uport && xhci->slots[slotid - 1].uport->dev
            && xhci->slots[slotid - 1].uport->dev->attached);
}
...
static void xhci_kick_epctx(XHCIEPContext* epctx, unsigned int streamid)
{
    ...
    if (!xhci_slot_ok(xhci, epctx->slotid)) { return; }
    ...
    if (epctx->retry) {
        ...
        if (xfer->timed_xfer) {
            ...
            mfindex = xhci_mfindex_get(xhci);
            xhci_check_intr_iso_kick(xhci, xfer, epctx, mfindex);
            if (xfer->running_retry) { return; }
            xfer->timed_xfer    = 0;
            xfer->running_retry = 1;
        }
        if (xfer->iso_xfer) {
            /* retry iso transfer */
            if (xhci_setup_packet(xfer) < 0) { return; }
            usb_handle_packet(xfer->packet.ep->dev, &xfer->packet);
    ...
    while (1) {
        ...
        if (length <= 0) {
            if (epctx->type == ET_ISO_OUT || epctx->type == ET_ISO_IN) {
                ...
                ev.ccode     = epctx->type == ET_ISO_IN ? CC_RING_OVERRUN : CC_RING_UNDERRUN;
        ...
        if (epctx->epid == 1) { xhci_fire_ctl_transfer(xhci, xfer); }
        else {
        ...
        if (epctx->state == EP_HALTED) { break; }
    ...
    ep = xhci_epid_to_usbep(epctx);
    if (ep) { usb_device_flush_ep_queue(ep->dev, ep); }
}
...
static TRBCCode xhci_disable_slot(XHCIState* xhci, unsigned int slotid)
{
    ...
    for (i = 1; i <= 31; i++) {
        if (xhci->slots[slotid - 1].eps[i - 1]) { xhci_disable_ep(xhci, slotid, i); }
    }
...
static USBPort* xhci_lookup_uport(XHCIState* xhci, uint32_t* slot_ctx)
{
    ...
    if (port < 1 || port > xhci->numports) { return NULL; }
    ...
    QTAILQ_FOREACH (uport, &xhci->bus.used, next) {
        if (strcmp(uport->path, path) == 0) { return uport; }
    }
    return NULL;
}

static TRBCCode xhci_address_slot(XHCIState* xhci, unsigned int slotid, uint64_t pictx, bool bsr)
{
    XHCISlot*  slot;
    USBPort*   uport;
    USBDevice* dev;
    ...
    if (bsr) { slot_ctx[3] = SLOT_DEFAULT << SLOT_STATE_SHIFT; }
    else {
        USBPacket p;
        uint8_t   buf[1];
...
static TRBCCode xhci_configure_slot(XHCIState* xhci, unsigned int slotid, uint64_t pictx, bool dc)
{
    ...
    for (i = 2; i <= 31; i++) {
        if (ictl_ctx[0] & (1 << i)) { xhci_disable_ep(xhci, slotid, i); }
        if (ictl_ctx[1] & (1 << i)) {
            xhci_dma_read_u32s(xhci, ictx + 32 + (32 * i), ep_ctx, sizeof(ep_ctx));
            DPRINTF("xhci: input ep%d.%d context: %08x %08x %08x %08x %08x\n", i / 2, i % 2, ep_ctx[0], ep_ctx[1],
                    ep_ctx[2], ep_ctx[3], ep_ctx[4]);
            xhci_disable_ep(xhci, slotid, i);
            res = xhci_enable_ep(xhci, slotid, i, octx + (32 * i), ep_ctx);
            if (res != CC_SUCCESS) { return res; }
    ...
    if (res != CC_SUCCESS) {
        for (i = 2; i <= 31; i++) {
            if (ictl_ctx[1] & (1u << i)) { xhci_disable_ep(xhci, slotid, i); }
        }
        return res;
    }
...
static TRBCCode xhci_evaluate_slot(XHCIState* xhci, unsigned int slotid, uint64_t pictx)
{
    ...
    if (ictl_ctx[1] & 0x1) {
        ...
        DPRINTF("xhci: input slot context: %08x %08x %08x %08x\n", islot_ctx[0], islot_ctx[1], islot_ctx[2],
                islot_ctx[3]);
...
static void xhci_detach_slot(XHCIState* xhci, USBPort* uport)
{
    ...
    for (slot = 0; slot < xhci->numslots; slot++) {
        if (xhci->slots[slot].uport == uport) { break; }
    }
    if (slot == xhci->numslots) { return; }

    for (ep = 0; ep < 31; ep++) {
        if (xhci->slots[slot].eps[ep]) { xhci_ep_nuke_xfers(xhci, slot + 1, ep + 1, 0); }
    }
...
static TRBCCode xhci_get_port_bandwidth(XHCIState* xhci, uint64_t pctx)
{
    ...
    if (stb_dma(xhci->as, ctx, 0, MEMTXATTRS_UNSPECIFIED) != MEMTX_OK
        || dma_memory_set(xhci->as, ctx + 1, 80, xhci->numports, MEMTXATTRS_UNSPECIFIED) != MEMTX_OK)
    {
    ...
    while ((type = xhci_ring_fetch(xhci, &xhci->cmd_ring, &trb, &addr))) {
        ...
        switch (type) {
            ...
            case CR_DISABLE_SLOT:
                ...
                if (slotid) { event.ccode = xhci_disable_slot(xhci, slotid); }
                break;
            case CR_ADDRESS_DEVICE:
                slotid = xhci_get_slot(xhci, &event, &trb);
                if (slotid) { event.ccode = xhci_address_slot(xhci, slotid, trb.parameter, trb.control & TRB_CR_BSR); }
                break;
            case CR_CONFIGURE_ENDPOINT:
                slotid = xhci_get_slot(xhci, &event, &trb);
                if (slotid) { event.ccode = xhci_configure_slot(xhci, slotid, trb.parameter, trb.control & TRB_CR_DC); }
                break;
            case CR_EVALUATE_CONTEXT:
                slotid = xhci_get_slot(xhci, &event, &trb);
                if (slotid) { event.ccode = xhci_evaluate_slot(xhci, slotid, trb.parameter); }
                break;
            ...
            case CR_RESET_DEVICE:
                slotid = xhci_get_slot(xhci, &event, &trb);
                if (slotid) { event.ccode = xhci_reset_slot(xhci, slotid); }
                break;
            case CR_GET_PORT_BANDWIDTH: event.ccode = xhci_get_port_bandwidth(xhci, trb.parameter); break;
            case CR_NOOP              : event.ccode = CC_SUCCESS; break;
...
{
    if (!port->uport->dev || !port->uport->dev->attached) { return false; /* no device present */ }
    if (!((1 << port->uport->dev->speed) & port->speedmask)) { return false; /* speed mismatch */ }
    return true;
}
...
static void xhci_port_notify(XHCIPort* port, uint32_t bits)
{
    ...
    if ((port->portsc & bits) == bits) { return; }
    trace_usb_xhci_port_notify(port->portnr, bits);
    port->portsc |= bits;
    if (!xhci_running(port->xhci)) { return; }
...
static void xhci_port_reset(XHCIPort* port, bool warm_reset)
{
    ...
    if (!xhci_port_have_device(port)) { return; }
...
static uint64_t xhci_cap_read(void* ptr, hwaddr reg, unsigned size)
{
    ...
    switch (reg) {
        case 0x00: /* HCIVERSION, CAPLENGTH */ ret = 0x01000000 | LEN_CAP; break;
        case 0x04: /* HCSPARAMS 1 */
            ret = ((xhci->numports_2 + xhci->numports_3) << 24) | (xhci->numintrs << 8) | xhci->numslots;
            break;
        case 0x08: /* HCSPARAMS 2 */ ret = 0x0000000f; break;
        case 0x0c: /* HCSPARAMS 3 */ ret = 0x00000000; break;
        case 0x10: /* HCCPARAMS */
            if (sizeof(dma_addr_t) == 4) { ret = 0x00080000 | (xhci->max_pstreams_mask << 12); }
            else {
                ret = 0x00080001 | (xhci->max_pstreams_mask << 12);
            }
            break;
        case 0x14: /* DBOFF */ ret = OFF_DOORBELL; break;
        case 0x18: /* RTSOFF */ ret = OFF_RUNTIME; break;
        ...
        case 0x24:            /* Supported Protocol:04 */
            ret = 0x20425355; /* "USB " */
            break;
        case 0x28: /* Supported Protocol:08 */ ret = (xhci->numports_2 << 8) | (xhci->numports_3 + 1); break;
        ...
        case 0x34:            /* Supported Protocol:04 */
            ret = 0x20425355; /* "USB " */
            break;
        case 0x38: /* Supported Protocol:08 */ ret = (xhci->numports_3 << 8) | 1; break;
        case 0x3c:            /* Supported Protocol:0c */
            ret = 0x00000000; /* reserved */
            break;
        default: trace_usb_xhci_unimplemented("cap read", reg); ret = 0;
    }
...
static uint64_t xhci_port_read(void* ptr, hwaddr reg, unsigned size)
{
    ...
    switch (reg) {
        case 0x00: /* PORTSC */ ret = port->portsc; break;
        case 0x04: /* PORTPMSC */
        case 0x08: /* PORTLI */ ret = 0; break;
        case 0x0c: /* PORTHLPMC */
            ret = 0;
            qemu_log_mask(LOG_UNIMP, "%s: read from port register PORTHLPMC", __func__);
            break;
        default: qemu_log_mask(LOG_GUEST_ERROR, "%s: read from port offset 0x%" HWADDR_PRIx, __func__, reg); ret = 0;
    }
...
static void xhci_port_write(void* ptr, hwaddr reg, uint64_t val, unsigned size)
{
    ...
    switch (reg) {
        case 0x00: /* PORTSC */
            ...
            portsc &=
                ~(val & (PORTSC_CSC | PORTSC_PEC | PORTSC_WRC | PORTSC_OCC | PORTSC_PRC | PORTSC_PLC | PORTSC_CEC));
            if (val & PORTSC_LWS) {
            ...
            port->portsc  = portsc;
            if (notify) { xhci_port_notify(port, notify); }
            break;
        case 0x04: /* PORTPMSC */
        case 0x0c: /* PORTHLPMC */
            qemu_log_mask(LOG_UNIMP, "%s: write 0x%" PRIx64 " (%u bytes) to port register at offset 0x%" HWADDR_PRIx,
                          __func__, val, size, reg);
            break;
        case 0x08: /* PORTLI */
            qemu_log_mask(LOG_GUEST_ERROR, "%s: Write to read-only PORTLI register", __func__);
            break;
        default:
...
static uint64_t xhci_oper_read(void* ptr, hwaddr reg, unsigned size)
{
    ...
    switch (reg) {
        case 0x00: /* USBCMD */ ret = xhci->usbcmd; break;
        case 0x04: /* USBSTS */ ret = xhci->usbsts; break;
        case 0x08:   /* PAGESIZE */
            ret = 1; /* 4KiB */
            break;
        case 0x14: /* DNCTRL */ ret = xhci->dnctrl; break;
        case 0x18: /* CRCR low */ ret = xhci->crcr_low & ~0xe; break;
        case 0x1c: /* CRCR high */ ret = xhci->crcr_high; break;
        case 0x30: /* DCBAAP low */ ret = xhci->dcbaap_low; break;
        case 0x34: /* DCBAAP high */ ret = xhci->dcbaap_high; break;
        case 0x38: /* CONFIG */ ret = xhci->config; break;
        default  : trace_usb_xhci_unimplemented("oper read", reg); ret = 0;
    }
...
static void xhci_oper_write(void* ptr, hwaddr reg, uint64_t val, unsigned size)
{
    ...
    switch (reg) {
        case 0x00: /* USBCMD */
            if ((val & USBCMD_RS) && !(xhci->usbcmd & USBCMD_RS)) { xhci_run(xhci); }
        ...
        case 0x14: /* DNCTRL */ xhci->dnctrl = val & 0xffff; break;
        case 0x18: /* CRCR low */ xhci->crcr_low = (val & 0xffffffcf) | (xhci->crcr_low & CRCR_CRR); break;
        case 0x1c: /* CRCR high */
            xhci->crcr_high = val;
            ...
            break;
        case 0x30: /* DCBAAP low */ xhci->dcbaap_low = val & 0xffffffc0; break;
        case 0x34: /* DCBAAP high */ xhci->dcbaap_high = val; break;
        case 0x38: /* CONFIG */ xhci->config = val & 0xff; break;
...
static uint64_t xhci_runtime_read(void* ptr, hwaddr reg, unsigned size)
{
    ...
    if (reg < 0x20) {
        switch (reg) {
            case 0x00: /* MFINDEX */ ret = xhci_mfindex_get(xhci) & 0x3fff; break;
            default  : trace_usb_xhci_unimplemented("runtime read", reg); break;
        }
    }
    else {
        int              v    = (reg - 0x20) / 0x20;
        XHCIInterrupter* intr = &xhci->intr[v];
        switch (reg & 0x1f) {
            case 0x00: /* IMAN */ ret = intr->iman; break;
            case 0x04: /* IMOD */ ret = intr->imod; break;
            case 0x08: /* ERSTSZ */ ret = intr->erstsz; break;
            case 0x10: /* ERSTBA low */ ret = intr->erstba_low; break;
            case 0x14: /* ERSTBA high */ ret = intr->erstba_high; break;
            case 0x18: /* ERDP low */ ret = intr->erdp_low; break;
            case 0x1c: /* ERDP high */ ret = intr->erdp_high; break;
        }
    }
...
static void xhci_runtime_write(void* ptr, hwaddr reg, uint64_t val, unsigned size)
{
    ...
    switch (reg & 0x1f) {
        case 0x00: /* IMAN */
            if (val & IMAN_IP) { intr->iman &= ~IMAN_IP; }
            intr->iman &= ~IMAN_IE;
            intr->iman |= val & IMAN_IE;
            xhci_intr_update(xhci, v);
            break;
        case 0x04: /* IMOD */ intr->imod = val; break;
        case 0x08: /* ERSTSZ */ intr->erstsz = val & 0xffff; break;
        ...
        case 0x14: /* ERSTBA high */
            ...
            break;
        case 0x18: /* ERDP low */
            if (val & ERDP_EHB) { intr->erdp_low &= ~ERDP_EHB; }
            ...
            break;
        case 0x1c: /* ERDP high */ intr->erdp_high = val; break;
        default  : trace_usb_xhci_unimplemented("oper write", reg);
    }
}
...
static void xhci_doorbell_write(void* ptr, hwaddr reg, uint64_t val, unsigned size)
{
    ...
    if (reg == 0) {
        if (val == 0) { xhci_process_commands(xhci); }
        else {
        ...
        if (reg > xhci->numslots) { DPRINTF("xhci: bad doorbell %d\n", (int)reg); }
...
static void xhci_wakeup(USBPort* usbport)
{
    ...
    assert(port);
    if (get_field(port->portsc, PORTSC_PLS) != PLS_U3) { return; }
...
static void xhci_complete(USBPort* port, USBPacket* packet)
{
    ...
    if (xfer->complete) { xhci_ep_free_xfer(xfer); }
}
...
{
    if (ep->nr == 0) { return 1; }
    if (ep->pid == USB_TOKEN_IN) { return ep->nr * 2 + 1; }
    else {
        return ep->nr * 2;
    }
}
...
static USBEndpoint* xhci_epid_to_usbep(XHCIEPContext* epctx)
{
    ...
    if (!uport || !uport->dev) { return NULL; }
...
static void xhci_wakeup_endpoint(USBBus* bus, USBEndpoint* ep, unsigned int stream)
{
    ...
    DPRINTF("%s\n", __func__);
    slotid = ep->dev->addr;
    if (slotid == 0 || slotid > xhci->numslots || !xhci->slots[slotid - 1].enabled) {
...
static void usb_xhci_init(XHCIState* xhci)
{
    ...
    if (xhci->numports_2 > XHCI_MAXPORTS_2) { xhci->numports_2 = XHCI_MAXPORTS_2; }
    if (xhci->numports_3 > XHCI_MAXPORTS_3) { xhci->numports_3 = XHCI_MAXPORTS_3; }
...
static void usb_xhci_realize(DeviceState* dev, Error** errp)
{
    ...
    if (xhci->numintrs > XHCI_MAXINTRS) { xhci->numintrs = XHCI_MAXINTRS; }
    while (xhci->numintrs & (xhci->numintrs - 1)) { /* ! power of 2 */
        xhci->numintrs++;
    }
    if (xhci->numintrs < 1) { xhci->numintrs = 1; }
    if (xhci->numslots > XHCI_MAXSLOTS) { xhci->numslots = XHCI_MAXSLOTS; }
    if (xhci->numslots < 1) { xhci->numslots = 1; }
    if (xhci_get_flag(xhci, XHCI_FLAG_ENABLE_STREAMS)) { xhci->max_pstreams_mask = 7; /* == 256 primary streams */ }
    else {
    ...
    memory_region_init_io(&xhci->mem_cap, OBJECT(dev), &xhci_cap_ops, xhci, "capabilities", LEN_CAP);
    memory_region_init_io(&xhci->mem_oper, OBJECT(dev), &xhci_oper_ops, xhci, "operational", 0x400);
    memory_region_init_io(&xhci->mem_runtime, OBJECT(dev), &xhci_runtime_ops, xhci, "runtime", LEN_RUNTIME);
    memory_region_init_io(&xhci->mem_doorbell, OBJECT(dev), &xhci_doorbell_ops, xhci, "doorbell", LEN_DOORBELL);
    ...
    for (i = 0; i < xhci->numports; i++) {
        ...
        port->xhci       = xhci;
        memory_region_init_io(&port->mem, OBJECT(dev), &xhci_port_ops, port, port->name, 0x10);
...
static const Property xhci_properties[] = {
    DEFINE_PROP_BIT("streams", XHCIState, flags, XHCI_FLAG_ENABLE_STREAMS, true),
    DEFINE_PROP_UINT32("p2", XHCIState, numports_2, 4),
    DEFINE_PROP_UINT32("p3", XHCIState, numports_3, 4),
    DEFINE_PROP_LINK("host", XHCIState, hostOpaque, TYPE_DEVICE, DeviceState*),
};
