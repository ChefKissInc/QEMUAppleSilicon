...
{
    if (_fifo == 0) { return s->gnptxfsiz & 0xFFFF; }
    else {
...
static inline void dwc2_update_irq(DWC2State* s)
{
    ...
    if ((s->gintsts & s->gintmsk) && (s->gahbcfg & GAHBCFG_GLBL_INTR_EN)) { level = 1; }
    if (level != oldlevel) {
        oldlevel = level;
...
static inline void dwc2_raise_host_irq(DWC2State* s, uint32_t host_intr)
{
    if (!(s->haint & host_intr)) {
        ...
        if (s->haint & s->haintmsk) { dwc2_raise_global_irq(s, GINTSTS_HCHINT); }
    }
}
...
static inline void dwc2_lower_host_irq(DWC2State* s, uint32_t host_intr)
{
    if (s->haint & host_intr) {
        ...
        if (!(s->haint & s->haintmsk)) { dwc2_lower_global_irq(s, GINTSTS_HCHINT); }
    }
}
...
static inline void dwc2_raise_device_irq(DWC2State* s, uint32_t ep, bool out)
{
    ...
    if (!(s->daint & device_intr)) {
        ...
        if (s->daint & s->daintmsk) {
            if (s->daint & 0xffff) { dwc2_raise_global_irq(s, GINTSTS_IEPINT); }

            if ((s->daint >> 16) & 0xffff) { dwc2_raise_global_irq(s, GINTSTS_OEPINT); }
        }
    }
}
...
static inline void dwc2_lower_device_irq(DWC2State* s, uint32_t ep, bool out)
{
    ...
    if (s->daint & device_intr) {
        ...
        if (!(s->daint & s->daintmsk)) {
            if (!(s->daint & 0xffff)) { dwc2_lower_global_irq(s, GINTSTS_IEPINT); }

            if (!((s->daint >> 16) & 0xffff)) { dwc2_lower_global_irq(s, GINTSTS_OEPINT); }
        }
    }
}
...
static inline void dwc2_update_hc_irq(DWC2State* s, int index)
{
    ...
    if (s->hreg1[index + 2] & s->hreg1[index + 3]) { dwc2_raise_host_irq(s, host_intr); }
    else {
...
{
    if (s->diepint(ep) & s->diepmsk) { dwc2_raise_device_irq(s, ep, false); }
    else {
        dwc2_lower_device_irq(s, ep, false);
    }

    if (s->doepint(ep) & s->doepmsk) { dwc2_raise_device_irq(s, ep, true); }
    else {
...
/* set a timer for EOF */
static void dwc2_eof_timer(DWC2State* s) { timer_mod(s->eof_timer, s->sof_time + s->usb_frame_time); }
...
static USBDevice* dwc2_find_device(DWC2State* s, uint8_t addr)
{
    ...
    if (!(s->hprt0 & HPRT0_ENA)) { trace_usb_dwc2_port_disabled(0); }
    else {
...
static const char* pstatus[] = {"USB_RET_SUCCESS", "USB_RET_NODEV",        "USB_RET_NAK",
                                "USB_RET_STALL",   "USB_RET_BABBLE",       "USB_RET_IOERROR",
                                "USB_RET_ASYNC",   "USB_RET_ADD_TO_QUEUE", "USB_RET_REMOVE_FROM_QUEUE"};

static uint32_t pintr[] = {HCINTMSK_XFERCOMPL, HCINTMSK_XACTERR, HCINTMSK_NAK,     HCINTMSK_STALL,  HCINTMSK_BBLERR,
                           HCINTMSK_XACTERR,   HCINTMSK_XACTERR, HCINTMSK_XACTERR, HCINTMSK_XACTERR};

static const char* types[] = {"Ctrl", "Isoc", "Bulk", "Intr"};

static const char* dirs[] = {"Out", "In"};

static void dwc2_handle_packet(DWC2State* s, uint32_t devadr, USBDevice* dev, USBEndpoint* ep, uint32_t index,
                               bool send)
{
    DWC2Packet* p;
    ...
    trace_usb_dwc2_handle_packet(chan, dev, &p->packet, epnum, types[eptype], dirs[epdir], mps, len, pcnt);
    ...
    if (eptype == USB_ENDPOINT_XFER_CONTROL && pid == TSIZ_SC_MC_PID_SETUP) { pid = USB_TOKEN_SETUP; }
    else {
    ...
    if (send) {
        tlen = len;
        if (p->small) {
            if (tlen > mps) { tlen = mps; }
        }

        if (pid != USB_TOKEN_IN) {
            trace_usb_dwc2_memory_read(hcdma, tlen);
            if (dma_memory_read(&s->dma_as, hcdma, s->usb_buf[chan], tlen, MEMTXATTRS_UNSPECIFIED) != MEMTX_OK) {
                qemu_log_mask(LOG_GUEST_ERROR, "%s: dma_memory_read failed\n", __func__);
            }
        }

        usb_packet_init(&p->packet);
        usb_packet_setup(&p->packet, pid, ep, 0, hcdma, pid != USB_TOKEN_IN, true);
...
babble:
    if (p->packet.status != USB_RET_SUCCESS && p->packet.status != USB_RET_NAK && p->packet.status != USB_RET_STALL
    ...
    if (p->packet.status == USB_RET_ASYNC) {
        trace_usb_dwc2_async_packet(&p->packet, chan, dev, epnum, dirs[epdir], tlen);
    ...
    if (p->packet.status == USB_RET_SUCCESS) {
        ...
        if (pid == USB_TOKEN_IN) {
            ...
            if (dma_memory_write(&s->dma_as, hcdma, s->usb_buf[chan], actual, MEMTXATTRS_UNSPECIFIED) != MEMTX_OK) {
        ...
        if (!pcnt || len == 0 || actual == 0) { done = true; }
    }
    else {
        intr |= pintr[stsidx];
        if (p->packet.status == USB_RET_NAK
            && (eptype == USB_ENDPOINT_XFER_CONTROL || eptype == USB_ENDPOINT_XFER_BULK))
        {
            /*
    ...
    if (done) {
        ...
        if (!(intr & HCINTMSK_CHHLTD)) { intr |= HCINTMSK_CHHLTD | HCINTMSK_XFERCOMPL; }
        intr                &= ~HCINTMSK_RESERVED14_31;
        s->hreg1[index + 2] |= intr;
        p->needs_service     = false;
    ...
    if (do_intr) { dwc2_update_hc_irq(s, index); }
}
...
static void dwc2_attach(USBPort* port)
{
    ...
    if (!port->dev || !port->dev->attached) { return; }
    ...
    switch (port->dev->speed) {
        case USB_SPEED_LOW : s->hprt0 |= HPRT0_SPD_LOW_SPEED << HPRT0_SPD_SHIFT; break;
        case USB_SPEED_FULL: s->hprt0 |= HPRT0_SPD_FULL_SPEED << HPRT0_SPD_SHIFT; break;
        case USB_SPEED_HIGH:
    ...
    if (hispd) {
        ...
        if (NANOSECONDS_PER_SECOND >= USB_HZ_HS) { s->usb_bit_time = NANOSECONDS_PER_SECOND / USB_HZ_HS; /* 10.4 */ }
        else {
            s->usb_bit_time = 1;
        }
    }
        ...
        if (NANOSECONDS_PER_SECOND >= USB_HZ_FS) { s->usb_bit_time = NANOSECONDS_PER_SECOND / USB_HZ_FS; /* 83.3 */ }
        else {
            s->usb_bit_time = 1;
        }
    }
...
static void dwc2_async_packet_complete(USBPort* port, USBPacket* packet)
{
    ...
    trace_usb_dwc2_async_packet_complete(port, packet, p->index >> 3, dev, p->epnum, dirs[p->epdir], p->len);
    assert(p->async == DWC2_ASYNC_INFLIGHT);
...
static uint32_t dwc2_get_frame_remaining(DWC2State* s)
{
    ...
    if (tks >= s->usb_frame_time) { goto out; }
...
static const char* glbregnm[] = {"GOTGCTL  ", "GOTGINT  ", "GAHBCFG  ", "GUSBCFG  ", "GRSTCTL  ", "GINTSTS  ",
...
static uint64_t dwc2_glbreg_read(void* ptr, hwaddr addr, int index, unsigned size)
{
    DWC2State* s = ptr;
    uint32_t   val;
    ...
    switch (addr) {
        case GRSTCTL:
            ...
            val &= ~(GRSTCTL_TXFFLSH | GRSTCTL_RXFFLSH | GRSTCTL_IN_TKNQ_FLSH | GRSTCTL_FRMCNTRRST | GRSTCTL_HSFTRST
                     | GRSTCTL_CSFTRST);
            s->glbreg[index] = val;
            break;
        default: break;
...
static void dwc2_glbreg_write(void* ptr, hwaddr addr, int index, uint64_t val, unsigned size)
{
    DWC2State* s    = ptr;
    uint64_t   orig = val;
    uint32_t*  mmio;
    ...
    switch (addr) {
        case GOTGCTL:
            ...
            val &= ~(GOTGCTL_MULT_VALID_BC_MASK | GOTGCTL_BSESVLD | GOTGCTL_ASESVLD | GOTGCTL_DBNC_SHORT
                     | GOTGCTL_CONID_B | GOTGCTL_HSTNEGSCS | GOTGCTL_SESREQSCS);
            /* don't allow clearing of read-only bits */
            val |= old
                   & (GOTGCTL_MULT_VALID_BC_MASK | GOTGCTL_BSESVLD | GOTGCTL_ASESVLD | GOTGCTL_DBNC_SHORT
                      | GOTGCTL_CONID_B | GOTGCTL_HSTNEGSCS | GOTGCTL_SESREQSCS);
            break;
        case GAHBCFG:
            if ((val & GAHBCFG_GLBL_INTR_EN) && !(old & GAHBCFG_GLBL_INTR_EN)) { iflg = 1; }
            break;
        case GRSTCTL:
            val |= GRSTCTL_AHBIDLE;
            val &= ~GRSTCTL_DMAREQ;
            ...
            val |= old
                   & (GRSTCTL_TXFFLSH | GRSTCTL_RXFFLSH | GRSTCTL_IN_TKNQ_FLSH | GRSTCTL_FRMCNTRRST | GRSTCTL_HSFTRST
                      | GRSTCTL_CSFTRST);
            break;
        case GINTSTS:
            ...
            /* don't allow clearing of read-only bits */
            val  |= old
                    & (GINTSTS_PTXFEMP | GINTSTS_HCHINT | GINTSTS_PRTINT | GINTSTS_OEPINT | GINTSTS_IEPINT
                       | GINTSTS_GOUTNAKEFF | GINTSTS_GINNAKEFF | GINTSTS_NPTXFEMP | GINTSTS_RXFLVL | GINTSTS_OTGINT
                       | GINTSTS_CURMODE_HOST);
            iflg  = 1;
            break;
        case GINTMSK: iflg = 1; break;
        default     : break;
    }
    ...
    *mmio = val;
    ...
    if (iflg) { dwc2_update_irq(s); }
}

static uint64_t dwc2_fszreg_read(void* ptr, hwaddr addr, int index, unsigned size)
{
    DWC2State* s = ptr;
    uint32_t   val;
...
static void dwc2_fszreg_write(void* ptr, hwaddr addr, int index, uint64_t val, unsigned size)
{
    DWC2State* s    = ptr;
    uint64_t   orig = val;
    uint32_t*  mmio;
...
static uint64_t dwc2_dfszreg_read(void* ptr, hwaddr addr, int index, unsigned size)
{
    DWC2State* s = ptr;
    uint32_t   val;
...
static void dwc2_dfszreg_write(void* ptr, hwaddr addr, int index, uint64_t val, unsigned size)
{
    DWC2State* s = ptr;
    uint32_t*  mmio;
...
static const char* hreg0nm[] = {"HCFG     ", "HFIR     ", "HFNUM    ", "<rsvd>   ", "HPTXSTS  ", "HAINT    ",
                                "HAINTMSK ", "HFLBADDR ", "<rsvd>   ", "<rsvd>   ", "<rsvd>   ", "<rsvd>   ",
                                "<rsvd>   ", "<rsvd>   ", "<rsvd>   ", "<rsvd>   ", "HPRT0    "};

static uint64_t dwc2_hreg0_read(void* ptr, hwaddr addr, int index, unsigned size)
{
    DWC2State* s = ptr;
    uint32_t   val;
    ...
    switch (addr) {
        case HFNUM: val = (dwc2_get_frame_remaining(s) << HFNUM_FRREM_SHIFT) | (s->hfnum << HFNUM_FRNUM_SHIFT); break;
        default   : break;
    }
...
static void dwc2_hreg0_write(void* ptr, hwaddr addr, int index, uint64_t val, unsigned size)
{
    DWC2State* s    = ptr;
    ...
    switch (addr) {
        ...
        case HFNUM   :
        case HPTXSTS :
        case HAINT   : qemu_log_mask(LOG_GUEST_ERROR, "%s: write to read-only register\n", __func__); return;
        case HAINTMSK: val &= 0xffff; break;
        case HPRT0:
            /* don't allow clearing of read-only bits */
            val |= old & (HPRT0_SPD_MASK | HPRT0_LNSTS_MASK | HPRT0_OVRCURRACT | HPRT0_CONNSTS);
            /* don't allow clearing of self-clearing bits */
            val |= old & (HPRT0_SUSP | HPRT0_RES);
            /* don't allow setting of self-setting bits */
            if (!(old & HPRT0_ENA) && (val & HPRT0_ENA)) { val &= ~HPRT0_ENA; }
            /* clear the write-1-to-clear bits */
            ...
            if (val & (HPRT0_OVRCURRCHG | HPRT0_ENACHG | HPRT0_CONNDET)) { iflg = 1; }
            else {
                iflg = -1;
            }
            break;
    ...
    if (prst) {
        trace_usb_dwc2_hreg0_write(addr, hreg0nm[index], orig, old, val & ~HPRT0_CONNDET);
...
static const char* hreg1nm[] = {"HCCHAR  ", "HCSPLT  ", "HCINT   ", "HCINTMSK",
                                "HCTSIZ  ", "HCDMA   ", "<rsvd>  ", "HCDMAB  "};

static uint64_t dwc2_hreg1_read(void* ptr, hwaddr addr, int index, unsigned size)
{
    DWC2State* s = ptr;
    uint32_t   val;
...
static void dwc2_hreg1_write(void* ptr, hwaddr addr, int index, uint64_t val, unsigned size)
{
    DWC2State* s    = ptr;
    uint64_t   orig = val;
    uint32_t*  mmio;
    ...
    switch (HSOTG_REG(0x500) + (addr & 0x1c)) {
        ...
        case HCINTMSK(0):
            ...
            iflg  = 1;
            break;
        case HCDMAB(0): qemu_log_mask(LOG_GUEST_ERROR, "%s: write to read-only register\n", __func__); return;
        default       : break;
    }

    trace_usb_dwc2_hreg1_write(addr, hreg1nm[index & 7], index >> 3, orig, old, val);
    *mmio = val;
    ...
    if (enflg) { dwc2_enable_chan(s, index & ~7); }

    if (iflg) { dwc2_update_hc_irq(s, index & ~7); }
}
...
static void dwc2_device_process_packet(DWC2State* s, USBPacket* p)
{
    ...
    switch (p->pid) {
        case USB_TOKEN_IN:
            ...
            if ((!(s->diepctl(ep) & DXEPCTL_USBACTEP)) || (s->diepctl(ep) & DXEPCTL_NAKSTS)
            ...
            if (s->diepctl(ep) & DXEPCTL_EPENA) {
                ...
                if (ep == 0) {
                    ...
                    switch (s->diepctl(0) & D0EPCTL_MPS_MASK) {
                        case D0EPCTL_MPS_64: mps = 64; break;
                ...
                if (s->dcfg & DCFG_DESCDMA_EN) {
                    ...
                    QEMUSGList           sglist;
                    bool                 ioc = false;
                    qemu_sglist_init(&sglist, DEVICE(s), MAX_DMA_DESC_NUM_GENERIC, &s->dma_as);
                    while (dma_memory_read(&s->dma_as, s->diepdma(ep) | SOC_DMA_BASE, &desc, sizeof(desc),
                                           MEMTXATTRS_UNSPECIFIED)
                           == MEMTX_OK)
                    {
                        uint32_t amtDone2 = 0;
                        if (DEV_DMA_BUFF_STS_GET(desc.status)) { break; }
                        ...
                        dma_memory_write(&s->dma_as, s->diepdma(ep) | SOC_DMA_BASE, &desc, sizeof(desc),
                                         MEMTXATTRS_UNSPECIFIED);
                        s->diepdma(ep) += sizeof(desc);
                        if (desc.status & DEV_DMA_L) { break; }
                    }
#if 0
                qemu_log_mask(LOG_UNIMP, "%s: starting IN transfer on EP %d (%zu/%d)...\n",
                    ...
                    dma_buf_write(buffer, sglist.size, &residual, &sglist, MEMTXATTRS_UNSPECIFIED);
                ...
                qemu_hexdump(stderr, __func__, buffer, sglist.size);
                    ...
                    if (amtDone > pktsize) { amtDone = pktsize; }
                    ...
                    if (amtDone > 0) {
                        ...
                        if (s->diepdma(ep)) {
                            dma_memory_read(&s->dma_as, s->diepdma(ep) | SOC_DMA_BASE, buffer, amtDone,
                                            MEMTXATTRS_UNSPECIFIED);
                    ...
                    if (ep == 0) {
                        s->dieptsiz(ep) = (s->dieptsiz(ep) & ~DIEPTSIZ0_PKTCNT_MASK) | DIEPTSIZ0_PKTCNT(pktcnt);

                        s->dieptsiz(ep) =
                            (s->dieptsiz(ep) & ~DIEPTSIZ0_XFERSIZE_MASK) | DIEPTSIZ0_XFERSIZE(sz - amtDone);
                    }
                    else {
                        s->dieptsiz(ep) = (s->dieptsiz(ep) & ~DXEPTSIZ_PKTCNT_MASK) | DXEPTSIZ_PKTCNT(pktcnt);

                        s->dieptsiz(ep) = (s->dieptsiz(ep) & ~DXEPTSIZ_XFERSIZE_MASK) | DXEPTSIZ_XFERSIZE(sz - amtDone);
                    }
                    if (sz == amtDone) {
                        s->diepctl(ep) &= ~DXEPCTL_EPENA;
                        s->diepint(ep) |= DXEPINT_XFERCOMPL;
                ...
                if (amtDone < pktsize && amtDone % mps == 0 && amtDone > 0) { p->status = USB_RET_ASYNC; }
                else {
        ...
        case USB_TOKEN_OUT:
            ...
            if (((s->doepctl(ep) & DXEPCTL_NAKSTS) != 0 && p->pid != USB_TOKEN_SETUP)
                || !(s->doepctl(ep) & DXEPCTL_USBACTEP) || (s->gintsts & GINTSTS_GOUTNAKEFF))
            {
                p->status = USB_RET_NAK;
                break;
            }
            if (s->doepctl(ep) & DXEPCTL_EPENA) {
                ...
                if (s->dcfg & DCFG_DESCDMA_EN) {
                    ...
                    dma_addr_t           residual;
                    qemu_sglist_init(&sglist, DEVICE(s), MAX_DMA_DESC_NUM_GENERIC, &s->dma_as);
                    while (dma_memory_read(&s->dma_as, s->doepdma(ep) | SOC_DMA_BASE, &desc, sizeof(desc),
                                           MEMTXATTRS_UNSPECIFIED)
                           == MEMTX_OK)
                    {
                        amtDone = 0;
                        ...
                        if (sglist.size + nbytes >= pktsize) {
                            ...
                            nbytes  -= pktsize - sglist.size;
                            if ((sglist.size + amtDone) % mps || (sglist.size + amtDone) == 0) {
                                desc.status |= DEV_DMA_SHORT;
                            }
                            if (p->pid == USB_TOKEN_SETUP) { desc.status |= DEV_DMA_SR; }
                            desc.status |= DEV_DMA_L;
                        }
                        else {
                            amtDone += nbytes;
                        ...
                        desc.status &= ~DEV_DMA_BUFF_STS_MASK;
                        desc.status |= DEV_DMA_BUFF_STS_DMADONE << DEV_DMA_BUFF_STS_SHIFT;
                        dma_memory_write(&s->dma_as, s->doepdma(ep) | SOC_DMA_BASE, &desc, sizeof(desc),
                                         MEMTXATTRS_UNSPECIFIED);
                        ioc |= (desc.status & DEV_DMA_IOC) != 0;
                ...
                qemu_log_mask(LOG_UNIMP, "%s: starting OUT transfer on EP %d (%zu/%d)...\n",
                    ...
                    dma_buf_read(buffer, sglist.size, &residual, &sglist, MEMTXATTRS_UNSPECIFIED);
                    amtDone = sglist.size - residual;
#if 0
                qemu_hexdump(stderr, __func__, buffer, sglist.size);
#endif
                    ...
                    qemu_log_mask(LOG_UNIMP, "%s: starting OUT transfer on EP %d (%d/%d/%zu/%d/%d)...\n", __func__, ep,
                                  amtDone, pktsize, p->iov.size, sz, pktcnt);
#endif
                    if (amtDone > 0) {
                        ...
                        if (s->doepdma(ep)) {
                            ...
                            dma_memory_write(&s->dma_as, s->doepdma(ep) | SOC_DMA_BASE, buffer, amtDone,
                                             MEMTXATTRS_UNSPECIFIED);
                    ...
                    if (ep == 0) {
                        if (p->pid != USB_TOKEN_SETUP) {
                            s->doeptsiz(ep) = (s->doeptsiz(ep) & ~DOEPTSIZ0_PKTCNT_MASK) | DOEPTSIZ0_PKTCNT(pktcnt);
                        }
                        s->doeptsiz(ep) =
                            (s->doeptsiz(ep) & ~DOEPTSIZ0_XFERSIZE_MASK) | DOEPTSIZ0_XFERSIZE(sz - amtDone);
                    }
                    else {
                        s->doeptsiz(ep) = (s->doeptsiz(ep) & ~DXEPTSIZ_PKTCNT_MASK) | DXEPTSIZ_PKTCNT(pktcnt);

                        s->doeptsiz(ep) = (s->doeptsiz(ep) & ~DXEPTSIZ_XFERSIZE_MASK) | DXEPTSIZ_XFERSIZE(sz - amtDone);
                    }
                }
                if (amtDone < pktsize && amtDone % mps == 0 && amtDone > 0) { p->status = USB_RET_ASYNC; }
                else {
                ...
                qemu_log_mask(LOG_UNIMP, "%s: SETUP {%02x,%02x,%04x,%04x,%04x}\n",
                    ...
                    if (setup.bmRequestType == 0 && setup.bRequest == USB_REQ_SET_ADDRESS && ep == 0) {
                ...
                if (ep == 0) { s->doepint(ep) |= DXEPINT_OUTTKNEPDIS; }
                p->status = USB_RET_ASYNC;
            }
            break;
        default: assert_not_reached(); break;
...
{
    USBPacket* p = NULL;
    if (unlikely(ep == NULL)) { return; }

    assert(bql_locked());
    if ((p = QTAILQ_FIRST(&ep->queue)) == NULL) { return; }
    if (p->state != USB_PACKET_ASYNC) { return; }

    dwc2_device_process_packet(s, p);

    if (p->status == USB_RET_NAK) { p->status = USB_RET_IOERROR; }
    if (p->status != USB_RET_ASYNC) { usb_packet_complete(USB_DEVICE(s->device), p); }
}
...
static void dwc2_device_work_bh(void* opaque)
{
    ...
    dwc2_device_process_async(s, usb_ep_get(USB_DEVICE(s->device), USB_TOKEN_SETUP, 0));

    for (int i = 1; i < DWC2_NB_EP; i++) {
        dwc2_device_process_async(s, usb_ep_get(USB_DEVICE(s->device), USB_TOKEN_OUT, i));
        dwc2_device_process_async(s, usb_ep_get(USB_DEVICE(s->device), USB_TOKEN_IN, i));
    }
}

static const char* dregnm[] = {"DCFG      ", "DCTL      ", "DSTS      ", "<rsvd>    ", "DIEPMSK   ",
                               "DOEPMSK   ", "DAINT     ", "DAINTMSK  ", "DTKNQR1   ", "DTKNQR2   ",
                               "DVBUSDIS  ", "DVBUSPULSE", "DTKNQR3   ", "DTKNQR4   "};
...
static void dwc2_dreg_write(void* ptr, hwaddr addr, int index, uint64_t val, unsigned size)
{
    DWC2State* s    = ptr;
    uint64_t   orig = val;
    uint32_t*  mmio;
    ...
    switch (addr) {
        ...
        case DCTL:
            ...
            if (!(s->dctl & DCTL_SFTDISCON) && (val & DCTL_SFTDISCON)) {
                /* go off bus */
                if (USB_DEVICE(s->device)->attached) { usb_device_detach(USB_DEVICE(s->device)); }
                pflg = 0;
            }
            iflg = 1;
            break;
    ...
    *mmio = val;
    ...
    if (iflg) {
        ...
        for (i = 0; i < DWC2_NB_EP; i++) { dwc2_update_ep_irq(s, i); }
        dwc2_update_irq(s);
    }

    if (pflg) { qemu_bh_schedule(s->device_async_bh); }
}

static const char* diepregnm[] = {"DIEPCTL ", "<rsvd>  ", "DIEPINT ", "<rsvd>   ",
                                  "DIEPTSIZ", "DIEPDMA ", "DTXFSTS ", "<rsvd>  "};

static uint64_t dwc2_diepreg_read(void* ptr, hwaddr addr, int index, unsigned size)
{
    DWC2State* s   = ptr;
    uint32_t   val = 0;
...
static void dwc2_diepreg_write(void* ptr, hwaddr addr, int index, uint64_t val, unsigned size)
{
    DWC2State* s    = ptr;
    uint64_t   orig = val;
    uint32_t*  mmio;
    ...
    switch (DIEPCTL0 + (addr & 0x1c)) {
        case DIEPCTL(0): {
            ...
            if ((index >> 3) != 0) { usb_ep_set_type(USB_DEVICE(s->device), USB_TOKEN_IN, index >> 3, eptype); }
    ...
    *mmio = val;
    ...
    if (uflg) { dwc2_update_in_ep(s, index >> 3); }

    if (pflg) { qemu_bh_schedule(s->device_async_bh); }

    if (iflg) { dwc2_update_ep_irq(s, index >> 3); }
    trace_usb_dwc2_diepreg_write(addr, diepregnm[index & 7], index >> 3, orig, old, val);
}

static const char* doepregnm[] = {"DOEPCTL ", "<rsvd>  ", "DOEPINT ", "<rsvd>   ",
                                  "DOEPTSIZ", "DOEPDMA ", "<rsvd>  ", "<rsvd>  "};

static uint64_t dwc2_doepreg_read(void* ptr, hwaddr addr, int index, unsigned size)
{
    DWC2State* s   = ptr;
    uint32_t   val = 0;
...
static void dwc2_doepreg_write(void* ptr, hwaddr addr, int index, uint64_t val, unsigned size)
{
    DWC2State* s    = ptr;
    uint64_t   orig = val;
    uint32_t*  mmio;
    ...
    switch (DOEPCTL0 + (addr & 0x1c)) {
        case DOEPCTL(0): {
            uint32_t eptype = (val & DXEPCTL_EPTYPE_MASK) >> DXEPCTL_EPTYPE_SHIFT;
            if ((index >> 3) != 0) { usb_ep_set_type(USB_DEVICE(s->device), USB_TOKEN_OUT, index >> 3, eptype); }
    ...
    *mmio = val;
    ...
    if (uflg) { dwc2_update_out_ep(s, index >> 3); }
...
static uint64_t dwc2_pcgreg_read(void* ptr, hwaddr addr, int index, unsigned size)
{
    DWC2State* s = ptr;
    uint32_t   val;
...
static void dwc2_pcgreg_write(void* ptr, hwaddr addr, int index, uint64_t val, unsigned size)
{
    DWC2State* s    = ptr;
    uint64_t   orig = val;
    uint32_t*  mmio;
...
static uint64_t dwc2_hsotg_read(void* ptr, hwaddr addr, unsigned size)
{
    ...
    switch (addr) {
        case HSOTG_REG(0x000)... HSOTG_REG(0x0fc):
            ...
            break;
        case HSOTG_REG(0x100): val = dwc2_fszreg_read(ptr, addr, (addr - HSOTG_REG(0x100)) >> 2, size); break;
...
static void dwc2_hsotg_write(void* ptr, hwaddr addr, uint64_t val, unsigned size)
{
    switch (addr) {
        case HSOTG_REG(0x000)... HSOTG_REG(0x0fc):
            dwc2_glbreg_write(ptr, addr, (addr - HSOTG_REG(0x000)) >> 2, val, size);
            break;
        case HSOTG_REG(0x100): dwc2_fszreg_write(ptr, addr, (addr - HSOTG_REG(0x100)) >> 2, val, size); break;
        case HSOTG_REG(0x104)... HSOTG_REG(0x3fc):
...
static void dwc2_wakeup_endpoint(USBBus* bus, USBEndpoint* ep, unsigned int stream)
{
...
static void dwc2_reset_enter(Object* obj, ResetType type)
{
    ...
    if (s->uport.dev && s->uport.dev->attached) { usb_detach(&s->uport); }
    ...
    s->ghwcfg1   = 0;
    s->ghwcfg2   = (8 << GHWCFG2_DEV_TOKEN_Q_DEPTH_SHIFT) | (4 << GHWCFG2_HOST_PERIO_TX_Q_DEPTH_SHIFT)
                   | (4 << GHWCFG2_NONPERIO_TX_Q_DEPTH_SHIFT) | GHWCFG2_DYNAMIC_FIFO | GHWCFG2_PERIO_EP_SUPPORTED
    ...
    s->ghwcfg3   = (4096 << GHWCFG3_DFIFO_DEPTH_SHIFT) | (4 << GHWCFG3_PACKET_SIZE_CNTR_WIDTH_SHIFT)
    ...
    for (i = 0; i < DWC2_NB_EP; i++) { s->dptxfsiz[i] = (0x100 << FIFOSIZE_DEPTH_SHIFT) | 0x100; }
    ...
    for (i = 0; i < DWC2_NB_CHAN; i++) { s->packet[i].needs_service = false; }
}
...
static void dwc2_realize(DeviceState* dev, Error** errp)
{
    ...
    usb_register_port(&s->bus, &s->uport, s, 0, &dwc2_port_ops,
                      USB_SPEED_MASK_LOW | USB_SPEED_MASK_FULL | (s->usb_version == 2 ? USB_SPEED_MASK_HIGH : 0));
    s->uport.dev = 0;
    ...
    s->async_bh        = qemu_bh_new_guarded(dwc2_work_bh, s, &dev->mem_reentrancy_guard);
    s->device_async_bh = qemu_bh_new_guarded(dwc2_device_work_bh, s, &dev->mem_reentrancy_guard);
...
static void dwc2_init(Object* obj)
{
    ...
    memory_region_init_io(&s->hsotg, obj, &dwc2_mmio_hsotg_ops, s, "dwc2-io", 4 * KiB);
    memory_region_add_subregion(&s->container, 0x0000, &s->hsotg);

    memory_region_init_io(&s->fifos, obj, &dwc2_mmio_fifo_ops, s, "dwc2-fifo", 64 * KiB);
...
static void dwc2_usb_device_cancel_packet(USBDevice* dev, USBPacket* p) { qemu_log_mask(LOG_UNIMP, "%s\n", __func__); }
...
static void dwc2_usb_device_class_initfn_common(ObjectClass* klass, const void* data)
{
