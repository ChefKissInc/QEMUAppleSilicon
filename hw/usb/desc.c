...
int usb_desc_device(const USBDescID* id, const USBDescDevice* dev, uint8_t* dest, size_t len)
{
    uint8_t        bLength = 0x12;
    USBDescriptor* d       = (void*)dest;

    if (len < bLength) { return -1; }
...
int usb_desc_device_qualifier(const USBDescDevice* dev, uint8_t* dest, size_t len)
{
...
int usb_desc_config(const USBDescConfig* conf, int flags, uint8_t* dest, size_t len)
{
    ...
    for (i = 0; i < conf->nif_groups; i++) {
        rc = usb_desc_iface_group(&(conf->if_groups[i]), flags, dest + wTotalLength, len - wTotalLength);
        if (rc < 0) { return rc; }
        wTotalLength += rc;
    }
    ...
    for (i = 0; i < conf->nif; i++) {
        rc = usb_desc_iface(conf->ifs + i, flags, dest + wTotalLength, len - wTotalLength);
        if (rc < 0) { return rc; }
        wTotalLength += rc;
    }
...
int usb_desc_iface_group(const USBDescIfaceAssoc* iad, int flags, uint8_t* dest, size_t len)
{
    int pos = 0;
    int i   = 0;
...
int usb_desc_iface(const USBDescIface* iface, int flags, uint8_t* dest, size_t len)
{
    uint8_t        bLength = 0x09;
    int            i, rc, pos = 0;
    USBDescriptor* d = (void*)dest;
...
int usb_desc_endpoint(const USBDescEndpoint* ep, int flags, uint8_t* dest, size_t len)
{
    ...
    if (len < bLength + extralen + superlen) { return -1; }
    ...
    if (superlen) {
        ...
        d->u.super_endpoint.wBytesPerInterval_lo = usb_lo(ep->wBytesPerInterval);
        d->u.super_endpoint.wBytesPerInterval_hi = usb_hi(ep->wBytesPerInterval);
    }

    if (ep->extra) { memcpy(dest + bLength + superlen, ep->extra, extralen); }
...
static int usb_desc_cap_super(const USBDesc* desc, uint8_t* dest, size_t len)
{
    ...
    if (desc->high) {
        ...
        if (!d->u.cap.u.super.bFunctionalitySupport) { d->u.cap.u.super.bFunctionalitySupport = 2; }
    }
    if (desc->super) {
        d->u.cap.u.super.wSpeedsSupported_lo |= (1 << 3);
        if (!d->u.cap.u.super.bFunctionalitySupport) { d->u.cap.u.super.bFunctionalitySupport = 3; }
    }
...
static int usb_desc_bos(const USBDesc* desc, uint8_t* dest, size_t len)
{
    ...
    if (desc->high != NULL) {
        rc = usb_desc_cap_usb2_ext(desc, dest + wTotalLength, len - wTotalLength);
        if (rc < 0) { return rc; }
        wTotalLength += rc;
        bNumDeviceCaps++;
    }
    ...
    if (desc->super != NULL) {
        rc = usb_desc_cap_super(desc, dest + wTotalLength, len - wTotalLength);
        if (rc < 0) { return rc; }
        wTotalLength += rc;
        bNumDeviceCaps++;
    }
...
static void usb_desc_ep_init(USBDevice* dev)
{
    ...
    for (i = 0; i < dev->ninterfaces; i++) {
        ...
        for (e = 0; e < iface->bNumEndpoints; e++) {
            ...
            usb_ep_set_max_packet_size(dev, pid, ep, iface->eps[e].wMaxPacketSize);
            usb_ep_set_max_streams(dev, pid, ep, iface->eps[e].bmAttributes_super);
        }
    }
}

static const USBDescIface* usb_desc_find_interface(USBDevice* dev, int nif, int alt)
{
    const USBDescIface* iface;
    int                 g, i;

    if (!dev->config) { return NULL; }
    for (g = 0; g < dev->config->nif_groups; g++) {
        for (i = 0; i < dev->config->if_groups[g].nif; i++) {
            iface = &dev->config->if_groups[g].ifs[i];
            if (iface->bInterfaceNumber == nif && iface->bAlternateSetting == alt) { return iface; }
        }
    }
...
static int usb_desc_set_interface(USBDevice* dev, int index, int value)
{
    ...
    if (old != value) { usb_device_set_interface(dev, index, old, value); }
    return 0;
}
...
static int usb_desc_set_config(USBDevice* dev, int value)
{
    ...
    if (value == 0) {
        ...
        if (i < dev->device->bNumConfigurations) { return -1; }
    }

    for (i = 0; i < dev->ninterfaces; i++) { usb_desc_set_interface(dev, i, 0); }
...
void usb_desc_init(USBDevice* dev)
{
    ...
    dev->speedmask = 0;
    if (desc->full) { dev->speedmask |= USB_SPEED_MASK_FULL; }
    if (desc->high) { dev->speedmask |= USB_SPEED_MASK_HIGH; }
    if (desc->super) { dev->speedmask |= USB_SPEED_MASK_SUPER; }
    usb_desc_setdefaults(dev);
}

void usb_desc_attach(USBDevice* dev) { usb_desc_setdefaults(dev); }
...
void usb_desc_create_serial(USBDevice* dev)
{
    ...
    if (path) { serial = g_strdup_printf("%s-%s-%s", desc->str[index], path, dev->port->path); }
    else {
...
int usb_desc_get_descriptor(USBDevice* dev, USBPacket* p, int value, uint8_t* dest, size_t len)
{
    const USBDesc*       desc = usb_device_get_usb_desc(dev);
    ...
    if (dev->speed == USB_SPEED_HIGH) { other_dev = usb_device_get_usb_desc(dev)->full; }
    else {
        other_dev = usb_device_get_usb_desc(dev)->high;
    }

    flags = 0;
    if (dev->device->bcdUSB >= 0x0300) { flags |= USB_DESC_FLAG_SUPER; }

    switch (type) {
        case USB_DT_DEVICE:
            ret = usb_desc_device(&desc->id, dev->device, buf, buflen);
            trace_usb_desc_device(dev->addr, len, ret);
            break;
        case USB_DT_CONFIG:
            if (index < dev->device->bNumConfigurations) {
                ret = usb_desc_config(dev->device->confs + index, flags, buf, buflen);
            }
        ...
        case USB_DT_DEVICE_QUALIFIER:
            if (other_dev != NULL) { ret = usb_desc_device_qualifier(other_dev, buf, buflen); }
            trace_usb_desc_device_qualifier(dev->addr, len, ret);
            break;
        case USB_DT_OTHER_SPEED_CONFIG:
            if (other_dev != NULL && index < other_dev->bNumConfigurations) {
                ret       = usb_desc_config(other_dev->confs + index, flags, buf, buflen);
        ...
        default: fprintf(stderr, "%s: %d unknown type %d (len %zd)\n", __func__, dev->addr, type, len); break;
    }
...
int usb_desc_handle_control(USBDevice* dev, USBPacket* p, int request, int value, int index, int length, uint8_t* data)
{
    const USBDesc* desc = usb_device_get_usb_desc(dev);
    int            ret  = -1;
    ...
    switch (request) {
        ...
        case DeviceRequest | USB_REQ_GET_DESCRIPTOR: ret = usb_desc_get_descriptor(dev, p, value, data, length); break;
        ...
        case DeviceRequest | USB_REQ_GET_STATUS: {
             ...
             */
            if (config->bmAttributes & USB_CFG_ATT_SELFPOWER) { data[0] |= 1 << USB_DEVICE_SELF_POWERED; }
            if (dev->remote_wakeup) { data[0] |= 1 << USB_DEVICE_REMOTE_WAKEUP; }
            data[1]          = 0x00;
            p->actual_length = 2;
            ret              = 0;
            break;
        ...
        case DeviceOutRequest | USB_REQ_SET_ISOCH_DELAY:
            if (dev->speed == USB_SPEED_SUPER) { ret = 0; }
            break;

        case InterfaceRequest | USB_REQ_GET_INTERFACE:
            if (index < 0 || index >= dev->ninterfaces) { break; }
