...
#define OHCI_BM(val, field) (((val) & OHCI_##field##_MASK) >> OHCI_##field##_SHIFT)
...
static const char* ohci_reg_names[] = {
    "HcRevision",         "HcControl",       "HcCommandStatus",   "HcInterruptStatus", "HcInterruptEnable",
    "HcInterruptDisable", "HcHCCA",          "HcPeriodCurrentED", "HcControlHeadED",   "HcControlCurrentED",
    "HcBulkHeadED",       "HcBulkCurrentED", "HcDoneHead",        "HcFmInterval",      "HcFmRemaining",
    "HcFmNumber",         "HcPeriodicStart", "HcLSThreshold",     "HcRhDescriptorA",   "HcRhDescriptorB",
    "HcRhStatus"};

static const char* ohci_reg_name(hwaddr addr)
{
    if (addr >> 2 < ARRAY_SIZE(ohci_reg_names)) { return ohci_reg_names[addr >> 2]; }
    else {
        return "<unknown>";
    }
}

static void ohci_die(OHCIState* ohci) { ohci->ohci_die(ohci); }
...
static USBDevice* ohci_find_device(OHCIState* ohci, uint8_t addr)
{
    ...
    for (i = 0; i < ohci->num_ports; i++) {
        if ((ohci->rhport[i].ctrl & OHCI_PORT_PES) == 0) { continue; }
...
static void ohci_roothub_reset(OHCIState* ohci)
{
    ...
    for (i = 0; i < ohci->num_ports; i++) {
        port       = &ohci->rhport[i];
        port->ctrl = 0;
        if (port->port.dev && port->port.dev->attached) { usb_port_reset(&port->port); }
    }
...
static inline int get_dwords(OHCIState* ohci, dma_addr_t addr, uint32_t* buf, int num)
{
    ...
    for (i = 0; i < num; i++, buf++, addr += sizeof(*buf)) {
        if (dma_memory_read(ohci->as, addr, buf, sizeof(*buf), MEMTXATTRS_UNSPECIFIED)) { return -1; }
        *buf = le32_to_cpu(*buf);
    }
...
static inline int put_dwords(OHCIState* ohci, dma_addr_t addr, uint32_t* buf, int num)
{
    ...
    for (i = 0; i < num; i++, buf++, addr += sizeof(*buf)) {
        ...
        if (dma_memory_write(ohci->as, addr, &tmp, sizeof(tmp), MEMTXATTRS_UNSPECIFIED)) { return -1; }
    }
...
{ return dma_memory_read(ohci->as, addr + ohci->localmem_base, hcca, sizeof(*hcca), MEMTXATTRS_UNSPECIFIED); }
...
{
    return dma_memory_write(ohci->as, addr + ohci->localmem_base + HCCA_WRITEBACK_OFFSET,
                            (char*)hcca + HCCA_WRITEBACK_OFFSET, HCCA_WRITEBACK_SIZE, MEMTXATTRS_UNSPECIFIED);
}
...
static int ohci_copy_td(OHCIState* ohci, struct ohci_td* td, uint8_t* buf, int len, DMADirection dir)
{
    ...
    n   = 0x1000 - (ptr & 0xfff);
    if (n > len) { n = len; }
    if (dma_memory_rw(ohci->as, ptr + ohci->localmem_base, buf, n, dir, MEMTXATTRS_UNSPECIFIED)) { return -1; }
    if (n == len) { return 0; }
    ptr  = td->be & ~0xfffu;
    buf += n;
    if (dma_memory_rw(ohci->as, ptr + ohci->localmem_base, buf, len - n, dir, MEMTXATTRS_UNSPECIFIED)) { return -1; }
    return 0;
}

/* Read/Write the contents of an ISO TD from/to main memory.  */
static int ohci_copy_iso_td(OHCIState* ohci, uint32_t start_addr, uint32_t end_addr, uint8_t* buf, int len,
                            DMADirection dir)
{
    dma_addr_t ptr, n;
...
static int ohci_service_iso_td(OHCIState* ohci, struct ohci_ed* ed)
{
    ...
    trace_usb_ohci_iso_td_head(ed->head & OHCI_DPTR_MASK, ed->tail & OHCI_DPTR_MASK, iso_td.flags, iso_td.bp,
                               iso_td.next, iso_td.be, ohci->frame_number, starting_frame, frame_count,
                               relative_frame_number);
    trace_usb_ohci_iso_td_head_offset(iso_td.offset[0], iso_td.offset[1], iso_td.offset[2], iso_td.offset[3],
    ...
    switch (dir) {
        ...
        case OHCI_TD_DIR_SETUP:
            ...
            pid = USB_TOKEN_SETUP;
            break;
        default: trace_usb_ohci_iso_td_bad_direction(dir); return 1;
    }
    ...
    if (!(OHCI_BM(start_offset, TD_PSW_CC) & 0xe)
        || ((relative_frame_number < frame_count) && !(OHCI_BM(next_offset, TD_PSW_CC) & 0xe)))
    {
    ...
    if ((start_offset & 0x1000) == 0) { start_addr = (iso_td.bp & OHCI_PAGE_MASK) | (start_offset & OHCI_OFFSET_MASK); }
    else {
    ...
    if (relative_frame_number < frame_count) {
        ...
        if ((end_offset & 0x1000) == 0) { end_addr = (iso_td.bp & OHCI_PAGE_MASK) | (end_offset & OHCI_OFFSET_MASK); }
        else {
    ...
    if (len && dir != OHCI_TD_DIR_IN) {
        if (ohci_copy_iso_td(ohci, start_addr, end_addr, buf, len, DMA_DIRECTION_TO_DEVICE)) {
            ohci_die(ohci);
            return 1;
        }
    }
    ...
    usb_packet_init(pkt);
    int_req = relative_frame_number == frame_count && OHCI_BM(iso_td.flags, TD_DI) == 0;
    ...
    if (pkt->status == USB_RET_SUCCESS) { ret = pkt->actual_length; }
    else {
        ret = pkt->status;
    }
    g_free(pkt);
    ...
    if (dir == OHCI_TD_DIR_IN && ret >= 0 && ret <= len) {
        ...
        if (ohci_copy_iso_td(ohci, start_addr, end_addr, buf, ret, DMA_DIRECTION_FROM_DEVICE)) {
            ohci_die(ohci);
            return 1;
        }
    ...
    if (ohci_put_iso_td(ohci, addr, &iso_td)) { ohci_die(ohci); }
    return 1;
}
...
static int ohci_service_td(OHCIState* ohci, struct ohci_ed* ed)
{
    ...
    switch (dir) {
        ...
        case OHCI_TD_DIR_IN:
            /* Same value. */
            break;
        default: dir = OHCI_BM(td.flags, TD_DP); break;
    }
    ...
    switch (dir) {
        ...
        case OHCI_TD_DIR_SETUP:
            ...
            break;
        default: trace_usb_ohci_td_bad_direction(dir); return 1;
    }
    if (td.cbp && td.be) {
        if ((td.cbp & 0xfffff000) != (td.be & 0xfffff000)) { len = (td.be & 0xfff) + 0x1001 - (td.cbp & 0xfff); }
        else {
        ...
        if (len > sizeof(ohci->usb_buf)) { len = sizeof(ohci->usb_buf); }

        pktlen = len;
        if (len && dir != OHCI_TD_DIR_IN) {
            /* The endpoint may not allow us to transfer it all now */
            pktlen = (ed->flags & OHCI_ED_MPS_MASK) >> OHCI_ED_MPS_SHIFT;
            if (pktlen > len) { pktlen = len; }
            if (!completion) {
                if (ohci_copy_td(ohci, &td, ohci->usb_buf, pktlen, DMA_DIRECTION_TO_DEVICE)) { ohci_die(ohci); }
            }
        }
    }
    ...
    if (ohci->usb_packet.status == USB_RET_SUCCESS) { ret = ohci->usb_packet.actual_length; }
    else {
    ...
    if (ret >= 0) {
        if (dir == OHCI_TD_DIR_IN) {
            if (ohci_copy_td(ohci, &td, ohci->usb_buf, ret, DMA_DIRECTION_FROM_DEVICE)) { ohci_die(ohci); }
    ...
    if (ret == pktlen || (dir == OHCI_TD_DIR_IN && ret >= 0 && flag_r)) {
        ...
        if (ret == len) { td.cbp = 0; }
            ...
            if ((td.cbp & 0xfff) + ret > 0xfff) { td.cbp = (td.be & ~0xfff) + ((td.cbp + ret) & 0xfff); }
            else {
                td.cbp += ret;
            }
        }
        ...
        if (ret >= 0) {
            ...
            switch (ret) {
                ...
                case USB_RET_NODEV:
                    ...
                    break;
                case USB_RET_NAK: trace_usb_ohci_td_nak(); return 1;
                case USB_RET_STALL:
...
static int ohci_service_ed_list(OHCIState* ohci, uint32_t head)
{
    ...
    for (cur = head; cur && link_cnt++ < ED_LINK_LIMIT; cur = next_ed) {
        ...
        while ((ed.head & OHCI_DPTR_MASK) != ed.tail) {
            trace_usb_ohci_ed_pkt(cur, (ed.head & OHCI_ED_H) != 0, (ed.head & OHCI_ED_C) != 0, ed.head & OHCI_DPTR_MASK,
                                  ed.tail & OHCI_DPTR_MASK, ed.next & OHCI_DPTR_MASK);
            trace_usb_ohci_ed_pkt_flags(OHCI_BM(ed.flags, ED_FA), OHCI_BM(ed.flags, ED_EN), OHCI_BM(ed.flags, ED_D),
                                        (ed.flags & OHCI_ED_S) != 0, (ed.flags & OHCI_ED_K) != 0,
                                        (ed.flags & OHCI_ED_F) != 0, OHCI_BM(ed.flags, ED_MPS));
            ...
            if ((ed.flags & OHCI_ED_F) == 0) {
                ...
                if (ohci_service_iso_td(ohci, &ed)) { break; }
            }
        }
...
/* set a timer for EOF */
static void ohci_eof_timer(OHCIState* ohci) { timer_mod(ohci->eof_timer, ohci->sof_time + usb_frame_time); }
...
static void ohci_frame_boundary(void* opaque)
{
    ...
    if (ohci->old_ctl & (~ohci->ctl) & (OHCI_CTL_BLE | OHCI_CTL_CLE)) { ohci_stop_endpoints(ohci); }
    ohci->old_ctl = ohci->ctl;
    ohci_process_lists(ohci);

    /* Stop if UnrecoverableError happened or ohci_sof will crash */
    if (ohci->intr_status & OHCI_INTR_UE) { return; }
    ...
    /* Writeback HCCA */
    if (ohci_put_hcca(ohci, ohci->hcca, &hcca)) { ohci_die(ohci); }
}
...
static void ohci_set_frame_interval(OHCIState* ohci, uint16_t val)
{
    ...
    if (val != ohci->fi) { trace_usb_ohci_set_frame_interval(ohci->name, ohci->fi, ohci->fi); }
...
static void ohci_set_ctl(OHCIState* ohci, uint32_t val)
{
    ...
    switch (new_state) {
        case OHCI_USB_OPERATIONAL: ohci_bus_start(ohci); break;
        case OHCI_USB_SUSPEND:
            ...
            ohci_intr_update(ohci);
            break;
        case OHCI_USB_RESUME: trace_usb_ohci_resume(ohci->name); break;
        case OHCI_USB_RESET : ohci_roothub_reset(ohci); break;
    }
}
...
static void ohci_set_hub_status(OHCIState* ohci, uint32_t val)
{
    ...
    if (val & OHCI_RHS_LPS) {
        ...
        for (i = 0; i < ohci->num_ports; i++) { ohci_port_power(ohci, i, 0); }
    ...
    if (val & OHCI_RHS_LPSC) {
        ...
        for (i = 0; i < ohci->num_ports; i++) { ohci_port_power(ohci, i, 1); }
...
static int ohci_port_set_if_connected(OHCIState* ohci, int i, uint32_t val)
{
    ...
    if (!(ohci->rhport[i].ctrl & OHCI_PORT_CCS)) {
        ...
        if (ohci->rhstatus & OHCI_RHS_DRWE) {
            ...
            if (ohci_resume(ohci)) { ohci_set_interrupt(ohci, OHCI_INTR_RD); }
        }
        return 0;
    }
...
static void ohci_port_set_status(OHCIState* ohci, int portnum, uint32_t val)
{
    ...
    if (ohci_port_set_if_connected(ohci, portnum, val & OHCI_PORT_PSS)) { trace_usb_ohci_port_suspend(portnum); }
...
static uint64_t ohci_mem_read(void* opaque, hwaddr addr, unsigned size)
{
    ...
    if (addr & 3) {
        ...
        trace_usb_ohci_mem_port_read(size, "HcRhPortStatus", (addr - 0x50) >> 2, addr, addr >> 2, retval);
    }
    else {
        switch (addr >> 2) {
            case 0: /* HcRevision */ retval = 0x10; break;

            case 1: /* HcControl */ retval = ohci->ctl; break;

            case 2: /* HcCommandStatus */ retval = ohci->status; break;

            case 3: /* HcInterruptStatus */ retval = ohci->intr_status; break;

            case 4: /* HcInterruptEnable */
            case 5: /* HcInterruptDisable */ retval = ohci->intr; break;

            case 6: /* HcHCCA */ retval = ohci->hcca; break;

            case 7: /* HcPeriodCurrentED */ retval = ohci->per_cur; break;

            case 8: /* HcControlHeadED */ retval = ohci->ctrl_head; break;

            case 9: /* HcControlCurrentED */ retval = ohci->ctrl_cur; break;

            case 10: /* HcBulkHeadED */ retval = ohci->bulk_head; break;

            case 11: /* HcBulkCurrentED */ retval = ohci->bulk_cur; break;

            case 12: /* HcDoneHead */ retval = ohci->done; break;

            case 13: /* HcFmInterretval */ retval = (ohci->fit << 31) | (ohci->fsmps << 16) | (ohci->fi); break;

            case 14: /* HcFmRemaining */ retval = ohci_get_frame_remaining(ohci); break;

            case 15: /* HcFmNumber */ retval = ohci->frame_number; break;

            case 16: /* HcPeriodicStart */ retval = ohci->pstart; break;

            case 17: /* HcLSThreshold */ retval = ohci->lst; break;

            case 18: /* HcRhDescriptorA */ retval = ohci->rhdesc_a; break;

            case 19: /* HcRhDescriptorB */ retval = ohci->rhdesc_b; break;

            case 20: /* HcRhStatus */ retval = ohci->rhstatus; break;

            /* PXA27x specific registers */
            case 24: /* HcStatus */ retval = ohci->hstatus & ohci->hmask; break;

            case 25: /* HcHReset */ retval = ohci->hreset; break;

            case 26: /* HcHInterruptEnable */ retval = ohci->hmask; break;

            case 27: /* HcHInterruptTest */ retval = ohci->htest; break;

            default: trace_usb_ohci_mem_read_bad_offset(addr); retval = 0xffffffff;
        }
        if (addr != 0xc || retval) { trace_usb_ohci_mem_read(size, ohci_reg_name(addr), addr, addr >> 2, retval); }
    }
...
static void ohci_mem_write(void* opaque, hwaddr addr, uint64_t val, unsigned size)
{
    ...
    if (addr >= 0x54 && addr < 0x54 + ohci->num_ports * 4) {
        /* HcRhPortStatus */
        trace_usb_ohci_mem_port_write(size, "HcRhPortStatus", (addr - 0x50) >> 2, addr, addr >> 2, val);
    ...
    switch (addr >> 2) {
        case 1: /* HcControl */ ohci_set_ctl(ohci, val); break;
        ...
        case 6: /* HcHCCA */ ohci->hcca = val & OHCI_HCCA_MASK; break;

        case 7: /* HcPeriodCurrentED */
            /* Ignore writes to this read-only register, Linux does them */
            break;

        case 8: /* HcControlHeadED */ ohci->ctrl_head = val & OHCI_EDPTR_MASK; break;

        case 9: /* HcControlCurrentED */ ohci->ctrl_cur = val & OHCI_EDPTR_MASK; break;

        case 10: /* HcBulkHeadED */ ohci->bulk_head = val & OHCI_EDPTR_MASK; break;

        case 11: /* HcBulkCurrentED */ ohci->bulk_cur = val & OHCI_EDPTR_MASK; break;
        ...
        case 15: /* HcFmNumber */ break;

        case 16: /* HcPeriodicStart */ ohci->pstart = val & 0xffff; break;

        case 17: /* HcLSThreshold */ ohci->lst = val & 0xffff; break;
        ...
        case 19: /* HcRhDescriptorB */ break;

        case 20: /* HcRhStatus */ ohci_set_hub_status(ohci, val); break;

        /* PXA27x specific registers */
        case 24: /* HcStatus */ ohci->hstatus &= ~(val & ohci->hmask); break;
        ...
        case 26: /* HcHInterruptEnable */ ohci->hmask = val; break;

        case 27: /* HcHInterruptTest */ ohci->htest = val; break;

        default: trace_usb_ohci_mem_write_bad_offset(addr); break;
    }
}
...
static void ohci_attach(USBPort* port1)
{
    ...
    /* update speed */
    if (port->port.dev->speed == USB_SPEED_LOW) { port->ctrl |= OHCI_PORT_LSDA; }
    else {
        port->ctrl &= ~OHCI_PORT_LSDA;
    }

    /* notify of remote-wakeup */
    if ((s->ctl & OHCI_CTL_HCFS) == OHCI_USB_SUSPEND) { ohci_set_interrupt(s, OHCI_INTR_RD); }

    trace_usb_ohci_port_attach(port1->index);

    if (old_state != port->ctrl) { ohci_set_interrupt(s, OHCI_INTR_RHSC); }
}
                   ...
                   uint32_t firstport, AddressSpace* as, void (*ohci_die_fn)(OHCIState*), Error** errp)
{
    ...
    if (num_ports > OHCI_MAX_PORTS) {
        error_setg(errp, "OHCI num-ports=%u is too big (limit is %u ports)", num_ports, OHCI_MAX_PORTS);
        return;
    }
    ...
    if (usb_frame_time == 0) {
        ...
        if (NANOSECONDS_PER_SECOND >= USB_HZ) { usb_bit_time = NANOSECONDS_PER_SECOND / USB_HZ; }
        else {
            usb_bit_time = 1;
        }
#endif
    ...
    if (masterbus) {
        ...
        for (i = 0; i < num_ports; i++) { ports[i] = &ohci->rhport[i].port; }
        usb_register_companion(masterbus, ports, num_ports, firstport, ohci, &ohci_port_ops,
                               USB_SPEED_MASK_LOW | USB_SPEED_MASK_FULL, &err);
        if (err) {
