...
static void do_usb_pcap_ctrl(FILE* fp, USBPacket* p, bool setup)
{
    ...
    if (data_len > CTRL_LEN) { data_len = CTRL_LEN; }
    if (setup) { memcpy(packet.s.setup, dev->setup_buf, 8); }
    else {
    ...
    if (data_len) { fwrite(dev->data_buf, data_len, 1, fp); }
...
static void do_usb_pcap_data(FILE* fp, USBPacket* p, bool setup)
{
    ...
    if (!setup) {
        packet.status = usbmon_status(p);
        if (packet.length > p->actual_length) { packet.length = p->actual_length; }
        if (data_len > p->actual_length) { data_len = p->actual_length; }
    }
