...
#define PTIMER_POLICY                                                                                            \
    (PTIMER_POLICY_WRAP_AFTER_ONE_PERIOD | PTIMER_POLICY_CONTINUOUS_TRIGGER | PTIMER_POLICY_NO_IMMEDIATE_TRIGGER \
     | PTIMER_POLICY_NO_IMMEDIATE_RELOAD | PTIMER_POLICY_NO_COUNTER_ROUND_DOWN)
...
static inline void timerblock_update_irq(TimerBlock* tb) { qemu_set_irq(tb->irq, tb->status && (tb->control & 4)); }

/* Return conversion factor from mpcore timer ticks to qemu timer ticks.  */
static inline uint32_t timerblock_scale(uint32_t control) { return (((control >> 8) & 0xff) + 1) * 10; }

/* Must be called within a ptimer transaction block */
static inline void timerblock_set_count(struct ptimer_state* timer, uint32_t control, uint64_t* count)
{
    /* PTimer would trigger interrupt for periodic timer when counter set
     * to 0, MPtimer under certain condition only.
     */
    if ((control & 3) == 3 && (control & 0xff00) == 0 && *count == 0) { *count = ptimer_get_limit(timer); }
    ptimer_set_count(timer, *count);
}

/* Must be called within a ptimer transaction block */
static inline void timerblock_run(struct ptimer_state* timer, uint32_t control, uint32_t load)
{
    if ((control & 1) && ((control & 0xff00) || load != 0)) { ptimer_run(timer, !(control & 2)); }
}
...
static void timerblock_tick(void* opaque)
{
     ...
     */
    if ((tb->control & 2) && (tb->control & 0xff00) == 0 && ptimer_get_limit(tb->timer) == 0) {
...
static uint64_t timerblock_read(void* opaque, hwaddr addr, unsigned size)
{
    TimerBlock* tb = (TimerBlock*)opaque;
    switch (addr) {
        case 0: /* Load */ return ptimer_get_limit(tb->timer);
        case 4: /* Counter.  */ return ptimer_get_count(tb->timer);
        case 8: /* Control.  */ return tb->control;
        case 12: /* Interrupt status.  */ return tb->status;
        default: return 0;
    }
}
...
static void timerblock_write(void* opaque, hwaddr addr, uint64_t value, unsigned size)
{
    ...
    switch (addr) {
        case 0: /* Load */
             ...
             * prescaler = 0.
             */
            if ((control & 1) && (control & 0xff00) == 0 && value == 0) { ptimer_stop(tb->timer); }
        ...
        case 8: /* Control.  */
            ptimer_transaction_begin(tb->timer);
            if ((control & 3) != (value & 3)) { ptimer_stop(tb->timer); }
            if ((control & 0xff00) != (value & 0xff00)) { ptimer_set_period(tb->timer, timerblock_scale(value)); }
            if (value & 1) {
                uint64_t count = ptimer_get_count(tb->timer);
                /* Re-load periodic timer counter if needed.  */
                if ((value & 2) && count == 0) { timerblock_set_count(tb->timer, value, &count); }
...
static void arm_mptimer_reset(DeviceState* dev)
{
    ...
    for (i = 0; i < ARRAY_SIZE(s->timerblock); i++) { timerblock_reset(&s->timerblock[i]); }
}
...
static void arm_mptimer_init(Object* obj)
{
    ...
    memory_region_init_io(&s->iomem, obj, &arm_thistimer_ops, s, "arm_mptimer_timer", 0x20);
...
static void arm_mptimer_realize(DeviceState* dev, Error** errp)
{
    ...
    if (s->num_cpu < 1 || s->num_cpu > ARM_MPTIMER_MAX_CPUS) {
        error_setg(errp, "num-cpu must be between 1 and %d", ARM_MPTIMER_MAX_CPUS);
        return;
    }
...
static void arm_mptimer_register_types(void) { type_register_static(&arm_mptimer_info); }
