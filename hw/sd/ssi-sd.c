...
static uint32_t ssi_sd_transfer(SSIPeripheral* dev, uint32_t val)
{
    ...
    if (!sdbus_get_inserted(&s->sdbus)) { return SSI_DUMMY; }
    ...
    switch (s->mode) {
        case SSI_SD_CMD:
            switch (val) {
                ...
                case SSI_TOKEN_STOP_TRAN:
                    ...
                    request.cmd = 12;
                    request.arg = 0;
                    s->arglen   = sdbus_do_command(&s->sdbus, &request, longresp, sizeof(longresp));
                    if (s->arglen == 0) {
                        s->arglen = 1;
        ...
        case SSI_SD_CMDARG:
            if (s->arglen == 4) {
                ...
                if (s->cmd == 28 || s->cmd == 29 || s->cmd == 38) { s->stopping = 1; }
        ...
        case SSI_SD_DATA_CRC16:
            ...
            if (s->response_pos == 2) {
                ...
                if (s->read_bytes == 512 && s->cmd != 17) { s->mode = SSI_SD_PREP_DATA; }
                else {
                    s->mode = SSI_SD_CMD;
                }
                s->read_bytes   = 0;
...
static int ssi_sd_post_load(void* opaque, int version_id)
{
    ...
    if (s->mode > SSI_SD_SKIP_CRC16) { return -EINVAL; }
    if (s->mode == SSI_SD_CMDARG && (s->arglen >= ARRAY_SIZE(s->cmdarg))) { return -EINVAL; }
