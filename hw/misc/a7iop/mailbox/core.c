...
static bool is_interrupt_enabled(AppleA7IOPMailbox* s, uint32_t status)
{
    ...
    if ((status & 0xF0000) == 0x10000 && (s->glb_cfg & KIC_GLB_CFG_EXT_INT_EN) != 0) {
        ...
        uint32_t interrupt_enabled = s->interrupts_enabled[interrupt_group] & BIT(interrupt % 32);
...
static bool apple_mbox_interrupt_status_empty(AppleA7IOPMailbox* s)
{
    ...
    QTAILQ_FOREACH (msg, &s->interrupt_status, entry) {
        if (is_interrupt_enabled(s, msg->status)) { return false; }
    }
...
void apple_a7iop_mailbox_update_irq_status(AppleA7IOPMailbox* s)
{
    ...
    trace_apple_a7iop_mailbox_update_irq(s->role, iop_empty, ap_empty, !iop_nonempty_unmasked, !iop_empty_unmasked,
                                         !ap_nonempty_unmasked, !ap_empty_unmasked);
    ...
    qemu_set_irq(s->irqs[APPLE_A7IOP_IRQ_IOP_NONEMPTY], (iop_nonempty_unmasked && !iop_empty) || iop_underflow);
    qemu_set_irq(s->irqs[APPLE_A7IOP_IRQ_IOP_EMPTY], iop_empty_unmasked && iop_empty);

    qemu_set_irq(s->irqs[APPLE_A7IOP_IRQ_AP_NONEMPTY], (ap_nonempty_unmasked && !ap_empty) || ap_underflow);
    qemu_set_irq(s->irqs[APPLE_A7IOP_IRQ_AP_EMPTY], ap_empty_unmasked && ap_empty);
...
void apple_a7iop_mailbox_update_irq(AppleA7IOPMailbox* s)
{
    ...
    if (!strcmp(s->role, "SEP-iop")) { qemu_set_irq(s->sep_cpu_irq, sep_cpu_irq_raised); }
    smp_mb();
    if (!strcmp(s->role, "SEP-ap")) { apple_a7iop_mailbox_update_irq(s->iop_mailbox); }
}
...
static void apple_a7iop_mailbox_send(AppleA7IOPMailbox* s, AppleA7IOPMessage* msg)
{
    assert_nonnull(msg);

    QEMU_LOCK_GUARD(&s->lock);

    trace_apple_a7iop_mailbox_send(s->role, ldq_le_p(msg->data), ldq_le_p(msg->data + sizeof(uint64_t)));

    QTAILQ_INSERT_TAIL(&s->inbox, msg, next);
    s->count++;
    apple_a7iop_mailbox_update_irq(s);

    if (s->handle_messages_bh != NULL) { qemu_bh_schedule(s->handle_messages_bh); }
}
...
AppleA7IOPMessage* apple_a7iop_inbox_peek(AppleA7IOPMailbox* s) { return QTAILQ_FIRST(&s->inbox); }

static AppleA7IOPMessage* apple_a7iop_mailbox_recv(AppleA7IOPMailbox* s)
{
    AppleA7IOPMessage* msg;

    if (s->underflow) { return NULL; }
    ...
    trace_apple_a7iop_mailbox_recv(s->role, ldq_le_p(msg->data), ldq_le_p(msg->data + sizeof(uint64_t)));
    s->count--;
    apple_a7iop_mailbox_update_irq(s);
...
{
    if (s->underflow) { return CTRL_UNDERFLOW; }

    return (s->count >= MAX_MESSAGE_COUNT ? CTRL_FULL : 0) | (QTAILQ_EMPTY(&s->inbox) ? CTRL_EMPTY : 0)
...
uint32_t apple_a7iop_mailbox_get_iop_ctrl(AppleA7IOPMailbox* s)
{
    ...
    return (s->iop_dir_en ? CTRL_ENABLE : 0) | apple_a7iop_mailbox_ctrl(s->iop_mailbox);
}
...
uint32_t apple_a7iop_mailbox_get_ap_ctrl(AppleA7IOPMailbox* s)
{
    ...
    return (s->ap_dir_en ? CTRL_ENABLE : 0) | apple_a7iop_mailbox_ctrl(s->ap_mailbox);
}
    ...
    QTAILQ_FOREACH (msg, &s->interrupt_status, entry) {
        ...
        if (is_interrupt_enabled(s, msg->status)) {
            if (preferred_msg == NULL
                || (interrupt_group_msg == interrupt_group_preferred_msg && msg->status < preferred_msg->status)
                || (interrupt_group_msg != interrupt_group_preferred_msg && interrupt_group_msg == 0x7))
            {
                preferred_msg                 = msg;
                                           ...
                                           QEMUBHFunc* handle_messages_func)
{
    ...
    for (i = 0; i < APPLE_A7IOP_IRQ_MAX; i++) { sysbus_init_irq(sbd, s->irqs + i); }
    ...
    switch (version) {
        case APPLE_A7IOP_V2: apple_a7iop_mailbox_init_mmio_v2(s, name); break;
        case APPLE_A7IOP_V4: apple_a7iop_mailbox_init_mmio_v4(s, name); break;
    }
...
static void apple_a7iop_mailbox_reset_enter(Object* obj, ResetType type)
{
    ...
    QTAILQ_FOREACH_SAFE (intr_status_msg, &s->interrupt_status, entry, intr_status_msg_next) {
        QTAILQ_REMOVE(&s->interrupt_status, intr_status_msg, entry);
        g_free(intr_status_msg);
    }

    for (i = 0; i < ARRAY_SIZE(s->interrupts_enabled); i++) { s->interrupts_enabled[i] = 0; }
