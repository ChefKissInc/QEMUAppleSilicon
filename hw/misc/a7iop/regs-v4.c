...
#define KIC_TMR_CFG_NMI          (KIC_TMR_CFG_FSL_SW | KIC_TMR_CFG_SMD_FIQ | KIC_TMR_CFG_IMD_FIQ | KIC_TMR_CFG_EN)
...
static void apple_a7iop_v4_reg_write(void* opaque, hwaddr addr, const uint64_t data, unsigned size)
{
    AppleA7IOP* s = opaque;
    ...
    switch (addr) {
        ...
        case REG_SEP_AKF_DISABLE_INTERRUPT_BASE + 0x0C:    // group 3
            ...
            {
                s->iop_mailbox->interrupts_enabled[(addr - REG_SEP_AKF_DISABLE_INTERRUPT_BASE) >> 2] &= ~data;
        ...
        case REG_SEP_AKF_ENABLE_INTERRUPT_BASE + 0x0C:    // group 3
            WITH_QEMU_LOCK_GUARD(&s->iop_mailbox->lock)
            {
                s->iop_mailbox->interrupts_enabled[(addr - REG_SEP_AKF_ENABLE_INTERRUPT_BASE) >> 2] |= data;
                apple_a7iop_mailbox_update_irq(s->iop_mailbox);
            }
            break;
        ...
        default: {
            qemu_log_mask(LOG_UNIMP,
                          "A7IOP_V4(%s): Unknown write to 0x" HWADDR_FMT_plx " of value 0x" HWADDR_FMT_plx "\n",
                          s->role, addr, data);
            break;
        }
    }
...
static uint64_t apple_a7iop_v4_reg_read(void* opaque, hwaddr addr, unsigned size)
{
    AppleA7IOP* s   = opaque;
    uint64_t    ret = 0;

    switch (addr) {
        case REG_CPU_CTRL  : ret = apple_a7iop_get_cpu_ctrl(s); break;
        case REG_CPU_STATUS: ret = apple_a7iop_get_cpu_status(s); break;
        case REG_UNKNOWN_4C:
        ...
        case REG_SEP_AKF_DISABLE_INTERRUPT_BASE + 0x0C:    // group 3
            ...
            ret = ~(s->iop_mailbox->interrupts_enabled[(addr - REG_SEP_AKF_DISABLE_INTERRUPT_BASE) >> 2]);
            break;
        case REG_SEP_AKF_ENABLE_INTERRUPT_BASE + 0x00:    // group 0
        case REG_SEP_AKF_ENABLE_INTERRUPT_BASE + 0x04:    // group 1
        case REG_SEP_AKF_ENABLE_INTERRUPT_BASE + 0x08:    // group 2
        case REG_SEP_AKF_ENABLE_INTERRUPT_BASE + 0x0C:    // group 3
            // shows disabled as 0b1, not enabled!
            ret = ~(s->iop_mailbox->interrupts_enabled[(addr - REG_SEP_AKF_ENABLE_INTERRUPT_BASE) >> 2]);
            break;
        case REG_KIC_GLB_CFG     : ret = s->iop_mailbox->glb_cfg; break;
        ...
        case REG_KIC_TMR0_EN          : ret = s->iop_mailbox->timer0_enabled; break;
        case REG_KIC_TMR1_EN          : ret = s->iop_mailbox->timer1_enabled; break;
        case REG_KIC_TMR0_INT_MASK_SET:
        case REG_KIC_TMR0_INT_MASK_CLR: ret = s->iop_mailbox->timer0_masked; break;
        case REG_KIC_TMR1_INT_MASK_SET:
        case REG_KIC_TMR1_INT_MASK_CLR: ret = s->iop_mailbox->timer1_masked; break;
        default                       : {
            qemu_log_mask(LOG_UNIMP, "A7IOP_V4(%s): Unknown read from 0x" HWADDR_FMT_plx "\n", s->role, addr);
            ret = 0;
            break;
        }
    }
...
void apple_a7iop_init_mmio_v4(AppleA7IOP* s, uint64_t mmio_size)
{
    ...
    memory_region_init_io(&s->mmio, OBJECT(s), &apple_a7iop_v4_reg_ops, s, name, mmio_size);
    sysbus_init_mmio(sbd, &s->mmio);

    memory_region_add_subregion_overlap(&s->mmio, AKF_STRIDE + AKF_MAILBOX_OFF, &s->iop_mailbox->mmio, 1);
    memory_region_add_subregion_overlap(&s->mmio, (AKF_STRIDE * 2) + AKF_MAILBOX_OFF, &s->ap_mailbox->mmio, 1);
}
