...
#define DPRINTF(fmt, ...)                                        \
    do {                                                         \
        if (DEBUG_AUX) { qemu_log("aux: " fmt, ##__VA_ARGS__); } \
    }                                                            \
    while (0)
...
AUXBus* aux_bus_init(DeviceState* parent, const char* name)
{
    ...
    auxtoi2c = object_new_with_props(TYPE_AUXTOI2C, OBJECT(bus), "i2c", &error_abort, NULL);
...
void aux_bus_realize(AUXBus* bus) { qdev_realize(DEVICE(bus->bridge), BUS(bus), &error_fatal); }
...
AUXReply aux_request(AUXBus* bus, AUXCommand cmd, uint32_t address, uint8_t len, uint8_t* data)
{
    ...
    switch (cmd) {
        ...
        case READ_AUX:
            for (i = 0; i < len; i++) {
                if (!address_space_rw(&bus->aux_addr_space, address++, MEMTXATTRS_UNSPECIFIED, data++, 1,
                                      cmd == WRITE_AUX))
                {
                    ret = AUX_I2C_ACK;
                }
         ...
         */
        case READ_I2C:
            if (i2c_bus_busy(i2c_bus)) { i2c_end_transfer(i2c_bus); }
            ...
            ret = AUX_I2C_ACK;
            for (i = 0; i < len; i++) { data[i] = i2c_recv(i2c_bus); }
        ...
        case WRITE_I2C_MOT:
            ...
            if (!i2c_bus_busy(i2c_bus)) {
                 ...
                 */
                if (i2c_start_send(i2c_bus, address)) { break; }
            }
            else if ((address != bus->last_i2c_address) || (bus->last_transaction != cmd)) {
                /*
        ...
        case READ_I2C_MOT:
            ...
            if (!i2c_bus_busy(i2c_bus)) {
                 ...
                 * No transactions started..
                 */
                if (i2c_start_recv(i2c_bus, address)) { break; }
            }
            else if ((address != bus->last_i2c_address) || (bus->last_transaction != cmd)) {
                /*
                 * Transaction started but we need to restart..
            ...
            ret = AUX_I2C_ACK;
            break;
        default: qemu_log_mask(LOG_UNIMP, "AUX cmd=%u not implemented\n", cmd); return AUX_NACK;
    }
...
static void aux_slave_dev_print(Monitor* mon, DeviceState* dev, int indent)
{
    ...
    if (aux_bus_is_bridge(bus, dev)) { return; }

    s = AUX_SLAVE(dev);

    monitor_printf(mon, "%*smemory " HWADDR_FMT_plx "/" HWADDR_FMT_plx "\n", indent, "",
                   object_property_get_uint(OBJECT(s->mmio), "addr", NULL), memory_region_size(s->mmio));
}
