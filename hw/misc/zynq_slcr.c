...
/* fields for [ARM|DDR|IO]_PLL_CTRL registers */
    REG_FIELD(xxx_PLL_CTRL, PLL_RESET, 0, 1)
    REG_FIELD(xxx_PLL_CTRL, PLL_PWRDWN, 1, 1)
    REG_FIELD(xxx_PLL_CTRL, PLL_BYPASS_QUAL, 3, 1)
    REG_FIELD(xxx_PLL_CTRL, PLL_BYPASS_FORCE, 4, 1)
    REG_FIELD(xxx_PLL_CTRL, PLL_FPDIV, 12, 7)
...
REG32(UART_CLK_CTRL, 0x154)
    REG_FIELD(UART_CLK_CTRL, CLKACT0, 0, 1)
    REG_FIELD(UART_CLK_CTRL, CLKACT1, 1, 1)
    REG_FIELD(UART_CLK_CTRL, SRCSEL,  4, 2)
    REG_FIELD(UART_CLK_CTRL, DIVISOR, 8, 6)
...
REG32(PSS_RST_CTRL, 0x200)
    REG_FIELD(PSS_RST_CTRL, SOFT_RST, 0, 1)
...
REG32(BOOT_MODE, 0x25c)
    REG_FIELD(BOOT_MODE, BOOT_MODE, 0, 4)
                          ...
                          uint64_t val, unsigned size)
{
    ...
    switch (offset) {
    case R_PSS_RST_CTRL:
        if (REG_FIELD_EX32(val, PSS_RST_CTRL, SOFT_RST)) {
