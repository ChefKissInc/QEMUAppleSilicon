...
static void apple_a7iop_v2_reg_write(void* opaque, hwaddr addr, const uint64_t data, unsigned size)
{
    AppleA7IOP* s = opaque;

    switch (addr) {
        case REG_CPU_CTRL  : apple_a7iop_set_cpu_ctrl(s, (uint32_t)data); break;
        case REG_CPU_STATUS: apple_a7iop_set_cpu_status(s, (uint32_t)data); break;
        default:
            qemu_log_mask(LOG_UNIMP,
                          "A7IOP_V2(%s): Unknown write to 0x" HWADDR_FMT_plx " of value 0x" HWADDR_FMT_plx "\n",
                          s->role, addr, data);
            break;
    }
}

static uint64_t apple_a7iop_v2_reg_read(void* opaque, hwaddr addr, unsigned size)
{
    AppleA7IOP* s = opaque;

    switch (addr) {
        case REG_CPU_CTRL  : return apple_a7iop_get_cpu_ctrl(s);
        case REG_CPU_STATUS: return apple_a7iop_get_cpu_status(s);
        default:
            qemu_log_mask(LOG_UNIMP, "A7IOP_V2(%s): Unknown read from 0x" HWADDR_FMT_plx "\n", s->role, addr);
            return 0;
    }
}
...
void apple_a7iop_init_mmio_v2(AppleA7IOP* s, uint64_t mmio_size)
{
    ...
    memory_region_init_io(&s->mmio, OBJECT(s), &apple_a7iop_v2_reg_ops, s, name, mmio_size);
    sysbus_init_mmio(sbd, &s->mmio);

    memory_region_add_subregion_overlap(&s->mmio, IOP_MAILBOX_REG_BASE, &s->iop_mailbox->mmio, 1);
    memory_region_add_subregion_overlap(&s->mmio, AKF_STRIDE, &s->ap_mailbox->mmio, 1);
}
