...
static void unimp_write(void* opaque, hwaddr offset, uint64_t value, unsigned size)
{
    UnimplementedDeviceState* s = UNIMPLEMENTED_DEVICE(opaque);

    qemu_log_mask(LOG_UNIMP,
                  "%s: unimplemented device write "
                  "(size %d, offset 0x%0*" HWADDR_PRIx ", value 0x%0*" PRIx64 ")\n",
...
static void unimp_realize(DeviceState* dev, Error** errp)
{
    ...
    memory_region_init_io(&s->iomem, OBJECT(s), &unimp_ops, s, s->name, s->size);
