...
bool ftrace_init(void)
{
    ...
    if (tracefs_found) {
        if (snprintf(path, PATH_MAX, "%s%s/tracing_on", mount_point, subdir) >= sizeof(path)) {
        ...
        if (trace_fd < 0) {
            if (errno == EACCES) {
                ...
                if (trace_marker_fd != -1) { return true; }
            }
        ...
        if (snprintf(path, PATH_MAX, "%s%s/trace_marker", mount_point, subdir) >= sizeof(path)) {
            fprintf(stderr, "Using tracefs mountpoint would exceed PATH_MAX\n");
            return false;
        }
