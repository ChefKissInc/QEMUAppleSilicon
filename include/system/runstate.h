...
VMChangeStateEntry*
     ...
     * Returns: an entry to be freed using qemu_del_vm_change_state_handler()
     */
    qemu_add_vm_change_state_handler_prio_full(VMChangeStateHandler* cb, VMChangeStateHandler* prepare_cb,
                                               VMChangeStateHandlerWithRet* cb_ret, void* opaque, int priority);
...
static inline bool shutdown_caused_by_guest(ShutdownCause cause) { return cause >= SHUTDOWN_CAUSE_GUEST_SHUTDOWN; }
