...
struct MachineClass
{
    ...
    bool                  (*hotplug_allowed)(MachineState* state, DeviceState* dev, Error** errp);
    ...
    bool                  (*create_default_memdev)(MachineState* ms, const char* path, Error** errp);
};
 ...
 */
#define _MACHINE_VER_STR2(major, minor) #major "." #minor

#define _MACHINE_VER_STR3(major, minor, micro) #major "." #minor "." #micro

#define _MACHINE_VER_STR4(major, minor, _unused_, tag) #major "." #minor "-" #tag

#define _MACHINE_VER_STR5(major, minor, micro, _unused_, tag) #major "." #minor "." #micro "-" #tag

#define MACHINE_VER_STR(...)                                                                \
    _MACHINE_VER_PICK(__VA_ARGS__, _MACHINE_VER_STR5, _MACHINE_VER_STR4, _MACHINE_VER_STR3, \
 ...
 */
#define _MACHINE_VER_TYPE_NAME2(prefix, major, minor) prefix "-" #major "." #minor TYPE_MACHINE_SUFFIX

#define _MACHINE_VER_TYPE_NAME3(prefix, major, minor, micro) prefix "-" #major "." #minor "." #micro TYPE_MACHINE_SUFFIX
...
#define MACHINE_VER_TYPE_NAME(prefix, ...)                                                                    \
    _MACHINE_VER_PICK(__VA_ARGS__, _MACHINE_VER_TYPE_NAME5, _MACHINE_VER_TYPE_NAME4, _MACHINE_VER_TYPE_NAME3, \
 ...
 */
#define _MACHINE_VER_SYM2(sym, prefix, major, minor) prefix##_machine_##major##_##minor##_##sym

#define _MACHINE_VER_SYM3(sym, prefix, major, minor, micro) prefix##_machine_##major##_##minor##_##micro##_##sym

#define _MACHINE_VER_SYM4(sym, prefix, major, minor, _unused_, tag) prefix##_machine_##major##_##minor##_##tag##_##sym

#define _MACHINE_VER_SYM5(sym, prefix, major, minor, micro, _unused_, tag) \
    prefix##_machine_##major##_##minor##_##micro##_##tag##_##sym

#define MACHINE_VER_SYM(sym, prefix, ...)                                                   \
    _MACHINE_VER_PICK(__VA_ARGS__, _MACHINE_VER_SYM5, _MACHINE_VER_SYM4, _MACHINE_VER_SYM3, \
...
#define MACHINE_VER_DEPRECATION_MSG                                                                         \
    "machines more than " stringify(MACHINE_VER_DEPRECATION_MAJOR) " years old are subject to deletion "    \
                                                                   "after " stringify(                      \
                                                                       MACHINE_VER_DELETION_MAJOR) " years"

#define _MACHINE_VER_IS_CURRENT_EXPIRED(cutoff, major, minor)                            \
    (((QEMU_VERSION_MAJOR - major) > cutoff)                                             \
     || (((QEMU_VERSION_MAJOR - major) == cutoff) && (QEMU_VERSION_MINOR - minor) >= 0))

#define _MACHINE_VER_IS_NEXT_MINOR_EXPIRED(cutoff, major, minor)                               \
    (((QEMU_VERSION_MAJOR - major) > cutoff)                                                   \
     || (((QEMU_VERSION_MAJOR - major) == cutoff) && ((QEMU_VERSION_MINOR + 1) - minor) >= 0))

#define _MACHINE_VER_IS_NEXT_MAJOR_EXPIRED(cutoff, major, minor)              \
    ((((QEMU_VERSION_MAJOR + 1) - major) > cutoff)                            \
     || ((((QEMU_VERSION_MAJOR + 1) - major) == cutoff) && (0 - minor) >= 0))
...
#define _MACHINE_VER_IS_EXPIRED_IMPL(cutoff, major, minor)                              \
    ((QEMU_VERSION_MICRO < 50 && _MACHINE_VER_IS_CURRENT_EXPIRED(cutoff, major, minor)) \
...
#define _MACHINE_VER_IS_EXPIRED2(cutoff, major, minor)               _MACHINE_VER_IS_EXPIRED_IMPL(cutoff, major, minor)
#define _MACHINE_VER_IS_EXPIRED3(cutoff, major, minor, micro)        _MACHINE_VER_IS_EXPIRED_IMPL(cutoff, major, minor)
#define _MACHINE_VER_IS_EXPIRED4(cutoff, major, minor, _unused, tag) _MACHINE_VER_IS_EXPIRED_IMPL(cutoff, major, minor)
#define _MACHINE_VER_IS_EXPIRED5(cutoff, major, minor, micro, _unused, tag) \
    _MACHINE_VER_IS_EXPIRED_IMPL(cutoff, major, minor)

#define _MACHINE_IS_EXPIRED(cutoff, ...)                                                                         \
    _MACHINE_VER_PICK(__VA_ARGS__, _MACHINE_VER_IS_EXPIRED5, _MACHINE_VER_IS_EXPIRED4, _MACHINE_VER_IS_EXPIRED3, \
 ...
 * lifecycle rules
 */
#define MACHINE_VER_IS_DEPRECATED(...) _MACHINE_IS_EXPIRED(MACHINE_VER_DEPRECATION_MAJOR, __VA_ARGS__)
 ...
 * lifecycle rules
 */
#define MACHINE_VER_SHOULD_DELETE(...) _MACHINE_IS_EXPIRED(MACHINE_VER_DELETION_MAJOR, __VA_ARGS__)
...
#define MACHINE_VER_DEPRECATION(...)                                                                          \
    do {                                                                                                      \
        if (MACHINE_VER_IS_DEPRECATED(__VA_ARGS__)) { mc->deprecation_reason = MACHINE_VER_DEPRECATION_MSG; } \
    }                                                                                                         \
    while (0)
...
#define MACHINE_VER_DELETION(...)                               \
    do {                                                        \
        if (MACHINE_VER_SHOULD_DELETE(__VA_ARGS__)) { return; } \
    }                                                           \
    while (0)
...
#define DEFINE_MACHINE(namestr, machine_initfn)                                                             \
    ...
    static void machine_initfn##_register_types(void) { type_register_static(&machine_initfn##_typeinfo); } \
