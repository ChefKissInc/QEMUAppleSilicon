...
#define PCI_SBDF(seg, bus, dev, func) ((((uint32_t)(seg)) << 16) | (PCI_BUILD_BDF(bus, PCI_DEVFN(dev, func))))
...
typedef void     PCIMapIORegionFunc(PCIDevice* pci_dev, int region_num, pcibus_t addr, pcibus_t size, int type);
...
typedef int  (*MSIVectorUseNotifier)(PCIDevice* dev, unsigned int vector, MSIMessage msg);
...
void          pci_default_write_config(PCIDevice* d, uint32_t address, uint32_t val, int len);
...
bool     pci_bus_add_fw_cfg_extra_pci_roots(FWCfgState* fw_cfg, PCIBus* bus, Error** errp);
/* 0 <= pin <= 3 0 = INTA, 1 = INTB, 2 = INTC, 3 = INTD */
static inline int pci_swizzle(int slot, int pin) { return (slot + pin) % PCI_NUM_PINS; }
...
bool pci_init_nic_in_slot(PCIBus* rootbus, const char* default_model, const char* alias, const char* devaddr);

static inline PCIBus* pci_get_bus(const PCIDevice* dev) { return PCI_BUS(qdev_get_parent_bus(DEVICE(dev))); }
int                   pci_bus_num(PCIBus* s);
void                  pci_bus_range(PCIBus* bus, int* min_bus, int* max_bus);
static inline int     pci_dev_bus_num(const PCIDevice* dev) { return pci_bus_num(pci_get_bus(dev)); }
...
typedef void (*IOMMUPRINotify)(struct IOMMUPRINotifier* notifier, IOMMUPRIResponse* response);
...
typedef struct PCIIOMMUOps
{
     ...
     */
    void (*init_iotlb_notifier)(PCIBus* bus, void* opaque, int devfn, IOMMUNotifier* n, IOMMUNotify fn,
                                void* user_opaque);
    /**
     ...
     */
    ssize_t (*ats_request_translation)(PCIBus* bus, void* opaque, int devfn, uint32_t pasid, bool priv_req,
                                       bool exec_req, hwaddr addr, size_t length, bool no_write, IOMMUTLBEntry* result,
                                       size_t result_length, uint32_t* err_count);
    /**
     ...
     */
    int (*pri_request_page)(PCIBus* bus, void* opaque, int devfn, uint32_t pasid, bool priv_req, bool exec_req,
                            hwaddr addr, bool lpig, uint16_t prgi, bool is_read, bool is_write);
} PCIIOMMUOps;
...
static inline void pci_set_byte(uint8_t* config, uint8_t val) { *config = val; }

static inline uint8_t pci_get_byte(const uint8_t* config) { return *config; }

static inline void pci_set_word(uint8_t* config, uint16_t val) { stw_le_p(config, val); }

static inline uint16_t pci_get_word(const uint8_t* config) { return lduw_le_p(config); }

static inline void pci_set_long(uint8_t* config, uint32_t val) { stl_le_p(config, val); }

static inline uint32_t pci_get_long(const uint8_t* config) { return ldl_le_p(config); }
 ...
 *
 */
static inline void pci_set_quad(uint8_t* config, uint64_t val) { stq_le_p(config, val); }

static inline uint64_t pci_get_quad(const uint8_t* config) { return ldq_le_p(config); }

static inline void pci_config_set_vendor_id(uint8_t* pci_config, uint16_t val)
{ pci_set_word(&pci_config[PCI_VENDOR_ID], val); }

static inline void pci_config_set_device_id(uint8_t* pci_config, uint16_t val)
{ pci_set_word(&pci_config[PCI_DEVICE_ID], val); }

static inline void pci_config_set_revision(uint8_t* pci_config, uint8_t val)
{ pci_set_byte(&pci_config[PCI_REVISION_ID], val); }

static inline void pci_config_set_class(uint8_t* pci_config, uint16_t val)
{ pci_set_word(&pci_config[PCI_CLASS_DEVICE], val); }

static inline void pci_config_set_prog_interface(uint8_t* pci_config, uint8_t val)
{ pci_set_byte(&pci_config[PCI_CLASS_PROG], val); }

static inline void pci_config_set_interrupt_pin(uint8_t* pci_config, uint8_t val)
 ...
 */
static inline uint8_t pci_byte_test_and_clear_mask(uint8_t* config, uint8_t mask)
{
...
static inline uint8_t pci_byte_test_and_set_mask(uint8_t* config, uint8_t mask)
{
    uint8_t val = pci_get_byte(config);
    pci_set_byte(config, val | mask);
    return val & mask;
...
static inline uint16_t pci_word_test_and_clear_mask(uint8_t* config, uint16_t mask)
{
...
static inline uint16_t pci_word_test_and_set_mask(uint8_t* config, uint16_t mask)
{
    uint16_t val = pci_get_word(config);
    pci_set_word(config, val | mask);
    return val & mask;
...
static inline uint32_t pci_long_test_and_clear_mask(uint8_t* config, uint32_t mask)
{
...
static inline uint32_t pci_long_test_and_set_mask(uint8_t* config, uint32_t mask)
{
    uint32_t val = pci_get_long(config);
    pci_set_long(config, val | mask);
    return val & mask;
...
static inline uint64_t pci_quad_test_and_clear_mask(uint8_t* config, uint64_t mask)
{
...
static inline uint64_t pci_quad_test_and_set_mask(uint8_t* config, uint64_t mask)
{
    uint64_t val = pci_get_quad(config);
    pci_set_quad(config, val | mask);
    return val & mask;
...
static inline void pci_set_byte_by_mask(uint8_t* config, uint8_t mask, uint8_t reg)
{
    uint8_t val = pci_get_byte(config);
    uint8_t rval;
...
static inline void pci_set_word_by_mask(uint8_t* config, uint16_t mask, uint16_t reg)
{
    uint16_t val = pci_get_word(config);
    uint16_t rval;
...
static inline void pci_set_long_by_mask(uint8_t* config, uint32_t mask, uint32_t reg)
{
    uint32_t val = pci_get_long(config);
    uint32_t rval;
...
static inline void pci_set_quad_by_mask(uint8_t* config, uint64_t mask, uint64_t reg)
{
    uint64_t val = pci_get_quad(config);
    uint64_t rval;
...
static inline void pci_irq_assert(PCIDevice* pci_dev) { pci_set_irq(pci_dev, 1); }

static inline void pci_irq_deassert(PCIDevice* pci_dev) { pci_set_irq(pci_dev, 0); }
