...
{
    if (small_nbits(nbits)) { *dst = ~(*src) & BITMAP_LAST_WORD_MASK(nbits); }
    else {
        slow_bitmap_complement(dst, src, nbits);
    }
}

static inline int bitmap_equal(const unsigned long* src1, const unsigned long* src2, long nbits)
{
    if (small_nbits(nbits)) { return !((*src1 ^ *src2) & BITMAP_LAST_WORD_MASK(nbits)); }
    else {
...
{
    if (small_nbits(nbits)) { return !(*src & BITMAP_LAST_WORD_MASK(nbits)); }
    else {
...
static inline int bitmap_intersects(const unsigned long* src1, const unsigned long* src2, long nbits)
{
    if (small_nbits(nbits)) { return ((*src1 & *src2) & BITMAP_LAST_WORD_MASK(nbits)) != 0; }
    else {
...
{
    if (unlikely(!nbits)) { return 0; }

    if (small_nbits(nbits)) { return ctpopl(*bitmap & BITMAP_LAST_WORD_MASK(nbits)); }
    else {
        return slow_bitmap_count_one(bitmap, nbits);
    }
}

static inline long bitmap_count_one_with_offset(const unsigned long* bitmap, long offset, long nbits)
{
    ...
    return bitmap_count_one(bitmap_start, bits_to_count) - bitmap_count_one(bitmap_start, redundant_bits);
}
...
static inline unsigned long* bitmap_zero_extend(unsigned long* old, long old_nbits, long new_nbits)
{
...
void bitmap_copy_with_src_offset(unsigned long* dst, const unsigned long* src, unsigned long offset,
                                 unsigned long nbits);
void bitmap_copy_with_dst_offset(unsigned long* dst, const unsigned long* src, unsigned long shift,
                                 unsigned long nbits);
