 ...
 */
    ...
    #define assert_cmphex(_expr, _op, _expected)                                                \
        assert_cmp((unsigned long long)(_expr), _op, "0x%llX", (unsigned long long)(_expected))
     ...
     */
    G_NORETURN
    void QEMU_ERROR("code path is reachable") qemu_build_not_reached_always(void);
...
#define qemu_build_assert(test)              \
    while (!(test)) qemu_build_not_reached()
...
#define MAX(a, b) MAX_INTERNAL((a), (b), MAKE_IDENTIFIER(_a), MAKE_IDENTIFIER(_b))
    ...
    #define MIN_CONST(a, b)                                                                                         \
        __builtin_choose_expr(__builtin_constant_p(a) && __builtin_constant_p(b), (a) < (b) ? (a) : (b), ((void)0))
...
#define MIN_NON_ZERO(a, b) MIN_NON_ZERO_INTERNAL((a), (b), MAKE_IDENTIFIER(_a), MAKE_IDENTIFIER(_b))
...
#define QEMU_IS_ARRAY(x) (!__builtin_types_compatible_p(typeof(x), typeof(&(x)[0])))
...
#else
    #if defined(__linux__) || defined(__sun__) || defined(__FreeBSD__) || defined(__NetBSD__) || defined(__OpenBSD__) \
        || defined(__DragonFly__) || defined(__GLIBC__) || defined(__APPLE__)
        #define HAVE_CHARDEV_SERIAL 1
    #endif
...
#if defined(__linux__) && (defined(__x86_64__) || defined(__arm__) || defined(__aarch64__) || defined(__powerpc64__))
     ...
     */
    #define QEMU_VMALLOC_ALIGN (qemu_real_host_page_size() * qemu_real_host_page_size() / sizeof(long))
#else
...
#ifdef _WIN32
    static inline void qemu_timersub(const struct timeval* val1, const struct timeval* val2, struct timeval* res)
    {
    ...
    ssize_t qemu_write_full(int fd, const void* buf, size_t count) G_GNUC_WARN_UNUSED_RESULT;
    ...
    static inline intptr_t qemu_real_host_page_mask(void) { return -(intptr_t)qemu_real_host_page_size(); }
