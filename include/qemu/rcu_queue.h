 ...
 */
#define QLIST_INSERT_AFTER_RCU(listelm, elm, field)                                                        \
    do {                                                                                                   \
        (elm)->field.le_next = (listelm)->field.le_next;                                                   \
        (elm)->field.le_prev = &(listelm)->field.le_next;                                                  \
        qatomic_rcu_set(&(listelm)->field.le_next, (elm));                                                 \
        if ((elm)->field.le_next != NULL) { (elm)->field.le_next->field.le_prev = &(elm)->field.le_next; } \
    }                                                                                                      \
    while (/*CONSTCOND*/ 0)
 ...
 */
#define QLIST_INSERT_BEFORE_RCU(listelm, elm, field)      \
    do {                                                  \
 ...
 */
#define QLIST_REMOVE_RCU(elm, field)                                                                      \
    do {                                                                                                  \
...
#define QLIST_FOREACH_RCU(var, head, field)                                                                   \
    for ((var) = qatomic_rcu_read(&(head)->lh_first); (var); (var) = qatomic_rcu_read(&(var)->field.le_next))

/* List traversal must occur within an RCU critical section.  */
#define QLIST_FOREACH_SAFE_RCU(var, head, field, next_var)                                       \
    for ((var) = (qatomic_rcu_read(&(head)->lh_first));                                          \
         (var) && ((next_var) = qatomic_rcu_read(&(var)->field.le_next), 1); (var) = (next_var))
...
#define QSIMPLEQ_EMPTY_RCU(head)      (qatomic_read(&(head)->sqh_first) == NULL)
...
#define QSIMPLEQ_INSERT_HEAD_RCU(head, elm, field)                                        \
    do {                                                                                  \
        (elm)->field.sqe_next = (head)->sqh_first;                                        \
        if ((elm)->field.sqe_next == NULL) { (head)->sqh_last = &(elm)->field.sqe_next; } \
        qatomic_rcu_set(&(head)->sqh_first, (elm));                                       \
    }                                                                                     \
    while (/*CONSTCOND*/ 0)

#define QSIMPLEQ_INSERT_TAIL_RCU(head, elm, field) \
    do {                                           \
...
#define QSIMPLEQ_INSERT_AFTER_RCU(head, listelm, elm, field)                              \
    do {                                                                                  \
...
#define QSIMPLEQ_REMOVE_HEAD_RCU(head, field)                                     \
    do {                                                                          \
        qatomic_set(&(head)->sqh_first, (head)->sqh_first->field.sqe_next);       \
        if ((head)->sqh_first == NULL) { (head)->sqh_last = &(head)->sqh_first; } \
    }                                                                             \
    while (/*CONSTCOND*/ 0)

#define QSIMPLEQ_REMOVE_RCU(head, elm, type, field)                                           \
    do {                                                                                      \
        if ((head)->sqh_first == (elm)) { QSIMPLEQ_REMOVE_HEAD_RCU((head), field); }          \
        else {                                                                                \
            struct type* curr = (head)->sqh_first;                                            \
            while (curr->field.sqe_next != (elm)) { curr = curr->field.sqe_next; }            \
            qatomic_set(&curr->field.sqe_next, curr->field.sqe_next->field.sqe_next);         \
            if (curr->field.sqe_next == NULL) { (head)->sqh_last = &(curr)->field.sqe_next; } \
        }                                                                                     \
    }                                                                                         \
    while (/*CONSTCOND*/ 0)

#define QSIMPLEQ_FOREACH_RCU(var, head, field)                                                                  \
    for ((var) = qatomic_rcu_read(&(head)->sqh_first); (var); (var) = qatomic_rcu_read(&(var)->field.sqe_next))

#define QSIMPLEQ_FOREACH_SAFE_RCU(var, head, field, next)                                                               \
    for ((var)                                                                  = qatomic_rcu_read(&(head)->sqh_first); \
         (var) && ((next) = qatomic_rcu_read(&(var)->field.sqe_next), 1); (var) = (next))
...
#define QTAILQ_INSERT_HEAD_RCU(head, elm, field)                                                                    \
    do {                                                                                                            \
        (elm)->field.tqe_next = (head)->tqh_first;                                                                  \
        if ((elm)->field.tqe_next != NULL) { (head)->tqh_first->field.tqe_circ.tql_prev = &(elm)->field.tqe_circ; } \
        else {                                                                                                      \
...
#define QTAILQ_INSERT_TAIL_RCU(head, elm, field)                      \
    do {                                                              \
...
#define QTAILQ_INSERT_AFTER_RCU(head, listelm, elm, field)                           \
    do {                                                                             \
...
#define QTAILQ_INSERT_BEFORE_RCU(listelm, elm, field)                          \
    do {                                                                       \
...
#define QTAILQ_REMOVE_RCU(head, elm, field)                                                  \
    do {                                                                                     \
        if (((elm)->field.tqe_next) != NULL) {                                               \
        ...
        qatomic_set(&(elm)->field.tqe_circ.tql_prev->tql_next, (elm)->field.tqe_next);       \
        (elm)->field.tqe_circ.tql_prev = NULL;                                               \
    }                                                                                        \
    while (/*CONSTCOND*/ 0)

#define QTAILQ_FOREACH_RCU(var, head, field)                                                                    \
    for ((var) = qatomic_rcu_read(&(head)->tqh_first); (var); (var) = qatomic_rcu_read(&(var)->field.tqe_next))

#define QTAILQ_FOREACH_SAFE_RCU(var, head, field, next)                                                                 \
    for ((var)                                                                  = qatomic_rcu_read(&(head)->tqh_first); \
         (var) && ((next) = qatomic_rcu_read(&(var)->field.tqe_next), 1); (var) = (next))
...
#define QSLIST_INSERT_HEAD_RCU(head, elm, field)    \
    do {                                            \
...
#define QSLIST_INSERT_AFTER_RCU(head, listelm, elm, field)  \
    do {                                                    \
...
#define QSLIST_REMOVE_HEAD_RCU(head, field)                                 \
    do {                                                                    \
        qatomic_set(&(head)->slh_first, (head)->slh_first->field.sle_next); \
    }                                                                       \
    while (/*CONSTCOND*/ 0)

#define QSLIST_REMOVE_RCU(head, elm, type, field)                                     \
    do {                                                                              \
        if ((head)->slh_first == (elm)) { QSLIST_REMOVE_HEAD_RCU((head), field); }    \
        else {                                                                        \
            struct type* curr = (head)->slh_first;                                    \
            while (curr->field.sle_next != (elm)) { curr = curr->field.sle_next; }    \
            qatomic_set(&curr->field.sle_next, curr->field.sle_next->field.sle_next); \
        }                                                                             \
    }                                                                                 \
    while (/*CONSTCOND*/ 0)

#define QSLIST_FOREACH_RCU(var, head, field)                                                                    \
    for ((var) = qatomic_rcu_read(&(head)->slh_first); (var); (var) = qatomic_rcu_read(&(var)->field.sle_next))

#define QSLIST_FOREACH_SAFE_RCU(var, head, field, next)                                                                 \
    for ((var)                                                                  = qatomic_rcu_read(&(head)->slh_first); \
         (var) && ((next) = qatomic_rcu_read(&(var)->field.sle_next), 1); (var) = (next))
