...
static inline uint64_t stat64_get(const Stat64* s) { return qatomic_read__nocheck(&s->value); }

static inline void stat64_set(Stat64* s, uint64_t value) { qatomic_set__nocheck(&s->value, value); }

static inline void stat64_add(Stat64* s, uint64_t value) { qatomic_add(&s->value, value); }

static inline void stat64_min(Stat64* s, uint64_t value)
{
    uint64_t orig = qatomic_read__nocheck(&s->value);
    while (orig > value) { orig = qatomic_cmpxchg__nocheck(&s->value, orig, value); }
}
...
static inline void stat64_add(Stat64* s, uint64_t value)
{
    ...
    low  = (uint32_t)value;
    if (!low) {
        if (high) { qatomic_add(&s->high, high); }
        return;
    }
    ...
    for (;;) {
        ...
        if (result < low || high) {
            ...
            if (stat64_add32_carry(s, low, high)) { return; }
            continue;
        }
