...
static inline bool range_contains(const Range* range, uint64_t val) { return val >= range->lob && val <= range->upb; }
 ...
 */
static inline void range_set_bounds1(Range* range, uint64_t lob, uint64_t upb_plus1)
{
    if (!lob && !upb_plus1) { *range = range_empty; }
    else {
        range->lob = lob;
 ...
 */
static inline uint64_t range_size(const Range* range) { return range->upb - range->lob + 1; }

/*
 * Check if @range1 overlaps with @range2. If one of the ranges is empty,
 * the result is always "false".
 */
static inline bool range_overlaps_range(const Range* range1, const Range* range2)
{
    if (range_is_empty(range1) || range_is_empty(range2)) { return false; }
 ...
 * the result is always "false".
 */
static inline bool range_contains_range(const Range* range1, const Range* range2)
{
    if (range_is_empty(range1) || range_is_empty(range2)) { return false; }
...
{
    if (range_is_empty(extend_by)) { return; }
    if (range_is_empty(range)) {
        *range = *extend_by;
        return;
    }
    if (range->lob > extend_by->lob) { range->lob = extend_by->lob; }
    if (range->upb < extend_by->upb) { range->upb = extend_by->upb; }
    range_invariant(range);
}

/* Get last byte of a range from offset + length.
 * Undefined for ranges that wrap around 0. */
static inline uint64_t range_get_last(uint64_t offset, uint64_t len) { return offset + len - 1; }

/* Check whether a given range covers a given byte. */
static inline int range_covers_byte(uint64_t offset, uint64_t len, uint64_t byte)
{ return offset <= byte && byte <= range_get_last(offset, len); }

/* Check whether 2 given ranges overlap.
 * Undefined if ranges that wrap around 0. */
static inline bool ranges_overlap(uint64_t first1, uint64_t len1, uint64_t first2, uint64_t len2)
{
...
{
    if (range_is_empty(range)) { return -1; }
 ...
 */
void range_inverse_array(GList* in_ranges, GList** out_ranges, uint64_t low, uint64_t high);
