...
static inline size_t iov_from_buf(const struct iovec* iov, unsigned int iov_cnt, size_t offset, const void* buf,
                                  size_t bytes)
{
...
static inline size_t iov_to_buf(const struct iovec* iov, const unsigned int iov_cnt, size_t offset, void* buf,
                                size_t bytes)
{
    if (__builtin_constant_p(bytes) && iov_cnt && offset <= iov[0].iov_len && bytes <= iov[0].iov_len - offset) {
 ...
 */
ssize_t iov_send_recv_with_flags(int sockfd, int sockflags, const struct iovec* iov, unsigned iov_cnt, size_t offset,
...
QEMU_BUILD_BUG_ON(offsetof(QEMUIOVector, size) != offsetof(QEMUIOVector, local_iov.iov_len));
 ...
 */
static inline void qemu_iovec_init_buf(QEMUIOVector* qiov, const void* buf, size_t len)
