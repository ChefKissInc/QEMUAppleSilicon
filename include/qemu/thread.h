...
int TSA_NO_TSA  qemu_mutex_trylock_impl(QemuMutex* mutex, const char* file, const int line);
void TSA_NO_TSA qemu_mutex_lock_impl(QemuMutex* mutex, const char* file, const int line);
void TSA_NO_TSA qemu_mutex_unlock_impl(QemuMutex* mutex, const char* file, const int line);
...
typedef bool (*QemuCondTimedWaitFunc)(QemuCond* c, QemuMutex* m, int ms, const char* f, int l);
...
#define qemu_mutex_trylock__raw(m) qemu_mutex_trylock_impl(m, __FILE__, __LINE__)
    ...
    #define qemu_rec_mutex_trylock(m)     qemu_rec_mutex_trylock_impl(m, __FILE__, __LINE__)
    #define qemu_cond_wait(c, m)          qemu_cond_wait_impl(c, m, __FILE__, __LINE__)
    #define qemu_cond_timedwait(c, m, ms) qemu_cond_timedwait_impl(c, m, ms, __FILE__, __LINE__)
#else
...
#define qemu_rec_mutex_unlock(mutex) qemu_rec_mutex_unlock_impl(mutex, __FILE__, __LINE__)

static inline void(qemu_mutex_lock)(QemuMutex* mutex) { qemu_mutex_lock(mutex); }

static inline int(qemu_mutex_trylock)(QemuMutex* mutex) { return qemu_mutex_trylock(mutex); }

static inline void(qemu_mutex_unlock)(QemuMutex* mutex) { qemu_mutex_unlock(mutex); }

static inline void(qemu_rec_mutex_lock)(QemuRecMutex* mutex) { qemu_rec_mutex_lock(mutex); }

static inline int(qemu_rec_mutex_trylock)(QemuRecMutex* mutex) { return qemu_rec_mutex_trylock(mutex); }

static inline void(qemu_rec_mutex_unlock)(QemuRecMutex* mutex) { qemu_rec_mutex_unlock(mutex); }
...
void TSA_NO_TSA qemu_cond_wait_impl(QemuCond* cond, QemuMutex* mutex, const char* file, const int line);
bool            qemu_cond_timedwait_impl(QemuCond* cond, QemuMutex* mutex, int ms, const char* file, const int line);

static inline void(qemu_cond_wait)(QemuCond* cond, QemuMutex* mutex) { qemu_cond_wait(cond, mutex); }

/* Returns true if timeout has not expired, and false otherwise */
static inline bool(qemu_cond_timedwait)(QemuCond* cond, QemuMutex* mutex, int ms)
...
static inline void qemu_spin_lock(QemuSpin* spin)
{
    ...
    while (unlikely(qatomic_xchg(&spin->value, 1))) {
        while (qatomic_read(&spin->value)) { cpu_relax(); }
    }
#ifdef CONFIG_TSAN
...
static inline bool qemu_spin_locked(QemuSpin* spin) { return qatomic_read(&spin->value); }
