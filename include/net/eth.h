...
#define PKT_GET_VLAN_HDR(p)   ((struct vlan_header*)(((uint8_t*)(p)) + sizeof(struct eth_header)))
#define PKT_GET_DVLAN_HDR(p)  (PKT_GET_VLAN_HDR(p) + 1)
#define PKT_GET_IP_HDR(p)     ((struct ip_header*)(((uint8_t*)(p)) + eth_get_l2_hdr_length(p)))
#define IP_HDR_GET_LEN(p)     ((ldub_p(p + offsetof(struct ip_header, ip_ver_len)) & 0x0F) << 2)
#define IP_HDR_GET_P(p)       (ldub_p(p + offsetof(struct ip_header, ip_p)))
#define PKT_GET_IP_HDR_LEN(p) (IP_HDR_GET_LEN(PKT_GET_IP_HDR(p)))
#define PKT_GET_IP6_HDR(p)    ((struct ip6_header*)(((uint8_t*)(p)) + eth_get_l2_hdr_length(p)))
#define IP_HEADER_VERSION(ip) (((ip)->ip_ver_len >> 4) & 0xf)
#define IP4_IS_FRAGMENT(ip)   ((be16_to_cpu((ip)->ip_off) & (IP_OFFMASK | IP_MF)) != 0)
...
#define IS_SPECIAL_VLAN_ID(x) (((x) == 0) || ((x) == 0xFFF))

#define ETH_MAX_L2_HDR_LEN (sizeof(struct eth_header) + 2 * sizeof(struct vlan_header))
...
#define IP6_EXT_GRANULARITY          \
    (8) /* Size granularity for      \
...
static inline int is_multicast_ether_addr(const uint8_t* addr) { return 0x01 & addr[0]; }

static inline int is_broadcast_ether_addr(const uint8_t* addr)
{ return (addr[0] & addr[1] & addr[2] & addr[3] & addr[4] & addr[5]) == 0xff; }

static inline int is_unicast_ether_addr(const uint8_t* addr) { return !is_multicast_ether_addr(addr); }
...
static inline eth_pkt_types_e get_eth_packet_type(const struct eth_header* ehdr)
{
    if (is_broadcast_ether_addr(ehdr->h_dest)) { return ETH_PKT_BCAST; }
    else if (is_multicast_ether_addr(ehdr->h_dest)) {
        return ETH_PKT_MCAST;
    }
    else { /* unicast */ return ETH_PKT_UCAST;
    }
}

static inline uint32_t eth_get_l2_hdr_length(const void* p)
{
    uint16_t            proto = be16_to_cpu(PKT_GET_ETH_HDR(p)->h_proto);
    struct vlan_header* hvlan = PKT_GET_VLAN_HDR(p);
    switch (proto) {
        case ETH_P_VLAN: return sizeof(struct eth_header) + sizeof(struct vlan_header);
        case ETH_P_DVLAN:
        ...
        default: return sizeof(struct eth_header);
    }
}

static inline uint32_t eth_get_l2_hdr_length_iov(const struct iovec* iov, size_t iovcnt, size_t iovoff)
{
    uint8_t p[sizeof(struct eth_header) + sizeof(struct vlan_header)];
    size_t  copied = iov_to_buf(iov, iovcnt, iovoff, p, ARRAY_SIZE(p));

    if (copied < ARRAY_SIZE(p)) { return copied; }

    return eth_get_l2_hdr_length(p);
}

static inline uint16_t eth_get_pkt_tci(const void* p)
{
    uint16_t            proto = be16_to_cpu(PKT_GET_ETH_HDR(p)->h_proto);
    struct vlan_header* hvlan = PKT_GET_VLAN_HDR(p);
    switch (proto) {
