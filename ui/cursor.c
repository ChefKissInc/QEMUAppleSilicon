...
static QEMUCursor* cursor_parse_xpm(const char* xpm[])
{
    ...
    if (sscanf(xpm[line], "%u %u %u %u", &width, &height, &colors, &chars) != 4) {
...
void cursor_print_ascii_art(QEMUCursor* c, const char* prefix)
{
    ...
    for (y = 0; y < c->height; y++) {
        ...
        for (x = 0; x < c->width; x++, data++) {
            if ((*data & 0xff000000) != 0xff000000) { fprintf(stderr, " "); /* transparent */ }
...
QEMUCursor* cursor_builtin_left_ptr(void) { return cursor_parse_xpm(cursor_left_ptr_xpm); }
...
int cursor_get_mono_bpl(QEMUCursor* c) { return DIV_ROUND_UP(c->width, 8); }
                     ...
                     uint8_t* mask)
{
    ...
    if (has_inverted_colors) {
        ...
        for (y = 0; y < c->height; y++) {
            for (x = 0; x < c->width; x++, data++) {
                if (*data == 0 /* transparent */
                    && ((x > 0 && data[-1] == inverted) || (x + 1 < c->width && data[1] == inverted)
                        || (y > 0 && data[-c->width] == inverted) || (y + 1 < c->height && data[c->width] == inverted)))
                {
                    *data = 0xff000000 | background;
                }
            }
        ...
        for (x = 0; x < c->width * c->height; x++, data++) {
            if (*data == inverted) { *data = 0xff000000 | foreground; }
        }
    }
}
