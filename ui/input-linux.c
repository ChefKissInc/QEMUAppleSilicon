...
{
    if (lnx < 0x100) { return false; }
    if (lnx >= 0x160 && lnx < 0x2c0) { return false; }
    return true;
}
...
static void input_linux_toggle_grab(InputLinux* il)
{
    ...
    if (!il->grab_all) { return; }
    QTAILQ_FOREACH (item, &inputs, next) {
        if (item == il || item->grab_all) {
            /* avoid endless loops */
            continue;
        }
        if (item->grab_active != il->grab_active) { input_linux_toggle_grab(item); }
    }
}
...
static bool input_linux_check_toggle(InputLinux* il)
{
    switch (il->grab_toggle) {
        case GRAB_TOGGLE_KEYS_CTRL_CTRL: return il->keydown[KEY_LEFTCTRL] && il->keydown[KEY_RIGHTCTRL];

        case GRAB_TOGGLE_KEYS_ALT_ALT: return il->keydown[KEY_LEFTALT] && il->keydown[KEY_RIGHTALT];

        case GRAB_TOGGLE_KEYS_SHIFT_SHIFT: return il->keydown[KEY_LEFTSHIFT] && il->keydown[KEY_RIGHTSHIFT];

        case GRAB_TOGGLE_KEYS_META_META: return il->keydown[KEY_LEFTMETA] && il->keydown[KEY_RIGHTMETA];

        case GRAB_TOGGLE_KEYS_SCROLLLOCK: return il->keydown[KEY_SCROLLLOCK];

        case GRAB_TOGGLE_KEYS_CTRL_SCROLLLOCK:
            return (il->keydown[KEY_LEFTCTRL] || il->keydown[KEY_RIGHTCTRL]) && il->keydown[KEY_SCROLLLOCK];
...
static bool input_linux_should_skip(InputLinux* il, struct input_event* event)
{
    return (il->grab_toggle == GRAB_TOGGLE_KEYS_SCROLLLOCK || il->grab_toggle == GRAB_TOGGLE_KEYS_CTRL_SCROLLLOCK)
           && event->code == KEY_SCROLLLOCK;
}

static void input_linux_handle_keyboard(InputLinux* il, struct input_event* event)
{
    if (event->type == EV_KEY) {
        ...
        if (input_linux_check_toggle(il)) { il->grab_request = true; }
...
{
    if (!il->grab_active) { return; }

    switch (event->type) {
        case EV_KEY:
            switch (event->code) {
                case BTN_LEFT     : qemu_input_queue_btn(NULL, INPUT_BUTTON_LEFT, event->value); break;
                case BTN_RIGHT    : qemu_input_queue_btn(NULL, INPUT_BUTTON_RIGHT, event->value); break;
                case BTN_MIDDLE   : qemu_input_queue_btn(NULL, INPUT_BUTTON_MIDDLE, event->value); break;
                case BTN_GEAR_UP  : qemu_input_queue_btn(NULL, INPUT_BUTTON_WHEEL_UP, event->value); break;
                case BTN_GEAR_DOWN: qemu_input_queue_btn(NULL, INPUT_BUTTON_WHEEL_DOWN, event->value); break;
                case BTN_SIDE     : qemu_input_queue_btn(NULL, INPUT_BUTTON_SIDE, event->value); break;
                case BTN_EXTRA    : qemu_input_queue_btn(NULL, INPUT_BUTTON_EXTRA, event->value); break;
            };
            break;
        case EV_REL:
            switch (event->code) {
                case REL_X    : qemu_input_queue_rel(NULL, INPUT_AXIS_X, event->value); break;
                case REL_Y    : qemu_input_queue_rel(NULL, INPUT_AXIS_Y, event->value); break;
                case REL_WHEEL: il->wheel = event->value; break;
            }
            break;
        case EV_ABS:
            switch (event->code) {
                case ABS_X: qemu_input_queue_abs(NULL, INPUT_AXIS_X, event->value, il->abs_x_min, il->abs_x_max); break;
                case ABS_Y: qemu_input_queue_abs(NULL, INPUT_AXIS_Y, event->value, il->abs_y_min, il->abs_y_max); break;
            }
            break;
        case EV_SYN:
            qemu_input_event_sync();
            if (il->wheel != 0) {
                input_linux_event_mouse_button((il->wheel > 0) ? INPUT_BUTTON_WHEEL_UP : INPUT_BUTTON_WHEEL_DOWN);
                il->wheel = 0;
            }
            break;
    }
...
static void input_linux_event(void* opaque)
{
    ...
    for (;;) {
        ...
        if (il->num_keys) { input_linux_handle_keyboard(il, &il->event); }
...
static void input_linux_complete(UserCreatable* uc, Error** errp)
{
    ...
    if (!qemu_set_blocking(il->fd, false, errp)) { return; }
    ...
    if (rc < 0) { goto err_read_event_bits; }

    if (evtmap & (1 << EV_REL)) {
        relmap = 0;
        rc     = ioctl(il->fd, EVIOCGBIT(EV_REL, sizeof(relmap)), &relmap);
        if (rc < 0) { goto err_read_event_bits; }
        if (relmap & (1 << REL_X)) { il->has_rel_x = true; }
    }
    ...
    if (evtmap & (1 << EV_ABS)) {
        ...
        if (absmap & (1 << ABS_X)) {
            ...
            if (rc < 0) {
                error_setg(errp, "%s: failed to get get absolute X value", il->evdev);
                goto err_close;
            }
            ...
            if (rc < 0) {
                error_setg(errp, "%s: failed to get get absolute Y value", il->evdev);
                goto err_close;
            }
    ...
    if (evtmap & (1 << EV_KEY)) {
        ...
        for (i = 0; i < KEY_CNT; i++) {
            if (keymap[i / 8] & (1 << (i % 8))) {
                if (linux_is_button(i)) { il->num_btns++; }
                else {
                    il->num_keys++;
                }
...
static void input_linux_set_evdev(Object* obj, const char* value, Error** errp)
{
    InputLinux* il = INPUT_LINUX(obj);
...
static void input_linux_set_grab_all(Object* obj, bool value, Error** errp)
{
    InputLinux* il = INPUT_LINUX(obj);
...
static void input_linux_set_repeat(Object* obj, bool value, Error** errp)
{
    InputLinux* il = INPUT_LINUX(obj);
...
static void input_linux_set_grab_toggle(Object* obj, int value, Error** errp)
{
    InputLinux* il = INPUT_LINUX(obj);
...
static void input_linux_class_init(ObjectClass* oc, const void* data)
{
    ...
    object_class_property_add_bool(oc, "grab_all", input_linux_get_grab_all, input_linux_set_grab_all);
    object_class_property_add_bool(oc, "repeat", input_linux_get_repeat, input_linux_set_repeat);
    object_class_property_add_enum(oc, "grab-toggle", "GrabToggleKeys", &GrabToggleKeys_lookup,
...
static void register_types(void) { type_register_static(&input_linux_info); }
