...
#define TEXT_ATTRIBUTES_DEFAULT ((TextAttributes){.fgcol = QEMU_COLOR_WHITE, .bgcol = QEMU_COLOR_BLACK})
...
const char* qemu_text_console_get_label(QemuTextConsole* c) { return c->chr ? c->chr->label : NULL; }

static void qemu_console_fill_rect(QemuConsole* con, int posx, int posy, int width, int height, pixman_color_t color)
{
    DisplaySurface*      surface = qemu_console_surface(con);
    pixman_rectangle16_t rect    = {.x = posx, .y = posy, .width = width, .height = height};
...
static void qemu_console_bitblt(QemuConsole* con, int xs, int ys, int xd, int yd, int w, int h)
{
    DisplaySurface* surface = qemu_console_surface(con);
...
static void vga_putcharxy(QemuConsole* s, int x, int y, int ch, TextAttributes* t_attrib)
{
    ...
    if (!glyphs[ch]) { glyphs[ch] = qemu_pixman_glyph_from_vgafont(FONT_HEIGHT, vgafont16, ch); }
...
{
    if (!qemu_console_is_visible(QEMU_CONSOLE(s))) { return; }
    if (s->update_x0 > x * FONT_WIDTH) { s->update_x0 = x * FONT_WIDTH; }
    if (s->update_y0 > y * FONT_HEIGHT) { s->update_y0 = y * FONT_HEIGHT; }
    if (s->update_x1 < (x + 1) * FONT_WIDTH) { s->update_x1 = (x + 1) * FONT_WIDTH; }
    if (s->update_y1 < (y + 1) * FONT_HEIGHT) { s->update_y1 = (y + 1) * FONT_HEIGHT; }
}
...
static void console_show_cursor(QemuTextConsole* s, int show)
{
    ...
    if (x >= s->width) { x = s->width - 1; }
    y1 = (s->y_base + s->y) % s->total_height;
    y  = y1 - s->y_displayed;
    if (y < 0) { y += s->total_height; }
    if (y < s->height) {
...
static void console_refresh(QemuTextConsole* s)
{
    ...
    for (y = 0; y < s->height; y++) {
        ...
        if (++y1 == s->total_height) { y1 = 0; }
    }
    console_show_cursor(s, 1);
    dpy_gfx_update(QEMU_CONSOLE(s), 0, 0, surface_width(surface), surface_height(surface));
}
...
static void console_scroll(QemuTextConsole* s, int ydelta)
{
    ...
    if (ydelta > 0) {
        for (i = 0; i < ydelta; i++) {
            if (s->y_displayed == s->y_base) { break; }
            if (++s->y_displayed == s->total_height) { s->y_displayed = 0; }
        }
    }
    else {
        ydelta = -ydelta;
        i      = s->backscroll_height;
        if (i > s->total_height - s->height) { i = s->total_height - s->height; }
        y1 = s->y_base - i;
        if (y1 < 0) { y1 += s->total_height; }
        for (i = 0; i < ydelta; i++) {
            if (s->y_displayed == y1) { break; }
            if (--s->y_displayed < 0) { s->y_displayed = s->total_height - 1; }
        }
    }
    console_refresh(s);
}
...
void qemu_text_console_handle_keysym(QemuTextConsole* s, int keysym)
{
    ...
    switch (keysym) {
        case QEMU_KEY_CTRL_UP      : console_scroll(s, -1); break;
        case QEMU_KEY_CTRL_DOWN    : console_scroll(s, 1); break;
        case QEMU_KEY_CTRL_PAGEUP  : console_scroll(s, -10); break;
        case QEMU_KEY_CTRL_PAGEDOWN: console_scroll(s, 10); break;
        default:
            ...
            if (s->echo) { qemu_chr_write(s->chr, buf, q - buf, true); }
    ...
    if (s->text_x[0] <= s->text_x[1]) {
        ...
        for (i = s->text_y[0]; i <= s->text_y[1]; i++) {
            for (j = 0; j < s->width; j++, src++) {
                console_write_ch(chardata++, ATTR2CHTYPE(s->cells[src].ch, s->cells[src].t_attrib.fgcol,
                                                         s->cells[src].t_attrib.bgcol, s->cells[src].t_attrib.bold));
            }
        }
        dpy_text_update(QEMU_CONSOLE(s), s->text_x[0], s->text_y[0], s->text_x[1] - s->text_x[0], i - s->text_y[0]);
        s->text_x[0] = s->width;
        s->text_y[0] = s->height;
        s->text_x[1] = 0;
        s->text_y[1] = 0;
...
static void vc_put_lf(VCChardev* vc)
{
    ...
    if (s->y >= s->height) {
        ...
        if (++s->y_base == s->total_height) { s->y_base = 0; }
        if (s->backscroll_height < s->total_height) { s->backscroll_height++; }
        ...
        if (s->y_displayed == s->y_base) {
            ...
            qemu_console_bitblt(QEMU_CONSOLE(s), 0, FONT_HEIGHT, 0, 0, s->width * FONT_WIDTH,
                                (s->height - 1) * FONT_HEIGHT);
            qemu_console_fill_rect(QEMU_CONSOLE(s), 0, (s->height - 1) * FONT_HEIGHT, s->width * FONT_WIDTH,
                                   FONT_HEIGHT, color_table_rgb[0][TEXT_ATTRIBUTES_DEFAULT.bgcol]);
            s->update_x0 = 0;
            s->update_y0 = 0;
...
static void vc_handle_escape(VCChardev* vc)
{
    ...
    for (i = 0; i < vc->nb_esc_params; i++) {
        switch (vc->esc_params[i]) {
            ...
            case 7 : vc->t_attrib.invers = 1; break;
            case 8 : vc->t_attrib.unvisible = 1; break;
            case 22: vc->t_attrib.bold = 0; break;
            case 24: vc->t_attrib.uline = 0; break;
            case 25: vc->t_attrib.blink = 0; break;
            case 27: vc->t_attrib.invers = 0; break;
            case 28: vc->t_attrib.unvisible = 0; break;
            /* set foreground color */
            case 30: vc->t_attrib.fgcol = QEMU_COLOR_BLACK; break;
            case 31: vc->t_attrib.fgcol = QEMU_COLOR_RED; break;
            case 32: vc->t_attrib.fgcol = QEMU_COLOR_GREEN; break;
            case 33: vc->t_attrib.fgcol = QEMU_COLOR_YELLOW; break;
            case 34: vc->t_attrib.fgcol = QEMU_COLOR_BLUE; break;
            case 35: vc->t_attrib.fgcol = QEMU_COLOR_MAGENTA; break;
            case 36: vc->t_attrib.fgcol = QEMU_COLOR_CYAN; break;
            case 37: vc->t_attrib.fgcol = QEMU_COLOR_WHITE; break;
            /* set background color */
            case 40: vc->t_attrib.bgcol = QEMU_COLOR_BLACK; break;
            case 41: vc->t_attrib.bgcol = QEMU_COLOR_RED; break;
            case 42: vc->t_attrib.bgcol = QEMU_COLOR_GREEN; break;
            case 43: vc->t_attrib.bgcol = QEMU_COLOR_YELLOW; break;
            case 44: vc->t_attrib.bgcol = QEMU_COLOR_BLUE; break;
            case 45: vc->t_attrib.bgcol = QEMU_COLOR_MAGENTA; break;
            case 46: vc->t_attrib.bgcol = QEMU_COLOR_CYAN; break;
            case 47: vc->t_attrib.bgcol = QEMU_COLOR_WHITE; break;
        }
    }
}
...
static void vc_update_xy(VCChardev* vc, int x, int y)
{
    ...
    y2 = y1 - s->y_displayed;
    if (y2 < 0) { y2 += s->total_height; }
    if (y2 < s->height) {
        if (x >= s->width) { x = s->width - 1; }
        c = &s->cells[y1 * s->width + x];
        vga_putcharxy(QEMU_CONSOLE(s), x, y2, c->ch, &(c->t_attrib));
...
static void vc_csi_P(struct VCChardev* vc, unsigned int nr)
{
    ...
    if (len) {
        ...
        for (end = x1 + len; x1 < end; x1++) { vc_update_xy(vc, x1, s->y); }
    }
    /* Clear the rest */
    for (; x1 < s->width; x1++) { vc_clear_xy(vc, x1, s->y); }
}
...
static void vc_csi_at(struct VCChardev* vc, unsigned int nr)
{
    ...
    /* Insert blanks */
    for (x1 = s->x; x1 < s->x + nr; x1++) { vc_clear_xy(vc, x1, s->y); }
}
...
static void vc_putchar(VCChardev* vc, int ch)
{
    ...
    switch (vc->state) {
        case TTY_STATE_NORM:
            switch (ch) {
                case '\r': /* carriage return */ s->x = 0; break;
                case '\n': /* newline */ vc_put_lf(vc); break;
                case '\b': /* backspace */
                    if (s->x > 0) { s->x--; }
                    break;
                case '\t': /* tabspace */
                ...
                case 15:
                    ...
                    break;
                case 27: /* esc (introducing an escape sequence) */ vc->state = TTY_STATE_ESC; break;
                default: vc_put_one(vc, ch); break;
            }
            break;
        case TTY_STATE_ESC: /* check if it is a terminal escape sequence */
            if (ch == '[') {
                for (i = 0; i < MAX_ESC_PARAMS; i++) { vc->esc_params[i] = 0; }
                vc->nb_esc_params = 0;
        ...
        case TTY_STATE_CSI: /* handle escape sequence parameters */
            if (ch >= '0' && ch <= '9') {
                ...
                if (vc->nb_esc_params < MAX_ESC_PARAMS) { vc->nb_esc_params++; }
                if (ch == ';' || ch == '?') { break; }
                trace_console_putchar_csi(vc->esc_params[0], vc->esc_params[1], ch, vc->nb_esc_params);
                vc->state = TTY_STATE_NORM;
                switch (ch) {
                    case 'A':
                        /* move cursor up */
                        if (vc->esc_params[0] == 0) { vc->esc_params[0] = 1; }
                    ...
                    case 'J':
                        switch (vc->esc_params[0]) {
                            ...
                            case 2:
                                /* clear entire screen */
                                for (y = 0; y <= s->height; y++) {
                                    for (x = 0; x < s->width; x++) { vc_clear_xy(vc, x, y); }
                                }
                                break;
                        }
                        break;
                    case 'K':
                        switch (vc->esc_params[0]) {
                            case 0:
                                /* clear to eol */
                                for (x = s->x; x < s->width; x++) { vc_clear_xy(vc, x, s->y); }
                                break;
                            case 1:
                                /* clear from beginning of line */
                                for (x = 0; x <= s->x && x < s->width; x++) { vc_clear_xy(vc, x, s->y); }
                                break;
                            case 2:
                                /* clear entire line */
                                for (x = 0; x < s->width; x++) { vc_clear_xy(vc, x, s->y); }
                                break;
                        }
                        break;
                    case 'P': vc_csi_P(vc, vc->esc_params[0]); break;
                    case 'm': vc_handle_escape(vc); break;
                    case 'n':
                        switch (vc->esc_params[0]) {
                            case 5:
                            ...
                            case 6:
                                ...
                                response = g_strdup_printf("\033[%d;%dR", s->y + 1, s->x + 1);
                                vc_respond_str(vc, response);
                                break;
                        }
                        break;
                    case 's': vc_save_cursor(vc); break;
                    case 'u': vc_restore_cursor(vc); break;
                    case '@': vc_csi_at(vc, vc->esc_params[0]); break;
                    default : trace_console_putchar_unhandled(ch); break;
                }
                break;
            }
            break;
...
static int vc_chr_write(Chardev* chr, const uint8_t* buf, int len)
{
    ...
    for (i = 0; i < len; i++) { vc_putchar(drv, buf[i]); }
    console_show_cursor(s, 1);
    if (s->update_x0 < s->update_x1) {
        dpy_gfx_update(QEMU_CONSOLE(s), s->update_x0, s->update_y0, s->update_x1 - s->update_x0,
...
static void cursor_timer_cb(void* opaque) { qemu_text_console_update_cursor(); }

static void text_console_invalidate(void* opaque)
{
    QemuTextConsole* s = QEMU_TEXT_CONSOLE(opaque);

    if (!QEMU_IS_FIXED_TEXT_CONSOLE(s)) { text_console_resize(QEMU_TEXT_CONSOLE(s)); }
    console_refresh(s);
}
...
{
    if (!cursor_timer) { cursor_timer = timer_new_ms(QEMU_CLOCK_REALTIME, cursor_timer_cb, NULL); }
}
...
static void qemu_fixed_text_console_finalize(Object* obj) { }

static void qemu_fixed_text_console_class_init(ObjectClass* oc, const void* data) { }

static void qemu_fixed_text_console_init(Object* obj) { }
...
void qemu_text_console_update_size(QemuTextConsole* c) { dpy_text_resize(QEMU_CONSOLE(c), c->width, c->height); }
...
static bool vc_chr_open(Chardev* chr, ChardevBackend* backend, Error** errp)
{
    ...
    if (vc->has_width) { width = vc->width; }
    else if (vc->has_cols) {
        width = vc->cols * FONT_WIDTH;
    }

    if (vc->has_height) { height = vc->height; }
    else if (vc->has_rows) {
...
void qemu_console_early_init(void)
{
    ...
    if (!object_class_by_name(TYPE_CHARDEV_VC)) { type_register_static(&char_vc_type_info); }
}
