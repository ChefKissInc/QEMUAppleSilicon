...
static gboolean check_for_xwin(Display* dpy)
{
    ...
    if (strstr(vendor, "Cygwin/X")) { return TRUE; }
...
static gboolean check_for_xquartz(Display* dpy)
{
    ...
    for (i = 0; extensions != NULL && i < nextensions; i++) {
        ...
        if (strcmp(extensions[i], "Apple-WM") == 0 || strcmp(extensions[i], "Apple-DRI") == 0) { match = TRUE; }
    }
...
const guint16* qemu_xkeymap_mapping_table(Display* dpy, size_t* maplen)
{
    ...
    desc = XkbGetMap(dpy, XkbGBN_AllComponentsMask, XkbUseCoreKbd);
    if (desc) {
        if (XkbGetNames(dpy, XkbKeycodesNameMask, desc) == Success) {
            keycodes = XGetAtomName(dpy, desc->names->keycodes);
            if (!keycodes) { g_warning("could not lookup keycode name"); }
            else {
    ...
    else if ((keycodes && g_str_has_prefix(keycodes, "evdev")) || (XKeysymToKeycode(dpy, XK_Page_Up) == 0x70)) {
        trace_xkeymap_keymap("evdev");
        *maplen = qemu_input_map_xorgevdev_to_qcode_len;
        map     = qemu_input_map_xorgevdev_to_qcode;
    }
    else if ((keycodes && g_str_has_prefix(keycodes, "xfree86")) || (XKeysymToKeycode(dpy, XK_Page_Up) == 0x63)) {
    ...
    if (keycodes) { XFree((void*)keycodes); }
    return map;
}
