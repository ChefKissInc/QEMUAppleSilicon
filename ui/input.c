...
typedef QTAILQ_HEAD(QemuInputEventQueueHead, QemuInputEventQueue) QemuInputEventQueueHead;
...
QemuInputHandlerState* qemu_input_handler_register(DeviceState* dev, const QemuInputHandler* handler)
{
...
static QemuInputHandlerState* qemu_input_find_handler(uint32_t mask, QemuConsole* con)
{
    ...
    QTAILQ_FOREACH (s, &handlers, node) {
        if (s->con == NULL || s->con != con) { continue; }
        if (mask & s->handler->mask) { return s; }
    }

    QTAILQ_FOREACH (s, &handlers, node) {
        if (s->con != NULL) { continue; }
        if (mask & s->handler->mask) { return s; }
    }
    return NULL;
}
...
void qmp_input_send_event(const char* device, bool has_head, int64_t head, InputEventList* events, Error** errp)
{
    ...
    for (e = events; e != NULL; e = e->next) {
        ...
        if (evt->type == INPUT_EVENT_KIND_KEY && evt->u.key.data->key->type == KEY_VALUE_KIND_NUMBER) {
...
static void qemu_input_event_trace(QemuConsole* src, InputEvent* evt)
{
    ...
    if (src) { idx = qemu_console_get_index(src); }
    switch (evt->type) {
    ...
    while (!QTAILQ_EMPTY(queue)) {
        ...
        switch (item->type) {
            case QEMU_INPUT_QUEUE_DELAY:
                timer_mod(item->timer, qemu_clock_get_ms(QEMU_CLOCK_VIRTUAL) + item->delay_ms);
                return;
            case QEMU_INPUT_QUEUE_EVENT:
                qemu_input_event_send(item->src, item->evt);
                qapi_free_InputEvent(item->evt);
                break;
            case QEMU_INPUT_QUEUE_SYNC: qemu_input_event_sync(); break;
        }
        QTAILQ_REMOVE(queue, item, node);
        queue_count--;
        g_free(item);
...
static void qemu_input_queue_delay(QemuInputEventQueueHead* queue, QEMUTimer* timer, uint32_t delay_ms)
{
    ...
    if (start_timer) { timer_mod(item->timer, qemu_clock_get_ms(QEMU_CLOCK_VIRTUAL) + item->delay_ms); }
}

static void qemu_input_queue_event(QemuInputEventQueueHead* queue, QemuConsole* src, InputEvent* evt)
{
    QemuInputEventQueue* item = g_new0(QemuInputEventQueue, 1);
...
void qemu_input_event_send(QemuConsole* src, InputEvent* evt)
{
     ...
     */
    if (evt->type == INPUT_EVENT_KIND_KEY && evt->u.key.data->key->u.qcode.data == Q_KEY_CODE_SYSRQ) {
...
void qemu_input_event_send_key_delay(uint32_t delay_ms)
{
    ...
    if (!kbd_timer) {
        kbd_timer = timer_new_full(NULL, QEMU_CLOCK_VIRTUAL, SCALE_MS, QEMU_TIMER_ATTR_EXTERNAL,
...
void qemu_input_update_buttons(QemuConsole* src, uint32_t* button_map, uint32_t button_old, uint32_t button_new)
{
    ...
    for (btn = 0; btn < INPUT_BUTTON__MAX; btn++) {
        mask = button_map[btn];
        if ((button_old & mask) == (button_new & mask)) { continue; }
...
bool qemu_input_is_absolute(QemuConsole* con)
{
    ...
    s = qemu_input_find_handler(INPUT_EVENT_MASK_REL | INPUT_EVENT_MASK_ABS, con);
    return (s != NULL) && (s->handler->mask & INPUT_EVENT_MASK_ABS);
}

int qemu_input_scale_axis(int value, int min_in, int max_in, int min_out, int max_out)
{
    int64_t range_in  = (int64_t)max_in - min_in;
    int64_t range_out = (int64_t)max_out - min_out;

    if (range_in < 1) { return min_out + range_out / 2; }
...
void qemu_input_queue_abs(QemuConsole* src, InputAxis axis, int value, int min_in, int max_in)
{
    InputMoveEvent move = {
        .axis  = axis,
        .value = qemu_input_scale_axis(value, min_in, max_in, INPUT_EVENT_ABS_MIN, INPUT_EVENT_ABS_MAX),
    };
    InputEvent evt = {
...
void qemu_input_queue_mtt(QemuConsole* src, InputMultiTouchType type, int slot, int tracking_id)
{
    InputMultiTouchEvent mtt = {
        .type        = type,
        .slot        = slot,
...
void qemu_input_queue_mtt_abs(QemuConsole* src, InputAxis axis, int value, int min_in, int max_in, int slot,
                              int tracking_id)
{
    InputMultiTouchEvent mtt = {
...
void qemu_add_mouse_mode_change_notifier(Notifier* notify) { notifier_list_add(&mouse_mode_notifiers, notify); }

void qemu_remove_mouse_mode_change_notifier(Notifier* notify) { notifier_remove(notify); }
...
MouseInfoList* qmp_query_mice(Error** errp)
{
    ...
    QTAILQ_FOREACH (s, &handlers, node) {
        if (!(s->handler->mask & (INPUT_EVENT_MASK_REL | INPUT_EVENT_MASK_ABS))) { continue; }
...
bool qemu_mouse_set(int index, Error** errp)
{
    ...
    if (!(s->handler->mask & (INPUT_EVENT_MASK_REL | INPUT_EVENT_MASK_ABS))) {
        error_setg(errp, "Input device '%s' is not a mouse", s->handler->name);
        return false;
    }
