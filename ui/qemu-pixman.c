...
pixman_format_code_t qemu_default_pixman_format(int bpp, bool native_endian)
{
    if (native_endian) {
        switch (bpp) {
            case 15: return PIXMAN_x1r5g5b5;
            case 16: return PIXMAN_r5g6b5;
            case 24: return PIXMAN_r8g8b8;
            case 32: return PIXMAN_x8r8g8b8;
        }
    }
    else {
        switch (bpp) {
            case 24: return PIXMAN_b8g8r8;
            case 32: return PIXMAN_b8g8r8x8; break;
        }
    }
    return 0;
}
...
static const struct
{
    ...
    {DRM_FORMAT_XRGB8888, PIXMAN_LE_x8r8g8b8}, {DRM_FORMAT_XBGR8888, PIXMAN_LE_x8b8g8r8},
...
pixman_format_code_t qemu_drm_format_to_pixman(uint32_t drm_format)
{
    ...
    for (i = 0; i < ARRAY_SIZE(drm_format_pixman_map); i++) {
        if (drm_format == drm_format_pixman_map[i].drm_format) { return drm_format_pixman_map[i].pixman_format; }
    }
    return 0;
}
...
uint32_t qemu_pixman_to_drm_format(pixman_format_code_t pixman_format)
{
    ...
    for (i = 0; i < ARRAY_SIZE(drm_format_pixman_map); i++) {
        if (pixman_format == drm_format_pixman_map[i].pixman_format) { return drm_format_pixman_map[i].drm_format; }
    }
    return 0;
}
...
int qemu_pixman_get_type(int rshift, int gshift, int bshift, int endian)
{
    ...
    if (rshift > gshift && gshift > bshift) {
        if (bshift == 0) { type = native_endian ? PIXMAN_TYPE_ARGB : PIXMAN_TYPE_BGRA; }
        else {
            type = native_endian ? PIXMAN_TYPE_RGBA : PIXMAN_TYPE_ABGR;
        }
    }
    else if (rshift < gshift && gshift < bshift) {
        if (rshift == 0) { type = native_endian ? PIXMAN_TYPE_ABGR : PIXMAN_TYPE_RGBA; }
        else {
...
pixman_format_code_t qemu_pixman_get_format(PixelFormat* pf, int endian)
{
    ...
    format = PIXMAN_FORMAT(pf->bits_per_pixel, type, pf->abits, pf->rbits, pf->gbits, pf->bbits);
...
bool qemu_pixman_check_format(DisplayChangeListener* dcl, pixman_format_code_t format)
{
    switch (format) {
        ...
        case PIXMAN_r5g6b5  : return true;
...
{ pixman_image_composite(PIXMAN_OP_SRC, fb, NULL, linebuf, x, y, 0, 0, 0, 0, width, 1); }

pixman_image_t* qemu_pixman_mirror_create(pixman_format_code_t format, pixman_image_t* image)
{
...
pixman_image_t* qemu_pixman_glyph_from_vgafont(int height, const uint8_t* font, unsigned int ch)
{
    ...
    glyph = pixman_image_create_bits(PIXMAN_a8, 8, height, NULL, 0);
                              ...
                              pixman_color_t* bgcol, int x, int y, int cw, int ch)
{
    ...
    pixman_image_composite(PIXMAN_OP_SRC, ibg, NULL, surface, 0, 0, 0, 0, cw * x, ch * y, cw, ch);
    pixman_image_composite(PIXMAN_OP_OVER, ifg, glyph, surface, 0, 0, 0, 0, cw * x, ch * y, cw, ch);
...
static void qemu_pixman_shareable_free(qemu_pixman_shareable handle, void* ptr, size_t size)
{
#ifdef WIN32
...
static void qemu_pixman_shared_image_destroy(pixman_image_t* image, void* data)
{
...
bool qemu_pixman_image_new_shareable(pixman_image_t** image, qemu_pixman_shareable* handle, const char* name,
                                     ...
                                     Error** errp)
{
    ...
    *image = pixman_image_create_bits(format, width, height, bits, rowstride_bytes);
    ...
    pixman_image_set_destroy_function(*image, qemu_pixman_shared_image_destroy, SHAREABLE_TO_PTR(*handle));
