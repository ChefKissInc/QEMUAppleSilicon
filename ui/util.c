...
static bool append_pci_address(char* buf, size_t buf_size, const PCIDevice* pci)
{
     ...
     */
    if (bus->parent_dev != NULL) { append_pci_address(buf, buf_size, bus->parent_dev); }

    size_t  len     = strlen(buf);
    ssize_t written = snprintf(buf + len, buf_size - len, "/%02x.%x", PCI_SLOT(pci->devfn), PCI_FUNC(pci->devfn));
