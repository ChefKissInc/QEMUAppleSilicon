...
static const char* cmd_names[] = {
    [barrierCmdCNoop] = "CNOP",         [barrierCmdCClose] = "CBYE",       [barrierCmdCEnter] = "CINN",
    [barrierCmdCLeave] = "COUT",        [barrierCmdCClipboard] = "CCLP",   [barrierCmdCScreenSaver] = "CSEC",
    [barrierCmdCResetOptions] = "CROP", [barrierCmdCInfoAck] = "CIAK",     [barrierCmdCKeepAlive] = "CALV",
    [barrierCmdDKeyDown] = "DKDN",      [barrierCmdDKeyRepeat] = "DKRP",   [barrierCmdDKeyUp] = "DKUP",
    [barrierCmdDMouseDown] = "DMDN",    [barrierCmdDMouseUp] = "DMUP",     [barrierCmdDMouseMove] = "DMMV",
    [barrierCmdDMouseRelMove] = "DMRM", [barrierCmdDMouseWheel] = "DMWM",  [barrierCmdDClipboard] = "DCLP",
    [barrierCmdDInfo] = "DINF",         [barrierCmdDSetOptions] = "DSOP",  [barrierCmdDFileTransfer] = "DFTR",
    [barrierCmdDDragInfo] = "DDRG",     [barrierCmdQInfo] = "QINF",        [barrierCmdEIncompatible] = "EICV",
    [barrierCmdEBusy] = "EBSY",         [barrierCmdEUnknown] = "EUNK",     [barrierCmdEBad] = "EBAD",
    [barrierCmdHello] = "Barrier",      [barrierCmdHelloBack] = "Barrier",
};
...
static int input_barrier_to_qcode(uint16_t keyid, uint16_t keycode)
{
    ...
    if (keycode && keycode <= qemu_input_map_xorgkbd_to_qcode_len) { return qemu_input_map_xorgkbd_to_qcode[keycode]; }

    if (keyid >= 0xE000 && keyid <= 0xEFFF) { keyid += 0x1000; }
...
{
    switch (buttonid) {
        case barrierButtonLeft  : return INPUT_BUTTON_LEFT;
        case barrierButtonMiddle: return INPUT_BUTTON_MIDDLE;
        case barrierButtonRight : return INPUT_BUTTON_RIGHT;
...
#define read_char(x, p, l)                        \
    do {                                          \
        ...
        if (l < size) { return G_SOURCE_REMOVE; } \
        x  = *(char*)p;                           \
        p += size;                                \
        l -= size;                                \
    }                                             \
...
#define write_string(p, s, l)                                   \
    do {                                                        \
        int size = strlen(s);                                   \
        if (l < size + sizeof(int)) { return G_SOURCE_REMOVE; } \
        *(int*)p  = htonl(size);                                \
        p        += sizeof(size);                               \
        l        -= sizeof(size);                               \
        memcpy(p, s, size);                                     \
...
static gboolean readcmd(InputBarrier* ib, struct barrierMsg* msg)
{
    ...
    ret = qio_channel_read(QIO_CHANNEL(ib->sioc), (char*)&len, sizeof(len), NULL);
    if (ret < 0) { return G_SOURCE_REMOVE; }

    len = ntohl(len);
    if (len > MAX_HELLO_LENGTH) { return G_SOURCE_REMOVE; }
    ...
    if (len >= strlen(cmd_names[barrierCmdHello])
        && memcmp(p, cmd_names[barrierCmdHello], strlen(cmd_names[barrierCmdHello])) == 0)
    {
        cmd  = barrierCmdHello;
        ...
        for (cmd = 0; cmd < barrierCmdHello; cmd++) {
            if (memcmp(ib->buffer, cmd_names[cmd], 4) == 0) { break; }
        }

        if (cmd == barrierCmdHello) { return G_SOURCE_REMOVE; }
        p   += 4;
        len -= 4;
    }
    ...
    switch (cmd) {
        ...
        case barrierCmdDSetOptions:
            ...
            msg->set.nb /= 2;
            if (msg->set.nb > BARRIER_MAX_OPTIONS) { msg->set.nb = BARRIER_MAX_OPTIONS; }
            i = 0;
        ...
        case barrierCmdDMouseUp  : read_char(msg->mousebutton.buttonid, p, len); break;
        ...
        case barrierCmdDKeyUp:
            ...
            msg->key.button = 0;
            if (len) { read_short(msg->key.button, p, len); }
            break;
...
static gboolean writecmd(InputBarrier* ib, struct barrierMsg* msg)
{
    ...
    switch (msg->cmd) {
        case barrierCmdHello:
            if (msg->version.major < BARRIER_VERSION_MAJOR
                || (msg->version.major == BARRIER_VERSION_MAJOR && msg->version.minor < BARRIER_VERSION_MINOR))
            {
                ib->ioc_tag = 0;
            ...
            break;
        case barrierCmdCClose: ib->ioc_tag = 0; return G_SOURCE_REMOVE;
        ...
        case barrierCmdDSetOptions:
            /* TODO: set options */
            break;
        case barrierCmdCEnter    : break;
        case barrierCmdDClipboard: break;
        case barrierCmdCKeepAlive: write_cmd(p, barrierCmdCKeepAlive, avail); break;
        case barrierCmdCLeave    : break;

        /* mouse */
        case barrierCmdDMouseMove:
            qemu_input_queue_abs(NULL, INPUT_AXIS_X, msg->mousepos.x, ib->x_origin, ib->width);
            qemu_input_queue_abs(NULL, INPUT_AXIS_Y, msg->mousepos.y, ib->y_origin, ib->height);
        ...
        case barrierCmdDMouseRelMove:
            ...
            break;
        case barrierCmdDMouseDown:
            qemu_input_queue_btn(NULL, input_barrier_to_mouse(msg->mousebutton.buttonid), true);
            qemu_input_event_sync();
            break;
        case barrierCmdDMouseUp:
            qemu_input_queue_btn(NULL, input_barrier_to_mouse(msg->mousebutton.buttonid), false);
            qemu_input_event_sync();
            break;
        case barrierCmdDMouseWheel:
            qemu_input_queue_btn(NULL, (msg->mousepos.y > 0) ? INPUT_BUTTON_WHEEL_UP : INPUT_BUTTON_WHEEL_DOWN, true);
            qemu_input_event_sync();
            qemu_input_queue_btn(NULL, (msg->mousepos.y > 0) ? INPUT_BUTTON_WHEEL_UP : INPUT_BUTTON_WHEEL_DOWN, false);
            qemu_input_event_sync();
            break;

        /* keyboard */
        case barrierCmdDKeyDown:
            qemu_input_event_send_key_qcode(NULL, input_barrier_to_qcode(msg->key.keyid, msg->key.button), true);
            break;
        case barrierCmdDKeyRepeat:
            for (i = 0; i < msg->repeat.repeat; i++) {
                qemu_input_event_send_key_qcode(NULL, input_barrier_to_qcode(msg->repeat.keyid, msg->repeat.button),
                                                false);
                qemu_input_event_send_key_qcode(NULL, input_barrier_to_qcode(msg->repeat.keyid, msg->repeat.button),
                                                true);
            }
            break;
        case barrierCmdDKeyUp:
            qemu_input_event_send_key_qcode(NULL, input_barrier_to_qcode(msg->key.keyid, msg->key.button), false);
            break;
        default: write_cmd(p, barrierCmdEUnknown, avail); break;
    }
    ...
    if (len) {
        ...
        ret = qio_channel_write(QIO_CHANNEL(ib->sioc), ib->buffer, len + sizeof(len), NULL);
        if (ret < 0) {
            ib->ioc_tag = 0;
            return G_SOURCE_REMOVE;
        }
...
static gboolean input_barrier_event(QIOChannel* ioc G_GNUC_UNUSED, GIOCondition condition, void* opaque)
{
...
static void input_barrier_complete(UserCreatable* uc, Error** errp)
{
    ...
    if (qio_channel_socket_connect_sync(ib->sioc, &ib->saddr, errp) < 0) { return; }

    qio_channel_set_delay(QIO_CHANNEL(ib->sioc), false);

    ib->ioc_tag = qio_channel_add_watch(QIO_CHANNEL(ib->sioc), G_IO_IN, input_barrier_event, ib, NULL);
}
...
static void input_barrier_set_name(Object* obj, const char* value, Error** errp)
{
    InputBarrier* ib = INPUT_BARRIER(obj);
...
static void input_barrier_set_server(Object* obj, const char* value, Error** errp)
{
    InputBarrier* ib = INPUT_BARRIER(obj);
...
static void input_barrier_set_port(Object* obj, const char* value, Error** errp)
{
    InputBarrier* ib = INPUT_BARRIER(obj);
...
static void input_barrier_set_x_origin(Object* obj, const char* value, Error** errp)
{
    InputBarrier* ib = INPUT_BARRIER(obj);
    int           result, err;
    ...
    if (err < 0 || result < 0 || result > SHRT_MAX) {
        error_setg(errp, "x-origin property must be in the range [0..%d]", SHRT_MAX);
        return;
    }
    ib->x_origin = result;
}
...
static void input_barrier_set_y_origin(Object* obj, const char* value, Error** errp)
{
    InputBarrier* ib = INPUT_BARRIER(obj);
    int           result, err;
    ...
    err = qemu_strtoi(value, NULL, 0, &result);
    if (err < 0 || result < 0 || result > SHRT_MAX) {
        error_setg(errp, "y-origin property must be in the range [0..%d]", SHRT_MAX);
        return;
    }
    ib->y_origin = result;
}
...
static void input_barrier_set_width(Object* obj, const char* value, Error** errp)
{
    InputBarrier* ib = INPUT_BARRIER(obj);
    int           result, err;
    ...
    err = qemu_strtoi(value, NULL, 0, &result);
    if (err < 0 || result < 0 || result > SHRT_MAX) {
        error_setg(errp, "width property must be in the range [0..%d]", SHRT_MAX);
        return;
    }
    ib->width = result;
}
...
static void input_barrier_set_height(Object* obj, const char* value, Error** errp)
{
    InputBarrier* ib = INPUT_BARRIER(obj);
    int           result, err;
    ...
    err = qemu_strtoi(value, NULL, 0, &result);
    if (err < 0 || result < 0 || result > SHRT_MAX) {
        error_setg(errp, "height property must be in the range [0..%d]", SHRT_MAX);
        return;
    }
    ib->height = result;
}
...
static void input_barrier_instance_init(Object* obj)
{
    ...
    if (keyboard_layout && !kbd_layout) {
        ...
        kbd_layout = init_keyboard_layout(name2keysym, keyboard_layout, &error_fatal);
    }
...
static void input_barrier_class_init(ObjectClass* oc, const void* data)
{
    ...
    object_class_property_add_str(oc, "name", input_barrier_get_name, input_barrier_set_name);
    object_class_property_add_str(oc, "server", input_barrier_get_server, input_barrier_set_server);
    object_class_property_add_str(oc, "port", input_barrier_get_port, input_barrier_set_port);
    object_class_property_add_str(oc, "x-origin", input_barrier_get_x_origin, input_barrier_set_x_origin);
    object_class_property_add_str(oc, "y-origin", input_barrier_get_y_origin, input_barrier_set_y_origin);
    object_class_property_add_str(oc, "width", input_barrier_get_width, input_barrier_set_width);
...
static void register_types(void) { type_register_static(&input_barrier_info); }
