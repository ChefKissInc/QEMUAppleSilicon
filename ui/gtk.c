...
DECLARE_INSTANCE_CHECKER(VCChardev, VC_CHARDEV, TYPE_CHARDEV_VC)
...
static VirtualConsole* gd_vc_find_by_menu(GtkDisplayState* s)
{
    ...
    for (i = 0; i < s->nb_vcs; i++) {
        vc = &s->vc[i];
        if (gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(vc->menu_item))) { return vc; }
    }
    return NULL;
}
...
static void gd_update_cursor(VirtualConsole* vc)
{
    ...
    if (vc->type != GD_VC_GFX || !qemu_console_is_graphic(vc->gfx.dcl.con)) { return; }

    if (!gtk_widget_get_realized(vc->gfx.drawing_area)) { return; }
...
static void gd_update_caption(GtkDisplayState* s)
{
    ...
    if (is_paused) { status = _(" [Paused]"); }
    s->external_pause_update = true;
    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(s->pause_item), is_paused);
    ...
    for (i = 0; i < s->nb_vcs; i++) {
        ...
        if (!vc->window) { continue; }
        title = g_strdup_printf("%s: %s%s%s", prefix, vc->label, vc == s->kbd_owner ? " +kbd" : "",
                                vc == s->ptr_owner ? " +ptr" : "");
...
static void gd_update_geometry_hints(VirtualConsole* vc)
{
    ...
    if (vc->type == GD_VC_GFX) {
        ...
        gtk_style_context_get_padding(gtk_widget_get_style_context(vc->vte.terminal),
                                      gtk_widget_get_state_flags(vc->vte.terminal), &padding);
    #else
        {
            GtkBorder* ib = NULL;
...
void gd_update_windowsize(VirtualConsole* vc)
{
    ...
    if (vc->type == GD_VC_GFX && !s->full_screen && !s->free_scale) {
        gtk_window_resize(GTK_WINDOW(vc->window ? vc->window : s->window), VC_WINDOW_X_MIN, VC_WINDOW_Y_MIN);
    }
}
...
static void gd_widget_reparent(GtkWidget* from, GtkWidget* to, GtkWidget* widget)
{
...
{
#ifdef G_OS_WIN32
    return gdk_win32_window_get_impl_hwnd(gtk_widget_get_window(vc->window ? vc->window : vc->s->window));
#else
    return NULL;
#endif
}
...
static void gd_update(DisplayChangeListener* dcl, int fbx, int fby, int fbw, int fbh)
{
    ...
    if (vc->gfx.convert) {
        pixman_image_composite(PIXMAN_OP_SRC, vc->gfx.ds->image, NULL, vc->gfx.convert, fbx, fby, 0, 0, fbx, fby, fbw,
                               fbh);
    }
    ...
    if (ww_widget > ww_surface) { wx_offset = (ww_widget - ww_surface) / 2; }
    if (wh_widget > wh_surface) { wy_offset = (wh_widget - wh_surface) / 2; }

    gtk_widget_queue_draw_area(vc->gfx.drawing_area, wx_offset + wx1, wy_offset + wy1, (wx2 - wx1), (wy2 - wy1));
}

static void gd_refresh(DisplayChangeListener* dcl) { graphic_hw_update(dcl->con); }

static GdkDevice* gd_get_pointer(GdkDisplay* dpy) { return gdk_seat_get_pointer(gdk_display_get_default_seat(dpy)); }

static void gd_mouse_set(DisplayChangeListener* dcl, int x, int y, bool visible)
{
    VirtualConsole* vc = container_of(dcl, VirtualConsole, gfx.dcl);
    GdkDisplay*     dpy;
    gint            x_root, y_root;

    if (!gtk_widget_get_realized(vc->gfx.drawing_area) || qemu_input_is_absolute(dcl->con)) { return; }

    dpy = gtk_widget_get_display(vc->gfx.drawing_area);
    gdk_window_get_root_coords(gtk_widget_get_window(vc->gfx.drawing_area), x, y, &x_root, &y_root);
    gdk_device_warp(gd_get_pointer(dpy), gtk_widget_get_screen(vc->gfx.drawing_area), x_root, y_root);
    vc->s->last_x = x;
    vc->s->last_y = y;
}

static void gd_cursor_define(DisplayChangeListener* dcl, QEMUCursor* c)
{
    VirtualConsole* vc = container_of(dcl, VirtualConsole, gfx.dcl);
    GdkPixbuf*      pixbuf;
    GdkCursor*      cursor;
    ...
    pixbuf = gdk_pixbuf_new_from_data((guchar*)(c->data), GDK_COLORSPACE_RGB, true, 8, c->width, c->height,
                                      c->width * 4, NULL, NULL);
    cursor = gdk_cursor_new_from_pixbuf(gtk_widget_get_display(vc->gfx.drawing_area), pixbuf, c->hot_x, c->hot_y);
...
static void gd_switch(DisplayChangeListener* dcl, DisplaySurface* surface)
{
    ...
    if (vc->gfx.ds && surface_width(vc->gfx.ds) == surface_width(surface)
    ...
    if (surface_format(surface) == PIXMAN_x8r8g8b8) {
         ...
         */
        vc->gfx.surface =
            cairo_image_surface_create_for_data(surface_data(surface), CAIRO_FORMAT_RGB24, surface_width(surface),
                                                surface_height(surface), surface_stride(surface));
    }
    else {
        /* Must convert surface, use pixman to do it. */
        vc->gfx.convert =
            pixman_image_create_bits(PIXMAN_x8r8g8b8, surface_width(surface), surface_height(surface), NULL, 0);
        vc->gfx.surface = cairo_image_surface_create_for_data(
            (void*)pixman_image_get_data(vc->gfx.convert), CAIRO_FORMAT_RGB24, pixman_image_get_width(vc->gfx.convert),
            pixman_image_get_height(vc->gfx.convert), pixman_image_get_stride(vc->gfx.convert));
        pixman_image_composite(PIXMAN_OP_SRC, vc->gfx.ds->image, NULL, vc->gfx.convert, 0, 0, 0, 0, 0, 0,
                               pixman_image_get_width(vc->gfx.convert), pixman_image_get_height(vc->gfx.convert));
    }

    if (resized) { gd_update_windowsize(vc); }
    else {
...
static void gd_mouse_mode_change(Notifier* notify, void* data)
{
    ...
    if (s->ptr_owner && qemu_input_is_absolute(s->ptr_owner->gfx.dcl.con)) {
        if (!s->ptr_owner->window) { gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(s->grab_item), FALSE); }
        else {
            gd_ungrab_pointer(s);
        }
    }
...
static gboolean gd_window_close(GtkWidget* widget, GdkEvent* event, void* opaque)
{
    GtkDisplayState* s           = opaque;
    bool             allow_close = true;

    if (s->opts->has_window_close && !s->opts->window_close) { allow_close = false; }

    if (allow_close) { qmp_quit(NULL); }
...
static void gd_set_ui_refresh_rate(VirtualConsole* vc, int refresh_rate)
{
    ...
    if (!dpy_ui_info_supported(vc->gfx.dcl.con)) { return; }
...
void gd_update_monitor_refresh_rate(VirtualConsole* vc, GtkWidget* widget)
{
    ...
    vc->gfx.dcl.update_interval =
        refresh_rate ? MIN(1000 * 1000 / refresh_rate, GUI_REFRESH_INTERVAL_DEFAULT) : GUI_REFRESH_INTERVAL_DEFAULT;
#endif
}
...
void gd_update_scale(VirtualConsole* vc, int ww, int wh, int fbw, int fbh)
{
    ...
    if (vc->s->full_screen) {
        ...
        sx = (double)ww / fbw;
        sy = (double)wh / fbh;
        if (vc->s->keep_aspect_ratio) { vc->gfx.scale_x = vc->gfx.scale_y = MIN(sx, sy); }
        else {
            vc->gfx.scale_x = sx;
            vc->gfx.scale_y = sy;
        }
...
static gboolean gd_draw_event(GtkWidget* widget, cairo_t* cr, void* opaque)
{
    ...
    if (!vc->gfx.ds) { return FALSE; }
    if (!vc->gfx.surface) { return FALSE; }
    ...
    cairo_rectangle(cr, wx_offset + ww_surface, wy_offset, -1 * ww_surface, wh_surface);
    cairo_fill(cr);

    cairo_scale(cr, vc->gfx.scale_x, vc->gfx.scale_y);
    cairo_set_source_surface(cr, vc->gfx.surface, wx_offset / vc->gfx.scale_x, wy_offset / vc->gfx.scale_y);
    cairo_paint(cr);

    return TRUE;
}

static gboolean gd_motion_event(GtkWidget* widget, GdkEventMotion* motion, void* opaque)
{
    VirtualConsole*  vc = opaque;
    GtkDisplayState* s  = vc->s;
    int              fbx, fby;
    ...
    trace_gd_motion_event(ww_widget, wh_widget, gtk_widget_get_scale_factor(widget), fbx, fby);

    if (qemu_input_is_absolute(vc->gfx.dcl.con)) {
        if (fbx < 0 || fby < 0 || fbx >= surface_width(vc->gfx.ds) || fby >= surface_height(vc->gfx.ds)) {
            return TRUE;
        }
        qemu_input_queue_abs(vc->gfx.dcl.con, INPUT_AXIS_X, fbx, 0, surface_width(vc->gfx.ds));
        qemu_input_queue_abs(vc->gfx.dcl.con, INPUT_AXIS_Y, fby, 0, surface_height(vc->gfx.ds));
    ...
    if (!qemu_input_is_absolute(vc->gfx.dcl.con) && s->ptr_owner == vc) {
         ...
         * an invisible wall */
        if (xr <= geometry.x || xr - geometry.x >= geometry.width - 1 || yr <= geometry.y
            || yr - geometry.y >= geometry.height - 1)
        {
...
static gboolean gd_button_event(GtkWidget* widget, GdkEventButton* button, void* opaque)
{
    VirtualConsole*  vc = opaque;
    GtkDisplayState* s  = vc->s;
    InputButton      btn;
    ...
    if (button->button == 1 && button->type == GDK_BUTTON_PRESS && !qemu_input_is_absolute(vc->gfx.dcl.con)
        && s->ptr_owner != vc)
    {
        if (!vc->window) { gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(s->grab_item), TRUE); }
        else {
    ...
    if (button->button == 1) { btn = INPUT_BUTTON_LEFT; }
    ...
    if (button->type == GDK_2BUTTON_PRESS || button->type == GDK_3BUTTON_PRESS) { return TRUE; }

    qemu_input_queue_btn(vc->gfx.dcl.con, btn, button->type == GDK_BUTTON_PRESS);
    qemu_input_event_sync();
    return TRUE;
}

static gboolean gd_scroll_event(GtkWidget* widget, GdkEventScroll* scroll, void* opaque)
{
    VirtualConsole* vc = opaque;
    ...
    if (scroll->direction == GDK_SCROLL_UP) {
        ...
        if (!gdk_event_get_scroll_deltas((GdkEvent*)scroll, &delta_x, &delta_y)) { return TRUE; }
...
static gboolean gd_touch_event(GtkWidget* widget, GdkEventTouch* touch, void* opaque)
{
    VirtualConsole* vc       = opaque;
...
;
break;
default: warn_report("gtk: unexpected touch event type\n"); return FALSE;
    }

    console_handle_touch_event(vc->gfx.dcl.con, touch_slots, num_slot, surface_width(vc->gfx.ds),
                               surface_height(vc->gfx.ds), touch->x, touch->y, type, &error_warn);
    return TRUE;
    }
    ...
    static const guint16* gd_get_keymap(size_t* maplen)
    {
        ...
        if (GDK_IS_X11_DISPLAY(dpy)) {
            ...
            return qemu_xkeymap_mapping_table(gdk_x11_display_get_xdisplay(dpy), maplen);
        }
#endif
    ...
    static int gd_get_keycode(GdkEventKey* key)
    {
        ...
        switch (scancode & (KF_EXTENDED | 0xff)) {
            case 0x145: /* NUMLOCK */ return scancode & 0xff;
        }
    ...
    static gboolean gd_text_key_down(GtkWidget* widget, GdkEventKey* key, void* opaque)
    {
        VirtualConsole*  vc  = opaque;
        QemuTextConsole* con = QEMU_TEXT_CONSOLE(vc->gfx.dcl.con);

        if (key->keyval == GDK_KEY_Delete) { qemu_text_console_put_qcode(con, Q_KEY_CODE_DELETE, false); }
        else if (key->length) {
    ...
    static gboolean gd_key_event(GtkWidget* widget, GdkEventKey* key, void* opaque)
    {
        ...
        if (key->hardware_keycode == 0xff) { return false; }

        if (!vc->s->kbd_owner) {
            if (key->hardware_keycode == VK_LWIN || key->hardware_keycode == VK_RWIN) { return FALSE; }
        }
#endif
        ...
        {
            qkbd_state_key_event(vc->gfx.kbd, Q_KEY_CODE_PAUSE, key->type == GDK_KEY_PRESS);
            return TRUE;
        }
        ...
        trace_gd_key_event(vc->label, keycode, qcode, (key->type == GDK_KEY_PRESS) ? "down" : "up");

        qkbd_state_key_event(vc->gfx.kbd, qcode, key->type == GDK_KEY_PRESS);

        return TRUE;
    }

    static gboolean gd_grab_broken_event(GtkWidget* widget, GdkEventGrabBroken* event, void* opaque)
    {
#ifdef CONFIG_WIN32
        /*
    ...
    {
        if (event->type == GDK_MOTION_NOTIFY) { return gd_motion_event(widget, &event->motion, opaque); }
        return FALSE;
    }
    ...
    static void gd_menu_pause(GtkMenuItem* item, void* opaque)
    {
        ...
        if (s->external_pause_update) { return; }
        if (runstate_is_running()) { qmp_stop(NULL); }
        else {
            qmp_cont(NULL);
        }
    }

    static void gd_menu_reset(GtkMenuItem* item, void* opaque) { qmp_system_reset(NULL); }

    static void gd_menu_powerdown(GtkMenuItem* item, void* opaque) { qmp_system_powerdown(NULL); }

    static void gd_menu_quit(GtkMenuItem* item, void* opaque) { qmp_quit(NULL); }
    ...
    static gboolean gd_tab_window_close(GtkWidget* widget, GdkEvent* event, void* opaque)
    {
        VirtualConsole*  vc = opaque;
        GtkDisplayState* s  = vc->s;
        ...
        gtk_notebook_set_tab_label_text(GTK_NOTEBOOK(s->notebook), vc->tab_item, vc->label);
        gtk_widget_destroy(vc->window);
        vc->window = NULL;

        if (vc == gd_vc_find_by_menu(s)) { gtk_widget_grab_focus(vc->focus); }
    ...
    static gboolean gd_win_grab(void* opaque)
    {
        ...
        fprintf(stderr, "%s: %s\n", __func__, vc->label);
        if (vc->s->ptr_owner) { gd_ungrab_pointer(vc->s); }
        else {
    ...
    static void gd_menu_untabify(GtkMenuItem* item, void* opaque)
    {
        ...
        if (!vc->window) {
            ...
            g_signal_connect(vc->window, "delete-event", G_CALLBACK(gd_tab_window_close), vc);
    ...
    static void gd_menu_show_menubar(GtkMenuItem* item, void* opaque)
    {
        ...
        if (s->full_screen) { return; }

        if (gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(s->show_menubar_item))) { gtk_widget_show(s->menu_bar); }
        else {
    ...
    static void gd_menu_full_screen(GtkMenuItem* item, void* opaque)
    {
        ...
        if (!s->full_screen) {
            gtk_notebook_set_show_tabs(GTK_NOTEBOOK(s->notebook), FALSE);
            gtk_widget_hide(s->menu_bar);
            if (vc->type == GD_VC_GFX) { gtk_widget_set_size_request(vc->gfx.drawing_area, -1, -1); }
            ...
            if (gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(s->show_menubar_item))) {
    ...
    static void gd_menu_zoom_in(GtkMenuItem* item, void* opaque)
    {
        ...
        gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(s->zoom_fit_item), FALSE);
    ...
    static void gd_menu_zoom_fit(GtkMenuItem* item, void* opaque)
    {
        ...
        if (gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(s->zoom_fit_item))) { s->free_scale = TRUE; }
        else {
            s->free_scale   = FALSE;
            vc->gfx.scale_x = vc->gfx.preferred_scale;
            vc->gfx.scale_y = vc->gfx.preferred_scale;
    ...
    static void gd_grab_update(VirtualConsole* vc, bool kbd, bool ptr)
    {
        ...
        if (kbd) { caps |= GDK_SEAT_CAPABILITY_KEYBOARD; }
        if (ptr) {
            caps   |= GDK_SEAT_CAPABILITY_ALL_POINTING;
            cursor  = vc->s->null_cursor;
        }

        if (caps) { gdk_seat_grab(seat, window, caps, false, cursor, NULL, NULL, NULL); }
        else {
    ...
    static void gd_grab_pointer(VirtualConsole* vc, const char* reason)
    {
        ...
        gdk_device_get_position(gd_get_pointer(display), NULL, &vc->s->grab_x_root, &vc->s->grab_y_root);
        vc->s->ptr_owner = vc;
        gd_update_caption(vc->s);
    ...
    static void gd_ungrab_pointer(GtkDisplayState* s)
    {
        ...
        gdk_device_warp(gd_get_pointer(display), gtk_widget_get_screen(vc->gfx.drawing_area), vc->s->grab_x_root,
                        vc->s->grab_y_root);
        gd_update_caption(s);
    ...
    static void gd_change_page(GtkNotebook* nb, gpointer arg1, guint arg2, gpointer data)
    {
        GtkDisplayState* s = data;
        VirtualConsole*  vc;
        gboolean         on_vga;

        if (!gtk_widget_get_realized(s->notebook)) { return; }

        vc = gd_vc_find_by_page(s, arg2);
        if (!vc) { return; }
        gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(vc->menu_item), TRUE);
        on_vga = (vc->type == GD_VC_GFX && qemu_console_is_graphic(vc->gfx.dcl.con));
        if (!on_vga) { gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(s->grab_item), FALSE); }
    ...
    static gboolean gd_enter_event(GtkWidget* widget, GdkEventCrossing* crossing, gpointer opaque)
    {
        VirtualConsole*  vc = opaque;
        GtkDisplayState* s  = vc->s;

        if (gd_grab_on_hover(s)) { gd_grab_keyboard(vc, "grab-on-hover"); }
        return TRUE;
    }

    static gboolean gd_leave_event(GtkWidget* widget, GdkEventCrossing* crossing, gpointer opaque)
    {
        VirtualConsole*  vc = opaque;
        GtkDisplayState* s  = vc->s;

        if (gd_grab_on_hover(s)) { gd_ungrab_keyboard(s); }
        return TRUE;
    }

    static gboolean gd_focus_in_event(GtkWidget* widget, GdkEventFocus* event, gpointer opaque)
    {
        VirtualConsole* vc = opaque;
    ...
    static gboolean gd_focus_out_event(GtkWidget* widget, GdkEventFocus* event, gpointer opaque)
    {
        VirtualConsole*  vc = opaque;
        GtkDisplayState* s  = vc->s;
    ...
    static gboolean gd_configure(GtkWidget* widget, GdkEventConfigure* cfg, gpointer opaque)
    {
        VirtualConsole* vc = opaque;
    ...
    static GSList* gd_vc_menu_init(GtkDisplayState* s, VirtualConsole* vc, int idx, GSList* group, GtkWidget* view_menu)
    {
        vc->menu_item = gtk_radio_menu_item_new_with_mnemonic(group, vc->label);
        gtk_accel_group_connect(s->accel_group, GDK_KEY_1 + idx, HOTKEY_MODIFIERS, 0,
                                g_cclosure_new_swap(G_CALLBACK(gd_accel_switch_vc), vc, NULL));
        gtk_accel_label_set_accel(GTK_ACCEL_LABEL(gtk_bin_get_child(GTK_BIN(vc->menu_item))), GDK_KEY_1 + idx,
                                  HOTKEY_MODIFIERS);

        g_signal_connect(vc->menu_item, "activate", G_CALLBACK(gd_menu_switch_vc), s);
    ...
    static void gd_menu_copy(GtkMenuItem* item, void* opaque)
    {
        ...
        vte_terminal_copy_clipboard_format(VTE_TERMINAL(vc->vte.terminal), VTE_FORMAT_TEXT);
    #else
    ...
    static GSList* gd_vc_vte_init(GtkDisplayState* s, VirtualConsole* vc, Chardev* chr, int idx, GSList* group,
                                  GtkWidget* view_menu)
    {
        char           buffer[32];
        GtkWidget*     box;
        ...
        vte_terminal_set_size(VTE_TERMINAL(vc->vte.terminal), VC_TERM_X_MIN, VC_TERM_Y_MIN);

    #if VTE_CHECK_VERSION(0, 28, 0)
        vadjustment = gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(vc->vte.terminal));
    #else
        ...
        g_signal_connect(vadjustment, "changed", G_CALLBACK(gd_vc_adjustment_changed), vc);

        vc->type     = GD_VC_VTE;
        vc->tab_item = box;
        vc->focus    = vc->vte.terminal;
        gtk_notebook_append_page(GTK_NOTEBOOK(s->notebook), vc->tab_item, gtk_label_new(vc->label));
    ...
    static void gd_vcs_init(GtkDisplayState* s, GSList* group, GtkWidget* view_menu)
    {
        int i;
    ...
    {
        g_signal_connect(vc->gfx.drawing_area, "draw", G_CALLBACK(gd_draw_event), vc);
        if (qemu_console_is_graphic(vc->gfx.dcl.con)) {
            g_signal_connect(vc->gfx.drawing_area, "event", G_CALLBACK(gd_event), vc);
            g_signal_connect(vc->gfx.drawing_area, "button-press-event", G_CALLBACK(gd_button_event), vc);
            g_signal_connect(vc->gfx.drawing_area, "button-release-event", G_CALLBACK(gd_button_event), vc);
            g_signal_connect(vc->gfx.drawing_area, "scroll-event", G_CALLBACK(gd_scroll_event), vc);
            g_signal_connect(vc->gfx.drawing_area, "key-press-event", G_CALLBACK(gd_key_event), vc);
            g_signal_connect(vc->gfx.drawing_area, "key-release-event", G_CALLBACK(gd_key_event), vc);
            g_signal_connect(vc->gfx.drawing_area, "touch-event", G_CALLBACK(gd_touch_event), vc);

            g_signal_connect(vc->gfx.drawing_area, "enter-notify-event", G_CALLBACK(gd_enter_event), vc);
            g_signal_connect(vc->gfx.drawing_area, "leave-notify-event", G_CALLBACK(gd_leave_event), vc);
            g_signal_connect(vc->gfx.drawing_area, "focus-in-event", G_CALLBACK(gd_focus_in_event), vc);
            g_signal_connect(vc->gfx.drawing_area, "focus-out-event", G_CALLBACK(gd_focus_out_event), vc);
            g_signal_connect(vc->gfx.drawing_area, "configure-event", G_CALLBACK(gd_configure), vc);
            g_signal_connect(vc->gfx.drawing_area, "grab-broken-event", G_CALLBACK(gd_grab_broken_event), vc);
        }
        else {
            g_signal_connect(vc->gfx.drawing_area, "key-press-event", G_CALLBACK(gd_text_key_down), vc);
        }
    }

    static void gd_connect_signals(GtkDisplayState* s)
    {
        g_signal_connect(s->show_tabs_item, "activate", G_CALLBACK(gd_menu_show_tabs), s);
        g_signal_connect(s->untabify_item, "activate", G_CALLBACK(gd_menu_untabify), s);
        g_signal_connect(s->show_menubar_item, "activate", G_CALLBACK(gd_menu_show_menubar), s);

        g_signal_connect(s->window, "delete-event", G_CALLBACK(gd_window_close), s);

        g_signal_connect(s->pause_item, "activate", G_CALLBACK(gd_menu_pause), s);
        g_signal_connect(s->reset_item, "activate", G_CALLBACK(gd_menu_reset), s);
        g_signal_connect(s->powerdown_item, "activate", G_CALLBACK(gd_menu_powerdown), s);
        g_signal_connect(s->quit_item, "activate", G_CALLBACK(gd_menu_quit), s);
#if defined(CONFIG_VTE)
        g_signal_connect(s->copy_item, "activate", G_CALLBACK(gd_menu_copy), s);
#endif
        g_signal_connect(s->full_screen_item, "activate", G_CALLBACK(gd_menu_full_screen), s);
        g_signal_connect(s->zoom_in_item, "activate", G_CALLBACK(gd_menu_zoom_in), s);
        g_signal_connect(s->zoom_out_item, "activate", G_CALLBACK(gd_menu_zoom_out), s);
        g_signal_connect(s->zoom_fixed_item, "activate", G_CALLBACK(gd_menu_zoom_fixed), s);
        g_signal_connect(s->zoom_fit_item, "activate", G_CALLBACK(gd_menu_zoom_fit), s);
        g_signal_connect(s->grab_item, "activate", G_CALLBACK(gd_menu_grab_input), s);
        g_signal_connect(s->notebook, "switch-page", G_CALLBACK(gd_change_page), s);
    }
    ...
    static GtkWidget* gd_create_menu_machine(GtkDisplayState* s)
    {
        ...
        gtk_menu_item_set_accel_path(GTK_MENU_ITEM(s->quit_item), "<QEMU>/Machine/Quit");
        gtk_accel_map_add_entry("<QEMU>/Machine/Quit", GDK_KEY_q, HOTKEY_MODIFIERS);
        gtk_menu_shell_append(GTK_MENU_SHELL(machine_menu), s->quit_item);

        return machine_menu;
    }

    static bool gd_scale_valid(double scale) { return scale >= VC_SCALE_MIN && scale <= VC_SCALE_MAX; }

    static GSList* gd_vc_gfx_init(GtkDisplayState* s, VirtualConsole* vc, QemuConsole* con, int idx, GSList* group,
                                  GtkWidget* view_menu)
    {
        ...
        if (s->opts->u.gtk.has_scale) {
            if (gd_scale_valid(s->opts->u.gtk.scale)) { vc->gfx.preferred_scale = s->opts->u.gtk.scale; }
            else {
                error_report("Invalid scale value %lf given, being ignored", s->opts->u.gtk.scale);
        ...
        gtk_widget_add_events(vc->gfx.drawing_area, GDK_POINTER_MOTION_MASK | GDK_BUTTON_PRESS_MASK
                                                        | GDK_BUTTON_RELEASE_MASK | GDK_BUTTON_MOTION_MASK
                                                        | GDK_TOUCH_MASK | GDK_ENTER_NOTIFY_MASK | GDK_LEAVE_NOTIFY_MASK
        ...
        if (dpy_ui_info_supported(vc->gfx.dcl.con)) { zoom_to_fit = true; }
        if (s->opts->u.gtk.has_zoom_to_fit) { zoom_to_fit = s->opts->u.gtk.zoom_to_fit; }
        if (zoom_to_fit) {
        ...
        if (s->opts->u.gtk.has_keep_aspect_ratio) { s->keep_aspect_ratio = s->opts->u.gtk.keep_aspect_ratio; }
    ...
    static GtkWidget* gd_create_menu_view(GtkDisplayState* s, DisplayOptions* opts)
    {
        ...
        gtk_accel_label_set_accel(GTK_ACCEL_LABEL(gtk_bin_get_child(GTK_BIN(s->full_screen_item))), GDK_KEY_f,
                                  HOTKEY_MODIFIERS);
        ...
        gtk_menu_item_set_accel_path(GTK_MENU_ITEM(s->zoom_in_item), "<QEMU>/View/Zoom In");
        gtk_accel_map_add_entry("<QEMU>/View/Zoom In", GDK_KEY_plus, HOTKEY_MODIFIERS);
        ...
        gtk_menu_item_set_accel_path(GTK_MENU_ITEM(s->zoom_out_item), "<QEMU>/View/Zoom Out");
        gtk_accel_map_add_entry("<QEMU>/View/Zoom Out", GDK_KEY_minus, HOTKEY_MODIFIERS);
        ...
        gtk_menu_item_set_accel_path(GTK_MENU_ITEM(s->zoom_fixed_item), "<QEMU>/View/Zoom Fixed");
        gtk_accel_map_add_entry("<QEMU>/View/Zoom Fixed", GDK_KEY_0, HOTKEY_MODIFIERS);
        ...
        gtk_menu_item_set_accel_path(GTK_MENU_ITEM(s->grab_item), "<QEMU>/View/Grab Input");
        gtk_accel_map_add_entry("<QEMU>/View/Grab Input", GDK_KEY_g, HOTKEY_MODIFIERS);
        ...
        for (vc = 0;; vc++) {
            ...
            if (!con) { break; }
            group = gd_vc_gfx_init(s, &s->vc[vc], con, vc, group, view_menu);
            s->nb_vcs++;
        }
        ...
        s->show_menubar_item = gtk_check_menu_item_new_with_mnemonic(_("Show Menubar"));
        gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(s->show_menubar_item),
                                       !opts->u.gtk.has_show_menubar || opts->u.gtk.show_menubar);
        ...
        gtk_accel_label_set_accel(GTK_ACCEL_LABEL(gtk_bin_get_child(GTK_BIN(s->show_menubar_item))), GDK_KEY_m,
                                  HOTKEY_MODIFIERS);
    ...
    static void gd_create_menus(GtkDisplayState* s, DisplayOptions* opts)
    {
        ...
        gtk_menu_item_set_submenu(GTK_MENU_ITEM(s->machine_menu_item), s->machine_menu);
    ...
    static void gtk_display_init(DisplayState* ds, DisplayOptions* opts)
    {
        ...
        if (s->opts->has_show_cursor && s->opts->show_cursor) { s->null_cursor = NULL; /* default pointer */ }
        else {
            s->null_cursor = gdk_cursor_new_for_display(window_display, GDK_BLANK_CURSOR);
        }
        ...
        for (idx = 0;; idx++) {
            ...
            if (!con) { break; }
            if (s->vc[idx].type == GD_VC_GFX) { gtk_widget_realize(s->vc[idx].gfx.drawing_area); }
        }

        if (opts->u.gtk.has_show_menubar && !opts->u.gtk.show_menubar) { gtk_widget_hide(s->menu_bar); }

        vc = gd_vc_find_current(s);
        gtk_widget_set_sensitive(s->view_menu, vc != NULL);
#ifdef CONFIG_VTE
        gtk_widget_set_sensitive(s->copy_item, vc && vc->type == GD_VC_VTE);
#endif

        if (opts->has_full_screen && opts->full_screen) { gtk_menu_item_activate(GTK_MENU_ITEM(s->full_screen_item)); }
        if (opts->u.gtk.has_grab_on_hover && opts->u.gtk.grab_on_hover) {
            gtk_menu_item_activate(GTK_MENU_ITEM(s->grab_on_hover_item));
        }
        if (opts->u.gtk.has_show_tabs && opts->u.gtk.show_tabs) {
    ...
    static void register_gtk(void) { qemu_display_register(&qemu_display_gtk); }
