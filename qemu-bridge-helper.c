...
static int parse_acl_file(const char* filename, ACLList* acl_list)
{
    ...
    while (fgets(line, sizeof(line), f) != NULL) {
        ...
        *arg = 0;
        ...
        while (arg != argend && g_ascii_isspace(*(argend - 1))) { argend--; }
        *argend = 0;
        ...
        if (strcmp(cmd, "deny") == 0) {
            ...
            if (strcmp(arg, "all") == 0) { acl_rule->type = ACL_DENY_ALL; }
            else {
            ...
            acl_rule = g_malloc(sizeof(*acl_rule));
            if (strcmp(arg, "all") == 0) { acl_rule->type = ACL_ALLOW_ALL; }
            else {
...
static bool has_vnet_hdr(int fd)
{
    ...
    if (ioctl(fd, TUNGETFEATURES, &features) == -1) { return false; }

    if (!(features & IFF_VNET_HDR)) { return false; }
...
static int drop_privileges(void)
{
    ...
    if (capng_update(CAPNG_ADD, CAPNG_EFFECTIVE | CAPNG_PERMITTED, CAP_NET_ADMIN) < 0) { return -1; }

    /* change to calling user's real uid and gid, retaining supplemental
     * groups and CAP_NET_ADMIN */
    if (capng_change_id(getuid(), getgid(), CAPNG_CLEAR_BOUNDING)) { return -1; }
...
int main(int argc, char** argv)
{
    ...
    for (index = 1; index < argc; index++) {
        if (strcmp(argv[index], "--use-vnet") == 0) { use_vnet = 1; }
    ...
    QSIMPLEQ_FOREACH (acl_rule, &acl_list, entry) {
        switch (acl_rule->type) {
            case ACL_ALLOW_ALL: access_allowed = 1; break;
            case ACL_ALLOW:
                if (strcmp(bridge, acl_rule->iface) == 0) { access_allowed = 1; }
                break;
            case ACL_DENY_ALL: access_denied = 1; break;
            case ACL_DENY:
                if (strcmp(bridge, acl_rule->iface) == 0) { access_denied = 1; }
                break;
        }
    }
    ...
    if (use_vnet && has_vnet_hdr(fd)) { ifr.ifr_flags |= IFF_VNET_HDR; }
