...
static int qio_dns_resolver_lookup_sync_inet(QIODNSResolver* resolver, SocketAddress* addr, size_t* naddrs,
                                             SocketAddress*** addrs, Error** errp)
{
    ...
    *addrs  = NULL;
    ...
    if (iaddr->has_numeric && iaddr->numeric) { ai.ai_flags |= AI_NUMERICHOST | AI_NUMERICSERV; }
    ...
    if (iaddr->port != NULL) { pstrcpy(port, sizeof(port), iaddr->port); }
    else {
        port[0] = '\0';
    }

    rc = getaddrinfo(strlen(iaddr->host) ? iaddr->host : NULL, strlen(port) ? port : NULL, &ai, &res);
    if (rc != 0) {
        error_setg(errp, "address resolution failed for %s:%s: %s", iaddr->host, port, gai_strerror(rc));
        return -1;
    }

    for (e = res; e != NULL; e = e->ai_next) { (*naddrs)++; }
    ...
    *addrs = g_new0(SocketAddress*, *naddrs);
    ...
    for (i = 0, e = res; e != NULL; i++, e = e->ai_next) {
        ...
        newaddr->u.inet.host = g_strdup(uaddr), newaddr->u.inet.port = g_strdup(uport),
        newaddr->u.inet.has_numeric = true, newaddr->u.inet.numeric = true, (*addrs)[i] = newaddr;
    }
    freeaddrinfo(res);
    return 0;
}

static int qio_dns_resolver_lookup_sync_nop(QIODNSResolver* resolver, SocketAddress* addr, size_t* naddrs,
                                            SocketAddress*** addrs, Error** errp)
{
    *naddrs     = 1;
...
static void qio_dns_resolver_lookup_data_free(gpointer opaque)
{
    ...
    for (i = 0; i < data->naddrs; i++) { qapi_free_SocketAddress(data->addrs[i]); }

    g_free(data->addrs);
    g_free(data);
}

static void qio_dns_resolver_lookup_worker(QIOTask* task, gpointer opaque)
{
    QIODNSResolver*                  resolver = QIO_DNS_RESOLVER(qio_task_get_source(task));
    struct QIODNSResolverLookupData* data     = opaque;
    Error*                           err      = NULL;

    qio_dns_resolver_lookup_sync(resolver, data->addr, &data->naddrs, &data->addrs, &err);
    if (err) { qio_task_set_error(task, err); }
    else {
...
{
    QIOTask*                         task;
    struct QIODNSResolverLookupData* data = g_new0(struct QIODNSResolverLookupData, 1);

    data->addr = QAPI_CLONE(SocketAddress, addr);

    task = qio_task_new(OBJECT(resolver), func, opaque, notify);

    qio_task_run_in_thread(task, qio_dns_resolver_lookup_worker, data, qio_dns_resolver_lookup_data_free, NULL);
}

void qio_dns_resolver_lookup_result(QIODNSResolver* resolver, QIOTask* task, size_t* naddrs, SocketAddress*** addrs)
{
    struct QIODNSResolverLookupData* data = qio_task_get_result_pointer(task);
    size_t                           i;
    ...
    *addrs  = g_new0(SocketAddress*, data->naddrs);
    for (i = 0; i < data->naddrs; i++) { (*addrs)[i] = QAPI_CLONE(SocketAddress, data->addrs[i]); }
}
