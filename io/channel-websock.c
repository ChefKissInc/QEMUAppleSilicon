...
static void G_GNUC_PRINTF(2, 3) qio_channel_websock_handshake_send_res(QIOChannelWebsock* ioc, const char* resmsg, ...)
{
    va_list vargs;
    char*   response;
    size_t  responselen;
...
static size_t qio_channel_websock_extract_headers(QIOChannelWebsock* ioc, char* buffer,
                                                  QIOChannelWebsockHTTPHeader* hdrs, size_t nhdrsalloc, Error** errp)
{
    char * nl, *sep, *tmp;
    size_t nhdrs = 0;
    ...
    *tmp = '\0';
    ...
    do {
        ...
        *sep = '\0';
        ...
        for (tmp = hdr->name; *tmp; tmp++) { *tmp = g_ascii_tolower(*tmp); }

        if (nl) { buffer = nl + strlen(QIO_CHANNEL_WEBSOCK_HANDSHAKE_DELIM); }
    }
    while (nl != NULL);
...
bad_request:
    qio_channel_websock_handshake_send_res_err(ioc, QIO_CHANNEL_WEBSOCK_HANDSHAKE_RES_BAD_REQUEST);
    return 0;
}
...
static const char* qio_channel_websock_find_header(QIOChannelWebsockHTTPHeader* hdrs, size_t nhdrs, const char* name)
{
    ...
    for (i = 0; i < nhdrs; i++) {
        if (g_str_equal(hdrs[i].name, name)) { return hdrs[i].value; }
    }

    return NULL;
}

static void qio_channel_websock_handshake_send_res_ok(QIOChannelWebsock* ioc, const char* key, const bool use_protocols,
                                                      Error** errp)
{
    ...
    g_strlcat(combined_key, QIO_CHANNEL_WEBSOCK_GUID,
              QIO_CHANNEL_WEBSOCK_CLIENT_KEY_LEN + QIO_CHANNEL_WEBSOCK_GUID_LEN + 1);

    /* hash and encode it */
    if (qcrypto_hash_base64(QCRYPTO_HASH_ALGO_SHA1, combined_key,
                            QIO_CHANNEL_WEBSOCK_CLIENT_KEY_LEN + QIO_CHANNEL_WEBSOCK_GUID_LEN, &accept, errp)
        < 0)
    {
    ...
    if (use_protocols) {
        qio_channel_websock_handshake_send_res(ioc, QIO_CHANNEL_WEBSOCK_HANDSHAKE_WITH_PROTO_RES_OK, date, accept);
    }
    else {
...
static void qio_channel_websock_handshake_process(QIOChannelWebsock* ioc, char* buffer, Error** errp)
{
    ...
    trace_qio_channel_websock_http_request(ioc, protocols, version, host, connection, upgrade, key);

    if (protocols) {
        if (!g_strrstr(protocols, QIO_CHANNEL_WEBSOCK_PROTOCOL_BINARY)) {
            error_setg(errp, "No '%s' protocol is supported by client '%s'", QIO_CHANNEL_WEBSOCK_PROTOCOL_BINARY,
                       protocols);
            goto bad_request;
        }
    }
    ...
    if (!g_str_equal(version, QIO_CHANNEL_WEBSOCK_SUPPORTED_VERSION)) {
        error_setg(errp, "Version '%s' is not supported by client '%s'", QIO_CHANNEL_WEBSOCK_SUPPORTED_VERSION,
                   version);
        goto bad_request;
    }
    ...
    for (i = 0; connectionv != NULL && connectionv[i] != NULL; i++) {
        ...
        if (strcasecmp(connectionv[i], QIO_CHANNEL_WEBSOCK_CONNECTION_UPGRADE) == 0) { upgraded = true; }
    }
...
static int qio_channel_websock_handshake_read(QIOChannelWebsock* ioc, Error** errp)
{
    ...
    if (!handshake_end) {
        if (ioc->encinput.offset >= 4096) {
            ...
            error_setg(errp, "End of headers not found before connection closed");
            return -1;
        }
        return 0;
    }
    *handshake_end = '\0';
    ...
    buffer_advance(&ioc->encinput,
                   handshake_end - (char*)ioc->encinput.buffer + strlen(QIO_CHANNEL_WEBSOCK_HANDSHAKE_END));
    return 1;
}
...
static gboolean qio_channel_websock_handshake_io(QIOChannel* ioc, GIOCondition condition, gpointer user_data)
{
    ...
    wioc->hs_io_tag = qio_channel_add_watch(wioc->master, G_IO_OUT, qio_channel_websock_handshake_send, task, NULL);
    return FALSE;
}

static void qio_channel_websock_encode(QIOChannelWebsock* ioc, uint8_t opcode, const struct iovec* iov, size_t niov,
                                       size_t size)
{
    size_t header_size;
    size_t i;
...
static void qio_channel_websock_write_close(QIOChannelWebsock* ioc, uint16_t code, const char* reason)
{
    ...
    qio_channel_websock_encode(ioc, QIO_CHANNEL_WEBSOCK_OPCODE_CLOSE, iov, niov, size);
...
static int qio_channel_websock_decode_header(QIOChannelWebsock* ioc, Error** errp)
{
    ...
    if (!fin) {
        if (opcode != QIO_CHANNEL_WEBSOCK_OPCODE_BINARY_FRAME) {
            ...
            qio_channel_websock_write_close(ioc, QIO_CHANNEL_WEBSOCK_STATUS_POLICY,
                                            "only binary frames may be fragmented");
            return -1;
        }
    }
    else {
        if (opcode != QIO_CHANNEL_WEBSOCK_OPCODE_BINARY_FRAME && opcode != QIO_CHANNEL_WEBSOCK_OPCODE_CLOSE
            && opcode != QIO_CHANNEL_WEBSOCK_OPCODE_PING && opcode != QIO_CHANNEL_WEBSOCK_OPCODE_PONG)
        {
            error_setg(errp,
                       "unsupported opcode: 0x%04x; only binary, close, "
                       "ping, and pong websocket frames are supported",
                       opcode);
            qio_channel_websock_write_close(ioc, QIO_CHANNEL_WEBSOCK_STATUS_INVALID_DATA,
...
static int qio_channel_websock_decode_payload(QIOChannelWebsock* ioc, Error** errp)
{
    ...
    if (ioc->payload_remain) {
        ...
        if (ioc->encinput.offset < ioc->payload_remain) {
            ...
            if (ioc->opcode & QIO_CHANNEL_WEBSOCK_CONTROL_OPCODE_MASK) { return QIO_CHANNEL_ERR_BLOCK; }
            payload_len = ioc->encinput.offset - (ioc->encinput.offset % 4);
        }
        else {
            payload_len = ioc->payload_remain;
        }
        if (payload_len == 0) { return QIO_CHANNEL_ERR_BLOCK; }
        ...
        for (i = 0; i < payload_len / 4; i++) { payload32[i] ^= ioc->mask.u; }
        /* process the remaining bytes (if any) */
        for (i *= 4; i < payload_len; i++) { ioc->encinput.buffer[i] ^= ioc->mask.c[i % 4]; }
    }
    ...
    if (ioc->opcode == QIO_CHANNEL_WEBSOCK_OPCODE_BINARY_FRAME) {
        ...
        if (payload_len) {
            /* echo client status */
            struct iovec iov = {.iov_base = ioc->encinput.buffer, .iov_len = ioc->encinput.offset};
            qio_channel_websock_encode(ioc, QIO_CHANNEL_WEBSOCK_OPCODE_CLOSE, &iov, 1, iov.iov_len);
            qio_channel_websock_write_wire(ioc, NULL);
            qio_channel_shutdown(ioc->master, QIO_CHANNEL_SHUTDOWN_BOTH, NULL);
        }
        else {
        ...
        if (ioc->pong_remain == 0) {
            struct iovec iov = {.iov_base = ioc->encinput.buffer, .iov_len = ioc->encinput.offset};
            qio_channel_websock_encode(ioc, QIO_CHANNEL_WEBSOCK_OPCODE_PONG, &iov, 1, iov.iov_len);
            ioc->pong_remain = ioc->encoutput.offset;
        }
    } /* pong frames are ignored */

    if (payload_len) { buffer_advance(&ioc->encinput, payload_len); }
    return 0;
}
...
void qio_channel_websock_handshake(QIOChannelWebsock* ioc, QIOTaskFunc func, gpointer opaque, GDestroyNotify destroy)
{
    ...
    trace_qio_channel_websock_handshake_pending(ioc, G_IO_IN);
    ioc->hs_io_tag = qio_channel_add_watch(ioc->master, G_IO_IN, qio_channel_websock_handshake_io, task, NULL);
}
...
static void qio_channel_websock_finalize(Object* obj)
{
    ...
    if (ioc->hs_io_tag) { g_source_remove(ioc->hs_io_tag); }
    if (ioc->io_tag) { g_source_remove(ioc->io_tag); }
    if (ioc->io_err) { error_free(ioc->io_err); }
...
static ssize_t qio_channel_websock_write_wire(QIOChannelWebsock* ioc, Error** errp)
{
    ...
    while (ioc->encoutput.offset > 0) {
        ...
        if (ret < 0) {
            if (ret == QIO_CHANNEL_ERR_BLOCK && done > 0) { return done; }
            else {
                return ret;
            }
        }
        ...
        if (ioc->pong_remain < ret) { ioc->pong_remain = 0; }
        else {
            ioc->pong_remain -= ret;
        }
    }
...
static void qio_channel_websock_set_watch(QIOChannelWebsock* ioc)
{
    ...
    if (ioc->io_err) { return; }

    if (ioc->encoutput.offset) { cond |= G_IO_OUT; }
    if (ioc->encinput.offset < QIO_CHANNEL_WEBSOCK_MAX_BUFFER && !ioc->io_eof) { cond |= G_IO_IN; }

    if (cond) {
        object_ref(OBJECT(ioc));
        ioc->io_tag =
            qio_channel_add_watch(ioc->master, cond, qio_channel_websock_flush, ioc, qio_channel_websock_flush_free);
    }
}
                                         ...
                                         int flags, Error** errp)
{
    ...
    for (i = 0; i < niov; i++) {
        size_t want = iov[i].iov_len;
        if (want > (wioc->rawinput.offset - got)) { want = (wioc->rawinput.offset - got); }

        memcpy(iov[i].iov_base, wioc->rawinput.buffer + got, want);
        got += want;

        if (want < iov[i].iov_len) { break; }
    }
                                          ...
                                          int flags, Error** errp)
{
    ...
    avail = wioc->encoutput.offset >= QIO_CHANNEL_WEBSOCK_MAX_BUFFER ?
                0 :
                (QIO_CHANNEL_WEBSOCK_MAX_BUFFER - wioc->encoutput.offset);
    if (want > avail) { want = avail; }

    if (want) { qio_channel_websock_encode(wioc, QIO_CHANNEL_WEBSOCK_OPCODE_BINARY_FRAME, iov, niov, want); }
    ...
    if (want == 0) { return QIO_CHANNEL_ERR_BLOCK; }
...
static int qio_channel_websock_close(QIOChannel* ioc, Error** errp)
{
    ...
    if (wioc->hs_io_tag) { g_clear_handle_id(&wioc->hs_io_tag, g_source_remove); }
    if (wioc->io_tag) { g_clear_handle_id(&wioc->io_tag, g_source_remove); }
    if (wioc->io_err) { g_clear_pointer(&wioc->io_err, error_free); }
...
static gboolean qio_channel_websock_source_check(GSource* source)
{
    ...
    if (wsource->wioc->rawinput.offset) { cond |= G_IO_IN; }
    if (wsource->wioc->encoutput.offset < QIO_CHANNEL_WEBSOCK_MAX_BUFFER) { cond |= G_IO_OUT; }
    if (wsource->wioc->io_eof) { cond |= G_IO_HUP; }
    if (wsource->wioc->io_err) { cond |= G_IO_ERR; }
...
GSourceFuncs qio_channel_websock_source_funcs = {qio_channel_websock_source_prepare, qio_channel_websock_source_check,
...
static void qio_channel_websock_class_init(ObjectClass* klass, const void* class_data G_GNUC_UNUSED)
{
