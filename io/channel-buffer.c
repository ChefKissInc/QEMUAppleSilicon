...
static ssize_t qio_channel_buffer_readv(QIOChannel* ioc, const struct iovec* iov, size_t niov, int** fds, size_t* nfds,
                                        int flags, Error** errp)
{
    ...
    for (i = 0; i < niov; i++) {
        ...
        if (bioc->offset >= bioc->usage) { break; }
        if ((bioc->offset + want) > bioc->usage) { want = bioc->usage - bioc->offset; }
...
static ssize_t qio_channel_buffer_writev(QIOChannel* ioc, const struct iovec* iov, size_t niov, int* fds, size_t nfds,
                                         int flags, Error** errp)
{
    QIOChannelBuffer* bioc = QIO_CHANNEL_BUFFER(ioc);
    ssize_t           ret  = 0;
    ...
    for (i = 0; i < niov; i++) { towrite += iov[i].iov_len; }
...
static int qio_channel_buffer_set_blocking(QIOChannel* ioc G_GNUC_UNUSED, bool enabled G_GNUC_UNUSED,
                                           Error** errp G_GNUC_UNUSED)
{ return 0; }

static off_t qio_channel_buffer_seek(QIOChannel* ioc, off_t offset, int whence, Error** errp)
{
    QIOChannelBuffer* bioc = QIO_CHANNEL_BUFFER(ioc);
...
static int qio_channel_buffer_close(QIOChannel* ioc, Error** errp)
{
    QIOChannelBuffer* bioc = QIO_CHANNEL_BUFFER(ioc);
...
static gboolean qio_channel_buffer_source_prepare(GSource* source, gint* timeout)
{
...
static gboolean qio_channel_buffer_source_check(GSource* source)
{
    QIOChannelBufferSource* bsource = (QIOChannelBufferSource*)source;
...
static gboolean qio_channel_buffer_source_dispatch(GSource* source, GSourceFunc callback, gpointer user_data)
{
    ...
    return (*func)(QIO_CHANNEL(bsource->bioc), ((G_IO_IN | G_IO_OUT) & bsource->condition), user_data);
}

static void qio_channel_buffer_source_finalize(GSource* source)
{
...
GSourceFuncs qio_channel_buffer_source_funcs = {qio_channel_buffer_source_prepare, qio_channel_buffer_source_check,
                                                qio_channel_buffer_source_dispatch, qio_channel_buffer_source_finalize};

static GSource* qio_channel_buffer_create_watch(QIOChannel* ioc, GIOCondition condition)
{
    QIOChannelBuffer*       bioc = QIO_CHANNEL_BUFFER(ioc);
    QIOChannelBufferSource* ssource;
    GSource*                source;

    source  = g_source_new(&qio_channel_buffer_source_funcs, sizeof(QIOChannelBufferSource));
...
static void qio_channel_buffer_class_init(ObjectClass* klass, const void* class_data G_GNUC_UNUSED)
{
