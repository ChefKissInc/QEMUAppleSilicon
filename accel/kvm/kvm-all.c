...
static const KVMCapabilityInfo kvm_required_capabilites[] = {KVM_CAP_INFO(USER_MEMORY),
 ...
 */
static QLIST_HEAD(, KVMResampleFd) kvm_resample_fd_list = QLIST_HEAD_INITIALIZER(kvm_resample_fd_list);
...
static bool kvm_slots_grow(KVMMemoryListener* kml, unsigned int nr_slots_new)
{
    ...
    if (nr_slots_new > kvm_state->nr_slots_max) { nr_slots_new = kvm_state->nr_slots_max; }
    ...
    if (cur == 0) { slots = g_new0(KVMSlot, nr_slots_new); }
    else {
        assert(kml->slots);
    ...
    for (i = cur; i < nr_slots_new; i++) { slots[i].slot = i; }
...
static bool kvm_slots_double(KVMMemoryListener* kml) { return kvm_slots_grow(kml, kml->nr_slots_allocated * 2); }
...
static KVMSlot* kvm_get_free_slot(KVMMemoryListener* kml)
{
    ...
    if (kvm_slots_double(kml)) { return &kml->slots[n]; }
...
static KVMSlot* kvm_lookup_matching_slot(KVMMemoryListener* kml, hwaddr start_addr, hwaddr size)
{
    int i;
    ...
    for (i = 0; i < kml->nr_slots_allocated; i++) {
        ...
        if (start_addr == mem->start_addr && size == mem->memory_size) { return mem; }
    }
...
static int kvm_set_user_memory_region(KVMMemoryListener* kml, KVMSlot* slot, bool new)
{
    ...
    if (slot->memory_size && !new && (mem.flags ^ slot->old_flags) & KVM_MEM_READONLY) {
        ...
        if (kvm_guest_memfd_supported) { ret = kvm_vm_ioctl(s, KVM_SET_USER_MEMORY_REGION2, &mem); }
        else {
...
err:
    trace_kvm_set_user_memory(mem.slot >> 16, (uint16_t)mem.slot, mem.flags, mem.guest_phys_addr, mem.memory_size,
                              mem.userspace_addr, mem.guest_memfd, mem.guest_memfd_offset, ret);
    if (ret < 0) {
        if (kvm_guest_memfd_supported) {
            error_report("%s: KVM_SET_USER_MEMORY_REGION2 failed, slot=%d,"
                         ...
                         __func__, mem.slot, slot->start_addr, (uint64_t)mem.memory_size, mem.flags, mem.guest_memfd,
                         (uint64_t)mem.guest_memfd_offset, strerror(errno));
        }
        else {
            error_report("%s: KVM_SET_USER_MEMORY_REGION failed, slot=%d,"
                         " start=0x%" PRIx64 ", size=0x%" PRIx64 ": %s",
                         __func__, mem.slot, slot->start_addr, (uint64_t)mem.memory_size, strerror(errno));
        }
    }
    return ret;
}
...
static void kvm_reset_parked_vcpus(KVMState* s)
{
    ...
    QLIST_FOREACH (cpu, &s->kvm_parked_vcpus, node) { kvm_arch_reset_parked_vcpu(cpu->vcpu_id, cpu->kvm_fd); }
}
...
static int do_kvm_destroy_vcpu(CPUState* cpu)
{
    ...
    if (s->coalesced_mmio_ring == (void*)cpu->kvm_run + PAGE_SIZE) { s->coalesced_mmio_ring = NULL; }
...
int kvm_init_vcpu(CPUState* cpu, Error** errp)
{
    ...
    ret = kvm_create_vcpu(cpu);
    if (ret < 0) {
        error_setg_errno(errp, -ret, "kvm_init_vcpu: kvm_create_vcpu failed (%lu)", kvm_arch_vcpu_id(cpu));
        goto err;
    }
    ...
    if (mmap_size < 0) {
        ret = mmap_size;
        error_setg_errno(errp, -mmap_size, "kvm_init_vcpu: KVM_GET_VCPU_MMAP_SIZE failed");
        goto err;
    }
    ...
    if (cpu->kvm_run == MAP_FAILED) {
        ret = -errno;
        error_setg_errno(errp, ret, "kvm_init_vcpu: mmap'ing vcpu state failed (%lu)", kvm_arch_vcpu_id(cpu));
        goto err;
    }
    ...
    if (s->kvm_dirty_ring_size) {
        ...
        cpu->kvm_dirty_gfns = mmap(NULL, s->kvm_dirty_ring_bytes, PROT_READ | PROT_WRITE, MAP_SHARED, cpu->kvm_fd,
    ...
    if (ret < 0) {
        error_setg_errno(errp, -ret, "kvm_init_vcpu: kvm_arch_init_vcpu failed (%lu)", kvm_arch_vcpu_id(cpu));
    }
...
void kvm_close(void)
{
    ...
    if (!kvm_state || kvm_state->fd == -1) { return; }
...
static int kvm_mem_flags(MemoryRegion* mr)
{
    ...
    if (memory_region_get_dirty_log_mask(mr) != 0) { flags |= KVM_MEM_LOG_DIRTY_PAGES; }
    if (readonly && kvm_readonly_mem_allowed) { flags |= KVM_MEM_READONLY; }
...
/* Called with KVMMemoryListener.slots_lock held */
static int kvm_slot_update_flags(KVMMemoryListener* kml, KVMSlot* mem, MemoryRegion* mr)
{
    mem->flags = kvm_mem_flags(mr);

    /* If nothing changed effectively, no need to issue ioctl */
    if (mem->flags == mem->old_flags) { return 0; }

    kvm_slot_init_dirty_bitmap(mem);
    return kvm_set_user_memory_region(kml, mem, false);
}

static int kvm_section_update_flags(KVMMemoryListener* kml, MemoryRegionSection* section)
{
...
static void kvm_slot_reset_dirty_pages(KVMSlot* slot) { memset(slot->dirty_bmap, 0, slot->dirty_bmap_size); }
...
static void kvm_slot_init_dirty_bitmap(KVMSlot* mem)
{
    if (!(mem->flags & KVM_MEM_LOG_DIRTY_PAGES) || mem->dirty_bmap) { return; }
...
static bool kvm_slot_get_dirty_log(KVMState* s, KVMSlot* slot)
{
    ...
    if (ret) { error_report_once("%s: KVM_GET_DIRTY_LOG failed with %d", __func__, ret); }
    return ret == 0;
}

/* Should be with all slots_lock held for the address spaces. */
static void kvm_dirty_ring_mark_page(KVMState* s, uint32_t as_id, uint32_t slot_id, uint64_t offset)
{
    KVMMemoryListener* kml;
    KVMSlot*           mem;

    if (as_id >= s->nr_as) { return; }

    kml = s->as[as_id].ml;
    mem = &kml->slots[slot_id];

    if (!mem->memory_size || offset >= (mem->memory_size / qemu_real_host_page_size())) { return; }
...
static uint32_t kvm_dirty_ring_reap_one(KVMState* s, CPUState* cpu)
{
    ...
    while (true) {
        ...
        if (!dirty_gfn_is_dirtied(cur)) { break; }
...
static uint64_t kvm_dirty_ring_reap_locked(KVMState* s, CPUState* cpu)
{
    ...
    if (cpu) { total = kvm_dirty_ring_reap_one(s, cpu); }
    else {
        CPU_FOREACH (cpu) { total += kvm_dirty_ring_reap_one(s, cpu); }
    }
    ...
    if (total) { trace_kvm_dirty_ring_reap(total, stamp / 1000); }
...
static void do_kvm_cpu_synchronize_kick(CPUState* cpu, run_on_cpu_data arg) { /* No need to do anything */ }
...
static void kvm_cpu_synchronize_kick_all(void)
{
    ...
    CPU_FOREACH (cpu) { run_on_cpu(cpu, do_kvm_cpu_synchronize_kick, RUN_ON_CPU_NULL); }
}
...
static void kvm_physical_sync_dirty_bitmap(KVMMemoryListener* kml, MemoryRegionSection* section)
{
    ...
    while (size) {
        ...
        if (kvm_slot_get_dirty_log(s, mem)) { kvm_slot_sync_dirty_pages(mem); }
        start_addr += slot_size;
        size       -= slot_size;
    }
}
...
static int kvm_log_clear_one_slot(KVMSlot* mem, int as_id, uint64_t start, uint64_t size)
{
     ...
     */
    bmap_npages = DIV_ROUND_UP(size + start_delta, KVM_CLEAR_LOG_ALIGN) << KVM_CLEAR_LOG_SHIFT;
    end         = mem->memory_size / psize;
    if (bmap_npages > end - bmap_start) { bmap_npages = end - bmap_start; }
    start_delta /= psize;
    ...
    if (start_delta || bmap_npages - size / psize) {
        ...
        bitmap_copy_with_src_offset(bmap_clear, mem->dirty_bmap, bmap_start, start_delta + size / psize);
        /*
    ...
    if (ret < 0 && ret != -ENOENT) {
        error_report("%s: KVM_CLEAR_DIRTY_LOG failed, slot=%d, "
                     ...
                     __func__, d.slot, (uint64_t)d.first_page, (uint32_t)d.num_pages, ret);
    }
    else {
        ret = 0;
     ...
     */
    bitmap_clear(mem->dirty_bmap, bmap_start + start_delta, size / psize);
 ...
 */
static int kvm_physical_log_clear(KVMMemoryListener* kml, MemoryRegionSection* section)
{
    KVMState* s = kvm_state;
    ...
    for (i = 0; i < kml->nr_slots_allocated; i++) {
        ...
        if (!mem->memory_size || mem->start_addr > start + size - 1 || start > mem->start_addr + mem->memory_size - 1) {
            continue;
        }
...
static void kvm_coalesce_mmio_region(MemoryListener* listener, MemoryRegionSection* secion, hwaddr start, hwaddr size)
{
    KVMState* s = kvm_state;
...
static void kvm_uncoalesce_mmio_region(MemoryListener* listener, MemoryRegionSection* secion, hwaddr start, hwaddr size)
{
    KVMState* s = kvm_state;
...
static void kvm_coalesce_pio_add(MemoryListener* listener, MemoryRegionSection* section, hwaddr start, hwaddr size)
{
    KVMState* s = kvm_state;
...
static void kvm_coalesce_pio_del(MemoryListener* listener, MemoryRegionSection* section, hwaddr start, hwaddr size)
{
    KVMState* s = kvm_state;
...
static QLIST_HEAD(, HWPoisonPage) hwpoison_page_list = QLIST_HEAD_INITIALIZER(hwpoison_page_list);
...
void kvm_hwpoison_page_add(ram_addr_t ram_addr)
{
    ...
    QLIST_FOREACH (page, &hwpoison_page_list, list) {
        if (page->ram_addr == ram_addr) { return; }
    }
...
bool kvm_hwpoisoned_mem(void) { return !QLIST_EMPTY(&hwpoison_page_list); }
...
static uint32_t adjust_ioeventfd_endianness(uint32_t val, uint32_t size)
{
    if (target_needs_bswap()) {
         ...
         */
        switch (size) {
            case 2: val = bswap16(val); break;
            case 4: val = bswap32(val); break;
        }
    }
    return val;
}

static int kvm_set_ioeventfd_mmio(int fd, hwaddr addr, uint32_t val, bool assign, uint32_t size, bool datamatch)
{
    int                  ret;
    ...
    trace_kvm_set_ioeventfd_mmio(fd, (uint64_t)addr, val, assign, size, datamatch);
    if (!kvm_enabled()) { return -ENOSYS; }
...
static int kvm_set_ioeventfd_pio(int fd, uint16_t addr, uint16_t val, bool assign, uint32_t size, bool datamatch)
{
    ...
    if (!kvm_enabled()) { return -ENOSYS; }
    if (datamatch) { kick.flags |= KVM_IOEVENTFD_FLAG_DATAMATCH; }
    if (!assign) { kick.flags |= KVM_IOEVENTFD_FLAG_DEASSIGN; }
    r = kvm_vm_ioctl(kvm_state, KVM_IOEVENTFD, &kick);
    if (r < 0) { return r; }
    return 0;
}

static const KVMCapabilityInfo* kvm_check_extension_list(KVMState* s, const KVMCapabilityInfo* list)
{
    while (list->name) {
        if (!kvm_check_extension(s, list->value)) { return list; }
        list++;
    }
    return NULL;
}
...
{
    assert(ROUND_UP(max_slot_size, qemu_real_host_page_size()) == max_slot_size);
...
int kvm_set_memory_attributes_shared(hwaddr start, uint64_t size) { return kvm_set_memory_attributes(start, size, 0); }
...
static void kvm_set_phys_mem(KVMMemoryListener* kml, MemoryRegionSection* section, bool add)
{
    ...
    if (!memory_region_is_ram(mr)) {
        if (writable || !kvm_readonly_mem_allowed) { return; }
    ...
    do {
        ...
        mem->guest_memfd_offset = mem->guest_memfd >= 0 ? (uint8_t*)ram - mr->ram_block->host : 0;
        ...
        if (memory_region_has_guest_memfd(mr)) {
            ...
            if (err) {
                error_report("%s: failed to set memory attribute private: %s", __func__, strerror(-err));
                exit(1);
            }
        }
...
static void kvm_dirty_ring_reaper_init(KVMState* s)
{
    ...
    qemu_thread_create(&r->reaper_thr, "kvm-reaper", kvm_dirty_ring_reaper_thread, s, QEMU_THREAD_JOINABLE);
}
...
static void kvm_region_commit(MemoryListener* listener)
{
    ...
    if (QSIMPLEQ_EMPTY(&kml->transaction_add) && QSIMPLEQ_EMPTY(&kml->transaction_del)) { return; }
    ...
    while (u1 && u2) {
        ...
        if (range_lob(&r1) < range_lob(&r2)) { u1 = QSIMPLEQ_NEXT(u1, next); }
        else {
    ...
    kvm_slots_lock();
    if (need_inhibit) { accel_ioctl_inhibit_begin(); }
...
static void kvm_log_sync(MemoryListener* listener, MemoryRegionSection* section)
{
    KVMMemoryListener* kml = container_of(listener, KVMMemoryListener, listener);
...
static void kvm_log_clear(MemoryListener* listener, MemoryRegionSection* section)
{
    KVMMemoryListener* kml = container_of(listener, KVMMemoryListener, listener);
    int                r;
    ...
    if (r < 0) {
        error_report_once("%s: kvm log clear failed: mr=%s "
                          "offset=%" HWADDR_PRIx " size=%" PRIx64,
                          __func__, section->mr->name, section->offset_within_region, int128_get64(section->size));
        abort();
    }
}

static void kvm_mem_ioeventfd_add(MemoryListener* listener, MemoryRegionSection* section, bool match_data,
                                  uint64_t data, EventNotifier* e)
{
    int fd = event_notifier_get_fd(e);
    int r;

    r = kvm_set_ioeventfd_mmio(fd, section->offset_within_address_space, data, true, int128_get64(section->size),
                               match_data);
    if (r < 0) {
...
static void kvm_mem_ioeventfd_del(MemoryListener* listener, MemoryRegionSection* section, bool match_data,
                                  uint64_t data, EventNotifier* e)
{
    int fd = event_notifier_get_fd(e);
    int r;

    r = kvm_set_ioeventfd_mmio(fd, section->offset_within_address_space, data, false, int128_get64(section->size),
                               match_data);
    if (r < 0) {
...
static void kvm_io_ioeventfd_add(MemoryListener* listener, MemoryRegionSection* section, bool match_data, uint64_t data,
                                 EventNotifier* e)
{
    int fd = event_notifier_get_fd(e);
    int r;

    r = kvm_set_ioeventfd_pio(fd, section->offset_within_address_space, data, true, int128_get64(section->size),
                              match_data);
    if (r < 0) {
        fprintf(stderr, "%s: error adding ioeventfd: %s (%d)\n", __func__, strerror(-r), -r);
        abort();
...
static void kvm_io_ioeventfd_del(MemoryListener* listener, MemoryRegionSection* section, bool match_data, uint64_t data,
                                 EventNotifier* e)

{
    ...
    r = kvm_set_ioeventfd_pio(fd, section->offset_within_address_space, data, false, int128_get64(section->size),
                              match_data);
    if (r < 0) {
        fprintf(stderr, "%s: error deleting ioeventfd: %s (%d)\n", __func__, strerror(-r), -r);
        abort();
...
void kvm_memory_listener_register(KVMState* s, KVMMemoryListener* kml, AddressSpace* as, int as_id, const char* name)
{
    ...
    if (s->kvm_dirty_ring_size) { kml->listener.log_sync_global = kvm_log_sync_global; }
    else {
...
static void set_gsi(KVMState* s, unsigned int gsi) { set_bit(gsi, s->used_gsi_bitmap); }
...
static int kvm_update_routing_entry(KVMState* s, struct kvm_irq_routing_entry* new_entry)
{
    ...
    for (n = 0; n < s->irq_routes->nr; n++) {
        ...
        if (entry->gsi != new_entry->gsi) { continue; }

        if (!memcmp(entry, new_entry, sizeof *entry)) { return 0; }
...
bool kvm_dirty_ring_enabled(void) { return kvm_state && kvm_state->kvm_dirty_ring_size; }

static void query_stats_cb(StatsResultList** result, StatsTarget target, strList* names, strList* targets,
                           Error** errp);
static void query_stats_schemas_cb(StatsSchemaList** result, Error** errp);

uint32_t kvm_dirty_ring_size(void) { return kvm_state->kvm_dirty_ring_size; }
...
static int do_kvm_create_vm(KVMState* s, int type)
{
    ...
    if (ret < 0) { error_report("ioctl(KVM_CREATE_VM) failed: %s", strerror(-ret)); }
...
static int kvm_setup_dirty_ring(KVMState* s)
{
    ...
    if (!s->kvm_dirty_ring_size) {
        ...
        if (dirty_log_manual_caps) {
            ret = kvm_vm_enable_cap(s, KVM_CAP_MANUAL_DIRTY_LOG_PROTECT2, 0, dirty_log_manual_caps);
            if (ret) {
...
{
    MachineClass*     mc             = MACHINE_GET_CLASS(ms);
    static const char upgrade_note[] = "Please upgrade to at least kernel 4.5.\n";
    const struct
    {
        const char* name;
        int         num;
    } num_cpus[] = {{"SMP", ms->smp.cpus}, {"hotpluggable", ms->smp.max_cpus}, {/* end of list */}}, *nc = num_cpus;
    ...
    if (!s->nr_slots_max) { s->nr_slots_max = KVM_MEMSLOTS_NR_MAX_DEFAULT; }
    ...
    if (!missing_cap) { missing_cap = kvm_check_extension_list(s, kvm_arch_required_capabilities); }
    if (missing_cap) {
        ret = -EINVAL;
    ...
    if (kvm_check_extension(s, KVM_CAP_IRQ_INJECT_STATUS)) { s->irq_set_ioctl = KVM_IRQ_LINE_STATUS; }

    kvm_readonly_mem_allowed = (kvm_vm_check_extension(s, KVM_CAP_READONLY_MEM) > 0);
...
#ifdef TARGET_KVM_HAVE_GUEST_DEBUG
    kvm_has_guest_debug = (kvm_check_extension(s, KVM_CAP_SET_GUEST_DEBUG) > 0);
#endif
    ...
    if (kvm_has_guest_debug) {
        ...
        int guest_debug_flags = kvm_check_extension(s, KVM_CAP_SET_GUEST_DEBUG2);

        if (guest_debug_flags & KVM_GUESTDBG_BLOCKIRQ) { kvm_sstep_flags |= SSTEP_NOIRQ; }
#endif
    }
    ...
    if (s->kvm_dirty_ring_size) { kvm_dirty_ring_reaper_init(s); }

    if (kvm_check_extension(kvm_state, KVM_CAP_BINARY_STATS_FD)) {
        add_stats_callbacks(STATS_PROVIDER_KVM, query_stats_cb, query_stats_schemas_cb);
    }
...
void kvm_set_sigmask_len(KVMState* s, unsigned int sigmask_len) { s->sigmask_len = sigmask_len; }

static void kvm_handle_io(uint16_t port, MemTxAttrs attrs, void* data, int direction, int size, uint32_t count)
{
    int      i;
    uint8_t* ptr = data;
...
void kvm_flush_coalesced_mmio_buffer(void)
{
    ...
    if (!s || s->coalesced_flush_in_progress) { return; }
    ...
    if (s->coalesced_mmio_ring) {
        ...
        while (ring->first != ring->last) {
            ...
            if (ent->pio == 1) {
                address_space_write(&address_space_io, ent->phys_addr, MEMTXATTRS_UNSPECIFIED, ent->data, ent->len);
            }
            else {
...
static void do_kvm_cpu_synchronize_state(CPUState* cpu, run_on_cpu_data arg)
{
    if (!cpu->vcpu_dirty && !kvm_state->guest_state_protected) {
        ...
        if (ret) {
            if (err) { error_reportf_err(err, "Failed to synchronize CPU state: "); }
            else {
...
static void do_kvm_cpu_synchronize_post_reset(CPUState* cpu, run_on_cpu_data arg)
{
    ...
    if (ret) {
        if (err) { error_reportf_err(err, "Restoring resisters after reset: "); }
        else {
...
void kvm_cpu_synchronize_post_reset(CPUState* cpu)
{
    ...
    if (cpu == first_cpu) { kvm_reset_parked_vcpus(kvm_state); }
}
...
static void do_kvm_cpu_synchronize_post_init(CPUState* cpu, run_on_cpu_data arg)
{
    ...
    if (ret) {
        if (err) { error_reportf_err(err, "Putting registers after init: "); }
        else {
...
static void do_kvm_cpu_synchronize_pre_loadvm(CPUState* cpu, run_on_cpu_data arg) { cpu->vcpu_dirty = true; }
...
static void kvm_cpu_kick(CPUState* cpu) { qatomic_set(&cpu->kvm_run->immediate_exit, 1); }

static void kvm_cpu_kick_self(void)
{
    if (kvm_immediate_exit) { kvm_cpu_kick(current_cpu); }
    else {
        qemu_cpu_kick_self();
    }
}
...
int kvm_convert_memory(hwaddr start, hwaddr size, bool to_private)
{
    ...
    if (!memory_region_has_guest_memfd(mr)) {
         ...
         */
        if (!to_private && !memory_region_is_ram(mr) && !memory_region_is_ram_device(mr) && !memory_region_is_rom(mr)
    ...
    if (to_private) { ret = kvm_set_memory_attributes_private(start, size); }
    else {
    ...
    ret = ram_block_attributes_state_change(RAM_BLOCK_ATTRIBUTES(mr->rdm), offset, size, to_private);
    if (ret) {
...
int kvm_cpu_exec(CPUState* cpu)
{
    ...
    do {
        ...
        if (unlikely(have_sigbus_pending)) {
            bql_lock();
            kvm_arch_on_sigbus_vcpu(cpu, pending_sigbus_code, pending_sigbus_addr);
        ...
        switch (run->exit_reason) {
            case KVM_EXIT_IO:
                /* Called outside BQL */
                kvm_handle_io(run->io.port, attrs, (uint8_t*)run + run->io.data_offset, run->io.direction, run->io.size,
                              run->io.count);
                ret = 0;
                break;
            case KVM_EXIT_MMIO:
                /* Called outside BQL */
                address_space_rw(&address_space_memory, run->mmio.phys_addr, attrs, run->mmio.data, run->mmio.len,
                                 run->mmio.is_write);
                ret = 0;
                break;
            case KVM_EXIT_IRQ_WINDOW_OPEN: ret = EXCP_INTERRUPT; break;
            ...
            case KVM_EXIT_UNKNOWN:
                ...
                ret = -1;
                break;
            case KVM_EXIT_INTERNAL_ERROR: ret = kvm_handle_internal_error(cpu, run); break;
            ...
            case KVM_EXIT_SYSTEM_EVENT:
                ...
                switch (run->system_event.type) {
                    ...
                    case KVM_SYSTEM_EVENT_CRASH:
                        ...
                        ret = 0;
                        break;
                    default: ret = kvm_arch_handle_exit(cpu, run); break;
                }
                break;
            case KVM_EXIT_MEMORY_FAULT:
                trace_kvm_memory_fault(run->memory_fault.gpa, run->memory_fault.size, run->memory_fault.flags);
                if (run->memory_fault.flags & ~KVM_MEMORY_EXIT_FLAG_PRIVATE) {
                    error_report("KVM_EXIT_MEMORY_FAULT: Unknown flag 0x%" PRIx64, (uint64_t)run->memory_fault.flags);
                    ret = -1;
                    break;
                }
...
int kvm_vm_check_attr(KVMState* s, uint32_t group, uint64_t attr)
{
    ...
    if (!kvm_vm_attributes_allowed) { return 0; }
...
int kvm_device_access(int fd, int group, uint64_t attr, void* val, bool write, Error** errp)
{
    ...
    err = kvm_device_ioctl(fd, write ? KVM_SET_DEVICE_ATTR : KVM_GET_DEVICE_ATTR, &kvmattr);
    if (err < 0) {
...
bool kvm_has_sync_mmu(void) { return kvm_state->sync_mmu; }

int kvm_has_vcpu_events(void) { return kvm_state->vcpu_events; }

int kvm_max_nested_state_length(void) { return kvm_state->max_nested_state_len; }
...
int kvm_sw_breakpoints_active(CPUState* cpu) { return !QTAILQ_EMPTY(&cpu->kvm_state->kvm_sw_breakpoints); }
...
{
    struct kvm_set_guest_debug_data* dbg_data = (struct kvm_set_guest_debug_data*)data.host_ptr;
...
int kvm_update_guest_debug(CPUState* cpu, unsigned long reinject_trap)
{
    ...
    if (cpu->singlestep_enabled) {
        ...
        if (cpu->singlestep_enabled & SSTEP_NOIRQ) { data.dbg.control |= KVM_GUESTDBG_BLOCKIRQ; }
    }
...
void kvm_remove_all_breakpoints(CPUState* cpu)
{
    ...
    QTAILQ_FOREACH_SAFE (bp, &s->kvm_sw_breakpoints, entry, next) {
        if (kvm_arch_remove_sw_breakpoint(cpu, bp) != 0) {
            ...
            CPU_FOREACH (tmpcpu) {
                if (kvm_arch_remove_sw_breakpoint(tmpcpu, bp) == 0) { break; }
            }
        }
    ...
    CPU_FOREACH (cpu) { kvm_update_guest_debug(cpu, 0); }
}
...
void kvm_init_cpu_signals(CPUState* cpu)
{
    ...
    if (kvm_immediate_exit) { r = pthread_sigmask(SIG_SETMASK, &set, NULL); }
    else {
...
{
#ifdef KVM_HAVE_MCE_INJECTION
    if (have_sigbus_pending) { return 1; }
...
int kvm_create_device(KVMState* s, uint64_t type, bool test)
{
    ...
    if (!kvm_check_extension(s, KVM_CAP_DEVICE_CTRL)) { return -ENOTSUP; }
...
bool kvm_device_supported(int vmfd, uint64_t type)
{
    ...
    if (ioctl(vmfd, KVM_CHECK_EXTENSION, KVM_CAP_DEVICE_CTRL) <= 0) { return false; }
...
int kvm_set_one_reg(CPUState* cs, uint64_t id, void* source)
{
    ...
    if (r) { trace_kvm_failed_reg_set(id, strerror(-r)); }
    return r;
}
...
int kvm_get_one_reg(CPUState* cs, uint64_t id, void* target)
{
    ...
    if (r) { trace_kvm_failed_reg_get(id, strerror(-r)); }
    return r;
}

static bool kvm_accel_has_memory(AccelState* accel, AddressSpace* as, hwaddr start_addr, hwaddr size)
{
    ...
    for (i = 0; i < kvm->nr_as; ++i) {
        if (kvm->as[i].as == as && kvm->as[i].ml) {
            ...
            return NULL != kvm_lookup_matching_slot(kvm->as[i].ml, start_addr, size);
        }
    }
...
static void kvm_get_kvm_shadow_mem(Object* obj, Visitor* v, const char* name, void* opaque, Error** errp)
{
...
static void kvm_set_kvm_shadow_mem(Object* obj, Visitor* v, const char* name, void* opaque, Error** errp)
{
    KVMState* s = KVM_STATE(obj);
    int64_t   value;
...
static void kvm_set_dirty_ring_size(Object* obj, Visitor* v, const char* name, void* opaque, Error** errp)
{
    KVMState* s = KVM_STATE(obj);
    uint32_t  value;
...
static char* kvm_get_device(Object* obj, Error** errp G_GNUC_UNUSED)
{
    KVMState* s = KVM_STATE(obj);
...
static void kvm_set_device(Object* obj, const char* value, Error** errp G_GNUC_UNUSED)
{
    KVMState* s = KVM_STATE(obj);
...
static void kvm_set_kvm_rapl_socket_path(Object* obj, const char* str, Error** errp)
{
    KVMState* s = KVM_STATE(obj);
 ...
 */
static int kvm_gdbstub_sstep_flags(AccelState* as) { return kvm_sstep_flags; }
...
static void kvm_accel_class_init(ObjectClass* oc, const void* data)
{
    ...
    object_class_property_add(oc, "dirty-ring-size", "uint32", kvm_get_dirty_ring_size, kvm_set_dirty_ring_size, NULL,
                              NULL);
    object_class_property_set_description(oc, "dirty-ring-size",
                                          "Size of KVM dirty page ring buffer (default: 0, i.e. use bitmap)");

    object_class_property_add_str(oc, "device", kvm_get_device, kvm_set_device);
    object_class_property_set_description(oc, "device", "Path to the device node to use (default: /dev/kvm)");

    object_class_property_add_bool(oc, "rapl", NULL, kvm_set_kvm_rapl);
    object_class_property_set_description(oc, "rapl", "Allow energy related MSRs for RAPL interface in Guest");

    object_class_property_add_str(oc, "rapl-helper-socket", NULL, kvm_set_kvm_rapl_socket_path);
...
static StatsList* add_kvmstat_entry(struct kvm_stats_desc* pdesc, uint64_t* stats_data, StatsList* stats_list,
                                    Error** errp)
{
    ...
    switch (pdesc->flags & KVM_STATS_BASE_MASK) {
        ...
        case KVM_STATS_BASE_POW2 : break;
        default                  : return stats_list;
    }
    ...
    if ((pdesc->flags & KVM_STATS_UNIT_MASK) == KVM_STATS_UNIT_BOOLEAN) {
        ...
        int i;
        for (i = 0; i < pdesc->size; i++) { QAPI_LIST_PREPEND(val_list, stats_data[i]); }
...
static StatsSchemaValueList* add_kvmschema_entry(struct kvm_stats_desc* pdesc, StatsSchemaValueList* list, Error** errp)
{
    StatsSchemaValueList* schema_entry = g_new0(StatsSchemaValueList, 1);
    schema_entry->value                = g_new0(StatsSchemaValue, 1);

    switch (pdesc->flags & KVM_STATS_TYPE_MASK) {
        case KVM_STATS_TYPE_CUMULATIVE: schema_entry->value->type = STATS_TYPE_CUMULATIVE; break;
        case KVM_STATS_TYPE_INSTANT   : schema_entry->value->type = STATS_TYPE_INSTANT; break;
        case KVM_STATS_TYPE_PEAK      : schema_entry->value->type = STATS_TYPE_PEAK; break;
        case KVM_STATS_TYPE_LINEAR_HIST:
            ...
            break;
        case KVM_STATS_TYPE_LOG_HIST: schema_entry->value->type = STATS_TYPE_LOG2_HISTOGRAM; break;
        default                     : goto exit;
    }

    switch (pdesc->flags & KVM_STATS_UNIT_MASK) {
        case KVM_STATS_UNIT_NONE: break;
...
static QTAILQ_HEAD(, StatsDescriptors) stats_descriptors = QTAILQ_HEAD_INITIALIZER(stats_descriptors);

/*
 * Return the descriptors for 'target', that either have already been read
 * or are retrieved from 'stats_fd'.
 */
static StatsDescriptors* find_stats_descriptors(StatsTarget target, int stats_fd, Error** errp)
{
    ...
    QTAILQ_FOREACH (descriptors, &stats_descriptors, next) {
        if (g_str_equal(descriptors->ident, ident)) { return descriptors; }
    }
    ...
    ret = pread(stats_fd, kvm_stats_desc, size_desc * kvm_stats_header->num_desc, kvm_stats_header->desc_offset);
...
static void query_stats(StatsResultList** result, StatsTarget target, strList* names, int stats_fd, CPUState* cpu,
                        Error** errp)
{
    struct kvm_stats_desc*   kvm_stats_desc;
    struct kvm_stats_header* kvm_stats_header;
    ...
    for (i = 0; i < kvm_stats_header->num_desc; ++i) {
        ...
        if (!apply_str_list_filter(pdesc->name, names)) { continue; }
        stats_list = add_kvmstat_entry(pdesc, stats, stats_list, errp);
    }

    if (!stats_list) { return; }

    switch (target) {
        case STATS_TARGET_VM: add_stats_entry(result, STATS_PROVIDER_KVM, NULL, stats_list); break;
        case STATS_TARGET_VCPU:
            add_stats_entry(result, STATS_PROVIDER_KVM, cpu->parent_obj.canonical_path, stats_list);
            break;
        default: assert_not_reached();
    }
}

static void query_stats_schema(StatsSchemaList** result, StatsTarget target, int stats_fd, Error** errp)
{
    struct kvm_stats_desc*   kvm_stats_desc;
    struct kvm_stats_header* kvm_stats_header;
    StatsDescriptors*        descriptors;
...
static void query_stats_vcpu(CPUState* cpu, StatsArgs* kvm_stats_args)
{
    ...
    query_stats(kvm_stats_args->result.stats, STATS_TARGET_VCPU, kvm_stats_args->names, stats_fd, cpu,
                kvm_stats_args->errp);
}
...
static void query_stats_schema_vcpu(CPUState* cpu, StatsArgs* kvm_stats_args)
{
    ...
    query_stats_schema(kvm_stats_args->result.schema, STATS_TARGET_VCPU, stats_fd, kvm_stats_args->errp);
}

static void query_stats_cb(StatsResultList** result, StatsTarget target, strList* names, strList* targets, Error** errp)
{
    KVMState* s = kvm_state;
    CPUState* cpu;
    int       stats_fd;
    ...
    switch (target) {
        ...
        case STATS_TARGET_VCPU: {
            ...
            CPU_FOREACH (cpu) {
                if (!apply_str_list_filter(cpu->parent_obj.canonical_path, targets)) { continue; }
...
void kvm_mark_guest_state_protected(void) { kvm_state->guest_state_protected = true; }
