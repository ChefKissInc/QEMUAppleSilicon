...
#define tlb_debug(fmt, ...)                                                                          \
    do {                                                                                             \
        if (DEBUG_TLB_LOG_GATE) { qemu_log_mask(CPU_LOG_MMU, "%s: " fmt, __func__, ##__VA_ARGS__); } \
...
#define assert_cpu_is_self(cpu)                                                   \
    do {                                                                          \
        if (DEBUG_TLB_GATE) { assert(!(cpu)->created || qemu_cpu_is_self(cpu)); } \
    }                                                                             \
    while (0)
...
static inline size_t tlb_n_entries(CPUTLBDescFast* fast) { return (fast->mask >> CPU_TLB_ENTRY_BITS) + 1; }

static inline size_t sizeof_tlb(CPUTLBDescFast* fast) { return fast->mask + (1 << CPU_TLB_ENTRY_BITS); }

static inline uint64_t tlb_read_idx(const CPUTLBEntry* entry, MMUAccessType access_type)
{
    /* Do not rearrange the CPUTLBEntry structure members. */
    QEMU_BUILD_BUG_ON(offsetof(CPUTLBEntry, addr_read) != MMU_DATA_LOAD * sizeof(uintptr_t));
    QEMU_BUILD_BUG_ON(offsetof(CPUTLBEntry, addr_write) != MMU_DATA_STORE * sizeof(uintptr_t));
    QEMU_BUILD_BUG_ON(offsetof(CPUTLBEntry, addr_code) != MMU_INST_FETCH * sizeof(uintptr_t));
...
static inline uint64_t tlb_addr_write(const CPUTLBEntry* entry) { return tlb_read_idx(entry, MMU_DATA_STORE); }
...
static inline CPUTLBEntry* tlb_entry(CPUState* cpu, uintptr_t mmu_idx, vaddr addr)
 ...
 * conflict misses.
 */
static void tlb_mmu_resize_locked(CPUTLBDesc* desc, CPUTLBDescFast* fast, int64_t now)
{
    ...
    if (desc->n_used_entries > desc->window_max_entries) { desc->window_max_entries = desc->n_used_entries; }
    rate = desc->window_max_entries * 100 / old_size;

    if (rate > 70) { new_size = MIN(old_size << 1, 1 << CPU_TLB_DYN_MAX_BITS); }
         ...
         */
        if (expected_rate > 70) { ceil *= 2; }
        new_size = MAX(ceil, 1 << CPU_TLB_DYN_MIN_BITS);
    }

    if (new_size == old_size) {
        if (window_expired) { tlb_window_reset(desc, now, desc->n_used_entries); }
        return;
    }
...
static void tlb_flush_one_mmuidx_locked(CPUState* cpu, int mmu_idx, int64_t now)
{
...
static void tlb_flush_by_mmuidx_async_work(CPUState* cpu, run_on_cpu_data data)
{
    ...
    if (to_clean == ALL_MMUIDX_BITS) {
        ...
        qatomic_set(&cpu->neg.tlb.c.part_flush_count, cpu->neg.tlb.c.part_flush_count + ctpop32(to_clean));
        if (to_clean != asked) {
            qatomic_set(&cpu->neg.tlb.c.elide_flush_count,
                        cpu->neg.tlb.c.elide_flush_count + ctpop32(asked & ~to_clean));
        }
    }
}
...
static bool tlb_hit_page_mask_anyprot(CPUTLBEntry* tlb_entry, vaddr page, vaddr mask)
{
    page &= mask;
    mask &= TARGET_PAGE_MASK | TLB_INVALID_MASK;

    return (page == (tlb_entry->addr_read & mask) || page == (tlb_addr_write(tlb_entry) & mask)
...
static bool tlb_flush_entry_mask_locked(CPUTLBEntry* tlb_entry, vaddr page, vaddr mask)
{
...
/* Called with tlb_c.lock held */
static void tlb_flush_vtlb_page_mask_locked(CPUState* cpu, int mmu_idx, vaddr page, vaddr mask)
{
    ...
    for (k = 0; k < CPU_VTLB_SIZE; k++) {
        if (tlb_flush_entry_mask_locked(&d->vtable[k], page, mask)) { tlb_n_used_entries_dec(cpu, mmu_idx); }
    }
}
...
static void tlb_flush_page_locked(CPUState* cpu, int midx, vaddr page)
{
    ...
    if ((page & lp_mask) == lp_addr) {
        ...
        if (tlb_flush_entry_locked(tlb_entry(cpu, midx, page), page)) { tlb_n_used_entries_dec(cpu, midx); }
 ...
 */
static void tlb_flush_page_by_mmuidx_async_0(CPUState* cpu, vaddr addr, MMUIdxMap idxmap)
{
    int mmu_idx;
    ...
    for (mmu_idx = 0; mmu_idx < NB_MMU_MODES; mmu_idx++) {
        if ((idxmap >> mmu_idx) & 1) { tlb_flush_page_locked(cpu, mmu_idx, addr); }
    }
    qemu_spin_unlock(&cpu->neg.tlb.c.lock);
 ...
 */
static void tlb_flush_page_by_mmuidx_async_1(CPUState* cpu, run_on_cpu_data data)
{
 ...
 */
static void tlb_flush_page_by_mmuidx_async_2(CPUState* cpu, run_on_cpu_data data)
{
...
void tlb_flush_page(CPUState* cpu, vaddr addr) { tlb_flush_page_by_mmuidx(cpu, addr, ALL_MMUIDX_BITS); }

void tlb_flush_page_by_mmuidx_all_cpus_synced(CPUState* src_cpu, vaddr addr, MMUIdxMap idxmap)
{
    tlb_debug("addr: %016" VADDR_PRIx " mmu_idx:%" PRIx32 "\n", addr, idxmap);
...
static void tlb_flush_range_locked(CPUState* cpu, int midx, vaddr addr, vaddr len, unsigned bits)
{
    ...
    for (vaddr i = 0; i < len; i += TARGET_PAGE_SIZE) {
        ...
        if (tlb_flush_entry_mask_locked(entry, page, mask)) { tlb_n_used_entries_dec(cpu, midx); }
...
static void tlb_flush_range_by_mmuidx_async_0(CPUState* cpu, TLBFlushRangeData d)
{
    int mmu_idx;
    ...
    qemu_spin_lock(&cpu->neg.tlb.c.lock);
    for (mmu_idx = 0; mmu_idx < NB_MMU_MODES; mmu_idx++) {
        if ((d.idxmap >> mmu_idx) & 1) { tlb_flush_range_locked(cpu, mmu_idx, d.addr, d.len, d.bits); }
    }
    qemu_spin_unlock(&cpu->neg.tlb.c.lock);
...
static void tlb_flush_range_by_mmuidx_async_1(CPUState* cpu, run_on_cpu_data data)
{
...
void tlb_flush_range_by_mmuidx(CPUState* cpu, vaddr addr, vaddr len, MMUIdxMap idxmap, unsigned bits)
{
    TLBFlushRangeData d;
...
void tlb_flush_page_bits_by_mmuidx(CPUState* cpu, vaddr addr, MMUIdxMap idxmap, unsigned bits)
{ tlb_flush_range_by_mmuidx(cpu, addr, TARGET_PAGE_SIZE, idxmap, bits); }

void tlb_flush_range_by_mmuidx_all_cpus_synced(CPUState* src_cpu, vaddr addr, vaddr len, MMUIdxMap idxmap,
                                               unsigned bits)
{
...
{ cpu_physical_memory_test_and_clear_dirty(ram_addr & TARGET_PAGE_MASK, TARGET_PAGE_SIZE, DIRTY_MEMORY_CODE); }

/* update the TLB so that writes in physical page 'phys_addr' are no longer
   tested for self modifying code */
void tlb_unprotect_code(ram_addr_t ram_addr) { cpu_physical_memory_set_dirty_flag(ram_addr, DIRTY_MEMORY_CODE); }
 ...
 * Called with tlb_c.lock held.
 */
static void tlb_reset_dirty_range_locked(CPUTLBEntryFull* full, CPUTLBEntry* ent, uintptr_t start, uintptr_t length)
{
    ...
    if (flags == 0) {
        ...
        if ((host - start) < length) { qatomic_set(&ent->addr_write, addr | TLB_NOTDIRTY); }
    }
}
 ...
 */
static inline void copy_tlb_helper_locked(CPUTLBEntry* d, const CPUTLBEntry* s) { *d = *s; }
...
void tlb_reset_dirty(CPUState* cpu, uintptr_t start, uintptr_t length)
{
    ...
    for (mmu_idx = 0; mmu_idx < NB_MMU_MODES; mmu_idx++) {
        ...
        for (i = 0; i < n; i++) { tlb_reset_dirty_range_locked(&desc->fulltlb[i], &fast->table[i], start, length); }

        for (i = 0; i < CPU_VTLB_SIZE; i++) {
            tlb_reset_dirty_range_locked(&desc->vfulltlb[i], &desc->vtable[i], start, length);
        }
    }
    qemu_spin_unlock(&cpu->neg.tlb.c.lock);
}
...
/* Called with tlb_c.lock held */
static inline void tlb_set_dirty1_locked(CPUTLBEntry* tlb_entry, vaddr addr)
{
    if (tlb_entry->addr_write == (addr | TLB_NOTDIRTY)) { tlb_entry->addr_write = addr; }
}
...
static void tlb_set_dirty(CPUState* cpu, vaddr addr)
{
    ...
    addr &= TARGET_PAGE_MASK;
    qemu_spin_lock(&cpu->neg.tlb.c.lock);
    for (mmu_idx = 0; mmu_idx < NB_MMU_MODES; mmu_idx++) { tlb_set_dirty1_locked(tlb_entry(cpu, mmu_idx, addr), addr); }

    for (mmu_idx = 0; mmu_idx < NB_MMU_MODES; mmu_idx++) {
        int k;
        for (k = 0; k < CPU_VTLB_SIZE; k++) { tlb_set_dirty1_locked(&cpu->neg.tlb.d[mmu_idx].vtable[k], addr); }
    }
    qemu_spin_unlock(&cpu->neg.tlb.c.lock);
}
...
static void tlb_add_large_page(CPUState* cpu, int mmu_idx, vaddr addr, uint64_t size)
{
    ...
    if (lp_addr == (vaddr)-1) {
        ...
        while (((lp_addr ^ addr) & lp_mask) != 0) { lp_mask <<= 1; }
    }
    cpu->neg.tlb.d[mmu_idx].large_page_addr = lp_addr & lp_mask;
    cpu->neg.tlb.d[mmu_idx].large_page_mask = lp_mask;
}

static inline void tlb_set_compare(CPUTLBEntryFull* full, CPUTLBEntry* ent, vaddr address, int flags,
                                   MMUAccessType access_type, bool enable)
{
    if (enable) {
        address |= flags & TLB_FLAGS_MASK;
        flags   &= TLB_SLOW_FLAGS_MASK;
        if (flags) { address |= TLB_FORCE_SLOW; }
    }
    else {
        address = -1;
        flags   = 0;
 ...
 * critical section.
 */
void tlb_set_page_full(CPUState* cpu, int mmu_idx, vaddr addr, CPUTLBEntryFull* full)
{
    ...
    if (full->lg_page_size <= TARGET_PAGE_BITS) { sz = TARGET_PAGE_SIZE; }
    else {
    ...
    section = address_space_translate_for_iotlb(cpu, asidx, paddr_page, &xlat, &sz, full->attrs, &prot);
    assert(sz >= TARGET_PAGE_SIZE);

    tlb_debug("vaddr=%016" VADDR_PRIx " paddr=0x" HWADDR_FMT_plx " prot=%x idx=%d\n", addr, full->phys_addr, prot,
              mmu_idx);
    ...
    if (is_ram) {
         ...
         */
        if (prot & PAGE_WRITE) {
            if (section->readonly) { write_flags |= TLB_DISCARD_WRITE; }
        ...
        if (!is_romd) { read_flags = write_flags; }
    }
    ...
    tlb_set_compare(full, &tn, addr_page, read_flags, MMU_INST_FETCH, prot & PAGE_EXEC);

    if (wp_flags & BP_MEM_READ) { read_flags |= TLB_WATCHPOINT; }
    tlb_set_compare(full, &tn, addr_page, read_flags, MMU_DATA_LOAD, prot & PAGE_READ);

    if (prot & PAGE_WRITE_INV) { write_flags |= TLB_INVALID_MASK; }
    if (wp_flags & BP_MEM_WRITE) { write_flags |= TLB_WATCHPOINT; }
    tlb_set_compare(full, &tn, addr_page, write_flags, MMU_DATA_STORE, prot & PAGE_WRITE);
...
void tlb_set_page_with_attrs(CPUState* cpu, vaddr addr, hwaddr paddr, MemTxAttrs attrs, int prot, int mmu_idx,
                             vaddr size)
{
    CPUTLBEntryFull full = {.phys_addr = paddr, .attrs = attrs, .prot = prot, .lg_page_size = ctz64(size)};

    assert(is_power_of_2(size));
    tlb_set_page_full(cpu, mmu_idx, addr, &full);
}

void tlb_set_page(CPUState* cpu, vaddr addr, hwaddr paddr, int prot, int mmu_idx, vaddr size)
{ tlb_set_page_with_attrs(cpu, addr, paddr, MEMTXATTRS_UNSPECIFIED, prot, mmu_idx, size); }
 ...
 * @tlb_addr: TLB entry address (a CPUTLBEntry addr_read/write/code value)
 */
static inline bool tlb_hit(uint64_t tlb_addr, vaddr addr) { return tlb_hit_page(tlb_addr, addr & TARGET_PAGE_MASK); }
 ...
 * (e.g. via tlb_entry()).
 */
static bool tlb_fill_align(CPUState* cpu, vaddr addr, MMUAccessType type, int mmu_idx, MemOp memop, int size,
                           bool probe, uintptr_t ra)
{
    ...
    if (ops->tlb_fill_align) {
        ...
        if (ops->tlb_fill(cpu, addr, size, type, mmu_idx, probe, ra)) { return true; }
    }
    assert(probe);
    return false;
}

static inline void cpu_unaligned_access(CPUState* cpu, vaddr addr, MMUAccessType access_type, int mmu_idx,
                                        uintptr_t retaddr)
{ cpu->cc->tcg_ops->do_unaligned_access(cpu, addr, access_type, mmu_idx, retaddr); }

static MemoryRegionSection* io_prepare(hwaddr* out_offset, CPUState* cpu, CPUTLBEntryFull* full, vaddr addr,
                                       uintptr_t retaddr)
{
    MemoryRegionSection* section;
    hwaddr               mr_offset;
...
static void io_failed(CPUState* cpu, CPUTLBEntryFull* full, vaddr addr, unsigned size, MMUAccessType access_type,
                      int mmu_idx, MemTxResult response, uintptr_t retaddr)
{
    if (!cpu->ignore_memory_transaction_failures && cpu->cc->tcg_ops->do_transaction_failed) {
        hwaddr physaddr = full->phys_addr | (addr & ~TARGET_PAGE_MASK);

        cpu->cc->tcg_ops->do_transaction_failed(cpu, physaddr, addr, size, access_type, mmu_idx, full->attrs, response,
                                                retaddr);
    }
}
...
static bool victim_tlb_hit(CPUState* cpu, size_t mmu_idx, size_t index, MMUAccessType access_type, vaddr page)
{
    size_t vidx;
...
static void notdirty_write(CPUState* cpu, vaddr mem_vaddr, unsigned size, CPUTLBEntryFull* full, uintptr_t retaddr)
{
...
static int probe_access_internal(CPUState* cpu, vaddr addr, int fault_size, MMUAccessType access_type, int mmu_idx,
...
{
    int flags = probe_access_internal(env_cpu(env), addr, size, access_type, mmu_idx, nonfault, phost, pfull, retaddr);
                          ...
                          CPUTLBEntryFull** pfull)
{
    ...
    int flags = probe_access_internal(env_cpu(env), addr, size, access_type, mmu_idx, true, phost, pfull, 0);
                       ...
                       void** phost, uintptr_t retaddr)
{
    ...
    flags = probe_access_internal(env_cpu(env), addr, size, access_type, mmu_idx, nonfault, phost, &full, retaddr);
...
void* probe_access(CPUArchState* env, vaddr addr, int size, MMUAccessType access_type, int mmu_idx, uintptr_t retaddr)
{
    ...
    flags = probe_access_internal(env_cpu(env), addr, size, access_type, mmu_idx, false, &host, &full, retaddr);
    ...
    if (unlikely(flags & (TLB_NOTDIRTY | TLB_WATCHPOINT))) {
        ...
        if (flags & TLB_WATCHPOINT) {
            int wp_access = (access_type == MMU_DATA_STORE ? BP_MEM_WRITE : BP_MEM_READ);
            cpu_check_watchpoint(env_cpu(env), addr, size, full->attrs, wp_access, retaddr);
        }

        /* Handle clean RAM pages.  */
        if (flags & TLB_NOTDIRTY) { notdirty_write(env_cpu(env), addr, size, full, retaddr); }
    }
...
void* tlb_vaddr_to_host(CPUArchState* env, vaddr addr, MMUAccessType access_type, int mmu_idx)
{
    ...
    flags = probe_access_internal(env_cpu(env), addr, 0, access_type, mmu_idx, true, &host, &full, 0);
...
tb_page_addr_t get_page_addr_code_hostp(CPUArchState* env, vaddr addr, void** hostp)
{
    ...
    (void)probe_access_internal(env_cpu(env), addr, 1, MMU_INST_FETCH, cpu_mmu_index(env_cpu(env), true), false, hostp,
                                &full, 0);
 ...
 */
static bool mmu_lookup1(CPUState* cpu, MMULookupPageData* data, MemOp memop, int mmu_idx, MMUAccessType access_type,
                        uintptr_t ra)
{
    ...
    if (!tlb_hit(tlb_addr, addr)) {
        if (!victim_tlb_hit(cpu, mmu_idx, index, access_type, addr & TARGET_PAGE_MASK)) {
            tlb_fill_align(cpu, addr, access_type, mmu_idx, memop, data->size, false, ra);
            maybe_resized = true;
            index         = tlb_index(cpu, mmu_idx, addr);
            entry         = tlb_entry(cpu, mmu_idx, addr);
        }
    ...
    if (likely(!maybe_resized)) {
        ...
        if (unlikely(addr & ((1 << a_bits) - 1))) { cpu_unaligned_access(cpu, addr, access_type, mmu_idx, ra); }
    }
 ...
 */
static void mmu_watch_or_dirty(CPUState* cpu, MMULookupPageData* data, MMUAccessType access_type, uintptr_t ra)
{
 ...
 */
static bool mmu_lookup(CPUState* cpu, vaddr addr, MemOpIdx oi, uintptr_t ra, MMUAccessType type, MMULookupLocals* l)
{
    bool  crosspage;
    vaddr last;
    int   flags;
    ...
    if (likely(!crosspage)) {
        ...
        if (unlikely(flags & (TLB_WATCHPOINT | TLB_NOTDIRTY))) { mmu_watch_or_dirty(cpu, &l->page[0], type, ra); }
        if (unlikely(flags & TLB_BSWAP)) { l->memop ^= MO_BSWAP; }
    }
    else {
        ...
        l->page[0].size = size0;
        l->page[1].addr = cpu->cc->tcg_ops->pointer_wrap(cpu, l->mmu_idx, addr1, addr);
 ...
 */
static void* atomic_mmu_lookup(CPUState* cpu, vaddr addr, MemOpIdx oi, int size, uintptr_t retaddr)
{
    ...
    if (!tlb_hit(tlb_addr, addr)) {
        if (!victim_tlb_hit(cpu, mmu_idx, index, MMU_DATA_STORE, addr & TARGET_PAGE_MASK)) {
    ...
    if (unlikely(tlb_addr & TLB_NOTDIRTY)) { notdirty_write(cpu, addr, size, full, retaddr); }

    if (unlikely(tlb_addr & TLB_WATCHPOINT)) {
        int wp_flags = 0;

        if (full->slow_flags[MMU_DATA_STORE] & TLB_WATCHPOINT) { wp_flags |= BP_MEM_WRITE; }
        if (full->slow_flags[MMU_DATA_LOAD] & TLB_WATCHPOINT) { wp_flags |= BP_MEM_READ; }
        cpu_check_watchpoint(cpu, addr, size, full->attrs, wp_flags, retaddr);
    }
 ...
 * @mmu_idx: virtual address context
 * @ra: return address into tcg generated code, or 0
 * Context: BQL held unless the target MemoryRegion opted into lockless I/O
 *
 * Load @size bytes from @addr, which is memory-mapped i/o.
 * The bytes are concatenated in big-endian order with @ret_be.
 */
static uint64_t int_ld_mmio_beN(CPUState* cpu, CPUTLBEntryFull* full, uint64_t ret_be, vaddr addr, int size,
                                int mmu_idx, MMUAccessType type, uintptr_t ra, MemoryRegion* mr, hwaddr mr_offset)
{
    do {
        MemOp       this_mop;
        unsigned    this_size;
        ...
        r = memory_region_dispatch_read(mr, mr_offset, &val, this_mop, full->attrs);
        if (unlikely(r != MEMTX_OK)) { io_failed(cpu, full, addr, this_size, type, mmu_idx, r, ra); }
        if (this_size == 8) { return val; }
...
static uint64_t do_ld_mmio_beN(CPUState* cpu, CPUTLBEntryFull* full, uint64_t ret_be, vaddr addr, int size, int mmu_idx,
                               MMUAccessType type, uintptr_t ra)
{
    ...
    BQL_LOCK_GUARD_IF(!mr->lockless_io);
    return int_ld_mmio_beN(cpu, full, ret_be, addr, size, mmu_idx, type, ra, mr, mr_offset);
}

static Int128 do_ld16_mmio_beN(CPUState* cpu, CPUTLBEntryFull* full, uint64_t ret_be, vaddr addr, int size, int mmu_idx,
                               uintptr_t ra)
{
    MemoryRegionSection* section;
    MemoryRegion*        mr;
    ...
    BQL_LOCK_GUARD_IF(!mr->lockless_io);
    a = int_ld_mmio_beN(cpu, full, ret_be, addr, size - 8, mmu_idx, MMU_DATA_LOAD, ra, mr, mr_offset);
    b = int_ld_mmio_beN(cpu, full, ret_be, addr + size - 8, 8, mmu_idx, MMU_DATA_LOAD, ra, mr, mr_offset + size - 8);
...
static uint64_t do_ld_bytes_beN(MMULookupPageData* p, uint64_t ret_be)
{
    ...
    for (i = 0; i < size; i++) { ret_be = (ret_be << 8) | haddr[i]; }
    return ret_be;
}
 ...
 */
static uint64_t do_ld_whole_be8(CPUState* cpu, uintptr_t ra, MMULookupPageData* p, uint64_t ret_be)
{
    int      o = p->addr & 7;
 ...
 */
static Int128 do_ld_whole_be16(CPUState* cpu, uintptr_t ra, MMULookupPageData* p, uint64_t ret_be)
{
    int    o = p->addr & 15;
    Int128 x, y = load_atomic16_or_exit(cpu, ra, p->haddr - o);
    int    size = p->size;

    if (!HOST_BIG_ENDIAN) { y = bswap128(y); }
 ...
 */
static uint64_t do_ld_beN(CPUState* cpu, MMULookupPageData* p, uint64_t ret_be, int mmu_idx, MMUAccessType type,
                          MemOp mop, uintptr_t ra)
{
    ...
    if (unlikely(p->flags & TLB_MMIO)) {
        return do_ld_mmio_beN(cpu, p->full, ret_be, p->addr, p->size, mmu_idx, type, ra);
    }
    ...
    switch (atom) {
        case MO_ATOM_SUBALIGN: return do_ld_parts_beN(p, ret_be);
        ...
        case MO_ATOM_WITHIN16_PAIR:
            ...
            tmp       = tmp ? tmp - 1 : 0;
            half_size = 1 << tmp;
            if (atom == MO_ATOM_IFALIGN_PAIR ? p->size == half_size : p->size >= half_size) {
                if (!HAVE_al8_fast && p->size < 4) { return do_ld_whole_be4(p, ret_be); }
                else {
 ...
 */
static Int128 do_ld16_beN(CPUState* cpu, MMULookupPageData* p, uint64_t a, int mmu_idx, MemOp mop, uintptr_t ra)
{
    int      size = p->size;
    uint64_t b;
    MemOp    atom;

    if (unlikely(p->flags & TLB_MMIO)) { return do_ld16_mmio_beN(cpu, p->full, a, p->addr, size, mmu_idx, ra); }
...
static uint8_t do_ld_1(CPUState* cpu, MMULookupPageData* p, int mmu_idx, MMUAccessType type, uintptr_t ra)
{
    if (unlikely(p->flags & TLB_MMIO)) { return do_ld_mmio_beN(cpu, p->full, 0, p->addr, 1, mmu_idx, type, ra); }
    else {
        return *(uint8_t*)p->haddr;
    }
}

static uint16_t do_ld_2(CPUState* cpu, MMULookupPageData* p, int mmu_idx, MMUAccessType type, MemOp memop, uintptr_t ra)
{
    uint16_t ret;

    if (unlikely(p->flags & TLB_MMIO)) {
        ret = do_ld_mmio_beN(cpu, p->full, 0, p->addr, 2, mmu_idx, type, ra);
        if ((memop & MO_BSWAP) == MO_LE) { ret = bswap16(ret); }
    }
    else {
        /* Perform the load host endian, then swap if necessary. */
        ret = load_atom_2(cpu, ra, p->haddr, memop);
        if (memop & MO_BSWAP) { ret = bswap16(ret); }
    }
    return ret;
}

static uint32_t do_ld_4(CPUState* cpu, MMULookupPageData* p, int mmu_idx, MMUAccessType type, MemOp memop, uintptr_t ra)
{
    uint32_t ret;

    if (unlikely(p->flags & TLB_MMIO)) {
        ret = do_ld_mmio_beN(cpu, p->full, 0, p->addr, 4, mmu_idx, type, ra);
        if ((memop & MO_BSWAP) == MO_LE) { ret = bswap32(ret); }
    }
    else {
        /* Perform the load host endian. */
        ret = load_atom_4(cpu, ra, p->haddr, memop);
        if (memop & MO_BSWAP) { ret = bswap32(ret); }
    }
    return ret;
}

static uint64_t do_ld_8(CPUState* cpu, MMULookupPageData* p, int mmu_idx, MMUAccessType type, MemOp memop, uintptr_t ra)
{
    uint64_t ret;

    if (unlikely(p->flags & TLB_MMIO)) {
        ret = do_ld_mmio_beN(cpu, p->full, 0, p->addr, 8, mmu_idx, type, ra);
        if ((memop & MO_BSWAP) == MO_LE) { ret = bswap64(ret); }
    }
    else {
        /* Perform the load host endian. */
        ret = load_atom_8(cpu, ra, p->haddr, memop);
        if (memop & MO_BSWAP) { ret = bswap64(ret); }
    }
    return ret;
}

static uint8_t do_ld1_mmu(CPUState* cpu, vaddr addr, MemOpIdx oi, uintptr_t ra, MMUAccessType access_type)
{
    MMULookupLocals l;
    bool            crosspage;
...
static uint16_t do_ld2_mmu(CPUState* cpu, vaddr addr, MemOpIdx oi, uintptr_t ra, MMUAccessType access_type)
{
    MMULookupLocals l;
    bool            crosspage;
    uint16_t        ret;
    ...
    cpu_req_mo(cpu, TCG_MO_LD_LD | TCG_MO_ST_LD);
    crosspage = mmu_lookup(cpu, addr, oi, ra, access_type, &l);
    if (likely(!crosspage)) { return do_ld_2(cpu, &l.page[0], l.mmu_idx, access_type, l.memop, ra); }

    a = do_ld_1(cpu, &l.page[0], l.mmu_idx, access_type, ra);
    b = do_ld_1(cpu, &l.page[1], l.mmu_idx, access_type, ra);

    if ((l.memop & MO_BSWAP) == MO_LE) { ret = a | (b << 8); }
    else {
        ret = b | (a << 8);
    }
    return ret;
...
static uint32_t do_ld4_mmu(CPUState* cpu, vaddr addr, MemOpIdx oi, uintptr_t ra, MMUAccessType access_type)
{
    MMULookupLocals l;
    bool            crosspage;
    uint32_t        ret;
    ...
    cpu_req_mo(cpu, TCG_MO_LD_LD | TCG_MO_ST_LD);
    crosspage = mmu_lookup(cpu, addr, oi, ra, access_type, &l);
    if (likely(!crosspage)) { return do_ld_4(cpu, &l.page[0], l.mmu_idx, access_type, l.memop, ra); }
    ...
    if ((l.memop & MO_BSWAP) == MO_LE) { ret = bswap32(ret); }
    return ret;
}

static uint64_t do_ld8_mmu(CPUState* cpu, vaddr addr, MemOpIdx oi, uintptr_t ra, MMUAccessType access_type)
{
    MMULookupLocals l;
    bool            crosspage;
    uint64_t        ret;
    ...
    cpu_req_mo(cpu, TCG_MO_LD_LD | TCG_MO_ST_LD);
    crosspage = mmu_lookup(cpu, addr, oi, ra, access_type, &l);
    if (likely(!crosspage)) { return do_ld_8(cpu, &l.page[0], l.mmu_idx, access_type, l.memop, ra); }

    ret = do_ld_beN(cpu, &l.page[0], 0, l.mmu_idx, access_type, l.memop, ra);
    ret = do_ld_beN(cpu, &l.page[1], ret, l.mmu_idx, access_type, l.memop, ra);
    if ((l.memop & MO_BSWAP) == MO_LE) { ret = bswap64(ret); }
    return ret;
}

static Int128 do_ld16_mmu(CPUState* cpu, vaddr addr, MemOpIdx oi, uintptr_t ra)
{
    MMULookupLocals l;
    bool            crosspage;
    uint64_t        a, b;
    ...
    if (likely(!crosspage)) {
        if (unlikely(l.page[0].flags & TLB_MMIO)) {
            ret = do_ld16_mmio_beN(cpu, l.page[0].full, 0, addr, 16, l.mmu_idx, ra);
            if ((l.memop & MO_BSWAP) == MO_LE) { ret = bswap128(ret); }
        }
        else {
            /* Perform the load host endian. */
            ret = load_atom_16(cpu, ra, l.page[0].haddr, l.memop);
            if (l.memop & MO_BSWAP) { ret = bswap128(ret); }
        }
        return ret;
    }
    ...
    if (first == 8) {
        ...
        if ((mop8 & MO_BSWAP) == MO_LE) { ret = int128_make128(a, b); }
        else {
    ...
    if (first < 8) {
        a   = do_ld_beN(cpu, &l.page[0], 0, l.mmu_idx, MMU_DATA_LOAD, l.memop, ra);
        ...
        a   = int128_gethi(ret);
        b   = do_ld_beN(cpu, &l.page[1], b, l.mmu_idx, MMU_DATA_LOAD, l.memop, ra);
        ret = int128_make128(b, a);
    }
    if ((l.memop & MO_BSWAP) == MO_LE) { ret = bswap128(ret); }
    return ret;
 ...
 */
static uint64_t int_st_mmio_leN(CPUState* cpu, CPUTLBEntryFull* full, uint64_t val_le, vaddr addr, int size,
                                int mmu_idx, uintptr_t ra, MemoryRegion* mr, hwaddr mr_offset)
{
    do {
        MemOp       this_mop;
        unsigned    this_size;
        ...
        r = memory_region_dispatch_write(mr, mr_offset, val_le, this_mop, full->attrs);
        if (unlikely(r != MEMTX_OK)) { io_failed(cpu, full, addr, this_size, MMU_DATA_STORE, mmu_idx, r, ra); }
...
static uint64_t do_st_mmio_leN(CPUState* cpu, CPUTLBEntryFull* full, uint64_t val_le, vaddr addr, int size, int mmu_idx,
                               uintptr_t ra)
{
    MemoryRegionSection* section;
    hwaddr               mr_offset;
    ...
    BQL_LOCK_GUARD_IF(!mr->lockless_io);
    return int_st_mmio_leN(cpu, full, val_le, addr, size, mmu_idx, ra, mr, mr_offset);
}

static uint64_t do_st16_mmio_leN(CPUState* cpu, CPUTLBEntryFull* full, Int128 val_le, vaddr addr, int size, int mmu_idx,
                                 uintptr_t ra)
{
    MemoryRegionSection* section;
    MemoryRegion*        mr;
    ...
    BQL_LOCK_GUARD_IF(!mr->lockless_io);
    int_st_mmio_leN(cpu, full, int128_getlo(val_le), addr, 8, mmu_idx, ra, mr, mr_offset);
    return int_st_mmio_leN(cpu, full, int128_gethi(val_le), addr + 8, size - 8, mmu_idx, ra, mr, mr_offset + 8);
}

/*
 * Wrapper for the above.
 */
static uint64_t do_st_leN(CPUState* cpu, MMULookupPageData* p, uint64_t val_le, int mmu_idx, MemOp mop, uintptr_t ra)
{
    MemOp    atom;
    unsigned tmp, half_size;

    if (unlikely(p->flags & TLB_MMIO)) { return do_st_mmio_leN(cpu, p->full, val_le, p->addr, p->size, mmu_idx, ra); }
    ...
    atom = mop & MO_ATOM_MASK;
    switch (atom) {
        case MO_ATOM_SUBALIGN: return store_parts_leN(p->haddr, p->size, val_le);
        ...
        case MO_ATOM_WITHIN16_PAIR:
            ...
            half_size = 1 << tmp;
            if (atom == MO_ATOM_IFALIGN_PAIR ? p->size == half_size : p->size >= half_size) {
                if (!HAVE_al8_fast && p->size <= 4) { return store_whole_le4(p->haddr, p->size, val_le); }
                else if (HAVE_al8) {
 ...
 * Wrapper for the above, for 8 < size < 16.
 */
static uint64_t do_st16_leN(CPUState* cpu, MMULookupPageData* p, Int128 val_le, int mmu_idx, MemOp mop, uintptr_t ra)
{
    int   size = p->size;
    MemOp atom;

    if (unlikely(p->flags & TLB_MMIO)) { return do_st16_mmio_leN(cpu, p->full, val_le, p->addr, size, mmu_idx, ra); }
    else if (unlikely(p->flags & TLB_DISCARD_WRITE)) {
    ...
    switch (atom) {
        ...
        case MO_ATOM_WITHIN16_PAIR:
            /* Since size > 8, this is the half that must be atomic. */
            if (!HAVE_CMPXCHG128) { cpu_loop_exit_atomic(cpu, ra); }
...
static void do_st_1(CPUState* cpu, MMULookupPageData* p, uint8_t val, int mmu_idx, uintptr_t ra)
{
    if (unlikely(p->flags & TLB_MMIO)) { do_st_mmio_leN(cpu, p->full, val, p->addr, 1, mmu_idx, ra); }
    else if (unlikely(p->flags & TLB_DISCARD_WRITE)) {
        /* nothing */
    }
    else {
...
static void do_st_2(CPUState* cpu, MMULookupPageData* p, uint16_t val, int mmu_idx, MemOp memop, uintptr_t ra)
{
    if (unlikely(p->flags & TLB_MMIO)) {
        if ((memop & MO_BSWAP) != MO_LE) { val = bswap16(val); }
        ...
        if (memop & MO_BSWAP) { val = bswap16(val); }
        store_atom_2(cpu, ra, p->haddr, memop, val);
    }
}

static void do_st_4(CPUState* cpu, MMULookupPageData* p, uint32_t val, int mmu_idx, MemOp memop, uintptr_t ra)
{
    if (unlikely(p->flags & TLB_MMIO)) {
        if ((memop & MO_BSWAP) != MO_LE) { val = bswap32(val); }
        ...
        /* Swap to host endian if necessary, then store. */
        if (memop & MO_BSWAP) { val = bswap32(val); }
        store_atom_4(cpu, ra, p->haddr, memop, val);
    }
}

static void do_st_8(CPUState* cpu, MMULookupPageData* p, uint64_t val, int mmu_idx, MemOp memop, uintptr_t ra)
{
    if (unlikely(p->flags & TLB_MMIO)) {
        if ((memop & MO_BSWAP) != MO_LE) { val = bswap64(val); }
        ...
        /* Swap to host endian if necessary, then store. */
        if (memop & MO_BSWAP) { val = bswap64(val); }
        store_atom_8(cpu, ra, p->haddr, memop, val);
    }
}

static void do_st1_mmu(CPUState* cpu, vaddr addr, uint8_t val, MemOpIdx oi, uintptr_t ra)
{
    MMULookupLocals l;
    bool            crosspage;
...
static void do_st2_mmu(CPUState* cpu, vaddr addr, uint16_t val, MemOpIdx oi, uintptr_t ra)
{
    MMULookupLocals l;
    bool            crosspage;
    uint8_t         a, b;
    ...
    if ((l.memop & MO_BSWAP) == MO_LE) { a = val, b = val >> 8; }
    else {
        b = val, a = val >> 8;
    }
...
static void do_st4_mmu(CPUState* cpu, vaddr addr, uint32_t val, MemOpIdx oi, uintptr_t ra)
{
    MMULookupLocals l;
    bool            crosspage;
    ...
    if ((l.memop & MO_BSWAP) != MO_LE) { val = bswap32(val); }
    val = do_st_leN(cpu, &l.page[0], val, l.mmu_idx, l.memop, ra);
    (void)do_st_leN(cpu, &l.page[1], val, l.mmu_idx, l.memop, ra);
}

static void do_st8_mmu(CPUState* cpu, vaddr addr, uint64_t val, MemOpIdx oi, uintptr_t ra)
{
    MMULookupLocals l;
    bool            crosspage;
    ...
    /* Swap to little endian for simplicity, then store by bytes. */
    if ((l.memop & MO_BSWAP) != MO_LE) { val = bswap64(val); }
    val = do_st_leN(cpu, &l.page[0], val, l.mmu_idx, l.memop, ra);
    (void)do_st_leN(cpu, &l.page[1], val, l.mmu_idx, l.memop, ra);
}

static void do_st16_mmu(CPUState* cpu, vaddr addr, Int128 val, MemOpIdx oi, uintptr_t ra)
{
    MMULookupLocals l;
    bool            crosspage;
    uint64_t        a, b;
    ...
    if (likely(!crosspage)) {
        if (unlikely(l.page[0].flags & TLB_MMIO)) {
            if ((l.memop & MO_BSWAP) != MO_LE) { val = bswap128(val); }
            ...
            /* Swap to host endian if necessary, then store. */
            if (l.memop & MO_BSWAP) { val = bswap128(val); }
    ...
    if (first == 8) {
        ...
        if (l.memop & MO_BSWAP) { val = bswap128(val); }
        if (HOST_BIG_ENDIAN) { b = int128_getlo(val), a = int128_gethi(val); }
        else {
...
uint8_t cpu_ldb_code_mmu(CPUArchState* env, vaddr addr, MemOpIdx oi, uintptr_t retaddr)
{ return do_ld1_mmu(env_cpu(env), addr, oi, retaddr, MMU_INST_FETCH); }

uint16_t cpu_ldw_code_mmu(CPUArchState* env, vaddr addr, MemOpIdx oi, uintptr_t retaddr)
{ return do_ld2_mmu(env_cpu(env), addr, oi, retaddr, MMU_INST_FETCH); }

uint32_t cpu_ldl_code_mmu(CPUArchState* env, vaddr addr, MemOpIdx oi, uintptr_t retaddr)
{ return do_ld4_mmu(env_cpu(env), addr, oi, retaddr, MMU_INST_FETCH); }

uint64_t cpu_ldq_code_mmu(CPUArchState* env, vaddr addr, MemOpIdx oi, uintptr_t retaddr)
 ...
 */
vaddr cpu_pointer_wrap_notreached(CPUState* cs, int idx, vaddr res, vaddr base) { assert_not_reached(); }

/* To be used for strict 32-bit targets. */
vaddr cpu_pointer_wrap_uint32(CPUState* cs, int idx, vaddr res, vaddr base) { return (uint32_t)res; }
