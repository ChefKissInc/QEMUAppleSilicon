...
static CommandLineParameterInfoList* query_option_descs(const QemuOptDesc* desc)
{
    ...
    for (i = 0; desc[i].name != NULL; i++) {
        ...
        switch (desc[i].type) {
            case QEMU_OPT_STRING: info->type = COMMAND_LINE_PARAMETER_TYPE_STRING; break;
            case QEMU_OPT_BOOL  : info->type = COMMAND_LINE_PARAMETER_TYPE_BOOLEAN; break;
            case QEMU_OPT_NUMBER: info->type = COMMAND_LINE_PARAMETER_TYPE_NUMBER; break;
            case QEMU_OPT_SIZE  : info->type = COMMAND_LINE_PARAMETER_TYPE_SIZE; break;
        }
...
static void connect_infolist(CommandLineParameterInfoList* head, CommandLineParameterInfoList* new)
{
...
static CommandLineParameterInfoList* get_drive_infolist(void)
{
    ...
    for (i = 0; drive_config_groups[i] != NULL; i++) {
        if (!head) { head = query_option_descs(drive_config_groups[i]->desc); }
        else {
...
static CommandLineParameterInfo* objprop_to_cmdline_prop(ObjectProperty* prop)
{
    ...
    if (prop->description) { info->help = g_strdup(prop->description); }
...
CommandLineOptionInfoList* qmp_query_command_line_options(const char* option, Error** errp)
{
    ...
    for (i = 0; vm_config_groups[i] != NULL; i++) {
        if (!option || !strcmp(option, vm_config_groups[i]->name)) {
            ...
            if (!strcmp("drive", vm_config_groups[i]->name)) { info->parameters = get_drive_infolist(); }
            else {
    ...
    if (conf_list == NULL) { error_setg(errp, "invalid option name: %s", option); }
