...
static StringInputVisitor* to_siv(Visitor* v) { return container_of(v, StringInputVisitor, visitor); }
...
static int try_parse_int64_list_entry(StringInputVisitor* siv, int64_t* obj)
{
    ...
    if (qemu_strtoi64(siv->unparsed_string, &endptr, 0, &start)) { return -EINVAL; }
    end = start;

    switch (endptr[0]) {
        case '\0': siv->unparsed_string = endptr; break;
        case ',' : siv->unparsed_string = endptr + 1; break;
        case '-':
            /* parse the end of the range */
            if (qemu_strtoi64(endptr + 1, &endptr, 0, &end)) { return -EINVAL; }
            if (start > end || end - start >= RANGE_MAX_ELEMENTS) { return -EINVAL; }
            switch (endptr[0]) {
                case '\0': siv->unparsed_string = endptr; break;
                case ',' : siv->unparsed_string = endptr + 1; break;
...
static bool parse_type_int64(Visitor* v, const char* name, int64_t* obj, Error** errp)
{
    StringInputVisitor* siv = to_siv(v);
    int64_t             val;
    ...
    switch (siv->lm) {
        case LM_NONE:
            ...
            if (qemu_strtoi64(siv->string, NULL, 0, &val)) {
                error_setg(errp, QERR_INVALID_PARAMETER_VALUE, name ? name : "null", "int64");
                return false;
            }
            *obj = val;
            return true;
        case LM_UNPARSED:
            if (try_parse_int64_list_entry(siv, obj)) {
                error_setg(errp, QERR_INVALID_PARAMETER_VALUE, name ? name : "null", "list of int64 values or ranges");
                return false;
            }
        ...
        case LM_INT64_RANGE:
            ...
            *obj = siv->rangeNext.i64++;
            ...
            if (siv->rangeNext.i64 > siv->rangeEnd.i64 || *obj == INT64_MAX) {
                ...
                siv->lm = siv->unparsed_string[0] ? LM_UNPARSED:
LM_END
;
}
return true;
...
default    : abort();
    ...
    static int try_parse_uint64_list_entry(StringInputVisitor* siv, uint64_t* obj)
    {
        ...
        if (qemu_strtou64(siv->unparsed_string, &endptr, 0, &start)) { return -EINVAL; }
        end = start;
        ...
        switch (endptr[0]) {
            ...
            case '-':
                /* parse the end of the range */
                if (qemu_strtou64(endptr + 1, &endptr, 0, &end)) { return -EINVAL; }
                if (start > end || end - start >= RANGE_MAX_ELEMENTS) { return -EINVAL; }
                switch (endptr[0]) {
                    case '\0': siv->unparsed_string = endptr; break;
                    case ',' : siv->unparsed_string = endptr + 1; break;
    ...
    static bool parse_type_uint64(Visitor* v, const char* name, uint64_t* obj, Error** errp)
    {
        StringInputVisitor* siv = to_siv(v);
        uint64_t            val;
        ...
        switch (siv->lm) {
            case LM_NONE:
                ...
                if (qemu_strtou64(siv->string, NULL, 0, &val)) {
                    error_setg(errp, QERR_INVALID_PARAMETER_VALUE, name ? name : "null", "uint64");
                    return false;
                }
                *obj = val;
                return true;
...
default    : abort();
    ...
    static bool parse_type_size(Visitor* v, const char* name, uint64_t* obj, Error** errp)
    {
        StringInputVisitor* siv = to_siv(v);
        uint64_t            val;

        assert(siv->lm == LM_NONE);
        if (!parse_option_size(name, siv->string, &val, errp)) { return false; }

        *obj = val;
        return true;
    }

    static bool parse_type_bool(Visitor* v, const char* name, bool* obj, Error** errp)
    {
        StringInputVisitor* siv = to_siv(v);
    ...
    static bool parse_type_str(Visitor* v, const char* name, char** obj, Error** errp)
    {
        StringInputVisitor* siv = to_siv(v);
    ...
    static bool parse_type_number(Visitor* v, const char* name, double* obj, Error** errp)
    {
        StringInputVisitor* siv = to_siv(v);
        double              val;
        ...
        if (qemu_strtod_finite(siv->string, NULL, &val)) {
            error_setg(errp, "Invalid parameter type for '%s', expected: number", name ? name : "null");
            return false;
        }
    ...
    static bool parse_type_null(Visitor* v, const char* name, QNull** obj, Error** errp)
    {
        StringInputVisitor* siv = to_siv(v);
        ...
        *obj = NULL;
        ...
        if (siv->string[0]) {
            error_setg(errp, "Invalid parameter type for '%s', expected: null", name ? name : "null");
            return false;
        }
