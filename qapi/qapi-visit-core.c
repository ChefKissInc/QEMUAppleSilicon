...
void visit_complete(Visitor* v, void* opaque)
{
    ...
    if (v->complete) { v->complete(v, opaque); }
}
...
bool visit_start_struct(Visitor* v, const char* name, void** obj, size_t size, Error** errp)
{
    ...
    if (obj && (v->type & VISITOR_INPUT)) { assert(ok != !*obj); }
    return ok;
}
...
bool visit_start_list(Visitor* v, const char* name, GenericList** list, size_t size, Error** errp)
{
    ...
    if (list && (v->type & VISITOR_INPUT)) { assert(ok || !*list); }
    return ok;
}
...
bool visit_start_alternate(Visitor* v, const char* name, GenericAlternate** obj, size_t size, Error** errp)
{
    ...
    if (v->type & VISITOR_INPUT) { assert(ok != !*obj); }
    return ok;
}
...
void visit_end_alternate(Visitor* v, void** obj)
{
    ...
    if (v->end_alternate) { v->end_alternate(v, obj); }
}

bool visit_optional(Visitor* v, const char* name, bool* present)
{
    trace_visit_optional(v, name, present);
    if (v->optional) { v->optional(v, name, present); }
    return *present;
}
...
bool visit_policy_reject(Visitor* v, const char* name, uint64_t features, Error** errp)
{
    ...
    if (v->policy_reject) { return v->policy_reject(v, name, features, errp); }
    return false;
}
...
bool visit_policy_skip(Visitor* v, const char* name, uint64_t features)
{
    ...
    if (v->policy_skip) { return v->policy_skip(v, name, features); }
    return false;
}

void visit_set_policy(Visitor* v, CompatPolicy* policy) { v->compat_policy = *policy; }

bool visit_is_input(Visitor* v) { return v->type == VISITOR_INPUT; }

bool visit_is_dealloc(Visitor* v) { return v->type == VISITOR_DEALLOC; }
...
static bool visit_type_uintN(Visitor* v, uint64_t* obj, const char* name, uint64_t max, const char* type, Error** errp)
{
    uint64_t value = *obj;

    assert(v->type == VISITOR_INPUT || value <= max);

    if (!v->type_uint64(v, name, &value, errp)) { return false; }
    if (value > max) {
        assert(v->type == VISITOR_INPUT);
                            ...
                            Error** errp)
{
    ...
    if (!v->type_int64(v, name, &value, errp)) { return false; }
...
bool visit_type_size(Visitor* v, const char* name, uint64_t* obj, Error** errp)
{
    ...
    if (v->type_size) { return v->type_size(v, name, obj, errp); }
    return v->type_uint64(v, name, obj, errp);
}
...
static bool input_type_enum(Visitor* v, const char* name, int* obj, const QEnumLookup* lookup, Error** errp)
{
    ...
    if (!visit_type_str(v, name, &enum_str, errp)) { return false; }
    ...
    if (lookup->features
        && !compat_policy_input_ok(lookup->features[value], &v->compat_policy, ERROR_CLASS_GENERIC_ERROR, "value",
                                   enum_str, errp))
    {
        return false;
    }
...
bool visit_type_enum(Visitor* v, const char* name, int* obj, const QEnumLookup* lookup, Error** errp)
{
    ...
    switch (v->type) {
        case VISITOR_INPUT : return input_type_enum(v, name, obj, lookup, errp);
        case VISITOR_OUTPUT: return output_type_enum(v, name, obj, lookup, errp);
        case VISITOR_CLONE:
