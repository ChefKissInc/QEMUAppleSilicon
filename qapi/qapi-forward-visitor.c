...
static ForwardFieldVisitor* to_ffv(Visitor* v) { return container_of(v, ForwardFieldVisitor, visitor); }

static bool forward_field_translate_name(ForwardFieldVisitor* v, const char** name, Error** errp)
{
    if (v->depth) { return true; }
...
static bool forward_field_start_struct(Visitor* v, const char* name, void** obj, size_t size, Error** errp)
{
    ...
    if (!forward_field_translate_name(ffv, &name, errp)) { return false; }
    if (!visit_start_struct(ffv->target, name, obj, size, errp)) { return false; }
    ffv->depth++;
    return true;
}
...
static bool forward_field_policy_reject(Visitor* v, const char* name, uint64_t features, Error** errp)
{
    ForwardFieldVisitor* ffv = to_ffv(v);

    if (!forward_field_translate_name(ffv, &name, errp)) { return true; }
    return visit_policy_reject(ffv->target, name, features, errp);
}

static bool forward_field_policy_skip(Visitor* v, const char* name, uint64_t features)
{
    ForwardFieldVisitor* ffv = to_ffv(v);

    if (!forward_field_translate_name(ffv, &name, NULL)) { return true; }
