...
static QObjectOutputVisitor* to_qov(Visitor* v) { return container_of(v, QObjectOutputVisitor, visitor); }
...
static GenericList* qobject_output_next_list(Visitor* v, GenericList* tail, size_t size) { return tail->next; }
...
{
    QObjectOutputVisitor* qov = to_qov(v);
    if (*obj) { qobject_output_add(qov, name, qstring_from_str(*obj)); }
    else {
...
static bool qobject_output_policy_skip(Visitor* v, const char* name, uint64_t features)
{
    CompatPolicy* pol = &v->compat_policy;

    return ((features & 1u << QAPI_DEPRECATED) && pol->deprecated_output == COMPAT_POLICY_OUTPUT_HIDE)
           || ((features & 1u << QAPI_UNSTABLE) && pol->unstable_output == COMPAT_POLICY_OUTPUT_HIDE);
}
