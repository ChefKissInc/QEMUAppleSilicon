...
void hmp_qom_set(Monitor* mon, const QDict* qdict)
{
    ...
    if (!json) {
        ...
        if (!obj) { error_set(&err, ERROR_CLASS_DEVICE_NOT_FOUND, "Device '%s' not found", path); }
        else {
        ...
        if (!err) { qmp_qom_set(path, property, obj, &err); }
    }
...
static void print_qom_composition(Monitor* mon, Object* obj, int indent)
{
    ...
    if (obj == object_get_root()) { name = ""; }
    else {
        name = object_get_canonical_path_component(obj);
    }
    monitor_printf(mon, "%*s/%s (%s)\n", indent, "", name, object_get_typename(obj));

    object_child_foreach(obj, insert_qom_composition_child, children);
    g_array_sort(children, qom_composition_compare);

    for (i = 0; i < children->len; i++) { print_qom_composition(mon, g_array_index(children, Object*, i), indent + 2); }
...
void object_add_completion(ReadLineState* rs, int nb_args, const char* str)
{
    ...
    while (elt) {
        ...
        if (strcmp(name, TYPE_USER_CREATABLE)) { readline_add_completion_of(rs, str, name); }
        elt = elt->next;
    }
    g_slist_free(list);
}
...
void object_del_completion(ReadLineState* rs, int nb_args, const char* str)
{
    ...
    while (list) {
        ...
        if (!strncmp(info->type, "child<", 5)) { readline_add_completion_of(rs, str, info->name); }
        list = list->next;
    }
    qapi_free_ObjectPropertyInfoList(start);
}
