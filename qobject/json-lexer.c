    ...
    while (flush ? lexer->state != lexer->start_state : !char_consumed) {
        ...
        switch (new_state) {
            ...
            case JSON_ERROR:
                ...
                /* fall through */
            case IN_RECOVERY: g_string_truncate(lexer->token, 0); break;
            default         : break;
        }
...
void json_lexer_feed(JSONLexer* lexer, const char* buffer, size_t size)
{
    ...
    for (i = 0; i < size; i++) { json_lexer_feed_char(lexer, buffer[i], false); }
}
...
void json_lexer_destroy(JSONLexer* lexer) { g_string_free(lexer->token, true); }
