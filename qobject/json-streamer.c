...
static void json_message_free_tokens(JSONMessageParser* parser)
{
    ...
    while ((token = g_queue_pop_head(&parser->tokens))) { g_free(token); }
}
...
void json_message_process_token(JSONLexer* lexer, GString* input, JSONTokenType type, int x, int y)
{
    ...
    switch (type) {
        case JSON_LCURLY : parser->brace_count++; break;
        case JSON_RCURLY : parser->brace_count--; break;
        case JSON_LSQUARE: parser->bracket_count++; break;
        case JSON_RSQUARE: parser->bracket_count--; break;
        case JSON_ERROR  : error_setg(&err, "JSON parse error, stray '%s'", input->str); goto out_emit;
        case JSON_END_OF_INPUT:
            if (g_queue_is_empty(&parser->tokens)) { return; }
