...
static int cvt4hex(const char* s)
{
    ...
    cp = 0;
    for (i = 0; i < 4; i++) {
        if (!qemu_isxdigit(s[i])) { return -1; }
        cp <<= 4;
        if (s[i] >= '0' && s[i] <= '9') { cp |= s[i] - '0'; }
        else if (s[i] >= 'a' && s[i] <= 'f') {
            cp |= 10 + s[i] - 'a';
        }
        else if (s[i] >= 'A' && s[i] <= 'F') {
        ...
        switch (*ptr) {
            case '\\':
                ...
                switch (*ptr++) {
                    ...
                    case '\\': g_string_append_c(str, '\\'); break;
                    case '/' : g_string_append_c(str, '/'); break;
                    case 'b' : g_string_append_c(str, '\b'); break;
                    case 'f' : g_string_append_c(str, '\f'); break;
                    ...
                    case 'u':
                        ...
                        break;
                    default: parse_error(ctxt, token, "invalid escape sequence in string"); goto out;
                }
                break;
            case '%':
                if (ctxt->ap) {
...
static JSONToken* parser_context_peek_token(JSONParserContext* ctxt) { return g_queue_peek_head(ctxt->buf); }
...
static QObject* parse_object(JSONParserContext* ctxt)
{
    ...
    if (peek->type != JSON_RCURLY) {
        if (parse_pair(ctxt, dict) == -1) { goto out; }
...
static QObject* parse_keyword(JSONParserContext* ctxt)
{
    ...
    if (!strcmp(token->str, "true")) { return QOBJECT(qbool_from_bool(true)); }
...
static QObject* parse_interpolation(JSONParserContext* ctxt)
{
    ...
    if (!strcmp(token->str, "%p")) { return va_arg(*ctxt->ap, QObject*); }
...
static QObject* parse_literal(JSONParserContext* ctxt)
{
    ...
    switch (token->type) {
        case JSON_STRING : return QOBJECT(parse_string(ctxt, token));
        case JSON_INTEGER: {
            /*
            ...
            if (!ret) { return QOBJECT(qnum_from_int(value)); }
            assert(ret == -ERANGE);

            if (token->str[0] != '-') {
                ret = qemu_strtou64(token->str, NULL, 10, &uvalue);
                if (!ret) { return QOBJECT(qnum_from_uint(uvalue)); }
                assert(ret == -ERANGE);
            }
        }
...
static QObject* parse_value(JSONParserContext* ctxt)
{
    ...
    switch (token->type) {
        case JSON_LCURLY : return parse_object(ctxt);
        case JSON_LSQUARE: return parse_array(ctxt);
        case JSON_INTERP : return parse_interpolation(ctxt);
        case JSON_INTEGER:
        case JSON_FLOAT  :
        case JSON_STRING : return parse_literal(ctxt);
        case JSON_KEYWORD: return parse_keyword(ctxt);
        default          : parse_error(ctxt, token, "expecting value"); return NULL;
    }
}
...
QObject* json_parser_parse(GQueue* tokens, va_list* ap, Error** errp)
{
    ...
    while (!g_queue_is_empty(ctxt.buf)) { parser_context_pop_token(&ctxt); }
    g_free(ctxt.current);
