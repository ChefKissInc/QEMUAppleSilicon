...
{
    if (writer) { g_string_free(json_writer_get_and_free(writer), true); }
}
...
{
    if (writer->pretty) { g_string_append_printf(writer->contents, "\n%*s", writer->container_is_array->len * 4, ""); }
}
...
static void quoted_str(JSONWriter* writer, const char* str)
{
    ...
    for (ptr = str; *ptr; ptr = end) {
        ...
        switch (cp) {
            case '\"': g_string_append(writer->contents, "\\\""); break;
            case '\\': g_string_append(writer->contents, "\\\\"); break;
            case '\b': g_string_append(writer->contents, "\\b"); break;
            case '\f': g_string_append(writer->contents, "\\f"); break;
            case '\n': g_string_append(writer->contents, "\\n"); break;
            case '\r': g_string_append(writer->contents, "\\r"); break;
            case '\t': g_string_append(writer->contents, "\\t"); break;
            default:
                if (cp < 0) { cp = 0xFFFD; /* replacement character */ }
                if (cp > 0xFFFF) {
...
static void maybe_comma_name(JSONWriter* writer, const char* name)
{
    if (writer->need_comma) {
        ...
        if (writer->contents->len) { pretty_newline(writer); }
        writer->need_comma = true;
    }
