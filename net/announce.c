...
int64_t qemu_announce_timer_step(AnnounceTimer* timer)
{
    ...
    step = timer->params.initial + (timer->params.rounds - timer->round - 1) * timer->params.step;

    if (step < 0 || step > timer->params.max) { step = timer->params.max; }
...
static const char* qemu_ether_ntoa(const MACAddr* mac)
{
    ...
    snprintf(ret, sizeof(ret), "%02x:%02x:%02x:%02x:%02x:%02x", mac->a[0], mac->a[1], mac->a[2], mac->a[3], mac->a[4],
             mac->a[5]);
...
static void qemu_announce_self_iter(NICState* nic, void* opaque)
{
    ...
    trace_qemu_announce_self_iter(timer->params.id ?: "_", nic->ncs->name, qemu_ether_ntoa(&nic->conf->macaddr), skip);
    ...
    if (!skip) {
        ...
        if (nic->ncs->info->announce) { nic->ncs->info->announce(nic->ncs); }
    }
}
static void qemu_announce_self_once(void* opaque)
{
    ...
    if (--timer->round) { qemu_announce_timer_step(timer); }
    else {
...
void qemu_announce_self(AnnounceTimer* timer, AnnounceParameters* params)
{
    ...
    if (params->rounds) { qemu_announce_self_once(timer); }
    else {
        qemu_announce_timer_del(timer, true);
    }
}
...
void qmp_announce_self(AnnounceParameters* params, Error** errp)
{
    ...
    if (!params->id) { params->id = g_strdup(""); }
