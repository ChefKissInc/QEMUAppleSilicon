...
static ssize_t net_hub_receive(NetHub* hub, NetHubPort* source_port, const uint8_t* buf, size_t len)
{
    NetHubPort* port;
...
static ssize_t net_hub_receive_iov(NetHub* hub, NetHubPort* source_port, const struct iovec* iov, int iovcnt)
{
    NetHubPort* port;
...
static bool net_hub_port_can_receive(NetClientState* nc)
{
    ...
    QLIST_FOREACH (port, &hub->ports, next) {
        ...
        if (qemu_can_send_packet(&port->nc)) { return true; }
    }

    return false;
}

static ssize_t net_hub_port_receive(NetClientState* nc, const uint8_t* buf, size_t len)
{
...
static ssize_t net_hub_port_receive_iov(NetClientState* nc, const struct iovec* iov, int iovcnt)
{
    NetHubPort* port = DO_UPCAST(NetHubPort, nc, nc);
...
static NetHubPort* net_hub_port_new(NetHub* hub, const char* name, NetClientState* hubpeer)
{
    NetClientState* nc;
    NetHubPort*     port;
    ...
    if (!name) {
        snprintf(default_name, sizeof(default_name), "hub%dport%d", hub->id, id);
        name = default_name;
    }
 ...
 */
NetClientState* net_hub_add_port(int hub_id, const char* name, NetClientState* hubpeer)
{
    NetHub*     hub;
    NetHubPort* port;

    QLIST_FOREACH (hub, &hubs, next) {
        if (hub->id == hub_id) { break; }
    }

    if (!hub) { hub = net_hub_new(hub_id); }
...
int net_hub_id_for_client(NetClientState* nc, int* id)
{
    ...
    if (nc->info->type == NET_CLIENT_DRIVER_HUBPORT) { port = DO_UPCAST(NetHubPort, nc, nc); }
    else if (nc->peer != NULL && nc->peer->info->type == NET_CLIENT_DRIVER_HUBPORT) {
    ...
    QLIST_FOREACH (hub, &hubs, next) {
        ...
        QLIST_FOREACH (port, &hub->ports, next) {
            ...
            switch (peer->info->type) {
                case NET_CLIENT_DRIVER_NIC   : has_nic = 1; break;
                case NET_CLIENT_DRIVER_USER  :
                case NET_CLIENT_DRIVER_TAP   :
                case NET_CLIENT_DRIVER_SOCKET:
                case NET_CLIENT_DRIVER_STREAM:
                case NET_CLIENT_DRIVER_DGRAM :
                case NET_CLIENT_DRIVER_VDE   : has_host_dev = 1; break;
                default                      : break;
            }
        }
        if (has_host_dev && !has_nic) { warn_report("hub %d with no nics", hub->id); }
        if (has_nic && !has_host_dev) { warn_report("hub %d is not connected to host network", hub->id); }
    }
}
...
bool net_hub_flush(NetClientState* nc)
{
    ...
    QLIST_FOREACH (port, &source_port->hub->ports, next) {
        if (port != source_port) { ret += qemu_net_queue_flush(port->nc.incoming_queue); }
    }
