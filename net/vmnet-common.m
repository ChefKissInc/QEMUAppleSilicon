...
{
    switch (status) {
        case VMNET_SUCCESS             : return "success";
        ...
        case VMNET_INVALID_ACCESS      : return "invalid access, permission denied";
        case VMNET_PACKET_TOO_BIG      : return "packet size is larger than MTU";
        ...
        case VMNET_SHARING_SERVICE_BUSY: return "conflict, sharing service is in use";
        default                        : return "unknown vmnet error";
    }
}
 ...
 */
ssize_t vmnet_receive_common(NetClientState* nc, const uint8_t* buf, size_t size)
{
    ...
    if (size > s->max_packet_size) {
        warn_report("vmnet: packet is too big, %zu > %" PRIu64, packet.vm_pkt_size, s->max_packet_size);
        return -1;
    }
    ...
    if (if_status != VMNET_SUCCESS) {
        error_report("vmnet: write error: %s", vmnet_status_map_str(if_status));
        return -1;
    }

    if (pkt_cnt) { return size; }
    return 0;
}
...
static void vmnet_write_packets_to_qemu(VmnetState* s)
{
    ...
    while (s->packets_send_current_pos < s->packets_send_end_pos) {
        ...
        size = qemu_send_packet_async(&s->nc, pkt, pktsz, vmnet_send_completed);
...
static void vmnet_send_bh(void* opaque)
{
     ...
     */
    if (s->packets_send_current_pos < s->packets_send_end_pos) { return; }
...
static void vmnet_send_completed(NetClientState* nc, ssize_t len)
{
    ...
    if (s->packets_send_current_pos < s->packets_send_end_pos) { qemu_bh_schedule(s->send_bh); }
}
...
static void vmnet_vm_state_change_cb(void* opaque, bool running, RunState state)
{
    ...
    if (running) {
        vmnet_interface_set_event_callback(s->vmnet_if, VMNET_INTERFACE_PACKETS_AVAILABLE, s->if_queue,
        ...
        vmnet_interface_set_event_callback(s->vmnet_if, VMNET_INTERFACE_PACKETS_AVAILABLE, NULL, NULL);
    }
}

int vmnet_if_create(NetClientState* nc, xpc_object_t if_desc, Error** errp)
{
    VmnetState*            s              = DO_UPCAST(VmnetState, nc, nc);
    dispatch_semaphore_t   if_created_sem = dispatch_semaphore_create(0);
    __block vmnet_return_t if_status;

    s->if_queue = dispatch_queue_create("org.qemu.vmnet.if_queue", DISPATCH_QUEUE_SERIAL);

    xpc_dictionary_set_bool(if_desc, vmnet_allocate_mac_address_key, false);

#ifdef DEBUG
    qemu_log("vmnet.start.interface_desc:\n");
    xpc_dictionary_apply(if_desc, ^bool(const char* k, xpc_object_t v) {
    ...
    s->vmnet_if = vmnet_start_interface(if_desc, s->if_queue, ^(vmnet_return_t status, xpc_object_t interface_param) {
      if_status = status;
      ...
      xpc_dictionary_apply(interface_param, ^bool(const char* k, xpc_object_t v) {
        char* desc = xpc_copy_description(v);
        qemu_log("  %s=%s\n", k, desc);
        free(desc);
        return true;
      ...
      s->mtu             = xpc_dictionary_get_uint64(interface_param, vmnet_mtu_key);
      s->max_packet_size = xpc_dictionary_get_uint64(interface_param, vmnet_max_packet_size_key);
    ...
    if (if_status != VMNET_SUCCESS) {
        dispatch_release(s->if_queue);
        error_setg(errp, "cannot create vmnet interface: %s", vmnet_status_map_str(if_status));
        return -1;
    }
...
void vmnet_cleanup_common(NetClientState* nc)
{
    ...
    if (s->vmnet_if == NULL) { return; }

    vmnet_vm_state_change_cb(s, 0, RUN_STATE_SHUTDOWN);
    qemu_del_vm_change_state_handler(s->change);
    if_stopped_sem = dispatch_semaphore_create(0);
    vmnet_stop_interface(s->vmnet_if, s->if_queue, ^(vmnet_return_t status) {
    ...
    for (int i = 0; i < VMNET_PACKETS_LIMIT; ++i) { g_free(s->iov_buf[i].iov_base); }
}
